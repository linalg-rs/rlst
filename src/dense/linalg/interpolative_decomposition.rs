...
use crate::dense::traits::{RawAccessMut, Shape, Stride, UnsafeRandomAccessByValue, UnsafeRandomAccessMut, UnsafeRandomAccessByRef, MultIntoResize, DefaultIterator};
...
pub trait MatrixId: RlstScalar {
    ...
    >(
       arr: Array<Self, ArrayImpl, 2>, tol: <Self as RlstScalar>::Real, k: Option<usize>
    ...
    > Array<Item, ArrayImplId, 2>
{
    ...
    pub fn into_id_alloc(self, tol: <Item as RlstScalar>::Real, k: Option<usize>) -> RlstResult<IdDecomposition<Item, ArrayImplId>> {
...
pub trait MatrixIdDecomposition: Sized {
    ...
    fn new(arr: Array<Self::Item, Self::ArrayImpl, 2>, tol: <Self::Item as RlstScalar>::Real, k: Option<usize>) -> RlstResult<Self>;
...
macro_rules! impl_id {
    ($scalar:ty) => {
            ...
            > MatrixIdDecomposition for IdDecomposition<$scalar, ArrayImpl>
        {
            ...
            fn new(mut arr: Array<$scalar, ArrayImpl, 2>, tol: <$scalar as RlstScalar>::Real, k: Option<usize>) -> RlstResult<Self>{
                //We assume that for a matrix of m rows and n columns, n>m, so we apply ID the transpose
                let mut arr_trans: Array<$scalar, BaseArray<$scalar, VectorContainer<$scalar>, 2>, 2> = rlst_dynamic_array2!($scalar, [arr.shape()[1], arr.shape()[0]]);
                arr_trans.fill_from(arr.view().conj().transpose());

                //We compute the QR decomposition using rlst QR decomposition
                let mut arr_qr: Array<$scalar, BaseArray<$scalar, VectorContainer<$scalar>, 2>, 2> = rlst_dynamic_array2!($scalar, [arr.shape()[1], arr.shape()[0]]);
                ...
                Self::get_p(perm_mat.view_mut(), perm);
                ...
                for col in 0..arr.shape()[1]{
                    for row in 0..arr.shape()[0]{
                        arr.data_mut()[col*arr_qr_shape[1] + row] = *perm_arr.get_mut([row, col]).unwrap()
                    }
                }
                ...
                if rank == 0 || rank >= dim{
                    ...
                    id_mat.set_identity();
                    Ok(Self{arr, perm_mat, rank, id_mat})
                }
                else{

                    let shape: [usize; 2] = r_mat.shape();
                    let mut id_mat: Array<$scalar, BaseArray<$scalar, VectorContainer<$scalar>, 2>, 2>= rlst_dynamic_array2!($scalar, [dim-rank, rank]);
                    
                    let mut k11: Array<$scalar, BaseArray<$scalar, VectorContainer<$scalar>, 2>, 2> = rlst_dynamic_array2!($scalar, [rank, rank]);
                    k11.fill_from(r_mat.view_mut().into_subview([0, 0], [rank, rank]));
                    k11.view_mut().into_inverse_alloc().unwrap();
                    let mut k12: Array<$scalar, BaseArray<$scalar, VectorContainer<$scalar>, 2>, 2> = rlst_dynamic_array2!($scalar, [rank, dim-rank]);
