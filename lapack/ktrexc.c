...
/*  ===================================================================== */
void  ktrexc_(char *compq, integer *n, halfcomplex *t, 
	integer *ldt, halfcomplex *q, integer *ldq, integer *ifst, integer *
	ilst, integer *info)
{
...
/*        Determine the transformation to perform the interchange. */
	...
	klartg_(&t[k + (k + 1) * t_dim1], &z__1, &cs, &sn, &temp);

/*        Apply transformation to the matrix T. */

	if (k + 2 <= *n) {
	    i__3 = *n - k - 1;
	    krot_(&i__3, &t[k + (k + 2) * t_dim1], ldt, &t[k + 1 + (k + 2) * 
		    t_dim1], ldt, &cs, &sn);
	}
	i__3 = k - 1;
	d_cnjg(&z__1, &sn);
	krot_(&i__3, &t[k * t_dim1 + 1], &c__1, &t[(k + 1) * t_dim1 + 1], &
		c__1, &cs, &z__1);
...
/*           Accumulate transformation in the matrix Q. */
	    ...
	    d_cnjg(&z__1, &sn);
	    krot_(n, &q[k * q_dim1 + 1], &c__1, &q[(k + 1) * q_dim1 + 1], &
		    c__1, &cs, &z__1);
	}
