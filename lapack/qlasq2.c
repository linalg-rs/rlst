...
/* > */
/*  ===================================================================== */
void  qlasq2_(integer *n, quadreal *z__, integer *info)
{
    ...
    quadreal dn;
    integer pp;
    quadreal dn1, dn2, dee, eps, tau, tol;
    integer ipn4;
    quadreal tol2;
    logical ieee;
    integer nbig;
    quadreal dmin__, emin, emax;
    integer kmin, ndiv, iter;
    quadreal qmin, temp, qmax, zmax;
    integer splt;
    quadreal dmin1, dmin2;
    integer nfail;
    quadreal desig, trace, sigma;
    integer iinfo;
    quadreal tempe, tempq;
    integer ttype;
    extern void  qlasq3_(integer *, integer *, quadreal *, 
...
/*        2-by-2 case. */
	...
	if (z__[2] > z__[3] * tol2) {
	    ...
	    s = z__[3] * (z__[2] / t);
	    if (s <= t) {
		s = z__[3] * (z__[2] / (t * (M(sqrt)(s / t + 1.) + 1.)));
	    } else {
		s = z__[3] * (z__[2] / (t + M(sqrt)(t) * M(sqrt)(t + s)));
	    }
	    t = z__[1] + (s + z__[2]);
	    z__[3] *= z__[1] / t;
	    z__[1] = t;
...
/* Computing MAX */
	d__1 = 0., d__2 = qmin - M(sqrt)(qmin) * 2. * M(sqrt)(emax);
...
/*           While submatrix unfinished take a good dqds step. */
	    ...
	    qlasq3_(&i0, &n0, &z__[1], &pp, &dmin__, &sigma, &desig, &qmax, &
...
/*     Store trace, sum(eigenvalues) and information on performance. */
    ...
    z__[(*n << 1) + 1] = trace;
    z__[(*n << 1) + 2] = e;
    z__[(*n << 1) + 3] = (quadreal) iter;
/* Computing 2nd power */
    i__1 = *n;
    z__[(*n << 1) + 4] = (quadreal) ndiv / (quadreal) (i__1 * i__1);
    z__[(*n << 1) + 5] = nfail * 100. / (quadreal) iter;
    return;
