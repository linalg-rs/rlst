...
/*  ===================================================================== */
void  kpptri_(char *uplo, integer *n, halfcomplex *ap, 
	integer *info)
{
    ...
    extern void  ktpmv_(char *, char *, char *, integer *, 
	    halfcomplex *, halfcomplex *, integer *), xerbla_(char *, integer *), whscal_(integer *, 
	    halfreal *, halfcomplex *, integer *), ktptri_(char *, char *,
...
/*     Invert the triangular Cholesky factor U or L. */
    ...
    ktptri_(uplo, "Non-unit", n, &ap[1], info);
    if (*info > 0) {
	return;
    }
    if (upper) {
...
/*        Compute the product inv(U) * inv(U)**H. */
	...
	for (j = 1; j <= i__1; ++j) {
	    ...
	    if (j > 1) {
		i__2 = j - 1;
		khpr_("Upper", &i__2, &c_b8, &ap[jc], &c__1, &ap[1]);
	    }
	    i__2 = jj;
	    ajj = ap[i__2].r;
	    whscal_(&j, &ajj, &ap[jc], &c__1);
/* L10: */
	}
...
/*        Compute the product inv(L)**H * inv(L). */
	...
	for (j = 1; j <= i__1; ++j) {
	    ...
	    i__2 = jj;
	    i__3 = *n - j + 1;
	    whotc_(&z__1, &i__3, &ap[jj], &c__1, &ap[jj], &c__1);
	    d__1 = z__1.r;
	    ap[i__2].r = d__1, ap[i__2].i = 0.;
	    if (j < *n) {
		i__2 = *n - j;
		ktpmv_("Lower", "Conjugate transpose", "Non-unit", &i__2, &ap[
			jjn], &ap[jj + 1], &c__1);
	    }
	    jj = jjn;
/* L20: */
