...
/*  ===================================================================== */
void  wpbcon_(char *uplo, integer *n, integer *kd, 
	quadcomplex *ab, integer *ldab, quadreal *anorm, quadreal *
	rcond, quadcomplex *work, quadreal *rwork, integer *info)
{
...
/*           Multiply by inv(U**H). */
	    ...
	    wlatbs_("Upper", "Conjugate transpose", "Non-unit", normin, n, kd,
		     &ab[ab_offset], ldab, &work[1], &scalel, &rwork[1], info);
	    *(unsigned char *)normin = 'Y';

/*           Multiply by inv(U). */

	    wlatbs_("Upper", "No transpose", "Non-unit", normin, n, kd, &ab[
		    ab_offset], ldab, &work[1], &scaleu, &rwork[1], info);
	} else {

/*           Multiply by inv(L). */

	    wlatbs_("Lower", "No transpose", "Non-unit", normin, n, kd, &ab[
		    ab_offset], ldab, &work[1], &scalel, &rwork[1], info);
	    *(unsigned char *)normin = 'Y';

/*           Multiply by inv(L**H). */

	    wlatbs_("Lower", "Conjugate transpose", "Non-unit", normin, n, kd,
