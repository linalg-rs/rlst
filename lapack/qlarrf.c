...
/*  ===================================================================== */
void  qlarrf_(integer *n, quadreal *d__, quadreal *l, 
	quadreal *ld, integer *clstrt, integer *clend, quadreal *w, 
	quadreal *wgap, quadreal *werr, quadreal *spdiam, quadreal *
	clgapl, quadreal *clgapr, quadreal *pivmin, quadreal *sigma, 
	quadreal *dplus, quadreal *lplus, quadreal *work, integer *info)
{
    ...
    /* Local variables */
    integer i__;
    quadreal s, bestshift, smlgrowth, eps, tmp, max1, max2, rrr1, rrr2, 
	    znm2, growthbound, fail, fact, oldp;
    integer indx;
    quadreal prod;
    integer ktry;
    quadreal fail2, avgap, ldmax, rdmax;
    integer shift;
    ...
    quadreal ldelta;
    logical nofail;
    quadreal mingap, lsigma, rdelta;
    extern logical qisnan_(quadreal *);
    logical forcer;
    quadreal rsigma, clwdth;
...
/*     Compute the average gap length of the cluster */
    clwdth = (d__1 = w[*clend] - w[*clstrt], abs(d__1)) + werr[*clend] + werr[
	    *clstrt];
    avgap = clwdth / (quadreal) (*clend - *clstrt);
...
/*     Use a small fudge to make sure that we doublereally shift to the outside */
...
/*     Initialize the record of the best representation found */
    ...
    s = qlamch_("S");
    smlgrowth = 1. / s;
    fail = (quadreal) (*n - 1) * mingap / (*spdiam * eps);
    fail2 = (quadreal) (*n - 1) * mingap / (*spdiam * M(sqrt)(eps));
    bestshift = lsigma;
...
/*        store new L and D back into DPLUS, LPLUS */
	qcopy_(n, &work[1], &c__1, &dplus[1], &c__1);
	i__1 = *n - 1;
	qcopy_(&i__1, &work[*n + 1], &c__1, &lplus[1], &c__1);
    }
    return;
