...
/* > */
/*  ===================================================================== */
void  wtrsen_(char *job, char *compq, logical *select, integer 
	*n, quadcomplex *t, integer *ldt, quadcomplex *q, integer *ldq, 
	quadcomplex *w, integer *m, quadreal *s, quadreal *sep, 
	quadcomplex *work, integer *lwork, integer *info)
{
    ...
    extern void  wtrexc_(char *, integer *, quadcomplex *, 
	    ...
	    integer *);
    logical lquery;
    extern void  wtrsyl_(char *, char *, integer *, integer *, 
...
/*     Quick return if possible */
	...
	if (wantsp) {
	    *sep = wlange_("1", n, n, &t[t_offset], ldt, rwork);
	}
	goto L40;
    }
...
/*           Swap the K-th eigenvalue to position KS. */
	    ...
	    if (k != ks) {
		wtrexc_(compq, n, &t[t_offset], ldt, &q[q_offset], ldq, &k, &
			ks, &ierr);
	    }
	}
/* L20: */
...
/*           T11*R - R*T22 = scale*T12 */
	...
	wlacpy_("F", &n1, &n2, &t[(n1 + 1) * t_dim1 + 1], ldt, &work[1], &n1);
	wtrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 
		+ 1) * t_dim1], ldt, &work[1], &n1, &scale, &ierr);

/*        Estimate the reciprocal of the condition number of the cluster */
/*        of eigenvalues. */

	rnorm = wlange_("F", &n1, &n2, &work[1], &n1, rwork);
	if (rnorm == 0.) {
	    *s = 1.;
	} else {
...
/*        Estimate sep(T11,T22). */
	...
	kase = 0;
L30:
	wlacn2_(&nn, &work[nn + 1], &work[1], &est, &kase, isave);
	if (kase != 0) {
	    if (kase == 1) {

/*              Solve T11*R - R*T22 = scale*X. */

		wtrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 
...
/*              Solve T11**H*R - R*T22**H = scale*X. */
		...
		wtrsyl_("C", "C", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 
			1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, &
			ierr);
	    }
	    goto L30;
