...
/*  ===================================================================== */
void  qgelsd_(integer *m, integer *n, integer *nrhs, 
	quadreal *a, integer *lda, quadreal *b, integer *ldb, quadreal *
	s, quadreal *rcond, integer *rank, quadreal *work, integer *lwork,
	 integer *iwork, integer *info)
{
    ...
    extern void  qlabad_(quadreal *, quadreal *), qgebrd_(
    ...
    extern void  qgelqf_(integer *, integer *, quadreal *, 
	    ...
	    qlalsd_(char *, integer *, integer *, integer *, quadreal *, 
	    quadreal *, quadreal *, integer *, quadreal *, integer *, 
	    quadreal *, integer *, integer *), qlascl_(char *, 
	    integer *, integer *, quadreal *, quadreal *, integer *, 
	    integer *, quadreal *, integer *, integer *), qgeqrf_(
	    integer *, integer *, quadreal *, integer *, quadreal *, 
	    quadreal *, integer *, integer *), qlacpy_(char *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, integer *), qlaset_(char *, integer *, integer *, quadreal *, 
...
/* Computing MAX */
    i__1 = (integer) (M(log)((quadreal) minmn / (quadreal) (smlsiz + 1)) / 
	    M(log)(2.)) + 1;
    nlvl = f2cmax(i__1,0);
...
/*        Matrix all zero. Return zero solution. */
	...
	i__1 = f2cmax(*m,*n);
	qlaset_("F", &i__1, nrhs, &c_b82, &c_b82, &b[b_offset], ldb);
	qlaset_("F", &minmn, &c__1, &c_b82, &c_b82, &s[1], &c__1);
	*rank = 0;
	goto L10;
    }
...
/*     Scale B if f2cmax entry outside range [SMLNUM,BIGNUM]. */
    ...
    bnrm = qlange_("M", m, nrhs, &b[b_offset], ldb, &work[1]);
    ibscl = 0;
    if (bnrm > 0. && bnrm < smlnum) {

/*        Scale matrix norm up to SMLNUM. */

	qlascl_("G", &c__0, &c__0, &bnrm, &smlnum, m, nrhs, &b[b_offset], ldb,
		 info);
	ibscl = 1;
    } else if (bnrm > bignum) {
...
/*        Scale matrix norm down to BIGNUM. */
	...
	qlascl_("G", &c__0, &c__0, &bnrm, &bignum, m, nrhs, &b[b_offset], ldb,
		 info);
	ibscl = 2;
    }
...
/*     If M < N make sure certain entries of B are zero. */
    ...
    if (*m < *n) {
	i__1 = *n - *m;
	qlaset_("F", &i__1, nrhs, &c_b82, &c_b82, &b[*m + 1 + b_dim1], ldb);
    }
...
/*           (Workspace: need 2*N, prefer N+N*NB) */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[nwork], &i__1,
		     info);
...
/*           (Workspace: need N+NRHS, prefer N+NRHS*NB) */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qormqr_("L", "T", m, nrhs, n, &a[a_offset], lda, &work[itau], &b[
...
/*           Zero out below R. */
		...
		i__1 = *n - 1;
		i__2 = *n - 1;
		qlaset_("L", &i__1, &i__2, &c_b82, &c_b82, &a[a_dim1 + 2], 
			lda);
	    }
	}
...
/*        (Workspace: need 3*N+MM, prefer 3*N+(MM+N)*NB) */
	...
	i__1 = *lwork - nwork + 1;
	qgebrd_(&mm, n, &a[a_offset], lda, &s[1], &work[ie], &work[itauq], &
...
/*        (Workspace: need 3*N+NRHS, prefer 3*N+NRHS*NB) */
	...
	i__1 = *lwork - nwork + 1;
	qormbr_("Q", "L", "T", &mm, nrhs, n, &a[a_offset], lda, &work[itauq], 
		&b[b_offset], ldb, &work[nwork], &i__1, info);

/*        Solve the bidiagonal least squares problem. */

	qlalsd_("U", &smlsiz, n, nrhs, &s[1], &work[ie], &b[b_offset], ldb, 
...
/*        Multiply B by right bidiagonalizing vectors of R. */
	...
	i__1 = *lwork - nwork + 1;
	qormbr_("P", "L", "N", n, nrhs, n, &a[a_offset], lda, &work[itaup], &
		b[b_offset], ldb, &work[nwork], &i__1, info);
...
/*        (Workspace: need 2*M, prefer M+M*NB) */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[nwork], &i__1,
		     info);
	    il = nwork;

/*        Copy L to WORK(IL), zeroing out above its diagonal. */

	    qlacpy_("L", m, m, &a[a_offset], lda, &work[il], &ldwork);
	    i__1 = *m - 1;
	    i__2 = *m - 1;
	    qlaset_("U", &i__1, &i__2, &c_b82, &c_b82, &work[il + ldwork], &
		    ldwork);
	    ie = il + ldwork * *m;
	    itauq = ie + *m;
	    itaup = itauq + *m;
...
/*        Solve the bidiagonal least squares problem. */
	    ...
	    qlalsd_("U", &smlsiz, m, nrhs, &s[1], &work[ie], &b[b_offset], 
...
/*        Multiply B by right bidiagonalizing vectors of L. */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qormbr_("P", "L", "N", m, nrhs, m, &work[il], &ldwork, &work[
		    itaup], &b[b_offset], ldb, &work[nwork], &i__1, info);

/*        Zero out below first M rows of B. */

	    i__1 = *n - *m;
	    qlaset_("F", &i__1, nrhs, &c_b82, &c_b82, &b[*m + 1 + b_dim1], 
		    ldb);
	    nwork = itau + *m;
...
/*        (Workspace: need M+NRHS, prefer M+NRHS*NB) */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qormlq_("L", "T", n, nrhs, m, &a[a_offset], lda, &work[itau], &b[
		    b_offset], ldb, &work[nwork], &i__1, info);
...
/*        (Workspace: need 3*M+NRHS, prefer 3*M+NRHS*NB) */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qormbr_("Q", "L", "T", m, nrhs, n, &a[a_offset], lda, &work[itauq]
		    , &b[b_offset], ldb, &work[nwork], &i__1, info);

/*        Solve the bidiagonal least squares problem. */

	    qlalsd_("L", &smlsiz, m, nrhs, &s[1], &work[ie], &b[b_offset], 
		    ldb, rcond, rank, &work[nwork], &iwork[1], info);
	    if (*info != 0) {
		goto L10;
	    }
...
/*        Multiply B by right bidiagonalizing vectors of A. */
	    ...
	    i__1 = *lwork - nwork + 1;
	    qormbr_("P", "L", "N", n, nrhs, m, &a[a_offset], lda, &work[itaup]
		    , &b[b_offset], ldb, &work[nwork], &i__1, info);
...
/*     Undo scaling. */
    ...
    if (iascl == 1) {
	qlascl_("G", &c__0, &c__0, &anrm, &smlnum, n, nrhs, &b[b_offset], ldb,
		 info);
	qlascl_("G", &c__0, &c__0, &smlnum, &anrm, &minmn, &c__1, &s[1], &
		minmn, info);
    } else if (iascl == 2) {
	qlascl_("G", &c__0, &c__0, &anrm, &bignum, n, nrhs, &b[b_offset], ldb,
		 info);
	qlascl_("G", &c__0, &c__0, &bignum, &anrm, &minmn, &c__1, &s[1], &
		minmn, info);
    }
    if (ibscl == 1) {
	qlascl_("G", &c__0, &c__0, &smlnum, &bnrm, n, nrhs, &b[b_offset], ldb,
		 info);
    } else if (ibscl == 2) {
	qlascl_("G", &c__0, &c__0, &bignum, &bnrm, n, nrhs, &b[b_offset], ldb,
		 info);
    }
