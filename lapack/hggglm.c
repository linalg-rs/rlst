...
/*  ===================================================================== */
void  hggglm_(integer *n, integer *m, integer *p, halfreal *
	a, integer *lda, halfreal *b, integer *ldb, halfreal *d__, 
	halfreal *x, halfreal *y, halfreal *work, integer *lwork, 
	integer *info)
{
    ...
    extern void  hgemv_(char *, integer *, integer *, 
	    ...
	    halfreal *, integer *, halfreal *, integer *), hggqrf_(
    ...
    extern void  hormqr_(char *, char *, integer *, integer *, 
	    ...
	    hormrq_(char *, char *, integer *, integer *, integer *, 
...
/*     orthogonal. */
    ...
    i__1 = *lwork - *m - np;
    hggqrf_(n, m, p, &a[a_offset], lda, &work[1], &b[b_offset], ldb, &work[*m 
...
/*                                               ( d2 ) N-M */
    ...
    i__1 = f2cmax(1,*n);
    i__2 = *lwork - *m - np;
    hormqr_("Left", "Transpose", n, &c__1, m, &a[a_offset], lda, &work[1], &
...
/*     Solve T22*y2 = d2 for y2 */
	...
	i__1 = *n - *m;
	i__2 = *n - *m;
	htrtrs_("Upper", "No transpose", "Non unit", &i__1, &c__1, &b[*m + 1 
	...
	i__1 = *n - *m;
	hcopy_(&i__1, &d__[*m + 1], &c__1, &y[*m + *p - *n + 1], &c__1);
    }
...
/*     Update d1 = d1 - T12*y2 */
    ...
    i__1 = *n - *m;
    hgemv_("No transpose", m, &i__1, &c_b32, &b[(*m + *p - *n + 1) * b_dim1 + 
	    1], ldb, &y[*m + *p - *n + 1], &c__1, &c_b34, &d__[1], &c__1);

/*     Solve triangular system: R11*x = d1 */

    if (*m > 0) {
	htrtrs_("Upper", "No Transpose", "Non unit", m, &c__1, &a[a_offset], 
		lda, &d__[1], m, info);
...
/* Computing MAX */
    ...
    i__3 = f2cmax(1,*p);
    i__4 = *lwork - *m - np;
    hormrq_("Left", "Transpose", p, &c__1, &np, &b[f2cmax(i__1,i__2) + b_dim1], 
	    ldb, &work[*m + 1], &y[1], &i__3, &work[*m + np + 1], &i__4, info);
/* Computing MAX */
    i__1 = lopt, i__2 = (integer) work[*m + np + 1];
    work[1] = (halfreal) (*m + np + f2cmax(i__1,i__2));
