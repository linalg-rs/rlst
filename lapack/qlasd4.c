...
/* > */
/*  ===================================================================== */
void  qlasd4_(integer *n, integer *i__, quadreal *d__, 
	quadreal *z__, quadreal *delta, quadreal *rho, quadreal *
	sigma, quadreal *work, integer *info)
{
    ...
    quadreal dw, zz[3];
    integer ip1;
    quadreal sq2, eta, phi, eps, tau, psi;
    integer iim1, iip1;
    quadreal tau2, dphi, sglb, dpsi, sgub;
    integer iter;
    quadreal temp, prew, temp1, temp2, dtiim, delsq, dtiip;
    integer niter;
    quadreal dtisq;
    logical swtch;
    quadreal dtnsq;
    extern void  qlaed6_(integer *, logical *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, integer *)
	    , qlasd5_(integer *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *);
    quadreal delsq2, dtnsq1;
    logical swtch3;
    extern quadreal qlamch_(char *);
    logical orgati;
    quadreal erretm, dtipsq, rhoinv;
    logical geomavg;
...
/*        Presumably, I=1 upon entry */
	...
	*sigma = M(sqrt)(d__[1] * d__[1] + *rho * z__[1] * z__[1]);
	delta[1] = 1.;
	work[1] = 1.;
	return;
    }
    if (*n == 2) {
	qlasd5_(i__, &d__[1], &z__[1], &delta[1], rho, sigma, &work[1]);
	return;
    }
...
/*        RHO * ||Z||_2^2 / TWO */
	...
	temp1 = temp / (d__[*n] + M(sqrt)(d__[*n] * d__[*n] + temp));
	i__1 = *n;
	for (j = 1; j <= i__1; ++j) {
...
/*           SIGMA_n^2 - D( N )*D( N ) */
		...
		b = z__[*n] * z__[*n] * delsq;
		if (a < 0.) {
		    tau2 = b * 2. / (M(sqrt)(a * a + b * 4. * c__) - a);
		} else {
		    tau2 = (a + M(sqrt)(a * a + b * 4. * c__)) / (c__ * 2.);
		}
		tau = tau2 / (d__[*n] + M(sqrt)(d__[*n] * d__[*n] + tau2));
	    }
...
/*        Calculate the new step */
	...
	if (c__ == 0.) {
	    ...
	    eta = (a + M(sqrt)((d__1 = a * a - b * 4. * c__, abs(d__1)))) / (c__ 
		    * 2.);
	} else {
	    eta = b * 2. / (a - M(sqrt)((d__1 = a * a - b * 4. * c__, abs(d__1)))
		    );
	}
...
/*        will guarantee eta*w < 0. */
	...
	eta /= *sigma + M(sqrt)(eta + *sigma * *sigma);
	tau += eta;
	*sigma += eta;
...
/*           Calculate the new step */
	    ...
	    b = dtnsq1 * dtnsq * w;
	    if (a >= 0.) {
		eta = (a + M(sqrt)((d__1 = a * a - b * 4. * c__, abs(d__1)))) / (
			c__ * 2.);
	    } else {
		eta = b * 2. / (a - M(sqrt)((d__1 = a * a - b * 4. * c__, abs(
			d__1))));
	    }
...
/*        Calculate initial guess */
	...
	delsq2 = delsq / 2.;
	sq2 = M(sqrt)((d__[*i__] * d__[*i__] + d__[ip1] * d__[ip1]) / 2.);
...
/*           We choose d(i) as origin. */
	    ...
	    if (a > 0.) {
		tau2 = b * 2. / (a + M(sqrt)((d__1 = a * a - b * 4. * c__, abs(
			d__1))));
	    } else {
		tau2 = (a - M(sqrt)((d__1 = a * a - b * 4. * c__, abs(d__1)))) / 
			(c__ * 2.);
	    }
...
/*           SIGMA - D( I ). */
	    ...
	    tau = tau2 / (d__[*i__] + M(sqrt)(d__[*i__] * d__[*i__] + tau2));
	    temp = M(sqrt)(eps);
...
/*           SIGMA - D( IP1 ). */
	    ...
	    tau = tau2 / (d__[ip1] + M(sqrt)((d__1 = d__[ip1] * d__[ip1] + tau2, 
		    abs(d__1))));
	}
...
/*           Interpolation using THREE most relevant poles */
	    ...
	    dd[2] = dtiip;
	    qlaed6_(&niter, &orgati, &c__, dd, zz, &w, &eta, info);
...
/* Computing 2nd power */
		...
		if (c__ == 0.) {
		    ...
		    eta = b * 2. / (a + M(sqrt)((d__1 = a * a - b * 4. * c__, 
			    abs(d__1))));
		}
	    }
	}
...
/*        will guarantee eta*w < 0. */
	...
	eta /= *sigma + M(sqrt)(*sigma * *sigma + eta);
	temp = tau + eta;
	if (temp > sgub || temp < sglb) {
		...
		if (w < 0.) {
		    if (tau > 0.) {
			eta = M(sqrt)(sgub * tau) - tau;
		    }
		} else {
		    if (sglb > 0.) {
			eta = M(sqrt)(sglb * tau) - tau;
		    }
		}
	    }
	}
...
/* Computing 2nd power */
			...
			eta = (a - M(sqrt)((d__1 = a * a - b * 4. * c__, abs(
				d__1)))) / (c__ * 2.);
		    } else {
			eta = b * 2. / (a + M(sqrt)((d__1 = a * a - b * 4. * c__,
				 abs(d__1))));
