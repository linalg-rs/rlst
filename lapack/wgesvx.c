...
/*  ===================================================================== */
void  wgesvx_(char *fact, char *trans, integer *n, integer *
	nrhs, quadcomplex *a, integer *lda, quadcomplex *af, integer *
	ldaf, integer *ipiv, char *equed, quadreal *r__, quadreal *c__, 
	quadcomplex *b, integer *ldb, quadcomplex *x, integer *ldx, 
	quadreal *rcond, quadreal *ferr, quadreal *berr, quadcomplex *
	work, quadreal *rwork, integer *info)
{
    ...
    extern quadreal wlange_(char *, integer *, integer *, quadcomplex *, 
	    integer *, quadreal *);
    quadreal bignum;
    extern void  wlaqge_(integer *, integer *, quadcomplex *,
	     integer *, quadreal *, quadreal *, quadreal *, quadreal *
	    , quadreal *, char *), wgecon_(char *, integer *, 
	    quadcomplex *, integer *, quadreal *, quadreal *, 
	    quadcomplex *, quadreal *, integer *);
    integer infequ;
    logical colequ;
    quadreal rowcnd;
    extern void  wgeequ_(integer *, integer *, quadcomplex *,
	     integer *, quadreal *, quadreal *, quadreal *, quadreal *
	    , quadreal *, integer *);
    logical notran;
    extern void  wgerfs_(char *, integer *, integer *, 
	    ...
	    integer *), wgetrf_(integer *, integer *, quadcomplex *,
	     integer *, integer *, integer *), wlacpy_(char *, integer *, 
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *);
    extern quadreal wlantr_(char *, char *, char *, integer *, integer *, 
	    quadcomplex *, integer *, quadreal *);
    quadreal smlnum;
    extern void  wgetrs_(char *, integer *, integer *, 
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	wgeequ_(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &colcnd, &
		amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    wlaqge_(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &
		    colcnd, &amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed,
		     "B");
	    colequ = lsame_(equed, "C") || lsame_(equed,
...
/*        Compute the LU factorization of A. */
	...
	wlacpy_("Full", n, n, &a[a_offset], lda, &af[af_offset], ldaf);
	wgetrf_(n, n, &af[af_offset], ldaf, &ipiv[1], info);
...
/*           leading rank-deficient INFO columns of A. */
	    ...
	    rpvgrw = wlantr_("M", "U", "N", info, info, &af[af_offset], ldaf, 
		    &rwork[1]);
	    if (rpvgrw == 0.) {
		rpvgrw = 1.;
	    } else {
		rpvgrw = wlange_("M", n, info, &a[a_offset], lda, &rwork[1]) / rpvgrw;
	    }
	    rwork[1] = rpvgrw;
	    *rcond = 0.;
	    return;
...
/*     reciprocal pivot growth factor RPVGRW. */
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = wlange_(norm, n, n, &a[a_offset], lda, &rwork[1]);
    rpvgrw = wlantr_("M", "U", "N", n, n, &af[af_offset], ldaf, &rwork[1]);
    if (rpvgrw == 0.) {
	rpvgrw = 1.;
    } else {
	rpvgrw = wlange_("M", n, n, &a[a_offset], lda, &rwork[1]) /
		 rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    wgecon_(norm, n, &af[af_offset], ldaf, &anorm, rcond, &work[1], &rwork[1],
	     info);

/*     Compute the solution matrix X. */

    wlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    wgetrs_(trans, n, nrhs, &af[af_offset], ldaf, &ipiv[1], &x[x_offset], ldx,
	     info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    wgerfs_(trans, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
