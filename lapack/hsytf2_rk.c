...
/* > \brief \b DSYTF2_RK computes the factorization of a doublereal symmetric indefinite matrix using the bounded Bu
...
/* > */
/* > \verbatim */
/* > DSYTF2_RK computes the factorization of a doublereal symmetric matrix A */
...
/*  ===================================================================== */
void  dsytf2_rk__(char *uplo, integer *n, halfreal *a, 
	integer *lda, halfreal *e, integer *ipiv, integer *info)
{
    ...
    /* Local variables */
    integer i__, j, k, p;
    halfreal t, d11, d12, d21, d22;
    integer ii, kk, kp;
    halfreal wk, wkm1, wkp1;
    logical done;
    integer imax, jmax;
    extern void  hsyr_(char *, integer *, halfreal *, 
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    imax = ihamax_(&i__1, &a[k * a_dim1 + 1], &c__1);
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = k - imax;
		    jmax = imax + ihamax_(&i__1, &a[imax + (imax + 1) * 
			    a_dim1], lda);
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    itemp = ihamax_(&i__1, &a[imax * a_dim1 + 1], &c__1);
...
/*              submatrix A(1:k,1:k) if we have a 2-by-2 pivot */
		...
		if (p > 1) {
		    i__1 = p - 1;
		    hswap_(&i__1, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 
			    1], &c__1);
		}
		if (p < k - 1) {
		    i__1 = k - p - 1;
		    hswap_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 
			    1) * a_dim1], lda);
		}
		t = a[k + k * a_dim1];
...
/*              the interchanges in columns k+1:N. */
		...
		if (k < *n) {
		    i__1 = *n - k;
		    hswap_(&i__1, &a[k + (k + 1) * a_dim1], lda, &a[p + (k + 
			    1) * a_dim1], lda);
		}
...
/*              submatrix A(1:k,1:k) */
		...
		if (kp > 1) {
		    i__1 = kp - 1;
		    hswap_(&i__1, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 
			    + 1], &c__1);
		}
		if (kk > 1 && kp < kk - 1) {
		    i__1 = kk - kp - 1;
		    hswap_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (
			    kp + 1) * a_dim1], lda);
		}
		t = a[kk + kk * a_dim1];
...
/*                       = A - W(k)*1/D(k)*W(k)**T */
			...
			i__1 = k - 1;
			d__1 = -d11;
			hsyr_(uplo, &i__1, &d__1, &a[k * a_dim1 + 1], &c__1, &
				a[a_offset], lda);

/*                    Store U(k) in column k */

			i__1 = k - 1;
			hscal_(&i__1, &d11, &a[k * a_dim1 + 1], &c__1);
		    } else {
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + ihamax_(&i__1, &a[k + 1 + k * a_dim1], &c__1);
	    colmax = (d__1 = a[imax + k * a_dim1], abs(d__1));
	} else {
	    colmax = 0.;
	}
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = imax - k;
		    jmax = k - 1 + ihamax_(&i__1, &a[imax + k * a_dim1], lda);
		    rowmax = (d__1 = a[imax + jmax * a_dim1], abs(d__1));
		} else {
		    rowmax = 0.;
		}
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    itemp = imax + ihamax_(&i__1, &a[imax + 1 + imax * a_dim1]
			    , &c__1);
		    dtemp = (d__1 = a[itemp + imax * a_dim1], abs(d__1));
		    if (dtemp > rowmax) {
			rowmax = dtemp;
...
/*              submatrix A(k:n,k:n) if we have a 2-by-2 pivot */
		...
		if (p < *n) {
		    i__1 = *n - p;
		    hswap_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + 1 + p 
			    * a_dim1], &c__1);
		}
		if (p > k + 1) {
		    i__1 = p - k - 1;
		    hswap_(&i__1, &a[k + 1 + k * a_dim1], &c__1, &a[p + (k + 
			    1) * a_dim1], lda);
		}
		t = a[k + k * a_dim1];
		a[k + k * a_dim1] = a[p + p * a_dim1];
...
/*              the interchanges in columns 1:k-1. */
		...
		if (k > 1) {
		    i__1 = k - 1;
		    hswap_(&i__1, &a[k + a_dim1], lda, &a[p + a_dim1], lda);
		}
...
/*              submatrix A(k:n,k:n) */
		...
		if (kp < *n) {
		    i__1 = *n - kp;
		    hswap_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + 1 
			    + kp * a_dim1], &c__1);
		}
		if (kk < *n && kp > kk + 1) {
		    i__1 = kp - kk - 1;
		    hswap_(&i__1, &a[kk + 1 + kk * a_dim1], &c__1, &a[kp + (
			    kk + 1) * a_dim1], lda);
		}
		t = a[kk + kk * a_dim1];
		a[kk + kk * a_dim1] = a[kp + kp * a_dim1];
...
/*                       = A - W(k)*(1/D(k))*W(k)**T */
			...
			i__1 = *n - k;
			d__1 = -d11;
			hsyr_(uplo, &i__1, &d__1, &a[k + 1 + k * a_dim1], &
				c__1, &a[k + 1 + (k + 1) * a_dim1], lda);

/*                    Store L(k) in column k */

			i__1 = *n - k;
			hscal_(&i__1, &d11, &a[k + 1 + k * a_dim1], &c__1);
		    } else {
