...
/* > */
/*  ===================================================================== */
void  qlatrs_(char *uplo, char *trans, char *diag, char *
	normin, integer *n, quadreal *a, integer *lda, quadreal *x, 
	quadreal *scale, quadreal *cnorm, integer *info)
{
    ...
    logical upper;
    extern void  qtrsv_(char *, char *, char *, integer *, 
...
/*           A is upper triangular. */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = j - 1;
		cnorm[j] = qasum_(&i__2, &a[j * a_dim1 + 1], &c__1);
/* L10: */
	    }
	} else {
...
/*           A is lower triangular. */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j;
		cnorm[j] = qasum_(&i__2, &a[j + 1 + j * a_dim1], &c__1);
/* L20: */
	    }
	    cnorm[*n] = 0.;
	}
...
/*        elements of X is not too small. */
	...
	qtrsv_(uplo, trans, diag, n, &a[a_offset], lda, &x[1], &c__1);
    } else {
...
/*                       x(1:j-1) := x(1:j-1) - x(j) * A(1:j-1,j) */
			...
			i__3 = j - 1;
			d__1 = -x[j] * tscal;
			qaxpy_(&i__3, &d__1, &a[j * a_dim1 + 1], &c__1, &x[1],
				 &c__1);
			i__3 = j - 1;
...
/*                       x(j+1:n) := x(j+1:n) - x(j) * A(j+1:n,j) */
			...
			i__3 = *n - j;
			d__1 = -x[j] * tscal;
			qaxpy_(&i__3, &d__1, &a[j + 1 + j * a_dim1], &c__1, &
				x[j + 1], &c__1);
			i__3 = *n - j;
...
/*                 call DDOT to perform the dot product. */
		    ...
		    if (upper) {
			i__3 = j - 1;
			sumj = qdot_(&i__3, &a[j * a_dim1 + 1], &c__1, &x[1], 
				&c__1);
		    } else if (j < *n) {
			i__3 = *n - j;
			sumj = qdot_(&i__3, &a[j + 1 + j * a_dim1], &c__1, &x[
				j + 1], &c__1);
		    }
		} else {
