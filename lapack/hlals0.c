...
/*  ===================================================================== */
void  hlals0_(integer *icompq, integer *nl, integer *nr, 
	integer *sqre, integer *nrhs, halfreal *b, integer *ldb, halfreal 
	*bx, integer *ldbx, integer *perm, integer *givptr, integer *givcol, 
	integer *ldgcol, halfreal *givnum, integer *ldgnum, halfreal *
	poles, halfreal *difl, halfreal *difr, halfreal *z__, integer *
	k, halfreal *c__, halfreal *s, halfreal *work, integer *info)
{
    ...
    extern void  hlascl_(char *, integer *, integer *, 
	    ...
	    integer *, integer *), hlacpy_(char *, integer *, integer 
	    *, halfreal *, integer *, halfreal *, integer *), 
...
/*        Step (1L): apply back the Givens rotations performed. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    hrot_(nrhs, &b[givcol[i__ + (givcol_dim1 << 1)] + b_dim1], ldb, &
...
/*        Step (2L): permute rows of B. */
	...
	hcopy_(nrhs, &b[nlp1 + b_dim1], ldb, &bx[bx_dim1 + 1], ldbx);
	i__1 = n;
	for (i__ = 2; i__ <= i__1; ++i__) {
	    hcopy_(nrhs, &b[perm[i__] + b_dim1], ldb, &bx[i__ + bx_dim1], 
		    ldbx);
/* L20: */
	}
...
/*        matrix to BX. */
	...
	if (*k == 1) {
	    hcopy_(nrhs, &bx[bx_offset], ldbx, &b[b_offset], ldb);
	    if (z__[1] < 0.) {
		hscal_(nrhs, &c_b5, &b[b_offset], ldb);
	    }
	} else {
	    i__1 = *k;
	    for (j = 1; j <= i__1; ++j) {
...
/* L40: */
		...
		hgemv_("T", k, nrhs, &c_b11, &bx[bx_offset], ldbx, &work[1], &
			c__1, &c_b13, &b[j + b_dim1], ldb);
		hlascl_("G", &c__0, &c__0, &temp, &c_b11, &c__1, nrhs, &b[j + 
			b_dim1], ldb, info);
/* L50: */
	    }
	}
...
/*        Move the deflated rows of BX to B also. */
	...
	if (*k < f2cmax(m,n)) {
	    i__1 = n - *k;
	    hlacpy_("A", &i__1, nrhs, &bx[*k + 1 + bx_dim1], ldbx, &b[*k + 1 
		    + b_dim1], ldb);
	}
    } else {
...
/*        to B. */
	...
	if (*k == 1) {
	    hcopy_(nrhs, &b[b_offset], ldb, &bx[bx_offset], ldbx);
	} else {
	    i__1 = *k;
	    for (j = 1; j <= i__1; ++j) {
		dsigj = poles[j + (poles_dim1 << 1)];
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
			...
			work[i__] = z__[j] / (hlamc3_(&dsigj, &d__1) - difr[
...
/* L60: */
		...
		for (i__ = j + 1; i__ <= i__2; ++i__) {
			...
			work[i__] = z__[j] / (hlamc3_(&dsigj, &d__1) - difl[
				i__]) / (dsigj + poles[i__ + poles_dim1]) / 
				difr[i__ + (difr_dim1 << 1)];
		    }
/* L70: */
		}
		hgemv_("T", k, nrhs, &c_b11, &b[b_offset], ldb, &work[1], &
...
/*        related to the right null space of the subproblem. */
	...
	if (*sqre == 1) {
	    hcopy_(nrhs, &b[m + b_dim1], ldb, &bx[m + bx_dim1], ldbx);
	    hrot_(nrhs, &bx[bx_dim1 + 1], ldbx, &bx[m + bx_dim1], ldbx, c__, 
		    s);
	}
	if (*k < f2cmax(m,n)) {
	    i__1 = n - *k;
	    hlacpy_("A", &i__1, nrhs, &b[*k + 1 + b_dim1], ldb, &bx[*k + 1 + 
		    bx_dim1], ldbx);
	}

/*        Step (3R): permute rows of B. */

	hcopy_(nrhs, &bx[bx_dim1 + 1], ldbx, &b[nlp1 + b_dim1], ldb);
	if (*sqre == 1) {
	    hcopy_(nrhs, &bx[m + bx_dim1], ldbx, &b[m + b_dim1], ldb);
	}
	i__1 = n;
	for (i__ = 2; i__ <= i__1; ++i__) {
	    hcopy_(nrhs, &bx[i__ + bx_dim1], ldbx, &b[perm[i__] + b_dim1], 
		    ldb);
/* L90: */
	}
