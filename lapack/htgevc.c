...
/* > a pair of doublereal matrices (S,P), where S is a quasi-triangular matrix */
...
/* >          computed.  If w(j) is a doublereal eigenvalue, the corresponding */
/* >          doublereal eigenvector is computed if SELECT(j) is .TRUE.. */
...
/* >          is set to N.  Each selected doublereal eigenvector occupies one */
...
/* >     WORK( 2*N+1:3*N ) = doublereal part of eigenvector */
/* >     WORK( 3*N+1:4*N ) = imaginary part of eigenvector */
/* >     WORK( 4*N+1:5*N ) = doublereal part of back-transformed eigenvector */
...
/* >  are doublereal). The equation to be solved is: */
/* >       n                   i */
...
/* > */
/*  ===================================================================== */
void  htgevc_(char *side, char *howmny, logical *select, 
	integer *n, halfreal *s, integer *lds, halfreal *p, integer *ldp, 
	halfreal *vl, integer *ldvl, halfreal *vr, integer *ldvr, integer 
	*mm, integer *m, halfreal *work, integer *info)
{
    ...
    halfreal big;
    logical lsa, lsb;
    halfreal ulp, sum[4]	/* was [2][2] */;
    integer ibeg, ieig, iend;
    halfreal dmin__, temp, xmax, sump[4]	/* was [2][2] */, sums[4]	
	    /* was [2][2] */;
    extern void  hlag2_(halfreal *, integer *, halfreal *, 
	    integer *, halfreal *, halfreal *, halfreal *, halfreal *,
	     halfreal *, halfreal *);
    halfreal cim2a, cim2b, cre2a, cre2b, temp2, bdiag[2], acoef, scale;
    logical ilall;
    integer iside;
    halfreal sbeta;
    ...
    halfreal acoefa, bcoefa, cimaga, cimagb;
    logical ilback;
    halfreal bcoefi, ascale, bscale, creala, crealb;
    extern halfreal hlamch_(char *);
    halfreal bcoefr, salfar, safmin;
    extern void  hlacpy_(char *, integer *, integer *, 
	    halfreal *, integer *, halfreal *, integer *);
    halfreal xscale, bignum;
...
/*     Machine Constants */
    ...
    big = 1. / safmin;
    hlabad_(&safmin, &big);
    ulp = hlamch_("Epsilon") * hlamch_("Base");
    small = safmin * *n / ulp;
    big = 1. / small;
    bignum = 1. / (safmin * *n);
...
/*           Decide if (a) singular pencil, (b) doublereal eigenvalue, or */
...
/*              Complex eigenvalue */
		...
		d__1 = safmin * 100.;
		hlag2_(&s[je + je * s_dim1], lds, &p[je + je * p_dim1], ldp, &
...
/*              with scaling and perturbation of the denominator */
		...
		hlaln2_(&c_true, &na, &nw, &dmin__, &acoef, &s[j + j * s_dim1]
...
/*           HOWMNY='B'. */
		...
		for (jw = 0; jw <= i__2; ++jw) {
		    i__3 = *n + 1 - je;
		    hgemv_("N", n, &i__3, &c_b34, &vl[je * vl_dim1 + 1], ldvl,
			     &work[(jw + 2) * *n + je], &c__1, &c_b36, &work[(
			    jw + 4) * *n + 1], &c__1);
/* L170: */
		}
		hlacpy_(" ", n, &nw, &work[(*n << 2) + 1], n, &vl[je * 
			vl_dim1 + 1], ldvl);
		ibeg = 1;
	    } else {
		hlacpy_(" ", n, &nw, &work[(*n << 1) + 1], n, &vl[ieig * 
			vl_dim1 + 1], ldvl);
		ibeg = je;
	    }
...
/*              Complex eigenvalue */
		...
		d__1 = safmin * 100.;
		hlag2_(&s[je - 1 + (je - 1) * s_dim1], lds, &p[je - 1 + (je - 
...
/*              Compute x(j) (and x(j+1), if 2-by-2 block) */
		...
		hlaln2_(&c_false, &na, &nw, &dmin__, &acoef, &s[j + j * 
