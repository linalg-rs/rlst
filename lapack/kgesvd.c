...
/*  ===================================================================== */
void  kgesvd_(char *jobu, char *jobvt, integer *m, integer *n, 
	...
	integer *lwork, halfreal *rwork, integer *info)
{
    ...
    extern void  kgelqf_(integer *, integer *, halfcomplex *,
	    ...
	    ), klascl_(char *, integer *, integer *, halfreal *, halfreal 
	    *, integer *, integer *, halfcomplex *, integer *, integer *), kgeqrf_(integer *, integer *, halfcomplex *, integer *,
	     halfcomplex *, halfcomplex *, integer *, integer *), klacpy_(
	    char *, integer *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *), klaset_(char *, integer *, 
	    integer *, halfcomplex *, halfcomplex *, halfcomplex *, 
	    integer *);
    integer ldwrkr;
    extern void  kbdsqr_(char *, integer *, integer *, integer 
...
/*           Compute space needed for ZGEQRF */
	    kgeqrf_(m, n, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
	    lwork_zgeqrf__ = (integer) cdum[0].r;
/*           Compute space needed for ZUNGQR */
	    kungqr_(m, n, n, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
	    lwork_zungqr_n__ = (integer) cdum[0].r;
	    kungqr_(m, m, n, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
	    lwork_zungqr_m__ = (integer) cdum[0].r;
/*           Compute space needed for ZGEBRD */
	    kgebrd_(n, n, &a[a_offset], lda, &s[1], dum, cdum, cdum, cdum, &
		    c_n1, &ierr);
	    lwork_zgebrd__ = (integer) cdum[0].r;
/*           Compute space needed for ZUNGBR */
	    kungbr_("P", n, n, n, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
	    lwork_zungbr_p__ = (integer) cdum[0].r;
	    kungbr_("Q", n, n, n, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
...
/*              Path 10 (M at least N, but not much larger) */
		...
		kgebrd_(m, n, &a[a_offset], lda, &s[1], dum, cdum, cdum, cdum,
			 &c_n1, &ierr);
		lwork_zgebrd__ = (integer) cdum[0].r;
		maxwrk = (*n << 1) + lwork_zgebrd__;
		if (wntus || wntuo) {
		    kungbr_("Q", m, n, n, &a[a_offset], lda, cdum, cdum, &
			    c_n1, &ierr);
		    lwork_zungbr_q__ = (integer) cdum[0].r;
/* Computing MAX */
		...
		if (wntua) {
		    kungbr_("Q", m, m, n, &a[a_offset], lda, cdum, cdum, &
			    c_n1, &ierr);
		    lwork_zungbr_q__ = (integer) cdum[0].r;
/* Computing MAX */
		    i__2 = maxwrk, i__3 = (*n << 1) + lwork_zungbr_q__;
...
/*           Compute space needed for ZGELQF */
	    kgelqf_(m, n, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
...
/*           Compute space needed for ZUNGLQ */
	    ...
	    kunglq_(m, n, m, &a[a_offset], lda, cdum, cdum, &c_n1, &ierr);
	    lwork_zunglq_m__ = (integer) cdum[0].r;
/*           Compute space needed for ZGEBRD */
	    kgebrd_(m, m, &a[a_offset], lda, &s[1], dum, cdum, cdum, cdum, &
		    c_n1, &ierr);
	    lwork_zgebrd__ = (integer) cdum[0].r;
/*            Compute space needed for ZUNGBR P */
	    kungbr_("P", m, m, m, &a[a_offset], n, cdum, cdum, &c_n1, &ierr);
	    lwork_zungbr_p__ = (integer) cdum[0].r;
/*           Compute space needed for ZUNGBR Q */
	    kungbr_("Q", m, m, m, &a[a_offset], n, cdum, cdum, &c_n1, &ierr);
	    lwork_zungbr_q__ = (integer) cdum[0].r;
	    if (*n >= mnthr) {
		if (wntvn) {
...
/*              Path 10t(N greater than M, but not much larger) */
		...
		if (wntvs || wntvo) {
/*                Compute space needed for ZUNGBR P */
		    kungbr_("P", m, n, m, &a[a_offset], n, cdum, cdum, &c_n1, 
			    &ierr);
		    lwork_zungbr_p__ = (integer) cdum[0].r;
/* Computing MAX */
		...
		if (wntva) {
		    kungbr_("P", n, n, m, &a[a_offset], n, cdum, cdum, &c_n1, 
			    &ierr);
		    lwork_zungbr_p__ = (integer) cdum[0].r;
/* Computing MAX */
		    i__2 = maxwrk, i__3 = (*m << 1) + lwork_zungbr_p__;
...
/*              (RWorkspace: need 0) */
		...
		i__2 = *lwork - iwork + 1;
		kgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[iwork], &
			i__2, &ierr);
...
/*              Zero out below R */
		...
		if (*n > 1) {
		    i__2 = *n - 1;
		    i__3 = *n - 1;
		    klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &a[a_dim1 + 2], 
			    lda);
		}
		ie = 1;
		itauq = 1;
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kungbr_("P", n, n, n, &a[a_offset], lda, &work[itaup], &
...
/*              (RWorkspace: need BDSPAC) */
		...
		kbdsqr_("U", n, &ncvt, &c__0, &c__0, &s[1], &rwork[ie], &a[
...
/*              If right singular vectors desired in VT, copy them there */
		...
		if (wntvas) {
		    klacpy_("F", n, n, &a[a_offset], lda, &vt[vt_offset], 
			    ldvt);
		}
...
/*                 Copy R to WORK(IR) and zero out below it */
		    ...
		    i__2 = *n - 1;
		    i__3 = *n - 1;
		    klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &work[ir + 1], &
			    ldwrkr);
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kungqr_(m, n, n, &a[a_offset], lda, &work[itau], &work[
			    iwork], &i__2, &ierr);
		    ie = 1;
		    itauq = itau;
		    itaup = itauq + *n;
...
/*                 (RWorkspace: need N) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kgebrd_(n, n, &work[ir], &ldwrkr, &s[1], &rwork[ie], &
...
/*                 (RWorkspace: need 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kungbr_("Q", n, n, n, &work[ir], &ldwrkr, &work[itauq], &
			    work[iwork], &i__2, &ierr);
		    irwork = ie + *n;
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", n, &c__0, n, &c__0, &s[1], &rwork[ie], cdum, 
...
/*                 (RWorkspace: N) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    kgebrd_(m, n, &a[a_offset], lda, &s[1], &rwork[ie], &work[
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", n, &c__0, m, &c__0, &s[1], &rwork[ie], cdum, 
...
/*                 Copy R to VT, zeroing out below it */
		    ...
		    klacpy_("U", n, n, &a[a_offset], lda, &vt[vt_offset], 
			    ldvt);
		    if (*n > 1) {
			i__3 = *n - 1;
			i__2 = *n - 1;
			klaset_("L", &i__3, &i__2, &c_b1, &c_b1, &vt[vt_dim1 
				+ 2], ldvt);
		    }
...
/*                 (RWorkspace: need N) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    kgebrd_(n, n, &vt[vt_offset], ldvt, &s[1], &rwork[ie], &
			    work[itauq], &work[itaup], &work[iwork], &i__3, &
			    ierr);
		    klacpy_("L", n, n, &vt[vt_offset], ldvt, &work[ir], &
			    ldwrkr);
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", n, n, n, &c__0, &s[1], &rwork[ie], &vt[
...
/*                 Copy R to VT, zeroing out below it */
			...
			i__2 = *n - 1;
			i__3 = *n - 1;
			klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &vt[vt_dim1 
				+ 2], ldvt);
		    }
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kunmbr_("Q", "R", "N", m, n, n, &vt[vt_offset], ldvt, &
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", n, n, m, &c__0, &s[1], &rwork[ie], &vt[
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[
				iwork], &i__2, &ierr);

/*                    Copy R to WORK(IR), zeroing out below it */

			klacpy_("U", n, n, &a[a_offset], lda, &work[ir], &
				ldwrkr);
			i__2 = *n - 1;
			i__3 = *n - 1;
			klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &work[ir + 1]
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungqr_(m, n, n, &a[a_offset], lda, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = 1;
			itauq = itau;
			itaup = itauq + *n;
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", n, &c__0, n, &c__0, &s[1], &rwork[ie], 
...
/*                    (RWorkspace: 0) */
			...
			kgemm_("N", "N", m, n, n, &c_b2, &a[a_offset], lda, &
...
/*                    (RWorkspace: 0) */
			...
			kgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[
				iwork], &i__2, &ierr);
			klacpy_("L", m, n, &a[a_offset], lda, &u[u_offset], 
				ldu);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungqr_(m, n, n, &u[u_offset], ldu, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = 1;
			itauq = itau;
			itaup = itauq + *n;
...
/*                    Zero out below R in A */
			...
			if (*n > 1) {
			    i__2 = *n - 1;
			    i__3 = *n - 1;
			    klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &a[
				    a_dim1 + 2], lda);
			}
...
/*                    (RWorkspace: need N) */
			...
			i__2 = *lwork - iwork + 1;
			kgebrd_(n, n, &a[a_offset], lda, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kunmbr_("Q", "R", "N", m, n, n, &a[a_offset], lda, &
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", n, &c__0, m, &c__0, &s[1], &rwork[ie], 
...
/*                    Copy R to WORK(IU), zeroing out below it */
			...
			klacpy_("U", n, n, &a[a_offset], lda, &work[iu], &
				ldwrku);
			i__2 = *n - 1;
			i__3 = *n - 1;
			klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &work[iu + 1]
				, &ldwrku);
...
/*                    (RWorkspace: need   N) */
			...
			i__2 = *lwork - iwork + 1;
			kgebrd_(n, n, &work[iu], &ldwrku, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
			klacpy_("U", n, n, &work[iu], &ldwrku, &work[ir], &
				ldwrkr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungbr_("Q", n, n, n, &work[iu], &ldwrku, &work[itauq]
				, &work[iwork], &i__2, &ierr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungbr_("P", n, n, n, &work[ir], &ldwrkr, &work[itaup]
				, &work[iwork], &i__2, &ierr);
			irwork = ie + *n;
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", n, n, n, &c__0, &s[1], &rwork[ie], &work[
...
/*                    (RWorkspace: 0) */
			...
			klacpy_("F", n, n, &work[ir], &ldwrkr, &a[a_offset], 
				lda);
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", n, n, m, &c__0, &s[1], &rwork[ie], &a[
...
/*                    (RWorkspace: need N) */
			...
			kgebrd_(n, n, &work[iu], &ldwrku, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
			klacpy_("U", n, n, &work[iu], &ldwrku, &vt[vt_offset],
				 ldvt);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungbr_("P", n, n, n, &vt[vt_offset], ldvt, &work[
...
/*                    Copy R to VT, zeroing out below it */
			...
			if (*n > 1) {
			    i__2 = *n - 1;
			    i__3 = *n - 1;
			    klaset_("L", &i__2, &i__3, &c_b1, &c_b1, &vt[
				    vt_dim1 + 2], ldvt);
			}
			ie = 1;
			itauq = itau;
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungqr_(m, m, n, &u[u_offset], ldu, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = 1;
			itauq = itau;
			itaup = itauq + *n;
...
/*                    (RWorkspace: 0) */
			...
			kgemm_("N", "N", m, n, n, &c_b2, &u[u_offset], ldu, &
				work[ir], &ldwrkr, &c_b1, &a[a_offset], lda);

/*                    Copy left singular vectors of A from A to U */

			klacpy_("F", m, n, &a[a_offset], lda, &u[u_offset], 
				ldu);
...
/*              (RWorkspace: 0) */
		...
		i__2 = *lwork - iwork + 1;
		kungbr_("Q", m, &ncu, n, &u[u_offset], ldu, &work[itauq], &
			work[iwork], &i__2, &ierr);
	    }
	    if (wntvas) {
...
/*              (RWorkspace: 0) */
		...
		i__2 = *lwork - iwork + 1;
		kungbr_("P", n, n, n, &a[a_offset], lda, &work[itaup], &work[
			iwork], &i__2, &ierr);
	    }
	    irwork = ie + *n;
	    if (wntuas || wntuo) {
...
/*              (RWorkspace: need BDSPAC) */
		...
		kbdsqr_("U", n, &ncvt, &nru, &c__0, &s[1], &rwork[ie], &vt[
...
/*              (RWorkspace: need BDSPAC) */
		...
		kbdsqr_("U", n, &ncvt, &nru, &c__0, &s[1], &rwork[ie], &a[
...
/*              (RWorkspace: 0) */
		...
		i__2 = *lwork - iwork + 1;
		kgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[iwork], &
			i__2, &ierr);
...
/*              Zero out above L */
		...
		i__2 = *m - 1;
		i__3 = *m - 1;
		klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &a[(a_dim1 << 1) + 1]
			, lda);
		ie = 1;
		itauq = 1;
		itaup = itauq + *m;
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kungbr_("Q", m, m, m, &a[a_offset], lda, &work[itauq], &
			    work[iwork], &i__2, &ierr);
		}
		irwork = ie + *m;
		nru = 0;
...
/*              (RWorkspace: need BDSPAC) */
		...
		kbdsqr_("U", m, &c__0, &nru, &c__0, &s[1], &rwork[ie], cdum, &
...
/*                 Copy L to WORK(IR) and zero out above it */
		    ...
		    klacpy_("L", m, m, &a[a_offset], lda, &work[ir], &ldwrkr);
		    i__2 = *m - 1;
		    i__3 = *m - 1;
		    klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &work[ir + 
			    ldwrkr], &ldwrkr);
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kunglq_(m, n, m, &a[a_offset], lda, &work[itau], &work[
			    iwork], &i__2, &ierr);
		    ie = 1;
		    itauq = itau;
		    itaup = itauq + *m;
...
/*                 (RWorkspace: need M) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kgebrd_(m, m, &work[ir], &ldwrkr, &s[1], &rwork[ie], &
			    work[itauq], &work[itaup], &work[iwork], &i__2, &
			    ierr);
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kungbr_("P", m, m, m, &work[ir], &ldwrkr, &work[itaup], &
			    work[iwork], &i__2, &ierr);
		    irwork = ie + *m;
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", m, m, &c__0, &c__0, &s[1], &rwork[ie], &work[
...
/* Computing MIN */
			...
			kgemm_("N", "N", m, &blk, m, &c_b2, &work[ir], &
				ldwrkr, &a[i__ * a_dim1 + 1], lda, &c_b1, &
				work[iu], &ldwrku);
			klacpy_("F", m, &blk, &work[iu], &ldwrku, &a[i__ * 
				a_dim1 + 1], lda);
/* L30: */
		    }
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("L", m, n, &c__0, &c__0, &s[1], &rwork[ie], &a[
			    a_offset], lda, cdum, &c__1, cdum, &c__1, &rwork[
			    irwork], info);
...
/*                 Copy L to U, zeroing about above it */
		    ...
		    i__3 = *m - 1;
		    i__2 = *m - 1;
		    klaset_("U", &i__3, &i__2, &c_b1, &c_b1, &u[(u_dim1 << 1) 
			    + 1], ldu);
...
/*                 (RWorkspace: need M) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    kgebrd_(m, m, &u[u_offset], ldu, &s[1], &rwork[ie], &work[
			    itauq], &work[itaup], &work[iwork], &i__3, &ierr);
		    klacpy_("U", m, m, &u[u_offset], ldu, &work[ir], &ldwrkr);
...
/*                 (RWorkspace: 0) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    kungbr_("Q", m, m, m, &u[u_offset], ldu, &work[itauq], &
			    work[iwork], &i__3, &ierr);
		    irwork = ie + *m;
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", m, m, m, &c__0, &s[1], &rwork[ie], &work[ir],
...
/*                 Copy L to U, zeroing out above it */
		    ...
		    i__2 = *m - 1;
		    i__3 = *m - 1;
		    klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &u[(u_dim1 << 1) 
			    + 1], ldu);
...
/*                 (RWorkspace: 0) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    kunmbr_("P", "L", "C", m, n, m, &u[u_offset], ldu, &work[
...
/*                 (RWorkspace: need BDSPAC) */
		    ...
		    kbdsqr_("U", m, n, m, &c__0, &s[1], &rwork[ie], &a[
			    a_offset], lda, &u[u_offset], ldu, cdum, &c__1, &
			    rwork[irwork], info);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[
				iwork], &i__2, &ierr);

/*                    Copy L to WORK(IR), zeroing out above it */

			klacpy_("L", m, m, &a[a_offset], lda, &work[ir], &
				ldwrkr);
			i__2 = *m - 1;
			i__3 = *m - 1;
			klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &work[ir + 
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kunglq_(m, n, m, &a[a_offset], lda, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = 1;
			itauq = itau;
			itaup = itauq + *m;
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", m, m, &c__0, &c__0, &s[1], &rwork[ie], &
...
/*                    (RWorkspace: 0) */
			...
			kgemm_("N", "N", m, n, m, &c_b2, &work[ir], &ldwrkr, &
...
/*                    Copy result to VT */
			...
			klacpy_("U", m, n, &a[a_offset], lda, &vt[vt_offset], 
				ldvt);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kunglq_(m, n, m, &vt[vt_offset], ldvt, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = 1;
			itauq = itau;
			itaup = itauq + *m;
...
/*                    Zero out above L in A */
			...
			i__2 = *m - 1;
			i__3 = *m - 1;
			klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &a[(a_dim1 <<
				 1) + 1], lda);
...
/*                    (RWorkspace: need M) */
			...
			i__2 = *lwork - iwork + 1;
			kgebrd_(m, m, &a[a_offset], lda, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kunmbr_("P", "L", "C", m, n, m, &a[a_offset], lda, &
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", m, n, &c__0, &c__0, &s[1], &rwork[ie], &
...
/*                    Copy L to WORK(IU), zeroing out below it */
			...
			klacpy_("L", m, m, &a[a_offset], lda, &work[iu], &
				ldwrku);
			i__2 = *m - 1;
			i__3 = *m - 1;
			klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &work[iu + 
				ldwrku], &ldwrku);
...
/*                    (RWorkspace: need   M) */
			...
			i__2 = *lwork - iwork + 1;
			kgebrd_(m, m, &work[iu], &ldwrku, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
			klacpy_("L", m, m, &work[iu], &ldwrku, &work[ir], &
				ldwrkr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungbr_("P", m, m, m, &work[iu], &ldwrku, &work[itaup]
				, &work[iwork], &i__2, &ierr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kungbr_("Q", m, m, m, &work[ir], &ldwrkr, &work[itauq]
				, &work[iwork], &i__2, &ierr);
			irwork = ie + *m;
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", m, m, m, &c__0, &s[1], &rwork[ie], &work[
...
/*                    (RWorkspace: 0) */
			...
			kgemm_("N", "N", m, n, m, &c_b2, &work[iu], &ldwrku, &
				a[a_offset], lda, &c_b1, &vt[vt_offset], ldvt);
...
/*                    (RWorkspace: 0) */
			...
			klacpy_("F", m, m, &work[ir], &ldwrkr, &a[a_offset], 
				lda);
...
/*                    (RWorkspace: need BDSPAC) */
			...
			kbdsqr_("U", m, n, m, &c__0, &s[1], &rwork[ie], &vt[
...
/*                    (RWorkspace: need M) */
			...
			kgebrd_(m, m, &work[iu], &ldwrku, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
			klacpy_("L", m, m, &work[iu], &ldwrku, &u[u_offset], 
				ldu);
...
/*                    Copy L to U, zeroing out above it */
			...
			klacpy_("L", m, m, &a[a_offset], lda, &u[u_offset], 
				ldu);
			i__2 = *m - 1;
			i__3 = *m - 1;
			klaset_("U", &i__2, &i__3, &c_b1, &c_b1, &u[(u_dim1 <<
				 1) + 1], ldu);
			ie = 1;
			itauq = itau;
			itaup = itauq + *m;
...
/*                    (RWorkspace: need M) */
			...
			i__2 = *lwork - iwork + 1;
			kgebrd_(m, m, &u[u_offset], ldu, &s[1], &rwork[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kunmbr_("P", "L", "C", m, n, m, &u[u_offset], ldu, &
				work[itaup], &vt[vt_offset], ldvt, &work[
				iwork], &i__2, &ierr);
...
/*                    (RWorkspace: 0) */
			...
			i__2 = *lwork - iwork + 1;
			kunglq_(n, n, m, &vt[vt_offset], ldvt, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = 1;
			itauq = itau;
			itaup = itauq + *m;
...
/*                    Copy right singular vectors of A from A to VT */
			...
			klacpy_("F", m, n, &a[a_offset], lda, &vt[vt_offset], 
				ldvt);
...
/*              (RWorkspace: 0) */
		...
		i__2 = *lwork - iwork + 1;
		kungbr_("P", &nrvt, n, m, &vt[vt_offset], ldvt, &work[itaup], 
			&work[iwork], &i__2, &ierr);
	    }
	    if (wntuo) {
...
/*              (RWorkspace: 0) */
		...
		i__2 = *lwork - iwork + 1;
		kungbr_("Q", m, m, n, &a[a_offset], lda, &work[itauq], &work[
			iwork], &i__2, &ierr);
	    }
	    if (wntvo) {
...
/*              (RWorkspace: need BDSPAC) */
		...
		kbdsqr_("L", m, &ncvt, &nru, &c__0, &s[1], &rwork[ie], &vt[
			vt_offset], ldvt, &u[u_offset], ldu, cdum, &c__1, &
			rwork[irwork], info);
	    } else if (! wntuo && wntvo) {
...
/*              (RWorkspace: need BDSPAC) */
		...
		kbdsqr_("L", m, &ncvt, &nru, &c__0, &s[1], &rwork[ie], &a[
			a_offset], lda, &u[u_offset], ldu, cdum, &c__1, &
			rwork[irwork], info);
	    } else {
...
/*     Undo scaling if necessary */
	...
	if (*info != 0 && anrm > bignum) {
	    i__2 = minmn - 1;
	    hlascl_("G", &c__0, &c__0, &bignum, &anrm, &i__2, &c__1, &rwork[
		    ie], &minmn, &ierr);
	}
	if (anrm < smlnum) {
	...
	if (*info != 0 && anrm < smlnum) {
	    i__2 = minmn - 1;
	    hlascl_("G", &c__0, &c__0, &smlnum, &anrm, &i__2, &c__1, &rwork[
		    ie], &minmn, &ierr);
	}
    }
