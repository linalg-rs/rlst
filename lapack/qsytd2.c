...
/* > */
/*  ===================================================================== */
void  qsytd2_(char *uplo, integer *n, quadreal *a, integer *
	lda, quadreal *d__, quadreal *e, quadreal *tau, integer *info)
{
...
/*           to annihilate A(1:i-1,i+1) */
	    ...
	    qlarfg_(&i__, &a[i__ + (i__ + 1) * a_dim1], &a[(i__ + 1) * a_dim1 
		    + 1], &c__1, &taui);
...
/*              Compute  x := tau * A * v  storing x in TAU(1:i) */
		...
		qsymv_(uplo, &i__, &taui, &a[a_offset], lda, &a[(i__ + 1) * 
			a_dim1 + 1], &c__1, &c_b8, &tau[1], &c__1);

/*              Compute  w := x - 1/2 * tau * (x**T * v) * v */

		alpha = taui * -.5 * qdot_(&i__, &tau[1], &c__1, &a[(i__ + 1) 
			* a_dim1 + 1], &c__1);
		qaxpy_(&i__, &alpha, &a[(i__ + 1) * a_dim1 + 1], &c__1, &tau[
			1], &c__1);

/*              Apply the transformation as a rank-2 update: */
/*                 A := A - v * w**T - w * v**T */

		qsyr2_(uplo, &i__, &c_b14, &a[(i__ + 1) * a_dim1 + 1], &c__1, 
...
/* Computing MIN */
	    i__3 = i__ + 2;
	    qlarfg_(&i__2, &a[i__ + 1 + i__ * a_dim1], &a[f2cmin(i__3,*n) + i__ *
		     a_dim1], &c__1, &taui);
...
/*              Compute  x := tau * A * v  storing y in TAU(i:n-1) */
		...
		i__2 = *n - i__;
		qsymv_(uplo, &i__2, &taui, &a[i__ + 1 + (i__ + 1) * a_dim1], 
...
/*              Compute  w := x - 1/2 * tau * (x**T * v) * v */
		...
		i__2 = *n - i__;
		alpha = taui * -.5 * qdot_(&i__2, &tau[i__], &c__1, &a[i__ + 
			1 + i__ * a_dim1], &c__1);
		i__2 = *n - i__;
		qaxpy_(&i__2, &alpha, &a[i__ + 1 + i__ * a_dim1], &c__1, &tau[
			i__], &c__1);
...
/*                 A := A - v * w**T - w * v**T */
		...
		i__2 = *n - i__;
		qsyr2_(uplo, &i__2, &c_b14, &a[i__ + 1 + i__ * a_dim1], &c__1,
