...
/* > */
/*  ===================================================================== */
void  qhgeqz_(char *job, char *compq, char *compz, integer *n, 
	integer *ilo, integer *ihi, quadreal *h__, integer *ldh, quadreal 
	*t, integer *ldt, quadreal *alphar, quadreal *alphai, quadreal *
	beta, quadreal *q, integer *ldq, quadreal *z__, integer *ldz, 
	quadreal *work, integer *lwork, integer *info)
{
    ...
    quadreal c__;
    integer j;
    quadreal s, v[3], s1, s2, t1, u1, u2, a11, a12, a21, a22, b11, b22, c12,
	     c21;
    integer jc;
    quadreal an, bn, cl, cq, cr;
    integer in;
    quadreal u12, w11, w12, w21;
    integer jr;
    quadreal cz, w22, sl, wi, sr, vs, wr, b1a, b2a, a1i, a2i, b1i, b2i, a1r,
	     a2r, b1r, b2r, wr2, ad11, ad12, ad21, ad22, c11i, c22i;
    integer jch;
    quadreal c11r, c22r;
    logical ilq;
    quadreal u12l, tau, sqi;
    logical ilz;
    quadreal ulp, sqr, szi, szr, ad11l, ad12l, ad21l, ad22l, ad32l, wabs, 
	    atol, btol, temp;
    extern void  qrot_(integer *, quadreal *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *), qlag2_(
	    quadreal *, integer *, quadreal *, integer *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *);
    quadreal temp2, s1inv, scale;
    extern logical lsame_(char *, char *);
    integer iiter, ilast, jiter;
    quadreal anorm, bnorm;
    integer maxit;
    quadreal tempi, tempr;
    extern quadreal qlapy2_(quadreal *, quadreal *), qlapy3_(quadreal 
	    *, quadreal *, quadreal *);
    extern void  qlasv2_(quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
    ...
    extern void  qlarfg_(integer *, quadreal *, quadreal *,
	     integer *, quadreal *);
    extern quadreal qlanhs_(char *, integer *, quadreal *, integer *, 
	    quadreal *);
    extern void  qlaset_(char *, integer *, integer *, 
	    quadreal *, quadreal *, quadreal *, integer *);
    quadreal safmin;
    extern void  qlartg_(quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *);
    quadreal safmax;
...
/*     Initialize Q and Z */
    ...
    if (icompq == 3) {
	qlaset_("Full", n, n, &c_b12, &c_b13, &q[q_offset], ldq);
    }
    if (icompz == 3) {
	qlaset_("Full", n, n, &c_b12, &c_b13, &z__[z_offset], ldz);
    }
...
/*     Machine Constants */
    ...
    safmin = qlamch_("S");
    safmax = 1. / safmin;
    ulp = qlamch_("E") * qlamch_("B");
    anorm = qlanhs_("F", &in, &h__[*ilo + *ilo * h_dim1], ldh, &work[1]);
    bnorm = qlanhs_("F", &in, &t[*ilo + *ilo * t_dim1], ldt, &work[1]);
/* Computing MAX */
...
/*              this may have to be done repeatedly. */
		...
		if (ilazro || ilazr2) {
			...
			qlartg_(&temp, &h__[jch + 1 + jch * h_dim1], &c__, &s,
				 &h__[jch + jch * h_dim1]);
			h__[jch + 1 + jch * h_dim1] = 0.;
			i__4 = ilastm - jch;
			qrot_(&i__4, &h__[jch + (jch + 1) * h_dim1], ldh, &
				h__[jch + 1 + (jch + 1) * h_dim1], ldh, &c__, 
				&s);
			i__4 = ilastm - jch;
			qrot_(&i__4, &t[jch + (jch + 1) * t_dim1], ldt, &t[
				jch + 1 + (jch + 1) * t_dim1], ldt, &c__, &s);
			if (ilq) {
			    qrot_(n, &q[jch * q_dim1 + 1], &c__1, &q[(jch + 1)
				     * q_dim1 + 1], &c__1, &c__, &s);
			}
			if (ilazr2) {
...
/*                 Then process as in the case T(ILAST,ILAST)=0 */
			...
			qlartg_(&temp, &t[jch + 1 + (jch + 1) * t_dim1], &c__,
				 &s, &t[jch + (jch + 1) * t_dim1]);
			t[jch + 1 + (jch + 1) * t_dim1] = 0.;
			if (jch < ilastm - 1) {
			    i__4 = ilastm - jch - 1;
			    qrot_(&i__4, &t[jch + (jch + 2) * t_dim1], ldt, &
			...
			qlartg_(&temp, &h__[jch + 1 + (jch - 1) * h_dim1], &
				c__, &s, &h__[jch + 1 + jch * h_dim1]);
			h__[jch + 1 + (jch - 1) * h_dim1] = 0.;
			i__4 = jch + 1 - ifrstm;
			qrot_(&i__4, &h__[ifrstm + jch * h_dim1], &c__1, &h__[
				ifrstm + (jch - 1) * h_dim1], &c__1, &c__, &s)
				;
			i__4 = jch - ifrstm;
			qrot_(&i__4, &t[ifrstm + jch * t_dim1], &c__1, &t[
				ifrstm + (jch - 1) * t_dim1], &c__1, &c__, &s)
				;
			if (ilz) {
			    qrot_(n, &z__[jch * z_dim1 + 1], &c__1, &z__[(jch 
...
L70:
	...
	qlartg_(&temp, &h__[ilast + (ilast - 1) * h_dim1], &c__, &s, &h__[
		ilast + ilast * h_dim1]);
	h__[ilast + (ilast - 1) * h_dim1] = 0.;
	i__2 = ilast - ifrstm;
	qrot_(&i__2, &h__[ifrstm + ilast * h_dim1], &c__1, &h__[ifrstm + (
		ilast - 1) * h_dim1], &c__1, &c__, &s);
	i__2 = ilast - ifrstm;
	qrot_(&i__2, &t[ifrstm + ilast * t_dim1], &c__1, &t[ifrstm + (ilast - 
		1) * t_dim1], &c__1, &c__, &s);
	if (ilz) {
	    qrot_(n, &z__[ilast * z_dim1 + 1], &c__1, &z__[(ilast - 1) * 
		    z_dim1 + 1], &c__1, &c__, &s);
	}
...
/*           returned by DLAG2 is the Wilkinson shift (AEP p.512), */
	    ...
	    d__1 = safmin * 100.;
	    qlag2_(&h__[ilast - 1 + (ilast - 1) * h_dim1], ldh, &t[ilast - 1 
...
/*        Initial Q */
	...
	qlartg_(&temp, &temp2, &c__, &s, &tempr);
...
/*        Sweep */
	...
	for (j = istart; j <= i__2; ++j) {
		...
		qlartg_(&temp, &h__[j + 1 + (j - 1) * h_dim1], &c__, &s, &h__[
			j + (j - 1) * h_dim1]);
...
/* L150: */
	    ...
	    qlartg_(&temp, &t[j + 1 + j * t_dim1], &c__, &s, &t[j + 1 + (j + 
		    1) * t_dim1]);
	    t[j + 1 + j * t_dim1] = 0.;
...
/*                       (  0  B22 ) */
	    ...
	    qlasv2_(&t[ilast - 1 + (ilast - 1) * t_dim1], &t[ilast - 1 + 
	    ...
	    qrot_(&i__2, &h__[ilast - 1 + (ilast - 1) * h_dim1], ldh, &h__[
		    ilast + (ilast - 1) * h_dim1], ldh, &cl, &sl);
	    i__2 = ilast + 1 - ifrstm;
	    qrot_(&i__2, &h__[ifrstm + (ilast - 1) * h_dim1], &c__1, &h__[
		    ifrstm + ilast * h_dim1], &c__1, &cr, &sr);

	    if (ilast < ilastm) {
		i__2 = ilastm - ilast;
		qrot_(&i__2, &t[ilast - 1 + (ilast + 1) * t_dim1], ldt, &t[
			ilast + (ilast + 1) * t_dim1], ldt, &cl, &sl);
	    }
	    if (ifrstm < ilast - 1) {
		i__2 = ifirst - ifrstm;
		qrot_(&i__2, &t[ifrstm + (ilast - 1) * t_dim1], &c__1, &t[
			ifrstm + ilast * t_dim1], &c__1, &cr, &sr);
	    }

	    if (ilq) {
		qrot_(n, &q[(ilast - 1) * q_dim1 + 1], &c__1, &q[ilast * 
			q_dim1 + 1], &c__1, &cl, &sl);
	    }
	    if (ilz) {
		qrot_(n, &z__[(ilast - 1) * z_dim1 + 1], &c__1, &z__[ilast * 
			z_dim1 + 1], &c__1, &cr, &sr);
	    }
...
/*              Zero (j-1)st column of A */
		...
		if (j > istart) {
		    ...
		    qlarfg_(&c__3, &h__[j + (j - 1) * h_dim1], &v[1], &c__1, &
			    tau);
		    v[0] = 1.;
		    h__[j + 1 + (j - 1) * h_dim1] = 0.;
...
/*           Rotations from the left */
	    ...
	    j = ilast - 1;
	    temp = h__[j + (j - 1) * h_dim1];
	    qlartg_(&temp, &h__[j + 1 + (j - 1) * h_dim1], &c__, &s, &h__[j + 
		    (j - 1) * h_dim1]);
	    h__[j + 1 + (j - 1) * h_dim1] = 0.;
