...
/*  ===================================================================== */
void  kstemr_(char *jobz, char *range, integer *n, halfreal *
	d__, halfreal *e, halfreal *vl, halfreal *vu, integer *il, 
	integer *iu, integer *m, halfreal *w, halfcomplex *z__, integer *
	ldz, integer *nzc, integer *isuppz, logical *tryrac, halfreal *work,
	 integer *lwork, integer *iwork, integer *liwork, integer *info)
{
    ...
    integer itmp;
    halfreal tnrm;
    extern void  hlae2_(halfreal *, halfreal *, halfreal 
	    *, halfreal *, halfreal *);
    integer inde2, itmp2;
    halfreal rtol1, rtol2;
    ...
    extern void  kswap_(integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *), hlaev2_(halfreal *, halfreal *, 
    ...
    integer iindbl;
    logical valeig;
    extern void  hlarrc_(char *, integer *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, halfreal *, integer *,
	     integer *, integer *, integer *), hlarre_(char *, 
    ...
    integer wbegin;
    halfreal safmin;
    extern void  hlarrj_(integer *, halfreal *, halfreal *,
    ...
    extern void  hlarrr_(integer *, halfreal *, halfreal *,
	     integer *), hlasrt_(char *, integer *, halfreal *, integer *);
    halfreal thresh;
    integer iinspl, indwrk, ifirst, liwmin, nzcmin;
    halfreal pivmin;
    integer nsplit;
    halfreal smlnum;
    extern void  klarrv_(integer *, halfreal *, halfreal *,
	     halfreal *, halfreal *, halfreal *, integer *, integer *, 
	    integer *, integer *, halfreal *, halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, integer *, integer *, 
	    halfreal *, halfcomplex *, integer *, integer *, halfreal *,
	     integer *, integer *);
...
/* Computing MIN */
	...
	if (wantz && alleig) {
	    ...
	    hlarrc_("T", n, vl, vu, &d__[1], &e[1], &safmin, &nzcmin, &itmp, &
		    itmp2, info);
...
/*           WANTZ .EQ. FALSE. */
	...
	if (zquery && *info == 0) {
	    i__1 = z_dim1 + 1;
	    z__[i__1].r = (halfreal) nzcmin, z__[i__1].i = 0.;
...
/*     Handle N = 0, 1, and 2 cases immediately */
    ...
    *m = 0;
    ...
    if (*n == 2) {
	if (! wantz) {
	    hlae2_(&d__[1], &e[1], &d__[2], &r1, &r2);
	} else if (wantz && ! zquery) {
	    hlaev2_(&d__[1], &e[1], &d__[2], &r1, &r2, &cs, &sn);
	}
...
/*        RMAX threshold. */
	...
	if (scale != 1.) {
	    hscal_(n, &scale, &d__[1], &c__1);
	    i__1 = *n - 1;
	    hscal_(&i__1, &scale, &e[1], &c__1);
	    tnrm *= scale;
	    if (valeig) {
...
/*           Copy original diagonal, needed to guarantee relative accuracy */
	    hcopy_(n, &d__[1], &c__1, &work[indd], &c__1);
	}
...
/* Computing MAX */
	...
	hlarre_(range, n, &wl, &wu, &iil, &iiu, &d__[1], &e[1], &work[inde2], 
...
/*           eigenvalues */
	    ...
	    klarrv_(n, &wl, &wu, &d__[1], &e[1], &pivmin, &iwork[iinspl], m, &
...
L36:
		...
		rtol2 = eps * 4.;
		hlarrj_(&in, &work[indd + ibegin - 1], &work[inde2 + ibegin - 
...
/* L50: */
		...
		if (i__ != 0) {
		    ...
		    w[j] = tmp;
		    if (wantz) {
			kswap_(n, &z__[i__ * z_dim1 + 1], &c__1, &z__[j * 
				z_dim1 + 1], &c__1);
			itmp = isuppz[(i__ << 1) - 1];
