...
/* > */
/*  ===================================================================== */
void  slasd4_(integer *n, integer *i__, real *d__, real *z__, 
	real *delta, real *rho, real *sigma, real *work, integer *info)
{
    ...
    logical swtch;
    real dtnsq;
    extern void  slaed6_(integer *, logical *, real *, real *, 
	    real *, real *, real *, integer *);
    real delsq2;
    extern void  slasd5_(integer *, real *, real *, real *, 
	    real *, real *, real *);
    real dtnsq1;
    logical swtch3;
...
/*           SIGMA_n^2 - D( N )*D( N ) */
		...
		b = z__[*n] * z__[*n] * delsq;
		if (a < 0.f) {
		    tau2 = b * 2.f / (M(sqrt)(a * a + b * 4.f * c__) - a);
		} else {
		    tau2 = (a + M(sqrt)(a * a + b * 4.f * c__)) / (c__ * 2.f);
		}
...
/*        Calculate the new step */
	...
	if (c__ == 0.f) {
	    ...
	    eta = (a + M(sqrt)((r__1 = a * a - b * 4.f * c__, abs(r__1)))) / (
		    c__ * 2.f);
	} else {
	    eta = b * 2.f / (a - M(sqrt)((r__1 = a * a - b * 4.f * c__, abs(r__1)
		    )));
	}
...
/*           Calculate the new step */
		...
		eta = b * 2.f / (a - M(sqrt)((r__1 = a * a - b * 4.f * c__, abs(
			r__1))));
	    }
...
/*        Calculate initial guess */
	...
	delsq2 = delsq / 2.f;
	sq2 = M(sqrt)((d__[*i__] * d__[*i__] + d__[ip1] * d__[ip1]) / 2.f);
...
/*           We choose d(i) as origin. */
	    ...
	    if (a > 0.f) {
		tau2 = b * 2.f / (a + M(sqrt)((r__1 = a * a - b * 4.f * c__, abs(
			r__1))));
	    } else {
		tau2 = (a - M(sqrt)((r__1 = a * a - b * 4.f * c__, abs(r__1)))) /
			 (c__ * 2.f);
	    }
...
/*           SIGMA - D( IP1 ). */
	    ...
	    tau = tau2 / (d__[ip1] + M(sqrt)((r__1 = d__[ip1] * d__[ip1] + tau2, 
		    abs(r__1))));
	}
...
/* Computing 2nd power */
		...
		if (c__ == 0.f) {
		    ...
		    eta = b * 2.f / (a + M(sqrt)((r__1 = a * a - b * 4.f * c__, 
			    abs(r__1))));
		}
	    }
	}
...
/* Computing 2nd power */
			...
			eta = (a - M(sqrt)((r__1 = a * a - b * 4.f * c__, abs(
				r__1)))) / (c__ * 2.f);
		    } else {
			eta = b * 2.f / (a + M(sqrt)((r__1 = a * a - b * 4.f * 
				c__, abs(r__1))));
		    }
		}
	    }
