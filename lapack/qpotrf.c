...
/*  ===================================================================== */
void  qpotrf_(char *uplo, integer *n, quadreal *a, integer *
	lda, integer *info)
{
    ...
    extern void  qpotrf2_(char *, integer *, quadreal *, 
	    integer *, integer *);
...
/*        Use unblocked code. */
	...
	qpotrf2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/* Computing MIN */
		...
		jb = f2cmin(i__3,i__4);
		i__3 = j - 1;
		qsyrk_("Upper", "Transpose", &jb, &i__3, &c_b13, &a[j * 
			a_dim1 + 1], lda, &c_b14, &a[j + j * a_dim1], lda);
		qpotrf2_("Upper", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/* Computing MIN */
		...
		jb = f2cmin(i__3,i__4);
		i__3 = j - 1;
		qsyrk_("Lower", "No transpose", &jb, &i__3, &c_b13, &a[j + 
			a_dim1], lda, &c_b14, &a[j + j * a_dim1], lda);
		qpotrf2_("Lower", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
