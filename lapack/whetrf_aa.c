...
/*  ===================================================================== */
void  zhetrf_aa__(char *uplo, integer *n, quadcomplex *a, 
	integer *lda, integer *ipiv, quadcomplex *work, integer *lwork, 
	integer *info)
{
    ...
    /* Local variables */
    integer j;
    extern void  zlahef_aa__(char *, integer *, integer *, 
    ...
    extern void  wcopy_(integer *, quadcomplex *, integer *, 
	    ...
	    integer *, quadcomplex *, integer *), xerbla_(char *, integer *);
...
/*        copy first row A(1, 1:N) into H(1:n) (stored in WORK(1:N)) */
	...
	wcopy_(n, &a[a_dim1 + 1], lda, &work[1], &c__1);
...
/*        Panel factorization */
	...
	i__1 = 2 - k1;
	i__2 = *n - j;
	zlahef_aa__(uplo, &i__1, &i__2, &jb, &a[f2cmax(1,j) + (j + 1) * a_dim1], 
...
/* Computing MIN */
	...
	for (j2 = j + 2; j2 <= i__1; ++j2) {
		...
		i__2 = j1 - k1 - 2;
		wswap_(&i__2, &a[j2 * a_dim1 + 1], &c__1, &a[ipiv[j2] * 
			a_dim1 + 1], &c__1);
	    }
	}
	j += jb;
...
/*              Merge rank-1 update with BLAS-3 update */
		...
		i__1 = *n - j;
		wcopy_(&i__1, &a[j - 1 + (j + 1) * a_dim1], lda, &work[j + 1 
			- j1 + 1 + jb * *n], &c__1);
		i__1 = *n - j;
		wscal_(&i__1, &alpha, &work[j + 1 - j1 + 1 + jb * *n], &c__1);
...
/*                 Update (J2, J2) diagonal block with ZGEMV */
			...
			i__3 = jb + 1;
			z__1.r = -1., z__1.i = -0.;
			wgemm_("Conjugate transpose", "Transpose", &c__1, &mj,
...
/*                 Update off-diagonal block of J2-th block row with ZGEMM */
		    ...
		    i__4 = jb + 1;
		    z__1.r = -1., z__1.i = -0.;
		    wgemm_("Conjugate transpose", "Transpose", &nj, &i__3, &
...
/*           WORK(J+1, 1) stores H(J+1, 1) */
	    ...
	    i__2 = *n - j;
	    wcopy_(&i__2, &a[j + 1 + (j + 1) * a_dim1], lda, &work[1], &c__1);
	}
	goto L10;
    } else {
...
/*         (stored in WORK(1:N)) */
	...
	wcopy_(n, &a[a_dim1 + 1], &c__1, &work[1], &c__1);
...
/*        Panel factorization */
	...
	i__2 = 2 - k1;
	i__1 = *n - j;
	zlahef_aa__(uplo, &i__2, &i__1, &jb, &a[j + 1 + f2cmax(1,j) * a_dim1], 
		lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1])
		;
...
/* Computing MIN */
	...
	for (j2 = j + 2; j2 <= i__2; ++j2) {
	    ...
	    if (j2 != ipiv[j2] && j1 - k1 > 2) {
		i__1 = j1 - k1 - 2;
		wswap_(&i__1, &a[j2 + a_dim1], lda, &a[ipiv[j2] + a_dim1], 
			lda);
	    }
	}
	j += jb;
...
/*              Merge rank-1 update with BLAS-3 update */
		...
		i__2 = *n - j;
		wcopy_(&i__2, &a[j + 1 + (j - 1) * a_dim1], &c__1, &work[j + 
			1 - j1 + 1 + jb * *n], &c__1);
		i__2 = *n - j;
		wscal_(&i__2, &alpha, &work[j + 1 - j1 + 1 + jb * *n], &c__1);
...
/*                 Update (J2, J2) diagonal block with ZGEMV */
			...
			i__3 = jb + 1;
			z__1.r = -1., z__1.i = -0.;
			wgemm_("No transpose", "Conjugate transpose", &mj, &
...
/*                 Update off-diagonal block of J2-th block column with ZGEMM */
		    ...
		    i__4 = jb + 1;
		    z__1.r = -1., z__1.i = -0.;
		    wgemm_("No transpose", "Conjugate transpose", &i__3, &nj, 
...
/*           WORK(J+1, 1) stores H(J+1, 1) */
	    ...
	    i__1 = *n - j;
	    wcopy_(&i__1, &a[j + 1 + (j + 1) * a_dim1], &c__1, &work[1], &
		    c__1);
	}
	goto L11;
    }
