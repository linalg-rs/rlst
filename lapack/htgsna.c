...
/* > generalized doublereal Schur canonical form (or of any matrix pair */
/* > (Q*A*Z**T, Q*B*Z**T) with orthogonal matrices Q and Z, where */
/* > Z**T denotes the transpose of Z. */
/* > */
/* > (A, B) must be in generalized doublereal Schur form (as returned by DGGES), */
...
/* >          for the eigenpair corresponding to a doublereal eigenvalue w(j), */
...
/* >          the specified condition numbers; for each selected doublereal */
...
/* > */
/* >  a) If the i-th eigenvalue w = (a,b) is doublereal */
/* > */
...
/* > */
/* >         f2cmin( d1, f2cmax( 1, |doublereal(s11)/doublereal(s22)| )*d2 ) */
/* > */
...
/* >                    [ t11  -t22 ], */
/* > */
/* >     This is done by computing (using doublereal arithmetic) the */
...
/* > */
/*  ===================================================================== */
void  htgsna_(char *job, char *howmny, logical *select, 
	integer *n, halfreal *a, integer *lda, halfreal *b, integer *ldb, 
	halfreal *vl, integer *ldvl, halfreal *vr, integer *ldvr, 
	halfreal *s, halfreal *dif, integer *mm, integer *m, halfreal *
	work, integer *lwork, integer *iwork, integer *info)
{
    ...
    halfreal alprqt;
    extern void  htgsyl_(char *, integer *, integer *, integer 
...
/*              Complex eigenvalue pair. */
		...
		d__1 = hnrm2_(n, &vr[ks * vr_dim1 + 1], &c__1);
		d__2 = hnrm2_(n, &vr[(ks + 1) * vr_dim1 + 1], &c__1);
		rnrm = hlapy2_(&d__1, &d__2);
		d__1 = hnrm2_(n, &vl[ks * vl_dim1 + 1], &c__1);
		d__2 = hnrm2_(n, &vl[(ks + 1) * vl_dim1 + 1], &c__1);
		lnrm = hlapy2_(&d__1, &d__2);
		hgemv_("N", n, n, &c_b19, &a[a_offset], lda, &vr[ks * vr_dim1 
			+ 1], &c__1, &c_b21, &work[1], &c__1);
		tmprr = hdot_(n, &work[1], &c__1, &vl[ks * vl_dim1 + 1], &
			c__1);
		tmpri = hdot_(n, &work[1], &c__1, &vl[(ks + 1) * vl_dim1 + 1],
			 &c__1);
		hgemv_("N", n, n, &c_b19, &a[a_offset], lda, &vr[(ks + 1) * 
			vr_dim1 + 1], &c__1, &c_b21, &work[1], &c__1);
		tmpii = hdot_(n, &work[1], &c__1, &vl[(ks + 1) * vl_dim1 + 1],
			 &c__1);
		tmpir = hdot_(n, &work[1], &c__1, &vl[ks * vl_dim1 + 1], &
			c__1);
		uhav = tmprr + tmpii;
		uhavi = tmpir - tmpri;
		hgemv_("N", n, n, &c_b19, &b[b_offset], ldb, &vr[ks * vr_dim1 
			+ 1], &c__1, &c_b21, &work[1], &c__1);
		tmprr = hdot_(n, &work[1], &c__1, &vl[ks * vl_dim1 + 1], &
			c__1);
		tmpri = hdot_(n, &work[1], &c__1, &vl[(ks + 1) * vl_dim1 + 1],
			 &c__1);
		hgemv_("N", n, n, &c_b19, &b[b_offset], ldb, &vr[(ks + 1) * 
			vr_dim1 + 1], &c__1, &c_b21, &work[1], &c__1);
		tmpii = hdot_(n, &work[1], &c__1, &vl[(ks + 1) * vl_dim1 + 1],
			 &c__1);
		tmpir = hdot_(n, &work[1], &c__1, &vl[ks * vl_dim1 + 1], &
		...
		uhbv = tmprr + tmpii;
		uhbvi = tmpir - tmpri;
		uhav = hlapy2_(&uhav, &uhavi);
		uhbv = hlapy2_(&uhbv, &uhbvi);
		cond = hlapy2_(&uhav, &uhbv);
		s[ks] = cond / (rnrm * lnrm);
		s[ks + 1] = s[ks];
...
/*              Real eigenvalue. */
		...
		rnrm = hnrm2_(n, &vr[ks * vr_dim1 + 1], &c__1);
		lnrm = hnrm2_(n, &vl[ks * vl_dim1 + 1], &c__1);
		hgemv_("N", n, n, &c_b19, &a[a_offset], lda, &vr[ks * vr_dim1 
			+ 1], &c__1, &c_b21, &work[1], &c__1);
		uhav = hdot_(n, &work[1], &c__1, &vl[ks * vl_dim1 + 1], &c__1)
			;
		hgemv_("N", n, n, &c_b19, &b[b_offset], ldb, &vr[ks * vr_dim1 
			+ 1], &c__1, &c_b21, &work[1], &c__1);
		uhbv = hdot_(n, &work[1], &c__1, &vl[ks * vl_dim1 + 1], &c__1)
			;
		cond = hlapy2_(&uhav, &uhbv);
		if (cond == 0.) {
		    s[ks] = -1.;
	...
	if (wantdf) {
	    if (*n == 1) {
		dif[ks] = hlapy2_(&a[a_dim1 + 1], &b[b_dim1 + 1]);
		goto L20;
	    }
...
/*              Compute the eigenvalue(s) at position K. */
		...
		d__1 = smlnum * eps;
		hlag2_(&work[1], &c__2, &work[5], &c__2, &d__1, &beta, dummy1,
			 &alphar, dummy, &alphai);
		alprqt = 1.;
		c1 = (alphar * alphar + alphai * alphai + beta * beta) * 2.;
		c2 = beta * 4. * beta * alphai * alphai;
		root1 = c1 + M(sqrt)(c1 * c1 - c2 * 4.);
		root2 = c2 / root1;
		root1 /= 2.;
/* Computing MIN */
		d__1 = M(sqrt)(root1), d__2 = M(sqrt)(root2);
...
/*           diagonal block beginning at A(k,k) to the (1,1) position. */
	    ...
	    hlacpy_("Full", n, n, &a[a_offset], lda, &work[1], n);
	    hlacpy_("Full", n, n, &b[b_offset], ldb, &work[*n * *n + 1], n);
	    ifst = k;
	    ilst = 1;

	    i__2 = *lwork - (*n << 1) * *n;
	    htgexc_(&c_false, &c_false, n, &work[1], n, &work[*n * *n + 1], n,
...
/*              and compute estimate of Difl((A11,B11), (A22, B22)). */
		...
		if (n2 == 0) {
		    ...
		    iz = (*n << 1) * *n + 1;
		    i__2 = *lwork - (*n << 1) * *n;
		    htgsyl_("N", &c__3, &n2, &n1, &work[*n * n1 + n1 + 1], n, 
