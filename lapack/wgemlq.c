...
/* > */
/*  ===================================================================== */
void  wgemlq_(char *side, char *trans, integer *m, integer *n, 
	integer *k, quadcomplex *a, integer *lda, quadcomplex *t, integer 
	*tsize, quadcomplex *c__, integer *ldc, quadcomplex *work, 
	integer *lwork, integer *info)
{
    ...
    /* Local variables */
    extern void  wlamswlq_(char *, char *, integer *, integer *
...
/* Computing MAX */
    ...
    if (left && *m <= *k || right && *n <= *k || nb <= *k || nb >= f2cmax(i__1,*
	    k)) {
	wgemlqt_(side, trans, m, n, k, &mb, &a[a_offset], lda, &t[6], &mb, &
		c__[c_offset], ldc, &work[1], info);
    } else {
	wlamswlq_(side, trans, m, n, k, &mb, &nb, &a[a_offset], lda, &t[6], &
