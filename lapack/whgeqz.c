...
/* > */
/*  ===================================================================== */
void  whgeqz_(char *job, char *compq, char *compz, integer *n, 
	integer *ilo, integer *ihi, quadcomplex *h__, integer *ldh, 
	quadcomplex *t, integer *ldt, quadcomplex *alpha, quadcomplex *
	beta, quadcomplex *q, integer *ldq, quadcomplex *z__, integer *
	ldz, quadcomplex *work, integer *lwork, quadreal *rwork, integer *
	info)
{
    ...
    quadcomplex u12;
    integer jr;
    quadcomplex ad11, ad12, ad21, ad22;
    integer jch;
    logical ilq, ilz;
    quadreal ulp;
    quadcomplex abi22;
    quadreal absb, atol, btol, temp;
    ...
    quadcomplex rtdisc;
    integer ischur;
    extern quadreal wlanhs_(char *, integer *, quadcomplex *, integer *, 
	    quadreal *);
    logical ilazro;
    integer icompz, ifirst;
    extern void  wlartg_(quadcomplex *, quadcomplex *, 
	    quadreal *, quadcomplex *, quadcomplex *);
    integer ifrstm;
...
/*     Initialize Q and Z */
    ...
    if (icompz == 3) {
	wlaset_("Full", n, n, &c_b1, &c_b2, &z__[z_offset], ldz);
    }
...
/*     Machine Constants */
    ...
    anorm = wlanhs_("F", &in, &h__[*ilo + *ilo * h_dim1], ldh, &rwork[1]);
    bnorm = wlanhs_("F", &in, &t[*ilo + *ilo * t_dim1], ldt, &rwork[1]);
/* Computing MAX */
...
/*     Set Eigenvalues IHI+1:N */
	...
	if (absb > safmin) {
	    ...
	    if (ilschr) {
		i__2 = j - 1;
		wscal_(&i__2, &signbc, &t[j * t_dim1 + 1], &c__1);
		wscal_(&j, &signbc, &h__[j * h_dim1 + 1], &c__1);
	    } else {
		wscal_(&c__1, &signbc, &h__[j + j * h_dim1], &c__1);
	    }
	    if (ilz) {
		wscal_(n, &signbc, &z__[j * z_dim1 + 1], &c__1);
	    }
	} else {
	    i__2 = j + j * t_dim1;
...
/*              this may have to be done repeatedly. */
		...
		if (ilazro || ilazr2) {
			...
			wlartg_(&ctemp, &h__[jch + 1 + jch * h_dim1], &c__, &
				s, &h__[jch + jch * h_dim1]);
			i__4 = jch + 1 + jch * h_dim1;
			h__[i__4].r = 0., h__[i__4].i = 0.;
			i__4 = ilastm - jch;
			wrot_(&i__4, &h__[jch + (jch + 1) * h_dim1], ldh, &
				h__[jch + 1 + (jch + 1) * h_dim1], ldh, &c__, 
				&s);
			i__4 = ilastm - jch;
			wrot_(&i__4, &t[jch + (jch + 1) * t_dim1], ldt, &t[
				jch + 1 + (jch + 1) * t_dim1], ldt, &c__, &s);
			if (ilq) {
			    d_cnjg(&z__1, &s);
			    wrot_(n, &q[jch * q_dim1 + 1], &c__1, &q[(jch + 1)
...
/*                 Then process as in the case T(ILAST,ILAST)=0 */
			...
			wlartg_(&ctemp, &t[jch + 1 + (jch + 1) * t_dim1], &
				c__, &s, &t[jch + (jch + 1) * t_dim1]);
			i__4 = jch + 1 + (jch + 1) * t_dim1;
			t[i__4].r = 0., t[i__4].i = 0.;
			if (jch < ilastm - 1) {
			    i__4 = ilastm - jch - 1;
			    wrot_(&i__4, &t[jch + (jch + 2) * t_dim1], ldt, &
			...
			i__4 = jch + 1 + jch * h_dim1;
			ctemp.r = h__[i__4].r, ctemp.i = h__[i__4].i;
			wlartg_(&ctemp, &h__[jch + 1 + (jch - 1) * h_dim1], &
				c__, &s, &h__[jch + 1 + jch * h_dim1]);
			i__4 = jch + 1 + (jch - 1) * h_dim1;
			h__[i__4].r = 0., h__[i__4].i = 0.;
			i__4 = jch + 1 - ifrstm;
			wrot_(&i__4, &h__[ifrstm + jch * h_dim1], &c__1, &h__[
				ifrstm + (jch - 1) * h_dim1], &c__1, &c__, &s)
				;
			i__4 = jch - ifrstm;
			wrot_(&i__4, &t[ifrstm + jch * t_dim1], &c__1, &t[
				ifrstm + (jch - 1) * t_dim1], &c__1, &c__, &s)
				;
			if (ilz) {
			    wrot_(n, &z__[jch * z_dim1 + 1], &c__1, &z__[(jch 
...
L50:
	...
	wlartg_(&ctemp, &h__[ilast + (ilast - 1) * h_dim1], &c__, &s, &h__[
		ilast + ilast * h_dim1]);
	i__2 = ilast + (ilast - 1) * h_dim1;
	h__[i__2].r = 0., h__[i__2].i = 0.;
	i__2 = ilast - ifrstm;
	wrot_(&i__2, &h__[ifrstm + ilast * h_dim1], &c__1, &h__[ifrstm + (
		ilast - 1) * h_dim1], &c__1, &c__, &s);
	i__2 = ilast - ifrstm;
	wrot_(&i__2, &t[ifrstm + ilast * t_dim1], &c__1, &t[ifrstm + (ilast - 
		1) * t_dim1], &c__1, &c__, &s);
	if (ilz) {
	    wrot_(n, &z__[ilast * z_dim1 + 1], &c__1, &z__[(ilast - 1) * 
		    z_dim1 + 1], &c__1, &c__, &s);
	}
...
L60:
	...
	if (absb > safmin) {
	    ...
	    if (ilschr) {
		i__2 = ilast - ifrstm;
		wscal_(&i__2, &signbc, &t[ifrstm + ilast * t_dim1], &c__1);
		i__2 = ilast + 1 - ifrstm;
		wscal_(&i__2, &signbc, &h__[ifrstm + ilast * h_dim1], &c__1);
	    } else {
		wscal_(&c__1, &signbc, &h__[ilast + ilast * h_dim1], &c__1);
	    }
	    if (ilz) {
		wscal_(n, &signbc, &z__[ilast * z_dim1 + 1], &c__1);
	    }
	} else {
	    i__2 = ilast + ilast * t_dim1;
	    t[i__2].r = 0., t[i__2].i = 0.;
...
/*        Initial Q */
	...
	wlartg_(&ctemp, &ctemp2, &c__, &s, &ctemp3);
...
/*        Sweep */
	...
	for (j = istart; j <= i__2; ++j) {
		...
		wlartg_(&ctemp, &h__[j + 1 + (j - 1) * h_dim1], &c__, &s, &
			h__[j + (j - 1) * h_dim1]);
		i__3 = j + 1 + (j - 1) * h_dim1;
		h__[i__3].r = 0., h__[i__3].i = 0.;
	    }
...
/* L110: */
	    ...
	    wlartg_(&ctemp, &t[j + 1 + j * t_dim1], &c__, &s, &t[j + 1 + (j + 
		    1) * t_dim1]);
	    i__3 = j + 1 + j * t_dim1;
	    t[i__3].r = 0., t[i__3].i = 0.;
