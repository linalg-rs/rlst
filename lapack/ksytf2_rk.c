...
/*  ===================================================================== */
void  zsytf2_rk__(char *uplo, integer *n, halfcomplex *a, 
	integer *lda, halfcomplex *e, integer *ipiv, integer *info)
{
    ...
    /* Local variables */
    integer i__, j, k, p;
    halfcomplex t, d11, d12, d21, d22;
    integer ii, kk, kp;
    halfcomplex wk, wkm1, wkp1;
    logical done;
    integer imax, jmax;
    extern void  ksyr_(char *, integer *, halfcomplex *, 
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    imax = ikamax_(&i__1, &a[k * a_dim1 + 1], &c__1);
	    i__1 = imax + k * a_dim1;
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = k - imax;
		    jmax = imax + ikamax_(&i__1, &a[imax + (imax + 1) * 
			    a_dim1], lda);
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    itemp = ikamax_(&i__1, &a[imax * a_dim1 + 1], &c__1);
...
/*              submatrix A(1:k,1:k) if we have a 2-by-2 pivot */
		...
		if (p > 1) {
		    i__1 = p - 1;
		    kswap_(&i__1, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 
			    1], &c__1);
		}
		if (p < k - 1) {
		    i__1 = k - p - 1;
		    kswap_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 
			    1) * a_dim1], lda);
		}
		i__1 = k + k * a_dim1;
...
/*              submatrix A(1:k,1:k) */
		...
		if (kp > 1) {
		    i__1 = kp - 1;
		    kswap_(&i__1, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 
			    + 1], &c__1);
		}
		if (kk > 1 && kp < kk - 1) {
		    i__1 = kk - kp - 1;
		    kswap_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (
			    kp + 1) * a_dim1], lda);
		}
		i__1 = kk + kk * a_dim1;
		t.r = a[i__1].r, t.i = a[i__1].i;
...
/*                       = A - W(k)*1/D(k)*W(k)**T */
			...
			ksyr_(uplo, &i__1, &z__1, &a[k * a_dim1 + 1], &c__1, &
				a[a_offset], lda);

/*                    Store U(k) in column k */

			i__1 = k - 1;
			kscal_(&i__1, &d11, &a[k * a_dim1 + 1], &c__1);
		    } else {
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + ikamax_(&i__1, &a[k + 1 + k * a_dim1], &c__1);
	    i__1 = imax + k * a_dim1;
	    colmax = (d__1 = a[i__1].r, abs(d__1)) + (d__2 = d_imag(&a[imax + 
		    k * a_dim1]), abs(d__2));
	} else {
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = imax - k;
		    jmax = k - 1 + ikamax_(&i__1, &a[imax + k * a_dim1], lda);
		    i__1 = imax + jmax * a_dim1;
		    rowmax = (d__1 = a[i__1].r, abs(d__1)) + (d__2 = d_imag(&
			    a[imax + jmax * a_dim1]), abs(d__2));
		} else {
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    itemp = imax + ikamax_(&i__1, &a[imax + 1 + imax * a_dim1]
			    , &c__1);
		    i__1 = itemp + imax * a_dim1;
		    dtemp = (d__1 = a[i__1].r, abs(d__1)) + (d__2 = d_imag(&a[
			    itemp + imax * a_dim1]), abs(d__2));
...
/*              submatrix A(k:n,k:n) if we have a 2-by-2 pivot */
		...
		if (p < *n) {
		    i__1 = *n - p;
		    kswap_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + 1 + p 
			    * a_dim1], &c__1);
		}
		if (p > k + 1) {
		    i__1 = p - k - 1;
		    kswap_(&i__1, &a[k + 1 + k * a_dim1], &c__1, &a[p + (k + 
			    1) * a_dim1], lda);
		}
		i__1 = k + k * a_dim1;
		t.r = a[i__1].r, t.i = a[i__1].i;
...
/*              submatrix A(k:n,k:n) */
		...
		if (kp < *n) {
		    i__1 = *n - kp;
		    kswap_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + 1 
			    + kp * a_dim1], &c__1);
		}
		if (kk < *n && kp > kk + 1) {
		    i__1 = kp - kk - 1;
		    kswap_(&i__1, &a[kk + 1 + kk * a_dim1], &c__1, &a[kp + (
			    kk + 1) * a_dim1], lda);
		}
		i__1 = kk + kk * a_dim1;
		t.r = a[i__1].r, t.i = a[i__1].i;
...
/*                       = A - W(k)*(1/D(k))*W(k)**T */
			...
			i__1 = *n - k;
			z__1.r = -d11.r, z__1.i = -d11.i;
			ksyr_(uplo, &i__1, &z__1, &a[k + 1 + k * a_dim1], &
				c__1, &a[k + 1 + (k + 1) * a_dim1], lda);

/*                    Store L(k) in column k */

			i__1 = *n - k;
			kscal_(&i__1, &d11, &a[k + 1 + k * a_dim1], &c__1);
		    } else {
