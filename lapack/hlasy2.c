...
/*  ===================================================================== */
void  hlasy2_(logical *ltranl, logical *ltranr, integer *isgn, 
	integer *n1, integer *n2, halfreal *tl, integer *ldtl, halfreal *
	tr, integer *ldtr, halfreal *b, integer *ldb, halfreal *scale, 
	halfreal *x, integer *ldx, halfreal *xnorm, integer *info)
{
    /* Initialized data */
    ...
    /* Local variables */
    integer i__, j, k;
    halfreal x2[2], l21, u11, u12;
    integer ip, jp;
    halfreal u22, t16[16]	/* was [4][4] */, gam, bet, eps, sgn, tmp[4], 
	    tau1, btmp[4], smin;
    integer ipiv;
    halfreal temp;
    integer jpiv[4];
...
/* Computing MAX */
    ...
    btmp[0] = 0.;
    hcopy_(&c__16, btmp, &c__0, t16, &c__1);
...
/* L70: */
	...
	if (ipsv != i__) {
	    hswap_(&c__4, &t16[ipsv - 1], &c__4, &t16[i__ - 1], &c__4);
	    temp = btmp[i__ - 1];
	    btmp[i__ - 1] = btmp[ipsv - 1];
	    btmp[ipsv - 1] = temp;
	}
	if (jpsv != i__) {
	    hswap_(&c__4, &t16[(jpsv << 2) - 4], &c__1, &t16[(i__ << 2) - 4], 
		    &c__1);
	}
	jpiv[i__ - 1] = jpsv;
