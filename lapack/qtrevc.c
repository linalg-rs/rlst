...
static quadreal c_b22 = 1.;
static quadreal c_b25 = 0.;
...
/* > */
/*  ===================================================================== */
void  qtrevc_(char *side, char *howmny, logical *select, 
	...
	quadreal *work, integer *info)
{
    ...
    quadreal wi, wr, rec, ulp, beta, emax;
    logical pair;
...
/*                    1-by-1 diagonal block */
			...
			qlaln2_(&c_false, &c__1, &c__1, &smin, &c_b22, &t[j + 
...
/*                    Update right-hand side */
			...
			i__1 = j - 1;
			d__1 = -x[0];
			qaxpy_(&i__1, &d__1, &t[j * t_dim1 + 1], &c__1, &work[
				*n + 1], &c__1);
...
/*                    2-by-2 diagonal block */
			...
			qlaln2_(&c_false, &c__2, &c__1, &smin, &c_b22, &t[j - 
...
/*                    Update right-hand side */
			...
			i__1 = j - 2;
			d__1 = -x[0];
			qaxpy_(&i__1, &d__1, &t[(j - 1) * t_dim1 + 1], &c__1, 
				&work[*n + 1], &c__1);
			i__1 = j - 2;
			d__1 = -x[1];
			qaxpy_(&i__1, &d__1, &t[j * t_dim1 + 1], &c__1, &work[
...
/*              Copy the vector x or Q*x to VR and normalize. */
		...
		if (! over) {
		    qcopy_(&ki, &work[*n + 1], &c__1, &vr[is * vr_dim1 + 1], &
			    c__1);
...
/* L70: */
		    ...
		    if (ki > 1) {
			i__1 = ki - 1;
			qgemv_("N", n, &i__1, &c_b22, &vr[vr_offset], ldvr, &
...
/*                    1-by-1 diagonal block */
			...
			qlaln2_(&c_false, &c__1, &c__2, &smin, &c_b22, &t[j + 
				j * t_dim1], ldt, &c_b22, &c_b22, &work[j + *
...
/*                    2-by-2 diagonal block */
			...
			qlaln2_(&c_false, &c__2, &c__2, &smin, &c_b22, &t[j - 
				1 + (j - 1) * t_dim1], ldt, &c_b22, &c_b22, &
...
/*              Copy the vector x or Q*x to VR and normalize. */
		...
		if (! over) {
		    qcopy_(&ki, &work[*n + 1], &c__1, &vr[(is - 1) * vr_dim1 
			    + 1], &c__1);
		    qcopy_(&ki, &work[n2 + 1], &c__1, &vr[is * vr_dim1 + 1], &
			    c__1);
...
/* L110: */
			...
			qscal_(n, &work[ki - 1 + *n], &vr[(ki - 1) * vr_dim1 
				+ 1], &c__1);
			qscal_(n, &work[ki + n2], &vr[ki * vr_dim1 + 1], &
				c__1);
		    }
...
/*                    the right-hand side. */
			...
			i__3 = j - ki - 1;
			work[j + *n] -= qdot_(&i__3, &t[ki + 1 + j * t_dim1], 
...
/* Computing MAX */
			...
			i__3 = j - ki - 1;
			work[j + 1 + *n] -= qdot_(&i__3, &t[ki + 1 + (j + 1) *
...
/*                      [T(J+1,J)    T(J+1,J+1)-WR]                ( WORK2 ) */
			...
			qlaln2_(&c_true, &c__2, &c__1, &smin, &c_b22, &t[j + 
				j * t_dim1], ldt, &c_b22, &c_b22, &work[j + *
				n], n, &wr, &c_b25, x, &c__2, &scale, &xnorm, 
				&ierr);
...
/*              Copy the vector x or Q*x to VL and normalize. */
		...
		if (! over) {
		    i__2 = *n - ki + 1;
		    qcopy_(&i__2, &work[ki + *n], &c__1, &vl[ki + is * 
			    vl_dim1], &c__1);

		    i__2 = *n - ki + 1;
		    ii = iqamax_(&i__2, &vl[ki + is * vl_dim1], &c__1) + ki - 
			    1;
		    remax = 1. / (d__1 = vl[ii + is * vl_dim1], abs(d__1));
		    i__2 = *n - ki + 1;
		    qscal_(&i__2, &remax, &vl[ki + is * vl_dim1], &c__1);
...
/* L180: */
		    ...
		    if (ki < *n) {
			i__2 = *n - ki;
			qgemv_("N", n, &i__2, &c_b22, &vl[(ki + 1) * vl_dim1 
...
/*                    forming the right-hand side elements. */
			...
			i__3 = j - ki - 2;
			work[j + *n] -= qdot_(&i__3, &t[ki + 2 + j * t_dim1], 
				&c__1, &work[ki + 2 + *n], &c__1);
			i__3 = j - ki - 2;
			work[j + n2] -= qdot_(&i__3, &t[ki + 2 + j * t_dim1], 
...
/* Computing MAX */
			...
			i__3 = j - ki - 2;
			work[j + 1 + *n] -= qdot_(&i__3, &t[ki + 2 + (j + 1) *
				 t_dim1], &c__1, &work[ki + 2 + *n], &c__1);

			i__3 = j - ki - 2;
			work[j + 1 + n2] -= qdot_(&i__3, &t[ki + 2 + (j + 1) *
...
/*                      ([T(j+1,j) T(j+1,j+1)]               ) */
			...
			d__1 = -wi;
			qlaln2_(&c_true, &c__2, &c__2, &smin, &c_b22, &t[j + 
				j * t_dim1], ldt, &c_b22, &c_b22, &work[j + *
				n], n, &wr, &d__1, x, &c__2, &scale, &xnorm, &
				ierr);
...
/*              Copy the vector x or Q*x to VL and normalize. */
		...
		if (! over) {
		    ...
		    qcopy_(&i__2, &work[ki + *n], &c__1, &vl[ki + is * 
			    vl_dim1], &c__1);
		    i__2 = *n - ki + 1;
		    qcopy_(&i__2, &work[ki + n2], &c__1, &vl[ki + (is + 1) * 
			    vl_dim1], &c__1);
...
/* L220: */
		    ...
		    qscal_(&i__2, &remax, &vl[ki + is * vl_dim1], &c__1);
		    i__2 = *n - ki + 1;
		    qscal_(&i__2, &remax, &vl[ki + (is + 1) * vl_dim1], &c__1)
			    ;
...
/* L230: */
		    ...
		    if (ki < *n - 1) {
			i__2 = *n - ki - 1;
			qgemv_("N", n, &i__2, &c_b22, &vl[(ki + 2) * vl_dim1 
			...
			qscal_(n, &work[ki + *n], &vl[ki * vl_dim1 + 1], &
				c__1);
			qscal_(n, &work[ki + 1 + n2], &vl[(ki + 1) * vl_dim1 
				+ 1], &c__1);
		    }
