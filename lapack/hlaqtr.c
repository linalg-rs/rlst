...
static halfreal c_b21 = 1.;
static halfreal c_b25 = 0.;
static logical c_true = TRUE_;

/* > \brief \b DLAQTR solves a doublereal quasi-triangular system of equations, or a complex quasi-triangular system
 of special form, in doublereal arithmetic. */
...
/* > \verbatim */
/* > */
/* > DLAQTR solves the doublereal quasi-triangular system */
/* > */
...
/* > */
/* > in doublereal arithmetic, where T is upper quasi-triangular. */
...
/*  ===================================================================== */
void  hlaqtr_(logical *ltran, logical *lreal, integer *n, 
	halfreal *t, integer *ldt, halfreal *b, halfreal *w, halfreal 
	*scale, halfreal *x, halfreal *work, integer *info)
{
    ...
    /* Local variables */
    halfreal d__[4]	/* was [2][2] */;
    integer i__, j, k;
    halfreal v[4]	/* was [2][2] */, z__;
    integer j1, j2, n1, n2;
    halfreal si, xj, sr, rec, eps, tjj, tmp;
    ...
    halfreal scaloc;
    extern void  hladiv_(halfreal *, halfreal *, 
...
/*     Set constants to control overflow */
    ...
    xnorm = hlange_("M", n, n, &t[t_offset], ldt, d__);
    if (! (*lreal)) {
/* Computing MAX */
	d__1 = xnorm, d__2 = abs(*w), d__1 = f2cmax(d__1,d__2), d__2 = hlange_(
		"M", n, &c__1, &b[1], n, d__);
...
/*     part of T to control overflow in triangular solver. */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	work[j] = hasum_(&i__2, &t[j * t_dim1 + 1], &c__1);
/* L10: */
    }
...
/* L20: */
    ...
    k = ihamax_(&n1, &x[1], &c__1);
    xmax = (d__1 = x[k], abs(d__1));
    *scale = 1.;

    if (xmax > bignum) {
	*scale = bignum / xmax;
	hscal_(&n1, scale, &x[1], &c__1);
	xmax = bignum;
    }
...
/*                 multiple of column j1 of T. */
			...
			i__1 = j1 - 1;
			d__1 = -x[j1];
			haxpy_(&i__1, &d__1, &t[j1 * t_dim1 + 1], &c__1, &x[1]
				, &c__1);
			i__1 = j1 - 1;
			k = ihamax_(&i__1, &x[1], &c__1);
			xmax = (d__1 = x[k], abs(d__1));
		    }
...
/*                 care of possible overflow by scaling factor. */
		    ...
		    d__[0] = x[j1];
		    d__[1] = x[j2];
		    hlaln2_(&c_false, &c__2, &c__1, &smin, &c_b21, &t[j1 + j1 
...
/*                 Update right-hand side */
			...
			i__1 = j1 - 1;
			d__1 = -x[j2];
			haxpy_(&i__1, &d__1, &t[j2 * t_dim1 + 1], &c__1, &x[1]
				, &c__1);
			i__1 = j1 - 1;
			k = ihamax_(&i__1, &x[1], &c__1);
			xmax = (d__1 = x[k], abs(d__1));
...
/*                 right-hand side element by inner product. */
		    ...
		    i__2 = j1 - 1;
		    x[j1] -= hdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, &x[1], &
			    c__1);
...
/* Computing MAX */
		    ...
		    i__2 = j1 - 1;
		    d__[0] = x[j1] - hdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, 
			    &x[1], &c__1);
		    i__2 = j1 - 1;
		    d__[1] = x[j2] - hdot_(&i__2, &t[j2 * t_dim1 + 1], &c__1, 
			    &x[1], &c__1);

		    hlaln2_(&c_true, &c__2, &c__1, &smin, &c_b21, &t[j1 + j1 *
...
/*                 Scale if necessary to avoid overflow in division */
			...
			*info = 1;
			...
			if (xj > bignum * tjj) {
			    rec = 1. / xj;
			    hscal_(&n2, &rec, &x[1], &c__1);
			    *scale *= rec;
			    xmax *= rec;
			}
		    }
		    hladiv_(&x[j1], &x[*n + j1], &tmp, &z__, &sr, &si);
		    x[j1] = sr;
		    x[*n + j1] = si;
		    xj = (d__1 = x[j1], abs(d__1)) + (d__2 = x[*n + j1], abs(
			    d__2));
...
/*                 multiple of column j1 of T. */
			...
			i__1 = j1 - 1;
			d__1 = -x[*n + j1];
			haxpy_(&i__1, &d__1, &t[j1 * t_dim1 + 1], &c__1, &x[*
				n + 1], &c__1);
...
/*                 Meet 2 by 2 diagonal block */
		    ...
		    d__[3] = x[*n + j2];
		    d__1 = -(*w);
		    hlaln2_(&c_false, &c__2, &c__2, &sminw, &c_b21, &t[j1 + 
...
/*                 Update the right-hand side. */
			...
			i__1 = j1 - 1;
			d__1 = -x[*n + j2];
			haxpy_(&i__1, &d__1, &t[j2 * t_dim1 + 1], &c__1, &x[*
				n + 1], &c__1);
...
/*                 right-hand side element by inner product. */
		    ...
		    x[j1] -= hdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, &x[1], &
			    c__1);
		    i__2 = j1 - 1;
		    x[*n + j1] -= hdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, &x[
			    *n + 1], &c__1);
		    if (j1 > 1) {
			x[j1] -= b[j1] * x[*n + 1];
			x[*n + j1] += b[j1] * x[1];
...
/*                 complex division */
			...
			*info = 1;
		    ...
		    d__1 = -z__;
		    hladiv_(&x[j1], &x[*n + j1], &tmp, &d__1, &sr, &si);
		    x[j1] = sr;
		    x[j1 + *n] = si;
/* Computing MAX */
...
/* Computing MAX */
		    ...
		    d__[1] = x[j2] - hdot_(&i__2, &t[j2 * t_dim1 + 1], &c__1, 
			    &x[1], &c__1);
		    i__2 = j1 - 1;
		    d__[2] = x[*n + j1] - hdot_(&i__2, &t[j1 * t_dim1 + 1], &
			    c__1, &x[*n + 1], &c__1);
		    i__2 = j1 - 1;
		    d__[3] = x[*n + j2] - hdot_(&i__2, &t[j2 * t_dim1 + 1], &
			    c__1, &x[*n + 1], &c__1);
		    d__[0] -= b[j1] * x[*n + 1];
		    d__[1] -= b[j2] * x[*n + 1];
		    d__[2] += b[j1] * x[1];
		    ...
		    hlaln2_(&c_true, &c__2, &c__2, &sminw, &c_b21, &t[j1 + j1 
			    * t_dim1], ldt, &c_b21, &c_b21, d__, &c__2, &
