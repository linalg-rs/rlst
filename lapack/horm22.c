...
/* > */
/* > */
/* >  DORM22 overwrites the general doublereal M-by-N matrix C with */
/* > */
/* >                  SIDE = 'L'     SIDE = 'R' */
/* >  TRANS = 'N':      Q * C          C * Q */
/* >  TRANS = 'T':      Q**T * C       C * Q**T */
/* > */
/* >  where Q is a doublereal orthogonal matrix of order NQ, with NQ = M if */
...
/*  ===================================================================== */
void  horm22_(char *side, char *trans, integer *m, integer *n, 
	integer *n1, integer *n2, halfreal *q, integer *ldq, halfreal *
	c__, integer *ldc, halfreal *work, integer *lwork, integer *info)
{
    ...
    extern void  htrmm_(char *, char *, char *, char *, 
	    ...
	    halfreal *, integer *), hlacpy_(
	    char *, integer *, integer *, halfreal *, integer *, halfreal 
...
/*     Degenerate cases (N1 = 0 or N2 = 0) are handled using DTRMM. */
    ...
    if (*n1 == 0) {
	htrmm_(side, "Upper", trans, "Non-Unit", m, n, &c_b10, &q[q_offset], 
		ldq, &c__[c_offset], ldc);
	work[1] = 1.;
	return;
    } else if (*n2 == 0) {
	htrmm_(side, "Lower", trans, "Non-Unit", m, n, &c_b10, &q[q_offset], 
		ldq, &c__[c_offset], ldc);
	work[1] = 1.;
	return;
    }
...
/*              Multiply bottom part of C by Q12. */
		...
		hlacpy_("All", n1, &len, &c__[*n2 + 1 + i__ * c_dim1], ldc, &
			work[1], &ldwork);
		htrmm_("Left", "Lower", "No Transpose", "Non-Unit", n1, &len, 
			&c_b10, &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[1], &
			ldwork);

/*              Multiply top part of C by Q11. */

		hgemm_("No Transpose", "No Transpose", n1, &len, n2, &c_b10, &
			q[q_offset], ldq, &c__[i__ * c_dim1 + 1], ldc, &c_b10,
			 &work[1], &ldwork);

/*              Multiply top part of C by Q21. */

		hlacpy_("All", n2, &len, &c__[i__ * c_dim1 + 1], ldc, &work[*
			n1 + 1], &ldwork);
		htrmm_("Left", "Upper", "No Transpose", "Non-Unit", n2, &len, 
			&c_b10, &q[*n1 + 1 + q_dim1], ldq, &work[*n1 + 1], &
			ldwork);

/*              Multiply bottom part of C by Q22. */

		hgemm_("No Transpose", "No Transpose", n2, &len, n1, &c_b10, &
			q[*n1 + 1 + (*n2 + 1) * q_dim1], ldq, &c__[*n2 + 1 + 
			i__ * c_dim1], ldc, &c_b10, &work[*n1 + 1], &ldwork);

/*              Copy everything back. */

		hlacpy_("All", m, &len, &work[1], &ldwork, &c__[i__ * c_dim1 
			+ 1], ldc);
	    }
	} else {
	    i__2 = *n;
...
/*              Multiply bottom part of C by Q21**T. */
		...
		hlacpy_("All", n2, &len, &c__[*n1 + 1 + i__ * c_dim1], ldc, &
			work[1], &ldwork);
		htrmm_("Left", "Upper", "Transpose", "Non-Unit", n2, &len, &
			c_b10, &q[*n1 + 1 + q_dim1], ldq, &work[1], &ldwork);

/*              Multiply top part of C by Q11**T. */

		hgemm_("Transpose", "No Transpose", n2, &len, n1, &c_b10, &q[
			q_offset], ldq, &c__[i__ * c_dim1 + 1], ldc, &c_b10, &
			work[1], &ldwork);

/*              Multiply top part of C by Q12**T. */

		hlacpy_("All", n1, &len, &c__[i__ * c_dim1 + 1], ldc, &work[*
			n2 + 1], &ldwork);
		htrmm_("Left", "Lower", "Transpose", "Non-Unit", n1, &len, &
...
/*              Multiply bottom part of C by Q22**T. */
		...
		hgemm_("Transpose", "No Transpose", n1, &len, n2, &c_b10, &q[*
...
/*              Multiply right part of C by Q21. */
		...
		hlacpy_("All", &len, n2, &c__[i__ + (*n1 + 1) * c_dim1], ldc, 
			&work[1], &ldwork);
		htrmm_("Right", "Upper", "No Transpose", "Non-Unit", &len, n2,
			 &c_b10, &q[*n1 + 1 + q_dim1], ldq, &work[1], &ldwork);

/*              Multiply left part of C by Q11. */

		hgemm_("No Transpose", "No Transpose", &len, n2, n1, &c_b10, &
			c__[i__ + c_dim1], ldc, &q[q_offset], ldq, &c_b10, &
			work[1], &ldwork);

/*              Multiply left part of C by Q12. */

		hlacpy_("All", &len, n1, &c__[i__ + c_dim1], ldc, &work[*n2 * 
			ldwork + 1], &ldwork);
		htrmm_("Right", "Lower", "No Transpose", "Non-Unit", &len, n1,
			 &c_b10, &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[*n2 * 
			ldwork + 1], &ldwork);

/*              Multiply right part of C by Q22. */

		hgemm_("No Transpose", "No Transpose", &len, n1, n2, &c_b10, &
...
/*              Copy everything back. */
		...
		hlacpy_("All", &len, n, &work[1], &ldwork, &c__[i__ + c_dim1],
			 ldc);
	    }
	} else {
	    i__2 = *m;
...
/*              Multiply right part of C by Q12**T. */
		...
		hlacpy_("All", &len, n1, &c__[i__ + (*n2 + 1) * c_dim1], ldc, 
			&work[1], &ldwork);
		htrmm_("Right", "Lower", "Transpose", "Non-Unit", &len, n1, &
			c_b10, &q[(*n2 + 1) * q_dim1 + 1], ldq, &work[1], &
			ldwork);

/*              Multiply left part of C by Q11**T. */

		hgemm_("No Transpose", "Transpose", &len, n1, n2, &c_b10, &
			c__[i__ + c_dim1], ldc, &q[q_offset], ldq, &c_b10, &
			work[1], &ldwork);

/*              Multiply left part of C by Q21**T. */

		hlacpy_("All", &len, n2, &c__[i__ + c_dim1], ldc, &work[*n1 * 
			ldwork + 1], &ldwork);
		htrmm_("Right", "Upper", "Transpose", "Non-Unit", &len, n2, &
			c_b10, &q[*n1 + 1 + q_dim1], ldq, &work[*n1 * ldwork 
			+ 1], &ldwork);

/*              Multiply right part of C by Q22**T. */

		hgemm_("No Transpose", "Transpose", &len, n2, n1, &c_b10, &
