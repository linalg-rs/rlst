...
/*  ===================================================================== */
void  wpstrf_(char *uplo, integer *n, quadcomplex *a, 
	...
	quadreal *work, integer *info)
{
    ...
    extern void  wherk_(char *, char *, integer *, integer *, 
	    ...
	    quadcomplex *, integer *), wgemv_(char *, 
    ...
    extern void  wpstf2_(char *, integer *, quadcomplex *, 
...
/*        Use unblocked code */
	...
	wpstf2_(uplo, n, &a[a_dim1 + 1], lda, &piv[1], rank, tol, &work[1], 
		info);
	goto L230;
...
/*                    Pivot OK, so can now swap pivot rows and columns */
			...
			i__4 = j - 1;
			wswap_(&i__4, &a[j * a_dim1 + 1], &c__1, &a[pvt * 
				a_dim1 + 1], &c__1);
			if (pvt < *n) {
			    i__4 = *n - pvt;
			    wswap_(&i__4, &a[j + (pvt + 1) * a_dim1], lda, &a[
...
/*                 Compute elements J+1:N of row J. */
		    ...
		    if (j < *n) {
			i__4 = j - 1;
			wlacgv_(&i__4, &a[j * a_dim1 + 1], &c__1);
			i__4 = j - k;
			i__5 = *n - j;
			z__1.r = -1., z__1.i = -0.;
			wgemv_("Trans", &i__4, &i__5, &z__1, &a[k + (j + 1) * 
			...
			i__4 = *n - j;
			d__1 = 1. / ajj;
			wqscal_(&i__4, &d__1, &a[j + (j + 1) * a_dim1], lda);
		    }
...
/*              Update trailing matrix, J already incremented */
		...
		if (k + jb <= *n) {
		    i__3 = *n - j + 1;
		    wherk_("Upper", "Conj Trans", &i__3, &jb, &c_b32, &a[k + 
...
/*                    Pivot OK, so can now swap pivot rows and columns */
			...
			a[i__4].r = a[i__5].r, a[i__4].i = a[i__5].i;
			i__4 = j - 1;
			wswap_(&i__4, &a[j + a_dim1], lda, &a[pvt + a_dim1], 
				lda);
			if (pvt < *n) {
			    i__4 = *n - pvt;
			    wswap_(&i__4, &a[pvt + 1 + j * a_dim1], &c__1, &a[
...
/*                 Compute elements J+1:N of column J. */
		    ...
		    if (j < *n) {
			i__4 = j - 1;
			wlacgv_(&i__4, &a[j + a_dim1], lda);
			i__4 = *n - j;
			i__5 = j - k;
			z__1.r = -1., z__1.i = -0.;
			wgemv_("No Trans", &i__4, &i__5, &z__1, &a[j + 1 + k *
			...
			i__4 = *n - j;
			d__1 = 1. / ajj;
			wqscal_(&i__4, &d__1, &a[j + 1 + j * a_dim1], &c__1);
		    }
...
/*              Update trailing matrix, J already incremented */
		...
		if (k + jb <= *n) {
		    i__3 = *n - j + 1;
		    wherk_("Lower", "No Trans", &i__3, &jb, &c_b32, &a[j + k *
