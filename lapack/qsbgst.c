...
/* > \verbatim */
/* > */
/* > DSBGST reduces a doublereal symmetric-definite banded generalized */
...
/*  ===================================================================== */
void  qsbgst_(char *vect, char *uplo, integer *n, integer *ka, 
	integer *kb, quadreal *ab, integer *ldab, quadreal *bb, integer *
	ldbb, quadreal *x, integer *ldx, quadreal *work, integer *info)
{
    ...
    extern void  qger_(integer *, integer *, quadreal *, 
	    ...
	    integer *), qrot_(integer *, quadreal *, integer *, quadreal *
	    , integer *, quadreal *, quadreal *), qscal_(integer *, 
	    quadreal *, quadreal *, integer *);
    extern logical lsame_(char *, char *);
    logical upper, wantx;
    extern void  qlar2v_(integer *, quadreal *, quadreal *,
	     quadreal *, integer *, quadreal *, quadreal *, integer *), 
	    qlaset_(char *, integer *, integer *, quadreal *, quadreal *, 
	    quadreal *, integer *), qlartg_(quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *), xerbla_(
	    char *, integer *), qlargv_(integer *, quadreal *, 
	    integer *, quadreal *, integer *, quadreal *, integer *);
    logical update;
    extern void  qlartv_(integer *, quadreal *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, integer *);
...
/*     Initialize X to the unit matrix, if needed */
    ...
    if (wantx) {
	qlaset_("Full", n, n, &c_b8, &c_b9, &x[x_offset], ldx);
    }
...
/*              post-multiply X by inv(S(i)) */
		...
		i__3 = *n - m;
		d__1 = 1. / bii;
		qscal_(&i__3, &d__1, &x[m + 1 + i__ * x_dim1], &c__1);
		if (kbt > 0) {
		    i__3 = *n - m;
		    qger_(&i__3, &kbt, &c_b20, &x[m + 1 + i__ * x_dim1], &
...
/*                 generate rotation to annihilate a(i,i-k+ka+1) */
		    ...
		    qlartg_(&ab[k + 1 + (i__ - k + *ka) * ab_dim1], &ra1, &
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		qlargv_(&nrt, &ab[j2t * ab_dim1 + 1], &inca, &work[j2t - m], &
...
/*              apply rotations in 1st set from the right */
		...
		i__4 = *ka - 1;
		for (l = 1; l <= i__4; ++l) {
		    qlartv_(&nr, &ab[ka1 - l + j2 * ab_dim1], &inca, &ab[*ka 
...
/*              blocks */
		...
		qlar2v_(&nr, &ab[ka1 + j2 * ab_dim1], &ab[ka1 + (j2 + 1) * 
...
/*           start applying rotations in 1st set from the left */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[l + (j2 + ka1 - l) * ab_dim1], &inca, &
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j2; i__2 < 0 ? j >= i__4 : j <= i__4; j += i__2) {
		    i__1 = *n - m;
		    qrot_(&i__1, &x[m + 1 + j * x_dim1], &c__1, &x[m + 1 + (j 
...
/*           finish applying rotations in 2nd set from the left */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[l + (j2 - l + 1) * ab_dim1], &inca, &ab[
...
/*              which have been created outside the band */
		...
		qlargv_(&nr, &ab[j2 * ab_dim1 + 1], &inca, &work[j2], &ka1, &
			work[*n + j2], &ka1);
...
/*              post-multiply X by product of rotations in 2nd set */
		...
		for (j = j2; i__2 < 0 ? j >= i__3 : j <= i__3; j += i__2) {
		    i__4 = *n - m;
		    qrot_(&i__4, &x[m + 1 + j * x_dim1], &c__1, &x[m + 1 + (j 
...
/*              post-multiply X by inv(S(i)) */
		...
		i__4 = *n - m;
		d__1 = 1. / bii;
		qscal_(&i__4, &d__1, &x[m + 1 + i__ * x_dim1], &c__1);
		if (kbt > 0) {
		    i__4 = *n - m;
		    i__3 = *ldbb - 1;
		    qger_(&i__4, &kbt, &c_b20, &x[m + 1 + i__ * x_dim1], &
...
/*                 generate rotation to annihilate a(i-k+ka+1,i) */
		    ...
		    qlartg_(&ab[ka1 - k + i__ * ab_dim1], &ra1, &work[*n + 
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		qlargv_(&nrt, &ab[ka1 + (j2t - *ka) * ab_dim1], &inca, &work[
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    qlartv_(&nr, &ab[l + 1 + (j2 - l) * ab_dim1], &inca, &ab[
...
/*              blocks */
		...
		qlar2v_(&nr, &ab[j2 * ab_dim1 + 1], &ab[(j2 + 1) * ab_dim1 + 
...
/*           start applying rotations in 1st set from the right */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[ka1 - l + 1 + j2 * ab_dim1], &inca, &ab[
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j2; i__3 < 0 ? j >= i__1 : j <= i__1; j += i__3) {
		    i__2 = *n - m;
		    qrot_(&i__2, &x[m + 1 + j * x_dim1], &c__1, &x[m + 1 + (j 
			    + 1) * x_dim1], &c__1, &work[*n + j - m], &work[j 
			    - m]);
/* L350: */
		}
...
/*           finish applying rotations in 2nd set from the right */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[ka1 - l + 1 + (j2 - *ka) * ab_dim1], &
...
/*              which have been created outside the band */
		...
		qlargv_(&nr, &ab[ka1 + (j2 - *ka) * ab_dim1], &inca, &work[j2]
			, &ka1, &work[*n + j2], &ka1);
...
/*              post-multiply X by inv(S(i)) */
		...
		d__1 = 1. / bii;
		qscal_(&nx, &d__1, &x[i__ * x_dim1 + 1], &c__1);
		if (kbt > 0) {
		    i__3 = *ldbb - 1;
		    qger_(&nx, &kbt, &c_b20, &x[i__ * x_dim1 + 1], &c__1, &bb[
...
/*                 generate rotation to annihilate a(i+k-ka-1,i) */
		    ...
		    qlartg_(&ab[k + 1 + i__ * ab_dim1], &ra1, &work[*n + i__ 
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		qlargv_(&nrt, &ab[(j1 + *ka) * ab_dim1 + 1], &inca, &work[j1],
			 &ka1, &work[*n + j1], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    qlartv_(&nr, &ab[ka1 - l + (j1 + l) * ab_dim1], &inca, &
...
/*              blocks */
		...
		qlar2v_(&nr, &ab[ka1 + j1 * ab_dim1], &ab[ka1 + (j1 - 1) * 
...
/*           start applying rotations in 1st set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[l + j1t * ab_dim1], &inca, &ab[l + 1 + (
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j1; i__4 < 0 ? j >= i__1 : j <= i__1; j += i__4) {
		    qrot_(&nx, &x[j * x_dim1 + 1], &c__1, &x[(j - 1) * x_dim1 
...
/*           finish applying rotations in 2nd set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[l + (j1t + *ka) * ab_dim1], &inca, &ab[
...
/*              which have been created outside the band */
		...
		qlargv_(&nr, &ab[(j1 + *ka) * ab_dim1 + 1], &inca, &work[m - *
...
/*                 generate rotation to annihilate a(i,i+k-ka-1) */
		    ...
		    qlartg_(&ab[ka1 - k + (i__ + k - *ka) * ab_dim1], &ra1, &
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		qlargv_(&nrt, &ab[ka1 + j1 * ab_dim1], &inca, &work[j1], &ka1,
			 &work[*n + j1], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the right */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    qlartv_(&nr, &ab[l + 1 + j1 * ab_dim1], &inca, &ab[l + 2 
...
/*              blocks */
		...
		qlar2v_(&nr, &ab[j1 * ab_dim1 + 1], &ab[(j1 - 1) * ab_dim1 + 
...
/*           start applying rotations in 1st set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[ka1 - l + 1 + (j1t - ka1 + l) * ab_dim1]
...
/*           finish applying rotations in 2nd set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    qlartv_(&nrt, &ab[ka1 - l + 1 + (j1t + l - 1) * ab_dim1], 
...
/*              which have been created outside the band */
		...
		qlargv_(&nr, &ab[ka1 + j1 * ab_dim1], &inca, &work[m - *kb + 
