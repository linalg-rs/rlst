...
/*  ===================================================================== */
void  qgges3_(char *jobvsl, char *jobvsr, char *sort, L_fp 
	selctg, integer *n, quadreal *a, integer *lda, quadreal *b, 
	integer *ldb, integer *sdim, quadreal *alphar, quadreal *alphai, 
	quadreal *beta, quadreal *vsl, integer *ldvsl, quadreal *vsr, 
	integer *ldvsr, quadreal *work, integer *lwork, logical *bwork, 
	integer *info)
{
    ...
    extern void  qgghd3_(char *, char *, integer *, integer *, 
    ...
    extern void  qhgeqz_(char *, char *, char *, integer *, 
	    ...
	    integer *, quadreal *, quadreal *, quadreal *, quadreal *,
	     integer *, quadreal *, integer *, quadreal *, integer *, 
	    integer *), qtgsen_(integer *, logical *, 
	    logical *, logical *, integer *, quadreal *, integer *, 
...
/*     Compute workspace */
    ...
    if (*info == 0) {
	qgeqrf_(n, n, &b[b_offset], ldb, &work[1], &work[1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = *n * 6 + 16, i__2 = *n * 3 + (integer) work[1];
	lwkopt = f2cmax(i__1,i__2);
	qormqr_("L", "T", n, n, n, &b[b_offset], ldb, &work[1], &a[a_offset], 
		lda, &work[1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = lwkopt, i__2 = *n * 3 + (integer) work[1];
	lwkopt = f2cmax(i__1,i__2);
	if (ilvsl) {
	    qorgqr_(n, n, n, &vsl[vsl_offset], ldvsl, &work[1], &work[1], &
		    c_n1, &ierr);
/* Computing MAX */
	    i__1 = lwkopt, i__2 = *n * 3 + (integer) work[1];
	    lwkopt = f2cmax(i__1,i__2);
	}
	qgghd3_(jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[b_offset], 
		ldb, &vsl[vsl_offset], ldvsl, &vsr[vsr_offset], ldvsr, &work[
		1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = lwkopt, i__2 = *n * 3 + (integer) work[1];
	lwkopt = f2cmax(i__1,i__2);
	qhgeqz_("S", jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[
...
/* Computing MAX */
	...
	lwkopt = f2cmax(i__1,i__2);
	if (wantst) {
	    qtgsen_(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &
...
/*     Initialize VSL */
    ...
    if (ilvsl) {
	qlaset_("Full", n, n, &c_b36, &c_b37, &vsl[vsl_offset], ldvsl);
	if (irows > 1) {
	    i__1 = irows - 1;
	    i__2 = irows - 1;
...
/*     Initialize VSR */
    ...
    if (ilvsr) {
	qlaset_("Full", n, n, &c_b36, &c_b37, &vsr[vsr_offset], ldvsr);
    }

/*     Reduce to generalized Hessenberg form */

    i__1 = *lwork + 1 - iwrk;
    qgghd3_(jobvsl, jobvsr, n, &ilo, &ihi, &a[a_offset], lda, &b[b_offset], 
...
/* L10: */
	...
	i__1 = *lwork - iwrk + 1;
	qtgsen_(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &b[
		b_offset], ldb, &alphar[1], &alphai[1], &beta[1], &vsl[
		vsl_offset], ldvsl, &vsr[vsr_offset], ldvsr, sdim, &pvsl, &
