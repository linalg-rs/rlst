...
/*  ===================================================================== */
void  qlasq3_(integer *i0, integer *n0, quadreal *z__, 
	integer *pp, quadreal *dmin__, quadreal *sigma, quadreal *desig,
	 quadreal *qmax, integer *nfail, integer *iter, integer *ndiv, 
	logical *ieee, integer *ttype, quadreal *dmin1, quadreal *dmin2, 
	quadreal *dn, quadreal *dn1, quadreal *dn2, quadreal *g, 
	quadreal *tau)
{
...
L40:
	...
	if (s <= t) {
	    s = z__[nn - 3] * (z__[nn - 5] / (t * (M(sqrt)(s / t + 1.) + 1.)));
	} else {
	    s = z__[nn - 3] * (z__[nn - 5] / (t + M(sqrt)(t) * M(sqrt)(t + s)));
	}
...
/*     Choose a shift. */
    ...
    qlasq4_(i0, n0, &z__[1], pp, &n0in, dmin__, dmin1, dmin2, dn, dn1, dn2, 
	    tau, ttype, g);
...
L70:
    ...
    qlasq5_(i0, n0, &z__[1], pp, tau, sigma, dmin__, dmin1, dmin2, dn, dn1, 
	    dn2, ieee, &eps);
...
L80:
    qlasq6_(i0, n0, &z__[1], pp, dmin__, dmin1, dmin2, dn, dn1, dn2);
    *ndiv += *n0 - *i0 + 2;
    ++(*iter);
    *tau = 0.;
