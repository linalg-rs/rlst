...
/* > */
/*  ===================================================================== */
void  wpbtf2_(char *uplo, integer *n, integer *kd, 
...
/* Computing MIN */
	    ...
	    if (kn > 0) {
		d__1 = 1. / ajj;
		wqscal_(&kn, &d__1, &ab[*kd + (j + 1) * ab_dim1], &kld);
		wlacgv_(&kn, &ab[*kd + (j + 1) * ab_dim1], &kld);
		wher_("Upper", &kn, &c_b8, &ab[*kd + (j + 1) * ab_dim1], &kld,
...
/* Computing MIN */
	    ...
	    if (kn > 0) {
		d__1 = 1. / ajj;
		wqscal_(&kn, &d__1, &ab[j * ab_dim1 + 2], &c__1);
		wher_("Lower", &kn, &c_b8, &ab[j * ab_dim1 + 2], &c__1, &ab[(
			j + 1) * ab_dim1 + 1], &kld);
	    }
/* L20: */
	}
