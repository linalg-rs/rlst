...
/* > \brief \b ZGEQRT3 recursively computes a QR factorization of a general doublereal or complex matrix using the c
...
/* > */
/*  ===================================================================== */
void  wgeqrt3_(integer *m, integer *n, quadcomplex *a, 
	integer *lda, quadcomplex *t, integer *ldt, integer *info)
{
    ...
    extern void  wgemm_(char *, char *, integer *, integer *, 
	    ...
	    , quadcomplex *, integer *), 
	    xerbla_(char *, integer *), wlarfg_(integer *, 
...
/*        Compute Householder transform when N=1 */
	...
	wlarfg_(m, &a[a_dim1 + 1], &a[f2cmin(2,*m) + a_dim1], &c__1, &t[t_dim1 + 
		1]);
...
/*        Compute A(1:M,1:N1) <- (Y1,R1,T1), where Q1 = I - Y1 T1 Y1^H */
	...
	wgeqrt3_(m, &n1, &a[a_offset], lda, &t[t_offset], ldt, &iinfo);
...
/*        Compute A(1:M,J1:N) = Q1^H A(1:M,J1:N) [workspace: T(1:N1,J1:N)] */
	...
	wtrmm_("L", "L", "C", "U", &n1, &n2, &c_b1, &a[a_offset], lda, &t[j1 *
		 t_dim1 + 1], ldt)
		;

	i__1 = *m - n1;
	wgemm_("C", "N", &n1, &n2, &i__1, &c_b1, &a[j1 + a_dim1], lda, &a[j1 
		+ j1 * a_dim1], lda, &c_b1, &t[j1 * t_dim1 + 1], ldt);

	wtrmm_("L", "U", "C", "N", &n1, &n2, &c_b1, &t[t_offset], ldt, &t[j1 *
		 t_dim1 + 1], ldt)
		;

	i__1 = *m - n1;
	z__1.r = -1., z__1.i = -0.;
	wgemm_("N", "N", &i__1, &n2, &n1, &z__1, &a[j1 + a_dim1], lda, &t[j1 *
		 t_dim1 + 1], ldt, &c_b1, &a[j1 + j1 * a_dim1], lda);

	wtrmm_("L", "L", "N", "U", &n1, &n2, &c_b1, &a[a_offset], lda, &t[j1 *
		 t_dim1 + 1], ldt)
		;
...
/*        Compute A(J1:M,J1:N) <- (Y2,R2,T2) where Q2 = I - Y2 T2 Y2^H */
	...
	i__1 = *m - n1;
	wgeqrt3_(&i__1, &n2, &a[j1 + j1 * a_dim1], lda, &t[j1 + j1 * t_dim1], 
		ldt, &iinfo);
...
/*        Compute T3 = T(1:N1,J1:N) = -T1 Y1^H Y2 T2 */
	...
	wtrmm_("R", "L", "N", "U", &n1, &n2, &c_b1, &a[j1 + j1 * a_dim1], lda,
		 &t[j1 * t_dim1 + 1], ldt);

	i__1 = *m - *n;
	wgemm_("C", "N", &n1, &n2, &i__1, &c_b1, &a[i1 + a_dim1], lda, &a[i1 
		+ j1 * a_dim1], lda, &c_b1, &t[j1 * t_dim1 + 1], ldt);

	z__1.r = -1., z__1.i = -0.;
	wtrmm_("L", "U", "N", "N", &n1, &n2, &z__1, &t[t_offset], ldt, &t[j1 *
		 t_dim1 + 1], ldt)
		;

	wtrmm_("R", "U", "N", "N", &n1, &n2, &c_b1, &t[j1 + j1 * t_dim1], ldt,
		 &t[j1 * t_dim1 + 1], ldt);
