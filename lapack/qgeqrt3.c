...
/* > \brief \b DGEQRT3 recursively computes a QR factorization of a general doublereal or complex matrix using the c
...
/* > \verbatim */
/* > */
/* > DGEQRT3 recursively computes a QR factorization of a doublereal M-by-N */
...
/* > */
/*  ===================================================================== */
void  qgeqrt3_(integer *m, integer *n, quadreal *a, integer *
	lda, quadreal *t, integer *ldt, integer *info)
{
...
/*        Compute Householder transform when N=1 */
	...
	qlarfg_(m, &a[a_dim1 + 1], &a[f2cmin(2,*m) + a_dim1], &c__1, &t[t_dim1 + 
		1]);
...
/*        Compute A(1:M,1:N1) <- (Y1,R1,T1), where Q1 = I - Y1 T1 Y1^H */
	...
	qgeqrt3_(m, &n1, &a[a_offset], lda, &t[t_offset], ldt, &iinfo);
...
/*        Compute A(1:M,J1:N) = Q1^H A(1:M,J1:N) [workspace: T(1:N1,J1:N)] */
	...
	qtrmm_("L", "L", "T", "U", &n1, &n2, &c_b8, &a[a_offset], lda, &t[j1 *
		 t_dim1 + 1], ldt)
		;

	i__1 = *m - n1;
	qgemm_("T", "N", &n1, &n2, &i__1, &c_b8, &a[j1 + a_dim1], lda, &a[j1 
		+ j1 * a_dim1], lda, &c_b8, &t[j1 * t_dim1 + 1], ldt);

	qtrmm_("L", "U", "T", "N", &n1, &n2, &c_b8, &t[t_offset], ldt, &t[j1 *
		 t_dim1 + 1], ldt)
		;

	i__1 = *m - n1;
	qgemm_("N", "N", &i__1, &n2, &n1, &c_b20, &a[j1 + a_dim1], lda, &t[j1 
		* t_dim1 + 1], ldt, &c_b8, &a[j1 + j1 * a_dim1], lda);

	qtrmm_("L", "L", "N", "U", &n1, &n2, &c_b8, &a[a_offset], lda, &t[j1 *
		 t_dim1 + 1], ldt)
		;
...
/*        Compute A(J1:M,J1:N) <- (Y2,R2,T2) where Q2 = I - Y2 T2 Y2^H */
	...
	i__1 = *m - n1;
	qgeqrt3_(&i__1, &n2, &a[j1 + j1 * a_dim1], lda, &t[j1 + j1 * t_dim1], 
		ldt, &iinfo);
...
/*        Compute T3 = T(1:N1,J1:N) = -T1 Y1^H Y2 T2 */
	...
	qtrmm_("R", "L", "N", "U", &n1, &n2, &c_b8, &a[j1 + j1 * a_dim1], lda,
		 &t[j1 * t_dim1 + 1], ldt);

	i__1 = *m - *n;
	qgemm_("T", "N", &n1, &n2, &i__1, &c_b8, &a[i1 + a_dim1], lda, &a[i1 
		+ j1 * a_dim1], lda, &c_b8, &t[j1 * t_dim1 + 1], ldt);

	qtrmm_("L", "U", "N", "N", &n1, &n2, &c_b20, &t[t_offset], ldt, &t[j1 
		* t_dim1 + 1], ldt);

	qtrmm_("R", "U", "N", "N", &n1, &n2, &c_b8, &t[j1 + j1 * t_dim1], ldt,
		 &t[j1 * t_dim1 + 1], ldt);
