...
/* > \verbatim */
/* > */
/* > DPOTRF computes the Cholesky factorization of a doublereal symmetric */
...
/*  ===================================================================== */
void  hpotrf_(char *uplo, integer *n, halfreal *a, integer *
	lda, integer *info)
{
    ...
    extern void  hpotrf2_(char *, integer *, halfreal *, 
	    integer *, integer *);
...
/*        Use unblocked code. */
	...
	hpotrf2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/* Computing MIN */
		...
		jb = f2cmin(i__3,i__4);
		i__3 = j - 1;
		hsyrk_("Upper", "Transpose", &jb, &i__3, &c_b13, &a[j * 
			a_dim1 + 1], lda, &c_b14, &a[j + j * a_dim1], lda);
		hpotrf2_("Upper", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/*                 Compute the current block row. */
		    ...
		    i__3 = *n - j - jb + 1;
		    i__4 = j - 1;
		    hgemm_("Transpose", "No transpose", &jb, &i__3, &i__4, &
			    c_b13, &a[j * a_dim1 + 1], lda, &a[(j + jb) * 
			    a_dim1 + 1], lda, &c_b14, &a[j + (j + jb) * 
			    a_dim1], lda);
		    i__3 = *n - j - jb + 1;
		    htrsm_("Left", "Upper", "Transpose", "Non-unit", &jb, &
...
/* Computing MIN */
		...
		jb = f2cmin(i__3,i__4);
		i__3 = j - 1;
		hsyrk_("Lower", "No transpose", &jb, &i__3, &c_b13, &a[j + 
			a_dim1], lda, &c_b14, &a[j + j * a_dim1], lda);
		hpotrf2_("Lower", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/*                 Compute the current block column. */
		    ...
		    i__3 = *n - j - jb + 1;
		    i__4 = j - 1;
		    hgemm_("No transpose", "Transpose", &i__3, &jb, &i__4, &
			    c_b13, &a[j + jb + a_dim1], lda, &a[j + a_dim1], 
			    lda, &c_b14, &a[j + jb + j * a_dim1], lda);
		    i__3 = *n - j - jb + 1;
		    htrsm_("Right", "Lower", "Transpose", "Non-unit", &i__3, &
