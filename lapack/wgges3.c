...
/*  ===================================================================== */
void  wgges3_(char *jobvsl, char *jobvsr, char *sort, L_fp 
	selctg, integer *n, quadcomplex *a, integer *lda, quadcomplex *b, 
	integer *ldb, integer *sdim, quadcomplex *alpha, quadcomplex *
	beta, quadcomplex *vsl, integer *ldvsl, quadcomplex *vsr, integer 
	*ldvsr, quadcomplex *work, integer *lwork, quadreal *rwork, 
	logical *bwork, integer *info)
{
    ...
    extern void  wlacpy_(char *, integer *, integer *, 
	    ...
	    quadcomplex *, quadcomplex *, integer *), whgeqz_(
	    char *, char *, char *, integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadcomplex *, quadcomplex *, quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadreal *, integer *), wtgsen_(integer 
	    *, logical *, logical *, logical *, integer *, quadcomplex *, 
...
/*     Compute workspace */
    ...
    if (*info == 0) {
	wgeqrf_(n, n, &b[b_offset], ldb, &work[1], &work[1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = 1, i__2 = *n + (integer) work[1].r;
	lwkopt = f2cmax(i__1,i__2);
	wunmqr_("L", "C", n, n, n, &b[b_offset], ldb, &work[1], &a[a_offset], 
		lda, &work[1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = lwkopt, i__2 = *n + (integer) work[1].r;
	lwkopt = f2cmax(i__1,i__2);
	if (ilvsl) {
	    wungqr_(n, n, n, &vsl[vsl_offset], ldvsl, &work[1], &work[1], &
		    c_n1, &ierr);
/* Computing MAX */
	    i__1 = lwkopt, i__2 = *n + (integer) work[1].r;
	    lwkopt = f2cmax(i__1,i__2);
	}
	wgghd3_(jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[b_offset], 
		ldb, &vsl[vsl_offset], ldvsl, &vsr[vsr_offset], ldvsr, &work[
		1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = lwkopt, i__2 = *n + (integer) work[1].r;
	lwkopt = f2cmax(i__1,i__2);
	whgeqz_("S", jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[
...
/* Computing MAX */
	...
	lwkopt = f2cmax(i__1,i__2);
	if (wantst) {
	    wtgsen_(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &
...
/*     Initialize VSL */
    ...
    if (ilvsl) {
	wlaset_("Full", n, n, &c_b1, &c_b2, &vsl[vsl_offset], ldvsl);
	if (irows > 1) {
	    i__1 = irows - 1;
	    i__2 = irows - 1;
	    wlacpy_("L", &i__1, &i__2, &b[ilo + 1 + ilo * b_dim1], ldb, &vsl[
		    ilo + 1 + ilo * vsl_dim1], ldvsl);
	}
	i__1 = *lwork + 1 - iwrk;
	wungqr_(&irows, &irows, &irows, &vsl[ilo + ilo * vsl_dim1], ldvsl, &
...
/*     Initialize VSR */
    ...
    if (ilvsr) {
	wlaset_("Full", n, n, &c_b1, &c_b2, &vsr[vsr_offset], ldvsr);
    }

/*     Reduce to generalized Hessenberg form */

    i__1 = *lwork + 1 - iwrk;
    wgghd3_(jobvsl, jobvsr, n, &ilo, &ihi, &a[a_offset], lda, &b[b_offset], 
...
/*     Perform QZ algorithm, computing Schur vectors if desired */
    ...
    iwrk = itau;
    i__1 = *lwork + 1 - iwrk;
    whgeqz_("S", jobvsl, jobvsr, n, &ilo, &ihi, &a[a_offset], lda, &b[
	    b_offset], ldb, &alpha[1], &beta[1], &vsl[vsl_offset], ldvsl, &
...
/*        Undo scaling on eigenvalues before selecting */
	...
	if (ilascl) {
	    wlascl_("G", &c__0, &c__0, &anrm, &anrmto, n, &c__1, &alpha[1], n,
		     &ierr);
	}
	if (ilbscl) {
	    wlascl_("G", &c__0, &c__0, &bnrm, &bnrmto, n, &c__1, &beta[1], n, 
		    &ierr);
	}
...
/* L10: */
	...
	i__1 = *lwork - iwrk + 1;
	wtgsen_(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &b[
		b_offset], ldb, &alpha[1], &beta[1], &vsl[vsl_offset], ldvsl, 
...
/*     Undo scaling */
    ...
    if (ilascl) {
	wlascl_("U", &c__0, &c__0, &anrmto, &anrm, n, n, &a[a_offset], lda, &
		ierr);
    ...
    if (ilbscl) {
	wlascl_("U", &c__0, &c__0, &bnrmto, &bnrm, n, n, &b[b_offset], ldb, &
		ierr);
