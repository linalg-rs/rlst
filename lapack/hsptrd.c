...
/* > */
/*  ===================================================================== */
void  hsptrd_(char *uplo, integer *n, halfreal *ap, 
	halfreal *d__, halfreal *e, halfreal *tau, integer *info)
{
    ...
    extern void  hlarfg_(integer *, halfreal *, halfreal *,
	     integer *, halfreal *), xerbla_(char *, integer *);
...
/*           to annihilate A(1:i-1,i+1) */
	    ...
	    hlarfg_(&i__, &ap[i1 + i__ - 1], &ap[i1], &c__1, &taui);
	    e[i__] = ap[i1 + i__ - 1];
...
/*              Compute  y := tau * A * v  storing y in TAU(1:i) */
		...
		hspmv_(uplo, &i__, &taui, &ap[1], &ap[i1], &c__1, &c_b8, &tau[
			1], &c__1);

/*              Compute  w := y - 1/2 * tau * (y**T *v) * v */

		alpha = taui * -.5 * hdot_(&i__, &tau[1], &c__1, &ap[i1], &
			c__1);
		haxpy_(&i__, &alpha, &ap[i1], &c__1, &tau[1], &c__1);

/*              Apply the transformation as a rank-2 update: */
/*                 A := A - v * w**T - w * v**T */

		hspr2_(uplo, &i__, &c_b14, &ap[i1], &c__1, &tau[1], &c__1, &
			ap[1]);
...
/*           to annihilate A(i+2:n,i) */
	    ...
	    i__2 = *n - i__;
	    hlarfg_(&i__2, &ap[ii + 1], &ap[ii + 2], &c__1, &taui);
	    e[i__] = ap[ii + 1];
...
/*              Compute  y := tau * A * v  storing y in TAU(i:n-1) */
		...
		i__2 = *n - i__;
		hspmv_(uplo, &i__2, &taui, &ap[i1i1], &ap[ii + 1], &c__1, &
			c_b8, &tau[i__], &c__1);

/*              Compute  w := y - 1/2 * tau * (y**T *v) * v */

		i__2 = *n - i__;
		alpha = taui * -.5 * hdot_(&i__2, &tau[i__], &c__1, &ap[ii + 
			1], &c__1);
		i__2 = *n - i__;
		haxpy_(&i__2, &alpha, &ap[ii + 1], &c__1, &tau[i__], &c__1);

/*              Apply the transformation as a rank-2 update: */
/*                 A := A - v * w**T - w * v**T */

		i__2 = *n - i__;
		hspr2_(uplo, &i__2, &c_b14, &ap[ii + 1], &c__1, &tau[i__], &
			c__1, &ap[i1i1]);
