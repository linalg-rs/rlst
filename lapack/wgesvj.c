...
/* > */
/*  ===================================================================== */
void  wgesvj_(char *joba, char *jobu, char *jobv, integer *m, 
	integer *n, quadcomplex *a, integer *lda, quadreal *sva, integer *
	mv, quadcomplex *v, integer *ldv, quadcomplex *cwork, integer *
	lwork, quadreal *rwork, integer *lrwork, integer *info)
{
    ...
    integer ierr;
    quadcomplex ompq;
    extern void  wrot_(integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadreal *, quadcomplex *);
    quadreal aapp0, aapq1, temp1, apoaq, aqoap;
    extern logical lsame_(char *, char *);
    quadreal theta, small, sfmin;
    logical lsvec;
    quadreal epsln;
    ...
    extern void  wcopy_(integer *, quadcomplex *, integer *, 
	    ...
	    integer *, quadcomplex *, integer *), waxpy_(integer *, 
	    quadcomplex *, quadcomplex *, integer *, quadcomplex *, 
	    integer *);
    extern quadreal qwnrm2_(integer *, quadcomplex *, integer *);
    extern void  wgsvj0_(char *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, quadreal *, 
	    integer *, quadcomplex *, integer *, quadreal *, quadreal *,
	     quadreal *, integer *, quadcomplex *, integer *, integer *), wgsvj1_(char *, integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, quadreal *, 
	    integer *, quadcomplex *, integer *, quadreal *, quadreal *,
	     quadreal *, integer *, quadcomplex *, integer *, integer *);
...
/*     #:( */
	...
	i__1 = *m + *n;
	cwork[1].r = (quadreal) i__1, cwork[1].i = 0.;
	rwork[1] = (quadreal) f2cmax(*n,6);
	return;
    }
...
/* [!]  (Make sure that SLAMCH() works properly on the target machine.) */
    ...
    epsln = qlamch_("Epsilon");
    rooteps = M(sqrt)(epsln);
    sfmin = qlamch_("SafeMinimum");
    rootsfmin = M(sqrt)(sfmin);
    small = sfmin / epsln;
    big = qlamch_("Overflow");
/*     BIG         = ONE    / SFMIN */
    rootbig = 1. / rootsfmin;
...
/*     Initialize the right singular vector matrix. */
    ...
    if (rsvec) {
	mvl = *n;
	wlaset_("A", &mvl, n, &c_b1, &c_b2, &v[v_offset], ldv);
    } else if (applv) {
	mvl = *mv;
    }
    rsvec = rsvec || applv;
...
/*     in A are detected, the procedure returns with INFO=-6. */
    ...
    skl = 1. / M(sqrt)((quadreal) (*m) * (quadreal) (*n));
    noscale = TRUE_;
    goscale = TRUE_;
...
/*        the input matrix is M-by-N lower triangular (trapezoidal) */
	...
	for (p = 1; p <= i__1; ++p) {
	    ...
	    aaqq = 1.;
	    i__2 = *m - p + 1;
	    wlassq_(&i__2, &a[p + p * a_dim1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("ZGESVJ", &i__2);
...
/*        the input matrix is M-by-N upper triangular (trapezoidal) */
	...
	for (p = 1; p <= i__1; ++p) {
	    aapp = 0.;
	    aaqq = 1.;
	    wlassq_(&p, &a[p * a_dim1 + 1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("ZGESVJ", &i__2);
...
/*        the input matrix is M-by-N general dense */
	...
	for (p = 1; p <= i__1; ++p) {
	    aapp = 0.;
	    aaqq = 1.;
	    wlassq_(m, &a[p * a_dim1 + 1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("ZGESVJ", &i__2);
...
/* #:) Quick return for zero matrix */
    ...
    if (aapp == 0.) {
	if (lsvec) {
	    wlaset_("G", m, n, &c_b1, &c_b2, &a[a_offset], lda);
	}
	rwork[1] = 1.;
	rwork[2] = 0.;
	rwork[3] = 0.;
...
/* #:) Quick return for one-column matrix */
    ...
    if (*n == 1) {
	if (lsvec) {
	    wlascl_("G", &c__0, &c__0, &sva[1], &skl, m, &c__1, &a[a_dim1 + 1]
		    , lda, &ierr);
	}
	rwork[1] = 1. / skl;
	if (sva[1] >= sfmin) {
...
/* Computing MIN */
	d__1 = sn / aaqq, d__2 = big / (aapp * M(sqrt)((quadreal) (*n)));
	temp1 = f2cmin(d__1,d__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else if (aaqq >= sn && aapp >= temp1) {
...
/* Computing MIN */
	d__1 = sn / aaqq, d__2 = big / (M(sqrt)((quadreal) (*n)) * aapp);
	temp1 = f2cmin(d__1,d__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else {
...
/*     Scale, if necessary */
    ...
    if (temp1 != 1.) {
	qlascl_("G", &c__0, &c__0, &c_b42, &temp1, n, &c__1, &sva[1], n, &
		ierr);
    }
    skl = temp1 * skl;
    if (skl != 1.) {
	wlascl_(joba, &c__0, &c__0, &c_b42, &skl, m, n, &a[a_offset], lda, &
		ierr);
	skl = 1. / skl;
    }
...
/*     [+ + x x]                    [x x].             [x x] */
	    ...
	    i__2 = *n - n34;
	    i__3 = *lwork - *n;
	    wgsvj0_(jobv, &i__1, &i__2, &a[n34 + 1 + (n34 + 1) * a_dim1], lda,
	    ...
	    i__2 = n34 - n2;
	    i__3 = *lwork - *n;
	    wgsvj0_(jobv, &i__1, &i__2, &a[n2 + 1 + (n2 + 1) * a_dim1], lda, &
	    ...
	    i__2 = *n - n2;
	    i__3 = *lwork - *n;
	    wgsvj1_(jobv, &i__1, &i__2, &n4, &a[n2 + 1 + (n2 + 1) * a_dim1], 
	    ...
	    i__2 = n2 - n4;
	    i__3 = *lwork - *n;
	    wgsvj0_(jobv, &i__1, &i__2, &a[n4 + 1 + (n4 + 1) * a_dim1], lda, &
		    cwork[n4 + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 
		    1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &cwork[*n 
		    + 1], &i__3, &ierr);

	    i__1 = *lwork - *n;
	    wgsvj0_(jobv, m, &n4, &a[a_offset], lda, &cwork[1], &sva[1], &mvl,
		     &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &cwork[*
		    n + 1], &i__1, &ierr);

	    i__1 = *lwork - *n;
	    wgsvj1_(jobv, m, &n2, &n4, &a[a_offset], lda, &cwork[1], &sva[1], 
	    ...
	    i__1 = *lwork - *n;
	    wgsvj0_(jobv, &n4, &n4, &a[a_offset], lda, &cwork[1], &sva[1], &
		    mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__2, &
		    cwork[*n + 1], &i__1, &ierr);

	    i__1 = *lwork - *n;
	    wgsvj0_(jobv, &n2, &n4, &a[(n4 + 1) * a_dim1 + 1], lda, &cwork[n4 
		    + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 1) * 
		    v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &cwork[*n + 1],
		     &i__1, &ierr);

	    i__1 = *lwork - *n;
	    wgsvj1_(jobv, &n2, &n2, &n4, &a[a_offset], lda, &cwork[1], &sva[1]
		    , &mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &
		    cwork[*n + 1], &i__1, &ierr);

	    i__1 = n2 + n4;
	    i__2 = *lwork - *n;
	    wgsvj0_(jobv, &i__1, &n4, &a[(n2 + 1) * a_dim1 + 1], lda, &cwork[
...
/* Computing MIN */
		...
		for (p = igl; p <= i__3; ++p) {
		    ...
		    i__4 = *n - p + 1;
		    q = iqamax_(&i__4, &sva[p], &c__1) + p - 1;
		    if (p != q) {
			wswap_(m, &a[p * a_dim1 + 1], &c__1, &a[q * a_dim1 + 
				1], &c__1);
			if (rsvec) {
			    wswap_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
				    v_dim1 + 1], &c__1);
			}
			temp1 = sva[p];
			sva[p] = sva[q];
...
/*        below should be replaced with "AAPP = DZNRM2( M, A(1,p), 1 )". */
			...
			if (sva[p] < rootbig && sva[p] > rootsfmin) {
			    sva[p] = qwnrm2_(m, &a[p * a_dim1 + 1], &c__1);
			} else {
			    temp1 = 0.;
			    aapp = 1.;
			    wlassq_(m, &a[p * a_dim1 + 1], &c__1, &temp1, &
				    aapp);
...
/* Computing MIN */
			...
			for (q = p + 1; q <= i__4; ++q) {
				...
				if (aaqq >= 1.) {
					...
					wcopy_(m, &a[p * a_dim1 + 1], &c__1, &
						cwork[*n + 1], &c__1);
					wlascl_("G", &c__0, &c__0, &aapp, &
						c_b42, m, &c__1, &cwork[*n + 
						1], lda, &ierr);
					wqotc_(&z__2, m, &cwork[*n + 1], &
						c__1, &a[q * a_dim1 + 1], &
						c__1);
					...
					wcopy_(m, &a[q * a_dim1 + 1], &c__1, &
						cwork[*n + 1], &c__1);
					wlascl_("G", &c__0, &c__0, &aaqq, &
						c_b42, m, &c__1, &cwork[*n + 
						1], lda, &ierr);
					wqotc_(&z__2, m, &a[p * a_dim1 + 1], &
						c__1, &cwork[*n + 1], &c__1);
					z__1.r = z__2.r / aapp, z__1.i = 
						z__2.i / aapp;
					aapq.r = z__1.r, aapq.i = z__1.i;
...
/* [RTD]      ROTATED = ROTATED + ONE */
					...
					if (abs(theta) > bigtheta) {
					    ...
					    z__1.r = t * z__2.r, z__1.i = t * 
						    z__2.i;
					    wrot_(m, &a[p * a_dim1 + 1], &
						    c__1, &a[q * a_dim1 + 1], 
						    &c__1, &cs, &z__1);
					    if (rsvec) {
			  d_cnjg(&z__2, &ompq);
			  z__1.r = t * z__2.r, z__1.i = t * z__2.i;
			  wrot_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
...
/* Computing MAX */
			  ...
			  wrot_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
				  v_dim1 + 1], &c__1, &cs, &z__1);
					...
					waxpy_(m, &z__1, &cwork[*n + 1], &
						c__1, &a[q * a_dim1 + 1], &
						c__1);
					wlascl_("G", &c__0, &c__0, &c_b42, &
...
/* Computing 2nd power */
					...
					if (aaqq < rootbig && aaqq > 
						rootsfmin) {
					    sva[q] = qwnrm2_(m, &a[q * a_dim1 
						    + 1], &c__1);
					} else {
					    t = 0.;
					    aaqq = 1.;
					    wlassq_(m, &a[q * a_dim1 + 1], &
						    c__1, &t, &aaqq);
					    sva[q] = t * M(sqrt)(aaqq);
					}
				    }
					...
					if (aapp < rootbig && aapp > 
						rootsfmin) {
					    aapp = qwnrm2_(m, &a[p * a_dim1 + 
						    1], &c__1);
					} else {
					    t = 0.;
					    aapp = 1.;
					    wlassq_(m, &a[p * a_dim1 + 1], &
						    c__1, &t, &aapp);
					    aapp = t * M(sqrt)(aapp);
					}
					sva[p] = aapp;
...
/* 2000  end of the ibr-loop */
	...
	if (sva[*n] < rootbig && sva[*n] > rootsfmin) {
	    sva[*n] = qwnrm2_(m, &a[*n * a_dim1 + 1], &c__1);
	} else {
	    t = 0.;
	    aapp = 1.;
	    wlassq_(m, &a[*n * a_dim1 + 1], &c__1, &t, &aapp);
	    sva[*n] = t * M(sqrt)(aapp);
	}
...
/*     Additional steering devices */
	...
	if (i__ > swband + 1 && mxaapq < M(sqrt)((quadreal) (*n)) * tol && (
		quadreal) (*n) * mxaapq * mxsinj < tol) {
	    goto L1994;
	}
...
/*     the underflow threshold. */
    ...
    for (p = 1; p <= i__1; ++p) {
	i__2 = *n - p + 1;
	q = iqamax_(&i__2, &sva[p], &c__1) + p - 1;
	if (p != q) {
	    temp1 = sva[p];
	    sva[p] = sva[q];
	    sva[q] = temp1;
	    wswap_(m, &a[p * a_dim1 + 1], &c__1, &a[q * a_dim1 + 1], &c__1);
	    if (rsvec) {
		wswap_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * v_dim1 + 1], &
			c__1);
	    }
	}
	if (sva[p] != 0.) {
...
/*            CALL ZDSCAL( M, ONE / SVA( p ), A( 1, p ), 1 ) */
	    wlascl_("G", &c__0, &c__0, &sva[p], &c_b42, m, &c__1, &a[p * 
		    a_dim1 + 1], m, &ierr);
/* L1998: */
	}
    }
...
/*     Scale the product of Jacobi rotations. */
	...
	i__1 = *n;
	for (p = 1; p <= i__1; ++p) {
	    temp1 = 1. / qwnrm2_(&mvl, &v[p * v_dim1 + 1], &c__1);
	    wqscal_(&mvl, &temp1, &v[p * v_dim1 + 1], &c__1);
/* L2399: */
	}
    }
