...
/*  ===================================================================== */
void  wlaqp2_(integer *m, integer *n, integer *offset, 
	quadcomplex *a, integer *lda, integer *jpvt, quadcomplex *tau, 
	quadreal *vn1, quadreal *vn2, quadcomplex *work)
{
    ...
    extern void  wlarf_(char *, integer *, integer *, 
	    ...
	    integer *, quadcomplex *), wswap_(integer *, 
...
/*        Determine ith pivot column and swap if necessary. */
	...
	i__2 = *n - i__ + 1;
	pvt = i__ - 1 + iqamax_(&i__2, &vn1[i__], &c__1);

	if (pvt != i__) {
	    wswap_(m, &a[pvt * a_dim1 + 1], &c__1, &a[i__ * a_dim1 + 1], &
		    c__1);
	    itemp = jpvt[pvt];
	    jpvt[pvt] = jpvt[i__];
	    jpvt[i__] = itemp;
...
/*        Generate elementary reflector H(i). */
	...
	if (offpi < *m) {
	    i__2 = *m - offpi + 1;
	    wlarfg_(&i__2, &a[offpi + i__ * a_dim1], &a[offpi + 1 + i__ * 
		    a_dim1], &c__1, &tau[i__]);
	} else {
	    wlarfg_(&c__1, &a[*m + i__ * a_dim1], &a[*m + i__ * a_dim1], &
		    c__1, &tau[i__]);
	}
...
/*           Apply H(i)**H to A(offset+i:m,i+1:n) from the left. */
	    ...
	    i__3 = *n - i__;
	    d_cnjg(&z__1, &tau[i__]);
	    wlarf_("Left", &i__2, &i__3, &a[offpi + i__ * a_dim1], &c__1, &
...
/* Computing 2nd power */
		...
		if (temp2 <= tol3z) {
		    if (offpi < *m) {
			i__3 = *m - offpi;
			vn1[j] = qwnrm2_(&i__3, &a[offpi + 1 + j * a_dim1], &
				c__1);
			vn2[j] = vn1[j];
		    } else {
			vn1[j] = 0.;
