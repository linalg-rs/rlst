...
/*  ===================================================================== */
void  htbrfs_(char *uplo, char *trans, char *diag, integer *n, 
	integer *kd, integer *nrhs, halfreal *ab, integer *ldab, halfreal 
	*b, integer *ldb, halfreal *x, integer *ldx, halfreal *ferr, 
	halfreal *berr, halfreal *work, integer *iwork, integer *info)
{
    ...
    integer isave[3];
    extern void  htbmv_(char *, char *, char *, integer *, 
	    integer *, halfreal *, integer *, halfreal *, integer *), hcopy_(integer *, halfreal *, integer *
	    , halfreal *, integer *), htbsv_(char *, char *, char *, 
	    integer *, integer *, halfreal *, integer *, halfreal *, 
	    integer *), haxpy_(integer *, halfreal *
...
/*        where op(A) = A or A**T, depending on TRANS. */
	...
	hcopy_(n, &x[j * x_dim1 + 1], &c__1, &work[*n + 1], &c__1);
	htbmv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1], 
		&c__1);
	haxpy_(n, &c_b19, &b[j * b_dim1 + 1], &c__1, &work[*n + 1], &c__1);
...
/*              Multiply by diag(W)*inv(op(A)**T). */
		...
		htbsv_(uplo, transt, diag, n, kd, &ab[ab_offset], ldab, &work[
			*n + 1], &c__1);
		i__2 = *n;
		for (i__ = 1; i__ <= i__2; ++i__) {
...
/* L230: */
		...
		htbsv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*
			n + 1], &c__1);
	    }
	    goto L210;
	}
