...
void  zhetrd_he2hb__(char *uplo, integer *n, integer *kd, 
	halfcomplex *a, integer *lda, halfcomplex *ab, integer *ldab, 
	halfcomplex *tau, halfcomplex *work, integer *lwork, integer *
	info)
{
    ...
    extern void  kgemm_(char *, char *, integer *, integer *, 
	    ...
	    integer *), khemm_(char *, char *, integer *, 
	    integer *, halfcomplex *, halfcomplex *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, halfcomplex *, 
	    integer *);
    integer lwmin;
    ...
    extern void  kcopy_(integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *), kher2k_(char *, char *, integer *, 
	    integer *, halfcomplex *, halfcomplex *, integer *, 
    ...
    extern void  kgelqf_(integer *, integer *, halfcomplex *,
	    ...
	    ), kgeqrf_(integer *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, halfcomplex *, integer *, integer *), klarft_(
	    char *, char *, integer *, integer *, halfcomplex *, integer *, 
...
/* Computing MIN */
		i__2 = *kd + 1;
		lk = f2cmin(i__2,i__);
		kcopy_(&lk, &a[i__ - lk + 1 + i__ * a_dim1], &c__1, &ab[*kd + 
...
/* Computing MIN */
		...
		lk = f2cmin(i__2,i__3);
		kcopy_(&lk, &a[i__ + i__ * a_dim1], &c__1, &ab[i__ * ab_dim1 
			+ 1], &c__1);
/* L110: */
	    }
	}
...
/*     way everytime T is generated the upper/lower portion will be always zero */
    ...
    klaset_("A", &ldt, kd, &c_b1, &c_b1, &work[tpos], &ldt);
...
/*            Compute the LQ factorization of the current block */
	    ...
	    kgelqf_(kd, &pn, &a[i__ + (i__ + *kd) * a_dim1], lda, &tau[i__], &
...
/* Computing MIN */
		...
		i__4 = *ldab - 1;
		kcopy_(&lk, &a[j + j * a_dim1], lda, &ab[*kd + 1 + j * 
			ab_dim1], &i__4);
/* L20: */
	    }

	    klaset_("Lower", &pk, &pk, &c_b1, &c_b2, &a[i__ + (i__ + *kd) * 
		    a_dim1], lda);

/*            Form the matrix T */

	    klarft_("Forward", "Rowwise", &pn, &pk, &a[i__ + (i__ + *kd) * 
		    a_dim1], lda, &tau[i__], &work[tpos], &ldt);

/*            Compute W: */

	    kgemm_("Conjugate", "No transpose", &pk, &pn, &pk, &c_b2, &work[
		    tpos], &ldt, &a[i__ + (i__ + *kd) * a_dim1], lda, &c_b1, &
		    work[s2pos], &lds2);

	    khemm_("Right", uplo, &pk, &pn, &c_b2, &a[i__ + *kd + (i__ + *kd) 
		    * a_dim1], lda, &work[s2pos], &lds2, &c_b1, &work[wpos], &
		    ldw);

	    kgemm_("No transpose", "Conjugate", &pk, &pk, &pn, &c_b2, &work[
		    wpos], &ldw, &work[s2pos], &lds2, &c_b1, &work[s1pos], &
		    lds1);

	    z__1.r = -.5, z__1.i = -0.;
	    kgemm_("No transpose", "No transpose", &pk, &pn, &pk, &z__1, &
...
/*            an update of the form:  A := A - V'*W - W'*V */
	    ...
	    z__1.r = -1., z__1.i = -0.;
	    kher2k_(uplo, "Conjugate", &pn, &pk, &z__1, &a[i__ + (i__ + *kd) *
...
/* Computing MIN */
	    ...
	    i__1 = *ldab - 1;
	    kcopy_(&lk, &a[j + j * a_dim1], lda, &ab[*kd + 1 + j * ab_dim1], &
		    i__1);
/* L30: */
	}
...
/*            Compute the QR factorization of the current block */
	    ...
	    kgeqrf_(&pn, kd, &a[i__ + *kd + i__ * a_dim1], lda, &tau[i__], &
		    work[s2pos], &ls2, &iinfo);
...
/* Computing MIN */
		i__4 = *kd, i__5 = *n - j;
		lk = f2cmin(i__4,i__5) + 1;
		kcopy_(&lk, &a[j + j * a_dim1], &c__1, &ab[j * ab_dim1 + 1], &
			c__1);
/* L50: */
	    }

	    klaset_("Upper", &pk, &pk, &c_b1, &c_b2, &a[i__ + *kd + i__ * 
		    a_dim1], lda);

/*            Form the matrix T */

	    klarft_("Forward", "Columnwise", &pn, &pk, &a[i__ + *kd + i__ * 
		    a_dim1], lda, &tau[i__], &work[tpos], &ldt);

/*            Compute W: */

	    kgemm_("No transpose", "No transpose", &pn, &pk, &pk, &c_b2, &a[
		    i__ + *kd + i__ * a_dim1], lda, &work[tpos], &ldt, &c_b1, 
		    &work[s2pos], &lds2);

	    khemm_("Left", uplo, &pn, &pk, &c_b2, &a[i__ + *kd + (i__ + *kd) *
		     a_dim1], lda, &work[s2pos], &lds2, &c_b1, &work[wpos], &
		    ldw);

	    kgemm_("Conjugate", "No transpose", &pk, &pk, &pn, &c_b2, &work[
		    s2pos], &lds2, &work[wpos], &ldw, &c_b1, &work[s1pos], &
		    lds1);

	    z__1.r = -.5, z__1.i = -0.;
	    kgemm_("No transpose", "No transpose", &pn, &pk, &pk, &z__1, &a[
...
/*            an update of the form:  A := A - V*W' - W*V' */
	    ...
	    z__1.r = -1., z__1.i = -0.;
	    kher2k_(uplo, "No transpose", &pn, &pk, &z__1, &a[i__ + *kd + i__ 
		    * a_dim1], lda, &work[wpos], &ldw, &c_b33, &a[i__ + *kd + 
		    (i__ + *kd) * a_dim1], lda);
/*            ================================================================== */
