...
/*  ===================================================================== */
void  hlauu2_(char *uplo, integer *n, halfreal *a, integer *
	lda, integer *info)
{
...
/*        Compute the product U * U**T. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    if (i__ < *n) {
		i__2 = *n - i__ + 1;
		a[i__ + i__ * a_dim1] = hdot_(&i__2, &a[i__ + i__ * a_dim1], 
			lda, &a[i__ + i__ * a_dim1], lda);
		i__2 = i__ - 1;
		i__3 = *n - i__;
		hgemv_("No transpose", &i__2, &i__3, &c_b7, &a[(i__ + 1) * 
			a_dim1 + 1], lda, &a[i__ + (i__ + 1) * a_dim1], lda, &
			aii, &a[i__ * a_dim1 + 1], &c__1);
	    } else {
		hscal_(&i__, &aii, &a[i__ * a_dim1 + 1], &c__1);
	    }
/* L10: */
	}
...
/*        Compute the product L**T * L. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		i__2 = *n - i__;
		i__3 = i__ - 1;
		hgemv_("Transpose", &i__2, &i__3, &c_b7, &a[i__ + 1 + a_dim1],
			 lda, &a[i__ + 1 + i__ * a_dim1], &c__1, &aii, &a[i__ 
			+ a_dim1], lda);
	    } else {
		hscal_(&i__, &aii, &a[i__ + a_dim1], lda);
	    }
/* L20: */
	}
    }
