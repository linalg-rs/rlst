...
/*  ===================================================================== */
void  hlaqr0_(logical *wantt, logical *wantz, integer *n, 
	integer *ilo, integer *ihi, halfreal *h__, integer *ldh, halfreal 
	*wr, halfreal *wi, integer *iloz, integer *ihiz, halfreal *z__, 
	integer *ldz, halfreal *work, integer *lwork, integer *info)
{
    ...
    extern void  hlanv2_(halfreal *, halfreal *, 
	    ...
	    hlaqr4_(logical *, logical *, integer *, integer *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *, integer *, 
	    integer *, halfreal *, integer *, halfreal *, integer *, 
	    integer *), hlaqr5_(logical *, logical *, integer *, integer *, 
	    integer *, integer *, integer *, halfreal *, halfreal *, 
	    halfreal *, integer *, integer *, integer *, halfreal *, 
	    integer *, halfreal *, integer *, halfreal *, integer *, 
	    ...
	    integer *);
    integer nibble;
    extern void  hlahqr_(logical *, logical *, integer *, 
...
/*        ==== Tiny matrices must use DLAHQR. ==== */
	...
	lwkopt = 1;
	if (*lwork != -1) {
	    hlahqr_(wantt, wantz, n, ilo, ihi, &h__[h_offset], ldh, &wr[1], &
...
/*        ==== Workspace query call to DLAQR3 ==== */
	...
	i__1 = nwr + 1;
	hlaqr3_(wantt, wantz, n, ilo, ihi, &i__1, &h__[h_offset], ldh, iloz, 
...
/*           ==== Aggressive early deflation ==== */
	    ...
	    hlaqr3_(wantt, wantz, n, &ktop, &kbot, &nw, &h__[h_offset], ldh, 
...
/* Computing MAX */
			...
			cc = ss * -.4375;
			dd = aa;
			hlanv2_(&aa, &bb, &cc, &dd, &wr[i__ - 1], &wi[i__ - 1]
				, &wr[i__], &wi[i__], &cs, &sn);
/* L30: */
		    }
		    if (ks == ktop) {
...
/*                 .    to fit an NS-by-NS scratch array.) ==== */
			...
			ks = kbot - ns + 1;
			kt = *n - ns + 1;
			hlacpy_("A", &ns, &ns, &h__[ks + ks * h_dim1], ldh, &
				h__[kt + h_dim1], ldh);
			if (ns > nmin) {
			    hlaqr4_(&c_false, &c_false, &ns, &c__1, &ns, &h__[
				    kt + h_dim1], ldh, &wr[ks], &wi[ks], &
				    c__1, &c__1, zdum, &c__1, &work[1], lwork,
				     &inf);
			} else {
			    hlahqr_(&c_false, &c_false, &ns, &c__1, &ns, &h__[
				    kt + h_dim1], ldh, &wr[ks], &wi[ks], &
...
/*                    .    principal submatrix.  ==== */
			...
			if (ks >= kbot) {
			    ...
			    hlanv2_(&aa, &bb, &cc, &dd, &wr[kbot - 1], &wi[
...
/*              ==== Small-bulge multi-shift QR sweep ==== */
		...
		hlaqr5_(wantt, wantz, &kacc22, n, &ktop, &kbot, &ns, &wr[ks], 
