...
/*  ===================================================================== */
void  kpotrf_(char *uplo, integer *n, halfcomplex *a, 
	integer *lda, integer *info)
{
    ...
    extern void  kpotrf2_(char *, integer *, halfcomplex *, 
	    integer *, integer *);
...
/*        Use unblocked code. */
	...
	kpotrf2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/* Computing MIN */
		...
		jb = f2cmin(i__3,i__4);
		i__3 = j - 1;
		kherk_("Upper", "Conjugate transpose", &jb, &i__3, &c_b14, &a[
			j * a_dim1 + 1], lda, &c_b15, &a[j + j * a_dim1], lda);
		kpotrf2_("Upper", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/*                 Compute the current block row. */
		    ...
		    i__4 = j - 1;
		    z__1.r = -1., z__1.i = -0.;
		    kgemm_("Conjugate transpose", "No transpose", &jb, &i__3, 
			    &i__4, &z__1, &a[j * a_dim1 + 1], lda, &a[(j + jb)
			     * a_dim1 + 1], lda, &c_b1, &a[j + (j + jb) * 
			    a_dim1], lda);
		    i__3 = *n - j - jb + 1;
		    ktrsm_("Left", "Upper", "Conjugate transpose", "Non-unit",
...
/* Computing MIN */
		...
		jb = f2cmin(i__3,i__4);
		i__3 = j - 1;
		kherk_("Lower", "No transpose", &jb, &i__3, &c_b14, &a[j + 
			a_dim1], lda, &c_b15, &a[j + j * a_dim1], lda);
		kpotrf2_("Lower", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/*                 Compute the current block column. */
		    ...
		    i__4 = j - 1;
		    z__1.r = -1., z__1.i = -0.;
		    kgemm_("No transpose", "Conjugate transpose", &i__3, &jb, 
			    &i__4, &z__1, &a[j + jb + a_dim1], lda, &a[j + 
			    a_dim1], lda, &c_b1, &a[j + jb + j * a_dim1], lda);
		    i__3 = *n - j - jb + 1;
		    ktrsm_("Right", "Lower", "Conjugate transpose", "Non-unit"
