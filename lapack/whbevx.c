...
/*  ===================================================================== */
void  whbevx_(char *jobz, char *range, char *uplo, integer *n, 
	integer *kd, quadcomplex *ab, integer *ldab, quadcomplex *q, 
	integer *ldq, quadreal *vl, quadreal *vu, integer *il, integer *
	iu, quadreal *abstol, integer *m, quadreal *w, quadcomplex *z__,
	 integer *ldz, quadcomplex *work, quadreal *rwork, integer *iwork,
	 integer *ifail, integer *info)
{
    ...
    extern void  qsterf_(integer *, quadreal *, quadreal *,
	     integer *), wlascl_(char *, integer *, integer *, quadreal *, 
    ...
    extern void  wstein_(integer *, quadreal *, quadreal *,
	    ...
	    wsteqr_(char *, integer *, quadreal *, quadreal *, 
	    quadcomplex *, integer *, quadreal *, integer *);
...
/*     Scale matrix to allowable range, if necessary. */
    ...
    anrm = wlanhb_("M", uplo, n, kd, &ab[ab_offset], ldab, &rwork[1]);
    if (anrm > 0. && anrm < rmin) {
	iscale = 1;
	sigma = rmin / anrm;
    } else if (anrm > rmax) {
    ...
    if (iscale == 1) {
	if (lower) {
	    wlascl_("B", kd, kd, &c_b16, &sigma, n, n, &ab[ab_offset], ldab, 
		    info);
	} else {
	    wlascl_("Q", kd, kd, &c_b16, &sigma, n, n, &ab[ab_offset], ldab, 
		    info);
	}
	if (*abstol > 0.) {
	    abstll = *abstol * sigma;
...
/*     Call ZHBTRD to reduce Hermitian band matrix to tridiagonal form. */
    ...
    indrwk = inde + *n;
    indwrk = 1;
    whbtrd_(jobz, uplo, n, kd, &ab[ab_offset], ldab, &rwork[indd], &rwork[
