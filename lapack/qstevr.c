...
/* > */
/*  ===================================================================== */
void  qstevr_(char *jobz, char *range, integer *n, quadreal *
	...
	integer *lwork, integer *iwork, integer *liwork, integer *info)
{
    ...
    extern void  qstebz_(char *, char *, integer *, quadreal 
	    *, quadreal *, integer *, integer *, quadreal *, quadreal *,
	    ...
	    qstemr_(char *, char *, integer *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, integer *, integer *, integer *, 
	    quadreal *, quadreal *, integer *, integer *, integer *, 
	    logical *, quadreal *, integer *, integer *, integer *, integer 
	    *);
    integer liwmin;
    logical tryrac;
    integer nsplit;
...
/*     try DSTEBZ. */
	...
	if (! wantz) {
	    qcopy_(n, &d__[1], &c__1, &w[1], &c__1);
	    qsterf_(n, &w[1], &work[1], info);
	} else {
	    qcopy_(n, &d__[1], &c__1, &work[*n + 1], &c__1);
	    if (*abstol <= *n * 2. * eps) {
		tryrac = TRUE_;
	    } else {
		tryrac = FALSE_;
	    ...
	    i__1 = *lwork - (*n << 1);
	    qstemr_(jobz, "A", n, &work[*n + 1], &work[1], vl, vu, il, iu, m, 
...
/*     Otherwise, call DSTEBZ and, if eigenvectors are desired, DSTEIN. */
	...
	*(unsigned char *)order = 'E';
    ...
    qstebz_(range, order, n, &vll, &vuu, il, iu, abstol, &d__[1], &e[1], m, &
	    nsplit, &w[1], &iwork[indibl], &iwork[indisp], &work[1], &iwork[
	    indiwo], info);

    if (wantz) {
	qstein_(n, &d__[1], &e[1], m, &w[1], &iwork[indibl], &iwork[indisp], &
