...
/*  ===================================================================== */
void  qlarrb_(integer *n, quadreal *d__, quadreal *lld, 
	integer *ifirst, integer *ilast, quadreal *rtol1, quadreal *rtol2,
	 integer *offset, quadreal *w, quadreal *wgap, quadreal *werr, 
	quadreal *work, integer *iwork, quadreal *pivmin, quadreal *
	spdiam, integer *twist, integer *info)
{
    ...
    quadreal gap, mid, tmp, back, lgap, rgap, left;
    integer iter, nint, prev, next;
    quadreal cvrgd, right, width;
    extern integer qlaneg_(integer *, quadreal *, quadreal *, quadreal *
	    , quadreal *, integer *);
    integer negcnt;
    quadreal mnwdth;
...
/*     Quick return if possible */
    ...
    maxitr = (integer) ((M(log)(*spdiam + *pivmin) - M(log)(*pivmin)) / M(log)(2.)) + 
	    2;
    mnwdth = *pivmin * 2.;
...
/*        Do while( NEGCNT(LEFT).GT.I-1 ) */
	...
	back = werr[ii];
L20:
	negcnt = qlaneg_(n, &d__[1], &lld[1], &left, pivmin, &r__);
	if (negcnt > i__ - 1) {
	    left -= back;
	    back *= 2.;
	    goto L20;
...
/*        Compute negcount from dstqds facto L+D+L+^T = L D L^T - RIGHT */
	...
	back = werr[ii];
L50:
	negcnt = qlaneg_(n, &d__[1], &lld[1], &right, pivmin, &r__);
	if (negcnt < i__) {
	    right += back;
	    back *= 2.;
	    goto L50;
...
/*        Perform one bisection step */
	...
	negcnt = qlaneg_(n, &d__[1], &lld[1], &mid, pivmin, &r__);
	if (negcnt <= i__ - 1) {
	    work[k - 1] = mid;
	} else {
	    work[k] = mid;
