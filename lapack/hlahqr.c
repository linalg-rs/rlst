...
/* >          WI(i) = M(sqrt)(H(i+1,i)*H(i,i+1)) and WI(i+1) = -WI(i). */
/* > \endverbatim */
/* > */
/* > \param[in] ILOZ */
/* > \verbatim */
...
/* > */
/*  ===================================================================== */
void  hlahqr_(logical *wantt, logical *wantz, integer *n, 
	...
	integer *ldz, integer *info)
{
    ...
    halfreal s, v[3];
    integer i1, i2;
    halfreal t1, t2, t3, v2, v3, aa, ab, ba, bb, h11, h12, h21, h22, cs;
    integer nh;
    halfreal sn;
    integer nr;
    halfreal tr;
    ...
    halfreal det, h21s;
    integer its;
    halfreal ulp, sum, tst, rt1i, rt2i, rt1r, rt2r;
    extern void  hrot_(integer *, halfreal *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *), hcopy_(
    ...
    extern void  hlarfg_(integer *, halfreal *, halfreal *,
	     integer *, halfreal *);
    halfreal safmin, safmax, rtdisc, smlnum;
...
/*              ==== doublereal shifts (use only one of them)  ==== */
...
/* Computing MIN */
	    ...
	    nr = f2cmin(i__3,i__4);
	    if (k > m) {
		hcopy_(&nr, &h__[k + (k - 1) * h_dim1], &c__1, v, &c__1);
	    }
	    hlarfg_(&nr, v, &v[1], &c__1, &t1);
	    if (k > m) {
...
/*        and compute and store the eigenvalues. */
	...
	hlanv2_(&h__[i__ - 1 + (i__ - 1) * h_dim1], &h__[i__ - 1 + i__ * 
...
/*           Apply the transformation to the rest of H. */
	    ...
	    if (i2 > i__) {
		i__1 = i2 - i__;
		hrot_(&i__1, &h__[i__ - 1 + (i__ + 1) * h_dim1], ldh, &h__[
			i__ + (i__ + 1) * h_dim1], ldh, &cs, &sn);
	    }
	    i__1 = i__ - i1 - 1;
	    hrot_(&i__1, &h__[i1 + (i__ - 1) * h_dim1], &c__1, &h__[i1 + i__ *
		     h_dim1], &c__1, &cs, &sn);
	}
	if (*wantz) {
...
/*           Apply the transformation to Z. */
	    ...
	    hrot_(&nz, &z__[*iloz + (i__ - 1) * z_dim1], &c__1, &z__[*iloz + 
