...
/*  ===================================================================== */
void  qlasy2_(logical *ltranl, logical *ltranr, integer *isgn, 
	integer *n1, integer *n2, quadreal *tl, integer *ldtl, quadreal *
	tr, integer *ldtr, quadreal *b, integer *ldb, quadreal *scale, 
	quadreal *x, integer *ldx, quadreal *xnorm, integer *info)
{
    /* Initialized data */
    ...
    /* Local variables */
    integer i__, j, k;
    quadreal x2[2], l21, u11, u12;
    integer ip, jp;
    quadreal u22, t16[16]	/* was [4][4] */, gam, bet, eps, sgn, tmp[4], 
	    tau1, btmp[4], smin;
    integer ipiv;
    quadreal temp;
    integer jpiv[4];
...
/* L70: */
	...
	if (ipsv != i__) {
	    qswap_(&c__4, &t16[ipsv - 1], &c__4, &t16[i__ - 1], &c__4);
	    temp = btmp[i__ - 1];
	    btmp[i__ - 1] = btmp[ipsv - 1];
	    btmp[ipsv - 1] = temp;
	}
	if (jpsv != i__) {
	    qswap_(&c__4, &t16[(jpsv << 2) - 4], &c__1, &t16[(i__ << 2) - 4], 
		    &c__1);
	}
	jpiv[i__ - 1] = jpsv;
