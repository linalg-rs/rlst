...
/* > */
/*  ===================================================================== */
void  wgghd3_(char *compq, char *compz, integer *n, integer *
	ilo, integer *ihi, quadcomplex *a, integer *lda, quadcomplex *b, 
	integer *ldb, quadcomplex *q, integer *ldq, quadcomplex *z__, 
	integer *ldz, quadcomplex *work, integer *lwork, integer *info)
{
    ...
    extern void  wgemv_(char *, integer *, integer *, 
	    ...
	    integer *, quadcomplex *, quadcomplex *, integer *);
    logical wantq, initz;
    extern void  wunm22_(char *, char *, integer *, integer *, 
	    integer *, integer *, quadcomplex *, integer *, quadcomplex *,
	     integer *, quadcomplex *, integer *, integer *)
	    ;
    logical wantz;
    extern void  wtrmv_(char *, char *, char *, integer *, 
    ...
    extern void  wgghrd_(char *, char *, integer *, integer *, 
	    ...
	    ), wlaset_(char *, integer *, integer *, 
	    quadcomplex *, quadcomplex *, quadcomplex *, integer *), wlartg_(quadcomplex *, quadcomplex *, quadreal *, 
	    quadcomplex *, quadcomplex *), wlacpy_(char *, integer *, 
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *);
    integer lwkopt;
    logical lquery;
...
/*     Initialize Q and Z if desired. */
    ...
    if (initq) {
	wlaset_("All", n, n, &c_b2, &c_b1, &q[q_offset], ldq);
    }
    if (initz) {
	wlaset_("All", n, n, &c_b2, &c_b1, &z__[z_offset], ldz);
    }
...
/*     Zero out lower triangle of B. */
	...
	i__1 = *n - 1;
	i__2 = *n - 1;
	wlaset_("Lower", &i__1, &i__2, &c_b2, &c_b2, &b[b_dim1 + 2], ldb);
    }
...
/*                  factor. */
	    ...
	    wlaset_("All", &nblst, &nblst, &c_b2, &c_b1, &work[1], &nblst);
	    pw = nblst * nblst + 1;
	    i__3 = n2nb;
		...
		i__5 = nnb << 1;
		i__6 = nnb << 1;
		wlaset_("All", &i__4, &i__5, &c_b2, &c_b1, &work[pw], &i__6);
		pw += (nnb << 2) * nnb;
	    }
...
/*              column of A and B, respectively. */
		...
		for (i__ = *ihi; i__ >= i__4; --i__) {
		    ...
		    wlartg_(&temp, &a[i__ + j * a_dim1], &c__, &s, &a[i__ - 1 
			    + j * a_dim1]);
		    i__5 = i__ + j * a_dim1;
		    z__1.r = c__, z__1.i = 0.;
...
/*                 Annihilate B( JJ+1, JJ ). */
			...
			wlartg_(&temp, &b[jj + 1 + jj * b_dim1], &c__, &s, &b[
				jj + 1 + (jj + 1) * b_dim1]);
			i__6 = jj + 1 + jj * b_dim1;
			b[i__6].r = 0., b[i__6].i = 0.;
			i__6 = jj - top;
			wrot_(&i__6, &b[top + 1 + (jj + 1) * b_dim1], &c__1, &
...
/*              Update A by transformations from right. */
		...
		if (jj > 0) {
			...
			z__1.r = -z__2.r, z__1.i = -z__2.i;
			wrot_(&i__5, &a[top + 1 + (j + i__ + 1) * a_dim1], &
...
/*                 triangular. */
		    ...
		    jrow = *ihi - nblst + 1;
		    wgemv_("Conjugate", &nblst, &len, &c_b1, &work[1], &nblst,
		    ...
		    i__5 = nblst - len;
		    wtrmv_("Lower", "Conjugate", "Non-unit", &i__5, &work[len 
			    * nblst + 1], &nblst, &work[pw + len], &c__1);
		    i__5 = nblst - len;
		    wgemv_("Conjugate", &len, &i__5, &c_b1, &work[(len + 1) * 
...
/*                 and U12 is an NNB-by-NNB lower triangular matrix. */
			...
			i__4 = nnb << 1;
			wtrmv_("Upper", "Conjugate", "Non-unit", &len, &work[
				ppwo + nnb], &i__4, &work[pw], &c__1);
			i__4 = nnb << 1;
			wtrmv_("Lower", "Conjugate", "Non-unit", &nnb, &work[
				ppwo + (len << 1) * nnb], &i__4, &work[pw + 
				len], &c__1);
			i__4 = nnb << 1;
			wgemv_("Conjugate", &nnb, &len, &c_b1, &work[ppwo], &
				i__4, &a[jrow + (j + 1) * a_dim1], &c__1, &
				c_b1, &work[pw], &c__1);
			i__4 = nnb << 1;
			wgemv_("Conjugate", &len, &nnb, &c_b1, &work[ppwo + (
...
/*           Apply accumulated unitary matrices to A. */
	    ...
	    cola = *n - jcol - nnb + 1;
	    j = *ihi - nblst + 1;
	    wgemm_("Conjugate", "No Transpose", &nblst, &cola, &nblst, &c_b1, 
		    &work[1], &nblst, &a[j + (jcol + nnb) * a_dim1], lda, &
		    c_b2, &work[pw], &nblst);
	    wlacpy_("All", &nblst, &cola, &work[pw], &nblst, &a[j + (jcol + 
		    nnb) * a_dim1], lda);
	    ppwo = nblst * nblst + 1;
	    j0 = j - nnb;
	    i__3 = jcol + 1;
...
/*                 triangular and U12 is lower triangular. */
		    ...
		    i__4 = nnb << 1;
		    i__7 = *lwork - pw + 1;
		    wunm22_("Left", "Conjugate", &i__5, &cola, &nnb, &nnb, &
...
/*                 Ignore the structure of U. */
		    ...
		    i__7 = nnb << 1;
		    i__8 = nnb << 1;
		    wgemm_("Conjugate", "No Transpose", &i__5, &cola, &i__4, &
			    c_b1, &work[ppwo], &i__7, &a[j + (jcol + nnb) * 
			    a_dim1], lda, &c_b2, &work[pw], &i__8);
		    i__5 = nnb << 1;
		    i__4 = nnb << 1;
		    wlacpy_("All", &i__5, &cola, &work[pw], &i__4, &a[j + (
			    jcol + nnb) * a_dim1], lda);
		}
		ppwo += (nnb << 2) * nnb;
	    }
...
/* Computing MAX */
		...
		wgemm_("No Transpose", "No Transpose", &nh, &nblst, &nblst, &
			c_b1, &q[topq + j * q_dim1], ldq, &work[1], &nblst, &
			c_b2, &work[pw], &nh);
		wlacpy_("All", &nh, &nblst, &work[pw], &nh, &q[topq + j * 
			q_dim1], ldq);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__6 = jcol + 1;
...
/*                    Exploit the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = *lwork - pw + 1;
			wunm22_("Right", "No Transpose", &nh, &i__5, &nnb, &
...
/*                    Ignore the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = nnb << 1;
			wgemm_("No Transpose", "No Transpose", &nh, &i__5, &
				i__4, &c_b1, &q[topq + j * q_dim1], ldq, &
				work[ppwo], &i__7, &c_b2, &work[pw], &nh);
			i__5 = nnb << 1;
			wlacpy_("All", &nh, &i__5, &work[pw], &nh, &q[topq + 
				j * q_dim1], ldq);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
...
/*              accumulated Givens rotations in workspace. */
		...
		for (i__ = 1; i__ <= i__3; ++i__) {
		    ...
		    i__5 = nnb << 1;
		    i__4 = nnb << 1;
		    wlaset_("All", &i__6, &i__5, &c_b2, &c_b1, &work[pw], &
			    i__4);
		    pw += (nnb << 2) * nnb;
		}
...
/*              Accumulate Givens rotations into workspace array. */
		...
		i__3 = *ihi - jcol - 1;
		wlaset_("Lower", &i__3, &nnb, &c_b2, &c_b2, &a[jcol + 2 + 
			jcol * a_dim1], lda);
		i__3 = *ihi - jcol - 1;
		wlaset_("Lower", &i__3, &nnb, &c_b2, &c_b2, &b[jcol + 2 + 
			jcol * b_dim1], ldb);
	    }
...
/*           Apply accumulated unitary matrices to A and B. */
	    ...
	    if (top > 0) {
		j = *ihi - nblst + 1;
		wgemm_("No Transpose", "No Transpose", &top, &nblst, &nblst, &
			c_b1, &a[j * a_dim1 + 1], lda, &work[1], &nblst, &
			c_b2, &work[pw], &top);
		wlacpy_("All", &top, &nblst, &work[pw], &top, &a[j * a_dim1 + 
			1], lda);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__3 = jcol + 1;
...
/*                    Exploit the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = *lwork - pw + 1;
			wunm22_("Right", "No Transpose", &top, &i__6, &nnb, &
...
/*                    Ignore the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = nnb << 1;
			wgemm_("No Transpose", "No Transpose", &top, &i__6, &
				i__4, &c_b1, &a[j * a_dim1 + 1], lda, &work[
				ppwo], &i__7, &c_b2, &work[pw], &top);
			i__6 = nnb << 1;
			wlacpy_("All", &top, &i__6, &work[pw], &top, &a[j * 
				a_dim1 + 1], lda);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
		...
		wgemm_("No Transpose", "No Transpose", &top, &nblst, &nblst, &
			...
			c_b2, &work[pw], &top);
		wlacpy_("All", &top, &nblst, &work[pw], &top, &b[j * b_dim1 + 
			1], ldb);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__5 = jcol + 1;
...
/*                    Ignore the structure of U. */
			...
			wgemm_("No Transpose", "No Transpose", &top, &i__6, &
				...
				ppwo], &i__7, &c_b2, &work[pw], &top);
			i__6 = nnb << 1;
			wlacpy_("All", &top, &i__6, &work[pw], &top, &b[j * 
				b_dim1 + 1], ldb);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
...
/* Computing MAX */
		...
		wgemm_("No Transpose", "No Transpose", &nh, &nblst, &nblst, &
			...
			&c_b2, &work[pw], &nh);
		wlacpy_("All", &nh, &nblst, &work[pw], &nh, &z__[topq + j * 
			z_dim1], ldz);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__3 = jcol + 1;
...
/*                    Exploit the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = *lwork - pw + 1;
			wunm22_("Right", "No Transpose", &nh, &i__6, &nnb, &
...
/*                    Ignore the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = nnb << 1;
			wgemm_("No Transpose", "No Transpose", &nh, &i__6, &
				i__4, &c_b1, &z__[topq + j * z_dim1], ldz, &
				work[ppwo], &i__7, &c_b2, &work[pw], &nh);
			i__6 = nnb << 1;
			wlacpy_("All", &nh, &i__6, &work[pw], &nh, &z__[topq 
				+ j * z_dim1], ldz);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
...
/*     Avoid re-initialization of modified Q and Z. */
    ...
    *(unsigned char *)compz2 = *(unsigned char *)compz;
    ...
    if (jcol < *ihi) {
	wgghrd_(compq2, compz2, n, &jcol, ihi, &a[a_offset], lda, &b[b_offset]
