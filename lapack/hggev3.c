...
/* > \verbatim */
/* > */
/* > DGGEV3 computes for a pair of N-by-N doublereal nonsymmetric matrices (A,B) */
...
/*  ===================================================================== */
void  hggev3_(char *jobvl, char *jobvr, integer *n, halfreal 
	*a, integer *lda, halfreal *b, integer *ldb, halfreal *alphar, 
	halfreal *alphai, halfreal *beta, halfreal *vl, integer *ldvl, 
	halfreal *vr, integer *ldvr, halfreal *work, integer *lwork, 
	integer *info)
{
    ...
    extern void  hgeqrf_(integer *, integer *, halfreal *, 
	    ...
	    integer *, halfreal *, halfreal *, halfreal *, integer *), htgevc_(char *, char *, logical *, integer *, halfreal 
	    *, integer *, halfreal *, integer *, halfreal *, integer *, 
	    halfreal *, integer *, integer *, integer *, halfreal *, 
	    integer *);
    logical ldumma[1];
...
/* Computing MAX */
	...
	lwkopt = f2cmax(i__1,i__2);
	if (ilvl) {
	    horgqr_(n, n, n, &vl[vl_offset], ldvl, &work[1], &work[1], &c_n1, 
		    &ierr);
/* Computing MAX */
	    i__1 = lwkopt, i__2 = *n * 3 + (integer) work[1];
	    lwkopt = f2cmax(i__1,i__2);
	...
	if (ilv) {
	    hgghd3_(jobvl, jobvr, n, &c__1, n, &a[a_offset], lda, &b[b_offset]
		    , ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &work[
		    1], &c_n1, &ierr);
/* Computing MAX */
	    i__1 = lwkopt, i__2 = *n * 3 + (integer) work[1];
	    lwkopt = f2cmax(i__1,i__2);
	    hhgeqz_("S", jobvl, jobvr, n, &c__1, n, &a[a_offset], lda, &b[
...
/* Computing MAX */
	    ...
	    hgghd3_("N", "N", n, &c__1, n, &a[a_offset], lda, &b[b_offset], 
		    ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &work[1],
		     &c_n1, &ierr);
/* Computing MAX */
	    i__1 = lwkopt, i__2 = *n * 3 + (integer) work[1];
	    lwkopt = f2cmax(i__1,i__2);
	    hhgeqz_("E", jobvl, jobvr, n, &c__1, n, &a[a_offset], lda, &b[
		    b_offset], ldb, &alphar[1], &alphai[1], &beta[1], &vl[
		    vl_offset], ldvl, &vr[vr_offset], ldvr, &work[1], &c_n1, &
		    ierr);
/* Computing MAX */
...
/*     Initialize VL */
    ...
    if (ilvl) {
	hlaset_("Full", n, n, &c_b38, &c_b39, &vl[vl_offset], ldvl)
		;
	if (irows > 1) {
	    i__1 = irows - 1;
	    i__2 = irows - 1;
	...
	i__1 = *lwork + 1 - iwrk;
	horgqr_(&irows, &irows, &irows, &vl[ilo + ilo * vl_dim1], ldvl, &work[
...
/*     Initialize VR */
    ...
    if (ilvr) {
	hlaset_("Full", n, n, &c_b38, &c_b39, &vr[vr_offset], ldvr)
		;
    }
...
/*        Eigenvectors requested -- work on whole matrix. */
	...
	i__1 = *lwork + 1 - iwrk;
	hgghd3_(jobvl, jobvr, n, &ilo, &ihi, &a[a_offset], lda, &b[b_offset], 
		ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &work[iwrk], 
		&i__1, &ierr);
    } else {
	i__1 = *lwork + 1 - iwrk;
	hgghd3_("N", "N", &irows, &c__1, &irows, &a[ilo + ilo * a_dim1], lda, 
...
/*     Schur forms and Schur vectors) */
	...
	*(unsigned char *)chtemp = 'E';
    ...
    i__1 = *lwork + 1 - iwrk;
    hhgeqz_(chtemp, jobvl, jobvr, n, &ilo, &ihi, &a[a_offset], lda, &b[
...
/*     Compute Eigenvectors */
	...
	htgevc_(chtemp, "B", ldumma, n, &a[a_offset], lda, &b[b_offset], ldb, 
...
/*        Undo balancing on VL and VR and normalization */
	...
	if (ilvl) {
	    hggbak_("P", "L", n, &ilo, &ihi, &work[ileft], &work[iright], n, &
...
L50:
	...
	if (ilvr) {
	    hggbak_("P", "R", n, &ilo, &ihi, &work[ileft], &work[iright], n, &
