...
/*  ===================================================================== */
void  wlahef_(char *uplo, integer *n, integer *nb, integer *kb,
	 quadcomplex *a, integer *lda, integer *ipiv, quadcomplex *w, 
...
/*        Copy column K of A to column KW of W and update it */
	...
	i__1 = k - 1;
	wcopy_(&i__1, &a[k * a_dim1 + 1], &c__1, &w[kw * w_dim1 + 1], &c__1);
	i__1 = k + kw * w_dim1;
	i__2 = k + k * a_dim1;
	d__1 = a[i__2].r;
...
/*              Determine only ROWMAX. */
		...
		i__1 = k - imax;
		jmax = imax + iwamax_(&i__1, &w[imax + 1 + (kw - 1) * w_dim1],
			 &c__1);
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    jmax = iwamax_(&i__1, &w[(kw - 1) * w_dim1 + 1], &c__1);
/* Computing MAX */
		    i__1 = jmax + (kw - 1) * w_dim1;
...
/*           Update the rectangular superdiagonal block */
	    ...
	    i__3 = *n - k;
	    z__1.r = -1., z__1.i = -0.;
	    wgemm_("No transpose", "Transpose", &i__2, &jb, &i__3, &z__1, &a[(
...
/*           of the rows to swap back doesn't include diagonal element) */
	...
	if (jp != jj && j <= *n) {
	    i__1 = *n - j + 1;
	    wswap_(&i__1, &a[jp + j * a_dim1], lda, &a[jj + j * a_dim1], lda);
	}
	if (j < *n) {
	    goto L60;
	}
...
/*        Copy column K of A to column K of W and update it */
	...
	i__2 = k - 1;
	z__1.r = -1., z__1.i = -0.;
	wgemv_("No transpose", &i__1, &i__2, &z__1, &a[k + a_dim1], lda, &w[k 
...
/*              Determine only ROWMAX. */
		...
		i__1 = imax - k;
		jmax = k - 1 + iwamax_(&i__1, &w[k + (k + 1) * w_dim1], &c__1)
			;
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    jmax = imax + iwamax_(&i__1, &w[imax + 1 + (k + 1) * 
			    w_dim1], &c__1);
/* Computing MAX */
		    i__1 = jmax + (k + 1) * w_dim1;
		    d__3 = rowmax, d__4 = (d__1 = w[i__1].r, abs(d__1)) + (
...
/*                 copy column K+1 of W to column K of W */
			...
			i__1 = *n - k + 1;
			wcopy_(&i__1, &w[k + (k + 1) * w_dim1], &c__1, &w[k + 
				k * w_dim1], &c__1);
...
/*           of the rows to swap back doesn't include diagonal element) */
	--j;
	if (jp != jj && j >= 1) {
	    wswap_(&j, &a[jp + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	if (j > 1) {
	    goto L120;
	}
