...
/* > */
/*  ===================================================================== */
void  wgemqr_(char *side, char *trans, integer *m, integer *n, 
	...
	integer *lwork, integer *info)
{
    ...
    /* Local variables */
    extern void  wlamtsqr_(char *, char *, integer *, integer *
...
/* Computing MAX */
    ...
    if (left && *m <= *k || right && *n <= *k || mb <= *k || mb >= f2cmax(i__1,*
	    k)) {
	wgemqrt_(side, trans, m, n, k, &nb, &a[a_offset], lda, &t[6], &nb, &
		c__[c_offset], ldc, &work[1], info);
    } else {
	wlamtsqr_(side, trans, m, n, k, &mb, &nb, &a[a_offset], lda, &t[6], &
