...
/*  ===================================================================== */
void  khptrf_(char *uplo, integer *n, halfcomplex *ap, 
	integer *ipiv, integer *info)
{
    ...
    integer imax, jmax;
    extern void  khpr_(char *, integer *, halfreal *, 
...
/*              submatrix A(1:k,1:k) */
		...
		i__1 = kp - 1;
		kswap_(&i__1, &ap[knc], &c__1, &ap[kpc], &c__1);
		kx = kpc + kp - 1;
		i__1 = kk - 1;
		for (j = kp + 1; j <= i__1; ++j) {
		    kx = kx + j - 1;
...
/*              A := A - U(k)*D(k)*U(k)**H = A - W(k)*1/D(k)*W(k)**H */
		...
		i__1 = k - 1;
		d__1 = -r1;
		khpr_(uplo, &i__1, &d__1, &ap[kc], &c__1, &ap[1]);

/*              Store U(k) in column k */

		i__1 = k - 1;
		whscal_(&i__1, &r1, &ap[kc], &c__1);
	    } else {
...
/*        column K, and COLMAX is its absolute value */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + ikamax_(&i__1, &ap[kc + 1], &c__1);
	    i__1 = kc + imax - k;
	    colmax = (d__1 = ap[i__1].r, abs(d__1)) + (d__2 = d_imag(&ap[kc + 
		    imax - k]), abs(d__2));
	} else {
...
/* L70: */
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    jmax = imax + ikamax_(&i__1, &ap[kpc + 1], &c__1);
/* Computing MAX */
		    i__1 = kpc + jmax - imax;
		    d__3 = rowmax, d__4 = (d__1 = ap[i__1].r, abs(d__1)) + (
...
/*              submatrix A(k:n,k:n) */
		...
		if (kp < *n) {
		    i__1 = *n - kp;
		    kswap_(&i__1, &ap[knc + kp - kk + 1], &c__1, &ap[kpc + 1],
			     &c__1);
		}
		kx = knc + kp - kk;
		i__1 = kp - 1;
...
/*                 A := A - L(k)*D(k)*L(k)**H = A - W(k)*(1/D(k))*W(k)**H */
		    ...
		    i__1 = *n - k;
		    d__1 = -r1;
		    khpr_(uplo, &i__1, &d__1, &ap[kc + 1], &c__1, &ap[kc + *n 
			    - k + 1]);

/*                 Store L(k) in column K */

		    i__1 = *n - k;
		    whscal_(&i__1, &r1, &ap[kc + 1], &c__1);
		}
	    } else {
