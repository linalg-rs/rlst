...
/*  ===================================================================== */
void  qtgsy2_(char *trans, integer *ijob, integer *m, integer *
	n, quadreal *a, integer *lda, quadreal *b, integer *ldb, 
	quadreal *c__, integer *ldc, quadreal *d__, integer *ldd, 
	quadreal *e, integer *lde, quadreal *f, integer *ldf, quadreal *
	scale, quadreal *rdsum, quadreal *rdscal, integer *iwork, integer 
	*pq, integer *info)
{
    ...
    extern void  qgemv_(char *, integer *, integer *, 
	    ...
	    *, quadreal *, quadreal *, integer *, quadreal *, integer *)
	    , qgesc2_(integer *, quadreal *, integer *, quadreal *, 
	    integer *, integer *, quadreal *), qgetc2_(integer *, 
	    quadreal *, integer *, integer *, integer *, integer *), 
	    qlatdf_(integer *, integer *, quadreal *, integer *, quadreal 
...
/*                 Solve Z * x = RHS */
		    ...
		    qgetc2_(&zdim, z__, &c__8, ipiv, jpiv, &ierr);
		    if (ierr > 0) {
			*info = ierr;
		    }

		    if (*ijob == 0) {
			qgesc2_(&zdim, z__, &c__8, rhs, ipiv, jpiv, &scaloc);
			if (scaloc != 1.) {
			    i__2 = *n;
			    for (k = 1; k <= i__2; ++k) {
				qscal_(m, &scaloc, &c__[k * c_dim1 + 1], &
					c__1);
				qscal_(m, &scaloc, &f[k * f_dim1 + 1], &c__1);
/* L50: */
			    }
			    *scale *= scaloc;
			}
		    } else {
			qlatdf_(ijob, &zdim, z__, &c__8, rhs, rdsum, rdscal, 
				ipiv, jpiv);
		    }
...
/*                 equation. */
			...
			alpha = -rhs[0];
			i__2 = is - 1;
			qaxpy_(&i__2, &alpha, &a[is * a_dim1 + 1], &c__1, &
				c__[js * c_dim1 + 1], &c__1);
			i__2 = is - 1;
			qaxpy_(&i__2, &alpha, &d__[is * d_dim1 + 1], &c__1, &
				f[js * f_dim1 + 1], &c__1);
		    }
		    if (j < q) {
			i__2 = *n - je;
			qaxpy_(&i__2, &rhs[1], &b[js + (je + 1) * b_dim1], 
				ldb, &c__[is + (je + 1) * c_dim1], ldc);
			i__2 = *n - je;
			qaxpy_(&i__2, &rhs[1], &e[js + (je + 1) * e_dim1], 
...
/*                 equation. */
		    ...
		    if (i__ > 1) {
			i__2 = is - 1;
			qger_(&i__2, &nb, &c_b27, &a[is * a_dim1 + 1], &c__1, 
				rhs, &c__1, &c__[js * c_dim1 + 1], ldc);
			i__2 = is - 1;
			qger_(&i__2, &nb, &c_b27, &d__[is * d_dim1 + 1], &
				c__1, rhs, &c__1, &f[js * f_dim1 + 1], ldf);
		    }
		    if (j < q) {
			i__2 = *n - je;
			qaxpy_(&i__2, &rhs[2], &b[js + (je + 1) * b_dim1], 
				ldb, &c__[is + (je + 1) * c_dim1], ldc);
			i__2 = *n - je;
			qaxpy_(&i__2, &rhs[2], &e[js + (je + 1) * e_dim1], 
				lde, &f[is + (je + 1) * f_dim1], ldf);
			i__2 = *n - je;
			qaxpy_(&i__2, &rhs[3], &b[jsp1 + (je + 1) * b_dim1], 
				ldb, &c__[is + (je + 1) * c_dim1], ldc);
			i__2 = *n - je;
			qaxpy_(&i__2, &rhs[3], &e[jsp1 + (je + 1) * e_dim1], 
				lde, &f[is + (je + 1) * f_dim1], ldf);
		    }
...
/*                 equation. */
		    ...
		    if (i__ > 1) {
			i__2 = is - 1;
			qgemv_("N", &i__2, &mb, &c_b27, &a[is * a_dim1 + 1], 
				lda, rhs, &c__1, &c_b42, &c__[js * c_dim1 + 1]
				, &c__1);
			i__2 = is - 1;
			qgemv_("N", &i__2, &mb, &c_b27, &d__[is * d_dim1 + 1],
				 ldd, rhs, &c__1, &c_b42, &f[js * f_dim1 + 1],
				 &c__1);
		    }
		    if (j < q) {
			i__2 = *n - je;
			qger_(&mb, &i__2, &c_b42, &rhs[2], &c__1, &b[js + (je 
				+ 1) * b_dim1], ldb, &c__[is + (je + 1) * 
				c_dim1], ldc);
			i__2 = *n - je;
			qger_(&mb, &i__2, &c_b42, &rhs[2], &c__1, &e[js + (je 
...
/*                 Build an 8-by-8 system Z * x = RHS */
		    ...
		    qlaset_("F", &c__8, &c__8, &c_b56, &c_b56, z__, &c__8);
...
/*                 Set up right hand side(s) */
		    ...
		    i__2 = nb - 1;
		    for (jj = 0; jj <= i__2; ++jj) {
			qcopy_(&mb, &c__[is + (js + jj) * c_dim1], &c__1, &
				rhs[k - 1], &c__1);
			qcopy_(&mb, &f[is + (js + jj) * f_dim1], &c__1, &rhs[
				ii - 1], &c__1);
			k += mb;
			ii += mb;
/* L80: */
...
/*                 Unpack solution vector(s) */
		    ...
		    i__2 = nb - 1;
		    for (jj = 0; jj <= i__2; ++jj) {
			qcopy_(&mb, &rhs[k - 1], &c__1, &c__[is + (js + jj) * 
				c_dim1], &c__1);
			qcopy_(&mb, &rhs[ii - 1], &c__1, &f[is + (js + jj) * 
				f_dim1], &c__1);
			k += mb;
			ii += mb;
/* L100: */
...
/*                 equation. */
		    ...
		    if (i__ > 1) {
			i__2 = is - 1;
			qgemm_("N", "N", &i__2, &nb, &mb, &c_b27, &a[is * 
				a_dim1 + 1], lda, rhs, &mb, &c_b42, &c__[js * 
				c_dim1 + 1], ldc);
			i__2 = is - 1;
			qgemm_("N", "N", &i__2, &nb, &mb, &c_b27, &d__[is * 
			...
			k = mb * nb + 1;
			i__2 = *n - je;
			qgemm_("N", "N", &mb, &i__2, &nb, &c_b42, &rhs[k - 1],
...
/*                 Solve Z**T * x = RHS */
			...
			*info = ierr;
			...
			i__3 = *n;
			for (k = 1; k <= i__3; ++k) {
			    qscal_(m, &scaloc, &c__[k * c_dim1 + 1], &c__1);
			    qscal_(m, &scaloc, &f[k * f_dim1 + 1], &c__1);
/* L130: */
			}
			*scale *= scaloc;
...
/*                 equation. */
			...
			alpha = rhs[0];
			i__3 = js - 1;
			qaxpy_(&i__3, &alpha, &b[js * b_dim1 + 1], &c__1, &f[
				is + f_dim1], ldf);
			alpha = rhs[1];
			i__3 = js - 1;
			qaxpy_(&i__3, &alpha, &e[js * e_dim1 + 1], &c__1, &f[
				is + f_dim1], ldf);
		    }
		    if (i__ < p) {
			alpha = -rhs[0];
			i__3 = *m - ie;
			qaxpy_(&i__3, &alpha, &a[is + (ie + 1) * a_dim1], lda,
				 &c__[ie + 1 + js * c_dim1], &c__1);
			alpha = -rhs[1];
			i__3 = *m - ie;
			qaxpy_(&i__3, &alpha, &d__[is + (ie + 1) * d_dim1], 
...
/*                 equation. */
		    ...
		    if (j > p + 2) {
			i__3 = js - 1;
			qaxpy_(&i__3, rhs, &b[js * b_dim1 + 1], &c__1, &f[is 
				+ f_dim1], ldf);
			i__3 = js - 1;
			qaxpy_(&i__3, &rhs[1], &b[jsp1 * b_dim1 + 1], &c__1, &
				f[is + f_dim1], ldf);
			i__3 = js - 1;
			qaxpy_(&i__3, &rhs[2], &e[js * e_dim1 + 1], &c__1, &f[
				is + f_dim1], ldf);
			i__3 = js - 1;
			qaxpy_(&i__3, &rhs[3], &e[jsp1 * e_dim1 + 1], &c__1, &
				f[is + f_dim1], ldf);
		    }
		    if (i__ < p) {
			i__3 = *m - ie;
			qger_(&i__3, &nb, &c_b27, &a[is + (ie + 1) * a_dim1], 
				lda, rhs, &c__1, &c__[ie + 1 + js * c_dim1], 
				ldc);
			i__3 = *m - ie;
			qger_(&i__3, &nb, &c_b27, &d__[is + (ie + 1) * d_dim1]
...
/*                 equation. */
		    ...
		    if (j > p + 2) {
			i__3 = js - 1;
			qger_(&mb, &i__3, &c_b42, rhs, &c__1, &b[js * b_dim1 
				+ 1], &c__1, &f[is + f_dim1], ldf);
			i__3 = js - 1;
			qger_(&mb, &i__3, &c_b42, &rhs[2], &c__1, &e[js * 
				e_dim1 + 1], &c__1, &f[is + f_dim1], ldf);
		    }
		    if (i__ < p) {
			i__3 = *m - ie;
			qgemv_("T", &mb, &i__3, &c_b27, &a[is + (ie + 1) * 
				a_dim1], lda, rhs, &c__1, &c_b42, &c__[ie + 1 
				+ js * c_dim1], &c__1);
			i__3 = *m - ie;
			qgemv_("T", &mb, &i__3, &c_b27, &d__[is + (ie + 1) * 
