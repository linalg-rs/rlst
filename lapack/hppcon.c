...
/* > 1-norm) of a doublereal symmetric positive definite packed matrix using */
...
/*  ===================================================================== */
void  hppcon_(char *uplo, integer *n, halfreal *ap, 
	halfreal *anorm, halfreal *rcond, halfreal *work, integer *
	iwork, integer *info)
{
    ...
    halfreal scaleu;
    extern void  xerbla_(char *, integer *), hlatps_(
	    char *, char *, char *, char *, integer *, halfreal *, 
...
/*           Multiply by inv(U**T). */
	    ...
	    hlatps_("Upper", "Transpose", "Non-unit", normin, n, &ap[1], &
		    work[1], &scalel, &work[(*n << 1) + 1], info);
	    *(unsigned char *)normin = 'Y';

/*           Multiply by inv(U). */

	    hlatps_("Upper", "No transpose", "Non-unit", normin, n, &ap[1], &
		    work[1], &scaleu, &work[(*n << 1) + 1], info);
	} else {

/*           Multiply by inv(L). */

	    hlatps_("Lower", "No transpose", "Non-unit", normin, n, &ap[1], &
		    work[1], &scalel, &work[(*n << 1) + 1], info);
	    *(unsigned char *)normin = 'Y';

/*           Multiply by inv(L**T). */

	    hlatps_("Lower", "Transpose", "Non-unit", normin, n, &ap[1], &
		    work[1], &scaleu, &work[(*n << 1) + 1], info);
	}
