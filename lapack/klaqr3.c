...
/* > */
/*  ===================================================================== */
void  klaqr3_(logical *wantt, logical *wantz, integer *n, 
	integer *ktop, integer *kbot, integer *nw, halfcomplex *h__, 
	integer *ldh, integer *iloz, integer *ihiz, halfcomplex *z__, 
	integer *ldz, integer *ns, integer *nd, halfcomplex *sh, 
	halfcomplex *v, integer *ldv, integer *nh, halfcomplex *t, 
	integer *ldt, integer *nv, halfcomplex *wv, integer *ldwv, 
	halfcomplex *work, integer *lwork)
{
    ...
    extern void  kcopy_(integer *, halfcomplex *, integer *, 
	    ...
	    klaqr4_(logical *, logical *, integer *, integer *, integer *, 
    ...
    extern void  kgehrd_(integer *, integer *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, halfcomplex *, 
	    integer *, integer *), klarfg_(integer *, halfcomplex *, 
	    halfcomplex *, integer *, halfcomplex *), klahqr_(logical *, 
	    logical *, integer *, integer *, integer *, halfcomplex *, 
	    integer *, halfcomplex *, integer *, integer *, halfcomplex *,
	     integer *, integer *), klacpy_(char *, integer *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, integer *), 
	    klaset_(char *, integer *, integer *, halfcomplex *, 
	    halfcomplex *, halfcomplex *, integer *);
    halfreal smlnum;
    extern void  ktrexc_(char *, integer *, halfcomplex *, 
	    integer *, halfcomplex *, integer *, integer *, integer *, 
	    integer *);
    integer lwkopt;
    extern void  kunmhr_(char *, char *, integer *, integer *, 
...
/*        ==== Workspace query call to ZGEHRD ==== */
	...
	i__1 = jw - 1;
	kgehrd_(&jw, &c__1, &i__1, &t[t_offset], ldt, &work[1], &work[1], &
		c_n1, &info);
...
/*        ==== Workspace query call to ZUNMHR ==== */
	...
	i__1 = jw - 1;
	kunmhr_("R", "N", &jw, &jw, &c__1, &i__1, &t[t_offset], ldt, &work[1],
		 &v[v_offset], ldv, &work[1], &c_n1, &info);
	lwk2 = (integer) work[1].r;

/*        ==== Workspace query call to ZLAQR4 ==== */

	klaqr4_(&c_true, &c_true, &jw, &c__1, &jw, &t[t_offset], ldt, &sh[1], 
...
/*     .    here and there to keep track.) ==== */
    ...
    klacpy_("U", &jw, &jw, &h__[kwtop + kwtop * h_dim1], ldh, &t[t_offset], 
	    ldt);
    i__1 = jw - 1;
    i__2 = *ldh + 1;
    i__3 = *ldt + 1;
    kcopy_(&i__1, &h__[kwtop + 1 + kwtop * h_dim1], &i__2, &t[t_dim1 + 2], &
	    i__3);

    klaset_("A", &jw, &jw, &c_b1, &c_b2, &v[v_offset], ldv);
    nmin = ilaenv_(&c__12, "ZLAQR3", "SV", &jw, &c__1, &jw, lwork, (ftnlen)6, 
	    (ftnlen)2);
    if (jw > nmin) {
	klaqr4_(&c_true, &c_true, &jw, &c__1, &jw, &t[t_offset], ldt, &sh[
		kwtop], &c__1, &jw, &v[v_offset], ldv, &work[1], lwork, &
		infqr);
    } else {
	klahqr_(&c_true, &c_true, &jw, &c__1, &jw, &t[t_offset], ldt, &sh[
...
/*           .    way.   (ZTREXC can not fail in this case.) ==== */
	    ...
	    ifst = *ns;
	    ktrexc_("V", &jw, &t[t_offset], ldt, &v[v_offset], ldv, &ifst, &
		    ilst, &info);
	    ++ilst;
	}
/* L10: */
...
/*           ==== Reflect spike back into lower triangle ==== */
	    ...
	    kcopy_(ns, &v[v_offset], ldv, &work[1], &c__1);
	    i__1 = *ns;
...
/* L50: */
	    ...
	    klarfg_(ns, &beta, &work[2], &c__1, &tau);
	    work[1].r = 1., work[1].i = 0.;

	    i__1 = jw - 2;
	    i__2 = jw - 2;
	    klaset_("L", &i__1, &i__2, &c_b1, &c_b1, &t[t_dim1 + 3], ldt);

	    d_cnjg(&z__1, &tau);
	    klarf_("L", ns, &jw, &work[1], &c__1, &z__1, &t[t_offset], ldt, &
		    work[jw + 1]);
	    klarf_("R", ns, ns, &work[1], &c__1, &tau, &t[t_offset], ldt, &
		    work[jw + 1]);
	    klarf_("R", &jw, ns, &work[1], &c__1, &tau, &v[v_offset], ldv, &
		    work[jw + 1]);

	    i__1 = *lwork - jw;
	    kgehrd_(&jw, &c__1, ns, &t[t_offset], ldt, &work[1], &work[jw + 1]
		    , &i__1, &info);
	}
...
/*        ==== Copy updated reduced window into place ==== */
	...
	klacpy_("U", &jw, &jw, &t[t_offset], ldt, &h__[kwtop + kwtop * h_dim1]
		, ldh);
	i__1 = jw - 1;
	i__2 = *ldt + 1;
	i__3 = *ldh + 1;
	kcopy_(&i__1, &t[t_dim1 + 2], &i__2, &h__[kwtop + 1 + kwtop * h_dim1],
		 &i__3);
...
/*        .    H and Z, if requested.  ==== */
	...
	if (*ns > 1 && (s.r != 0. || s.i != 0.)) {
	    i__1 = *lwork - jw;
	    kunmhr_("R", "N", &jw, ns, &c__1, ns, &t[t_offset], ldt, &work[1],
...
/* Computing MIN */
	    ...
	    kgemm_("N", "N", &kln, &jw, &jw, &c_b2, &h__[krow + kwtop * 
		    h_dim1], ldh, &v[v_offset], ldv, &c_b1, &wv[wv_offset], 
		    ldwv);
	    klacpy_("A", &kln, &jw, &wv[wv_offset], ldwv, &h__[krow + kwtop * 
		    h_dim1], ldh);
/* L60: */
	}
...
/* Computing MIN */
		...
		kln = f2cmin(i__3,i__4);
		kgemm_("C", "N", &jw, &kln, &jw, &c_b2, &v[v_offset], ldv, &
			h__[kwtop + kcol * h_dim1], ldh, &c_b1, &t[t_offset], 
			ldt);
		klacpy_("A", &jw, &kln, &t[t_offset], ldt, &h__[kwtop + kcol *
			 h_dim1], ldh);
/* L70: */
	    }
	}
...
/* Computing MIN */
		...
		kln = f2cmin(i__3,i__4);
		kgemm_("N", "N", &kln, &jw, &jw, &c_b2, &z__[krow + kwtop * 
			z_dim1], ldz, &v[v_offset], ldv, &c_b1, &wv[wv_offset]
			, ldwv);
		klacpy_("A", &kln, &jw, &wv[wv_offset], ldwv, &z__[krow + 
			kwtop * z_dim1], ldz);
/* L80: */
	    }
	}
