...
/* > */
/*  ===================================================================== */
void  wlaqr5_(logical *wantt, logical *wantz, integer *kacc22, 
	integer *n, integer *ktop, integer *kbot, integer *nshfts, 
	quadcomplex *s, quadcomplex *h__, integer *ldh, integer *iloz, 
	integer *ihiz, quadcomplex *z__, integer *ldz, quadcomplex *v, 
	integer *ldv, quadcomplex *u, integer *ldu, integer *nv, 
	quadcomplex *wv, integer *ldwv, integer *nh, quadcomplex *wh, 
	integer *ldwh)
{
    ...
    extern void  wgemm_(char *, char *, integer *, integer *, 
	    ...
	    , quadcomplex *, integer *), 
	    qlabad_(quadreal *, quadreal *), wlaqr1_(integer *, 
	    quadcomplex *, integer *, quadcomplex *, quadcomplex *, 
	    quadcomplex *);
    extern quadreal qlamch_(char *);
    quadreal safmin, safmax;
...
/*     ==== Create and chase chains of NBMPS bulges ==== */
	...
	ndcol = incol + kdu;
	if (accum) {
	    wlaset_("ALL", &kdu, &kdu, &c_b1, &c_b2, &u[u_offset], ldu);
	}
...
/*           .    one column.  (The minimum value of K is KTOP-1.) ==== */
		...
		k = krcol + (m - 1) * 3;
		if (k == *ktop - 1) {
		    wlaqr1_(&c__3, &h__[*ktop + *ktop * h_dim1], ldh, &s[(m <<
			     1) - 1], &s[m * 2], &v[m * v_dim1 + 1]);
		    i__5 = m * v_dim1 + 1;
		    alpha.r = v[i__5].r, alpha.i = v[i__5].i;
		    wlarfg_(&c__3, &alpha, &v[m * v_dim1 + 2], &c__1, &v[m * 
			    v_dim1 + 1]);
		} else {
		    i__5 = k + 1 + k * h_dim1;
		    ...
		    i__6 = k + 3 + k * h_dim1;
		    v[i__5].r = h__[i__6].r, v[i__5].i = h__[i__6].i;
		    wlarfg_(&c__3, &beta, &v[m * v_dim1 + 2], &c__1, &v[m * 
			    v_dim1 + 1]);
...
/*                    .    Otherwise, use the new one. ==== */
			...
			wlaqr1_(&c__3, &h__[k + 1 + (k + 1) * h_dim1], ldh, &
				s[(m << 1) - 1], &s[m * 2], vt);
			alpha.r = vt[0].r, alpha.i = vt[0].i;
			wlarfg_(&c__3, &alpha, &vt[1], &c__1, vt);
			d_cnjg(&z__2, vt);
			i__5 = k + 1 + k * h_dim1;
			d_cnjg(&z__5, &vt[1]);
			i__6 = k + 2 + k * h_dim1;
...
/*           ==== Generate a 2-by-2 reflection, if needed. ==== */
	    ...
	    if (bmp22) {
		if (k == *ktop - 1) {
		    wlaqr1_(&c__2, &h__[k + 1 + (k + 1) * h_dim1], ldh, &s[(
			    m22 << 1) - 1], &s[m22 * 2], &v[m22 * v_dim1 + 1])
			    ;
		    i__4 = m22 * v_dim1 + 1;
		    beta.r = v[i__4].r, beta.i = v[i__4].i;
		    wlarfg_(&c__2, &beta, &v[m22 * v_dim1 + 2], &c__1, &v[m22 
			    * v_dim1 + 1]);
		} else {
		    i__4 = k + 1 + k * h_dim1;
...
/* Computing MIN */
		    ...
		    wgemm_("C", "N", &nu, &jlen, &nu, &c_b2, &u[k1 + k1 * 
			    u_dim1], ldu, &h__[incol + k1 + jcol * h_dim1], 
			    ldh, &c_b1, &wh[wh_offset], ldwh);
		    wlacpy_("ALL", &nu, &jlen, &wh[wh_offset], ldwh, &h__[
...
/* Computing MIN */
		    ...
		    jlen = f2cmin(i__5,i__7);
		    wgemm_("N", "N", &jlen, &nu, &nu, &c_b2, &h__[jrow + (
			    incol + k1) * h_dim1], ldh, &u[k1 + k1 * u_dim1], 
			    ldu, &c_b1, &wv[wv_offset], ldwv);
		    wlacpy_("ALL", &jlen, &nu, &wv[wv_offset], ldwv, &h__[
...
/* Computing MIN */
			...
			jlen = f2cmin(i__5,i__7);
			wgemm_("N", "N", &jlen, &nu, &nu, &c_b2, &z__[jrow + (
				incol + k1) * z_dim1], ldz, &u[k1 + k1 * 
				u_dim1], ldu, &c_b1, &wv[wv_offset], ldwv);
			wlacpy_("ALL", &jlen, &nu, &wv[wv_offset], ldwv, &z__[
...
/*                  (The first KZS rows get multiplied by zero.) ==== */
		    ...
		    wlacpy_("ALL", &knz, &jlen, &h__[incol + 1 + j2 + jcol * 
			    h_dim1], ldh, &wh[kzs + 1 + wh_dim1], ldwh);

/*                 ==== Multiply by U21**H ==== */

		    wlaset_("ALL", &kzs, &jlen, &c_b1, &c_b1, &wh[wh_offset], 
			    ldwh);
		    wtrmm_("L", "U", "C", "N", &knz, &jlen, &c_b2, &u[j2 + 1 
			    + (kzs + 1) * u_dim1], ldu, &wh[kzs + 1 + wh_dim1]
			    , ldwh);

/*                 ==== Multiply top of H by U11**H ==== */

		    wgemm_("C", "N", &i2, &jlen, &j2, &c_b2, &u[u_offset], 
			    ldu, &h__[incol + 1 + jcol * h_dim1], ldh, &c_b2, 
			    &wh[wh_offset], ldwh);

/*                 ==== Copy top of H to bottom of WH ==== */

		    wlacpy_("ALL", &j2, &jlen, &h__[incol + 1 + jcol * h_dim1]
			    , ldh, &wh[i2 + 1 + wh_dim1], ldwh);

/*                 ==== Multiply by U21**H ==== */

		    wtrmm_("L", "L", "C", "N", &j2, &jlen, &c_b2, &u[(i2 + 1) 
...
/*                 ==== Multiply by U22 ==== */
		    ...
		    i__5 = i4 - i2;
		    i__7 = j4 - j2;
		    wgemm_("C", "N", &i__5, &jlen, &i__7, &c_b2, &u[j2 + 1 + (
...
/*                 ==== Copy it back ==== */
		    ...
		    wlacpy_("ALL", &kdu, &jlen, &wh[wh_offset], ldwh, &h__[
...
/*                 .    columns get multiplied by zero) ==== */
		    ...
		    wlacpy_("ALL", &jlen, &knz, &h__[jrow + (incol + 1 + j2) *
			     h_dim1], ldh, &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

/*                 ==== Multiply by U21 ==== */

		    wlaset_("ALL", &jlen, &kzs, &c_b1, &c_b1, &wv[wv_offset], 
			    ldwv);
		    wtrmm_("R", "U", "N", "N", &jlen, &knz, &c_b2, &u[j2 + 1 
			    + (kzs + 1) * u_dim1], ldu, &wv[(kzs + 1) * 
			    wv_dim1 + 1], ldwv);

/*                 ==== Multiply by U11 ==== */

		    wgemm_("N", "N", &jlen, &i2, &j2, &c_b2, &h__[jrow + (
			    incol + 1) * h_dim1], ldh, &u[u_offset], ldu, &
			    c_b2, &wv[wv_offset], ldwv);

/*                 ==== Copy left of H to right of scratch ==== */

		    wlacpy_("ALL", &jlen, &j2, &h__[jrow + (incol + 1) * 
			    h_dim1], ldh, &wv[(i2 + 1) * wv_dim1 + 1], ldwv);

/*                 ==== Multiply by U21 ==== */

		    i__5 = i4 - i2;
		    wtrmm_("R", "L", "N", "N", &jlen, &i__5, &c_b2, &u[(i2 + 
...
/*                 ==== Multiply by U22 ==== */
		    ...
		    i__5 = i4 - i2;
		    i__7 = j4 - j2;
		    wgemm_("N", "N", &jlen, &i__5, &i__7, &c_b2, &h__[jrow + (
...
/*                 ==== Copy it back ==== */
		    ...
		    wlacpy_("ALL", &jlen, &kdu, &wv[wv_offset], ldwv, &h__[
...
/*                    .     KZS columns get multiplied by zero) ==== */
			...
			wlacpy_("ALL", &jlen, &knz, &z__[jrow + (incol + 1 + 
				j2) * z_dim1], ldz, &wv[(kzs + 1) * wv_dim1 + 
				1], ldwv);

/*                    ==== Multiply by U12 ==== */

			wlaset_("ALL", &jlen, &kzs, &c_b1, &c_b1, &wv[
				wv_offset], ldwv);
			wtrmm_("R", "U", "N", "N", &jlen, &knz, &c_b2, &u[j2 
				+ 1 + (kzs + 1) * u_dim1], ldu, &wv[(kzs + 1) 
				* wv_dim1 + 1], ldwv);

/*                    ==== Multiply by U11 ==== */

			wgemm_("N", "N", &jlen, &i2, &j2, &c_b2, &z__[jrow + (
				incol + 1) * z_dim1], ldz, &u[u_offset], ldu, 
				&c_b2, &wv[wv_offset], ldwv);

/*                    ==== Copy left of Z to right of scratch ==== */

			wlacpy_("ALL", &jlen, &j2, &z__[jrow + (incol + 1) * 
...
/*                    ==== Multiply by U21 ==== */
			...
			i__5 = i4 - i2;
			wtrmm_("R", "L", "N", "N", &jlen, &i__5, &c_b2, &u[(
...
/*                    ==== Multiply by U22 ==== */
			...
			i__5 = i4 - i2;
			i__7 = j4 - j2;
			wgemm_("N", "N", &jlen, &i__5, &i__7, &c_b2, &z__[
...
/*                    ==== Copy the result back to Z ==== */
			...
			wlacpy_("ALL", &jlen, &kdu, &wv[wv_offset], ldwv, &
