...
/* > */
/*  ===================================================================== */
quadreal wlanhf_(char *norm, char *transr, char *uplo, integer *n, 
	quadcomplex *a, quadreal *work)
{
...
/*                 A is upper */
		    ...
		    for (j = 0; j <= i__1; ++j) {
			i__2 = k - j - 2;
			wlassq_(&i__2, &a[k + j + 1 + j * lda], &c__1, &scale,
				 &s);
/*                    L at A(k,0) */
		    }
		    i__1 = k - 1;
		    for (j = 0; j <= i__1; ++j) {
			i__2 = k + j - 1;
			wlassq_(&i__2, &a[j * lda], &c__1, &scale, &s);
/*                    trap U at A(0,0) */
		    }
		    s += s;
...
/*                 ilu=1 & A is lower */
		    ...
		    for (j = 0; j <= i__1; ++j) {
			i__2 = *n - j - 1;
			wlassq_(&i__2, &a[j + 1 + j * lda], &c__1, &scale, &s)
				;
/*                    trap L at A(0,0) */
		    }
		    i__1 = k - 2;
		    for (j = 1; j <= i__1; ++j) {
			wlassq_(&j, &a[(j + 1) * lda], &c__1, &scale, &s);
/*                    U at A(0,1) */
		    }
		    s += s;
/*                 double s for the off diagonal elements */
...
/*                 A**H is upper */
		    i__1 = k - 2;
		    for (j = 1; j <= i__1; ++j) {
			wlassq_(&j, &a[(k + j) * lda], &c__1, &scale, &s);
/*                    U at A(0,k) */
		    }
		    i__1 = k - 2;
		    for (j = 0; j <= i__1; ++j) {
			wlassq_(&k, &a[j * lda], &c__1, &scale, &s);
/*                    k by k-1 rect. at A(0,0) */
		    }
		    i__1 = k - 2;
		    for (j = 0; j <= i__1; ++j) {
			i__2 = k - j - 1;
			wlassq_(&i__2, &a[j + 1 + (j + k - 1) * lda], &c__1, &
				scale, &s);
/*                    L at A(0,k-1) */
		    }
		    s += s;
...
/*                 A**H is lower */
		    i__1 = k - 1;
		    for (j = 1; j <= i__1; ++j) {
			wlassq_(&j, &a[j * lda], &c__1, &scale, &s);
/*                    U at A(0,0) */
		    }
		    i__1 = *n - 1;
		    for (j = k; j <= i__1; ++j) {
...
/*                    k by k-1 rect. at A(0,k) */
		    ...
		    for (j = 0; j <= i__1; ++j) {
			i__2 = k - j - 2;
			wlassq_(&i__2, &a[j + 2 + j * lda], &c__1, &scale, &s)
				;
/*                    L at A(1,0) */
		    }
		    s += s;
...
/*                 A is upper */
		    ...
		    for (j = 0; j <= i__1; ++j) {
			i__2 = k - j - 1;
			wlassq_(&i__2, &a[k + j + 2 + j * lda], &c__1, &scale,
				 &s);
/*                 L at A(k+1,0) */
		    }
		    i__1 = k - 1;
...
/*                 A**H is upper */
		    i__1 = k - 1;
		    for (j = 1; j <= i__1; ++j) {
			wlassq_(&j, &a[(k + 1 + j) * lda], &c__1, &scale, &s);
/*                 U at A(0,k+1) */
		    }
		    i__1 = k - 1;
		    for (j = 0; j <= i__1; ++j) {
...
/*                 k by k rect. at A(0,0) */
		    ...
		    for (j = 0; j <= i__1; ++j) {
			i__2 = k - j - 1;
			wlassq_(&i__2, &a[j + 1 + (j + k) * lda], &c__1, &
				scale, &s);
/*                 L at A(0,k) */
		    }
		    s += s;
