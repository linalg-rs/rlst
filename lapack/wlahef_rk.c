...
/*  ===================================================================== */
void  zlahef_rk__(char *uplo, integer *n, integer *nb, integer 
	*kb, quadcomplex *a, integer *lda, quadcomplex *e, integer *ipiv, 
	quadcomplex *w, integer *ldw, integer *info)
{
    /* System generated locals */
    integer a_dim1, a_offset, w_dim1, w_offset, i__1, i__2, i__3, i__4, i__5;
    quadreal d__1, d__2;
    quadcomplex z__1, z__2, z__3, z__4, z__5;

    /* Local variables */
    integer j, k, p;
    quadreal t, r1;
    quadcomplex d11, d21, d22;
    ...
    extern void  wgemv_(char *, integer *, integer *, 
	    ...
	    integer *), wswap_(integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *);
    ...
    extern void  wqscal_(integer *, quadreal *, 
	    quadcomplex *, integer *);
    quadreal colmax;
    extern void  wlacgv_(integer *, quadcomplex *, integer *)
	    ;
...
/*        Copy column K of A to column KW of W and update it */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    wcopy_(&i__1, &a[k * a_dim1 + 1], &c__1, &w[kw * w_dim1 + 1], &
		    c__1);
	}
	i__1 = k + kw * w_dim1;
	i__2 = k + k * a_dim1;
	...
	if (k < *n) {
	    i__1 = *n - k;
	    z__1.r = -1., z__1.i = -0.;
	    wgemv_("No transpose", &k, &i__1, &z__1, &a[(k + 1) * a_dim1 + 1],
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    imax = iwamax_(&i__1, &w[kw * w_dim1 + 1], &c__1);
	    i__1 = imax + kw * w_dim1;
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    if (k > 1) {
		i__1 = k - 1;
		wcopy_(&i__1, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], 
			&c__1);
	    }
...
/*                 Copy column IMAX to column KW-1 of W and update it */
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    wcopy_(&i__1, &a[imax * a_dim1 + 1], &c__1, &w[(kw - 1) * 
			    w_dim1 + 1], &c__1);
		}
		...
		i__1 = k - imax;
		wcopy_(&i__1, &a[imax + (imax + 1) * a_dim1], lda, &w[imax + 
			1 + (kw - 1) * w_dim1], &c__1);
		i__1 = k - imax;
		wlacgv_(&i__1, &w[imax + 1 + (kw - 1) * w_dim1], &c__1);

		if (k < *n) {
		    i__1 = *n - k;
		    z__1.r = -1., z__1.i = -0.;
		    wgemv_("No transpose", &k, &i__1, &z__1, &a[(k + 1) * 
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = k - imax;
		    jmax = imax + iwamax_(&i__1, &w[imax + 1 + (kw - 1) * 
			    w_dim1], &c__1);
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    itemp = iwamax_(&i__1, &w[(kw - 1) * w_dim1 + 1], &c__1);
...
/*                    copy column KW-1 of W to column KW of W */
		    ...
		    wcopy_(&k, &w[(kw - 1) * w_dim1 + 1], &c__1, &w[kw * 
			    w_dim1 + 1], &c__1);
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = k - 1 - p;
		wcopy_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 1) * 
			a_dim1], lda);
		i__1 = k - 1 - p;
		wlacgv_(&i__1, &a[p + (p + 1) * a_dim1], lda);
		if (p > 1) {
		    i__1 = p - 1;
		    wcopy_(&i__1, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 
			    1], &c__1);
		}
...
/*              in last KKW to NB columns of W. */
		...
		if (k < *n) {
		    i__1 = *n - k;
		    wswap_(&i__1, &a[k + (k + 1) * a_dim1], lda, &a[p + (k + 
			    1) * a_dim1], lda);
		}
		i__1 = *n - kk + 1;
		wswap_(&i__1, &w[k + kkw * w_dim1], ldw, &w[p + kkw * w_dim1],
			 ldw);
	    }
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = kk - 1 - kp;
		wcopy_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (kp + 
			1) * a_dim1], lda);
		i__1 = kk - 1 - kp;
		wlacgv_(&i__1, &a[kp + (kp + 1) * a_dim1], lda);
		if (kp > 1) {
		    i__1 = kp - 1;
		    wcopy_(&i__1, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 
			    + 1], &c__1);
		}
...
/*              in last KKW to NB columns of W. */
		...
		if (k < *n) {
		    i__1 = *n - k;
		    wswap_(&i__1, &a[kk + (k + 1) * a_dim1], lda, &a[kp + (k 
			    + 1) * a_dim1], lda);
		}
		i__1 = *n - kk + 1;
		wswap_(&i__1, &w[kk + kkw * w_dim1], ldw, &w[kp + kkw * 
			w_dim1], ldw);
	    }
...
/*              element D(k,k) from W (potentially saves only one load)) */
		wcopy_(&k, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &
			c__1);
		if (k > 1) {
...
/*                 Handle division by a small number */
			...
			r1 = 1. / t;
			i__1 = k - 1;
			wqscal_(&i__1, &r1, &a[k * a_dim1 + 1], &c__1);
		    } else {
			i__1 = k - 1;
			for (ii = 1; ii <= i__1; ++ii) {
			    i__2 = ii + k * a_dim1;
...
/*                 (2) Conjugate column W(kw) */
		    ...
		    i__1 = k - 1;
		    wlacgv_(&i__1, &w[kw * w_dim1 + 1], &c__1);
...
/*              (2) Conjugate columns W(kw) and W(kw-1) */
		...
		wlacgv_(&i__1, &w[kw * w_dim1 + 1], &c__1);
		i__1 = k - 2;
		wlacgv_(&i__1, &w[(kw - 1) * w_dim1 + 1], &c__1);
...
/*           Update the upper triangle of the diagonal block */
		...
		i__4 = *n - k;
		z__1.r = -1., z__1.i = -0.;
		wgemv_("No transpose", &i__3, &i__4, &z__1, &a[j + (k + 1) * 
...
/*           Update the rectangular superdiagonal block */
		...
		i__3 = *n - k;
		z__1.r = -1., z__1.i = -0.;
		wgemm_("No transpose", "Transpose", &i__2, &jb, &i__3, &z__1, 
...
/*        Copy column K of A to column K of W and update column K of W */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    wcopy_(&i__1, &a[k + 1 + k * a_dim1], &c__1, &w[k + 1 + k * 
		    w_dim1], &c__1);
	}
	if (k > 1) {
	    i__1 = *n - k + 1;
	    i__2 = k - 1;
	    z__1.r = -1., z__1.i = -0.;
	    wgemv_("No transpose", &i__1, &i__2, &z__1, &a[k + a_dim1], lda, &
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + iwamax_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
	    i__1 = imax + k * w_dim1;
	    colmax = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[imax + 
		    k * w_dim1]), abs(d__2));
	} else {
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    if (k < *n) {
		i__1 = *n - k;
		wcopy_(&i__1, &w[k + 1 + k * w_dim1], &c__1, &a[k + 1 + k * 
			a_dim1], &c__1);
	    }
...
/*                 Copy column IMAX to column k+1 of W and update it */
		...
		i__1 = imax - k;
		wcopy_(&i__1, &a[imax + k * a_dim1], lda, &w[k + (k + 1) * 
			w_dim1], &c__1);
		i__1 = imax - k;
		wlacgv_(&i__1, &w[k + (k + 1) * w_dim1], &c__1);
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    wcopy_(&i__1, &a[imax + 1 + imax * a_dim1], &c__1, &w[
		...
		if (k > 1) {
		    ...
		    i__2 = k - 1;
		    z__1.r = -1., z__1.i = -0.;
		    wgemv_("No transpose", &i__1, &i__2, &z__1, &a[k + a_dim1]
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = imax - k;
		    jmax = k - 1 + iwamax_(&i__1, &w[k + (k + 1) * w_dim1], &
			    c__1);
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    itemp = imax + iwamax_(&i__1, &w[imax + 1 + (k + 1) * 
			    w_dim1], &c__1);
...
/*                    copy column K+1 of W to column K of W */
		    ...
		    i__1 = *n - k + 1;
		    wcopy_(&i__1, &w[k + (k + 1) * w_dim1], &c__1, &w[k + k * 
			    w_dim1], &c__1);
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = p - k - 1;
		wcopy_(&i__1, &a[k + 1 + k * a_dim1], &c__1, &a[p + (k + 1) * 
			a_dim1], lda);
		i__1 = p - k - 1;
		wlacgv_(&i__1, &a[p + (k + 1) * a_dim1], lda);
		if (p < *n) {
		    i__1 = *n - p;
		    wcopy_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + 1 + p 
			    * a_dim1], &c__1);
		}
...
/*              in first KK columns of W. */
		...
		if (k > 1) {
		    i__1 = k - 1;
		    wswap_(&i__1, &a[k + a_dim1], lda, &a[p + a_dim1], lda);
		}
		wswap_(&kk, &w[k + w_dim1], ldw, &w[p + w_dim1], ldw);
	    }
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = kp - kk - 1;
		wcopy_(&i__1, &a[kk + 1 + kk * a_dim1], &c__1, &a[kp + (kk + 
			1) * a_dim1], lda);
		i__1 = kp - kk - 1;
		wlacgv_(&i__1, &a[kp + (kk + 1) * a_dim1], lda);
		if (kp < *n) {
		    i__1 = *n - kp;
		    wcopy_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + 1 
			    + kp * a_dim1], &c__1);
		}
...
/*              in first KK columns of W. */
		...
		if (k > 1) {
		    i__1 = k - 1;
		    wswap_(&i__1, &a[kk + a_dim1], lda, &a[kp + a_dim1], lda);
		}
		wswap_(&kk, &w[kk + w_dim1], ldw, &w[kp + w_dim1], ldw);
	    }
...
/*              element D(k,k) from W (potentially saves only one load)) */
		i__1 = *n - k + 1;
		wcopy_(&i__1, &w[k + k * w_dim1], &c__1, &a[k + k * a_dim1], &
			c__1);
		if (k < *n) {
...
/*                 Handle division by a small number */
			...
			r1 = 1. / t;
			i__1 = *n - k;
			wqscal_(&i__1, &r1, &a[k + 1 + k * a_dim1], &c__1);
		    } else {
			i__1 = *n;
...
/*                 (2) Conjugate column W(k) */
		    ...
		    i__1 = *n - k;
		    wlacgv_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
...
/*              (2) Conjugate columns W(k) and W(k+1) */
		...
		wlacgv_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
		i__1 = *n - k - 1;
		wlacgv_(&i__1, &w[k + 2 + (k + 1) * w_dim1], &c__1);
...
/*           Update the lower triangle of the diagonal block */
		...
		i__5 = k - 1;
		z__1.r = -1., z__1.i = -0.;
		wgemv_("No transpose", &i__4, &i__5, &z__1, &a[jj + a_dim1], 
...
/*           Update the rectangular subdiagonal block */
		...
		i__4 = k - 1;
		z__1.r = -1., z__1.i = -0.;
		wgemm_("No transpose", "Transpose", &i__3, &jb, &i__4, &z__1, 
