...
/* > */
/*  ===================================================================== */
void  wtrevc3_(char *side, char *howmny, logical *select, 
	integer *n, quadcomplex *t, integer *ldt, quadcomplex *vl, 
	integer *ldvl, quadcomplex *vr, integer *ldvr, integer *mm, integer 
	*m, quadcomplex *work, integer *lwork, quadreal *rwork, integer *
	lrwork, integer *info)
{
...
/*     Zero-out the workspace to avoid potential NaN propagation. */
	...
	nb = f2cmin(nb,128);
	i__1 = (nb << 1) + 1;
	wlaset_("F", n, &i__1, &c_b1, &c_b1, &work[1], n);
    } else {
	nb = 1;
    }
...
/*     part of T to control overflow in triangular solver. */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__3 = j - 1;
	rwork[j] = qwasum_(&i__3, &t[j * t_dim1 + 1], &c__1);
/* L30: */
    }
...
/* L50: */
	    ...
	    if (ki > 1) {
		i__1 = ki - 1;
		wlatrs_("Upper", "No transpose", "Non-unit", "Y", &i__1, &t[
...
/*              no back-transform: copy x to VR and normalize. */
		wcopy_(&ki, &work[iv * *n + 1], &c__1, &vr[is * vr_dim1 + 1], 
			&c__1);

		ii = iwamax_(&ki, &vr[is * vr_dim1 + 1], &c__1);
		i__1 = ii + is * vr_dim1;
		remax = 1. / ((d__1 = vr[i__1].r, abs(d__1)) + (d__2 = d_imag(
			&vr[ii + is * vr_dim1]), abs(d__2)));
		wqscal_(&ki, &remax, &vr[is * vr_dim1 + 1], &c__1);
...
/*              version 1: back-transform each vector with GEMV, Q*x. */
		if (ki > 1) {
		    ...
		    wgemv_("N", n, &i__1, &c_b2, &vr[vr_offset], ldvr, &work[
			    iv * *n + 1], &c__1, &z__1, &vr[ki * vr_dim1 + 1],
			     &c__1);
		}

		ii = iwamax_(n, &vr[ki * vr_dim1 + 1], &c__1);
		i__1 = ii + ki * vr_dim1;
		remax = 1. / ((d__1 = vr[i__1].r, abs(d__1)) + (d__2 = d_imag(
			&vr[ii + ki * vr_dim1]), abs(d__2)));
		wqscal_(n, &remax, &vr[ki * vr_dim1 + 1], &c__1);
...
/*              or if this was last vector, do the GEMM */
		if (iv == 1 || ki == 1) {
		    i__1 = nb - iv + 1;
		    i__3 = ki + nb - iv;
		    wgemm_("N", "N", n, &i__1, &i__3, &c_b2, &vr[vr_offset], 
			    ldvr, &work[iv * *n + 1], n, &c_b1, &work[(nb + 
			    iv) * *n + 1], n);
/*                 normalize vectors */
		    i__1 = nb;
		    for (k = iv; k <= i__1; ++k) {
			ii = iwamax_(n, &work[(nb + k) * *n + 1], &c__1);
			i__3 = ii + (nb + k) * *n;
			remax = 1. / ((d__1 = work[i__3].r, abs(d__1)) + (
				d__2 = d_imag(&work[ii + (nb + k) * *n]), abs(
				d__2)));
			wqscal_(n, &remax, &work[(nb + k) * *n + 1], &c__1);
		    }
		    i__1 = nb - iv + 1;
		    wlacpy_("F", n, &i__1, &work[(nb + iv) * *n + 1], n, &vr[
			    ki * vr_dim1 + 1], ldvr);
		    iv = nb;
		} else {
		    --iv;
...
/* L100: */
	    ...
	    if (ki < *n) {
		i__3 = *n - ki;
		wlatrs_("Upper", "Conjugate transpose", "Non-unit", "Y", &
...
/*              no back-transform: copy x to VL and normalize. */
		i__3 = *n - ki + 1;
		wcopy_(&i__3, &work[ki + iv * *n], &c__1, &vl[ki + is * 
			vl_dim1], &c__1);

		i__3 = *n - ki + 1;
		ii = iwamax_(&i__3, &vl[ki + is * vl_dim1], &c__1) + ki - 1;
		i__3 = ii + is * vl_dim1;
		remax = 1. / ((d__1 = vl[i__3].r, abs(d__1)) + (d__2 = d_imag(
			&vl[ii + is * vl_dim1]), abs(d__2)));
		i__3 = *n - ki + 1;
		wqscal_(&i__3, &remax, &vl[ki + is * vl_dim1], &c__1);
...
/*              version 1: back-transform each vector with GEMV, Q*x. */
		if (ki < *n) {
		    i__3 = *n - ki;
		    z__1.r = scale, z__1.i = 0.;
		    wgemv_("N", n, &i__3, &c_b2, &vl[(ki + 1) * vl_dim1 + 1], 
			    ldvl, &work[ki + 1 + iv * *n], &c__1, &z__1, &vl[
			    ki * vl_dim1 + 1], &c__1);
		}

		ii = iwamax_(n, &vl[ki * vl_dim1 + 1], &c__1);
		i__3 = ii + ki * vl_dim1;
		remax = 1. / ((d__1 = vl[i__3].r, abs(d__1)) + (d__2 = d_imag(
			&vl[ii + ki * vl_dim1]), abs(d__2)));
		wqscal_(n, &remax, &vl[ki * vl_dim1 + 1], &c__1);
...
/*              or if this was last vector, do the GEMM */
		if (iv == nb || ki == *n) {
		    i__3 = *n - ki + iv;
		    wgemm_("N", "N", n, &iv, &i__3, &c_b2, &vl[(ki - iv + 1) *
...
/*                 normalize vectors */
		    ...
		    wlacpy_("F", n, &iv, &work[(nb + 1) * *n + 1], n, &vl[(ki 
			    - iv + 1) * vl_dim1 + 1], ldvl);
		    iv = 1;
		} else {
		    ++iv;
