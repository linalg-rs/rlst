...
/* > */
/*  ===================================================================== */
void  qtrsen_(char *job, char *compq, logical *select, integer 
	*n, quadreal *t, integer *ldt, quadreal *q, integer *ldq, 
	quadreal *wr, quadreal *wi, integer *m, quadreal *s, quadreal 
	*sep, quadreal *work, integer *lwork, integer *iwork, integer *
	liwork, integer *info)
{
    ...
    extern void  qtrsyl_(char *, char *, integer *, integer *, 
...
/*     Quick return if possible. */
	...
	if (wantsp) {
	    *sep = qlange_("1", n, n, &t[t_offset], ldt, &work[1]);
	}
	goto L40;
    }
...
/*              Swap the K-th block to position KS. */
		...
		kk = k;
		if (k != ks) {
		    qtrexc_(compq, n, &t[t_offset], ldt, &q[q_offset], ldq, &
			    kk, &ks, &work[1], &ierr);
		}
		if (ierr == 1 || ierr == 2) {
...
/*           T11*R - R*T22 = scale*T12 */
	...
	qlacpy_("F", &n1, &n2, &t[(n1 + 1) * t_dim1 + 1], ldt, &work[1], &n1);
	qtrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 
...
/*        Estimate sep(T11,T22). */
	...
	kase = 0;
L30:
	qlacn2_(&nn, &work[nn + 1], &work[1], &iwork[1], &est, &kase, isave);
	if (kase != 0) {
	    if (kase == 1) {

/*              Solve  T11*R - R*T22 = scale*X. */

		qtrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 
...
/*              Solve T11**T*R - R*T22**T = scale*X. */
		...
		qtrsyl_("T", "T", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 
			1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, &
			ierr);
	    }
	    goto L30;
