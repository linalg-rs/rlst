...
/* > \verbatim */
/* > */
/* > DGEMQR overwrites the general doublereal M-by-N matrix C with */
/* > */
/* >                      SIDE = 'L'     SIDE = 'R' */
/* >      TRANS = 'N':      Q * C          C * Q */
/* >      TRANS = 'T':      Q**T * C       C * Q**T */
...
/* > */
/*  ===================================================================== */
void  qgemqr_(char *side, char *trans, integer *m, integer *n, 
	integer *k, quadreal *a, integer *lda, quadreal *t, integer *
	tsize, quadreal *c__, integer *ldc, quadreal *work, integer *
	lwork, integer *info)
{
    ...
    /* Local variables */
    extern void  qlamtsqr_(char *, char *, integer *, integer *
...
/* Computing MAX */
    ...
    if (left && *m <= *k || right && *n <= *k || mb <= *k || mb >= f2cmax(i__1,*
	    k)) {
	qgemqrt_(side, trans, m, n, k, &nb, &a[a_offset], lda, &t[6], &nb, &
		c__[c_offset], ldc, &work[1], info);
    } else {
	qlamtsqr_(side, trans, m, n, k, &mb, &nb, &a[a_offset], lda, &t[6], &
