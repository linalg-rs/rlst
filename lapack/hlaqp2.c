...
/*  ===================================================================== */
void  hlaqp2_(integer *m, integer *n, integer *offset, 
	halfreal *a, integer *lda, integer *jpvt, halfreal *tau, 
	halfreal *vn1, halfreal *vn2, halfreal *work)
{
...
/*        Determine ith pivot column and swap if necessary. */
	...
	if (pvt != i__) {
	    hswap_(m, &a[pvt * a_dim1 + 1], &c__1, &a[i__ * a_dim1 + 1], &
		    c__1);
	    itemp = jpvt[pvt];
	    jpvt[pvt] = jpvt[i__];
	    jpvt[i__] = itemp;
...
/*        Generate elementary reflector H(i). */
	...
	if (offpi < *m) {
	    i__2 = *m - offpi + 1;
	    hlarfg_(&i__2, &a[offpi + i__ * a_dim1], &a[offpi + 1 + i__ * 
		    a_dim1], &c__1, &tau[i__]);
	} else {
	    hlarfg_(&c__1, &a[*m + i__ * a_dim1], &a[*m + i__ * a_dim1], &
		    c__1, &tau[i__]);
	}
...
/*           Apply H(i)**T to A(offset+i:m,i+1:n) from the left. */
	    ...
	    i__2 = *m - offpi + 1;
	    i__3 = *n - i__;
	    hlarf_("Left", &i__2, &i__3, &a[offpi + i__ * a_dim1], &c__1, &
...
/* Computing 2nd power */
		...
		if (temp2 <= tol3z) {
		    if (offpi < *m) {
			i__3 = *m - offpi;
			vn1[j] = hnrm2_(&i__3, &a[offpi + 1 + j * a_dim1], &
				c__1);
			vn2[j] = vn1[j];
		    } else {
			vn1[j] = 0.;
