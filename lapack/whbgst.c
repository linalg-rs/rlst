...
/*  ===================================================================== */
void  whbgst_(char *vect, char *uplo, integer *n, integer *ka, 
	integer *kb, quadcomplex *ab, integer *ldab, quadcomplex *bb, 
	integer *ldbb, quadcomplex *x, integer *ldx, quadcomplex *work, 
	quadreal *rwork, integer *info)
{
    /* System generated locals */
    integer ab_dim1, ab_offset, bb_dim1, bb_offset, x_dim1, x_offset, i__1, 
	    i__2, i__3, i__4, i__5, i__6, i__7, i__8;
    quadreal d__1;
    quadcomplex z__1, z__2, z__3, z__4, z__5, z__6, z__7, z__8, z__9, z__10;
...
/*     Initialize X to the unit matrix, if needed */
    ...
    if (wantx) {
	wlaset_("Full", n, n, &c_b1, &c_b2, &x[x_offset], ldx);
    }
...
/*              post-multiply X by inv(S(i)) */
		...
		i__3 = *n - m;
		d__1 = 1. / bii;
		wqscal_(&i__3, &d__1, &x[m + 1 + i__ * x_dim1], &c__1);
		if (kbt > 0) {
		    i__3 = *n - m;
		    z__1.r = -1., z__1.i = -0.;
		    wgerc_(&i__3, &kbt, &z__1, &x[m + 1 + i__ * x_dim1], &
...
/*                 generate rotation to annihilate a(i,i-k+ka+1) */
		    ...
		    wlartg_(&ab[k + 1 + (i__ - k + *ka) * ab_dim1], &ra1, &
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		wlargv_(&nrt, &ab[j2t * ab_dim1 + 1], &inca, &work[j2t - m], &
			ka1, &rwork[j2t - m], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the right */
		...
		i__4 = *ka - 1;
		for (l = 1; l <= i__4; ++l) {
		    wlartv_(&nr, &ab[ka1 - l + j2 * ab_dim1], &inca, &ab[*ka 
...
/*              blocks */
		...
		wlar2v_(&nr, &ab[ka1 + j2 * ab_dim1], &ab[ka1 + (j2 + 1) * 
			ab_dim1], &ab[*ka + (j2 + 1) * ab_dim1], &inca, &
			rwork[j2 - m], &work[j2 - m], &ka1);

		wlacgv_(&nr, &work[j2 - m], &ka1);
	    }
...
/*           start applying rotations in 1st set from the left */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[l + (j2 + ka1 - l) * ab_dim1], &inca, &
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j2; i__2 < 0 ? j >= i__4 : j <= i__4; j += i__2) {
		    ...
		    wrot_(&i__1, &x[m + 1 + j * x_dim1], &c__1, &x[m + 1 + (j 
...
/*           finish applying rotations in 2nd set from the left */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[l + (j2 - l + 1) * ab_dim1], &inca, &ab[
...
/*              which have been created outside the band */
		...
		wlargv_(&nr, &ab[j2 * ab_dim1 + 1], &inca, &work[j2], &ka1, &
			rwork[j2], &ka1);
...
/*              post-multiply X by product of rotations in 2nd set */
		...
		for (j = j2; i__2 < 0 ? j >= i__3 : j <= i__3; j += i__2) {
		    i__4 = *n - m;
		    d_cnjg(&z__1, &work[j]);
		    wrot_(&i__4, &x[m + 1 + j * x_dim1], &c__1, &x[m + 1 + (j 
...
/*              post-multiply X by inv(S(i)) */
		...
		i__4 = *n - m;
		d__1 = 1. / bii;
		wqscal_(&i__4, &d__1, &x[m + 1 + i__ * x_dim1], &c__1);
		if (kbt > 0) {
		    i__4 = *n - m;
		    z__1.r = -1., z__1.i = -0.;
		    i__3 = *ldbb - 1;
		    wgeru_(&i__4, &kbt, &z__1, &x[m + 1 + i__ * x_dim1], &
...
/*                 generate rotation to annihilate a(i-k+ka+1,i) */
		    ...
		    wlartg_(&ab[ka1 - k + i__ * ab_dim1], &ra1, &rwork[i__ - 
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		wlargv_(&nrt, &ab[ka1 + (j2t - *ka) * ab_dim1], &inca, &work[
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    wlartv_(&nr, &ab[l + 1 + (j2 - l) * ab_dim1], &inca, &ab[
...
/*              blocks */
		...
		wlar2v_(&nr, &ab[j2 * ab_dim1 + 1], &ab[(j2 + 1) * ab_dim1 + 
...
/*           start applying rotations in 1st set from the right */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[ka1 - l + 1 + j2 * ab_dim1], &inca, &ab[
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j2; i__3 < 0 ? j >= i__1 : j <= i__1; j += i__3) {
		    i__2 = *n - m;
		    wrot_(&i__2, &x[m + 1 + j * x_dim1], &c__1, &x[m + 1 + (j 
...
/*           finish applying rotations in 2nd set from the right */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[ka1 - l + 1 + (j2 - *ka) * ab_dim1], &
...
/*              which have been created outside the band */
		...
		wlargv_(&nr, &ab[ka1 + (j2 - *ka) * ab_dim1], &inca, &work[j2]
			, &ka1, &rwork[j2], &ka1);
...
/*              post-multiply X by inv(S(i)) */
		...
		d__1 = 1. / bii;
		wqscal_(&nx, &d__1, &x[i__ * x_dim1 + 1], &c__1);
		if (kbt > 0) {
		    z__1.r = -1., z__1.i = -0.;
		    i__3 = *ldbb - 1;
		    wgeru_(&nx, &kbt, &z__1, &x[i__ * x_dim1 + 1], &c__1, &bb[
...
/*                 generate rotation to annihilate a(i+k-ka-1,i) */
		    ...
		    wlartg_(&ab[k + 1 + i__ * ab_dim1], &ra1, &rwork[i__ + k 
			    - *ka], &work[i__ + k - *ka], &ra);
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		wlargv_(&nrt, &ab[(j1 + *ka) * ab_dim1 + 1], &inca, &work[j1],
			 &ka1, &rwork[j1], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    wlartv_(&nr, &ab[ka1 - l + (j1 + l) * ab_dim1], &inca, &
...
/*              blocks */
		...
		wlar2v_(&nr, &ab[ka1 + j1 * ab_dim1], &ab[ka1 + (j1 - 1) * 
			ab_dim1], &ab[*ka + j1 * ab_dim1], &inca, &rwork[j1], 
			&work[j1], &ka1);

		wlacgv_(&nr, &work[j1], &ka1);
	    }
...
/*           start applying rotations in 1st set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[l + j1t * ab_dim1], &inca, &ab[l + 1 + (
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j1; i__4 < 0 ? j >= i__1 : j <= i__1; j += i__4) {
		    wrot_(&nx, &x[j * x_dim1 + 1], &c__1, &x[(j - 1) * x_dim1 
...
/*           finish applying rotations in 2nd set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[l + (j1t + *ka) * ab_dim1], &inca, &ab[
...
/*              which have been created outside the band */
		...
		wlargv_(&nr, &ab[(j1 + *ka) * ab_dim1 + 1], &inca, &work[m - *
...
/*              post-multiply X by inv(S(i)) */
		...
		if (kbt > 0) {
		    z__1.r = -1., z__1.i = -0.;
		    wgerc_(&nx, &kbt, &z__1, &x[i__ * x_dim1 + 1], &c__1, &bb[
...
/*                 generate rotation to annihilate a(i,i+k-ka-1) */
		    ...
		    wlartg_(&ab[ka1 - k + (i__ + k - *ka) * ab_dim1], &ra1, &
...
/*           have been created outside the band */
	    ...
	    if (nrt > 0) {
		wlargv_(&nrt, &ab[ka1 + j1 * ab_dim1], &inca, &work[j1], &ka1,
			 &rwork[j1], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the right */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    wlartv_(&nr, &ab[l + 1 + j1 * ab_dim1], &inca, &ab[l + 2 
...
/*              blocks */
		...
		wlar2v_(&nr, &ab[j1 * ab_dim1 + 1], &ab[(j1 - 1) * ab_dim1 + 
...
/*           start applying rotations in 1st set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[ka1 - l + 1 + (j1t - ka1 + l) * ab_dim1]
...
/*           finish applying rotations in 2nd set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    wlartv_(&nrt, &ab[ka1 - l + 1 + (j1t + l - 1) * ab_dim1], 
...
/*              which have been created outside the band */
		...
		wlargv_(&nr, &ab[ka1 + j1 * ab_dim1], &inca, &work[m - *kb + 
