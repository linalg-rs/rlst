...
/* >          will have abs(doublereal part) + abs(imag. part) = 1. */
...
/* > */
/*  ===================================================================== */
void  wggevx_(char *balanc, char *jobvl, char *jobvr, char *
	sense, integer *n, quadcomplex *a, integer *lda, quadcomplex *b, 
	integer *ldb, quadcomplex *alpha, quadcomplex *beta, 
	...
	rcondv, quadcomplex *work, integer *lwork, quadreal *rwork, 
	integer *iwork, logical *bwork, integer *info)
{
    ...
    extern void  qlascl_(char *, integer *, integer *, 
	    ...
	    integer *, integer *), wggbak_(char *, char *, integer *, 
	    integer *, integer *, quadreal *, quadreal *, integer *, 
	    quadcomplex *, integer *, integer *), wggbal_(
    ...
    extern void  wlacpy_(char *, integer *, integer *, 
	    ...
	    quadcomplex *, integer *, integer *, integer *, quadcomplex *,
	     quadreal *, integer *), wtgsna_(char *, char *,
	     logical *, integer *, quadcomplex *, integer *, quadcomplex *
	    , integer *, quadcomplex *, integer *, quadcomplex *, integer 
	    *, quadreal *, quadreal *, integer *, integer *, 
	    quadcomplex *, integer *, integer *, integer *);
    integer minwrk;
    extern void  whgeqz_(char *, char *, char *, integer *, 
...
/*     (Real Workspace: need 6*N if BALANC = 'S' or 'B', 1 otherwise) */
    ...
    wggbal_(balanc, n, &a[a_offset], lda, &b[b_offset], ldb, ilo, ihi, &
	    lscale[1], &rscale[1], &rwork[1], &ierr);

/*     Compute ABNRM and BBNRM */

    *abnrm = wlange_("1", n, n, &a[a_offset], lda, &rwork[1]);
    if (ilascl) {
	rwork[1] = *abnrm;
	qlascl_("G", &c__0, &c__0, &anrmto, &anrm, &c__1, &c__1, &rwork[1], &
		c__1, &ierr);
	*abnrm = rwork[1];
    }

    *bbnrm = wlange_("1", n, n, &b[b_offset], ldb, &rwork[1]);
    if (ilbscl) {
	rwork[1] = *bbnrm;
	qlascl_("G", &c__0, &c__0, &bnrmto, &bnrm, &c__1, &c__1, &rwork[1], &
		c__1, &ierr);
	*bbnrm = rwork[1];
    }
...
/*     (Workspace: need N, prefer N*NB) */
	...
	i__1 = *lwork + 1 - iwrk;
	wungqr_(&irows, &irows, &irows, &vl[*ilo + *ilo * vl_dim1], ldvl, &
...
/*        Eigenvectors requested -- work on whole matrix. */
	...
	wgghrd_("N", "N", &irows, &c__1, &irows, &a[*ilo + *ilo * a_dim1], 
...
/*     (Real Workspace: need N) */
	...
	*(unsigned char *)chtemp = 'E';
    ...
    i__1 = *lwork + 1 - iwrk;
    whgeqz_(chtemp, jobvl, jobvr, n, ilo, ihi, &a[a_offset], lda, &b[b_offset]
...
/*             (Integer Workspace: need N+2 ) */
	...
	if (ilv) {
		...
		*(unsigned char *)chtemp = 'R';
	    ...
	    wtgevc_(chtemp, "B", ldumma, n, &a[a_offset], lda, &b[b_offset], 
...
/* L10: */
		...
		if (wantse || wantsb) {
		    wtgevc_("B", "S", &bwork[1], n, &a[a_offset], lda, &b[
		...
		i__2 = *lwork - iwrk1 + 1;
		wtgsna_(sense, "S", &bwork[1], n, &a[a_offset], lda, &b[
...
/*     (Workspace: none needed) */
    ...
    if (ilvl) {
	wggbak_(balanc, "L", n, ilo, ihi, &lscale[1], &rscale[1], n, &vl[
		vl_offset], ldvl, &ierr);
...
L50:
    ...
    if (ilvr) {
	wggbak_(balanc, "R", n, ilo, ihi, &lscale[1], &rscale[1], n, &vr[
		vr_offset], ldvr, &ierr);
	i__1 = *n;
	for (jc = 1; jc <= i__1; ++jc) {
	    temp = 0.;
