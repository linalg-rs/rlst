...
/*  ===================================================================== */
void  wgetsls_(char *trans, integer *m, integer *n, integer *
	...
	quadcomplex *work, integer *lwork, integer *info)
{
    ...
    integer minmn, maxmn;
    extern void  wgelq_(integer *, integer *, quadcomplex *, 
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *,
	     integer *), wgeqr_(integer *, integer *, quadcomplex *, 
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *,
	     integer *);
    quadcomplex workq[1];
...
/*     Determine the block size and minimum LWORK */
	...
	if (*m >= *n) {
	    wgeqr_(m, n, &a[a_offset], lda, tq, &c_n1, workq, &c_n1, &info2);
	    tszo = (integer) tq[0].r;
	    lwo = (integer) workq[0].r;
	    wgemqr_("L", trans, m, nrhs, n, &a[a_offset], lda, tq, &tszo, &b[
		    b_offset], ldb, workq, &c_n1, &info2);
/* Computing MAX */
	    i__1 = lwo, i__2 = (integer) workq[0].r;
	    lwo = f2cmax(i__1,i__2);
	    wgeqr_(m, n, &a[a_offset], lda, tq, &c_n2, workq, &c_n2, &info2);
	    tszm = (integer) tq[0].r;
	    lwm = (integer) workq[0].r;
	    wgemqr_("L", trans, m, nrhs, n, &a[a_offset], lda, tq, &tszm, &b[
		    b_offset], ldb, workq, &c_n1, &info2);
/* Computing MAX */
	    ...
	    wgelq_(m, n, &a[a_offset], lda, tq, &c_n1, workq, &c_n1, &info2);
	    tszo = (integer) tq[0].r;
	    lwo = (integer) workq[0].r;
	    wgemlq_("L", trans, n, nrhs, m, &a[a_offset], lda, tq, &tszo, &b[
		    b_offset], ldb, workq, &c_n1, &info2);
/* Computing MAX */
	    i__1 = lwo, i__2 = (integer) workq[0].r;
	    lwo = f2cmax(i__1,i__2);
	    wgelq_(m, n, &a[a_offset], lda, tq, &c_n2, workq, &c_n2, &info2);
	    tszm = (integer) tq[0].r;
	    lwm = (integer) workq[0].r;
	    wgemlq_("L", trans, n, nrhs, m, &a[a_offset], lda, tq, &tszo, &b[
		    b_offset], ldb, workq, &c_n1, &info2);
...
/*        Matrix all zero. Return zero solution. */
	...
	wlaset_("F", &maxmn, nrhs, &c_b1, &c_b1, &b[b_offset], ldb)
		;
	goto L50;
    }
    ...
    bnrm = wlange_("M", &brow, nrhs, &b[b_offset], ldb, dum);
    ibscl = 0;
...
/*        compute QR factorization of A */
	...
	wgeqr_(m, n, &a[a_offset], lda, &work[lw2 + 1], &lw1, &work[1], &lw2, 
		info);
	if (! tran) {
...
/*           B(1:M,1:NRHS) := Q**T * B(1:M,1:NRHS) */
	    ...
	    wgemqr_("L", "C", m, nrhs, n, &a[a_offset], lda, &work[lw2 + 1], &
		    lw1, &b[b_offset], ldb, &work[1], &lw2, info);

/*           B(1:N,1:NRHS) := inv(R) * B(1:N,1:NRHS) */

	    wtrtrs_("U", "N", "N", n, nrhs, &a[a_offset], lda, &b[b_offset], 
		    ldb, info);
	    if (*info > 0) {
		return;
	    }
...
/*           B(1:N,1:NRHS) := inv(R**T) * B(1:N,1:NRHS) */
	    ...
	    wtrtrs_("U", "C", "N", n, nrhs, &a[a_offset], lda, &b[b_offset], 
		    ldb, info);
...
/*           B(1:M,1:NRHS) := Q(1:N,:) * B(1:N,1:NRHS) */
	    ...
	    wgemqr_("L", "N", m, nrhs, n, &a[a_offset], lda, &work[lw2 + 1], &
		    lw1, &b[b_offset], ldb, &work[1], &lw2, info);
...
/*        Compute LQ factorization of A */
	...
	wgelq_(m, n, &a[a_offset], lda, &work[lw2 + 1], &lw1, &work[1], &lw2, 
		info);
...
/*           B(1:M,1:NRHS) := inv(L) * B(1:M,1:NRHS) */
	    ...
	    wtrtrs_("L", "N", "N", m, nrhs, &a[a_offset], lda, &b[b_offset], 
		    ldb, info);
...
/*           B(1:N,1:NRHS) := Q(1:N,:)**T * B(1:M,1:NRHS) */
	    ...
	    wgemlq_("L", "C", n, nrhs, m, &a[a_offset], lda, &work[lw2 + 1], &
		    lw1, &b[b_offset], ldb, &work[1], &lw2, info);
...
/*           B(1:N,1:NRHS) := Q * B(1:N,1:NRHS) */
	    ...
	    wgemlq_("L", "N", n, nrhs, m, &a[a_offset], lda, &work[lw2 + 1], &
		    lw1, &b[b_offset], ldb, &work[1], &lw2, info);
...
/*           B(1:M,1:NRHS) := inv(L**T) * B(1:M,1:NRHS) */
	    ...
	    wtrtrs_("L", "C", "N", m, nrhs, &a[a_offset], lda, &b[b_offset], 
		    ldb, info);
