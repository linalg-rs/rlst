...
/* > */
/*  ===================================================================== */
void  qtgevc_(char *side, char *howmny, logical *select, 
	integer *n, quadreal *s, integer *lds, quadreal *p, integer *ldp, 
	quadreal *vl, integer *ldvl, quadreal *vr, integer *ldvr, integer 
	*mm, integer *m, quadreal *work, integer *info)
{
    ...
    quadreal big;
    logical lsa, lsb;
    quadreal ulp, sum[4]	/* was [2][2] */;
    integer ibeg, ieig, iend;
    quadreal dmin__, temp, xmax, sump[4]	/* was [2][2] */, sums[4]	
	    /* was [2][2] */;
    extern void  qlag2_(quadreal *, integer *, quadreal *, 
	    integer *, quadreal *, quadreal *, quadreal *, quadreal *,
	     quadreal *, quadreal *);
    quadreal cim2a, cim2b, cre2a, cre2b, temp2, bdiag[2], acoef, scale;
    logical ilall;
    integer iside;
    quadreal sbeta;
    ...
    quadreal acoefa, bcoefa, cimaga, cimagb;
    logical ilback;
    quadreal bcoefi, ascale, bscale, creala, crealb;
    extern quadreal qlamch_(char *);
    quadreal bcoefr, salfar, safmin;
    extern void  qlacpy_(char *, integer *, integer *, 
	    quadreal *, integer *, quadreal *, integer *);
    quadreal xscale, bignum;
...
/*              Complex eigenvalue */
		...
		d__1 = safmin * 100.;
		qlag2_(&s[je + je * s_dim1], lds, &p[je + je * p_dim1], ldp, &
...
/*              with scaling and perturbation of the denominator */
		...
		qlaln2_(&c_true, &na, &nw, &dmin__, &acoef, &s[j + j * s_dim1]
...
/*           HOWMNY='B'. */
		...
		for (jw = 0; jw <= i__2; ++jw) {
		    i__3 = *n + 1 - je;
		    qgemv_("N", n, &i__3, &c_b34, &vl[je * vl_dim1 + 1], ldvl,
			     &work[(jw + 2) * *n + je], &c__1, &c_b36, &work[(
			    jw + 4) * *n + 1], &c__1);
/* L170: */
		}
		qlacpy_(" ", n, &nw, &work[(*n << 2) + 1], n, &vl[je * 
			vl_dim1 + 1], ldvl);
		ibeg = 1;
	    } else {
		qlacpy_(" ", n, &nw, &work[(*n << 1) + 1], n, &vl[ieig * 
			vl_dim1 + 1], ldvl);
		ibeg = je;
	    }
...
/*              Complex eigenvalue */
		...
		d__1 = safmin * 100.;
		qlag2_(&s[je - 1 + (je - 1) * s_dim1], lds, &p[je - 1 + (je - 
...
/*              Compute x(j) (and x(j+1), if 2-by-2 block) */
		...
		qlaln2_(&c_false, &na, &nw, &dmin__, &acoef, &s[j + j * 
