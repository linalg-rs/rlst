...
/* > \verbatim */
/* > */
/* > DTPMQRT applies a doublereal orthogonal matrix Q obtained from a */
/* > "triangular-pentagonal" doublereal block reflector H to a general */
/* > doublereal matrix C, which consists of two blocks A and B. */
/* > \endverbatim */
...
/* > */
/* >  The doublereal orthogonal matrix Q is formed from V and T. */
/* > */
...
/* > */
/*  ===================================================================== */
void  htpmqrt_(char *side, char *trans, integer *m, integer *n,
	 integer *k, integer *l, integer *nb, halfreal *v, integer *ldv, 
	halfreal *t, integer *ldt, halfreal *a, integer *lda, halfreal *
	b, integer *ldb, halfreal *work, integer *info)
{
    ...
    logical right;
    extern void  xerbla_(char *, integer *), htprfb_(
...
/* Computing MIN */
	    ...
	    htprfb_("L", "T", "F", "C", &mb, n, &ib, &lb, &v[i__ * v_dim1 + 1]
...
/* Computing MIN */
	    ...
	    htprfb_("R", "N", "F", "C", m, &mb, &ib, &lb, &v[i__ * v_dim1 + 1]
...
/* Computing MIN */
	    ...
	    htprfb_("L", "N", "F", "C", &mb, n, &ib, &lb, &v[i__ * v_dim1 + 1]
		    , ldv, &t[i__ * t_dim1 + 1], ldt, &a[i__ + a_dim1], lda, &
		    b[b_offset], ldb, &work[1], &ib);
	}
...
/* Computing MIN */
	    ...
	    htprfb_("R", "T", "F", "C", m, &mb, &ib, &lb, &v[i__ * v_dim1 + 1]
		    , ldv, &t[i__ * t_dim1 + 1], ldt, &a[i__ * a_dim1 + 1], 
		    lda, &b[b_offset], ldb, &work[1], m);
	}
