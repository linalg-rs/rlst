...
/* > */
/*  ===================================================================== */
void  kgesvj_(char *joba, char *jobu, char *jobv, integer *m, 
	integer *n, halfcomplex *a, integer *lda, halfreal *sva, integer *
	mv, halfcomplex *v, integer *ldv, halfcomplex *cwork, integer *
	lwork, halfreal *rwork, integer *lrwork, integer *info)
{
    ...
    extern void  kgsvj0_(char *, integer *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, halfreal *, 
	    integer *, halfcomplex *, integer *, halfreal *, halfreal *,
	     halfreal *, integer *, halfcomplex *, integer *, integer *), kgsvj1_(char *, integer *, integer *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, halfreal *, 
	    integer *, halfcomplex *, integer *, halfreal *, halfreal *,
	     halfreal *, integer *, halfcomplex *, integer *, integer *);
...
/*     #:( */
	...
	i__1 = *m + *n;
	cwork[1].r = (halfreal) i__1, cwork[1].i = 0.;
	rwork[1] = (halfreal) f2cmax(*n,6);
	return;
    }
...
/*     Initialize the right singular vector matrix. */
    ...
    if (rsvec) {
	mvl = *n;
	klaset_("A", &mvl, n, &c_b1, &c_b2, &v[v_offset], ldv);
    } else if (applv) {
	mvl = *mv;
    }
    rsvec = rsvec || applv;
...
/*        the input matrix is M-by-N lower triangular (trapezoidal) */
	...
	for (p = 1; p <= i__1; ++p) {
	    ...
	    aaqq = 1.;
	    i__2 = *m - p + 1;
	    klassq_(&i__2, &a[p + p * a_dim1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("ZGESVJ", &i__2);
...
/*        the input matrix is M-by-N upper triangular (trapezoidal) */
	...
	for (p = 1; p <= i__1; ++p) {
	    aapp = 0.;
	    aaqq = 1.;
	    klassq_(&p, &a[p * a_dim1 + 1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("ZGESVJ", &i__2);
...
/*        the input matrix is M-by-N general dense */
	...
	for (p = 1; p <= i__1; ++p) {
	    aapp = 0.;
	    aaqq = 1.;
	    klassq_(m, &a[p * a_dim1 + 1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("ZGESVJ", &i__2);
...
/* #:) Quick return for zero matrix */
    ...
    if (aapp == 0.) {
	if (lsvec) {
	    klaset_("G", m, n, &c_b1, &c_b2, &a[a_offset], lda);
	}
	rwork[1] = 1.;
	rwork[2] = 0.;
	rwork[3] = 0.;
...
/* #:) Quick return for one-column matrix */
    ...
    if (*n == 1) {
	if (lsvec) {
	    klascl_("G", &c__0, &c__0, &sva[1], &skl, m, &c__1, &a[a_dim1 + 1]
		    , lda, &ierr);
	}
	rwork[1] = 1. / skl;
	if (sva[1] >= sfmin) {
...
/*     Scale, if necessary */
    ...
    if (temp1 != 1.) {
	hlascl_("G", &c__0, &c__0, &c_b42, &temp1, n, &c__1, &sva[1], n, &
		ierr);
    }
    skl = temp1 * skl;
    if (skl != 1.) {
	klascl_(joba, &c__0, &c__0, &c_b42, &skl, m, n, &a[a_offset], lda, &
		ierr);
	skl = 1. / skl;
    }
...
/*     [+ + x x]                    [x x].             [x x] */
	    ...
	    i__2 = *n - n34;
	    i__3 = *lwork - *n;
	    kgsvj0_(jobv, &i__1, &i__2, &a[n34 + 1 + (n34 + 1) * a_dim1], lda,
	    ...
	    i__2 = n34 - n2;
	    i__3 = *lwork - *n;
	    kgsvj0_(jobv, &i__1, &i__2, &a[n2 + 1 + (n2 + 1) * a_dim1], lda, &
	    ...
	    i__2 = *n - n2;
	    i__3 = *lwork - *n;
	    kgsvj1_(jobv, &i__1, &i__2, &n4, &a[n2 + 1 + (n2 + 1) * a_dim1], 
	    ...
	    i__2 = n2 - n4;
	    i__3 = *lwork - *n;
	    kgsvj0_(jobv, &i__1, &i__2, &a[n4 + 1 + (n4 + 1) * a_dim1], lda, &
		    cwork[n4 + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 
		    1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &cwork[*n 
		    + 1], &i__3, &ierr);

	    i__1 = *lwork - *n;
	    kgsvj0_(jobv, m, &n4, &a[a_offset], lda, &cwork[1], &sva[1], &mvl,
		     &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &cwork[*
		    n + 1], &i__1, &ierr);

	    i__1 = *lwork - *n;
	    kgsvj1_(jobv, m, &n2, &n4, &a[a_offset], lda, &cwork[1], &sva[1], 
	    ...
	    i__1 = *lwork - *n;
	    kgsvj0_(jobv, &n4, &n4, &a[a_offset], lda, &cwork[1], &sva[1], &
		    mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__2, &
		    cwork[*n + 1], &i__1, &ierr);

	    i__1 = *lwork - *n;
	    kgsvj0_(jobv, &n2, &n4, &a[(n4 + 1) * a_dim1 + 1], lda, &cwork[n4 
		    + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 1) * 
		    v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &cwork[*n + 1],
		     &i__1, &ierr);

	    i__1 = *lwork - *n;
	    kgsvj1_(jobv, &n2, &n2, &n4, &a[a_offset], lda, &cwork[1], &sva[1]
		    , &mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &
		    cwork[*n + 1], &i__1, &ierr);

	    i__1 = n2 + n4;
	    i__2 = *lwork - *n;
	    kgsvj0_(jobv, &i__1, &n4, &a[(n2 + 1) * a_dim1 + 1], lda, &cwork[
...
/* Computing MAX */
			  ...
			  krot_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
				  v_dim1 + 1], &c__1, &cs, &z__1);
					...
					kaxpy_(m, &z__1, &cwork[*n + 1], &
						c__1, &a[q * a_dim1 + 1], &
						c__1);
					klascl_("G", &c__0, &c__0, &c_b42, &
...
/*            CALL ZDSCAL( M, ONE / SVA( p ), A( 1, p ), 1 ) */
	    klascl_("G", &c__0, &c__0, &sva[p], &c_b42, m, &c__1, &a[p * 
		    a_dim1 + 1], m, &ierr);
/* L1998: */
	}
    }
...
/*     Scale the product of Jacobi rotations. */
	...
	i__1 = *n;
	for (p = 1; p <= i__1; ++p) {
	    temp1 = 1. / hwnrm2_(&mvl, &v[p * v_dim1 + 1], &c__1);
	    whscal_(&mvl, &temp1, &v[p * v_dim1 + 1], &c__1);
/* L2399: */
	}
    }
