...
/* > \verbatim */
/* > */
/* > ZGEMQR overwrites the general doublereal M-by-N matrix C with */
/* > */
/* >                      SIDE = 'L'     SIDE = 'R' */
/* >      TRANS = 'N':      Q * C          C * Q */
/* >      TRANS = 'T':      Q**H * C       C * Q**H */
...
/* > */
/*  ===================================================================== */
void  kgemqr_(char *side, char *trans, integer *m, integer *n, 
	...
	integer *lwork, integer *info)
{
    ...
    /* Local variables */
    extern void  klamtsqr_(char *, char *, integer *, integer *
...
/* Computing MAX */
    ...
    if (left && *m <= *k || right && *n <= *k || mb <= *k || mb >= f2cmax(i__1,*
	    k)) {
	kgemqrt_(side, trans, m, n, k, &nb, &a[a_offset], lda, &t[6], &nb, &
		c__[c_offset], ldc, &work[1], info);
    } else {
	klamtsqr_(side, trans, m, n, k, &mb, &nb, &a[a_offset], lda, &t[6], &
