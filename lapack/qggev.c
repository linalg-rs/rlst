...
/* > \verbatim */
/* > */
/* > DGGEV computes for a pair of N-by-N doublereal nonsymmetric matrices (A,B) */
...
/*  ===================================================================== */
void  qggev_(char *jobvl, char *jobvr, integer *n, quadreal *
	a, integer *lda, quadreal *b, integer *ldb, quadreal *alphar, 
	quadreal *alphai, quadreal *beta, quadreal *vl, integer *ldvl, 
	quadreal *vr, integer *ldvr, quadreal *work, integer *lwork, 
	integer *info)
{
    ...
    extern void  qgeqrf_(integer *, integer *, quadreal *, 
	    ...
	    integer *, quadreal *, quadreal *, quadreal *, integer *), qtgevc_(char *, char *, logical *, integer *, quadreal 
	    *, integer *, quadreal *, integer *, quadreal *, integer *, 
	    quadreal *, integer *, integer *, integer *, quadreal *, 
	    integer *);
    logical ldumma[1];
...
/*     (Workspace: need N, prefer N*NB) */
    ...
    if (ilvl) {
	qlaset_("Full", n, n, &c_b36, &c_b37, &vl[vl_offset], ldvl)
		;
	if (irows > 1) {
	    i__1 = irows - 1;
	    i__2 = irows - 1;
	...
	i__1 = *lwork + 1 - iwrk;
	qorgqr_(&irows, &irows, &irows, &vl[ilo + ilo * vl_dim1], ldvl, &work[
...
/*     Initialize VR */
    ...
    if (ilvr) {
	qlaset_("Full", n, n, &c_b36, &c_b37, &vr[vr_offset], ldvr)
		;
    }
...
/*        Eigenvectors requested -- work on whole matrix. */
	...
	qgghrd_("N", "N", &irows, &c__1, &irows, &a[ilo + ilo * a_dim1], lda, 
...
/*     (Workspace: need N) */
	...
	*(unsigned char *)chtemp = 'E';
    ...
    i__1 = *lwork + 1 - iwrk;
    qhgeqz_(chtemp, jobvl, jobvr, n, &ilo, &ihi, &a[a_offset], lda, &b[
...
/*     (Workspace: need 6*N) */
	...
	qtgevc_(chtemp, "B", ldumma, n, &a[a_offset], lda, &b[b_offset], ldb, 
...
/*        (Workspace: none needed) */
	...
	if (ilvl) {
	    qggbak_("P", "L", n, &ilo, &ihi, &work[ileft], &work[iright], n, &
...
L50:
	...
	if (ilvr) {
	    qggbak_("P", "R", n, &ilo, &ihi, &work[ileft], &work[iright], n, &
