...
/* > */
/*  ===================================================================== */
void  wlahqr_(logical *wantt, logical *wantz, integer *n, 
	...
	integer *ldz, integer *info)
{
    ...
    quadcomplex z__1, z__2, z__3, z__4, z__5, z__6, z__7;

    /* Local variables */
    integer i__, j, k, l, m;
    quadreal s;
    quadcomplex t, u, v[2], x, y;
    integer i1, i2;
    quadcomplex t1;
    quadreal t2;
    quadcomplex v2;
    quadreal aa, ab, ba, bb, h10;
    quadcomplex h11;
    quadreal h21;
    quadcomplex h22, sc;
    integer nh, nz;
...
/*           .    sudden underflow in ABS(H(I,I-1)) ==== */
	    ...
	    i__2 = jhi - i__ + 1;
	    wscal_(&i__2, &sc, &h__[i__ + i__ * h_dim1], ldh);
/* Computing MIN */
	    i__3 = jhi, i__4 = i__ + 1;
	    i__2 = f2cmin(i__3,i__4) - jlo + 1;
	    d_cnjg(&z__1, &sc);
	    wscal_(&i__2, &z__1, &h__[jlo + i__ * h_dim1], &c__1);
	    if (*wantz) {
		i__2 = *ihiz - *iloz + 1;
		d_cnjg(&z__1, &sc);
		wscal_(&i__2, &z__1, &z__[*iloz + i__ * z_dim1], &c__1);
	    }
	}
/* L20: */
    }
...
/*           after the call T2 ( = T1*V(2) ) is also doublereal. */
	    ...
	    if (k > m) {
		wcopy_(&c__2, &h__[k + (k - 1) * h_dim1], &c__1, v, &c__1);
	    }
	    wlarfg_(&c__2, v, &v[1], &c__1, &t1);
	    if (k > m) {
		i__3 = k + (k - 1) * h_dim1;
...
/*              doublereal. */
		...
		for (j = m; j <= i__3; ++j) {
			...
			if (i2 > j) {
			    i__4 = i2 - j;
			    wscal_(&i__4, &temp, &h__[j + (j + 1) * h_dim1], 
				    ldh);
			}
			i__4 = j - i1;
			d_cnjg(&z__1, &temp);
			wscal_(&i__4, &z__1, &h__[i1 + j * h_dim1], &c__1);
			if (*wantz) {
			    d_cnjg(&z__1, &temp);
			    wscal_(&nz, &z__1, &z__[*iloz + j * z_dim1], &
				    c__1);
			}
		    }
/* L110: */
...
/*        Ensure that H(I,I-1) is doublereal. */
	...
	if (d_imag(&temp) != 0.) {
		...
		i__2 = i2 - i__;
		d_cnjg(&z__1, &temp);
		wscal_(&i__2, &z__1, &h__[i__ + (i__ + 1) * h_dim1], ldh);
	    }
	    i__2 = i__ - i1;
	    wscal_(&i__2, &temp, &h__[i1 + i__ * h_dim1], &c__1);
	    if (*wantz) {
		wscal_(&nz, &temp, &z__[*iloz + i__ * z_dim1], &c__1);
	    }
	}
