...
/* > of T are non-negative doublereal numbers. */
/* > \endverbatim */
...
/* >          output by ZGGES3. The  BETA(j) will be non-negative doublereal. */
/* > */
...
/*  ===================================================================== */
void  kgges3_(char *jobvsl, char *jobvsr, char *sort, L_fp 
	selctg, integer *n, halfcomplex *a, integer *lda, halfcomplex *b, 
	integer *ldb, integer *sdim, halfcomplex *alpha, halfcomplex *
	beta, halfcomplex *vsl, integer *ldvsl, halfcomplex *vsr, integer 
	*ldvsr, halfcomplex *work, integer *lwork, halfreal *rwork, 
	logical *bwork, integer *info)
{
    ...
    integer irwrk, irows;
    extern void  kgghd3_(char *, char *, integer *, integer *, 
	    integer *, halfcomplex *, integer *, halfcomplex *, integer *,
	     halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *, integer *), hlabad_(
	    halfreal *, halfreal *);
    extern halfreal hlamch_(char *);
    extern void  kggbak_(char *, char *, integer *, integer *, 
	    integer *, halfreal *, halfreal *, integer *, halfcomplex *,
	     integer *, integer *), kggbal_(char *, integer *,
    ...
    extern void  klacpy_(char *, integer *, integer *, 
	    ...
	    halfcomplex *, halfcomplex *, integer *), khgeqz_(
	    char *, char *, char *, integer *, integer *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, halfcomplex *, halfcomplex *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfreal *, integer *), ktgsen_(integer 
	    *, logical *, logical *, logical *, integer *, halfcomplex *, 
    ...
    extern void  kungqr_(integer *, integer *, integer *, 
	    ...
	    integer *, integer *), kunmqr_(char *, char *, integer *, integer 
	    *, integer *, halfcomplex *, integer *, halfcomplex *, 
	    halfcomplex *, integer *, halfcomplex *, integer *, integer *);
...
/*     Compute workspace */
    ...
    if (*info == 0) {
	kgeqrf_(n, n, &b[b_offset], ldb, &work[1], &work[1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = 1, i__2 = *n + (integer) work[1].r;
	lwkopt = f2cmax(i__1,i__2);
	kunmqr_("L", "C", n, n, n, &b[b_offset], ldb, &work[1], &a[a_offset], 
		lda, &work[1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = lwkopt, i__2 = *n + (integer) work[1].r;
	lwkopt = f2cmax(i__1,i__2);
	if (ilvsl) {
	    kungqr_(n, n, n, &vsl[vsl_offset], ldvsl, &work[1], &work[1], &
		    c_n1, &ierr);
/* Computing MAX */
	    i__1 = lwkopt, i__2 = *n + (integer) work[1].r;
	    lwkopt = f2cmax(i__1,i__2);
	}
	kgghd3_(jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[b_offset], 
		ldb, &vsl[vsl_offset], ldvsl, &vsr[vsr_offset], ldvsr, &work[
		1], &c_n1, &ierr);
/* Computing MAX */
	i__1 = lwkopt, i__2 = *n + (integer) work[1].r;
	lwkopt = f2cmax(i__1,i__2);
	khgeqz_("S", jobvsl, jobvsr, n, &c__1, n, &a[a_offset], lda, &b[
...
/* Computing MAX */
	...
	lwkopt = f2cmax(i__1,i__2);
	if (wantst) {
	    ktgsen_(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &
...
/*     Scale A if f2cmax element outside range [SMLNUM,BIGNUM] */
    ...
    anrm = klange_("M", n, n, &a[a_offset], lda, &rwork[1]);
    ilascl = FALSE_;
    ...
    if (ilascl) {
	klascl_("G", &c__0, &c__0, &anrm, &anrmto, n, n, &a[a_offset], lda, &
		ierr);
    }

/*     Scale B if f2cmax element outside range [SMLNUM,BIGNUM] */

    bnrm = klange_("M", n, n, &b[b_offset], ldb, &rwork[1]);
    ilbscl = FALSE_;
    ...
    if (ilbscl) {
	klascl_("G", &c__0, &c__0, &bnrm, &bnrmto, n, n, &b[b_offset], ldb, &
		ierr);
    }
...
/*     Permute the matrix to make it more nearly triangular */
    ...
    iright = *n + 1;
    irwrk = iright + *n;
    kggbal_("P", n, &a[a_offset], lda, &b[b_offset], ldb, &ilo, &ihi, &rwork[
...
/*     Reduce B to triangular form (QR decomposition of B) */
    ...
    iwrk = itau + irows;
    i__1 = *lwork + 1 - iwrk;
    kgeqrf_(&irows, &icols, &b[ilo + ilo * b_dim1], ldb, &work[itau], &work[
	    iwrk], &i__1, &ierr);

/*     Apply the orthogonal transformation to matrix A */

    i__1 = *lwork + 1 - iwrk;
    kunmqr_("L", "C", &irows, &icols, &irows, &b[ilo + ilo * b_dim1], ldb, &
...
/*     Initialize VSL */
    ...
    if (ilvsl) {
	klaset_("Full", n, n, &c_b1, &c_b2, &vsl[vsl_offset], ldvsl);
	if (irows > 1) {
	    i__1 = irows - 1;
	    i__2 = irows - 1;
	    klacpy_("L", &i__1, &i__2, &b[ilo + 1 + ilo * b_dim1], ldb, &vsl[
		    ilo + 1 + ilo * vsl_dim1], ldvsl);
	}
	i__1 = *lwork + 1 - iwrk;
	kungqr_(&irows, &irows, &irows, &vsl[ilo + ilo * vsl_dim1], ldvsl, &
...
/*     Initialize VSR */
    ...
    if (ilvsr) {
	klaset_("Full", n, n, &c_b1, &c_b2, &vsr[vsr_offset], ldvsr);
    }

/*     Reduce to generalized Hessenberg form */

    i__1 = *lwork + 1 - iwrk;
    kgghd3_(jobvsl, jobvsr, n, &ilo, &ihi, &a[a_offset], lda, &b[b_offset], 
...
/*     Perform QZ algorithm, computing Schur vectors if desired */
    ...
    iwrk = itau;
    i__1 = *lwork + 1 - iwrk;
    khgeqz_("S", jobvsl, jobvsr, n, &ilo, &ihi, &a[a_offset], lda, &b[
	    b_offset], ldb, &alpha[1], &beta[1], &vsl[vsl_offset], ldvsl, &
...
/*        Undo scaling on eigenvalues before selecting */
	...
	if (ilascl) {
	    klascl_("G", &c__0, &c__0, &anrm, &anrmto, n, &c__1, &alpha[1], n,
		     &ierr);
	}
	if (ilbscl) {
	    klascl_("G", &c__0, &c__0, &bnrm, &bnrmto, n, &c__1, &beta[1], n, 
		    &ierr);
	}
...
/* L10: */
	...
	i__1 = *lwork - iwrk + 1;
	ktgsen_(&c__0, &ilvsl, &ilvsr, &bwork[1], n, &a[a_offset], lda, &b[
		b_offset], ldb, &alpha[1], &beta[1], &vsl[vsl_offset], ldvsl, 
...
/*     Apply back-permutation to VSL and VSR */
    ...
    if (ilvsl) {
	kggbak_("P", "L", n, &ilo, &ihi, &rwork[ileft], &rwork[iright], n, &
		vsl[vsl_offset], ldvsl, &ierr);
    }
    if (ilvsr) {
	kggbak_("P", "R", n, &ilo, &ihi, &rwork[ileft], &rwork[iright], n, &
...
/*     Undo scaling */
    ...
    if (ilascl) {
	klascl_("U", &c__0, &c__0, &anrmto, &anrm, n, n, &a[a_offset], lda, &
		ierr);
	klascl_("G", &c__0, &c__0, &anrmto, &anrm, n, &c__1, &alpha[1], n, &
		ierr);
    }

    if (ilbscl) {
	klascl_("U", &c__0, &c__0, &bnrmto, &bnrm, n, n, &b[b_offset], ldb, &
		ierr);
	klascl_("G", &c__0, &c__0, &bnrmto, &bnrm, n, &c__1, &beta[1], n, &
		ierr);
    }
