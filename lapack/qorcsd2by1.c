...
/*  ===================================================================== */
void  qorcsd2by1_(char *jobu1, char *jobu2, char *jobv1t, 
	integer *m, integer *p, integer *q, quadreal *x11, integer *ldx11, 
	quadreal *x21, integer *ldx21, quadreal *theta, quadreal *u1, 
	integer *ldu1, quadreal *u2, integer *ldu2, quadreal *v1t, 
	integer *ldv1t, quadreal *work, integer *lwork, integer *iwork, 
	integer *info)
{
    ...
    integer lworkmin, lworkopt, i__, j, r__, childinfo, lorglqmin, lorgqrmin, 
	    lorglqopt, lorgqropt;
    quadreal dum1[1], dum2[1]	/* was [1][1] */;
    ...
    extern void  qbbcsd_(char *, char *, char *, char *, char *
    ...
    extern void  qlacpy_(char *, integer *, integer *, 
	    ...
	    xerbla_(char *, integer *), qlapmr_(logical *, integer *, 
	    integer *, quadreal *, integer *, integer *), qlapmt_(logical *,
	     integer *, integer *, quadreal *, integer *, integer *), 
	    qorglq_(integer *, integer *, integer *, quadreal *, integer *, 
    ...
    extern void  qorbdb1_(integer *, integer *, integer *, 
	    quadreal *, integer *, quadreal *, integer *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, integer *, integer *), qorbdb2_(integer *, integer *
	    , integer *, quadreal *, integer *, quadreal *, integer *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, integer *, integer *), qorbdb3_(
	    integer *, integer *, integer *, quadreal *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, quadreal *,
	     quadreal *, quadreal *, quadreal *, integer *, integer *), 
	    qorbdb4_(integer *, integer *, integer *, quadreal *, integer *,
	     quadreal *, integer *, quadreal *, quadreal *, quadreal *
...
/* Computing MAX */
	...
	lorglqopt = 1;
	if (r__ == *q) {
	    qorbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum1, dum1, dum1, dum1, &work[1], &c_n1,
		     &childinfo);
	    lorbdb = (integer) work[1];
	    if (wantu1 && *p > 0) {
		qorgqr_(p, p, q, &u1[u1_offset], ldu1, dum1, &work[1], &c_n1, 
			&childinfo);
...
/* Computing MAX */
		...
		i__1 = *m - *p;
		i__2 = *m - *p;
		qorgqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, dum1, &work[1],
			 &c_n1, &childinfo);
/* Computing MAX */
...
/* Computing MAX */
		...
		i__2 = *q - 1;
		i__3 = *q - 1;
		qorglq_(&i__1, &i__2, &i__3, &v1t[v1t_offset], ldv1t, dum1, &
...
/* Computing MAX */
	    ...
	    qbbcsd_(jobu1, jobu2, jobv1t, "N", "N", m, p, q, &theta[1], dum1, 
	    ...
	    qorbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum1, dum1, dum1, dum1, &work[1], &c_n1,
		     &childinfo);
	    lorbdb = (integer) work[1];
	    if (wantu1 && *p > 0) {
		...
		i__2 = *p - 1;
		i__3 = *p - 1;
		qorgqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, 
...
/* Computing MAX */
	    ...
	    if (wantv1t && *q > 0) {
		qorglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, dum1, &work[1], &
			c_n1, &childinfo);
...
/* Computing MAX */
	    ...
	    qbbcsd_(jobv1t, "N", jobu1, jobu2, "T", m, q, p, &theta[1], dum1, 
	    ...
	    qorbdb3_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum1, dum1, dum1, dum1, &work[1], &c_n1,
		     &childinfo);
	    lorbdb = (integer) work[1];
	    if (wantu1 && *p > 0) {
...
/* Computing MAX */
		...
		i__2 = *m - *p - 1;
		i__3 = *m - *p - 1;
		qorgqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, 
			dum1, &work[1], &c_n1, &childinfo);
/* Computing MAX */
...
/* Computing MAX */
	    ...
	    i__1 = *m - *q;
	    i__2 = *m - *p;
	    qbbcsd_("N", jobv1t, jobu2, jobu1, "T", m, &i__1, &i__2, &theta[1]
	    ...
	    qorbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum1, dum1, dum1, dum1, dum1, &work[1], 
		    &c_n1, &childinfo);
	    lorbdb = *m + (integer) work[1];
	    if (wantu1 && *p > 0) {
		i__1 = *m - *q;
		qorgqr_(p, p, &i__1, &u1[u1_offset], ldu1, dum1, &work[1], &
			c_n1, &childinfo);
		lorgqrmin = f2cmax(lorgqrmin,*p);
/* Computing MAX */
		i__1 = lorgqropt, i__2 = (integer) work[1];
		...
		i__2 = *m - *p;
		i__3 = *m - *q;
		qorgqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, dum1, &
			work[1], &c_n1, &childinfo);
/* Computing MAX */
		i__1 = lorgqrmin, i__2 = *m - *p;
		lorgqrmin = f2cmax(i__1,i__2);
/* Computing MAX */
	    ...
	    if (wantv1t && *q > 0) {
		qorglq_(q, q, q, &v1t[v1t_offset], ldv1t, dum1, &work[1], &
			c_n1, &childinfo);
		lorglqmin = f2cmax(lorglqmin,*q);
/* Computing MAX */
		i__1 = lorglqopt, i__2 = (integer) work[1];
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *q;
	    qbbcsd_(jobu2, jobu1, "N", jobv1t, "N", m, &i__1, &i__2, &theta[1]
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	qorbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    qlacpy_("L", p, q, &x11[x11_offset], ldx11, &u1[u1_offset], ldu1);
	    qorgqr_(p, p, q, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorgqr], &lorgqr, &childinfo);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    qlacpy_("L", &i__1, q, &x21[x21_offset], ldx21, &u2[u2_offset], 
		    ldu2);
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    qorgqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, &work[itaup2], &
	...
	if (wantv1t && *q > 0) {
	    ...
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    qlacpy_("U", &i__1, &i__2, &x21[(x21_dim1 << 1) + 1], ldx21, &v1t[
		    (v1t_dim1 << 1) + 2], ldv1t);
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    i__3 = *q - 1;
	    qorglq_(&i__1, &i__2, &i__3, &v1t[(v1t_dim1 << 1) + 2], ldv1t, &
		    work[itauq1], &work[iorglq], &lorglq, &childinfo);
	}

/*        Simultaneously diagonalize X11 and X21. */

	qbbcsd_(jobu1, jobu2, jobv1t, "N", "N", m, p, q, &theta[1], &work[
...
/*        preferred positions */
	...
	if (*q > 0 && wantu2) {
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    qlapmt_(&c_false, &i__1, &i__2, &u2[u2_offset], ldu2, &iwork[1]);
	}
    } else if (r__ == *p) {
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	qorbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &work[iphi], &work[itaup1], &work[itaup2], &work[
		itauq1], &work[iorbdb], &lorbdb, &childinfo);
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    ...
	    i__1 = *p - 1;
	    i__2 = *p - 1;
	    qlacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &u1[(
		    u1_dim1 << 1) + 2], ldu1);
	    i__1 = *p - 1;
	    i__2 = *p - 1;
	    i__3 = *p - 1;
	    qorgqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, &work[
	...
	if (wantv1t && *q > 0) {
	    qlacpy_("U", p, q, &x11[x11_offset], ldx11, &v1t[v1t_offset], 
		    ldv1t);
	    qorglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}

/*        Simultaneously diagonalize X11 and X21. */

	qbbcsd_(jobv1t, "N", jobu1, jobu2, "T", m, q, p, &theta[1], &work[
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	qorbdb3_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &work[iphi], &work[itaup1], &work[itaup2], &work[
		itauq1], &work[iorbdb], &lorbdb, &childinfo);
...
/*        Accumulate Householder reflectors */
	...
	if (wantu2 && *m - *p > 0) {
	    ...
	    i__1 = *m - *p - 1;
	    i__2 = *m - *p - 1;
	    qlacpy_("L", &i__1, &i__2, &x21[x21_dim1 + 2], ldx21, &u2[(
		    u2_dim1 << 1) + 2], ldu2);
	    i__1 = *m - *p - 1;
	    i__2 = *m - *p - 1;
	    i__3 = *m - *p - 1;
	    qorgqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, &work[
		    itaup2], &work[iorgqr], &lorgqr, &childinfo);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *m - *p;
	    qlacpy_("U", &i__1, q, &x21[x21_offset], ldx21, &v1t[v1t_offset], 
		    ldv1t);
	    qorglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}
...
/*        preferred positions */
	...
	if (*q > r__) {
	    ...
	    if (wantu1) {
		qlapmt_(&c_false, p, q, &u1[u1_offset], ldu1, &iwork[1]);
	    }
	    if (wantv1t) {
		qlapmr_(&c_false, q, q, &v1t[v1t_offset], ldv1t, &iwork[1]);
	    }
	}
    } else {
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	i__1 = lorbdb - *m;
	qorbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &work[iphi], &work[itaup1], &work[itaup2], &work[
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    qcopy_(p, &work[iorbdb], &c__1, &u1[u1_offset], &c__1);
	    i__1 = *p;
	    for (j = 2; j <= i__1; ++j) {
		u1[j * u1_dim1 + 1] = 0.;
	    }
	    ...
	    qlacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &u1[(
		    u1_dim1 << 1) + 2], ldu1);
	    i__1 = *m - *q;
	    qorgqr_(p, p, &i__1, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorgqr], &lorgqr, &childinfo);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    qcopy_(&i__1, &work[iorbdb + *p], &c__1, &u2[u2_offset], &c__1);
	    i__1 = *m - *p;
	    for (j = 2; j <= i__1; ++j) {
		u2[j * u2_dim1 + 1] = 0.;
	    }
	    ...
	    i__2 = *m - *p;
	    i__3 = *m - *q;
	    qorgqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, &work[itaup2], 
		    &work[iorgqr], &lorgqr, &childinfo);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *m - *q;
	    ...
	    i__1 = *p - (*m - *q);
	    i__2 = *q - (*m - *q);
	    qlacpy_("U", &i__1, &i__2, &x11[*m - *q + 1 + (*m - *q + 1) * 
		    x11_dim1], ldx11, &v1t[*m - *q + 1 + (*m - *q + 1) * 
		    v1t_dim1], ldv1t);
	    i__1 = -(*p) + *q;
	    i__2 = *q - *p;
	    qlacpy_("U", &i__1, &i__2, &x21[*m - *q + 1 + (*p + 1) * x21_dim1]
		    , ldx21, &v1t[*p + 1 + (*p + 1) * v1t_dim1], ldv1t);
	    qorglq_(q, q, q, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}
...
/*        preferred positions */
	...
	if (*p > r__) {
	    ...
	    if (wantu1) {
		qlapmt_(&c_false, p, p, &u1[u1_offset], ldu1, &iwork[1]);
	    }
	    if (wantv1t) {
		qlapmr_(&c_false, p, q, &v1t[v1t_offset], ldv1t, &iwork[1]);
	    }
	}
    }
