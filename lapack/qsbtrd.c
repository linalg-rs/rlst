...
/* > */
/*  ===================================================================== */
void  qsbtrd_(char *vect, char *uplo, integer *n, integer *kd, 
	quadreal *ab, integer *ldab, quadreal *d__, quadreal *e, 
	quadreal *q, integer *ldq, quadreal *work, integer *info)
{
...
/*     Initialize Q to the unit matrix, if needed */
    ...
    if (initq) {
	qlaset_("Full", n, n, &c_b9, &c_b10, &q[q_offset], ldq);
    }
...
/*                    elements which have been created outside the band */
			...
			qlargv_(&nr, &ab[(j1 - 1) * ab_dim1 + 1], &inca, &
...
/*                    DLARTV or DROT is used */
			...
			if (nr >= (*kd << 1) - 1) {
			    i__2 = *kd - 1;
			    for (l = 1; l <= i__2; ++l) {
				qlartv_(&nr, &ab[l + 1 + (j1 - 1) * ab_dim1], 
...
/* L10: */
			    ...
			    for (jinc = j1; i__3 < 0 ? jinc >= i__2 : jinc <= 
				    i__2; jinc += i__3) {
				qrot_(&kdm1, &ab[(jinc - 1) * ab_dim1 + 2], &
...
/*                       within the band */
			    ...
			    qlartg_(&ab[*kd - k + 3 + (i__ + k - 2) * ab_dim1]
				    , &ab[*kd - k + 2 + (i__ + k - 1) * 
...
/*                 blocks */
		    ...
		    if (nr > 0) {
			qlar2v_(&nr, &ab[kd1 + (j1 - 1) * ab_dim1], &ab[kd1 + 
...
/* Computing MIN */
				...
				qrot_(&nq, &q[iqb + (j - 1) * q_dim1], &c__1, 
...
/*                    elements which have been created outside the band */
			...
			qlargv_(&nr, &ab[kd1 + (j1 - kd1) * ab_dim1], &inca, &
				work[j1], &kd1, &d__[j1], &kd1);
...
/* L130: */
			    ...
			    for (jinc = j1; i__2 < 0 ? jinc >= i__3 : jinc <= 
				    i__3; jinc += i__2) {
				qrot_(&kdm1, &ab[*kd + (jinc - *kd) * ab_dim1]
					, &incx, &ab[kd1 + (jinc - *kd) * 
...
/*                       within the band */
			    ...
			    qlartg_(&ab[k - 1 + i__ * ab_dim1], &ab[k + i__ * 
				    ab_dim1], &d__[i__ + k - 1], &work[i__ + 
				    k - 1], &temp);
...
/*                       apply rotation from the left */
			    ...
			    i__3 = *ldab - 1;
			    i__4 = *ldab - 1;
			    qrot_(&i__2, &ab[k - 2 + (i__ + 1) * ab_dim1], &
...
/*                 blocks */
		    ...
		    if (nr > 0) {
			qlar2v_(&nr, &ab[(j1 - 1) * ab_dim1 + 1], &ab[j1 * 
...
/* Computing MIN */
			    ...
			    last = j1end + kd1;
			    if (lend > 0) {
				qrot_(&lend, &ab[(last - 1) * ab_dim1 + 3], &
