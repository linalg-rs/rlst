...
/* > \verbatim */
/* > */
/* > DHGEQZ computes the eigenvalues of a doublereal matrix pair (H,T), */
/* > where H is an upper Hessenberg matrix and T is upper triangular, */
/* > using the double-shift QZ method. */
/* > Matrix pairs of this type are produced by the reduction to */
/* > generalized upper Hessenberg form of a doublereal matrix pair (A,B): */
/* > */
...
/* > diagonal blocks. */
/* > */
/* > The 1-by-1 blocks correspond to doublereal eigenvalues of the matrix pair */
...
/* >          The doublereal parts of each scalar alpha defining an eigenvalue */
/* >          of GNEP. */
/* > \endverbatim */
/* > */
/* > \param[out] ALPHAI */
...
/* > */
/*  ===================================================================== */
void  hhgeqz_(char *job, char *compq, char *compz, integer *n, 
	integer *ilo, integer *ihi, halfreal *h__, integer *ldh, halfreal 
	*t, integer *ldt, halfreal *alphar, halfreal *alphai, halfreal *
	beta, halfreal *q, integer *ldq, halfreal *z__, integer *ldz, 
	halfreal *work, integer *lwork, integer *info)
{
    ...
    halfreal c__;
    integer j;
    halfreal s, v[3], s1, s2, t1, u1, u2, a11, a12, a21, a22, b11, b22, c12,
	     c21;
    integer jc;
    halfreal an, bn, cl, cq, cr;
    integer in;
    halfreal u12, w11, w12, w21;
    integer jr;
    halfreal cz, w22, sl, wi, sr, vs, wr, b1a, b2a, a1i, a2i, b1i, b2i, a1r,
	     a2r, b1r, b2r, wr2, ad11, ad12, ad21, ad22, c11i, c22i;
    integer jch;
    halfreal c11r, c22r;
    logical ilq;
    halfreal u12l, tau, sqi;
    logical ilz;
    halfreal ulp, sqr, szi, szr, ad11l, ad12l, ad21l, ad22l, ad32l, wabs, 
	    atol, btol, temp;
    extern void  hrot_(integer *, halfreal *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *), hlag2_(
	    halfreal *, integer *, halfreal *, integer *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, halfreal *, 
	    halfreal *);
    halfreal temp2, s1inv, scale;
    extern logical lsame_(char *, char *);
    integer iiter, ilast, jiter;
    halfreal anorm, bnorm;
    integer maxit;
    halfreal tempi, tempr;
    extern halfreal hlapy2_(halfreal *, halfreal *), hlapy3_(halfreal 
	    *, halfreal *, halfreal *);
    extern void  hlasv2_(halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *);
    logical ilazr2;
    halfreal ascale, bscale;
    extern halfreal hlamch_(char *);
    extern void  hlarfg_(integer *, halfreal *, halfreal *,
	     integer *, halfreal *);
    extern halfreal hlanhs_(char *, integer *, halfreal *, integer *, 
	    halfreal *);
    extern void  hlaset_(char *, integer *, integer *, 
	    halfreal *, halfreal *, halfreal *, integer *);
    halfreal safmin;
    extern void  hlartg_(halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *);
    halfreal safmax;
...
/*     Initialize Q and Z */
    ...
    if (icompq == 3) {
	hlaset_("Full", n, n, &c_b12, &c_b13, &q[q_offset], ldq);
    }
    if (icompz == 3) {
	hlaset_("Full", n, n, &c_b12, &c_b13, &z__[z_offset], ldz);
    }
...
/*     Machine Constants */
    ...
    safmin = hlamch_("S");
    safmax = 1. / safmin;
    ulp = hlamch_("E") * hlamch_("B");
    anorm = hlanhs_("F", &in, &h__[*ilo + *ilo * h_dim1], ldh, &work[1]);
    bnorm = hlanhs_("F", &in, &t[*ilo + *ilo * t_dim1], ldt, &work[1]);
/* Computing MAX */
...
/*              this may have to be done repeatedly. */
		...
		if (ilazro || ilazr2) {
			...
			hlartg_(&temp, &h__[jch + 1 + jch * h_dim1], &c__, &s,
				 &h__[jch + jch * h_dim1]);
			h__[jch + 1 + jch * h_dim1] = 0.;
			i__4 = ilastm - jch;
			hrot_(&i__4, &h__[jch + (jch + 1) * h_dim1], ldh, &
				h__[jch + 1 + (jch + 1) * h_dim1], ldh, &c__, 
				&s);
			i__4 = ilastm - jch;
			hrot_(&i__4, &t[jch + (jch + 1) * t_dim1], ldt, &t[
				jch + 1 + (jch + 1) * t_dim1], ldt, &c__, &s);
			if (ilq) {
			    hrot_(n, &q[jch * q_dim1 + 1], &c__1, &q[(jch + 1)
				     * q_dim1 + 1], &c__1, &c__, &s);
			}
			if (ilazr2) {
...
/*                 Then process as in the case T(ILAST,ILAST)=0 */
			...
			hlartg_(&temp, &t[jch + 1 + (jch + 1) * t_dim1], &c__,
				 &s, &t[jch + (jch + 1) * t_dim1]);
			t[jch + 1 + (jch + 1) * t_dim1] = 0.;
			if (jch < ilastm - 1) {
			    i__4 = ilastm - jch - 1;
			    hrot_(&i__4, &t[jch + (jch + 2) * t_dim1], ldt, &
			...
			hlartg_(&temp, &h__[jch + 1 + (jch - 1) * h_dim1], &
				c__, &s, &h__[jch + 1 + jch * h_dim1]);
			h__[jch + 1 + (jch - 1) * h_dim1] = 0.;
			i__4 = jch + 1 - ifrstm;
			hrot_(&i__4, &h__[ifrstm + jch * h_dim1], &c__1, &h__[
				ifrstm + (jch - 1) * h_dim1], &c__1, &c__, &s)
				;
			i__4 = jch - ifrstm;
			hrot_(&i__4, &t[ifrstm + jch * t_dim1], &c__1, &t[
				ifrstm + (jch - 1) * t_dim1], &c__1, &c__, &s)
				;
			if (ilz) {
			    hrot_(n, &z__[jch * z_dim1 + 1], &c__1, &z__[(jch 
...
L70:
	...
	hlartg_(&temp, &h__[ilast + (ilast - 1) * h_dim1], &c__, &s, &h__[
		ilast + ilast * h_dim1]);
	h__[ilast + (ilast - 1) * h_dim1] = 0.;
	i__2 = ilast - ifrstm;
	hrot_(&i__2, &h__[ifrstm + ilast * h_dim1], &c__1, &h__[ifrstm + (
		ilast - 1) * h_dim1], &c__1, &c__, &s);
	i__2 = ilast - ifrstm;
	hrot_(&i__2, &t[ifrstm + ilast * t_dim1], &c__1, &t[ifrstm + (ilast - 
		1) * t_dim1], &c__1, &c__, &s);
	if (ilz) {
	    hrot_(n, &z__[ilast * z_dim1 + 1], &c__1, &z__[(ilast - 1) * 
		    z_dim1 + 1], &c__1, &c__, &s);
	}
...
/*           returned by DLAG2 is the Wilkinson shift (AEP p.512), */
	    ...
	    d__1 = safmin * 100.;
	    hlag2_(&h__[ilast - 1 + (ilast - 1) * h_dim1], ldh, &t[ilast - 1 
...
/*        Initial Q */
	...
	hlartg_(&temp, &temp2, &c__, &s, &tempr);
...
/*        Sweep */
	...
	for (j = istart; j <= i__2; ++j) {
		...
		hlartg_(&temp, &h__[j + 1 + (j - 1) * h_dim1], &c__, &s, &h__[
			j + (j - 1) * h_dim1]);
...
/* L150: */
	    ...
	    hlartg_(&temp, &t[j + 1 + j * t_dim1], &c__, &s, &t[j + 1 + (j + 
		    1) * t_dim1]);
	    t[j + 1 + j * t_dim1] = 0.;
...
/*        Note: the Francis double-shift should work with doublereal shifts, */
/*              but only if the block is at least 3x3. */
/*              This code may break if this point is reached with */
/*              a 2x2 block with doublereal eigenvalues. */
...
/*                       (  0  B22 ) */
	    ...
	    hlasv2_(&t[ilast - 1 + (ilast - 1) * t_dim1], &t[ilast - 1 + 
	    ...
	    hrot_(&i__2, &h__[ilast - 1 + (ilast - 1) * h_dim1], ldh, &h__[
		    ilast + (ilast - 1) * h_dim1], ldh, &cl, &sl);
	    i__2 = ilast + 1 - ifrstm;
	    hrot_(&i__2, &h__[ifrstm + (ilast - 1) * h_dim1], &c__1, &h__[
		    ifrstm + ilast * h_dim1], &c__1, &cr, &sr);

	    if (ilast < ilastm) {
		i__2 = ilastm - ilast;
		hrot_(&i__2, &t[ilast - 1 + (ilast + 1) * t_dim1], ldt, &t[
			ilast + (ilast + 1) * t_dim1], ldt, &cl, &sl);
	    }
	    if (ifrstm < ilast - 1) {
		i__2 = ifirst - ifrstm;
		hrot_(&i__2, &t[ifrstm + (ilast - 1) * t_dim1], &c__1, &t[
			ifrstm + ilast * t_dim1], &c__1, &cr, &sr);
	    }

	    if (ilq) {
		hrot_(n, &q[(ilast - 1) * q_dim1 + 1], &c__1, &q[ilast * 
			q_dim1 + 1], &c__1, &cl, &sl);
	    }
	    if (ilz) {
		hrot_(n, &z__[(ilast - 1) * z_dim1 + 1], &c__1, &z__[ilast * 
			z_dim1 + 1], &c__1, &cr, &sr);
	    }
...
/*           If standardization has perturbed the shift onto doublereal line, */
/*           do another (doublereal single-shift) QR step. */
...
/*                     ( SZ    CZ ) */
	    ...
	    if (abs(c11r) + abs(c11i) + abs(c12) > abs(c21) + abs(c22r) + abs(
		    c22i)) {
		t1 = hlapy3_(&c12, &c11r, &c11i);
		cz = c12 / t1;
		szr = -c11r / t1;
		szi = -c11i / t1;
	    } else {
		cz = hlapy2_(&c22r, &c22i);
		if (cz <= safmin) {
		    cz = 0.;
		    szr = 1.;
		    szi = 0.;
...
/*              Zero (j-1)st column of A */
		...
		if (j > istart) {
		    ...
		    hlarfg_(&c__3, &h__[j + (j - 1) * h_dim1], &v[1], &c__1, &
			    tau);
		    v[0] = 1.;
		    h__[j + 1 + (j - 1) * h_dim1] = 0.;
...
/* Computing 2nd power */
		d__3 = u2;
		t1 = M(sqrt)(d__1 * d__1 + d__2 * d__2 + d__3 * d__3);
		tau = scale / t1 + 1.;
		vs = -1. / (scale + t1);
		v[0] = 1.;
		v[1] = vs * u1;
...
/*           Rotations from the left */
	    ...
	    j = ilast - 1;
	    temp = h__[j + (j - 1) * h_dim1];
	    hlartg_(&temp, &h__[j + 1 + (j - 1) * h_dim1], &c__, &s, &h__[j + 
		    (j - 1) * h_dim1]);
	    h__[j + 1 + (j - 1) * h_dim1] = 0.;
