...
/* > \verbatim */
/* > */
/* > DSYTRD_SY2SB reduces a doublereal symmetric matrix A to doublereal symmetric */
...
/* > */
/*  ===================================================================== */
void  dsytrd_sy2sb__(char *uplo, integer *n, integer *kd, 
	quadreal *a, integer *lda, quadreal *ab, integer *ldab, 
	quadreal *tau, quadreal *work, integer *lwork, integer *info)
{
    ...
    extern void  qcopy_(integer *, quadreal *, integer *, 
	    quadreal *, integer *);
    integer lwmin;
    extern void  qsymm_(char *, char *, integer *, integer *, 
	    quadreal *, quadreal *, integer *, quadreal *, integer *, 
	    quadreal *, quadreal *, integer *);
    logical upper;
    extern void  qsyr2k_(char *, char *, integer *, integer *, 
	    quadreal *, quadreal *, integer *, quadreal *, integer *, 
	    quadreal *, quadreal *, integer *), qgelqf_(
	    integer *, integer *, quadreal *, integer *, quadreal *, 
	    quadreal *, integer *, integer *), qgeqrf_(integer *, integer *,
	     quadreal *, integer *, quadreal *, quadreal *, integer *, 
	    integer *), qlarft_(char *, char *, integer *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, integer *), xerbla_(char *, integer *), qlaset_(char 
...
/* Computing MIN */
		i__2 = *kd + 1;
		lk = f2cmin(i__2,i__);
		qcopy_(&lk, &a[i__ - lk + 1 + i__ * a_dim1], &c__1, &ab[*kd + 
...
/* Computing MIN */
		...
		lk = f2cmin(i__2,i__3);
		qcopy_(&lk, &a[i__ + i__ * a_dim1], &c__1, &ab[i__ * ab_dim1 
			+ 1], &c__1);
/* L110: */
	    }
	}
...
/*     way everytime T is generated the upper/lower portion will be always zero */
    ...
    qlaset_("A", &ldt, kd, &c_b17, &c_b17, &work[tpos], &ldt);
...
/*            Compute the LQ factorization of the current block */
	    ...
	    qgelqf_(kd, &pn, &a[i__ + (i__ + *kd) * a_dim1], lda, &tau[i__], &
...
/* Computing MIN */
		...
		i__4 = *ldab - 1;
		qcopy_(&lk, &a[j + j * a_dim1], lda, &ab[*kd + 1 + j * 
			ab_dim1], &i__4);
/* L20: */
	    }

	    qlaset_("Lower", &pk, &pk, &c_b17, &c_b23, &a[i__ + (i__ + *kd) * 
		    a_dim1], lda);

/*            Form the matrix T */

	    qlarft_("Forward", "Rowwise", &pn, &pk, &a[i__ + (i__ + *kd) * 
		    a_dim1], lda, &tau[i__], &work[tpos], &ldt);

/*            Compute W: */

	    qgemm_("Conjugate", "No transpose", &pk, &pn, &pk, &c_b23, &work[
		    tpos], &ldt, &a[i__ + (i__ + *kd) * a_dim1], lda, &c_b17, 
		    &work[s2pos], &lds2);

	    qsymm_("Right", uplo, &pk, &pn, &c_b23, &a[i__ + *kd + (i__ + *kd)
		     * a_dim1], lda, &work[s2pos], &lds2, &c_b17, &work[wpos],
		     &ldw);

	    qgemm_("No transpose", "Conjugate", &pk, &pk, &pn, &c_b23, &work[
		    wpos], &ldw, &work[s2pos], &lds2, &c_b17, &work[s1pos], &
		    lds1);

	    qgemm_("No transpose", "No transpose", &pk, &pn, &pk, &c_b39, &
...
/*            an update of the form:  A := A - V'*W - W'*V */
	    ...
	    qsyr2k_(uplo, "Conjugate", &pn, &pk, &c_b42, &a[i__ + (i__ + *kd) 
...
/* Computing MIN */
	    ...
	    i__1 = *ldab - 1;
	    qcopy_(&lk, &a[j + j * a_dim1], lda, &ab[*kd + 1 + j * ab_dim1], &
		    i__1);
/* L30: */
	}
...
/*            Compute the QR factorization of the current block */
	    ...
	    qgeqrf_(&pn, kd, &a[i__ + *kd + i__ * a_dim1], lda, &tau[i__], &
		    work[s2pos], &ls2, &iinfo);
...
/* Computing MIN */
		i__4 = *kd, i__5 = *n - j;
		lk = f2cmin(i__4,i__5) + 1;
		qcopy_(&lk, &a[j + j * a_dim1], &c__1, &ab[j * ab_dim1 + 1], &
			c__1);
/* L50: */
	    }

	    qlaset_("Upper", &pk, &pk, &c_b17, &c_b23, &a[i__ + *kd + i__ * 
		    a_dim1], lda);

/*            Form the matrix T */

	    qlarft_("Forward", "Columnwise", &pn, &pk, &a[i__ + *kd + i__ * 
		    a_dim1], lda, &tau[i__], &work[tpos], &ldt);

/*            Compute W: */

	    qgemm_("No transpose", "No transpose", &pn, &pk, &pk, &c_b23, &a[
		    i__ + *kd + i__ * a_dim1], lda, &work[tpos], &ldt, &c_b17,
		     &work[s2pos], &lds2);

	    qsymm_("Left", uplo, &pn, &pk, &c_b23, &a[i__ + *kd + (i__ + *kd) 
		    * a_dim1], lda, &work[s2pos], &lds2, &c_b17, &work[wpos], 
		    &ldw);

	    qgemm_("Conjugate", "No transpose", &pk, &pk, &pn, &c_b23, &work[
		    s2pos], &lds2, &work[wpos], &ldw, &c_b17, &work[s1pos], &
		    lds1);

	    qgemm_("No transpose", "No transpose", &pn, &pk, &pk, &c_b39, &a[
...
/*            an update of the form:  A := A - V*W' - W*V' */
	    ...
	    qsyr2k_(uplo, "No transpose", &pn, &pk, &c_b42, &a[i__ + *kd + 
