...
/* > */
/*  ===================================================================== */
void  horbdb1_(integer *m, integer *p, integer *q, halfreal *
	x11, integer *ldx11, halfreal *x21, integer *ldx21, halfreal *
	theta, halfreal *phi, halfreal *taup1, halfreal *taup2, 
	halfreal *tauq1, halfreal *work, integer *lwork, integer *info)
{
...
/*     Reduce columns 1, ..., Q of X11 and X21 */
	...
	i__2 = *p - i__ + 1;
	hlarfgp_(&i__2, &x11[i__ + i__ * x11_dim1], &x11[i__ + 1 + i__ * 
		x11_dim1], &c__1, &taup1[i__]);
	i__2 = *m - *p - i__ + 1;
	hlarfgp_(&i__2, &x21[i__ + i__ * x21_dim1], &x21[i__ + 1 + i__ * 
	...
	i__2 = *p - i__ + 1;
	i__3 = *q - i__;
	hlarf_("L", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], &c__1, &taup1[
		i__], &x11[i__ + (i__ + 1) * x11_dim1], ldx11, &work[ilarf]);
	i__2 = *m - *p - i__ + 1;
	i__3 = *q - i__;
	hlarf_("L", &i__2, &i__3, &x21[i__ + i__ * x21_dim1], &c__1, &taup2[
		i__], &x21[i__ + (i__ + 1) * x21_dim1], ldx21, &work[ilarf]);

	if (i__ < *q) {
	    i__2 = *q - i__;
	    hrot_(&i__2, &x11[i__ + (i__ + 1) * x11_dim1], ldx11, &x21[i__ + (
		    i__ + 1) * x21_dim1], ldx21, &c__, &s);
	    i__2 = *q - i__;
	    hlarfgp_(&i__2, &x21[i__ + (i__ + 1) * x21_dim1], &x21[i__ + (i__ 
		    + 2) * x21_dim1], ldx21, &tauq1[i__]);
	    s = x21[i__ + (i__ + 1) * x21_dim1];
	    x21[i__ + (i__ + 1) * x21_dim1] = 1.;
	    i__2 = *p - i__;
	    i__3 = *q - i__;
	    hlarf_("R", &i__2, &i__3, &x21[i__ + (i__ + 1) * x21_dim1], ldx21,
	    ...
	    i__2 = *p - i__;
/* Computing 2nd power */
	    d__1 = hnrm2_(&i__2, &x11[i__ + 1 + (i__ + 1) * x11_dim1], &c__1);
	    i__3 = *m - *p - i__;
/* Computing 2nd power */
	    d__2 = hnrm2_(&i__3, &x21[i__ + 1 + (i__ + 1) * x21_dim1], &c__1);
	    c__ = M(sqrt)(d__1 * d__1 + d__2 * d__2);
	    phi[i__] = atan2(s, c__);
	    i__2 = *p - i__;
	    i__3 = *m - *p - i__;
	    i__4 = *q - i__ - 1;
	    horbdb5_(&i__2, &i__3, &i__4, &x11[i__ + 1 + (i__ + 1) * x11_dim1]
