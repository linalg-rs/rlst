...
/*  ===================================================================== */
void  qtbrfs_(char *uplo, char *trans, char *diag, integer *n, 
	integer *kd, integer *nrhs, quadreal *ab, integer *ldab, quadreal 
	*b, integer *ldb, quadreal *x, integer *ldx, quadreal *ferr, 
	quadreal *berr, quadreal *work, integer *iwork, integer *info)
{
    ...
    integer isave[3];
    extern void  qtbmv_(char *, char *, char *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, integer *), qcopy_(integer *, quadreal *, integer *
	    , quadreal *, integer *), qtbsv_(char *, char *, char *, 
	    integer *, integer *, quadreal *, integer *, quadreal *, 
	    integer *), qaxpy_(integer *, quadreal *
...
/*        where op(A) = A or A**T, depending on TRANS. */
	...
	qcopy_(n, &x[j * x_dim1 + 1], &c__1, &work[*n + 1], &c__1);
	qtbmv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1], 
		&c__1);
	qaxpy_(n, &c_b19, &b[j * b_dim1 + 1], &c__1, &work[*n + 1], &c__1);
...
/*              Multiply by diag(W)*inv(op(A)**T). */
		...
		qtbsv_(uplo, transt, diag, n, kd, &ab[ab_offset], ldab, &work[
			*n + 1], &c__1);
		i__2 = *n;
		for (i__ = 1; i__ <= i__2; ++i__) {
...
/* L230: */
		...
		qtbsv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*
			n + 1], &c__1);
	    }
	    goto L210;
	}
