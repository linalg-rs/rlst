...
/*  ===================================================================== */
void  qgglse_(integer *m, integer *n, integer *p, quadreal *
	a, integer *lda, quadreal *b, integer *ldb, quadreal *c__, 
	quadreal *d__, quadreal *x, quadreal *work, integer *lwork, 
	integer *info)
{
...
/*     orthogonal. */
    ...
    i__1 = *lwork - *p - mn;
    qggrqf_(p, m, n, &b[b_offset], ldb, &work[1], &a[a_offset], lda, &work[*p 
...
/*                          ( c2 ) M+P-N */
    ...
    i__1 = f2cmax(1,*m);
    i__2 = *lwork - *p - mn;
    qormqr_("Left", "Transpose", m, &c__1, &mn, &a[a_offset], lda, &work[*p + 
...
/*     Solve T12*x2 = d for x2 */
    ...
    if (*p > 0) {
	qtrtrs_("Upper", "No transpose", "Non-unit", p, &c__1, &b[(*n - *p + 
...
/*        Put the solution in X */
	...
	qcopy_(p, &d__[1], &c__1, &x[*n - *p + 1], &c__1);

/*        Update c1 */

	i__1 = *n - *p;
	qgemv_("No transpose", &i__1, p, &c_b31, &a[(*n - *p + 1) * a_dim1 + 
...
/*     Solve R11*x1 = c1 for x1 */
	...
	i__1 = *n - *p;
	i__2 = *n - *p;
	qtrtrs_("Upper", "No transpose", "Non-unit", &i__1, &c__1, &a[
...
/*        Put the solutions in X */
	...
	i__1 = *n - *p;
	qcopy_(&i__1, &c__[1], &c__1, &x[1], &c__1);
    }
...
/*     Compute the residual vector: */
	...
	if (nr > 0) {
	    i__1 = *n - *m;
	    qgemv_("No transpose", &nr, &i__1, &c_b31, &a[*n - *p + 1 + (*m + 
    ...
    if (nr > 0) {
	qtrmv_("Upper", "No transpose", "Non unit", &nr, &a[*n - *p + 1 + (*n 
		- *p + 1) * a_dim1], lda, &d__[1], &c__1);
	qaxpy_(&nr, &c_b31, &d__[1], &c__1, &c__[*n - *p + 1], &c__1);
    }

/*     Backward transformation x = Q**T*x */

    i__1 = *lwork - *p - mn;
    qormrq_("Left", "Transpose", n, &c__1, p, &b[b_offset], ldb, &work[1], &x[
	    1], n, &work[*p + mn + 1], &i__1, info);
/* Computing MAX */
    i__1 = lopt, i__2 = (integer) work[*p + mn + 1];
    work[1] = (quadreal) (*p + mn + f2cmax(i__1,i__2));
