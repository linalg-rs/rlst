...
/*  ===================================================================== */
halfreal klantr_(char *norm, char *uplo, char *diag, integer *m, integer *n,
	 halfcomplex *a, integer *lda, halfreal *work)
{
...
/*        Find normF(A). */
	...
	if (lsame_(uplo, "U")) {
	    if (lsame_(diag, "U")) {
		scale = 1.;
		sum = (halfreal) f2cmin(*m,*n);
		i__1 = *n;
		for (j = 2; j <= i__1; ++j) {
/* Computing MIN */
		    i__3 = *m, i__4 = j - 1;
		    i__2 = f2cmin(i__3,i__4);
		    klassq_(&i__2, &a[j * a_dim1 + 1], &c__1, &scale, &sum);
/* L290: */
		}
	    } else {
		scale = 0.;
...
/* Computing MIN */
		    i__3 = *m, i__4 = j + 1;
		    klassq_(&i__2, &a[f2cmin(i__3,i__4) + j * a_dim1], &c__1, &
			    scale, &sum);
/* L310: */
		}
	    } else {
		...
		for (j = 1; j <= i__1; ++j) {
		    i__2 = *m - j + 1;
		    klassq_(&i__2, &a[j + j * a_dim1], &c__1, &scale, &sum);
/* L320: */
		}
	    }
	}
