...
/*  ===================================================================== */
void  qgsvj0_(char *jobv, integer *m, integer *n, quadreal *
	a, integer *lda, quadreal *d__, quadreal *sva, integer *mv, 
	quadreal *v, integer *ldv, quadreal *eps, quadreal *sfmin, 
	quadreal *tol, integer *nsweep, quadreal *work, integer *lwork, 
	integer *info)
{
    ...
    extern void  qaxpy_(integer *, quadreal *, quadreal *, 
	    integer *, quadreal *, integer *), qrotm_(integer *, quadreal 
    ...
    quadreal mxaapq;
    extern void  qlassq_(integer *, quadreal *, integer *, 
	    quadreal *, quadreal *);
    quadreal thsign, mxsinj;
...
/* Computing MIN */
		...
		for (p = igl; p <= i__4; ++p) {
		    i__5 = *n - p + 1;
		    q = iqamax_(&i__5, &sva[p], &c__1) + p - 1;
		    if (p != q) {
			qswap_(m, &a[p * a_dim1 + 1], &c__1, &a[q * a_dim1 + 
				1], &c__1);
			if (rsvec) {
			    qswap_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
				    v_dim1 + 1], &c__1);
			}
			temp1 = sva[p];
			sva[p] = sva[q];
...
/*        below should read "AAPP = DNRM2( M, A(1,p), 1 ) * D(p)". */
			...
			if (sva[p] < rootbig && sva[p] > rootsfmin) {
			    sva[p] = qnrm2_(m, &a[p * a_dim1 + 1], &c__1) * 
				    d__[p];
			} else {
			    temp1 = 0.;
			    aapp = 1.;
			    qlassq_(m, &a[p * a_dim1 + 1], &c__1, &temp1, &
				    aapp);
			    sva[p] = temp1 * M(sqrt)(aapp) * d__[p];
			}
			aapp = sva[p];
		    } else {
			aapp = sva[p];
...
/* Computing MIN */
			...
			for (q = p + 1; q <= i__5; ++q) {
				...
				if (aaqq >= 1.) {
				    ...
				    if (aapp < big / aaqq) {
					aapq = qdot_(m, &a[p * a_dim1 + 1], &
						c__1, &a[q * a_dim1 + 1], &
						c__1) * d__[p] * d__[q] / 
						aaqq / aapp;
				    } else {
					qcopy_(m, &a[p * a_dim1 + 1], &c__1, &
						work[1], &c__1);
					qlascl_("G", &c__0, &c__0, &aapp, &
						d__[p], m, &c__1, &work[1], 
						lda, &ierr);
					aapq = qdot_(m, &work[1], &c__1, &a[q 
						* a_dim1 + 1], &c__1) * d__[q]
						 / aaqq;
				    }
				} else {
					...
					qcopy_(m, &a[q * a_dim1 + 1], &c__1, &
						work[1], &c__1);
					qlascl_("G", &c__0, &c__0, &aaqq, &
						d__[q], m, &c__1, &work[1], 
						lda, &ierr);
					aapq = qdot_(m, &work[1], &c__1, &a[p 
						* a_dim1 + 1], &c__1) * d__[p]
						 / aapp;
				    }
				}
...
/*           ROTATED = ROTATED + ONE */
					...
					if (abs(theta) > bigtheta) {
					    ...
					    qrotm_(m, &a[p * a_dim1 + 1], &
						    c__1, &a[q * a_dim1 + 1], 
						    &c__1, fastr);
					    if (rsvec) {
			  qrotm_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
				  v_dim1 + 1], &c__1, fastr);
					    }
/* Computing MAX */
					    d__1 = 0., d__2 = t * apoaq * 
...
/* Computing MAX */
			  ...
			  if (d__[q] >= 1.) {
			      ...
			      d__[p] *= cs;
			      d__[q] *= cs;
			      qrotm_(m, &a[p * a_dim1 + 1], &c__1, &a[q * 
				      a_dim1 + 1], &c__1, fastr);
			      if (rsvec) {
				  qrotm_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[
					  q * v_dim1 + 1], &c__1, fastr);
			      }
			  } else {
			      d__1 = -t * aqoap;
			      qaxpy_(m, &d__1, &a[q * a_dim1 + 1], &c__1, &a[
				      p * a_dim1 + 1], &c__1);
			      d__1 = cs * sn * apoaq;
			      qaxpy_(m, &d__1, &a[p * a_dim1 + 1], &c__1, &a[
				      q * a_dim1 + 1], &c__1);
			      d__[p] *= cs;
			      d__[q] /= cs;
			      if (rsvec) {
				  d__1 = -t * aqoap;
				  qaxpy_(&mvl, &d__1, &v[q * v_dim1 + 1], &
					  c__1, &v[p * v_dim1 + 1], &c__1);
				  d__1 = cs * sn * apoaq;
				  qaxpy_(&mvl, &d__1, &v[p * v_dim1 + 1], &
			  ...
			  if (d__[q] >= 1.) {
			      ...
			      if (d__[p] >= d__[q]) {
				  d__1 = -t * aqoap;
				  qaxpy_(m, &d__1, &a[q * a_dim1 + 1], &c__1, 
					  &a[p * a_dim1 + 1], &c__1);
				  d__1 = cs * sn * apoaq;
				  qaxpy_(m, &d__1, &a[p * a_dim1 + 1], &c__1, 
					  &a[q * a_dim1 + 1], &c__1);
				  d__[p] *= cs;
				  d__[q] /= cs;
				  if (rsvec) {
				  ...
				  if (rsvec) {
					...
					qaxpy_(m, &temp1, &work[1], &c__1, &a[
						q * a_dim1 + 1], &c__1);
					qlascl_("G", &c__0, &c__0, &c_b42, &
...
/* Computing 2nd power */
					...
					if (aaqq < rootbig && aaqq > 
						rootsfmin) {
					    sva[q] = qnrm2_(m, &a[q * a_dim1 
						    + 1], &c__1) * d__[q];
					} else {
					    t = 0.;
					    aaqq = 1.;
					    qlassq_(m, &a[q * a_dim1 + 1], &
						    c__1, &t, &aaqq);
					...
					if (aapp < rootbig && aapp > 
						rootsfmin) {
					    aapp = qnrm2_(m, &a[p * a_dim1 + 
						    1], &c__1) * d__[p];
					} else {
					    t = 0.;
					    aapp = 1.;
					    qlassq_(m, &a[p * a_dim1 + 1], &
						    c__1, &t, &aapp);
...
/* 2000  end of the ibr-loop */
	...
	if (sva[*n] < rootbig && sva[*n] > rootsfmin) {
	    sva[*n] = qnrm2_(m, &a[*n * a_dim1 + 1], &c__1) * d__[*n];
	} else {
	    t = 0.;
	    aapp = 1.;
	    qlassq_(m, &a[*n * a_dim1 + 1], &c__1, &t, &aapp);
...
/*     Additional steering devices */
	...
	if (i__ > swband + 1 && mxaapq < (quadreal) (*n) * *tol && (
...
/*     Sort the vector D. */
	...
	if (p != q) {
	    ...
	    d__[p] = d__[q];
	    d__[q] = temp1;
	    qswap_(m, &a[p * a_dim1 + 1], &c__1, &a[q * a_dim1 + 1], &c__1);
	    if (rsvec) {
		qswap_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * v_dim1 + 1], &
			c__1);
	    }
	}
/* L5991: */
