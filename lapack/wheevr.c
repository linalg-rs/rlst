...
/* > */
/*  ===================================================================== */
void  wheevr_(char *jobz, char *range, char *uplo, integer *n, 
	quadcomplex *a, integer *lda, quadreal *vl, quadreal *vu, 
	integer *il, integer *iu, quadreal *abstol, integer *m, quadreal *
	w, quadcomplex *z__, integer *ldz, integer *isuppz, quadcomplex *
	work, integer *lwork, quadreal *rwork, integer *lrwork, integer *
	iwork, integer *liwork, integer *info)
{
    ...
    /* Local variables */
    integer i__, j, nb, jj;
    quadreal eps, vll, vuu, tmp1, anrm;
    integer imax;
    quadreal rmin, rmax;
    logical test;
    integer itmp1;
    ...
    logical lquery;
    integer lwkopt;
    extern quadreal wlansy_(char *, char *, integer *, quadcomplex *, 
	    integer *, quadreal *);
    extern void  wstemr_(char *, char *, integer *, quadreal 
	    *, quadreal *, quadreal *, quadreal *, integer *, integer *,
	     integer *, quadreal *, quadcomplex *, integer *, integer *, 
	    integer *, logical *, quadreal *, integer *, integer *, integer 
	    *, integer *), wunmtr_(char *, char *, char *, 
...
/*     Scale matrix to allowable range, if necessary. */
    ...
    anrm = wlansy_("M", uplo, n, &a[a_offset], lda, &rwork[1]);
    if (anrm > 0. && anrm < rmin) {
	iscale = 1;
	sigma = rmin / anrm;
    } else if (anrm > rmax) {
...
/*     Call ZHETRD to reduce Hermitian matrix to tridiagonal form. */
    ...
    whetrd_(uplo, n, &a[a_offset], lda, &rwork[indrd], &rwork[indre], &work[
...
/*     then call DSTERF or ZSTEMR and ZUNMTR. */
	...
	if (! wantz) {
	    qcopy_(n, &rwork[indrd], &c__1, &w[1], &c__1);
	    i__1 = *n - 1;
	    qcopy_(&i__1, &rwork[indre], &c__1, &rwork[indree], &c__1);
	    qsterf_(n, &w[1], &rwork[indree], info);
	} else {
	    i__1 = *n - 1;
	    qcopy_(&i__1, &rwork[indre], &c__1, &rwork[indree], &c__1);
	    qcopy_(n, &rwork[indrd], &c__1, &rwork[indrdd], &c__1);
	    ...
	    wstemr_(jobz, "A", n, &rwork[indrdd], &rwork[indree], vl, vu, il, 
...
/*           form to eigenvectors returned by ZSTEMR. */
		...
		wunmtr_("L", uplo, "N", n, m, &a[a_offset], lda, &work[indtau]
...
/*     Also call DSTEBZ and ZSTEIN if ZSTEMR fails. */
	...
	*(unsigned char *)order = 'E';
    ...
    qstebz_(range, order, n, &vll, &vuu, il, iu, &abstll, &rwork[indrd], &
	    rwork[indre], m, &nsplit, &w[1], &iwork[indibl], &iwork[indisp], &
	    rwork[indrwk], &iwork[indiwo], info);

    if (wantz) {
	wstein_(n, &rwork[indrd], &rwork[indre], m, &w[1], &iwork[indibl], &
