...
/*  ===================================================================== */
void  qlaqtr_(logical *ltran, logical *lreal, integer *n, 
	quadreal *t, integer *ldt, quadreal *b, quadreal *w, quadreal 
	*scale, quadreal *x, quadreal *work, integer *info)
{
    ...
    /* Local variables */
    quadreal d__[4]	/* was [2][2] */;
    integer i__, j, k;
    quadreal v[4]	/* was [2][2] */, z__;
    integer j1, j2, n1, n2;
    quadreal si, xj, sr, rec, eps, tjj, tmp;
...
/*     Set constants to control overflow */
    ...
    xnorm = qlange_("M", n, n, &t[t_offset], ldt, d__);
    if (! (*lreal)) {
/* Computing MAX */
	d__1 = xnorm, d__2 = abs(*w), d__1 = f2cmax(d__1,d__2), d__2 = qlange_(
		"M", n, &c__1, &b[1], n, d__);
...
/*     part of T to control overflow in triangular solver. */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	work[j] = qasum_(&i__2, &t[j * t_dim1 + 1], &c__1);
/* L10: */
    }
...
/*                 multiple of column j1 of T. */
			...
			i__1 = j1 - 1;
			d__1 = -x[j1];
			qaxpy_(&i__1, &d__1, &t[j1 * t_dim1 + 1], &c__1, &x[1]
				, &c__1);
			i__1 = j1 - 1;
...
/*                 care of possible overflow by scaling factor. */
		    ...
		    d__[0] = x[j1];
		    d__[1] = x[j2];
		    qlaln2_(&c_false, &c__2, &c__1, &smin, &c_b21, &t[j1 + j1 
...
/*                 Update right-hand side */
			...
			i__1 = j1 - 1;
			d__1 = -x[j2];
			qaxpy_(&i__1, &d__1, &t[j2 * t_dim1 + 1], &c__1, &x[1]
				, &c__1);
			i__1 = j1 - 1;
			k = iqamax_(&i__1, &x[1], &c__1);
			xmax = (d__1 = x[k], abs(d__1));
...
/*                 right-hand side element by inner product. */
		    ...
		    i__2 = j1 - 1;
		    x[j1] -= qdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, &x[1], &
			    c__1);
...
/* Computing MAX */
		    ...
		    i__2 = j1 - 1;
		    d__[0] = x[j1] - qdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, 
			    &x[1], &c__1);
		    i__2 = j1 - 1;
		    d__[1] = x[j2] - qdot_(&i__2, &t[j2 * t_dim1 + 1], &c__1, 
			    &x[1], &c__1);

		    qlaln2_(&c_true, &c__2, &c__1, &smin, &c_b21, &t[j1 + j1 *
...
/*                 Scale if necessary to avoid overflow in division */
			...
			*info = 1;
		    ...
		    qladiv_(&x[j1], &x[*n + j1], &tmp, &z__, &sr, &si);
		    x[j1] = sr;
		    x[*n + j1] = si;
		    xj = (d__1 = x[j1], abs(d__1)) + (d__2 = x[*n + j1], abs(
			    d__2));
...
/*                 multiple of column j1 of T. */
			...
			i__1 = j1 - 1;
			d__1 = -x[*n + j1];
			qaxpy_(&i__1, &d__1, &t[j1 * t_dim1 + 1], &c__1, &x[*
				n + 1], &c__1);
...
/*                 Meet 2 by 2 diagonal block */
		    ...
		    d__[3] = x[*n + j2];
		    d__1 = -(*w);
		    qlaln2_(&c_false, &c__2, &c__2, &sminw, &c_b21, &t[j1 + 
...
/*                 Update the right-hand side. */
			...
			i__1 = j1 - 1;
			d__1 = -x[*n + j2];
			qaxpy_(&i__1, &d__1, &t[j2 * t_dim1 + 1], &c__1, &x[*
				n + 1], &c__1);
...
/*                 right-hand side element by inner product. */
		    ...
		    x[j1] -= qdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, &x[1], &
			    c__1);
		    i__2 = j1 - 1;
		    x[*n + j1] -= qdot_(&i__2, &t[j1 * t_dim1 + 1], &c__1, &x[
			    *n + 1], &c__1);
		    if (j1 > 1) {
			x[j1] -= b[j1] * x[*n + 1];
			x[*n + j1] += b[j1] * x[1];
...
/*                 complex division */
			...
			*info = 1;
		    ...
		    d__1 = -z__;
		    qladiv_(&x[j1], &x[*n + j1], &tmp, &d__1, &sr, &si);
		    x[j1] = sr;
		    x[j1 + *n] = si;
/* Computing MAX */
...
/* Computing MAX */
		    ...
		    d__[1] = x[j2] - qdot_(&i__2, &t[j2 * t_dim1 + 1], &c__1, 
			    &x[1], &c__1);
		    i__2 = j1 - 1;
		    d__[2] = x[*n + j1] - qdot_(&i__2, &t[j1 * t_dim1 + 1], &
			    c__1, &x[*n + 1], &c__1);
		    i__2 = j1 - 1;
		    d__[3] = x[*n + j2] - qdot_(&i__2, &t[j2 * t_dim1 + 1], &
			    c__1, &x[*n + 1], &c__1);
		    d__[0] -= b[j1] * x[*n + 1];
		    d__[1] -= b[j2] * x[*n + 1];
		    d__[2] += b[j1] * x[1];
		    ...
		    qlaln2_(&c_true, &c__2, &c__2, &sminw, &c_b21, &t[j1 + j1 
			    * t_dim1], ldt, &c_b21, &c_b21, d__, &c__2, &
