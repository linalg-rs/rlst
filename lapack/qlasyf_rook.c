...
void  dlasyf_rook__(char *uplo, integer *n, integer *nb, 
	integer *kb, quadreal *a, integer *lda, integer *ipiv, quadreal *
...
/*        Copy column K of A to column KW of W and update it */
	...
	qcopy_(&k, &a[k * a_dim1 + 1], &c__1, &w[kw * w_dim1 + 1], &c__1);
	if (k < *n) {
	    i__1 = *n - k;
	    qgemv_("No transpose", &k, &i__1, &c_b9, &a[(k + 1) * a_dim1 + 1],
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    imax = iqamax_(&i__1, &w[kw * w_dim1 + 1], &c__1);
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    kp = k;
	    qcopy_(&k, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &c__1);
	} else {
...
/*                 Copy column IMAX to column KW-1 of W and update it */
		...
		qcopy_(&imax, &a[imax * a_dim1 + 1], &c__1, &w[(kw - 1) * 
			w_dim1 + 1], &c__1);
		i__1 = k - imax;
		qcopy_(&i__1, &a[imax + (imax + 1) * a_dim1], lda, &w[imax + 
			1 + (kw - 1) * w_dim1], &c__1);

		if (k < *n) {
		    i__1 = *n - k;
		    qgemv_("No transpose", &k, &i__1, &c_b9, &a[(k + 1) * 
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    itemp = iqamax_(&i__1, &w[(kw - 1) * w_dim1 + 1], &c__1);
...
/*                    copy column KW-1 of W to column KW of W */
		    ...
		    qcopy_(&k, &w[(kw - 1) * w_dim1 + 1], &c__1, &w[kw * 
			    w_dim1 + 1], &c__1);
...
/*              Copy non-updated column K to column P */
		...
		i__1 = k - p;
		qcopy_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 1) * 
			a_dim1], lda);
		qcopy_(&p, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 1], &
			c__1);
...
/*              and last N-K+2 columns of W */
		...
		i__1 = *n - k + 1;
		qswap_(&i__1, &a[k + k * a_dim1], lda, &a[p + k * a_dim1], 
			lda);
		i__1 = *n - kk + 1;
		qswap_(&i__1, &w[k + kkw * w_dim1], ldw, &w[p + kkw * w_dim1],
			 ldw);
	    }
...
/*              Copy non-updated column KK to column KP */
		...
		i__1 = k - 1 - kp;
		qcopy_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (kp + 
			1) * a_dim1], lda);
		qcopy_(&kp, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 + 1], &
			c__1);
...
/*              of A and W */
		...
		i__1 = *n - kk + 1;
		qswap_(&i__1, &a[kk + kk * a_dim1], lda, &a[kp + kk * a_dim1],
			 lda);
		i__1 = *n - kk + 1;
		qswap_(&i__1, &w[kk + kkw * w_dim1], ldw, &w[kp + kkw * 
			w_dim1], ldw);
	    }
...
/*              Store U(k) in column k of A */
		...
		qcopy_(&k, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &
			c__1);
		if (k > 1) {
		    if ((d__1 = a[k + k * a_dim1], abs(d__1)) >= sfmin) {
			r1 = 1. / a[k + k * a_dim1];
			i__1 = k - 1;
			qscal_(&i__1, &r1, &a[k * a_dim1 + 1], &c__1);
...
/*           Update the upper triangle of the diagonal block */
		...
		i__3 = jj - j + 1;
		i__4 = *n - k;
		qgemv_("No transpose", &i__3, &i__4, &c_b9, &a[j + (k + 1) * 
...
/*           Update the rectangular superdiagonal block */
		...
		i__2 = j - 1;
		i__3 = *n - k;
		qgemm_("No transpose", "Transpose", &i__2, &jb, &i__3, &c_b9, 
...
L60:
	...
	if (jp2 != jj && j <= *n) {
	    i__1 = *n - j + 1;
	    qswap_(&i__1, &a[jp2 + j * a_dim1], lda, &a[jj + j * a_dim1], lda)
		    ;
	}
	jj = j - 1;
	if (jp1 != jj && kstep == 2) {
	    i__1 = *n - j + 1;
	    qswap_(&i__1, &a[jp1 + j * a_dim1], lda, &a[jj + j * a_dim1], lda)
		    ;
	}
	if (j <= *n) {
	    goto L60;
...
/*        Copy column K of A to column K of W and update it */
	...
	i__1 = *n - k + 1;
	qcopy_(&i__1, &a[k + k * a_dim1], &c__1, &w[k + k * w_dim1], &c__1);
	if (k > 1) {
	    i__1 = *n - k + 1;
	    i__2 = k - 1;
	    qgemv_("No transpose", &i__1, &i__2, &c_b9, &a[k + a_dim1], lda, &
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + iqamax_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    kp = k;
	    i__1 = *n - k + 1;
	    qcopy_(&i__1, &w[k + k * w_dim1], &c__1, &a[k + k * a_dim1], &
		    c__1);
	} else {
...
/*                 Copy column IMAX to column K+1 of W and update it */
		...
		i__1 = imax - k;
		qcopy_(&i__1, &a[imax + k * a_dim1], lda, &w[k + (k + 1) * 
			w_dim1], &c__1);
		i__1 = *n - imax + 1;
		qcopy_(&i__1, &a[imax + imax * a_dim1], &c__1, &w[imax + (k + 
			1) * w_dim1], &c__1);
		if (k > 1) {
		    i__1 = *n - k + 1;
		    i__2 = k - 1;
		    qgemv_("No transpose", &i__1, &i__2, &c_b9, &a[k + a_dim1]
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = imax - k;
		    jmax = k - 1 + iqamax_(&i__1, &w[k + (k + 1) * w_dim1], &
			    c__1);
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    itemp = imax + iqamax_(&i__1, &w[imax + 1 + (k + 1) * 
			    w_dim1], &c__1);
...
/*                    copy column K+1 of W to column K of W */
		    ...
		    i__1 = *n - k + 1;
		    qcopy_(&i__1, &w[k + (k + 1) * w_dim1], &c__1, &w[k + k * 
			    w_dim1], &c__1);
...
/*              Copy non-updated column K to column P */
		...
		i__1 = p - k;
		qcopy_(&i__1, &a[k + k * a_dim1], &c__1, &a[p + k * a_dim1], 
			lda);
		i__1 = *n - p + 1;
		qcopy_(&i__1, &a[p + k * a_dim1], &c__1, &a[p + p * a_dim1], &
			c__1);

/*              Interchange rows K and P in first K columns of A */
/*              and first K+1 columns of W */

		qswap_(&k, &a[k + a_dim1], lda, &a[p + a_dim1], lda);
		qswap_(&kk, &w[k + w_dim1], ldw, &w[p + w_dim1], ldw);
	    }
...
/*              Copy non-updated column KK to column KP */
		...
		a[kp + k * a_dim1] = a[kk + k * a_dim1];
		i__1 = kp - k - 1;
		qcopy_(&i__1, &a[k + 1 + kk * a_dim1], &c__1, &a[kp + (k + 1) 
			* a_dim1], lda);
		i__1 = *n - kp + 1;
		qcopy_(&i__1, &a[kp + kk * a_dim1], &c__1, &a[kp + kp * 
			a_dim1], &c__1);

/*              Interchange rows KK and KP in first KK columns of A and W */

		qswap_(&kk, &a[kk + a_dim1], lda, &a[kp + a_dim1], lda);
		qswap_(&kk, &w[kk + w_dim1], ldw, &w[kp + w_dim1], ldw);
	    }
...
/*              Store L(k) in column k of A */
		...
		if (k < *n) {
			...
			r1 = 1. / a[k + k * a_dim1];
			i__1 = *n - k;
			qscal_(&i__1, &r1, &a[k + 1 + k * a_dim1], &c__1);
		    } else if (a[k + k * a_dim1] != 0.) {
			i__1 = *n;
...
/*           Update the lower triangle of the diagonal block */
		...
		i__4 = j + jb - jj;
		i__5 = k - 1;
		qgemv_("No transpose", &i__4, &i__5, &c_b9, &a[jj + a_dim1], 
...
/*           Update the rectangular subdiagonal block */
		...
		i__3 = *n - j - jb + 1;
		i__4 = k - 1;
		qgemm_("No transpose", "Transpose", &i__3, &jb, &i__4, &c_b9, 
...
L120:
	...
	--j;
	if (jp2 != jj && j >= 1) {
	    qswap_(&j, &a[jp2 + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	jj = j + 1;
	if (jp1 != jj && kstep == 2) {
	    qswap_(&j, &a[jp1 + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	if (j >= 1) {
	    goto L120;
	}
