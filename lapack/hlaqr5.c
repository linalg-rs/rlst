...
/* >             SR contains the doublereal parts and SI contains the imaginary */
...
/* > */
/*  ===================================================================== */
void  hlaqr5_(logical *wantt, logical *wantz, integer *kacc22, 
	integer *n, integer *ktop, integer *kbot, integer *nshfts, halfreal 
	*sr, halfreal *si, halfreal *h__, integer *ldh, integer *iloz, 
	integer *ihiz, halfreal *z__, integer *ldz, halfreal *v, integer *
	ldv, halfreal *u, integer *ldu, integer *nv, halfreal *wv, 
	integer *ldwv, integer *nh, halfreal *wh, integer *ldwh)
{
...
/*     ==== Shuffle shifts into pairs of doublereal shifts and pairs */
...
/*     .    ensures that the dropped shift is doublereal and that */
...
/*     ==== Create and chase chains of NBMPS bulges ==== */
	...
	ndcol = incol + kdu;
	if (accum) {
	    hlaset_("ALL", &kdu, &kdu, &c_b7, &c_b8, &u[u_offset], ldu);
	}
...
/*           .    one column.  (The minimum value of K is KTOP-1.) ==== */
		...
		k = krcol + (m - 1) * 3;
		if (k == *ktop - 1) {
		    hlaqr1_(&c__3, &h__[*ktop + *ktop * h_dim1], ldh, &sr[(m 
			    << 1) - 1], &si[(m << 1) - 1], &sr[m * 2], &si[m *
			     2], &v[m * v_dim1 + 1]);
		    alpha = v[m * v_dim1 + 1];
		    hlarfg_(&c__3, &alpha, &v[m * v_dim1 + 2], &c__1, &v[m * 
			    v_dim1 + 1]);
		} else {
		    beta = h__[k + 1 + k * h_dim1];
		    v[m * v_dim1 + 2] = h__[k + 2 + k * h_dim1];
		    v[m * v_dim1 + 3] = h__[k + 3 + k * h_dim1];
		    hlarfg_(&c__3, &beta, &v[m * v_dim1 + 2], &c__1, &v[m * 
			    v_dim1 + 1]);
...
/*                    .    Otherwise, use the new one. ==== */
			...
			hlaqr1_(&c__3, &h__[k + 1 + (k + 1) * h_dim1], ldh, &
				sr[(m << 1) - 1], &si[(m << 1) - 1], &sr[m * 
				2], &si[m * 2], vt);
			alpha = vt[0];
			hlarfg_(&c__3, &alpha, &vt[1], &c__1, vt);
...
/*           ==== Generate a 2-by-2 reflection, if needed. ==== */
	    ...
	    if (bmp22) {
		if (k == *ktop - 1) {
		    hlaqr1_(&c__2, &h__[k + 1 + (k + 1) * h_dim1], ldh, &sr[(
			    m22 << 1) - 1], &si[(m22 << 1) - 1], &sr[m22 * 2],
			     &si[m22 * 2], &v[m22 * v_dim1 + 1]);
		    beta = v[m22 * v_dim1 + 1];
		    hlarfg_(&c__2, &beta, &v[m22 * v_dim1 + 2], &c__1, &v[m22 
			    * v_dim1 + 1]);
		} else {
		    beta = h__[k + 1 + k * h_dim1];
...
/* Computing MIN */
		    ...
		    hgemm_("C", "N", &nu, &jlen, &nu, &c_b8, &u[k1 + k1 * 
			    u_dim1], ldu, &h__[incol + k1 + jcol * h_dim1], 
			    ldh, &c_b7, &wh[wh_offset], ldwh);
		    hlacpy_("ALL", &nu, &jlen, &wh[wh_offset], ldwh, &h__[
...
/* Computing MIN */
		    ...
		    jlen = f2cmin(i__5,i__7);
		    hgemm_("N", "N", &jlen, &nu, &nu, &c_b8, &h__[jrow + (
			    incol + k1) * h_dim1], ldh, &u[k1 + k1 * u_dim1], 
			    ldu, &c_b7, &wv[wv_offset], ldwv);
		    hlacpy_("ALL", &jlen, &nu, &wv[wv_offset], ldwv, &h__[
...
/* Computing MIN */
			...
			jlen = f2cmin(i__5,i__7);
			hgemm_("N", "N", &jlen, &nu, &nu, &c_b8, &z__[jrow + (
				incol + k1) * z_dim1], ldz, &u[k1 + k1 * 
				u_dim1], ldu, &c_b7, &wv[wv_offset], ldwv);
			hlacpy_("ALL", &jlen, &nu, &wv[wv_offset], ldwv, &z__[
...
/*                  (The first KZS rows get multiplied by zero.) ==== */
		    ...
		    hlacpy_("ALL", &knz, &jlen, &h__[incol + 1 + j2 + jcol * 
			    h_dim1], ldh, &wh[kzs + 1 + wh_dim1], ldwh);

/*                 ==== Multiply by U21**T ==== */

		    hlaset_("ALL", &kzs, &jlen, &c_b7, &c_b7, &wh[wh_offset], 
			    ldwh);
		    htrmm_("L", "U", "C", "N", &knz, &jlen, &c_b8, &u[j2 + 1 
			    + (kzs + 1) * u_dim1], ldu, &wh[kzs + 1 + wh_dim1]
			    , ldwh);

/*                 ==== Multiply top of H by U11**T ==== */

		    hgemm_("C", "N", &i2, &jlen, &j2, &c_b8, &u[u_offset], 
			    ldu, &h__[incol + 1 + jcol * h_dim1], ldh, &c_b8, 
			    &wh[wh_offset], ldwh);

/*                 ==== Copy top of H to bottom of WH ==== */

		    hlacpy_("ALL", &j2, &jlen, &h__[incol + 1 + jcol * h_dim1]
			    , ldh, &wh[i2 + 1 + wh_dim1], ldwh);

/*                 ==== Multiply by U21**T ==== */

		    htrmm_("L", "L", "C", "N", &j2, &jlen, &c_b8, &u[(i2 + 1) 
...
/*                 ==== Multiply by U22 ==== */
		    ...
		    i__5 = i4 - i2;
		    i__7 = j4 - j2;
		    hgemm_("C", "N", &i__5, &jlen, &i__7, &c_b8, &u[j2 + 1 + (
...
/*                 ==== Copy it back ==== */
		    ...
		    hlacpy_("ALL", &kdu, &jlen, &wh[wh_offset], ldwh, &h__[
...
/*                 .    columns get multiplied by zero) ==== */
		    ...
		    hlacpy_("ALL", &jlen, &knz, &h__[jrow + (incol + 1 + j2) *
			     h_dim1], ldh, &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

/*                 ==== Multiply by U21 ==== */

		    hlaset_("ALL", &jlen, &kzs, &c_b7, &c_b7, &wv[wv_offset], 
			    ldwv);
		    htrmm_("R", "U", "N", "N", &jlen, &knz, &c_b8, &u[j2 + 1 
			    + (kzs + 1) * u_dim1], ldu, &wv[(kzs + 1) * 
			    wv_dim1 + 1], ldwv);

/*                 ==== Multiply by U11 ==== */

		    hgemm_("N", "N", &jlen, &i2, &j2, &c_b8, &h__[jrow + (
			    incol + 1) * h_dim1], ldh, &u[u_offset], ldu, &
			    c_b8, &wv[wv_offset], ldwv);

/*                 ==== Copy left of H to right of scratch ==== */

		    hlacpy_("ALL", &jlen, &j2, &h__[jrow + (incol + 1) * 
			    h_dim1], ldh, &wv[(i2 + 1) * wv_dim1 + 1], ldwv);

/*                 ==== Multiply by U21 ==== */

		    i__5 = i4 - i2;
		    htrmm_("R", "L", "N", "N", &jlen, &i__5, &c_b8, &u[(i2 + 
...
/*                 ==== Multiply by U22 ==== */
		    ...
		    i__5 = i4 - i2;
		    i__7 = j4 - j2;
		    hgemm_("N", "N", &jlen, &i__5, &i__7, &c_b8, &h__[jrow + (
...
/*                 ==== Copy it back ==== */
		    ...
		    hlacpy_("ALL", &jlen, &kdu, &wv[wv_offset], ldwv, &h__[
...
/*                    .     KZS columns get multiplied by zero) ==== */
			...
			hlacpy_("ALL", &jlen, &knz, &z__[jrow + (incol + 1 + 
				j2) * z_dim1], ldz, &wv[(kzs + 1) * wv_dim1 + 
				1], ldwv);

/*                    ==== Multiply by U12 ==== */

			hlaset_("ALL", &jlen, &kzs, &c_b7, &c_b7, &wv[
				wv_offset], ldwv);
			htrmm_("R", "U", "N", "N", &jlen, &knz, &c_b8, &u[j2 
				+ 1 + (kzs + 1) * u_dim1], ldu, &wv[(kzs + 1) 
				* wv_dim1 + 1], ldwv);

/*                    ==== Multiply by U11 ==== */

			hgemm_("N", "N", &jlen, &i2, &j2, &c_b8, &z__[jrow + (
				incol + 1) * z_dim1], ldz, &u[u_offset], ldu, 
				&c_b8, &wv[wv_offset], ldwv);

/*                    ==== Copy left of Z to right of scratch ==== */

			hlacpy_("ALL", &jlen, &j2, &z__[jrow + (incol + 1) * 
...
/*                    ==== Multiply by U21 ==== */
			...
			i__5 = i4 - i2;
			htrmm_("R", "L", "N", "N", &jlen, &i__5, &c_b8, &u[(
...
/*                    ==== Multiply by U22 ==== */
			...
			i__5 = i4 - i2;
			i__7 = j4 - j2;
			hgemm_("N", "N", &jlen, &i__5, &i__7, &c_b8, &z__[
...
/*                    ==== Copy the result back to Z ==== */
			...
			hlacpy_("ALL", &jlen, &kdu, &wv[wv_offset], ldwv, &
