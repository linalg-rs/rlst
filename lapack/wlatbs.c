...
/* > */
/*  ===================================================================== */
void  wlatbs_(char *uplo, char *trans, char *diag, char *
	normin, integer *n, integer *kd, quadcomplex *ab, integer *ldab, 
	quadcomplex *x, quadreal *scale, quadreal *cnorm, integer *info)
{
    ...
    logical upper;
    extern /* Double Complex */ VOID wqotu_(quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *);
    extern void  wtbsv_(char *, char *, char *, integer *, 
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *), waxpy_(integer *, quadcomplex *, 
	    quadcomplex *, integer *, quadcomplex *, integer *), qlabad_(
	    quadreal *, quadreal *);
...
/* Computing MIN */
		...
		cnorm[j] = qwasum_(&jlen, &ab[*kd + 1 - jlen + j * ab_dim1], &
			c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = f2cmin(i__2,i__3);
		if (jlen > 0) {
		    cnorm[j] = qwasum_(&jlen, &ab[j * ab_dim1 + 2], &c__1);
		} else {
		    cnorm[j] = 0.;
		}
/* L20: */
...
/*        elements of X is not too small. */
	...
	wtbsv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &x[1], &c__1);
    } else {
...
/*                          Scale x by 1/b(j). */
			    ...
			    rec = 1. / xj;
			    wqscal_(n, &rec, &x[1], &c__1);
			    *scale *= rec;
			    xmax *= rec;
			}
		    }
		    i__3 = j;
		    wladiv_(&z__1, &x[j], &tjjs);
...
/* Computing MIN */
			...
			waxpy_(&jlen, &z__1, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
			i__3 = j - 1;
			i__ = iwamax_(&i__3, &x[1], &c__1);
			i__3 = i__;
...
/* Computing MIN */
			...
			z__2.r = -x[i__3].r, z__2.i = -x[i__3].i;
			z__1.r = tscal * z__2.r, z__1.i = tscal * z__2.i;
			waxpy_(&jlen, &z__1, &ab[j * ab_dim1 + 2], &c__1, &x[
				j + 1], &c__1);
		    }
		    i__3 = *n - j;
		    i__ = j + iwamax_(&i__3, &x[j + 1], &c__1);
		    i__3 = i__;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = f2cmin(i__3,i__4);
			wqotu_(&z__1, &jlen, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
...
/* Computing MIN */
			...
			jlen = f2cmin(i__3,i__4);
			if (jlen > 1) {
			    wqotu_(&z__1, &jlen, &ab[j * ab_dim1 + 2], &c__1, 
				    &x[j + 1], &c__1);
			    csumj.r = z__1.r, csumj.i = z__1.i;
			}
		    }
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = f2cmin(i__3,i__4);
			wqotc_(&z__1, &jlen, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    } else {
/* Computing MIN */
			...
			jlen = f2cmin(i__3,i__4);
			if (jlen > 1) {
			    wqotc_(&z__1, &jlen, &ab[j * ab_dim1 + 2], &c__1, 
				    &x[j + 1], &c__1);
			    csumj.r = z__1.r, csumj.i = z__1.i;
			}
		    }
