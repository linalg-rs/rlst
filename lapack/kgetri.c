...
/*  ===================================================================== */
void  kgetri_(integer *n, halfcomplex *a, integer *lda, 
	integer *ipiv, halfcomplex *work, integer *lwork, integer *info)
{
    ...
    extern void  kgemm_(char *, char *, integer *, integer *, 
	    ...
	    integer *), ktrsm_(char *, char *, char *, char *, integer *, 
	    integer *, halfcomplex *, halfcomplex *, integer *, 
	    halfcomplex *, integer *), 
...
/*     and the inverse is not computed. */
    ...
    ktrtri_("Upper", "Non-unit", n, &a[a_offset], lda, info);
    if (*info > 0) {
	return;
    }
...
/*           Compute current column of inv(A). */
		...
		i__1 = *n - j;
		z__1.r = -1., z__1.i = -0.;
		kgemv_("No transpose", n, &i__1, &z__1, &a[(j + 1) * a_dim1 + 
...
/*           Compute current block column of inv(A). */
		...
		i__2 = *n - j - jb + 1;
		z__1.r = -1., z__1.i = -0.;
		kgemm_("No transpose", "No transpose", n, &jb, &i__2, &z__1, &
			a[(j + jb) * a_dim1 + 1], lda, &work[j + jb], &ldwork,
			 &c_b2, &a[j * a_dim1 + 1], lda);
	    }
	    ktrsm_("Right", "Lower", "No transpose", "Unit", n, &jb, &c_b2, &
...
/*     Apply column interchanges. */
	...
	jp = ipiv[j];
	if (jp != j) {
	    kswap_(n, &a[j * a_dim1 + 1], &c__1, &a[jp * a_dim1 + 1], &c__1);
	}
/* L60: */
    }
