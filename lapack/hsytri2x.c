...
/*  ===================================================================== */
void  hsytri2x_(char *uplo, integer *n, halfreal *a, integer 
	*lda, integer *ipiv, halfreal *work, integer *nb, integer *info)
{
    /* System generated locals */
    integer a_dim1, a_offset, work_dim1, work_offset, i__1, i__2, i__3;

    /* Local variables */
    extern void  hsyswapr_(char *, integer *, halfreal *, 
    ...
    integer count;
    logical upper;
    halfreal u01_i_j__, u11_i_j__;
    extern void  xerbla_(char *, integer *), htrtri_(
	    char *, char *, integer *, halfreal *, integer *, integer *), hsyconv_(char *, char *, integer *, halfreal *,
	     integer *, integer *, halfreal *, integer *);
    halfreal u01_ip1_j__, u11_ip1_j__;
...
/*     Workspace got Non-diag elements of D */
    ...
    hsyconv_(uplo, "C", n, &a[a_offset], lda, &ipiv[1], &work[work_offset], &
	    iinfo);
...
/*        invA = P * inv(U**T)*inv(D)*inv(U)*P**T. */
	...
	htrtri_(uplo, "U", n, &a[a_offset], lda, info);
...
/*       U11**T*invD1*U11->U11 */
	    ...
	    i__1 = *n + *nb + 1;
	    htrmm_("L", "U", "T", "U", &nnb, &nnb, &c_b11, &a[cut + 1 + (cut 
...
/*          U01**T*invD*U01->A(CUT+I,CUT+J) */
	    ...
	    i__1 = *n + *nb + 1;
	    i__2 = *n + *nb + 1;
	    hgemm_("T", "N", &nnb, &nnb, &cut, &c_b11, &a[(cut + 1) * a_dim1 
...
/*        U01 =  U00**T*invD0*U01 */
	    ...
	    i__1 = *n + *nb + 1;
	    htrmm_("L", uplo, "T", "U", &cut, &nnb, &c_b11, &a[a_offset], lda,
...
/*        Apply PERMUTATIONS P and P**T: P * inv(U**T)*inv(D)*inv(U) *P**T */
	...
	while(i__ <= *n) {
		...
		ip = ipiv[i__];
		if (i__ < ip) {
		    hsyswapr_(uplo, n, &a[a_offset], lda, &i__, &ip);
		}
		if (i__ > ip) {
		    hsyswapr_(uplo, n, &a[a_offset], lda, &ip, &i__);
		}
	    } else {
		ip = -ipiv[i__];
		++i__;
		if (i__ - 1 < ip) {
		    i__1 = i__ - 1;
		    hsyswapr_(uplo, n, &a[a_offset], lda, &i__1, &ip);
		}
		if (i__ - 1 > ip) {
		    i__1 = i__ - 1;
		    hsyswapr_(uplo, n, &a[a_offset], lda, &ip, &i__1);
		}
	    }
	    ++i__;
	}
...
/*       L11**T*invD1*L11->L11 */
	    ...
	    i__1 = *n + *nb + 1;
	    htrmm_("L", uplo, "T", "U", &nnb, &nnb, &c_b11, &a[cut + 1 + (cut 
		    + 1) * a_dim1], lda, &work[u11 + 1 + work_dim1], &i__1);
...
/*          L21**T*invD2*L21->A(CUT+I,CUT+J) */
		...
		i__2 = *n + *nb + 1;
		i__3 = *n + *nb + 1;
		hgemm_("T", "N", &nnb, &nnb, &i__1, &c_b11, &a[cut + nnb + 1 
...
/*        L01 =  L22**T*invD2*L21 */
		...
		i__1 = *n - nnb - cut;
		i__2 = *n + *nb + 1;
		htrmm_("L", uplo, "T", "U", &i__1, &nnb, &c_b11, &a[cut + nnb 
