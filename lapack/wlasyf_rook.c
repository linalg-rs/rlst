...
void  zlasyf_rook__(char *uplo, integer *n, integer *nb, 
	integer *kb, quadcomplex *a, integer *lda, integer *ipiv, 
	quadcomplex *w, integer *ldw, integer *info)
{
    ...
    /* Local variables */
    integer j, k, p;
    quadcomplex t, r1, d11, d12, d21, d22;
...
/*        Copy column K of A to column KW of W and update it */
	...
	wcopy_(&k, &a[k * a_dim1 + 1], &c__1, &w[kw * w_dim1 + 1], &c__1);
	if (k < *n) {
	    i__1 = *n - k;
	    z__1.r = -1., z__1.i = -0.;
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    kp = k;
	    wcopy_(&k, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &c__1);
	} else {
...
/*                 Copy column IMAX to column KW-1 of W and update it */
		...
		wcopy_(&imax, &a[imax * a_dim1 + 1], &c__1, &w[(kw - 1) * 
			w_dim1 + 1], &c__1);
		i__1 = k - imax;
...
/*              Copy non-updated column K to column P */
		...
		wcopy_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 1) * 
			a_dim1], lda);
		wcopy_(&p, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 1], &
			c__1);
...
/*              and last N-K+2 columns of W */
		...
		i__1 = *n - k + 1;
		wswap_(&i__1, &a[k + k * a_dim1], lda, &a[p + k * a_dim1], 
			lda);
		i__1 = *n - kk + 1;
...
/*              Copy non-updated column KK to column KP */
		...
		wcopy_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (kp + 
			1) * a_dim1], lda);
		wcopy_(&kp, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 + 1], &
			c__1);
...
/*              of A and W */
		...
		i__1 = *n - kk + 1;
		wswap_(&i__1, &a[kk + kk * a_dim1], lda, &a[kp + kk * a_dim1],
			 lda);
		i__1 = *n - kk + 1;
...
/*              Store U(k) in column k of A */
		...
		if (k > 1) {
			...
			i__1 = k - 1;
			wscal_(&i__1, &r1, &a[k * a_dim1 + 1], &c__1);
...
L60:
	...
	if (jp2 != jj && j <= *n) {
	    i__1 = *n - j + 1;
	    wswap_(&i__1, &a[jp2 + j * a_dim1], lda, &a[jj + j * a_dim1], lda)
		    ;
	}
	jj = j - 1;
	if (jp1 != jj && kstep == 2) {
	    i__1 = *n - j + 1;
	    wswap_(&i__1, &a[jp1 + j * a_dim1], lda, &a[jj + j * a_dim1], lda)
		    ;
	}
	if (j <= *n) {
	    goto L60;
...
/*        Copy column K of A to column K of W and update it */
	...
	i__1 = *n - k + 1;
	wcopy_(&i__1, &a[k + k * a_dim1], &c__1, &w[k + k * w_dim1], &c__1);
	if (k > 1) {
	    i__1 = *n - k + 1;
	    i__2 = k - 1;
	    z__1.r = -1., z__1.i = -0.;
...
/*                 Copy column IMAX to column K+1 of W and update it */
		...
		wcopy_(&i__1, &a[imax + k * a_dim1], lda, &w[k + (k + 1) * 
			w_dim1], &c__1);
		i__1 = *n - imax + 1;
		wcopy_(&i__1, &a[imax + imax * a_dim1], &c__1, &w[imax + (k + 
			1) * w_dim1], &c__1);
		if (k > 1) {
		    i__1 = *n - k + 1;
		    i__2 = k - 1;
...
/*              Copy non-updated column K to column P */
		...
		i__1 = p - k;
		wcopy_(&i__1, &a[k + k * a_dim1], &c__1, &a[p + k * a_dim1], 
			lda);
		i__1 = *n - p + 1;
		wcopy_(&i__1, &a[p + k * a_dim1], &c__1, &a[p + p * a_dim1], &
			c__1);

/*              Interchange rows K and P in first K columns of A */
/*              and first K+1 columns of W */

		wswap_(&k, &a[k + a_dim1], lda, &a[p + a_dim1], lda);
...
/*              Copy non-updated column KK to column KP */
		...
		a[i__1].r = a[i__2].r, a[i__1].i = a[i__2].i;
		i__1 = kp - k - 1;
		wcopy_(&i__1, &a[k + 1 + kk * a_dim1], &c__1, &a[kp + (k + 1) 
			* a_dim1], lda);
		i__1 = *n - kp + 1;
		wcopy_(&i__1, &a[kp + kk * a_dim1], &c__1, &a[kp + kp * 
			a_dim1], &c__1);

/*              Interchange rows KK and KP in first KK columns of A and W */

		wswap_(&kk, &a[kk + a_dim1], lda, &a[kp + a_dim1], lda);
...
/*              Store L(k) in column k of A */
		...
		if (k < *n) {
			...
			r1.r = z__1.r, r1.i = z__1.i;
			i__1 = *n - k;
			wscal_(&i__1, &r1, &a[k + 1 + k * a_dim1], &c__1);
		    } else /* if(complicated condition) */ {
			i__1 = k + k * a_dim1;
			if (a[i__1].r != 0. || a[i__1].i != 0.) {
			    i__1 = *n;
...
L120:
	...
	--j;
	if (jp2 != jj && j >= 1) {
	    wswap_(&j, &a[jp2 + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	jj = j + 1;
	if (jp1 != jj && kstep == 2) {
	    wswap_(&j, &a[jp1 + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	if (j >= 1) {
	    goto L120;
	}
