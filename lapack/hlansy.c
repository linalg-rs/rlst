...
/*  ===================================================================== */
halfreal hlansy_(char *norm, char *uplo, integer *n, halfreal *a, integer 
	*lda, halfreal *work)
{
...
/*        Find normF(A). */
	...
	if (lsame_(uplo, "U")) {
	    ...
	    for (j = 2; j <= i__1; ++j) {
		i__2 = j - 1;
		hlassq_(&i__2, &a[j * a_dim1 + 1], &c__1, &scale, &sum);
/* L110: */
	    }
	} else {
	    i__1 = *n - 1;
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j;
		hlassq_(&i__2, &a[j + 1 + j * a_dim1], &c__1, &scale, &sum);
/* L120: */
	    }
	}
	sum *= 2;
	i__1 = *lda + 1;
	hlassq_(n, &a[a_offset], &i__1, &scale, &sum);
