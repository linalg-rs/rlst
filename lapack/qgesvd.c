...
static quadreal c_b57 = 0.;
static integer c__1 = 1;
static quadreal c_b79 = 1.;
...
/*  ===================================================================== */
void  qgesvd_(char *jobu, char *jobvt, integer *m, integer *n, 
	quadreal *a, integer *lda, quadreal *s, quadreal *u, integer *
	ldu, quadreal *vt, integer *ldvt, quadreal *work, integer *lwork, 
	integer *info)
{
    ...
    extern void  qgebrd_(integer *, integer *, quadreal *, 
    ...
    extern void  qgelqf_(integer *, integer *, quadreal *, 
	    ...
	    qbdsqr_(char *, integer *, integer *, integer *, integer *, 
	    quadreal *, quadreal *, quadreal *, integer *, quadreal *,
	     integer *, quadreal *, integer *, quadreal *, integer *), qorgbr_(char *, integer *, integer *, integer *, 
...
/*           Compute space needed for DGEQRF */
	    qgeqrf_(m, n, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
	    lwork_dgeqrf__ = (integer) dum[0];
/*           Compute space needed for DORGQR */
	    qorgqr_(m, n, n, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
	    lwork_dorgqr_n__ = (integer) dum[0];
	    qorgqr_(m, m, n, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
	    lwork_dorgqr_m__ = (integer) dum[0];
/*           Compute space needed for DGEBRD */
	    qgebrd_(n, n, &a[a_offset], lda, &s[1], dum, dum, dum, dum, &c_n1,
		     &ierr);
	    lwork_dgebrd__ = (integer) dum[0];
/*           Compute space needed for DORGBR P */
	    qorgbr_("P", n, n, n, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
	    lwork_dorgbr_p__ = (integer) dum[0];
/*           Compute space needed for DORGBR Q */
	    qorgbr_("Q", n, n, n, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
...
/*              Path 10 (M at least N, but not much larger) */
		...
		qgebrd_(m, n, &a[a_offset], lda, &s[1], dum, dum, dum, dum, &
			c_n1, &ierr);
		lwork_dgebrd__ = (integer) dum[0];
		maxwrk = *n * 3 + lwork_dgebrd__;
		if (wntus || wntuo) {
		    qorgbr_("Q", m, n, n, &a[a_offset], lda, dum, dum, &c_n1, 
			    &ierr);
		    lwork_dorgbr_q__ = (integer) dum[0];
/* Computing MAX */
		...
		if (wntua) {
		    qorgbr_("Q", m, m, n, &a[a_offset], lda, dum, dum, &c_n1, 
			    &ierr);
		    lwork_dorgbr_q__ = (integer) dum[0];
/* Computing MAX */
		    i__2 = maxwrk, i__3 = *n * 3 + lwork_dorgbr_q__;
...
/*           Compute space needed for DGELQF */
	    qgelqf_(m, n, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
	    lwork_dgelqf__ = (integer) dum[0];
/*           Compute space needed for DORGLQ */
	    qorglq_(n, n, m, dum, n, dum, dum, &c_n1, &ierr);
	    lwork_dorglq_n__ = (integer) dum[0];
	    qorglq_(m, n, m, &a[a_offset], lda, dum, dum, &c_n1, &ierr);
	    lwork_dorglq_m__ = (integer) dum[0];
/*           Compute space needed for DGEBRD */
	    qgebrd_(m, m, &a[a_offset], lda, &s[1], dum, dum, dum, dum, &c_n1,
		     &ierr);
	    lwork_dgebrd__ = (integer) dum[0];
/*            Compute space needed for DORGBR P */
	    ...
	    lwork_dorgbr_p__ = (integer) dum[0];
/*           Compute space needed for DORGBR Q */
	    qorgbr_("Q", m, m, m, &a[a_offset], n, dum, dum, &c_n1, &ierr);
	    lwork_dorgbr_q__ = (integer) dum[0];
	    if (*n >= mnthr) {
		if (wntvn) {
...
/*              Path 10t(N greater than M, but not much larger) */
		...
		if (wntvs || wntvo) {
/*                Compute space needed for DORGBR P */
		    qorgbr_("P", m, n, m, &a[a_offset], n, dum, dum, &c_n1, &
			    ierr);
		    lwork_dorgbr_p__ = (integer) dum[0];
/* Computing MAX */
		...
		if (wntva) {
		    qorgbr_("P", n, n, m, &a[a_offset], n, dum, dum, &c_n1, &
			    ierr);
		    lwork_dorgbr_p__ = (integer) dum[0];
/* Computing MAX */
		    i__2 = maxwrk, i__3 = *m * 3 + lwork_dorgbr_p__;
...
/*              (Workspace: need 2*N, prefer N + N*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[iwork], &
			i__2, &ierr);
...
/*              Zero out below R */
		...
		if (*n > 1) {
		    i__2 = *n - 1;
		    i__3 = *n - 1;
		    qlaset_("L", &i__2, &i__3, &c_b57, &c_b57, &a[a_dim1 + 2],
			     lda);
		}
		ie = 1;
		itauq = ie + *n;
...
/*                 (Workspace: need 4*N-1, prefer 3*N + (N-1)*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    qorgbr_("P", n, n, n, &a[a_offset], lda, &work[itaup], &
...
/*              (Workspace: need BDSPAC) */
		...
		qbdsqr_("U", n, &ncvt, &c__0, &c__0, &s[1], &work[ie], &a[
...
/*              If right singular vectors desired in VT, copy them there */
		...
		if (wntvas) {
		    qlacpy_("F", n, n, &a[a_offset], lda, &vt[vt_offset], 
			    ldvt);
		}
...
/*                 Copy R to WORK(IR) and zero out below it */
		    ...
		    i__2 = *n - 1;
		    i__3 = *n - 1;
		    qlaset_("L", &i__2, &i__3, &c_b57, &c_b57, &work[ir + 1], 
			    &ldwrkr);
...
/*                 (Workspace: need N*N + 2*N, prefer N*N + N + N*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    qorgqr_(m, n, n, &a[a_offset], lda, &work[itau], &work[
			    iwork], &i__2, &ierr);
		    ie = itau;
		    itauq = ie + *n;
		    itaup = itauq + *n;
...
/* Computing MIN */
			...
			qgemm_("N", "N", &chunk, n, n, &c_b79, &a[i__ + 
				a_dim1], lda, &work[ir], &ldwrkr, &c_b57, &
				work[iu], &ldwrku);
			qlacpy_("F", &chunk, n, &work[iu], &ldwrku, &a[i__ + 
				a_dim1], lda);
/* L10: */
		    }
...
/*                 (Workspace: need 3*N + M, prefer 3*N + (M + N)*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    qgebrd_(m, n, &a[a_offset], lda, &s[1], &work[ie], &work[
...
/*                 Copy R to VT, zeroing out below it */
		    ...
		    qlacpy_("U", n, n, &a[a_offset], lda, &vt[vt_offset], 
			    ldvt);
		    if (*n > 1) {
			i__3 = *n - 1;
			i__2 = *n - 1;
			qlaset_("L", &i__3, &i__2, &c_b57, &c_b57, &vt[
				vt_dim1 + 2], ldvt);
		    }
...
/*                 (Workspace: need N*N + 4*N, prefer N*N + 3*N + 2*N*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    qgebrd_(n, n, &vt[vt_offset], ldvt, &s[1], &work[ie], &
			    work[itauq], &work[itaup], &work[iwork], &i__3, &
			    ierr);
		    qlacpy_("L", n, n, &vt[vt_offset], ldvt, &work[ir], &
			    ldwrkr);
...
/*                 (Workspace: need N*N + 4*N-1, prefer N*N + 3*N + (N-1)*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    qorgbr_("P", n, n, n, &vt[vt_offset], ldvt, &work[itaup], 
			    &work[iwork], &i__3, &ierr);
		    iwork = ie + *n;
...
/*                 (Workspace: need N*N + BDSPAC) */
		    ...
		    qbdsqr_("U", n, n, n, &c__0, &s[1], &work[ie], &vt[
...
/*                 Copy R to VT, zeroing out below it */
			...
			i__2 = *n - 1;
			i__3 = *n - 1;
			qlaset_("L", &i__2, &i__3, &c_b57, &c_b57, &vt[
				vt_dim1 + 2], ldvt);
		    }
...
/*                 (Workspace: need 3*N + M, prefer 3*N + M*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    qormbr_("Q", "R", "N", m, n, n, &vt[vt_offset], ldvt, &
...
/*                 (Workspace: need BDSPAC) */
		    ...
		    qbdsqr_("U", n, n, m, &c__0, &s[1], &work[ie], &vt[
...
/*                    (Workspace: need N*N + 2*N, prefer N*N + N + N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[
				iwork], &i__2, &ierr);

/*                    Copy R to WORK(IR), zeroing out below it */

			qlacpy_("U", n, n, &a[a_offset], lda, &work[ir], &
				ldwrkr);
			i__2 = *n - 1;
			i__3 = *n - 1;
			qlaset_("L", &i__2, &i__3, &c_b57, &c_b57, &work[ir + 
				1], &ldwrkr);
...
/*                    (Workspace: need N*N + 2*N, prefer N*N + N + N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorgqr_(m, n, n, &a[a_offset], lda, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = itau;
			itauq = ie + *n;
			itaup = itauq + *n;
...
/*                    (Workspace: need N*N) */
			...
			qgemm_("N", "N", m, n, n, &c_b79, &a[a_offset], lda, &
...
/*                    (Workspace: need 2*N, prefer N + N*NB) */
			...
			qgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[
				iwork], &i__2, &ierr);
			qlacpy_("L", m, n, &a[a_offset], lda, &u[u_offset], 
				ldu);
...
/*                    (Workspace: need 2*N, prefer N + N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorgqr_(m, n, n, &u[u_offset], ldu, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = itau;
			itauq = ie + *n;
			itaup = itauq + *n;
...
/*                    Zero out below R in A */
			...
			if (*n > 1) {
			    i__2 = *n - 1;
			    i__3 = *n - 1;
			    qlaset_("L", &i__2, &i__3, &c_b57, &c_b57, &a[
				    a_dim1 + 2], lda);
			}
...
/*                    (Workspace: need 4*N, prefer 3*N + 2*N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qgebrd_(n, n, &a[a_offset], lda, &s[1], &work[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
...
/*                    (Workspace: need 3*N + M, prefer 3*N + M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qormbr_("Q", "R", "N", m, n, n, &a[a_offset], lda, &
...
/*                    Copy R to WORK(IU), zeroing out below it */
			...
			qlacpy_("U", n, n, &a[a_offset], lda, &work[iu], &
				ldwrku);
			i__2 = *n - 1;
			i__3 = *n - 1;
			qlaset_("L", &i__2, &i__3, &c_b57, &c_b57, &work[iu + 
				1], &ldwrku);
...
/*                    (Workspace: need N*N) */
			...
			qlacpy_("F", n, n, &work[ir], &ldwrkr, &a[a_offset], 
				lda);
...
/*                    (Workspace: need BDSPAC) */
			...
			qbdsqr_("U", n, n, m, &c__0, &s[1], &work[ie], &a[
...
/*                    (Workspace: need N*N + 4*N, prefer N*N + 3*N + 2*N*NB) */
			...
			qgebrd_(n, n, &work[iu], &ldwrku, &s[1], &work[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
			qlacpy_("U", n, n, &work[iu], &ldwrku, &vt[vt_offset],
				 ldvt);
...
/*                                prefer N*N+3*N+(N-1)*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorgbr_("P", n, n, n, &vt[vt_offset], ldvt, &work[
...
/*                    (Workspace: need N*N + N + M, prefer N*N + N + M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorgqr_(m, m, n, &u[u_offset], ldu, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = itau;
			itauq = ie + *n;
			itaup = itauq + *n;
...
/*                    (Workspace: need N*N) */
			...
			qgemm_("N", "N", m, n, n, &c_b79, &u[u_offset], ldu, &
				work[ir], &ldwrkr, &c_b57, &a[a_offset], lda);

/*                    Copy left singular vectors of A from A to U */

			qlacpy_("F", m, n, &a[a_offset], lda, &u[u_offset], 
				ldu);
...
/*              (Workspace: need 3*N + NCU, prefer 3*N + NCU*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qorgbr_("Q", m, &ncu, n, &u[u_offset], ldu, &work[itauq], &
			work[iwork], &i__2, &ierr);
	    }
	    if (wntvas) {
...
/*              (Workspace: need 4*N-1, prefer 3*N + (N-1)*NB) */
		...
		qlacpy_("U", n, n, &a[a_offset], lda, &vt[vt_offset], ldvt);
		i__2 = *lwork - iwork + 1;
		qorgbr_("P", n, n, n, &vt[vt_offset], ldvt, &work[itaup], &
			work[iwork], &i__2, &ierr);
	    }
...
/*              (Workspace: need 4*N, prefer 3*N + N*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qorgbr_("Q", m, n, n, &a[a_offset], lda, &work[itauq], &work[
			iwork], &i__2, &ierr);
	    }
	    if (wntvo) {
...
/*              (Workspace: need 4*N-1, prefer 3*N + (N-1)*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qorgbr_("P", n, n, n, &a[a_offset], lda, &work[itaup], &work[
			iwork], &i__2, &ierr);
	    }
	    iwork = ie + *n;
	    if (wntuas || wntuo) {
...
/*              (Workspace: need BDSPAC) */
		...
		qbdsqr_("U", n, &ncvt, &nru, &c__0, &s[1], &work[ie], &vt[
...
/*              (Workspace: need BDSPAC) */
		...
		qbdsqr_("U", n, &ncvt, &nru, &c__0, &s[1], &work[ie], &a[
...
/*              (Workspace: need 2*M, prefer M + M*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[iwork], &
			i__2, &ierr);
...
/*              Zero out above L */
		...
		i__2 = *m - 1;
		i__3 = *m - 1;
		qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &a[(a_dim1 << 1) + 
			1], lda);
		ie = 1;
		itauq = ie + *m;
		itaup = itauq + *m;
...
/*                 (Workspace: need 4*M, prefer 3*M + M*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    qorgbr_("Q", m, m, m, &a[a_offset], lda, &work[itauq], &
			    work[iwork], &i__2, &ierr);
		}
		iwork = ie + *m;
		nru = 0;
...
/*              If left singular vectors desired in U, copy them there */
		...
		if (wntuas) {
		    qlacpy_("F", m, m, &a[a_offset], lda, &u[u_offset], ldu);
		}
...
/*                 Copy L to WORK(IR) and zero out above it */
		    ...
		    qlacpy_("L", m, m, &a[a_offset], lda, &work[ir], &ldwrkr);
		    i__2 = *m - 1;
		    i__3 = *m - 1;
		    qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &work[ir + 
			    ldwrkr], &ldwrkr);
...
/*                 (Workspace: need M*M + 2*M, prefer M*M + M + M*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    qorglq_(m, n, m, &a[a_offset], lda, &work[itau], &work[
			    iwork], &i__2, &ierr);
		    ie = itau;
		    itauq = ie + *m;
		    itaup = itauq + *m;
...
/* Computing MIN */
			...
			qgemm_("N", "N", m, &blk, m, &c_b79, &work[ir], &
				ldwrkr, &a[i__ * a_dim1 + 1], lda, &c_b57, &
				work[iu], &ldwrku);
			qlacpy_("F", m, &blk, &work[iu], &ldwrku, &a[i__ * 
				a_dim1 + 1], lda);
/* L30: */
		    }
...
/*                 (Workspace: need BDSPAC) */
		    ...
		    qbdsqr_("L", m, n, &c__0, &c__0, &s[1], &work[ie], &a[
...
/*                 Copy L to U, zeroing about above it */
		    ...
		    qlacpy_("L", m, m, &a[a_offset], lda, &u[u_offset], ldu);
		    i__3 = *m - 1;
		    i__2 = *m - 1;
		    qlaset_("U", &i__3, &i__2, &c_b57, &c_b57, &u[(u_dim1 << 
			    1) + 1], ldu);
...
/*                 (Workspace: need M*M + 4*M, prefer M*M + 3*M + 2*M*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    qgebrd_(m, m, &u[u_offset], ldu, &s[1], &work[ie], &work[
			    itauq], &work[itaup], &work[iwork], &i__3, &ierr);
		    qlacpy_("U", m, m, &u[u_offset], ldu, &work[ir], &ldwrkr);
...
/*                 (Workspace: need M*M + 4*M, prefer M*M + 3*M + M*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    qorgbr_("Q", m, m, m, &u[u_offset], ldu, &work[itauq], &
			    work[iwork], &i__3, &ierr);
		    iwork = ie + *m;
...
/*                 Copy L to U, zeroing out above it */
		    ...
		    i__2 = *m - 1;
		    i__3 = *m - 1;
		    qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &u[(u_dim1 << 
			    1) + 1], ldu);
...
/*                 (Workspace: need 3*M + N, prefer 3*M + N*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    qormbr_("P", "L", "T", m, n, m, &u[u_offset], ldu, &work[
...
/*                 (Workspace: need BDSPAC) */
		    ...
		    qbdsqr_("U", m, n, m, &c__0, &s[1], &work[ie], &a[
			    a_offset], lda, &u[u_offset], ldu, dum, &c__1, &
			    work[iwork], info);
...
/*                    (Workspace: need M*M + 2*M, prefer M*M + M + M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[
				iwork], &i__2, &ierr);

/*                    Copy L to WORK(IR), zeroing out above it */

			qlacpy_("L", m, m, &a[a_offset], lda, &work[ir], &
				ldwrkr);
			i__2 = *m - 1;
			i__3 = *m - 1;
			qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &work[ir + 
...
/*                    (Workspace: need M*M + 2*M, prefer M*M + M + M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorglq_(m, n, m, &a[a_offset], lda, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = itau;
			itauq = ie + *m;
			itaup = itauq + *m;
...
/*                    (Workspace: need M*M) */
			...
			qgemm_("N", "N", m, n, m, &c_b79, &work[ir], &ldwrkr, 
...
/*                    Copy result to VT */
			...
			qlacpy_("U", m, n, &a[a_offset], lda, &vt[vt_offset], 
				ldvt);
...
/*                    (Workspace: need 2*M, prefer M + M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorglq_(m, n, m, &vt[vt_offset], ldvt, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = itau;
			itauq = ie + *m;
			itaup = itauq + *m;
...
/*                    Zero out above L in A */
			...
			i__2 = *m - 1;
			i__3 = *m - 1;
			qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &a[(a_dim1 
				<< 1) + 1], lda);
...
/*                    (Workspace: need 4*M, prefer 3*M + 2*M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qgebrd_(m, m, &a[a_offset], lda, &s[1], &work[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
...
/*                    (Workspace: need 3*M + N, prefer 3*M + N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qormbr_("P", "L", "T", m, n, m, &a[a_offset], lda, &
...
/*                    Copy L to WORK(IU), zeroing out below it */
			...
			qlacpy_("L", m, m, &a[a_offset], lda, &work[iu], &
				ldwrku);
			i__2 = *m - 1;
			i__3 = *m - 1;
			qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &work[iu + 
				ldwrku], &ldwrku);
...
/*                    (Workspace: need M*M) */
			...
			qgemm_("N", "N", m, n, m, &c_b79, &work[iu], &ldwrku, 
				&a[a_offset], lda, &c_b57, &vt[vt_offset], 
				ldvt);
...
/*                    (Workspace: need M*M) */
			...
			qlacpy_("F", m, m, &work[ir], &ldwrkr, &a[a_offset], 
				lda);
...
/*                    (Workspace: need BDSPAC) */
			...
			qbdsqr_("U", m, n, m, &c__0, &s[1], &work[ie], &vt[
...
/*                    (Workspace: need M*M + 4*M, prefer M*M + 3*M + 2*M*NB) */
			...
			qgebrd_(m, m, &work[iu], &ldwrku, &s[1], &work[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
			qlacpy_("L", m, m, &work[iu], &ldwrku, &u[u_offset], 
				ldu);
...
/*                    Copy L to U, zeroing out above it */
			...
			qlacpy_("L", m, m, &a[a_offset], lda, &u[u_offset], 
				ldu);
			i__2 = *m - 1;
			i__3 = *m - 1;
			qlaset_("U", &i__2, &i__3, &c_b57, &c_b57, &u[(u_dim1 
...
/*                    (Workspace: need 4*M, prefer 3*M + 2*M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qgebrd_(m, m, &u[u_offset], ldu, &s[1], &work[ie], &
				work[itauq], &work[itaup], &work[iwork], &
				i__2, &ierr);
...
/*                    (Workspace: need 3*M + N, prefer 3*M + N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qormbr_("P", "L", "T", m, n, m, &u[u_offset], ldu, &
				work[itaup], &vt[vt_offset], ldvt, &work[
				iwork], &i__2, &ierr);
...
/*                    (Workspace: need M*M + M + N, prefer M*M + M + N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			qorglq_(n, n, m, &vt[vt_offset], ldvt, &work[itau], &
				work[iwork], &i__2, &ierr);
			ie = itau;
			itauq = ie + *m;
			itaup = itauq + *m;
...
/*                    Copy right singular vectors of A from A to VT */
			...
			qlacpy_("F", m, n, &a[a_offset], lda, &vt[vt_offset], 
				ldvt);
...
/*              (Workspace: need 4*M-1, prefer 3*M + (M-1)*NB) */
		...
		qlacpy_("L", m, m, &a[a_offset], lda, &u[u_offset], ldu);
		i__2 = *lwork - iwork + 1;
		qorgbr_("Q", m, m, n, &u[u_offset], ldu, &work[itauq], &work[
			iwork], &i__2, &ierr);
	    }
	    if (wntvas) {
...
/*              (Workspace: need 3*M + NRVT, prefer 3*M + NRVT*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qorgbr_("P", &nrvt, n, m, &vt[vt_offset], ldvt, &work[itaup], 
			&work[iwork], &i__2, &ierr);
	    }
	    if (wntuo) {
...
/*              (Workspace: need 4*M-1, prefer 3*M + (M-1)*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qorgbr_("Q", m, m, n, &a[a_offset], lda, &work[itauq], &work[
			iwork], &i__2, &ierr);
	    }
	    if (wntvo) {
...
/*              (Workspace: need 4*M, prefer 3*M + M*NB) */
		...
		i__2 = *lwork - iwork + 1;
		qorgbr_("P", m, n, m, &a[a_offset], lda, &work[itaup], &work[
			iwork], &i__2, &ierr);
	    }
	    iwork = ie + *m;
	    if (wntuas || wntuo) {
...
/*              (Workspace: need BDSPAC) */
		...
		qbdsqr_("L", m, &ncvt, &nru, &c__0, &s[1], &work[ie], &vt[
			vt_offset], ldvt, &u[u_offset], ldu, dum, &c__1, &
			work[iwork], info);
	    } else if (! wntuo && wntvo) {
...
/*              (Workspace: need BDSPAC) */
		...
		qbdsqr_("L", m, &ncvt, &nru, &c__0, &s[1], &work[ie], &a[
			a_offset], lda, &u[u_offset], ldu, dum, &c__1, &work[
			iwork], info);
	    } else {
