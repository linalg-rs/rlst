...
/* > \brief \b DLAGV2 computes the Generalized Schur factorization of a doublereal 2-by-2 matrix pencil (A,B) where 
...
/* > \verbatim */
/* > */
/* > DLAGV2 computes the Generalized Schur factorization of a doublereal 2-by-2 */
/* > matrix pencil (A,B) where B is upper triangular. This routine */
/* > computes orthogonal (rotation) matrices given by CSL, SNL and CSR, */
/* > SNR such that */
/* > */
/* > 1) if the pencil (A,B) has two doublereal eigenvalues (include 0/0 or 1/0 */
/* >    types), then */
/* > */
...
/* >          pencil (A,B), k=1,2, i = M(sqrt)(-1).  Note that BETA(k) may */
/* >          be zero. */
/* > \endverbatim */
/* > */
/* > \param[out] CSL */
...
/*  ===================================================================== */
void  qlagv2_(quadreal *a, integer *lda, quadreal *b, 
	integer *ldb, quadreal *alphar, quadreal *alphai, quadreal *
	beta, quadreal *csl, quadreal *snl, quadreal *csr, quadreal *
	snr)
{
    ...
    /* Local variables */
    quadreal r__, t, h1, h2, h3, wi, qq, rr, wr1, wr2, ulp;
    extern void  qrot_(integer *, quadreal *, integer *, 
	    ...
	    quadreal *);
    quadreal anorm, bnorm, scale1, scale2;
...
/*     Check if B is singular */
	...
	qlartg_(&a[a_dim1 + 1], &a[a_dim1 + 2], csl, snl, &r__);
	*csr = 1.;
	*snr = 0.;
	qrot_(&c__2, &a[a_dim1 + 1], lda, &a[a_dim1 + 2], lda, csl, snl);
	qrot_(&c__2, &b[b_dim1 + 1], ldb, &b[b_dim1 + 2], ldb, csl, snl);
	a[a_dim1 + 2] = 0.;
	b[b_dim1 + 1] = 0.;
	b[b_dim1 + 2] = 0.;
	wi = 0.;
	...
	qlartg_(&a[(a_dim1 << 1) + 2], &a[a_dim1 + 2], csr, snr, &t);
	*snr = -(*snr);
	qrot_(&c__2, &a[a_dim1 + 1], &c__1, &a[(a_dim1 << 1) + 1], &c__1, csr,
		 snr);
	qrot_(&c__2, &b[b_dim1 + 1], &c__1, &b[(b_dim1 << 1) + 1], &c__1, csr,
		 snr);
	*csl = 1.;
	*snl = 0.;
	a[a_dim1 + 2] = 0.;
...
/*        B is nonsingular, first compute the eigenvalues of (A,B) */
	...
	qlag2_(&a[a_offset], lda, &b[b_offset], ldb, &safmin, &scale1, &
		scale2, &wr1, &wr2, &wi);

	if (wi == 0.) {

/*           two doublereal eigenvalues, compute s*A-w*B */
...
/*              (sA - wB) */
		...
		d__1 = scale1 * a[a_dim1 + 2];
		qlartg_(&h3, &d__1, csr, snr, &t);

	    }

	    *snr = -(*snr);
	    qrot_(&c__2, &a[a_dim1 + 1], &c__1, &a[(a_dim1 << 1) + 1], &c__1, 
		    csr, snr);
	    qrot_(&c__2, &b[b_dim1 + 1], &c__1, &b[(b_dim1 << 1) + 1], &c__1, 
		    csr, snr);
...
/*              find left rotation matrix Q to zero out B(2,1) */
		...
		qlartg_(&b[b_dim1 + 1], &b[b_dim1 + 2], csl, snl, &r__);
...
/*           first compute the SVD of the matrix B */
	    ...
	    qlasv2_(&b[b_dim1 + 1], &b[(b_dim1 << 1) + 1], &b[(b_dim1 << 1) + 
