...
/* > */
/*  ===================================================================== */
void  hgelqt3_(integer *m, integer *n, halfreal *a, integer *
...
/*        Compute Householder transform when N=1 */
	...
	hlarfg_(n, &a[a_offset], &a[f2cmin(2,*n) * a_dim1 + 1], lda, &t[t_offset]
		);
...
/*        Compute A(1:M1,1:N) <- (Y1,R1,T1), where Q1 = I - Y1 T1 Y1^H */
	...
	hgelqt3_(&m1, n, &a[a_offset], lda, &t[t_offset], ldt, &iinfo);
...
/*        Compute A(J1:M,1:N) = Q1^H A(J1:M,1:N) [workspace: T(1:N1,J1:N)] */
	...
	htrmm_("R", "U", "T", "U", &m2, &m1, &c_b7, &a[a_offset], lda, &t[i1 
		+ t_dim1], ldt);

	i__1 = *n - m1;
	hgemm_("N", "T", &m2, &m1, &i__1, &c_b7, &a[i1 + i1 * a_dim1], lda, &
		a[i1 * a_dim1 + 1], lda, &c_b7, &t[i1 + t_dim1], ldt);

	htrmm_("R", "U", "N", "N", &m2, &m1, &c_b7, &t[t_offset], ldt, &t[i1 
		+ t_dim1], ldt);

	i__1 = *n - m1;
	hgemm_("N", "N", &m2, &i__1, &m1, &c_b19, &t[i1 + t_dim1], ldt, &a[i1 
		* a_dim1 + 1], lda, &c_b7, &a[i1 + i1 * a_dim1], lda);

	htrmm_("R", "U", "N", "U", &m2, &m1, &c_b7, &a[a_offset], lda, &t[i1 
		+ t_dim1], ldt);
...
/*        Compute A(J1:M,J1:N) <- (Y2,R2,T2) where Q2 = I - Y2 T2 Y2^H */
	...
	i__1 = *n - m1;
	hgelqt3_(&m2, &i__1, &a[i1 + i1 * a_dim1], lda, &t[i1 + i1 * t_dim1], 
		ldt, &iinfo);
...
/*        Compute T3 = T(J1:N1,1:N) = -T1 Y1^H Y2 T2 */
	...
	htrmm_("R", "U", "T", "U", &m1, &m2, &c_b7, &a[i1 + i1 * a_dim1], lda,
		 &t[i1 * t_dim1 + 1], ldt);

	i__1 = *n - *m;
	hgemm_("N", "T", &m1, &m2, &i__1, &c_b7, &a[j1 * a_dim1 + 1], lda, &a[
		i1 + j1 * a_dim1], lda, &c_b7, &t[i1 * t_dim1 + 1], ldt);

	htrmm_("L", "U", "N", "N", &m1, &m2, &c_b19, &t[t_offset], ldt, &t[i1 
		* t_dim1 + 1], ldt);

	htrmm_("R", "U", "N", "N", &m1, &m2, &c_b7, &t[i1 + i1 * t_dim1], ldt,
		 &t[i1 * t_dim1 + 1], ldt);
