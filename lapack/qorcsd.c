...
/*  ===================================================================== */
void  qorcsd_(char *jobu1, char *jobu2, char *jobv1t, char *
	jobv2t, char *trans, char *signs, integer *m, integer *p, integer *q, 
	quadreal *x11, integer *ldx11, quadreal *x12, integer *ldx12, 
	quadreal *x21, integer *ldx21, quadreal *x22, integer *ldx22, 
	quadreal *theta, quadreal *u1, integer *ldu1, quadreal *u2, 
	integer *ldu2, quadreal *v1t, integer *ldv1t, quadreal *v2t, 
	integer *ldv2t, quadreal *work, integer *lwork, integer *iwork, 
	integer *info)
{
    ...
    extern void  qorbdb_(char *, char *, integer *, integer *, 
    ...
    extern void  qorglq_(integer *, integer *, integer *, 
...
/* Computing MIN */
	...
	qorcsd_(jobv1t, jobv2t, jobu1, jobu2, transt, signst, m, q, p, &x11[
...
/*     convenient */
	...
	i__1 = *m - *p;
	i__2 = *m - *q;
	qorcsd_(jobu2, jobu1, jobv2t, jobv1t, trans, signst, m, &i__1, &i__2, 
...
/* Computing MAX */
	...
	qorgqr_(&i__1, &i__2, &i__3, &u1[u1_offset], &i__4, &u1[u1_offset], &
...
/* Computing MAX */
	i__5 = 1, i__6 = *m - *q;
	i__4 = f2cmax(i__5,i__6);
	qorglq_(&i__1, &i__2, &i__3, &u1[u1_offset], &i__4, &u1[u1_offset], &
		work[1], &c_n1, &childinfo);
...
/* Computing MAX */
	...
	qorbdb_(trans, signs, m, p, q, &x11[x11_offset], ldx11, &x12[
...
/* Computing MAX */
	...
	qbbcsd_(jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, &theta[1], &
...
/* Computing MAX */
	...
	work[1] = (quadreal) f2cmax(lworkopt,lworkmin);
...
/*     Transform to bidiagonal block form */
    ...
    qorbdb_(trans, signs, m, p, q, &x11[x11_offset], ldx11, &x12[x12_offset], 
...
/*     Accumulate Householder reflectors */
	...
	if (wantv1t && *q > 0) {
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    qlacpy_("U", &i__1, &i__2, &x11[(x11_dim1 << 1) + 1], ldx11, &v1t[
		    (v1t_dim1 << 1) + 2], ldv1t);
	    v1t[v1t_dim1 + 1] = 1.;
	    i__1 = *q;
	    for (j = 2; j <= i__1; ++j) {
	...
	if (wantv2t && *m - *q > 0) {
	    i__1 = *m - *q;
	    qlacpy_("U", p, &i__1, &x12[x12_offset], ldx12, &v2t[v2t_offset], 
		    ldv2t);
	    if (*m - *p > *q) {
		i__1 = *m - *p - *q;
		i__2 = *m - *p - *q;
		qlacpy_("U", &i__1, &i__2, &x22[*q + 1 + (*p + 1) * x22_dim1],
		...
		i__2 = *m - *q;
		i__3 = *m - *q;
		qorglq_(&i__1, &i__2, &i__3, &v2t[v2t_offset], ldv2t, &work[
			itauq2], &work[iorglq], &lorglqwork, info);
	    }
	}
    } else {
	if (wantu1 && *p > 0) {
	    qlacpy_("U", q, p, &x11[x11_offset], ldx11, &u1[u1_offset], ldu1);
	    qorglq_(p, p, q, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorglq], &lorglqwork, info);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    qlacpy_("U", q, &i__1, &x21[x21_offset], ldx21, &u2[u2_offset], 
		    ldu2);
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    qorglq_(&i__1, &i__2, q, &u2[u2_offset], ldu2, &work[itaup2], &
		    work[iorglq], &lorglqwork, info);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    qlacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &v1t[(
		    v1t_dim1 << 1) + 2], ldv1t);
	    v1t[v1t_dim1 + 1] = 1.;
	    i__1 = *q;
	    for (j = 2; j <= i__1; ++j) {
	    ...
	    i__2 = *q - 1;
	    i__3 = *q - 1;
	    qorgqr_(&i__1, &i__2, &i__3, &v1t[(v1t_dim1 << 1) + 2], ldv1t, &
		    work[itauq1], &work[iorgqr], &lorgqrwork, info);
	}
	if (wantv2t && *m - *q > 0) {
	    i__1 = *m - *q;
	    qlacpy_("L", &i__1, p, &x12[x12_offset], ldx12, &v2t[v2t_offset], 
		    ldv2t);
	    i__1 = *m - *p - *q;
	    i__2 = *m - *p - *q;
	    qlacpy_("L", &i__1, &i__2, &x22[*p + 1 + (*q + 1) * x22_dim1], 
		    ldx22, &v2t[*p + 1 + (*p + 1) * v2t_dim1], ldv2t);
	    i__1 = *m - *q;
	    i__2 = *m - *q;
	    i__3 = *m - *q;
	    qorgqr_(&i__1, &i__2, &i__3, &v2t[v2t_offset], ldv2t, &work[
...
/*     Compute the CSD of the matrix in bidiagonal-block form */
    ...
    qbbcsd_(jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, &theta[1], &work[
...
/*     corner of (2,2)-block */
	...
	if (colmajor) {
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    qlapmr_(&c_false, &i__1, &i__2, &u2[u2_offset], ldu2, &iwork[1]);
	}
    }
    if (*m > 0 && wantv2t) {
	i__1 = *p;
	...
	if (! colmajor) {
	    i__1 = *m - *q;
	    i__2 = *m - *q;
	    qlapmt_(&c_false, &i__1, &i__2, &v2t[v2t_offset], ldv2t, &iwork[1]
		    );
	} else {
	    i__1 = *m - *q;
	    i__2 = *m - *q;
	    qlapmr_(&c_false, &i__1, &i__2, &v2t[v2t_offset], ldv2t, &iwork[1]
		    );
	}
    }
