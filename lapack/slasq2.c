...
void  slasq2_(integer *n, real *z__, integer *info)
{
    ...
    real tempe, tempq;
    integer ttype;
    extern void  slasq3_(integer *, integer *, real *, integer 
    ...
    real oldemn, safmin;
    extern void  xerbla_(char *, integer *), slasrt_(
...
/*        2-by-2 case. */
	...
	if (z__[2] > z__[3] * tol2) {
	    ...
	    s = z__[3] * (z__[2] / t);
	    if (s <= t) {
		s = z__[3] * (z__[2] / (t * (M(sqrt)(s / t + 1.f) + 1.f)));
	    } else {
...
/* Computing MAX */
	r__1 = 0.f, r__2 = qmin - M(sqrt)(qmin) * 2.f * M(sqrt)(emax);
