...
/*  ===================================================================== */
void  kgelss_(integer *m, integer *n, integer *nrhs, 
	halfcomplex *a, integer *lda, halfcomplex *b, integer *ldb, 
	halfreal *s, halfreal *rcond, integer *rank, halfcomplex *work, 
	integer *lwork, halfreal *rwork, integer *info)
{
    ...
    halfcomplex dum[1];
    halfreal eps, thr, anrm, bnrm;
    integer itau, lwork_zgebrd__, lwork_zgelqf__, lwork_zgeqrf__, 
    ...
    extern void  hlascl_(char *, integer *, integer *, 
	    ...
	    *, halfreal *, halfreal *, halfreal *, integer *), 
	    xerbla_(char *, integer *), kgebrd_(integer *, integer *, 
    ...
    extern void  kgelqf_(integer *, integer *, halfcomplex *,
	    ...
	    *, integer *, integer *, halfcomplex *, integer *, integer *), kgeqrf_(integer *, integer *, halfcomplex *, integer *,
	     halfcomplex *, halfcomplex *, integer *, integer *), whrscl_(
    ...
    extern void  klacpy_(char *, integer *, integer *, 
	    ...
	    halfcomplex *, halfcomplex *, integer *), kbdsqr_(
	    char *, integer *, integer *, integer *, integer *, halfreal *, 
	    halfreal *, halfcomplex *, integer *, halfcomplex *, 
	    integer *, halfcomplex *, integer *, halfreal *, integer *);
...
/*       to doublereal workspace. NB refers to the optimal block size for the */
...
/*              Compute space needed for ZGEQRF */
		kgeqrf_(m, n, &a[a_offset], lda, dum, dum, &c_n1, info);
		lwork_zgeqrf__ = (integer) dum[0].r;
/*              Compute space needed for ZUNMQR */
		kunmqr_("L", "C", m, nrhs, n, &a[a_offset], lda, dum, &b[
...
/*              Compute space needed for ZGEBRD */
		kgebrd_(&mm, n, &a[a_offset], lda, &s[1], &s[1], dum, dum, 
			dum, &c_n1, info);
		lwork_zgebrd__ = (integer) dum[0].r;
/*              Compute space needed for ZUNMBR */
		kunmbr_("Q", "L", "C", &mm, nrhs, n, &a[a_offset], lda, dum, &
			b[b_offset], ldb, dum, &c_n1, info);
		lwork_zunmbr__ = (integer) dum[0].r;
/*              Compute space needed for ZUNGBR */
		kungbr_("P", n, n, n, &a[a_offset], lda, dum, dum, &c_n1, 
			info);
...
/*                 Compute space needed for ZGELQF */
		    kgelqf_(m, n, &a[a_offset], lda, dum, dum, &c_n1, info);
		    lwork_zgelqf__ = (integer) dum[0].r;
/*                 Compute space needed for ZGEBRD */
		    kgebrd_(m, m, &a[a_offset], lda, &s[1], &s[1], dum, dum, 
			    dum, &c_n1, info);
		    lwork_zgebrd__ = (integer) dum[0].r;
/*                 Compute space needed for ZUNMBR */
		    kunmbr_("Q", "L", "C", m, nrhs, n, &a[a_offset], lda, dum,
			     &b[b_offset], ldb, dum, &c_n1, info);
		    lwork_zunmbr__ = (integer) dum[0].r;
/*                 Compute space needed for ZUNGBR */
		    kungbr_("P", m, m, m, &a[a_offset], lda, dum, dum, &c_n1, 
			    info);
		    lwork_zungbr__ = (integer) dum[0].r;
/*                 Compute space needed for ZUNMLQ */
		    kunmlq_("L", "C", n, nrhs, m, &a[a_offset], lda, dum, &b[
			    b_offset], ldb, dum, &c_n1, info);
...
/*                 Compute space needed for ZGEBRD */
		    kgebrd_(m, n, &a[a_offset], lda, &s[1], &s[1], dum, dum, 
			    dum, &c_n1, info);
		    lwork_zgebrd__ = (integer) dum[0].r;
/*                 Compute space needed for ZUNMBR */
		    kunmbr_("Q", "L", "C", m, nrhs, m, &a[a_offset], lda, dum,
			     &b[b_offset], ldb, dum, &c_n1, info);
		    lwork_zunmbr__ = (integer) dum[0].r;
/*                 Compute space needed for ZUNGBR */
		    kungbr_("P", m, n, m, &a[a_offset], lda, dum, dum, &c_n1, 
			    info);
		    lwork_zungbr__ = (integer) dum[0].r;
...
/*     Scale A if f2cmax element outside range [SMLNUM,BIGNUM] */
    ...
    anrm = klange_("M", m, n, &a[a_offset], lda, &rwork[1]);
    iascl = 0;
...
/*        Matrix all zero. Return zero solution. */
	...
	i__1 = f2cmax(*m,*n);
	klaset_("F", &i__1, nrhs, &c_b1, &c_b1, &b[b_offset], ldb);
	hlaset_("F", &minmn, &c__1, &c_b59, &c_b59, &s[1], &minmn);
	*rank = 0;
	goto L70;
    }
...
/*     Scale B if f2cmax element outside range [SMLNUM,BIGNUM] */
    ...
    bnrm = klange_("M", m, nrhs, &b[b_offset], ldb, &rwork[1]);
    ibscl = 0;
    if (bnrm > 0. && bnrm < smlnum) {

/*        Scale matrix norm up to SMLNUM */

	klascl_("G", &c__0, &c__0, &bnrm, &smlnum, m, nrhs, &b[b_offset], ldb,
		 info);
	ibscl = 1;
    } else if (bnrm > bignum) {
...
/*        Scale matrix norm down to BIGNUM */
	...
	klascl_("G", &c__0, &c__0, &bnrm, &bignum, m, nrhs, &b[b_offset], ldb,
		 info);
	ibscl = 2;
    }
...
/*           (RWorkspace: none) */
	    ...
	    i__1 = *lwork - iwork + 1;
	    kgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[iwork], &i__1,
		     info);
...
/*           (RWorkspace: none) */
	    ...
	    i__1 = *lwork - iwork + 1;
	    kunmqr_("L", "C", m, nrhs, n, &a[a_offset], lda, &work[itau], &b[
...
/*        (RWorkspace: need N) */
	...
	i__1 = *lwork - iwork + 1;
	kgebrd_(&mm, n, &a[a_offset], lda, &s[1], &rwork[ie], &work[itauq], &
...
/*        (RWorkspace: none) */
	...
	i__1 = *lwork - iwork + 1;
	kunmbr_("Q", "L", "C", &mm, nrhs, n, &a[a_offset], lda, &work[itauq], 
...
/*        (RWorkspace: none) */
	...
	i__1 = *lwork - iwork + 1;
	kungbr_("P", n, n, n, &a[a_offset], lda, &work[itaup], &work[iwork], &
		i__1, info);
	irwork = ie + *n;
...
/*        (RWorkspace: need BDSPAC) */
	...
	kbdsqr_("U", n, n, &c__0, nrhs, &s[1], &rwork[ie], &a[a_offset], lda, 
...
/* Computing MAX */
	...
	*rank = 0;
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    if (s[i__] > thr) {
		whrscl_(nrhs, &s[i__], &b[i__ + b_dim1], ldb);
		++(*rank);
	    } else {
		klaset_("F", &c__1, nrhs, &c_b1, &c_b1, &b[i__ + b_dim1], ldb);
	    }
/* L10: */
	}
...
/*        (RWorkspace: none) */
	...
	if (*lwork >= *ldb * *nrhs && *nrhs > 1) {
	    kgemm_("C", "N", n, nrhs, n, &c_b2, &a[a_offset], lda, &b[
		    b_offset], ldb, &c_b1, &work[1], ldb);
	    klacpy_("G", n, nrhs, &work[1], ldb, &b[b_offset], ldb)
		    ;
	} else if (*nrhs > 1) {
	    chunk = *lwork / *n;
	    i__1 = *nrhs;
...
/* Computing MIN */
		...
		kgemm_("C", "N", n, &bl, n, &c_b2, &a[a_offset], lda, &b[i__ *
			 b_dim1 + 1], ldb, &c_b1, &work[1], n);
		klacpy_("G", n, &bl, &work[1], n, &b[i__ * b_dim1 + 1], ldb);
/* L20: */
	    }
	} else {
	    kgemv_("C", n, n, &c_b2, &a[a_offset], lda, &b[b_offset], &c__1, &
		    c_b1, &work[1], &c__1);
	    kcopy_(n, &work[1], &c__1, &b[b_offset], &c__1);
	}
...
/*        (RWorkspace: none) */
	    ...
	    i__2 = *lwork - iwork + 1;
	    kgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[iwork], &i__2,
		     info);
	    il = iwork;

/*        Copy L to WORK(IL), zeroing out above it */

	    klacpy_("L", m, m, &a[a_offset], lda, &work[il], &ldwork);
	    i__2 = *m - 1;
	    i__1 = *m - 1;
	    klaset_("U", &i__2, &i__1, &c_b1, &c_b1, &work[il + ldwork], &
		    ldwork);
	    ie = 1;
	    itauq = il + ldwork * *m;
	    itaup = itauq + *m;
...
/*        (RWorkspace: need M) */
	    ...
	    i__2 = *lwork - iwork + 1;
	    kgebrd_(m, m, &work[il], &ldwork, &s[1], &rwork[ie], &work[itauq],
...
/*        (RWorkspace: none) */
	    ...
	    i__2 = *lwork - iwork + 1;
	    kunmbr_("Q", "L", "C", m, nrhs, m, &work[il], &ldwork, &work[
...
/*        (RWorkspace: none) */
	    ...
	    i__2 = *lwork - iwork + 1;
	    kungbr_("P", m, m, m, &work[il], &ldwork, &work[itaup], &work[
		    iwork], &i__2, info);
	    irwork = ie + *m;
...
/*        (RWorkspace: need BDSPAC) */
	    ...
	    kbdsqr_("U", m, m, &c__0, nrhs, &s[1], &rwork[ie], &work[il], &
...
/* Computing MAX */
	    ...
	    *rank = 0;
		...
		if (s[i__] > thr) {
		    ...
		    klaset_("F", &c__1, nrhs, &c_b1, &c_b1, &b[i__ + b_dim1], 
			    ldb);
		}
/* L30: */
	    }
...
/*        (RWorkspace: none) */
		...
		kgemm_("C", "N", m, nrhs, m, &c_b2, &work[il], &ldwork, &b[
			b_offset], ldb, &c_b1, &work[iwork], ldb);
		klacpy_("G", m, nrhs, &work[iwork], ldb, &b[b_offset], ldb);
	    } else if (*nrhs > 1) {
...
/* Computing MIN */
		    i__3 = *nrhs - i__ + 1;
		    bl = f2cmin(i__3,chunk);
		    kgemm_("C", "N", m, &bl, m, &c_b2, &work[il], &ldwork, &b[
			    i__ * b_dim1 + 1], ldb, &c_b1, &work[iwork], m);
		    klacpy_("G", m, &bl, &work[iwork], m, &b[i__ * b_dim1 + 1]
			    , ldb);
/* L40: */
		}
	    } else {
		kgemv_("C", m, m, &c_b2, &work[il], &ldwork, &b[b_dim1 + 1], &
			c__1, &c_b1, &work[iwork], &c__1);
		kcopy_(m, &work[iwork], &c__1, &b[b_dim1 + 1], &c__1);
	    }

/*        Zero out below first M rows of B */

	    i__1 = *n - *m;
	    klaset_("F", &i__1, nrhs, &c_b1, &c_b1, &b[*m + 1 + b_dim1], ldb);
	    iwork = itau + *m;
...
/*        (RWorkspace: none) */
	    ...
	    i__1 = *lwork - iwork + 1;
	    kunmlq_("L", "C", n, nrhs, m, &a[a_offset], lda, &work[itau], &b[
		    b_offset], ldb, &work[iwork], &i__1, info);
...
/*        (RWorkspace: none) */
	    ...
	    i__1 = *lwork - iwork + 1;
	    kunmbr_("Q", "L", "C", m, nrhs, n, &a[a_offset], lda, &work[itauq]
		    , &b[b_offset], ldb, &work[iwork], &i__1, info);
...
/*        (RWorkspace: need BDSPAC) */
	    ...
	    kbdsqr_("L", m, n, &c__0, nrhs, &s[1], &rwork[ie], &a[a_offset], 
...
/*        (RWorkspace: none) */
	    ...
	    if (*lwork >= *ldb * *nrhs && *nrhs > 1) {
		kgemm_("C", "N", n, nrhs, m, &c_b2, &a[a_offset], lda, &b[
			b_offset], ldb, &c_b1, &work[1], ldb);
		klacpy_("G", n, nrhs, &work[1], ldb, &b[b_offset], ldb);
	    } else if (*nrhs > 1) {
		chunk = *lwork / *n;
...
/* Computing MIN */
		    i__3 = *nrhs - i__ + 1;
		    bl = f2cmin(i__3,chunk);
		    kgemm_("C", "N", n, &bl, m, &c_b2, &a[a_offset], lda, &b[
			    i__ * b_dim1 + 1], ldb, &c_b1, &work[1], n);
		    klacpy_("F", n, &bl, &work[1], n, &b[i__ * b_dim1 + 1], 
			    ldb);
/* L60: */
		}
	    } else {
		kgemv_("C", m, n, &c_b2, &a[a_offset], lda, &b[b_offset], &
...
/*     Undo scaling */
    ...
    if (iascl == 1) {
	klascl_("G", &c__0, &c__0, &anrm, &smlnum, n, nrhs, &b[b_offset], ldb,
		 info);
	hlascl_("G", &c__0, &c__0, &smlnum, &anrm, &minmn, &c__1, &s[1], &
		minmn, info);
    } else if (iascl == 2) {
	klascl_("G", &c__0, &c__0, &anrm, &bignum, n, nrhs, &b[b_offset], ldb,
		 info);
	hlascl_("G", &c__0, &c__0, &bignum, &anrm, &minmn, &c__1, &s[1], &
		minmn, info);
    }
    if (ibscl == 1) {
	klascl_("G", &c__0, &c__0, &smlnum, &bnrm, n, nrhs, &b[b_offset], ldb,
		 info);
    } else if (ibscl == 2) {
	klascl_("G", &c__0, &c__0, &bignum, &bnrm, n, nrhs, &b[b_offset], ldb,
		 info);
    }
L70:
    work[1].r = (halfreal) maxwrk, work[1].i = 0.;
