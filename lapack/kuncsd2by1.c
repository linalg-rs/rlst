...
/*  ===================================================================== */
void  kuncsd2by1_(char *jobu1, char *jobu2, char *jobv1t, 
	integer *m, integer *p, integer *q, halfcomplex *x11, integer *
	ldx11, halfcomplex *x21, integer *ldx21, halfreal *theta, 
	halfcomplex *u1, integer *ldu1, halfcomplex *u2, integer *ldu2, 
	halfcomplex *v1t, integer *ldv1t, halfcomplex *work, integer *
	lwork, halfreal *rwork, integer *lrwork, integer *iwork, integer *
	info)
{
    ...
    extern void  kunglq_(integer *, integer *, integer *, 
	    ...
	    , integer *, integer *), kunbdb3_(integer *, integer *, integer *,
	     halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfreal *, halfreal *, halfcomplex *, halfcomplex *, 
	    halfcomplex *, halfcomplex *, integer *, integer *), kunbdb4_(
	    integer *, integer *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *, halfreal *, halfreal *, 
	    halfcomplex *, halfcomplex *, halfcomplex *, halfcomplex *
	    , halfcomplex *, integer *, integer *);
...
/* Computing MAX */
	...
	lorglqopt = 1;
	if (r__ == *q) {
	    kunbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, 
		    &childinfo);
	    lorbdb = (integer) work[1].r;
	    if (wantu1 && *p > 0) {
		kungqr_(p, p, q, &u1[u1_offset], ldu1, cdum, &work[1], &c_n1, 
			&childinfo);
...
/* Computing MAX */
		...
		i__1 = *m - *p;
		i__2 = *m - *p;
		kungqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, cdum, &work[1],
			 &c_n1, &childinfo);
/* Computing MAX */
...
/* Computing MAX */
		...
		i__2 = *q - 1;
		i__3 = *q - 1;
		kunglq_(&i__1, &i__2, &i__3, &v1t[v1t_offset], ldv1t, cdum, &
...
/* Computing MAX */
	    ...
	    kbbcsd_(jobu1, jobu2, jobv1t, "N", "N", m, p, q, &theta[1], dum, &
	    ...
	    kunbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, 
		    &childinfo);
	    lorbdb = (integer) work[1].r;
	    if (wantu1 && *p > 0) {
		...
		i__2 = *p - 1;
		i__3 = *p - 1;
		kungqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, 
...
/* Computing MAX */
	    ...
	    if (wantv1t && *q > 0) {
		kunglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, cdum, &work[1], &
			c_n1, &childinfo);
...
/* Computing MAX */
		...
		i__2 = *m - *p - 1;
		i__3 = *m - *p - 1;
		kungqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, 
			cdum, &work[1], &c_n1, &childinfo);
/* Computing MAX */
...
/* Computing MAX */
	    ...
	    i__1 = *m - *q;
	    i__2 = *m - *p;
	    kbbcsd_("N", jobv1t, jobu2, jobu1, "T", m, &i__1, &i__2, &theta[1]
	    ...
	    kunbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum, cdum, cdum, cdum, cdum, &work[1], &
		    c_n1, &childinfo);
	    lorbdb = *m + (integer) work[1].r;
	    if (wantu1 && *p > 0) {
		i__1 = *m - *q;
		kungqr_(p, p, &i__1, &u1[u1_offset], ldu1, cdum, &work[1], &
			c_n1, &childinfo);
		lorgqrmin = f2cmax(lorgqrmin,*p);
/* Computing MAX */
		i__1 = lorgqropt, i__2 = (integer) work[1].r;
		...
		i__2 = *m - *p;
		i__3 = *m - *q;
		kungqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, cdum, &
			work[1], &c_n1, &childinfo);
/* Computing MAX */
		i__1 = lorgqrmin, i__2 = *m - *p;
		lorgqrmin = f2cmax(i__1,i__2);
/* Computing MAX */
	    ...
	    if (wantv1t && *q > 0) {
		kunglq_(q, q, q, &v1t[v1t_offset], ldv1t, cdum, &work[1], &
			c_n1, &childinfo);
		lorglqmin = f2cmax(lorglqmin,*q);
/* Computing MAX */
		i__1 = lorglqopt, i__2 = (integer) work[1].r;
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *q;
	    kbbcsd_(jobu2, jobu1, "N", jobv1t, "N", m, &i__1, &i__2, &theta[1]
...
/* Computing MAX */
	...
	work[1].r = (halfreal) lworkopt, work[1].i = 0.;
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	kunbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    klacpy_("L", p, q, &x11[x11_offset], ldx11, &u1[u1_offset], ldu1);
	    kungqr_(p, p, q, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorgqr], &lorgqr, &childinfo);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    klacpy_("L", &i__1, q, &x21[x21_offset], ldx21, &u2[u2_offset], 
		    ldu2);
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    kungqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, &work[itaup2], &
	...
	if (wantv1t && *q > 0) {
	    ...
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    klacpy_("U", &i__1, &i__2, &x21[(x21_dim1 << 1) + 1], ldx21, &v1t[
		    (v1t_dim1 << 1) + 2], ldv1t);
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    i__3 = *q - 1;
	    kunglq_(&i__1, &i__2, &i__3, &v1t[(v1t_dim1 << 1) + 2], ldv1t, &
		    work[itauq1], &work[iorglq], &lorglq, &childinfo);
	}

/*        Simultaneously diagonalize X11 and X21. */

	kbbcsd_(jobu1, jobu2, jobv1t, "N", "N", m, p, q, &theta[1], &rwork[
...
/*        preferred positions */
	...
	if (*q > 0 && wantu2) {
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    klapmt_(&c_false, &i__1, &i__2, &u2[u2_offset], ldu2, &iwork[1]);
	}
    } else if (r__ == *p) {
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	kunbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &rwork[iphi], &work[itaup1], &work[itaup2], &work[
		itauq1], &work[iorbdb], &lorbdb, &childinfo);
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    ...
	    i__1 = *p - 1;
	    i__2 = *p - 1;
	    klacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &u1[(
		    u1_dim1 << 1) + 2], ldu1);
	    i__1 = *p - 1;
	    i__2 = *p - 1;
	    i__3 = *p - 1;
	    kungqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, &work[
	...
	if (wantv1t && *q > 0) {
	    klacpy_("U", p, q, &x11[x11_offset], ldx11, &v1t[v1t_offset], 
		    ldv1t);
	    kunglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}

/*        Simultaneously diagonalize X11 and X21. */

	kbbcsd_(jobv1t, "N", jobu1, jobu2, "T", m, q, p, &theta[1], &rwork[
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	kunbdb3_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &rwork[iphi], &work[itaup1], &work[itaup2], &work[
		itauq1], &work[iorbdb], &lorbdb, &childinfo);
...
/*        Accumulate Householder reflectors */
	...
	if (wantu2 && *m - *p > 0) {
	    ...
	    i__1 = *m - *p - 1;
	    i__2 = *m - *p - 1;
	    klacpy_("L", &i__1, &i__2, &x21[x21_dim1 + 2], ldx21, &u2[(
		    u2_dim1 << 1) + 2], ldu2);
	    i__1 = *m - *p - 1;
	    i__2 = *m - *p - 1;
	    i__3 = *m - *p - 1;
	    kungqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, &work[
		    itaup2], &work[iorgqr], &lorgqr, &childinfo);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *m - *p;
	    klacpy_("U", &i__1, q, &x21[x21_offset], ldx21, &v1t[v1t_offset], 
		    ldv1t);
	    kunglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}
...
/*        preferred positions */
	...
	if (*q > r__) {
	    ...
	    if (wantu1) {
		klapmt_(&c_false, p, q, &u1[u1_offset], ldu1, &iwork[1]);
	    }
	    if (wantv1t) {
		klapmr_(&c_false, q, q, &v1t[v1t_offset], ldv1t, &iwork[1]);
	    }
	}
    } else {
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	i__1 = lorbdb - *m;
	kunbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &rwork[iphi], &work[itaup1], &work[itaup2], &work[
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    kcopy_(p, &work[iorbdb], &c__1, &u1[u1_offset], &c__1);
	    i__1 = *p;
	    for (j = 2; j <= i__1; ++j) {
		i__2 = j * u1_dim1 + 1;
		u1[i__2].r = 0., u1[i__2].i = 0.;
	    ...
	    klacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &u1[(
		    u1_dim1 << 1) + 2], ldu1);
	    i__1 = *m - *q;
	    kungqr_(p, p, &i__1, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorgqr], &lorgqr, &childinfo);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    kcopy_(&i__1, &work[iorbdb + *p], &c__1, &u2[u2_offset], &c__1);
	    i__1 = *m - *p;
	    for (j = 2; j <= i__1; ++j) {
		i__2 = j * u2_dim1 + 1;
		u2[i__2].r = 0., u2[i__2].i = 0.;
	    ...
	    i__2 = *m - *p;
	    i__3 = *m - *q;
	    kungqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, &work[itaup2], 
		    &work[iorgqr], &lorgqr, &childinfo);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *m - *q;
	    ...
	    i__1 = *p - (*m - *q);
	    i__2 = *q - (*m - *q);
	    klacpy_("U", &i__1, &i__2, &x11[*m - *q + 1 + (*m - *q + 1) * 
		    x11_dim1], ldx11, &v1t[*m - *q + 1 + (*m - *q + 1) * 
		    v1t_dim1], ldv1t);
	    i__1 = -(*p) + *q;
	    i__2 = *q - *p;
	    klacpy_("U", &i__1, &i__2, &x21[*m - *q + 1 + (*p + 1) * x21_dim1]
		    , ldx21, &v1t[*p + 1 + (*p + 1) * v1t_dim1], ldv1t);
	    kunglq_(q, q, q, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}
...
/*        preferred positions */
	...
	if (*p > r__) {
	    ...
	    if (wantu1) {
		klapmt_(&c_false, p, p, &u1[u1_offset], ldu1, &iwork[1]);
	    }
	    if (wantv1t) {
		klapmr_(&c_false, p, q, &v1t[v1t_offset], ldv1t, &iwork[1]);
	    }
	}
    }
