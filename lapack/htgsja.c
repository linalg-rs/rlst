...
/* > of two doublereal upper triangular (or trapezoidal) matrices A and B. */
/* > */
...
/* > */
/*  ===================================================================== */
void  htgsja_(char *jobu, char *jobv, char *jobq, integer *m, 
	integer *p, integer *n, integer *k, integer *l, halfreal *a, 
	integer *lda, halfreal *b, integer *ldb, halfreal *tola, 
	halfreal *tolb, halfreal *alpha, halfreal *beta, halfreal *u, 
	integer *ldu, halfreal *v, integer *ldv, halfreal *q, integer *
	ldq, halfreal *work, integer *ncycle, integer *info)
{
    ...
    /* Local variables */
    integer i__, j;
    halfreal a1, a2, a3, b1, b2, b3, csq, csu, csv, snq, rwk, snu, snv;
    ...
    logical wantu, wantv;
    extern void  hlags2_(logical *, halfreal *, halfreal *,
	     halfreal *, halfreal *, halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, halfreal *, 
	    halfreal *, halfreal *), hlapll_(integer *, halfreal *, 
	    integer *, halfreal *, integer *, halfreal *);
    integer kcycle;
    extern void  hlartg_(halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *), hlaset_(char *, 
...
/*     Initialize U, V and Q, if necessary */
    ...
    if (initu) {
	hlaset_("Full", m, m, &c_b13, &c_b14, &u[u_offset], ldu);
    }
    if (initv) {
	hlaset_("Full", p, p, &c_b13, &c_b14, &v[v_offset], ldv);
    }
    if (initq) {
	hlaset_("Full", n, n, &c_b13, &c_b14, &q[q_offset], ldq);
    }
...
/*     Loop until convergence */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		hlags2_(&upper, &a1, &a2, &a3, &b1, &b2, &b3, &csu, &snu, &
			csv, &snv, &csq, &snq);

/*              Update (K+I)-th and (K+J)-th rows of matrix A: U**T *A */

		if (*k + j <= *m) {
		    hrot_(l, &a[*k + j + (*n - *l + 1) * a_dim1], lda, &a[*k 
			    + i__ + (*n - *l + 1) * a_dim1], lda, &csu, &snu);
		}

/*              Update I-th and J-th rows of matrix B: V**T *B */

		hrot_(l, &b[j + (*n - *l + 1) * b_dim1], ldb, &b[i__ + (*n - *
...
/* Computing MIN */
		i__4 = *k + *l;
		i__3 = f2cmin(i__4,*m);
		hrot_(&i__3, &a[(*n - *l + j) * a_dim1 + 1], &c__1, &a[(*n - *
			l + i__) * a_dim1 + 1], &c__1, &csq, &snq);

		hrot_(l, &b[(*n - *l + j) * b_dim1 + 1], &c__1, &b[(*n - *l + 
...
/*              Update orthogonal matrices U, V, Q, if desired. */
		...
		if (wantu && *k + j <= *m) {
		    hrot_(m, &u[(*k + j) * u_dim1 + 1], &c__1, &u[(*k + i__) *
			     u_dim1 + 1], &c__1, &csu, &snu);
		}

		if (wantv) {
		    hrot_(p, &v[j * v_dim1 + 1], &c__1, &v[i__ * v_dim1 + 1], 
			    &c__1, &csv, &snv);
		}

		if (wantq) {
		    hrot_(n, &q[(*n - *l + j) * q_dim1 + 1], &c__1, &q[(*n - *
...
/* Computing MIN */
	    ...
	    for (i__ = 1; i__ <= i__1; ++i__) {
		i__2 = *l - i__ + 1;
		hcopy_(&i__2, &a[*k + i__ + (*n - *l + i__) * a_dim1], lda, &
			work[1], &c__1);
		i__2 = *l - i__ + 1;
		hcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &work[*
			l + 1], &c__1);
		i__2 = *l - i__ + 1;
		hlapll_(&i__2, &work[1], &c__1, &work[*l + 1], &c__1, &ssmin);
...
/*           change sign if necessary */
	    ...
	    if (gamma < 0.) {
		i__2 = *l - i__ + 1;
		hscal_(&i__2, &c_b43, &b[i__ + (*n - *l + i__) * b_dim1], ldb)
			;
		if (wantv) {
		    hscal_(p, &c_b43, &v[i__ * v_dim1 + 1], &c__1);
		}
	    }

	    d__1 = abs(gamma);
	    hlartg_(&d__1, &c_b14, &beta[*k + i__], &alpha[*k + i__], &rwk);

	    if (alpha[*k + i__] >= beta[*k + i__]) {
		i__2 = *l - i__ + 1;
		d__1 = 1. / alpha[*k + i__];
		hscal_(&i__2, &d__1, &a[*k + i__ + (*n - *l + i__) * a_dim1], 
			lda);
	    } else {
		i__2 = *l - i__ + 1;
		d__1 = 1. / beta[*k + i__];
		hscal_(&i__2, &d__1, &b[i__ + (*n - *l + i__) * b_dim1], ldb);
		i__2 = *l - i__ + 1;
		hcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &a[*k 
