...
/* > */
/*  ===================================================================== */
void  wgelqt3_(integer *m, integer *n, quadcomplex *a, 
...
/*        Compute Householder transform when N=1 */
	...
	wlarfg_(n, &a[a_offset], &a[f2cmin(2,*n) * a_dim1 + 1], lda, &t[t_offset]
		);
	i__1 = t_dim1 + 1;
...
/*        Compute A(1:M1,1:N) <- (Y1,R1,T1), where Q1 = I - Y1 T1 Y1^H */
	...
	wgelqt3_(&m1, n, &a[a_offset], lda, &t[t_offset], ldt, &iinfo);
...
/*        Compute A(J1:M,1:N) =  A(J1:M,1:N) Q1^H [workspace: T(1:N1,J1:N)] */
	...
	wtrmm_("R", "U", "C", "U", &m2, &m1, &c_b1, &a[a_offset], lda, &t[i1 
		+ t_dim1], ldt);

	i__1 = *n - m1;
	wgemm_("N", "C", &m2, &m1, &i__1, &c_b1, &a[i1 + i1 * a_dim1], lda, &
		a[i1 * a_dim1 + 1], lda, &c_b1, &t[i1 + t_dim1], ldt);

	wtrmm_("R", "U", "N", "N", &m2, &m1, &c_b1, &t[t_offset], ldt, &t[i1 
		+ t_dim1], ldt);

	i__1 = *n - m1;
	z__1.r = -1., z__1.i = -0.;
	wgemm_("N", "N", &m2, &i__1, &m1, &z__1, &t[i1 + t_dim1], ldt, &a[i1 *
		 a_dim1 + 1], lda, &c_b1, &a[i1 + i1 * a_dim1], lda);

	wtrmm_("R", "U", "N", "U", &m2, &m1, &c_b1, &a[a_offset], lda, &t[i1 
		+ t_dim1], ldt);
...
/*        Compute A(J1:M,J1:N) <- (Y2,R2,T2) where Q2 = I - Y2 T2 Y2^H */
	...
	i__1 = *n - m1;
	wgelqt3_(&m2, &i__1, &a[i1 + i1 * a_dim1], lda, &t[i1 + i1 * t_dim1], 
		ldt, &iinfo);
...
/*        Compute T3 = T(J1:N1,1:N) = -T1 Y1^H Y2 T2 */
	...
	wtrmm_("R", "U", "C", "U", &m1, &m2, &c_b1, &a[i1 + i1 * a_dim1], lda,
		 &t[i1 * t_dim1 + 1], ldt);

	i__1 = *n - *m;
	wgemm_("N", "C", &m1, &m2, &i__1, &c_b1, &a[j1 * a_dim1 + 1], lda, &a[
		i1 + j1 * a_dim1], lda, &c_b1, &t[i1 * t_dim1 + 1], ldt);

	z__1.r = -1., z__1.i = -0.;
	wtrmm_("L", "U", "N", "N", &m1, &m2, &z__1, &t[t_offset], ldt, &t[i1 *
		 t_dim1 + 1], ldt)
		;

	wtrmm_("R", "U", "N", "N", &m1, &m2, &c_b1, &t[i1 + i1 * t_dim1], ldt,
		 &t[i1 * t_dim1 + 1], ldt);
