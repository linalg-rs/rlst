...
/* > */
/*  ===================================================================== */
void  qpbsvx_(char *fact, char *uplo, integer *n, integer *kd, 
	integer *nrhs, quadreal *ab, integer *ldab, quadreal *afb, 
	integer *ldafb, char *equed, quadreal *s, quadreal *b, integer *
	ldb, quadreal *x, integer *ldx, quadreal *rcond, quadreal *ferr,
	 quadreal *berr, quadreal *work, integer *iwork, integer *info)
{
    ...
    extern quadreal qlamch_(char *), qlansb_(char *, char *, 
	    integer *, integer *, quadreal *, integer *, quadreal *);
    extern void  qpbcon_(char *, integer *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, quadreal *,
	     integer *, integer *), qlaqsb_(char *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, quadreal *, 
	    quadreal *, char *);
    logical nofact;
    extern void  qlacpy_(char *, integer *, integer *, 
	    quadreal *, integer *, quadreal *, integer *), 
	    xerbla_(char *, integer *), qpbequ_(char *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, quadreal *, 
	    quadreal *, integer *);
    quadreal bignum;
    extern void  qpbrfs_(char *, integer *, integer *, integer 
	    *, quadreal *, integer *, quadreal *, integer *, quadreal *,
	     integer *, quadreal *, integer *, quadreal *, quadreal *, 
	    quadreal *, integer *, integer *), qpbtrf_(char *, 
	    integer *, integer *, quadreal *, integer *, integer *);
    integer infequ;
    extern void  qpbtrs_(char *, integer *, integer *, integer 
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	qpbequ_(uplo, n, kd, &ab[ab_offset], ldab, &s[1], &scond, &amax, &
		infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    qlaqsb_(uplo, n, kd, &ab[ab_offset], ldab, &s[1], &scond, &amax, 
		    equed);
	    rcequ = lsame_(equed, "Y");
	}
    }
...
/* Computing MAX */
		...
		j1 = f2cmax(i__2,1);
		i__2 = j - j1 + 1;
		qcopy_(&i__2, &ab[*kd + 1 - j + j1 + j * ab_dim1], &c__1, &
...
/* Computing MIN */
		...
		j2 = f2cmin(i__2,*n);
		i__2 = j2 - j + 1;
		qcopy_(&i__2, &ab[j * ab_dim1 + 1], &c__1, &afb[j * afb_dim1 
			+ 1], &c__1);
/* L50: */
	    }
	}

	qpbtrf_(uplo, n, kd, &afb[afb_offset], ldafb, info);
...
/*     Compute the norm of the matrix A. */
    ...
    anorm = qlansb_("1", uplo, n, kd, &ab[ab_offset], ldab, &work[1]);

/*     Compute the reciprocal of the condition number of A. */

    qpbcon_(uplo, n, kd, &afb[afb_offset], ldafb, &anorm, rcond, &work[1], &
	    iwork[1], info);

/*     Compute the solution matrix X. */

    qlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    qpbtrs_(uplo, n, kd, nrhs, &afb[afb_offset], ldafb, &x[x_offset], ldx, 
	    info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    qpbrfs_(uplo, n, kd, nrhs, &ab[ab_offset], ldab, &afb[afb_offset], ldafb, 
