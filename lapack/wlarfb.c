...
/* > */
/*  ===================================================================== */
void  wlarfb_(char *side, char *trans, char *direct, char *
	storev, integer *m, integer *n, integer *k, quadcomplex *v, integer 
	*ldv, quadcomplex *t, integer *ldt, quadcomplex *c__, integer *
	ldc, quadcomplex *work, integer *ldwork)
{
    ...
    extern void  wgemm_(char *, char *, integer *, integer *, 
	    ...
	    integer *), wcopy_(integer *, quadcomplex *, 
	    integer *, quadcomplex *, integer *), wtrmm_(char *, char *, 
	    char *, char *, integer *, integer *, quadcomplex *, 
	    quadcomplex *, integer *, quadcomplex *, integer *), wlacgv_(integer *, quadcomplex *, 
	    integer *);
    char transt[1];
...
/*              W := C1**H */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    wcopy_(n, &c__[j + c_dim1], ldc, &work[j * work_dim1 + 1],
			     &c__1);
		    wlacgv_(n, &work[j * work_dim1 + 1], &c__1);
/* L10: */
		}

/*              W := W * V1 */

		wtrmm_("Right", "Lower", "No transpose", "Unit", n, k, &c_b1, 
...
/*                 W := W + C2**H * V2 */
		    ...
		    i__1 = *m - *k;
		    wgemm_("Conjugate transpose", "No transpose", n, k, &i__1,
...
/*              W := W * T**H  or  W * T */
		...
		wtrmm_("Right", "Upper", transt, "Non-unit", n, k, &c_b1, &t[
...
/*                 C2 := C2 - V2 * W**H */
		    ...
		    i__1 = *m - *k;
		    z__1.r = -1., z__1.i = -0.;
		    wgemm_("No transpose", "Conjugate transpose", &i__1, n, k,
...
/*              W := W * V1**H */
		...
		wtrmm_("Right", "Lower", "Conjugate transpose", "Unit", n, k, 
...
/*              W := C1 */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    wcopy_(m, &c__[j * c_dim1 + 1], &c__1, &work[j * 
			    work_dim1 + 1], &c__1);
/* L40: */
		}
...
/*              W := W * V1 */
		...
		wtrmm_("Right", "Lower", "No transpose", "Unit", m, k, &c_b1, 
			&v[v_offset], ldv, &work[work_offset], ldwork);
		if (*n > *k) {
...
/*                 W := W + C2 * V2 */
		    ...
		    i__1 = *n - *k;
		    wgemm_("No transpose", "No transpose", m, k, &i__1, &c_b1,
...
/*              W := W * T  or  W * T**H */
		...
		wtrmm_("Right", "Upper", trans, "Non-unit", m, k, &c_b1, &t[
			t_offset], ldt, &work[work_offset], ldwork);
...
/*                 C2 := C2 - W * V2**H */
		    ...
		    i__1 = *n - *k;
		    z__1.r = -1., z__1.i = -0.;
		    wgemm_("No transpose", "Conjugate transpose", m, &i__1, k,
...
/*              W := W * V1**H */
		...
		wtrmm_("Right", "Lower", "Conjugate transpose", "Unit", m, k, 
			&c_b1, &v[v_offset], ldv, &work[work_offset], ldwork);
...
/*              W := C2**H */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    wcopy_(n, &c__[*m - *k + j + c_dim1], ldc, &work[j * 
			    work_dim1 + 1], &c__1);
		    wlacgv_(n, &work[j * work_dim1 + 1], &c__1);
/* L70: */
		}
...
/*              W := W * V2 */
		...
		wtrmm_("Right", "Upper", "No transpose", "Unit", n, k, &c_b1, 
...
/*              W := W * T**H  or  W * T */
		...
		wtrmm_("Right", "Lower", transt, "Non-unit", n, k, &c_b1, &t[
			t_offset], ldt, &work[work_offset], ldwork);
...
/*              W := W * V2**H */
		...
		wtrmm_("Right", "Upper", "Conjugate transpose", "Unit", n, k, 
...
/*              W := C2 */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    wcopy_(m, &c__[(*n - *k + j) * c_dim1 + 1], &c__1, &work[
			    j * work_dim1 + 1], &c__1);
/* L100: */
		}
...
/*              W := W * V2 */
		...
		wtrmm_("Right", "Upper", "No transpose", "Unit", m, k, &c_b1, 
...
/*              W := W * T  or  W * T**H */
		...
		wtrmm_("Right", "Lower", trans, "Non-unit", m, k, &c_b1, &t[
			t_offset], ldt, &work[work_offset], ldwork);
...
/*              W := W * V2**H */
		...
		wtrmm_("Right", "Upper", "Conjugate transpose", "Unit", m, k, 
...
/*                 W := W + C2**H * V2**H */
		    ...
		    i__1 = *m - *k;
		    wgemm_("Conjugate transpose", "Conjugate transpose", n, k,
...
/*                 C2 := C2 - V2**H * W**H */
		    ...
		    i__1 = *m - *k;
		    z__1.r = -1., z__1.i = -0.;
		    wgemm_("Conjugate transpose", "Conjugate transpose", &
...
/*                 W := W + C2 * V2**H */
		    ...
		    i__1 = *n - *k;
		    wgemm_("No transpose", "Conjugate transpose", m, k, &i__1,
...
/*                 C2 := C2 - W * V2 */
		    ...
		    i__1 = *n - *k;
		    z__1.r = -1., z__1.i = -0.;
		    wgemm_("No transpose", "No transpose", m, &i__1, k, &z__1,
