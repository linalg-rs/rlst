...
/*  ===================================================================== */
void  wgbsvx_(char *fact, char *trans, integer *n, integer *kl,
	 integer *ku, integer *nrhs, quadcomplex *ab, integer *ldab, 
	quadcomplex *afb, integer *ldafb, integer *ipiv, char *equed, 
	quadreal *r__, quadreal *c__, quadcomplex *b, integer *ldb, 
	quadcomplex *x, integer *ldx, quadreal *rcond, quadreal *ferr, 
	quadreal *berr, quadcomplex *work, quadreal *rwork, integer *
	info)
{
    ...
    quadreal colcnd;
    logical nofact;
    extern quadreal wlangb_(char *, integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadreal *);
    extern void  xerbla_(char *, integer *), wlaqgb_(
	    integer *, integer *, integer *, integer *, quadcomplex *, 
	    integer *, quadreal *, quadreal *, quadreal *, quadreal *,
	     quadreal *, char *);
    quadreal bignum;
    extern void  wgbcon_(char *, integer *, integer *, integer 
	    *, quadcomplex *, integer *, integer *, quadreal *, 
	    quadreal *, quadcomplex *, quadreal *, integer *);
    integer infequ;
    logical colequ;
    extern quadreal wlantb_(char *, char *, char *, integer *, integer *, 
	    quadcomplex *, integer *, quadreal *);
    quadreal rowcnd;
    extern void  wgbequ_(integer *, integer *, integer *, 
	    integer *, quadcomplex *, integer *, quadreal *, quadreal *,
	     quadreal *, quadreal *, quadreal *, integer *), wgbrfs_(
	    char *, integer *, integer *, integer *, integer *, quadcomplex 
	    *, integer *, quadcomplex *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadreal *, quadreal *, quadcomplex *, quadreal *, 
	    integer *), wgbtrf_(integer *, integer *, integer *, 
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	wgbequ_(n, n, kl, ku, &ab[ab_offset], ldab, &r__[1], &c__[1], &rowcnd,
		 &colcnd, &amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    wlaqgb_(n, n, kl, ku, &ab[ab_offset], ldab, &r__[1], &c__[1], &
...
/* Computing MIN */
	    ...
	    j2 = f2cmin(i__2,*n);
	    i__2 = j2 - j1 + 1;
	    wcopy_(&i__2, &ab[*ku + 1 - j + j1 + j * ab_dim1], &c__1, &afb[*
		    kl + *ku + 1 - j + j1 + j * afb_dim1], &c__1);
/* L70: */
	}

	wgbtrf_(n, n, kl, ku, &afb[afb_offset], ldafb, &ipiv[1], info);
...
/* Computing MAX */
	    ...
	    rpvgrw = wlantb_("M", "U", "N", info, &i__1, &afb[f2cmax(i__4,i__5) 
...
/*     reciprocal pivot growth factor RPVGRW. */
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = wlangb_(norm, n, kl, ku, &ab[ab_offset], ldab, &rwork[1]);
    i__1 = *kl + *ku;
    rpvgrw = wlantb_("M", "U", "N", n, &i__1, &afb[afb_offset], ldafb, &rwork[
	    1]);
    if (rpvgrw == 0.) {
	rpvgrw = 1.;
    } else {
	rpvgrw = wlangb_("M", n, kl, ku, &ab[ab_offset], ldab, &rwork[1]) / rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    wgbcon_(norm, n, kl, ku, &afb[afb_offset], ldafb, &ipiv[1], &anorm, rcond,
	     &work[1], &rwork[1], info);

/*     Compute the solution matrix X. */

    wlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    wgbtrs_(trans, n, kl, ku, nrhs, &afb[afb_offset], ldafb, &ipiv[1], &x[
	    x_offset], ldx, info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    wgbrfs_(trans, n, kl, ku, nrhs, &ab[ab_offset], ldab, &afb[afb_offset], 
