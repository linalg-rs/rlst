...
/*  ===================================================================== */
halfreal klangt_(char *norm, integer *n, halfcomplex *dl, halfcomplex *
	d__, halfcomplex *du)
{
...
/*        Find f2cmax(abs(A(i,j))). */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    d__1 = z_abs(&dl[i__]);
	    if (anorm < z_abs(&dl[i__]) || hisnan_(&d__1)) {
		anorm = z_abs(&dl[i__]);
	    }
	    d__1 = z_abs(&d__[i__]);
	    if (anorm < z_abs(&d__[i__]) || hisnan_(&d__1)) {
		anorm = z_abs(&d__[i__]);
	    }
	    d__1 = z_abs(&du[i__]);
	    if (anorm < z_abs(&du[i__]) || hisnan_(&d__1)) {
		anorm = z_abs(&du[i__]);
	    }
/* L10: */
	}
...
/*        Find normF(A). */
	...
	scale = 0.;
	sum = 1.;
	klassq_(n, &d__[1], &c__1, &scale, &sum);
	if (*n > 1) {
	    i__1 = *n - 1;
	    klassq_(&i__1, &dl[1], &c__1, &scale, &sum);
	    i__1 = *n - 1;
	    klassq_(&i__1, &du[1], &c__1, &scale, &sum);
	}
