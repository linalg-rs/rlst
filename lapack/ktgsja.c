...
/* > */
/*  ===================================================================== */
void  ktgsja_(char *jobu, char *jobv, char *jobq, integer *m, 
	integer *p, integer *n, integer *k, integer *l, halfcomplex *a, 
	integer *lda, halfcomplex *b, integer *ldb, halfreal *tola, 
	halfreal *tolb, halfreal *alpha, halfreal *beta, halfcomplex *
	u, integer *ldu, halfcomplex *v, integer *ldv, halfcomplex *q, 
	integer *ldq, halfcomplex *work, integer *ncycle, integer *info)
{
    ...
    /* Local variables */
    integer i__, j;
    halfreal a1, b1, a3, b3;
    halfcomplex a2, b2;
    halfreal csq, csu, csv;
    halfcomplex snq;
    halfreal rwk;
    halfcomplex snu, snv;
    ...
    halfreal error, ssmin;
    logical wantu, wantv;
    extern void  kcopy_(integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *), klags2_(logical *, halfreal *, 
	    halfcomplex *, halfreal *, halfreal *, halfcomplex *, 
	    halfreal *, halfreal *, halfcomplex *, halfreal *, 
	    halfcomplex *, halfreal *, halfcomplex *);
    integer kcycle;
    extern void  hlartg_(halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *), xerbla_(char *, 
	    integer *), whscal_(integer *, halfreal *, 
	    halfcomplex *, integer *), klapll_(integer *, halfcomplex *, 
	    integer *, halfcomplex *, integer *, halfreal *), klaset_(
	    char *, integer *, integer *, halfcomplex *, halfcomplex *, 
	    halfcomplex *, integer *);
...
/*     Initialize U, V and Q, if necessary */
    ...
    if (initu) {
	klaset_("Full", m, m, &c_b1, &c_b2, &u[u_offset], ldu);
    }
    if (initv) {
	klaset_("Full", p, p, &c_b1, &c_b2, &v[v_offset], ldv);
    }
    if (initq) {
...
/*     Loop until convergence */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		klags2_(&upper, &a1, &a2, &a3, &b1, &b2, &b3, &csu, &snu, &
			csv, &snv, &csq, &snq);
...
/*              Update (K+I)-th and (K+J)-th rows of matrix A: U**H *A */
		...
		if (*k + j <= *m) {
		    d_cnjg(&z__1, &snu);
		    krot_(l, &a[*k + j + (*n - *l + 1) * a_dim1], lda, &a[*k 
...
/*              Update I-th and J-th rows of matrix B: V**H *B */
		...
		d_cnjg(&z__1, &snv);
		krot_(l, &b[j + (*n - *l + 1) * b_dim1], ldb, &b[i__ + (*n - *
...
/* Computing MIN */
		i__4 = *k + *l;
		i__3 = f2cmin(i__4,*m);
		krot_(&i__3, &a[(*n - *l + j) * a_dim1 + 1], &c__1, &a[(*n - *
			l + i__) * a_dim1 + 1], &c__1, &csq, &snq);

		krot_(l, &b[(*n - *l + j) * b_dim1 + 1], &c__1, &b[(*n - *l + 
...
/*              Ensure that the diagonal elements of A and B are doublereal. */
...
/*              Update unitary matrices U, V, Q, if desired. */
		...
		if (wantu && *k + j <= *m) {
		    krot_(m, &u[(*k + j) * u_dim1 + 1], &c__1, &u[(*k + i__) *
			     u_dim1 + 1], &c__1, &csu, &snu);
		}

		if (wantv) {
		    krot_(p, &v[j * v_dim1 + 1], &c__1, &v[i__ * v_dim1 + 1], 
			    &c__1, &csv, &snv);
		}

		if (wantq) {
		    krot_(n, &q[(*n - *l + j) * q_dim1 + 1], &c__1, &q[(*n - *
...
/* Computing MIN */
	    ...
	    for (i__ = 1; i__ <= i__1; ++i__) {
		i__2 = *l - i__ + 1;
		kcopy_(&i__2, &a[*k + i__ + (*n - *l + i__) * a_dim1], lda, &
			work[1], &c__1);
		i__2 = *l - i__ + 1;
		kcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &work[*
			l + 1], &c__1);
		i__2 = *l - i__ + 1;
		klapll_(&i__2, &work[1], &c__1, &work[*l + 1], &c__1, &ssmin);
...
/* Computing MIN */
	...
	if (a1 != 0.) {
	    ...
	    if (gamma < 0.) {
		i__2 = *l - i__ + 1;
		whscal_(&i__2, &c_b39, &b[i__ + (*n - *l + i__) * b_dim1], 
			ldb);
		if (wantv) {
		    whscal_(p, &c_b39, &v[i__ * v_dim1 + 1], &c__1);
		}
	    }

	    d__1 = abs(gamma);
	    hlartg_(&d__1, &c_b42, &beta[*k + i__], &alpha[*k + i__], &rwk);

	    if (alpha[*k + i__] >= beta[*k + i__]) {
		i__2 = *l - i__ + 1;
		d__1 = 1. / alpha[*k + i__];
		whscal_(&i__2, &d__1, &a[*k + i__ + (*n - *l + i__) * a_dim1],
			 lda);
	    } else {
		i__2 = *l - i__ + 1;
		d__1 = 1. / beta[*k + i__];
		whscal_(&i__2, &d__1, &b[i__ + (*n - *l + i__) * b_dim1], ldb)
			;
		i__2 = *l - i__ + 1;
		kcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &a[*k 
