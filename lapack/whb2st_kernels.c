...
/* > */
/*  ===================================================================== */
void  zhb2st_kernels__(char *uplo, logical *wantz, integer *
	ttype, integer *st, integer *ed, integer *sweep, integer *n, integer *
	nb, integer *ib, quadcomplex *a, integer *lda, quadcomplex *v, 
	quadcomplex *tau, integer *ldvt, quadcomplex *work)
{
    ...
    integer ofdpos;
    extern void  wlarfx_(char *, integer *, integer *, 
	    quadcomplex *, quadcomplex *, quadcomplex *, integer *, 
	    quadcomplex *), wlarfy_(char *, integer *, 
...
/* L10: */
	    ...
	    wlarfg_(&lm, &ctmp, &v[vpos + 1], &c__1, &tau[taupos]);
	    ...
	    i__1 = *lda - 1;
	    wlarfy_(uplo, &lm, &v[vpos], &c__1, &z__1, &a[dpos + *st * a_dim1]
		    , &i__1, &work[1]);
	}
...
/* Computing MIN */
		...
		d_cnjg(&z__1, &tau[taupos]);
		i__1 = *lda - 1;
		wlarfx_("Left", &ln, &lm, &v[vpos], &z__1, &a[dpos - *nb + j1 
			* a_dim1], &i__1, &work[1]);
...
/* L30: */
		...
		i__1 = ln - 1;
		i__2 = *lda - 1;
		wlarfx_("Right", &i__1, &lm, &v[vpos], &tau[taupos], &a[dpos 
...
/* L20: */
	    ...
	    wlarfg_(&lm, &a[ofdpos + (*st - 1) * a_dim1], &v[vpos + 1], &c__1,
		     &tau[taupos]);
...
/* Computing MIN */
	    ...
	    if (lm > 0) {
		i__1 = *lda - 1;
		wlarfx_("Right", &lm, &ln, &v[vpos], &tau[taupos], &a[dpos + *
...
/* L40: */
		...
		wlarfg_(&lm, &a[dpos + *nb + *st * a_dim1], &v[vpos + 1], &
			c__1, &tau[taupos]);

		i__1 = ln - 1;
		d_cnjg(&z__1, &tau[taupos]);
		i__2 = *lda - 1;
		wlarfx_("Left", &lm, &i__1, &v[vpos], &z__1, &a[dpos + *nb - 
