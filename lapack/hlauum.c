...
/*  ===================================================================== */
void  hlauum_(char *uplo, integer *n, halfreal *a, integer *
	lda, integer *info)
{
    ...
    extern void  hsyrk_(char *, char *, integer *, integer *, 
	    halfreal *, halfreal *, integer *, halfreal *, halfreal *,
	     integer *), hlauu2_(char *, integer *, 
	    halfreal *, integer *, integer *), xerbla_(char *, 
	    integer *);
...
/*        Use unblocked code */
	...
	hlauu2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/* Computing MIN */
		...
		ib = f2cmin(i__3,i__4);
		i__3 = i__ - 1;
		htrmm_("Right", "Upper", "Transpose", "Non-unit", &i__3, &ib, 
			&c_b15, &a[i__ + i__ * a_dim1], lda, &a[i__ * a_dim1 
			+ 1], lda)
			;
		hlauu2_("Upper", &ib, &a[i__ + i__ * a_dim1], lda, info);
		if (i__ + ib <= *n) {
		    i__3 = i__ - 1;
		    i__4 = *n - i__ - ib + 1;
		    hgemm_("No transpose", "Transpose", &i__3, &ib, &i__4, &
			    c_b15, &a[(i__ + ib) * a_dim1 + 1], lda, &a[i__ + 
			    (i__ + ib) * a_dim1], lda, &c_b15, &a[i__ * 
			    a_dim1 + 1], lda);
		    i__3 = *n - i__ - ib + 1;
		    hsyrk_("Upper", "No transpose", &ib, &i__3, &c_b15, &a[
...
/* Computing MIN */
		...
		ib = f2cmin(i__3,i__4);
		i__3 = i__ - 1;
		htrmm_("Left", "Lower", "Transpose", "Non-unit", &ib, &i__3, &
			c_b15, &a[i__ + i__ * a_dim1], lda, &a[i__ + a_dim1], 
			lda);
		hlauu2_("Lower", &ib, &a[i__ + i__ * a_dim1], lda, info);
		if (i__ + ib <= *n) {
		    i__3 = i__ - 1;
		    i__4 = *n - i__ - ib + 1;
		    hgemm_("Transpose", "No transpose", &ib, &i__3, &i__4, &
			    c_b15, &a[i__ + ib + i__ * a_dim1], lda, &a[i__ + 
			    ib + a_dim1], lda, &c_b15, &a[i__ + a_dim1], lda);
		    i__3 = *n - i__ - ib + 1;
		    hsyrk_("Lower", "Transpose", &ib, &i__3, &c_b15, &a[i__ + 
