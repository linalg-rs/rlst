...
/*  ===================================================================== */
void  hlaqps_(integer *m, integer *n, integer *offset, integer 
	*nb, integer *kb, halfreal *a, integer *lda, integer *jpvt, 
	halfreal *tau, halfreal *vn1, halfreal *vn2, halfreal *auxv, 
	halfreal *f, integer *ldf)
{
...
/*        Determine ith pivot column and swap if necessary */
	...
	if (pvt != k) {
	    hswap_(m, &a[pvt * a_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &c__1);
	    i__1 = k - 1;
	    hswap_(&i__1, &f[pvt + f_dim1], ldf, &f[k + f_dim1], ldf);
	    itemp = jpvt[pvt];
	    jpvt[pvt] = jpvt[k];
	    jpvt[k] = itemp;
	    vn1[pvt] = vn1[k];
...
/*        A(RK:M,K) := A(RK:M,K) - A(RK:M,1:K-1)*F(K,1:K-1)**T. */
	...
	if (k > 1) {
	    i__1 = *m - rk + 1;
	    i__2 = k - 1;
	    hgemv_("No transpose", &i__1, &i__2, &c_b8, &a[rk + a_dim1], lda, 
...
/*        Generate elementary reflector H(k). */
	...
	if (rk < *m) {
	    i__1 = *m - rk + 1;
	    hlarfg_(&i__1, &a[rk + k * a_dim1], &a[rk + 1 + k * a_dim1], &
		    c__1, &tau[k]);
	} else {
	    hlarfg_(&c__1, &a[rk + k * a_dim1], &a[rk + k * a_dim1], &c__1, &
		    tau[k]);
	}
...
/*        Compute  F(K+1:N,K) := tau(K)*A(RK:M,K+1:N)**T*A(RK:M,K). */
	...
	if (k < *n) {
	    i__1 = *m - rk + 1;
	    i__2 = *n - k;
	    hgemv_("Transpose", &i__1, &i__2, &tau[k], &a[rk + (k + 1) * 
...
/*                    *A(RK:M,K). */
	...
	if (k > 1) {
	    ...
	    i__2 = k - 1;
	    d__1 = -tau[k];
	    hgemv_("Transpose", &i__1, &i__2, &d__1, &a[rk + a_dim1], lda, &a[
		    rk + k * a_dim1], &c__1, &c_b16, &auxv[1], &c__1);

	    i__1 = k - 1;
	    hgemv_("No transpose", n, &i__1, &c_b9, &f[f_dim1 + 1], ldf, &
...
/*        A(RK,K+1:N) := A(RK,K+1:N) - A(RK,1:K)*F(K+1:N,1:K)**T. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    hgemv_("No transpose", &i__1, &k, &c_b8, &f[k + 1 + f_dim1], ldf, 
...
/* Computing MIN */
	...
	i__1 = *m - rk;
	i__2 = *n - *kb;
	hgemm_("No transpose", "Transpose", &i__1, &i__2, kb, &c_b8, &a[rk + 
...
L40:
	...
	i__1 = *m - rk;
	vn1[lsticc] = hnrm2_(&i__1, &a[rk + 1 + lsticc * a_dim1], &c__1);
