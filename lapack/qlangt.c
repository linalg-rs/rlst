...
/*  ===================================================================== */
quadreal qlangt_(char *norm, integer *n, quadreal *dl, quadreal *d__, 
	quadreal *du)
{
    /* System generated locals */
    integer i__1;
    quadreal ret_val, d__1, d__2, d__3, d__4;

    /* Local variables */
    integer i__;
    quadreal sum, temp, scale;
...
/*        Find f2cmax(abs(A(i,j))). */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    if (anorm < (d__1 = dl[i__], abs(d__1)) || qisnan_(&d__3)) {
		anorm = (d__4 = dl[i__], abs(d__4));
	    }
	    d__3 = (d__2 = d__[i__], abs(d__2));
	    if (anorm < (d__1 = d__[i__], abs(d__1)) || qisnan_(&d__3)) {
		anorm = (d__4 = d__[i__], abs(d__4));
	    }
	    d__3 = (d__2 = du[i__], abs(d__2));
	    if (anorm < (d__1 = du[i__], abs(d__1)) || qisnan_(&d__3)) {
...
/*        Find normF(A). */
	...
	scale = 0.;
	sum = 1.;
	qlassq_(n, &d__[1], &c__1, &scale, &sum);
	if (*n > 1) {
	    i__1 = *n - 1;
