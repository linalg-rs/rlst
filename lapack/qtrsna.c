...
/* > */
/*  ===================================================================== */
void  qtrsna_(char *job, char *howmny, logical *select, 
	integer *n, quadreal *t, integer *ldt, quadreal *vl, integer *
	ldvl, quadreal *vr, integer *ldvr, quadreal *s, quadreal *sep, 
	integer *mm, integer *m, quadreal *work, integer *ldwork, integer *
	iwork, integer *info)
{
    ...
    quadreal prod1, prod2, scale, delta;
    ...
    quadreal bignum;
    logical wantbh;
    extern void  qlaqtr_(logical *, logical *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, quadreal *,
	     quadreal *, quadreal *, integer *), qtrexc_(char *, integer *
...
/*              Real eigenvalue. */
		...
		prod = qdot_(n, &vr[ks * vr_dim1 + 1], &c__1, &vl[ks * 
			vl_dim1 + 1], &c__1);
		rnrm = qnrm2_(n, &vr[ks * vr_dim1 + 1], &c__1);
		lnrm = qnrm2_(n, &vl[ks * vl_dim1 + 1], &c__1);
		s[ks] = abs(prod) / (rnrm * lnrm);
	    } else {

/*              Complex eigenvalue. */

		prod1 = qdot_(n, &vr[ks * vr_dim1 + 1], &c__1, &vl[ks * 
			vl_dim1 + 1], &c__1);
		prod1 += qdot_(n, &vr[(ks + 1) * vr_dim1 + 1], &c__1, &vl[(ks 
			+ 1) * vl_dim1 + 1], &c__1);
		prod2 = qdot_(n, &vl[ks * vl_dim1 + 1], &c__1, &vr[(ks + 1) * 
			vr_dim1 + 1], &c__1);
		prod2 -= qdot_(n, &vl[(ks + 1) * vl_dim1 + 1], &c__1, &vr[ks *
			 vr_dim1 + 1], &c__1);
		d__1 = qnrm2_(n, &vr[ks * vr_dim1 + 1], &c__1);
		d__2 = qnrm2_(n, &vr[(ks + 1) * vr_dim1 + 1], &c__1);
		rnrm = qlapy2_(&d__1, &d__2);
		d__1 = qnrm2_(n, &vl[ks * vl_dim1 + 1], &c__1);
		d__2 = qnrm2_(n, &vl[(ks + 1) * vl_dim1 + 1], &c__1);
...
/*           block beginning at T(k,k) to the (1,1) position. */
	    ...
	    qlacpy_("Full", n, n, &t[t_offset], ldt, &work[work_offset], 
		    ldwork);
	    ifst = k;
	    ilst = 1;
	    qtrexc_("No Q", n, &work[work_offset], ldwork, dummy, &c__1, &
...
/*                 with negative imaginary  part. */
		    ...
		    delta = qlapy2_(&mu, &work[work_dim1 + 2]);
		    cs = mu / delta;
...
/*              Estimate norm(inv(C**T)) */
		...
		kase = 0;
L50:
		qlacn2_(&nn, &work[(*n + 2) * work_dim1 + 1], &work[(*n + 4) *
...
/*                       Real eigenvalue: solve C**T*x = scale*c. */
			    ...
			    i__2 = *n - 1;
			    qlaqtr_(&c_true, &c_true, &i__2, &work[(work_dim1 
...
/*                       C**T*(p+iq) = scale*(c+id) in doublereal arithmetic. */
			    ...
			    i__2 = *n - 1;
			    qlaqtr_(&c_true, &c_false, &i__2, &work[(
...
/*                       Real eigenvalue: solve C*x = scale*c. */
			    ...
			    i__2 = *n - 1;
			    qlaqtr_(&c_false, &c_true, &i__2, &work[(
...
/*                       C*(p+iq) = scale*(c+id) in doublereal arithmetic. */
			    ...
			    i__2 = *n - 1;
			    qlaqtr_(&c_false, &c_false, &i__2, &work[(
				    work_dim1 << 1) + 2], ldwork, &work[(*n + 
				    1) * work_dim1 + 1], &mu, &scale, &work[(*
				    n + 4) * work_dim1 + 1], &work[(*n + 6) * 
				    work_dim1 + 1], &ierr);
