...
void  dsb2st_kernels__(char *uplo, logical *wantz, integer *
	...
	nb, integer *ib, quadreal *a, integer *lda, quadreal *v, 
	quadreal *tau, integer *ldvt, quadreal *work)
{
    ...
    extern void  qlarfg_(integer *, quadreal *, quadreal *,
	     integer *, quadreal *);
    integer ajeter;
    extern void  qlarfx_(char *, integer *, integer *, 
	    quadreal *, quadreal *, quadreal *, integer *, quadreal *), qlarfy_(char *, integer *, quadreal *, integer *, 
...
/* L10: */
	    ...
	    qlarfg_(&lm, &ctmp, &v[vpos + 1], &c__1, &tau[taupos]);
	    a[ofdpos + *st * a_dim1] = ctmp;

	    lm = *ed - *st + 1;
	    d__1 = tau[taupos];
	    i__1 = *lda - 1;
	    qlarfy_(uplo, &lm, &v[vpos], &c__1, &d__1, &a[dpos + *st * a_dim1]
		    , &i__1, &work[1]);
	}
...
/* Computing MIN */
		...
		d__1 = tau[taupos];
		i__1 = *lda - 1;
		qlarfx_("Left", &ln, &lm, &v[vpos], &d__1, &a[dpos - *nb + j1 
			* a_dim1], &i__1, &work[1]);
...
/* L30: */
		...
		i__1 = ln - 1;
		i__2 = *lda - 1;
		qlarfx_("Right", &i__1, &lm, &v[vpos], &tau[taupos], &a[dpos 
...
/* L20: */
	    ...
	    qlarfg_(&lm, &a[ofdpos + (*st - 1) * a_dim1], &v[vpos + 1], &c__1,
		     &tau[taupos]);
...
/* Computing MIN */
	    ...
	    if (lm > 0) {
		i__1 = *lda - 1;
		qlarfx_("Right", &lm, &ln, &v[vpos], &tau[taupos], &a[dpos + *
...
/* L40: */
		...
		qlarfg_(&lm, &a[dpos + *nb + *st * a_dim1], &v[vpos + 1], &
			c__1, &tau[taupos]);

		i__1 = ln - 1;
		d__1 = tau[taupos];
		i__2 = *lda - 1;
		qlarfx_("Left", &lm, &i__1, &v[vpos], &d__1, &a[dpos + *nb - 
