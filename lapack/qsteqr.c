...
/*  ===================================================================== */
void  qsteqr_(char *compz, integer *n, quadreal *d__, 
	...
	integer *info)
{
    ...
    extern void  qswap_(integer *, quadreal *, integer *, 
	    quadreal *, integer *), qlaev2_(quadreal *, quadreal *, 
...
/*     matrix. */
    ...
    if (icompz == 2) {
	qlaset_("Full", n, n, &c_b9, &c_b10, &z__[z_offset], ldz);
    }
...
/*     Scale submatrix in rows and columns L to LEND */
    ...
    i__1 = lend - l + 1;
    anorm = qlanst_("M", &i__1, &d__[l], &e[l]);
    iscale = 0;
    if (anorm == 0.) {
	goto L10;
    }
...
/*        If eigenvectors are desired, then apply saved rotations. */
	...
	if (icompz > 0) {
	    mm = m - l + 1;
	    qlasr_("R", "V", "B", n, &mm, &work[l], &work[*n - 1 + l], &z__[l 
		    * z_dim1 + 1], ldz);
	}
...
/*        If eigenvectors are desired, then apply saved rotations. */
	...
	if (icompz > 0) {
	    mm = l - m + 1;
	    qlasr_("R", "V", "F", n, &mm, &work[m], &work[*n - 1 + m], &z__[m 
		    * z_dim1 + 1], ldz);
	}
...
/* L170: */
		...
		d__[k] = d__[i__];
		d__[i__] = p;
		qswap_(n, &z__[i__ * z_dim1 + 1], &c__1, &z__[k * z_dim1 + 1],
			 &c__1);
	    }
/* L180: */
	}
