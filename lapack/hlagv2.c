...
/*  ===================================================================== */
void  hlagv2_(halfreal *a, integer *lda, halfreal *b, 
	integer *ldb, halfreal *alphar, halfreal *alphai, halfreal *
	beta, halfreal *csl, halfreal *snl, halfreal *csr, halfreal *
	snr)
{
    ...
    /* Local variables */
    halfreal r__, t, h1, h2, h3, wi, qq, rr, wr1, wr2, ulp;
    extern void  hrot_(integer *, halfreal *, integer *, 
	    ...
	    halfreal *);
    halfreal anorm, bnorm, scale1, scale2;
...
/*     Check if B is singular */
	...
	hlartg_(&a[a_dim1 + 1], &a[a_dim1 + 2], csl, snl, &r__);
	*csr = 1.;
	*snr = 0.;
	hrot_(&c__2, &a[a_dim1 + 1], lda, &a[a_dim1 + 2], lda, csl, snl);
	hrot_(&c__2, &b[b_dim1 + 1], ldb, &b[b_dim1 + 2], ldb, csl, snl);
	a[a_dim1 + 2] = 0.;
	b[b_dim1 + 1] = 0.;
	b[b_dim1 + 2] = 0.;
	wi = 0.;
	...
	hlartg_(&a[(a_dim1 << 1) + 2], &a[a_dim1 + 2], csr, snr, &t);
	*snr = -(*snr);
	hrot_(&c__2, &a[a_dim1 + 1], &c__1, &a[(a_dim1 << 1) + 1], &c__1, csr,
		 snr);
	hrot_(&c__2, &b[b_dim1 + 1], &c__1, &b[(b_dim1 << 1) + 1], &c__1, csr,
		 snr);
	*csl = 1.;
	*snl = 0.;
	a[a_dim1 + 2] = 0.;
...
/*        B is nonsingular, first compute the eigenvalues of (A,B) */
	...
	hlag2_(&a[a_offset], lda, &b[b_offset], ldb, &safmin, &scale1, &
		scale2, &wr1, &wr2, &wi);
...
/*              (sA - wB) */
		...
		d__1 = scale1 * a[a_dim1 + 2];
		hlartg_(&h3, &d__1, csr, snr, &t);

	    }

	    *snr = -(*snr);
	    hrot_(&c__2, &a[a_dim1 + 1], &c__1, &a[(a_dim1 << 1) + 1], &c__1, 
		    csr, snr);
	    hrot_(&c__2, &b[b_dim1 + 1], &c__1, &b[(b_dim1 << 1) + 1], &c__1, 
		    csr, snr);
...
/*              find left rotation matrix Q to zero out B(2,1) */
		...
		hlartg_(&b[b_dim1 + 1], &b[b_dim1 + 2], csl, snl, &r__);
...
/*           first compute the SVD of the matrix B */
	    ...
	    hlasv2_(&b[b_dim1 + 1], &b[(b_dim1 << 1) + 1], &b[(b_dim1 << 1) + 
