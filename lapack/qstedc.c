...
/* > */
/*  ===================================================================== */
void  qstedc_(char *compz, integer *n, quadreal *d__, 
	...
	integer *lwork, integer *iwork, integer *liwork, integer *info)
{
    ...
    extern void  qswap_(integer *, quadreal *, integer *, 
	    quadreal *, integer *);
    integer lwmin;
    extern void  qlaed0_(integer *, integer *, integer *, 
...
/*     solve the problem with another solver. */
	...
	qsteqr_(compz, n, &d__[1], &e[1], &z__[z_offset], ldz, &work[1], info);
...
/*        use. */
	...
	if (icompz == 2) {
	    qlaset_("Full", n, n, &c_b17, &c_b18, &z__[z_offset], ldz);
	}
...
/*              Scale. */
		...
		qlaed0_(&icompz, n, &m, &d__[start], &e[start], &z__[strtrw + 
...
/*                 workspace and then multiply back into Z. */
		    ...
		    qsteqr_("I", &m, &d__[start], &e[start], &work[1], &m, &
			    work[m * m + 1], info);
		    qlacpy_("A", n, &m, &z__[start * z_dim1 + 1], ldz, &work[
			    storez], n);
		    qgemm_("N", "N", n, &m, &m, &c_b18, &work[storez], n, &
			    work[1], &m, &c_b17, &z__[start * z_dim1 + 1], 
			    ldz);
		} else if (icompz == 2) {
		    qsteqr_("I", &m, &d__[start], &e[start], &z__[start + 
			    start * z_dim1], ldz, &work[1], info);
		} else {
		    qsterf_(&m, &d__[start], &e[start], info);
		}
		if (*info != 0) {
...
/* L30: */
		...
		if (k != i__) {
		    d__[k] = d__[i__];
		    d__[i__] = p;
		    qswap_(n, &z__[i__ * z_dim1 + 1], &c__1, &z__[k * z_dim1 
			    + 1], &c__1);
		}
/* L40: */
	    }
