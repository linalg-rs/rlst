...
/* > of a doublereal symmetric tridiagonal matrix T.  Eigenvalues and */
...
/* > */
/*  ===================================================================== */
void  hstevr_(char *jobz, char *range, integer *n, halfreal *
...
/*     try DSTEBZ. */
	...
	i__1 = *n - 1;
	hcopy_(&i__1, &e[1], &c__1, &work[1], &c__1);
	if (! wantz) {
	    hcopy_(n, &d__[1], &c__1, &w[1], &c__1);
	    hsterf_(n, &w[1], &work[1], info);
	} else {
	    hcopy_(n, &d__[1], &c__1, &work[*n + 1], &c__1);
	    if (*abstol <= *n * 2. * eps) {
		tryrac = TRUE_;
	    } else {
		tryrac = FALSE_;
	    ...
	    i__1 = *lwork - (*n << 1);
	    hstemr_(jobz, "A", n, &work[*n + 1], &work[1], vl, vu, il, iu, m, 
...
/*     Otherwise, call DSTEBZ and, if eigenvectors are desired, DSTEIN. */
	...
	*(unsigned char *)order = 'E';
    ...
    hstebz_(range, order, n, &vll, &vuu, il, iu, abstol, &d__[1], &e[1], m, &
	    nsplit, &w[1], &iwork[indibl], &iwork[indisp], &work[1], &iwork[
	    indiwo], info);

    if (wantz) {
	hstein_(n, &d__[1], &e[1], m, &w[1], &iwork[indibl], &iwork[indisp], &
