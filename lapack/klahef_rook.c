...
/*  ===================================================================== */
void  zlahef_rook__(char *uplo, integer *n, integer *nb, 
	integer *kb, halfcomplex *a, integer *lda, integer *ipiv, 
	halfcomplex *w, integer *ldw, integer *info)
{
    ...
    integer j, k, p;
    halfreal t, r1;
    halfcomplex d11, d21, d22;
    integer jb, ii, jj, kk, kp, kw, jp1, jp2, kkw;
    logical done;
    integer imax, jmax;
    halfreal alpha;
    extern logical lsame_(char *, char *);
    halfreal dtemp, sfmin;
    integer itemp;
    ...
    extern void  kgemv_(char *, integer *, integer *, 
	    ...
	    integer *), kswap_(integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *);
...
/*        Copy column K of A to column KW of W and update it */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    kcopy_(&i__1, &a[k * a_dim1 + 1], &c__1, &w[kw * w_dim1 + 1], &
		    c__1);
	}
	i__1 = k + kw * w_dim1;
	i__2 = k + k * a_dim1;
	...
	if (k < *n) {
	    i__1 = *n - k;
	    z__1.r = -1., z__1.i = -0.;
	    kgemv_("No transpose", &k, &i__1, &z__1, &a[(k + 1) * a_dim1 + 1],
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k > 1) {
	    i__1 = k - 1;
	    imax = ikamax_(&i__1, &w[kw * w_dim1 + 1], &c__1);
	    i__1 = imax + kw * w_dim1;
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    if (k > 1) {
		i__1 = k - 1;
		kcopy_(&i__1, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], 
			&c__1);
	    }
	} else {
...
/*                 Copy column IMAX to column KW-1 of W and update it */
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    kcopy_(&i__1, &a[imax * a_dim1 + 1], &c__1, &w[(kw - 1) * 
			    w_dim1 + 1], &c__1);
		}
		...
		i__1 = k - imax;
		kcopy_(&i__1, &a[imax + (imax + 1) * a_dim1], lda, &w[imax + 
			1 + (kw - 1) * w_dim1], &c__1);
		i__1 = k - imax;
		klacgv_(&i__1, &w[imax + 1 + (kw - 1) * w_dim1], &c__1);

		if (k < *n) {
		    i__1 = *n - k;
		    z__1.r = -1., z__1.i = -0.;
		    kgemv_("No transpose", &k, &i__1, &z__1, &a[(k + 1) * 
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = k - imax;
		    jmax = imax + ikamax_(&i__1, &w[imax + 1 + (kw - 1) * 
			    w_dim1], &c__1);
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    itemp = ikamax_(&i__1, &w[(kw - 1) * w_dim1 + 1], &c__1);
...
/*                    copy column KW-1 of W to column KW of W */
		    ...
		    kcopy_(&k, &w[(kw - 1) * w_dim1 + 1], &c__1, &w[kw * 
			    w_dim1 + 1], &c__1);
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = k - 1 - p;
		kcopy_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + (p + 1) * 
			a_dim1], lda);
		i__1 = k - 1 - p;
		klacgv_(&i__1, &a[p + (p + 1) * a_dim1], lda);
		if (p > 1) {
		    i__1 = p - 1;
		    kcopy_(&i__1, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 
			    1], &c__1);
		}
...
/*              in last KKW to NB columns of W. */
		...
		if (k < *n) {
		    i__1 = *n - k;
		    kswap_(&i__1, &a[k + (k + 1) * a_dim1], lda, &a[p + (k + 
			    1) * a_dim1], lda);
		}
		i__1 = *n - kk + 1;
		kswap_(&i__1, &w[k + kkw * w_dim1], ldw, &w[p + kkw * w_dim1],
			 ldw);
	    }
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = kk - 1 - kp;
		kcopy_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (kp + 
			1) * a_dim1], lda);
		i__1 = kk - 1 - kp;
		klacgv_(&i__1, &a[kp + (kp + 1) * a_dim1], lda);
		if (kp > 1) {
		    i__1 = kp - 1;
		    kcopy_(&i__1, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 
			    + 1], &c__1);
		}
...
/*              in last KKW to NB columns of W. */
		...
		if (k < *n) {
		    i__1 = *n - k;
		    kswap_(&i__1, &a[kk + (k + 1) * a_dim1], lda, &a[kp + (k 
			    + 1) * a_dim1], lda);
		}
		i__1 = *n - kk + 1;
		kswap_(&i__1, &w[kk + kkw * w_dim1], ldw, &w[kp + kkw * 
			w_dim1], ldw);
	    }
...
/*              element D(k,k) from W (potentially saves only one load)) */
		kcopy_(&k, &w[kw * w_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &
			c__1);
		if (k > 1) {
...
/*                 Handle division by a small number */
			...
			r1 = 1. / t;
			i__1 = k - 1;
			whscal_(&i__1, &r1, &a[k * a_dim1 + 1], &c__1);
		    } else {
			i__1 = k - 1;
			for (ii = 1; ii <= i__1; ++ii) {
			    i__2 = ii + k * a_dim1;
...
/*                 (2) Conjugate column W(kw) */
		    ...
		    i__1 = k - 1;
		    klacgv_(&i__1, &w[kw * w_dim1 + 1], &c__1);
		}
...
/*              (2) Conjugate columns W(kw) and W(kw-1) */
		...
		klacgv_(&i__1, &w[kw * w_dim1 + 1], &c__1);
		i__1 = k - 2;
		klacgv_(&i__1, &w[(kw - 1) * w_dim1 + 1], &c__1);
...
/*           Update the upper triangle of the diagonal block */
		...
		i__4 = *n - k;
		z__1.r = -1., z__1.i = -0.;
		kgemv_("No transpose", &i__3, &i__4, &z__1, &a[j + (k + 1) * 
...
/*           Update the rectangular superdiagonal block */
		...
		i__3 = *n - k;
		z__1.r = -1., z__1.i = -0.;
		kgemm_("No transpose", "Transpose", &i__2, &jb, &i__3, &z__1, 
...
/*           of the rows to swap back doesn't include diagonal element) */
	...
	if (jp2 != jj && j <= *n) {
	    i__1 = *n - j + 1;
	    kswap_(&i__1, &a[jp2 + j * a_dim1], lda, &a[jj + j * a_dim1], lda)
		    ;
	}
	++jj;
	if (kstep == 2 && jp1 != jj && j <= *n) {
	    i__1 = *n - j + 1;
	    kswap_(&i__1, &a[jp1 + j * a_dim1], lda, &a[jj + j * a_dim1], lda)
		    ;
	}
	if (j < *n) {
	    goto L60;
...
/*        Copy column K of A to column K of W and update column K of W */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    kcopy_(&i__1, &a[k + 1 + k * a_dim1], &c__1, &w[k + 1 + k * 
		    w_dim1], &c__1);
	}
	if (k > 1) {
	    i__1 = *n - k + 1;
	    i__2 = k - 1;
	    z__1.r = -1., z__1.i = -0.;
	    kgemv_("No transpose", &i__1, &i__2, &z__1, &a[k + a_dim1], lda, &
...
/*        Determine both COLMAX and IMAX. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + ikamax_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
	    i__1 = imax + k * w_dim1;
	    colmax = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[imax + 
		    k * w_dim1]), abs(d__2));
	} else {
...
/*           Column K is zero or underflow: set INFO and continue */
		...
		*info = k;
	    ...
	    if (k < *n) {
		i__1 = *n - k;
		kcopy_(&i__1, &w[k + 1 + k * w_dim1], &c__1, &a[k + 1 + k * 
			a_dim1], &c__1);
	    }
	} else {
...
/*                 Copy column IMAX to column k+1 of W and update it */
		...
		i__1 = imax - k;
		kcopy_(&i__1, &a[imax + k * a_dim1], lda, &w[k + (k + 1) * 
			w_dim1], &c__1);
		i__1 = imax - k;
		klacgv_(&i__1, &w[k + (k + 1) * w_dim1], &c__1);
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    kcopy_(&i__1, &a[imax + 1 + imax * a_dim1], &c__1, &w[
		...
		if (k > 1) {
		    ...
		    i__2 = k - 1;
		    z__1.r = -1., z__1.i = -0.;
		    kgemv_("No transpose", &i__1, &i__2, &z__1, &a[k + a_dim1]
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax != k) {
		    i__1 = imax - k;
		    jmax = k - 1 + ikamax_(&i__1, &w[k + (k + 1) * w_dim1], &
			    c__1);
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    itemp = imax + ikamax_(&i__1, &w[imax + 1 + (k + 1) * 
			    w_dim1], &c__1);
...
/*                    copy column K+1 of W to column K of W */
		    ...
		    i__1 = *n - k + 1;
		    kcopy_(&i__1, &w[k + (k + 1) * w_dim1], &c__1, &w[k + k * 
			    w_dim1], &c__1);
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = p - k - 1;
		kcopy_(&i__1, &a[k + 1 + k * a_dim1], &c__1, &a[p + (k + 1) * 
			a_dim1], lda);
		i__1 = p - k - 1;
		klacgv_(&i__1, &a[p + (k + 1) * a_dim1], lda);
		if (p < *n) {
		    i__1 = *n - p;
		    kcopy_(&i__1, &a[p + 1 + k * a_dim1], &c__1, &a[p + 1 + p 
			    * a_dim1], &c__1);
		}
...
/*              in first KK columns of W. */
		...
		if (k > 1) {
		    i__1 = k - 1;
		    kswap_(&i__1, &a[k + a_dim1], lda, &a[p + a_dim1], lda);
		}
		kswap_(&kk, &w[k + w_dim1], ldw, &w[p + w_dim1], ldw);
	    }
...
/*              will be later overwritten. */
		...
		a[i__1].r = d__1, a[i__1].i = 0.;
		i__1 = kp - kk - 1;
		kcopy_(&i__1, &a[kk + 1 + kk * a_dim1], &c__1, &a[kp + (kk + 
			1) * a_dim1], lda);
		i__1 = kp - kk - 1;
		klacgv_(&i__1, &a[kp + (kk + 1) * a_dim1], lda);
		if (kp < *n) {
		    i__1 = *n - kp;
		    kcopy_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + 1 
			    + kp * a_dim1], &c__1);
		}
...
/*              in first KK columns of W. */
		...
		if (k > 1) {
		    i__1 = k - 1;
		    kswap_(&i__1, &a[kk + a_dim1], lda, &a[kp + a_dim1], lda);
		}
		kswap_(&kk, &w[kk + w_dim1], ldw, &w[kp + w_dim1], ldw);
	    }
...
/*              element D(k,k) from W (potentially saves only one load)) */
		i__1 = *n - k + 1;
		kcopy_(&i__1, &w[k + k * w_dim1], &c__1, &a[k + k * a_dim1], &
			c__1);
		if (k < *n) {
...
/*                 Handle division by a small number */
			...
			r1 = 1. / t;
			i__1 = *n - k;
			whscal_(&i__1, &r1, &a[k + 1 + k * a_dim1], &c__1);
		    } else {
			i__1 = *n;
...
/*                 (2) Conjugate column W(k) */
		    ...
		    i__1 = *n - k;
		    klacgv_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
		}
...
/*              (2) Conjugate columns W(k) and W(k+1) */
		...
		klacgv_(&i__1, &w[k + 1 + k * w_dim1], &c__1);
		i__1 = *n - k - 1;
		klacgv_(&i__1, &w[k + 2 + (k + 1) * w_dim1], &c__1);
...
/*           Update the lower triangle of the diagonal block */
		...
		i__5 = k - 1;
		z__1.r = -1., z__1.i = -0.;
		kgemv_("No transpose", &i__4, &i__5, &z__1, &a[jj + a_dim1], 
...
/*           Update the rectangular subdiagonal block */
		...
		i__4 = k - 1;
		z__1.r = -1., z__1.i = -0.;
		kgemm_("No transpose", "Transpose", &i__3, &jb, &i__4, &z__1, 
...
/*           of the rows to swap back doesn't include diagonal element) */
	--j;
	if (jp2 != jj && j >= 1) {
	    kswap_(&j, &a[jp2 + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	--jj;
	if (kstep == 2 && jp1 != jj && j >= 1) {
	    kswap_(&j, &a[jp1 + a_dim1], lda, &a[jj + a_dim1], lda);
	}
	if (j > 1) {
	    goto L120;
	}
