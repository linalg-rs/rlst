...
/* > */
/*  ===================================================================== */
void  qlacon_(integer *n, quadreal *v, quadreal *x, 
	integer *isgn, quadreal *est, integer *kase)
{
...
L120:
    ...
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
	x[i__] = altsgn * ((quadreal) (i__ - 1) / (quadreal) (*n - 1) + 
		1.);
	altsgn = -altsgn;
/* L130: */
    }
...
L140:
    temp = qasum_(n, &x[1], &c__1) / (quadreal) (*n * 3) * 2.;
    if (temp > *est) {
	qcopy_(n, &x[1], &c__1, &v[1], &c__1);
	*est = temp;
    }
