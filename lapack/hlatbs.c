...
/* > */
/*  ===================================================================== */
void  hlatbs_(char *uplo, char *trans, char *diag, char *
	normin, integer *n, integer *kd, halfreal *ab, integer *ldab, 
	halfreal *x, halfreal *scale, halfreal *cnorm, integer *info)
{
    ...
    halfreal xbnd;
    integer imax;
    halfreal tmax, tjjs, xmax, grow, sumj;
    extern void  hscal_(integer *, halfreal *, halfreal *, 
	    integer *);
    integer maind;
    extern logical lsame_(char *, char *);
    halfreal tscal, uscal;
    extern halfreal hasum_(integer *, halfreal *, integer *);
    integer jlast;
    extern void  htbsv_(char *, char *, char *, integer *, 
	    integer *, halfreal *, integer *, halfreal *, integer *), haxpy_(integer *, halfreal *, 
...
/*     Determine machine dependent parameters to control overflow. */
    ...
    smlnum = hlamch_("Safe minimum") / hlamch_("Precision");
    bignum = 1. / smlnum;
    *scale = 1.;
...
/* Computing MIN */
		...
		cnorm[j] = hasum_(&jlen, &ab[*kd + 1 - jlen + j * ab_dim1], &
			c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = f2cmin(i__2,i__3);
		if (jlen > 0) {
		    cnorm[j] = hasum_(&jlen, &ab[j * ab_dim1 + 2], &c__1);
		} else {
		    cnorm[j] = 0.;
		}
/* L20: */
...
/*        elements of X is not too small. */
	...
	htbsv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &x[1], &c__1);
    } else {
...
/* Computing MIN */
			...
			d__1 = -x[j] * tscal;
			haxpy_(&jlen, &d__1, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
			i__3 = j - 1;
			i__ = ihamax_(&i__3, &x[1], &c__1);
...
/* Computing MIN */
		    ...
		    if (jlen > 0) {
			d__1 = -x[j] * tscal;
			haxpy_(&jlen, &d__1, &ab[j * ab_dim1 + 2], &c__1, &x[
				j + 1], &c__1);
		    }
		    i__3 = *n - j;
		    i__ = j + ihamax_(&i__3, &x[j + 1], &c__1);
		    xmax = (d__1 = x[i__], abs(d__1));
		}
/* L110: */
	    }
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = f2cmin(i__3,i__4);
			sumj = hdot_(&jlen, &ab[*kd + 1 - jlen + j * ab_dim1],
				 &c__1, &x[j - jlen], &c__1);
		    } else {
/* Computing MIN */
			i__3 = *kd, i__4 = *n - j;
			jlen = f2cmin(i__3,i__4);
			if (jlen > 0) {
			    sumj = hdot_(&jlen, &ab[j * ab_dim1 + 2], &c__1, &
				    x[j + 1], &c__1);
			}
		    }
		} else {
