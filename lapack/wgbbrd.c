...
/* > \verbatim */
/* > */
/* > ZGBBRD reduces a complex general m-by-n band matrix A to doublereal upper */
...
/*  ===================================================================== */
void  wgbbrd_(char *vect, integer *m, integer *n, integer *ncc,
	 integer *kl, integer *ku, quadcomplex *ab, integer *ldab, 
	quadreal *d__, quadreal *e, quadcomplex *q, integer *ldq, 
	quadcomplex *pt, integer *ldpt, quadcomplex *c__, integer *ldc, 
	quadcomplex *work, quadreal *rwork, integer *info)
{
    ...
    integer minmn;
    logical wantq;
    extern void  xerbla_(char *, integer *), wlaset_(
	    char *, integer *, integer *, quadcomplex *, quadcomplex *, 
	    quadcomplex *, integer *), wlartg_(quadcomplex *, 
	    quadcomplex *, quadreal *, quadcomplex *, quadcomplex *), 
	    wlargv_(integer *, quadcomplex *, integer *, quadcomplex *, 
	    integer *, quadreal *, integer *);
    logical wantpt;
    extern void  wlartv_(integer *, quadcomplex *, integer *,
	     quadcomplex *, integer *, quadreal *, quadcomplex *, 
	    integer *);
...
/*     Initialize Q and P**H to the unit matrix, if needed */
    ...
    if (wantq) {
	wlaset_("Full", m, m, &c_b1, &c_b2, &q[q_offset], ldq);
    }
    if (wantpt) {
	wlaset_("Full", n, n, &c_b1, &c_b2, &pt[pt_offset], ldpt);
    }
...
/*        and the doublereal cosines in RWORK. */
...
/*              which have been created below the band */
		...
		if (nr > 0) {
		    wlargv_(&nr, &ab[klu1 + (j1 - klm - 1) * ab_dim1], &inca, 
...
/*              apply plane rotations from the left */
		...
		for (l = 1; l <= i__3; ++l) {
		    ...
		    if (nrt > 0) {
			wlartv_(&nrt, &ab[klu1 - l + (j1 - klm + l - 1) * 
...
/*                    within the band, and apply rotation from the left */
			...
			wlartg_(&ab[*ku + ml - 1 + i__ * ab_dim1], &ab[*ku + 
...
/*                 accumulate product of plane rotations in Q */
		    ...
		    for (j = j1; i__4 < 0 ? j >= i__3 : j <= i__3; j += i__4) 
			    {
			d_cnjg(&z__1, &work[j]);
			wrot_(m, &q[(j - 1) * q_dim1 + 1], &c__1, &q[j * 
...
/*                 apply plane rotations to C */
		    ...
		    for (j = j1; i__3 < 0 ? j >= i__4 : j <= i__4; j += i__3) 
			    {
			wrot_(ncc, &c__[j - 1 + c_dim1], ldc, &c__[j + c_dim1]
				, ldc, &rwork[j], &work[j]);
/* L30: */
		    }
		}
...
/*              which have been generated above the band */
		...
		if (nr > 0) {
		    wlargv_(&nr, &ab[(j1 + kun - 1) * ab_dim1 + 1], &inca, &
...
/*              apply plane rotations from the right */
		...
		for (l = 1; l <= i__4; ++l) {
		    ...
		    if (nrt > 0) {
			wlartv_(&nrt, &ab[l + 1 + (j1 + kun - 1) * ab_dim1], &
...
/*                    within the band, and apply rotation from the right */
			...
			wlartg_(&ab[*ku - mu + 3 + (i__ + mu - 2) * ab_dim1], 
...
/* Computing MIN */
			...
			wrot_(&i__4, &ab[*ku - mu + 4 + (i__ + mu - 2) * 
...
/*                 accumulate product of plane rotations in P**H */
			...
			wrot_(n, &pt[j + kun - 1 + pt_dim1], ldpt, &pt[j + 
...
/* Computing MIN */
	...
	i__1 = f2cmin(i__2,*n);
	for (i__ = 1; i__ <= i__1; ++i__) {
	    wlartg_(&ab[i__ * ab_dim1 + 1], &ab[i__ * ab_dim1 + 2], &rc, &rs, 
		    &ra);
	    i__2 = i__ * ab_dim1 + 1;
	    ...
	    if (wantq) {
		d_cnjg(&z__1, &rs);
		wrot_(m, &q[i__ * q_dim1 + 1], &c__1, &q[(i__ + 1) * q_dim1 + 
			1], &c__1, &rc, &z__1);
	    }
	    if (wantc) {
		wrot_(ncc, &c__[i__ + c_dim1], ldc, &c__[i__ + 1 + c_dim1], 
			ldc, &rc, &rs);
	    }
/* L100: */
	}
...
/*           right */
	    ...
	    for (i__ = *m; i__ >= 1; --i__) {
		wlartg_(&ab[*ku + 1 + i__ * ab_dim1], &rb, &rc, &rs, &ra);
		...
		if (wantpt) {
		    d_cnjg(&z__1, &rs);
		    wrot_(n, &pt[i__ + pt_dim1], ldpt, &pt[*m + 1 + pt_dim1], 
			    ldpt, &rc, &z__1);
		}
/* L110: */
	    }
...
/*     Make diagonal and superdiagonal elements doublereal, storing them in D */
/*     and E */
	...
	if (wantq) {
	    wscal_(m, &t, &q[i__ * q_dim1 + 1], &c__1);
	}
	if (wantc) {
	    d_cnjg(&z__1, &t);
	    wscal_(ncc, &z__1, &c__[i__ + c_dim1], ldc);
	}
	if (i__ < minmn) {
	    if (*ku == 0 && *kl == 0) {
		e[i__] = 0.;
		...
		if (wantpt) {
		    wscal_(n, &t, &pt[i__ + 1 + pt_dim1], ldpt);
		}
