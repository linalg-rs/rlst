...
/*  ===================================================================== */
void  qgesvx_(char *fact, char *trans, integer *n, integer *
	nrhs, quadreal *a, integer *lda, quadreal *af, integer *ldaf, 
	integer *ipiv, char *equed, quadreal *r__, quadreal *c__, 
	quadreal *b, integer *ldb, quadreal *x, integer *ldx, quadreal *
	rcond, quadreal *ferr, quadreal *berr, quadreal *work, integer *
	iwork, integer *info)
{
    ...
    extern void  qlaqge_(integer *, integer *, quadreal *, 
	    integer *, quadreal *, quadreal *, quadreal *, quadreal *,
	     quadreal *, char *), qgecon_(char *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, quadreal *,
	     integer *, integer *);
    quadreal colcnd;
    logical nofact;
    extern void  qgeequ_(integer *, integer *, quadreal *, 
	    integer *, quadreal *, quadreal *, quadreal *, quadreal *,
	     quadreal *, integer *), qgerfs_(char *, integer *, integer *, 
	    ...
	    integer *, quadreal *, integer *), xerbla_(char *, 
	    integer *);
    quadreal bignum;
    extern quadreal qlantr_(char *, char *, char *, integer *, integer *, 
	    quadreal *, integer *, quadreal *);
    integer infequ;
    logical colequ;
    extern void  qgetrs_(char *, integer *, integer *, 
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	qgeequ_(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &colcnd, &
		amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    qlaqge_(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &
		    colcnd, &amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed,
		     "B");
	    colequ = lsame_(equed, "C") || lsame_(equed,
...
/*        Compute the LU factorization of A. */
	...
	qlacpy_("Full", n, n, &a[a_offset], lda, &af[af_offset], ldaf);
	qgetrf_(n, n, &af[af_offset], ldaf, &ipiv[1], info);
...
/*           leading rank-deficient INFO columns of A. */
	    ...
	    rpvgrw = qlantr_("M", "U", "N", info, info, &af[af_offset], ldaf, 
		    &work[1]);
	    if (rpvgrw == 0.) {
		rpvgrw = 1.;
	    } else {
		rpvgrw = qlange_("M", n, info, &a[a_offset], lda, &work[1]) / rpvgrw;
	    }
	    work[1] = rpvgrw;
	    *rcond = 0.;
	    return;
...
/*     reciprocal pivot growth factor RPVGRW. */
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = qlange_(norm, n, n, &a[a_offset], lda, &work[1]);
    rpvgrw = qlantr_("M", "U", "N", n, n, &af[af_offset], ldaf, &work[1]);
    if (rpvgrw == 0.) {
	rpvgrw = 1.;
    } else {
	rpvgrw = qlange_("M", n, n, &a[a_offset], lda, &work[1]) / 
		rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    qgecon_(norm, n, &af[af_offset], ldaf, &anorm, rcond, &work[1], &iwork[1],
	     info);

/*     Compute the solution matrix X. */

    qlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    qgetrs_(trans, n, nrhs, &af[af_offset], ldaf, &ipiv[1], &x[x_offset], ldx,
	     info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    qgerfs_(trans, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
