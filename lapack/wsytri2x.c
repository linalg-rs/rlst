...
/*  ===================================================================== */
void  wsytri2x_(char *uplo, integer *n, quadcomplex *a, 
	integer *lda, integer *ipiv, quadcomplex *work, integer *nb, 
	integer *info)
{
    ...
    quadcomplex d__;
    integer i__, j, k;
    extern void  wsyswapr_(char *, integer *, quadcomplex *, 
    ...
    extern void  wtrmm_(char *, char *, char *, char *, 
	    integer *, integer *, quadcomplex *, quadcomplex *, integer *,
	     quadcomplex *, integer *);
    quadcomplex u01_i_j__, u11_i_j__;
    extern void  xerbla_(char *, integer *), wtrtri_(
	    char *, char *, integer *, quadcomplex *, integer *, integer *);
    quadcomplex u01_ip1_j__, u11_ip1_j__;
    extern void  wsyconv_(char *, char *, integer *, 
...
/*     Workspace got Non-diag elements of D */
    ...
    wsyconv_(uplo, "C", n, &a[a_offset], lda, &ipiv[1], &work[work_offset], &
	    iinfo);
...
/*        invA = P * inv(U**T)*inv(D)*inv(U)*P**T. */
	...
	wtrtri_(uplo, "U", n, &a[a_offset], lda, info);
...
/*       U11**T*invD1*U11->U11 */
	    ...
	    i__1 = *n + *nb + 1;
	    wtrmm_("L", "U", "T", "U", &nnb, &nnb, &c_b1, &a[cut + 1 + (cut + 
...
/*          U01**T*invD*U01->A(CUT+I,CUT+J) */
	    ...
	    i__1 = *n + *nb + 1;
	    i__2 = *n + *nb + 1;
	    wgemm_("T", "N", &nnb, &nnb, &cut, &c_b1, &a[(cut + 1) * a_dim1 + 
...
/*        U01 =  U00**T*invD0*U01 */
	    ...
	    i__1 = *n + *nb + 1;
	    wtrmm_("L", uplo, "T", "U", &cut, &nnb, &c_b1, &a[a_offset], lda, 
...
/*        Apply PERMUTATIONS P and P**T: P * inv(U**T)*inv(D)*inv(U) *P**T */
	...
	while(i__ <= *n) {
		...
		ip = ipiv[i__];
		if (i__ < ip) {
		    wsyswapr_(uplo, n, &a[a_offset], lda, &i__, &ip);
		}
		if (i__ > ip) {
		    wsyswapr_(uplo, n, &a[a_offset], lda, &ip, &i__);
		}
	    } else {
		ip = -ipiv[i__];
		++i__;
		if (i__ - 1 < ip) {
		    i__1 = i__ - 1;
		    wsyswapr_(uplo, n, &a[a_offset], lda, &i__1, &ip);
		}
		if (i__ - 1 > ip) {
		    i__1 = i__ - 1;
		    wsyswapr_(uplo, n, &a[a_offset], lda, &ip, &i__1);
		}
	    }
	    ++i__;
	}
...
/*       L11**T*invD1*L11->L11 */
	    ...
	    i__1 = *n + *nb + 1;
	    wtrmm_("L", uplo, "T", "U", &nnb, &nnb, &c_b1, &a[cut + 1 + (cut 
		    + 1) * a_dim1], lda, &work[u11 + 1 + work_dim1], &i__1);
...
/*          L21**T*invD2*L21->A(CUT+I,CUT+J) */
		...
		i__2 = *n + *nb + 1;
		i__3 = *n + *nb + 1;
		wgemm_("T", "N", &nnb, &nnb, &i__1, &c_b1, &a[cut + nnb + 1 + 
...
/*        U01 =  L22**T*invD2*L21 */
		...
		i__1 = *n - nnb - cut;
		i__2 = *n + *nb + 1;
		wtrmm_("L", uplo, "T", "U", &i__1, &nnb, &c_b1, &a[cut + nnb 
