...
/* > */
/*  ===================================================================== */
void  htrevc_(char *side, char *howmny, logical *select, 
	integer *n, halfreal *t, integer *ldt, halfreal *vl, integer *
	ldvl, halfreal *vr, integer *ldvr, integer *mm, integer *m, 
	halfreal *work, integer *info)
{
    ...
    /* Local variables */
    integer i__, j, k;
    halfreal x[4]	/* was [2][2] */;
    integer j1, j2, n2, ii, ki, ip, is;
    halfreal wi, wr, rec, ulp, beta, emax;
    logical pair;
    ...
    logical somev;
    halfreal xnorm;
    extern void  hlaln2_(logical *, integer *, integer *, 
...
/*                    1-by-1 diagonal block */
			...
			hlaln2_(&c_false, &c__1, &c__1, &smin, &c_b22, &t[j + 
...
/*                    Update right-hand side */
			...
			i__1 = j - 1;
			d__1 = -x[0];
			haxpy_(&i__1, &d__1, &t[j * t_dim1 + 1], &c__1, &work[
				*n + 1], &c__1);
...
/*                    2-by-2 diagonal block */
			...
			hlaln2_(&c_false, &c__2, &c__1, &smin, &c_b22, &t[j - 
...
/*                    Update right-hand side */
			...
			i__1 = j - 2;
			d__1 = -x[0];
			haxpy_(&i__1, &d__1, &t[(j - 1) * t_dim1 + 1], &c__1, 
				&work[*n + 1], &c__1);
			i__1 = j - 2;
			d__1 = -x[1];
			haxpy_(&i__1, &d__1, &t[j * t_dim1 + 1], &c__1, &work[
...
/*              Copy the vector x or Q*x to VR and normalize. */
		...
		if (! over) {
		    hcopy_(&ki, &work[*n + 1], &c__1, &vr[is * vr_dim1 + 1], &
			    c__1);

		    ii = ihamax_(&ki, &vr[is * vr_dim1 + 1], &c__1);
		    remax = 1. / (d__1 = vr[ii + is * vr_dim1], abs(d__1));
		    hscal_(&ki, &remax, &vr[is * vr_dim1 + 1], &c__1);
...
/* L70: */
		    ...
		    if (ki > 1) {
			i__1 = ki - 1;
			hgemv_("N", n, &i__1, &c_b22, &vr[vr_offset], ldvr, &
				work[*n + 1], &c__1, &work[ki + *n], &vr[ki * 
				vr_dim1 + 1], &c__1);
		    }

		    ii = ihamax_(n, &vr[ki * vr_dim1 + 1], &c__1);
		    remax = 1. / (d__1 = vr[ii + ki * vr_dim1], abs(d__1));
		    hscal_(n, &remax, &vr[ki * vr_dim1 + 1], &c__1);
		}
...
/*                    1-by-1 diagonal block */
			...
			hlaln2_(&c_false, &c__1, &c__2, &smin, &c_b22, &t[j + 
				j * t_dim1], ldt, &c_b22, &c_b22, &work[j + *
...
/*                    Scale if necessary */
			...
			if (scale != 1.) {
			    hscal_(&ki, &scale, &work[*n + 1], &c__1);
			    hscal_(&ki, &scale, &work[n2 + 1], &c__1);
			}
			work[j + *n] = x[0];
			work[j + n2] = x[2];
...
/*                    2-by-2 diagonal block */
			...
			hlaln2_(&c_false, &c__2, &c__2, &smin, &c_b22, &t[j - 
				1 + (j - 1) * t_dim1], ldt, &c_b22, &c_b22, &
...
/*              Copy the vector x or Q*x to VR and normalize. */
		...
		if (! over) {
		    hcopy_(&ki, &work[*n + 1], &c__1, &vr[(is - 1) * vr_dim1 
			    + 1], &c__1);
		    hcopy_(&ki, &work[n2 + 1], &c__1, &vr[is * vr_dim1 + 1], &
			    c__1);
...
/* L100: */
		    ...
		    remax = 1. / emax;
		    hscal_(&ki, &remax, &vr[(is - 1) * vr_dim1 + 1], &c__1);
		    hscal_(&ki, &remax, &vr[is * vr_dim1 + 1], &c__1);
...
/* L110: */
			...
			hscal_(n, &work[ki - 1 + *n], &vr[(ki - 1) * vr_dim1 
				+ 1], &c__1);
			hscal_(n, &work[ki + n2], &vr[ki * vr_dim1 + 1], &
				c__1);
		    }
...
/* L120: */
		    ...
		    remax = 1. / emax;
		    hscal_(n, &remax, &vr[(ki - 1) * vr_dim1 + 1], &c__1);
		    hscal_(n, &remax, &vr[ki * vr_dim1 + 1], &c__1);
		}
	    }
...
/*                    the right-hand side. */
			...
			if (work[j] > vcrit) {
			    rec = 1. / vmax;
			    i__3 = *n - ki + 1;
			    hscal_(&i__3, &rec, &work[ki + *n], &c__1);
			    vmax = 1.;
			    vcrit = bignum;
			}

			i__3 = j - ki - 1;
			work[j + *n] -= hdot_(&i__3, &t[ki + 1 + j * t_dim1], 
...
/*                    Scale if necessary */
			...
			if (scale != 1.) {
			    i__3 = *n - ki + 1;
			    hscal_(&i__3, &scale, &work[ki + *n], &c__1);
			}
			work[j + *n] = x[0];
/* Computing MAX */
...
/* Computing MAX */
			...
			i__3 = j - ki - 1;
			work[j + 1 + *n] -= hdot_(&i__3, &t[ki + 1 + (j + 1) *
...
/*                      [T(J+1,J)    T(J+1,J+1)-WR]                ( WORK2 ) */
			...
			hlaln2_(&c_true, &c__2, &c__1, &smin, &c_b22, &t[j + 
				j * t_dim1], ldt, &c_b22, &c_b22, &work[j + *
				n], n, &wr, &c_b25, x, &c__2, &scale, &xnorm, 
				&ierr);
...
/*              Copy the vector x or Q*x to VL and normalize. */
		...
		if (! over) {
		    i__2 = *n - ki + 1;
		    hcopy_(&i__2, &work[ki + *n], &c__1, &vl[ki + is * 
			    vl_dim1], &c__1);

		    i__2 = *n - ki + 1;
		    ii = ihamax_(&i__2, &vl[ki + is * vl_dim1], &c__1) + ki - 
			    1;
		    remax = 1. / (d__1 = vl[ii + is * vl_dim1], abs(d__1));
		    i__2 = *n - ki + 1;
		    hscal_(&i__2, &remax, &vl[ki + is * vl_dim1], &c__1);
...
/* L180: */
		    ...
		    if (ki < *n) {
			i__2 = *n - ki;
			hgemv_("N", n, &i__2, &c_b22, &vl[(ki + 1) * vl_dim1 
				+ 1], ldvl, &work[ki + 1 + *n], &c__1, &work[
				ki + *n], &vl[ki * vl_dim1 + 1], &c__1);
		    }

		    ii = ihamax_(n, &vl[ki * vl_dim1 + 1], &c__1);
		    remax = 1. / (d__1 = vl[ii + ki * vl_dim1], abs(d__1));
		    hscal_(n, &remax, &vl[ki * vl_dim1 + 1], &c__1);
...
/*                    forming the right-hand side elements. */
			...
			if (work[j] > vcrit) {
			    ...
			    hscal_(&i__3, &rec, &work[ki + *n], &c__1);
			    i__3 = *n - ki + 1;
			    hscal_(&i__3, &rec, &work[ki + n2], &c__1);
			    vmax = 1.;
			    vcrit = bignum;
			}

			i__3 = j - ki - 2;
			work[j + *n] -= hdot_(&i__3, &t[ki + 2 + j * t_dim1], 
				&c__1, &work[ki + 2 + *n], &c__1);
			i__3 = j - ki - 2;
			work[j + n2] -= hdot_(&i__3, &t[ki + 2 + j * t_dim1], 
...
/*                    Scale if necessary */
			...
			if (scale != 1.) {
			    ...
			    hscal_(&i__3, &scale, &work[ki + *n], &c__1);
			    i__3 = *n - ki + 1;
			    hscal_(&i__3, &scale, &work[ki + n2], &c__1);
			}
			work[j + *n] = x[0];
			work[j + n2] = x[2];
/* Computing MAX */
...
/* Computing MAX */
			...
			i__3 = j - ki - 2;
			work[j + 1 + *n] -= hdot_(&i__3, &t[ki + 2 + (j + 1) *
				 t_dim1], &c__1, &work[ki + 2 + *n], &c__1);

			i__3 = j - ki - 2;
			work[j + 1 + n2] -= hdot_(&i__3, &t[ki + 2 + (j + 1) *
...
/*                      ([T(j+1,j) T(j+1,j+1)]               ) */
			...
			d__1 = -wi;
			hlaln2_(&c_true, &c__2, &c__2, &smin, &c_b22, &t[j + 
				j * t_dim1], ldt, &c_b22, &c_b22, &work[j + *
				n], n, &wr, &d__1, x, &c__2, &scale, &xnorm, &
				ierr);
...
/*              Copy the vector x or Q*x to VL and normalize. */
		...
		if (! over) {
		    ...
		    hcopy_(&i__2, &work[ki + *n], &c__1, &vl[ki + is * 
			    vl_dim1], &c__1);
		    i__2 = *n - ki + 1;
		    hcopy_(&i__2, &work[ki + n2], &c__1, &vl[ki + (is + 1) * 
			    vl_dim1], &c__1);
...
/* L220: */
		    ...
		    hscal_(&i__2, &remax, &vl[ki + is * vl_dim1], &c__1);
		    i__2 = *n - ki + 1;
		    hscal_(&i__2, &remax, &vl[ki + (is + 1) * vl_dim1], &c__1)
			    ;
...
/* L230: */
		    ...
		    if (ki < *n - 1) {
			i__2 = *n - ki - 1;
			hgemv_("N", n, &i__2, &c_b22, &vl[(ki + 2) * vl_dim1 
			...
			hscal_(n, &work[ki + *n], &vl[ki * vl_dim1 + 1], &
				c__1);
			hscal_(n, &work[ki + 1 + n2], &vl[(ki + 1) * vl_dim1 
				+ 1], &c__1);
		    }
...
/* L240: */
		    ...
		    remax = 1. / emax;
		    hscal_(n, &remax, &vl[ki * vl_dim1 + 1], &c__1);
		    hscal_(n, &remax, &vl[(ki + 1) * vl_dim1 + 1], &c__1);
