...
/*  ===================================================================== */
void  wbbcsd_(char *jobu1, char *jobu2, char *jobv1t, char *
	jobv2t, char *trans, integer *m, integer *p, integer *q, quadreal *
	theta, quadreal *phi, quadcomplex *u1, integer *ldu1, 
	quadcomplex *u2, integer *ldu2, quadcomplex *v1t, integer *ldv1t, 
	quadcomplex *v2t, integer *ldv2t, quadreal *b11d, quadreal *
	b11e, quadreal *b12d, quadreal *b12e, quadreal *b21d, 
	quadreal *b21e, quadreal *b22d, quadreal *b22e, quadreal *
	rwork, integer *lrwork, integer *info)
{
    ...
    integer maxit;
    quadreal dummy;
    extern void  wlasr_(char *, char *, char *, integer *, 
	    integer *, quadreal *, quadreal *, quadcomplex *, integer *), wswap_(integer *, quadcomplex *, 
	    integer *, quadcomplex *, integer *);
    integer iv1tcs, iv2tcs;
    logical wantu1, wantu2;
    integer iv1tsn, iv2tsn;
    extern quadreal qlamch_(char *);
    quadreal sigma11, sigma21;
    extern void  xerbla_(char *, integer *);
    quadreal thresh, tolmul;
    extern void  mecago_();
    logical lquery;
    quadreal b11bulge, b12bulge;
    logical wantv1t, wantv2t;
    quadreal b21bulge, b22bulge;
    extern void  qlartgp_(quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *), qlartgs_(quadreal *, 
...
/*        Compute the matrix entries */
	...
	b11d[imin] = M(cos)(theta[imin]);
	b21d[imin] = -M(sin)(theta[imin]);
	i__1 = imax - 1;
	for (i__ = imin; i__ <= i__1; ++i__) {
	    b11e[i__] = -M(sin)(theta[i__]) * M(sin)(phi[i__]);
	    b11d[i__ + 1] = M(cos)(theta[i__ + 1]) * M(cos)(phi[i__]);
	    b12d[i__] = M(sin)(theta[i__]) * M(cos)(phi[i__]);
	    b12e[i__] = M(cos)(theta[i__ + 1]) * M(sin)(phi[i__]);
	    b21e[i__] = -M(cos)(theta[i__]) * M(sin)(phi[i__]);
	    b21d[i__ + 1] = -M(sin)(theta[i__ + 1]) * M(cos)(phi[i__]);
	    b22d[i__] = M(cos)(theta[i__]) * M(cos)(phi[i__]);
	    b22e[i__] = -M(sin)(theta[i__ + 1]) * M(sin)(phi[i__]);
	}
	b12d[imax] = M(sin)(theta[imax]);
	b22d[imax] = M(cos)(theta[imax]);
...
/*           Compute shifts for B11 and B21 and use the lesser */
	    ...
	    qlas2_(&b11d[imax - 1], &b11e[imax - 1], &b11d[imax], &sigma11, &
		    dummy);
	    qlas2_(&b21d[imax - 1], &b21e[imax - 1], &b21d[imax], &sigma21, &
		    dummy);
...
/*        Rotate to produce bulges in B11 and B21 */
	...
	if (mu <= nu) {
	    qlartgs_(&b11d[imin], &b11e[imin], &mu, &rwork[iv1tcs + imin - 1],
		     &rwork[iv1tsn + imin - 1]);
	} else {
	    qlartgs_(&b21d[imin], &b21e[imin], &nu, &rwork[iv1tcs + imin - 1],
		     &rwork[iv1tsn + imin - 1]);
	}
...
/* Computing 2nd power */
	d__4 = b11bulge;
	theta[imin] = atan2(M(sqrt)(d__1 * d__1 + d__2 * d__2), M(sqrt)(d__3 * d__3 
		+ d__4 * d__4));
...
/* Computing 2nd power */
	...
	if (d__1 * d__1 + d__2 * d__2 > d__3 * d__3) {
	    qlartgp_(&b11bulge, &b11d[imin], &rwork[iu1sn + imin - 1], &rwork[
		    iu1cs + imin - 1], &r__);
	} else if (mu <= nu) {
	    qlartgs_(&b11e[imin], &b11d[imin + 1], &mu, &rwork[iu1cs + imin - 
		    1], &rwork[iu1sn + imin - 1]);
	} else {
	    qlartgs_(&b12d[imin], &b12e[imin], &nu, &rwork[iu1cs + imin - 1], 
		    &rwork[iu1sn + imin - 1]);
	}
/* Computing 2nd power */
	d__1 = b21d[imin];
...
/* Computing 2nd power */
	d__3 = thresh;
	if (d__1 * d__1 + d__2 * d__2 > d__3 * d__3) {
	    qlartgp_(&b21bulge, &b21d[imin], &rwork[iu2sn + imin - 1], &rwork[
		    iu2cs + imin - 1], &r__);
	} else if (nu < mu) {
	    qlartgs_(&b21e[imin], &b21d[imin + 1], &nu, &rwork[iu2cs + imin - 
		    1], &rwork[iu2sn + imin - 1]);
	} else {
	    qlartgs_(&b22d[imin], &b22e[imin], &mu, &rwork[iu2cs + imin - 1], 
		    &rwork[iu2sn + imin - 1]);
	}
...
/*           Compute PHI(I-1) */
	    ...
	    x1 = M(sin)(theta[i__ - 1]) * b11e[i__ - 1] + M(cos)(theta[i__ - 1]) * 
		    b21e[i__ - 1];
	    x2 = M(sin)(theta[i__ - 1]) * b11bulge + M(cos)(theta[i__ - 1]) * 
		    b21bulge;
	    y1 = M(sin)(theta[i__ - 1]) * b12d[i__ - 1] + M(cos)(theta[i__ - 1]) * 
		    b22d[i__ - 1];
	    y2 = M(sin)(theta[i__ - 1]) * b12bulge + M(cos)(theta[i__ - 1]) * 
		    b22bulge;
...
/* Computing 2nd power */
	    d__4 = y2;
	    phi[i__ - 1] = atan2(M(sqrt)(d__1 * d__1 + d__2 * d__2), M(sqrt)(d__3 * 
		    d__3 + d__4 * d__4));
...
/*           chasing by applying the original shift again. */
		...
		qlartgp_(&x2, &x1, &rwork[iv1tsn + i__ - 1], &rwork[iv1tcs + 
			i__ - 1], &r__);
	    } else if (! restart11 && restart21) {
		qlartgp_(&b11bulge, &b11e[i__ - 1], &rwork[iv1tsn + i__ - 1], 
			&rwork[iv1tcs + i__ - 1], &r__);
	    } else if (restart11 && ! restart21) {
		qlartgp_(&b21bulge, &b21e[i__ - 1], &rwork[iv1tsn + i__ - 1], 
			&rwork[iv1tcs + i__ - 1], &r__);
	    } else if (mu <= nu) {
		qlartgs_(&b11d[i__], &b11e[i__], &mu, &rwork[iv1tcs + i__ - 1]
			, &rwork[iv1tsn + i__ - 1]);
	    } else {
		qlartgs_(&b21d[i__], &b21e[i__], &nu, &rwork[iv1tcs + i__ - 1]
			, &rwork[iv1tsn + i__ - 1]);
	    }
	    rwork[iv1tcs + i__ - 1] = -rwork[iv1tcs + i__ - 1];
	    rwork[iv1tsn + i__ - 1] = -rwork[iv1tsn + i__ - 1];
	    if (! restart12 && ! restart22) {
		qlartgp_(&y2, &y1, &rwork[iv2tsn + i__ - 2], &rwork[iv2tcs + 
			i__ - 2], &r__);
	    } else if (! restart12 && restart22) {
		qlartgp_(&b12bulge, &b12d[i__ - 1], &rwork[iv2tsn + i__ - 2], 
			&rwork[iv2tcs + i__ - 2], &r__);
	    } else if (restart12 && ! restart22) {
		qlartgp_(&b22bulge, &b22d[i__ - 1], &rwork[iv2tsn + i__ - 2], 
			&rwork[iv2tcs + i__ - 2], &r__);
	    } else if (nu < mu) {
		qlartgs_(&b12e[i__ - 1], &b12d[i__], &nu, &rwork[iv2tcs + i__ 
			- 2], &rwork[iv2tsn + i__ - 2]);
	    } else {
		qlartgs_(&b22e[i__ - 1], &b22d[i__], &mu, &rwork[iv2tcs + i__ 
			- 2], &rwork[iv2tsn + i__ - 2]);
	    }
...
/*           Compute THETA(I) */
	    ...
	    x1 = M(cos)(phi[i__ - 1]) * b11d[i__] + M(sin)(phi[i__ - 1]) * b12e[i__ 
		    - 1];
	    x2 = M(cos)(phi[i__ - 1]) * b11bulge + M(sin)(phi[i__ - 1]) * b12bulge;
	    y1 = M(cos)(phi[i__ - 1]) * b21d[i__] + M(sin)(phi[i__ - 1]) * b22e[i__ 
		    - 1];
	    y2 = M(cos)(phi[i__ - 1]) * b21bulge + M(sin)(phi[i__ - 1]) * b22bulge;
...
/* Computing 2nd power */
	    d__4 = x2;
	    theta[i__] = atan2(M(sqrt)(d__1 * d__1 + d__2 * d__2), M(sqrt)(d__3 * 
		    d__3 + d__4 * d__4));
...
/*           chasing by applying the original shift again. */
		...
		qlartgp_(&x2, &x1, &rwork[iu1sn + i__ - 1], &rwork[iu1cs + 
			i__ - 1], &r__);
	    } else if (! restart11 && restart12) {
		qlartgp_(&b11bulge, &b11d[i__], &rwork[iu1sn + i__ - 1], &
			rwork[iu1cs + i__ - 1], &r__);
	    } else if (restart11 && ! restart12) {
		qlartgp_(&b12bulge, &b12e[i__ - 1], &rwork[iu1sn + i__ - 1], &
			rwork[iu1cs + i__ - 1], &r__);
	    } else if (mu <= nu) {
		qlartgs_(&b11e[i__], &b11d[i__ + 1], &mu, &rwork[iu1cs + i__ 
			- 1], &rwork[iu1sn + i__ - 1]);
	    } else {
		qlartgs_(&b12d[i__], &b12e[i__], &nu, &rwork[iu1cs + i__ - 1],
			 &rwork[iu1sn + i__ - 1]);
	    }
	    if (! restart21 && ! restart22) {
		qlartgp_(&y2, &y1, &rwork[iu2sn + i__ - 1], &rwork[iu2cs + 
			i__ - 1], &r__);
	    } else if (! restart21 && restart22) {
		qlartgp_(&b21bulge, &b21d[i__], &rwork[iu2sn + i__ - 1], &
			rwork[iu2cs + i__ - 1], &r__);
	    } else if (restart21 && ! restart22) {
		qlartgp_(&b22bulge, &b22e[i__ - 1], &rwork[iu2sn + i__ - 1], &
			rwork[iu2cs + i__ - 1], &r__);
	    } else if (nu < mu) {
		qlartgs_(&b21e[i__], &b21e[i__ + 1], &nu, &rwork[iu2cs + i__ 
			- 1], &rwork[iu2sn + i__ - 1]);
	    } else {
		qlartgs_(&b22d[i__], &b22e[i__], &mu, &rwork[iu2cs + i__ - 1],
			 &rwork[iu2sn + i__ - 1]);
	    }
...
/*        Compute PHI(IMAX-1) */
	...
	x1 = M(sin)(theta[imax - 1]) * b11e[imax - 1] + M(cos)(theta[imax - 1]) * 
		b21e[imax - 1];
	y1 = M(sin)(theta[imax - 1]) * b12d[imax - 1] + M(cos)(theta[imax - 1]) * 
		b22d[imax - 1];
	y2 = M(sin)(theta[imax - 1]) * b12bulge + M(cos)(theta[imax - 1]) * 
		b22bulge;
...
/* Computing 2nd power */
	d__2 = y2;
	phi[imax - 1] = atan2((abs(x1)), M(sqrt)(d__1 * d__1 + d__2 * d__2));
...
/* Computing 2nd power */
	...
	if (! restart12 && ! restart22) {
	    qlartgp_(&y2, &y1, &rwork[iv2tsn + imax - 2], &rwork[iv2tcs + 
		    imax - 2], &r__);
	} else if (! restart12 && restart22) {
	    qlartgp_(&b12bulge, &b12d[imax - 1], &rwork[iv2tsn + imax - 2], &
		    rwork[iv2tcs + imax - 2], &r__);
	} else if (restart12 && ! restart22) {
	    qlartgp_(&b22bulge, &b22d[imax - 1], &rwork[iv2tsn + imax - 2], &
		    rwork[iv2tcs + imax - 2], &r__);
	} else if (nu < mu) {
	    qlartgs_(&b12e[imax - 1], &b12d[imax], &nu, &rwork[iv2tcs + imax 
		    - 2], &rwork[iv2tsn + imax - 2]);
	} else {
	    qlartgs_(&b22e[imax - 1], &b22d[imax], &mu, &rwork[iv2tcs + imax 
		    - 2], &rwork[iv2tsn + imax - 2]);
	}
...
/*        Update singular vectors */
	...
	if (wantu1) {
	    if (colmajor) {
		i__1 = imax - imin + 1;
		wlasr_("R", "V", "F", p, &i__1, &rwork[iu1cs + imin - 1], &
			rwork[iu1sn + imin - 1], &u1[imin * u1_dim1 + 1], 
			ldu1);
	    } else {
		i__1 = imax - imin + 1;
		wlasr_("L", "V", "F", &i__1, p, &rwork[iu1cs + imin - 1], &
	...
	if (wantu2) {
		...
		i__1 = *m - *p;
		i__2 = imax - imin + 1;
		wlasr_("R", "V", "F", &i__1, &i__2, &rwork[iu2cs + imin - 1], 
			&rwork[iu2sn + imin - 1], &u2[imin * u2_dim1 + 1], 
			ldu2);
	    } else {
		i__1 = imax - imin + 1;
		i__2 = *m - *p;
		wlasr_("L", "V", "F", &i__1, &i__2, &rwork[iu2cs + imin - 1], 
	...
	if (wantv1t) {
	    if (colmajor) {
		i__1 = imax - imin + 1;
		wlasr_("L", "V", "F", &i__1, q, &rwork[iv1tcs + imin - 1], &
			rwork[iv1tsn + imin - 1], &v1t[imin + v1t_dim1], 
			ldv1t);
	    } else {
		i__1 = imax - imin + 1;
		wlasr_("R", "V", "F", q, &i__1, &rwork[iv1tcs + imin - 1], &
	...
	if (wantv2t) {
		...
		i__1 = imax - imin + 1;
		i__2 = *m - *q;
		wlasr_("L", "V", "F", &i__1, &i__2, &rwork[iv2tcs + imin - 1],
			 &rwork[iv2tsn + imin - 1], &v2t[imin + v2t_dim1], 
			ldv2t);
	    } else {
		i__1 = *m - *q;
		i__2 = imax - imin + 1;
		wlasr_("R", "V", "F", &i__1, &i__2, &rwork[iv2tcs + imin - 1],
...
/*        Fix signs on B11(IMAX-1,IMAX) and B21(IMAX-1,IMAX) */
	...
	if (b11e[imax - 1] + b21e[imax - 1] > 0.) {
	    ...
	    if (wantv1t) {
		if (colmajor) {
		    wscal_(q, &c_b1, &v1t[imax + v1t_dim1], ldv1t);
		} else {
		    wscal_(q, &c_b1, &v1t[imax * v1t_dim1 + 1], &c__1);
		}
	    }
	}
...
/*        Compute THETA(IMAX) */
	...
	x1 = M(cos)(phi[imax - 1]) * b11d[imax] + M(sin)(phi[imax - 1]) * b12e[imax 
		- 1];
	y1 = M(cos)(phi[imax - 1]) * b21d[imax] + M(sin)(phi[imax - 1]) * b22e[imax 
		- 1];
...
/*        and B22(IMAX,IMAX-1) */
	...
	if (b11d[imax] + b12e[imax - 1] < 0.) {
	    ...
	    if (wantu1) {
		if (colmajor) {
		    wscal_(p, &c_b1, &u1[imax * u1_dim1 + 1], &c__1);
		} else {
		    wscal_(p, &c_b1, &u1[imax + u1_dim1], ldu1);
		}
	    }
	}
	if (b21d[imax] + b22e[imax - 1] > 0.) {
		...
		if (colmajor) {
		    i__1 = *m - *p;
		    wscal_(&i__1, &c_b1, &u2[imax * u2_dim1 + 1], &c__1);
		} else {
		    i__1 = *m - *p;
		    wscal_(&i__1, &c_b1, &u2[imax + u2_dim1], ldu2);
		}
	    }
	}
...
/*        Fix signs on B12(IMAX,IMAX) and B22(IMAX,IMAX) */
	...
	if (b12d[imax] + b22d[imax] < 0.) {
		...
		if (colmajor) {
		    i__1 = *m - *q;
		    wscal_(&i__1, &c_b1, &v2t[imax + v2t_dim1], ldv2t);
		} else {
		    i__1 = *m - *q;
		    wscal_(&i__1, &c_b1, &v2t[imax * v2t_dim1 + 1], &c__1);
		}
	    }
	}
...
/*     Postprocessing: order THETA from least to greatest */
	...
	if (mini != i__) {
	    ...
	    if (colmajor) {
		if (wantu1) {
		    wswap_(p, &u1[i__ * u1_dim1 + 1], &c__1, &u1[mini * 
			    u1_dim1 + 1], &c__1);
		}
		if (wantu2) {
		    i__2 = *m - *p;
		    wswap_(&i__2, &u2[i__ * u2_dim1 + 1], &c__1, &u2[mini * 
			    u2_dim1 + 1], &c__1);
		}
		if (wantv1t) {
		    wswap_(q, &v1t[i__ + v1t_dim1], ldv1t, &v1t[mini + 
			    v1t_dim1], ldv1t);
		}
		if (wantv2t) {
		    i__2 = *m - *q;
		    wswap_(&i__2, &v2t[i__ + v2t_dim1], ldv2t, &v2t[mini + 
			    v2t_dim1], ldv2t);
		}
	    } else {
		if (wantu1) {
		    wswap_(p, &u1[i__ + u1_dim1], ldu1, &u1[mini + u1_dim1], 
			    ldu1);
		}
		if (wantu2) {
		    i__2 = *m - *p;
		    wswap_(&i__2, &u2[i__ + u2_dim1], ldu2, &u2[mini + 
			    u2_dim1], ldu2);
		}
		if (wantv1t) {
		    wswap_(q, &v1t[i__ * v1t_dim1 + 1], &c__1, &v1t[mini * 
			    v1t_dim1 + 1], &c__1);
		}
		if (wantv2t) {
		    i__2 = *m - *q;
		    wswap_(&i__2, &v2t[i__ * v2t_dim1 + 1], &c__1, &v2t[mini *
			     v2t_dim1 + 1], &c__1);
		}
	    }
	}
