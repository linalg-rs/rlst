...
/* > \verbatim */
/* > */
/* >     DGEMLQ overwrites the general doublereal M-by-N matrix C with */
/* > */
/* >                    SIDE = 'L'     SIDE = 'R' */
/* >    TRANS = 'N':      Q * C          C * Q */
/* >    TRANS = 'T':      Q**T * C       C * Q**T */
...
/* > */
/*  ===================================================================== */
void  hgemlq_(char *side, char *trans, integer *m, integer *n, 
	...
	lwork, integer *info)
{
    ...
    /* Local variables */
    extern void  hlamswlq_(char *, char *, integer *, integer *
...
/* Computing MAX */
    ...
    if (left && *m <= *k || right && *n <= *k || nb <= *k || nb >= f2cmax(i__1,*
	    k)) {
	hgemlqt_(side, trans, m, n, k, &mb, &a[a_offset], lda, &t[6], &mb, &
		c__[c_offset], ldc, &work[1], info);
    } else {
	hlamswlq_(side, trans, m, n, k, &mb, &nb, &a[a_offset], lda, &t[6], &
