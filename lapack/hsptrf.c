...
/* > \verbatim */
/* > */
/* > DSPTRF computes the factorization of a doublereal symmetric matrix A stored */
...
/* > */
/*  ===================================================================== */
void  hsptrf_(char *uplo, integer *n, halfreal *ap, integer *
	ipiv, integer *info)
{
...
/*              submatrix A(1:k,1:k) */
		...
		i__1 = kp - 1;
		hswap_(&i__1, &ap[knc], &c__1, &ap[kpc], &c__1);
		kx = kpc + kp - 1;
		i__1 = kk - 1;
		for (j = kp + 1; j <= i__1; ++j) {
		    kx = kx + j - 1;
...
/*              A := A - U(k)*D(k)*U(k)**T = A - W(k)*1/D(k)*W(k)**T */
		...
		i__1 = k - 1;
		d__1 = -r1;
		hspr_(uplo, &i__1, &d__1, &ap[kc], &c__1, &ap[1]);

/*              Store U(k) in column k */

		i__1 = k - 1;
		hscal_(&i__1, &r1, &ap[kc], &c__1);
	    } else {
...
/*        column K, and COLMAX is its absolute value */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    imax = k + ihamax_(&i__1, &ap[kc + 1], &c__1);
...
/* L70: */
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    jmax = imax + ihamax_(&i__1, &ap[kpc + 1], &c__1);
/* Computing MAX */
...
/*              submatrix A(k:n,k:n) */
		...
		if (kp < *n) {
		    i__1 = *n - kp;
		    hswap_(&i__1, &ap[knc + kp - kk + 1], &c__1, &ap[kpc + 1],
			     &c__1);
		}
		kx = knc + kp - kk;
		i__1 = kp - 1;
...
/*                 A := A - L(k)*D(k)*L(k)**T = A - W(k)*(1/D(k))*W(k)**T */
		    ...
		    i__1 = *n - k;
		    d__1 = -r1;
		    hspr_(uplo, &i__1, &d__1, &ap[kc + 1], &c__1, &ap[kc + *n 
			    - k + 1]);

/*                 Store L(k) in column K */

		    i__1 = *n - k;
		    hscal_(&i__1, &r1, &ap[kc + 1], &c__1);
		}
	    } else {
