...
/* > N-by-N and M-by-N, respectively, with doublereal entries. (A, D) and (B, E) */
...
/*  ===================================================================== */
void  htgsy2_(char *trans, integer *ijob, integer *m, integer *
	n, halfreal *a, integer *lda, halfreal *b, integer *ldb, 
	halfreal *c__, integer *ldc, halfreal *d__, integer *ldd, 
	halfreal *e, integer *lde, halfreal *f, integer *ldf, halfreal *
	scale, halfreal *rdsum, halfreal *rdscal, integer *iwork, integer 
	*pq, integer *info)
{
    ...
    extern void  hgemv_(char *, integer *, integer *, 
	    halfreal *, halfreal *, integer *, halfreal *, integer *, 
	    halfreal *, halfreal *, integer *), hcopy_(integer *, 
	    halfreal *, integer *, halfreal *, integer *), haxpy_(integer 
	    *, halfreal *, halfreal *, integer *, halfreal *, integer *)
	    , hgesc2_(integer *, halfreal *, integer *, halfreal *, 
	    integer *, integer *, halfreal *), hgetc2_(integer *, 
	    halfreal *, integer *, integer *, integer *, integer *), 
	    hlatdf_(integer *, integer *, halfreal *, integer *, halfreal 
...
/*                 Solve Z * x = RHS */
		    ...
		    hgetc2_(&zdim, z__, &c__8, ipiv, jpiv, &ierr);
		    if (ierr > 0) {
			*info = ierr;
		    }

		    if (*ijob == 0) {
			hgesc2_(&zdim, z__, &c__8, rhs, ipiv, jpiv, &scaloc);
			if (scaloc != 1.) {
			    i__2 = *n;
			    for (k = 1; k <= i__2; ++k) {
				hscal_(m, &scaloc, &c__[k * c_dim1 + 1], &
					c__1);
				hscal_(m, &scaloc, &f[k * f_dim1 + 1], &c__1);
/* L50: */
			    }
			    *scale *= scaloc;
			}
		    } else {
			hlatdf_(ijob, &zdim, z__, &c__8, rhs, rdsum, rdscal, 
				ipiv, jpiv);
		    }
...
/*                 equation. */
			...
			alpha = -rhs[0];
			i__2 = is - 1;
			haxpy_(&i__2, &alpha, &a[is * a_dim1 + 1], &c__1, &
				c__[js * c_dim1 + 1], &c__1);
			i__2 = is - 1;
			haxpy_(&i__2, &alpha, &d__[is * d_dim1 + 1], &c__1, &
				f[js * f_dim1 + 1], &c__1);
		    }
		    if (j < q) {
			i__2 = *n - je;
			haxpy_(&i__2, &rhs[1], &b[js + (je + 1) * b_dim1], 
				ldb, &c__[is + (je + 1) * c_dim1], ldc);
			i__2 = *n - je;
			haxpy_(&i__2, &rhs[1], &e[js + (je + 1) * e_dim1], 
...
/*                 equation. */
		    ...
		    if (i__ > 1) {
			i__2 = is - 1;
			hger_(&i__2, &nb, &c_b27, &a[is * a_dim1 + 1], &c__1, 
				rhs, &c__1, &c__[js * c_dim1 + 1], ldc);
			i__2 = is - 1;
			hger_(&i__2, &nb, &c_b27, &d__[is * d_dim1 + 1], &
				c__1, rhs, &c__1, &f[js * f_dim1 + 1], ldf);
		    }
		    if (j < q) {
			i__2 = *n - je;
			haxpy_(&i__2, &rhs[2], &b[js + (je + 1) * b_dim1], 
				ldb, &c__[is + (je + 1) * c_dim1], ldc);
			i__2 = *n - je;
			haxpy_(&i__2, &rhs[2], &e[js + (je + 1) * e_dim1], 
				lde, &f[is + (je + 1) * f_dim1], ldf);
			i__2 = *n - je;
			haxpy_(&i__2, &rhs[3], &b[jsp1 + (je + 1) * b_dim1], 
				ldb, &c__[is + (je + 1) * c_dim1], ldc);
			i__2 = *n - je;
			haxpy_(&i__2, &rhs[3], &e[jsp1 + (je + 1) * e_dim1], 
				lde, &f[is + (je + 1) * f_dim1], ldf);
		    }
...
/*                 equation. */
		    ...
		    if (i__ > 1) {
			i__2 = is - 1;
			hgemv_("N", &i__2, &mb, &c_b27, &a[is * a_dim1 + 1], 
				lda, rhs, &c__1, &c_b42, &c__[js * c_dim1 + 1]
				, &c__1);
			i__2 = is - 1;
			hgemv_("N", &i__2, &mb, &c_b27, &d__[is * d_dim1 + 1],
				 ldd, rhs, &c__1, &c_b42, &f[js * f_dim1 + 1],
				 &c__1);
		    }
		    if (j < q) {
			i__2 = *n - je;
			hger_(&mb, &i__2, &c_b42, &rhs[2], &c__1, &b[js + (je 
				+ 1) * b_dim1], ldb, &c__[is + (je + 1) * 
				c_dim1], ldc);
			i__2 = *n - je;
			hger_(&mb, &i__2, &c_b42, &rhs[2], &c__1, &e[js + (je 
...
/*                 Build an 8-by-8 system Z * x = RHS */
		    ...
		    hlaset_("F", &c__8, &c__8, &c_b56, &c_b56, z__, &c__8);
...
/*                 Set up right hand side(s) */
		    ...
		    i__2 = nb - 1;
		    for (jj = 0; jj <= i__2; ++jj) {
			hcopy_(&mb, &c__[is + (js + jj) * c_dim1], &c__1, &
				rhs[k - 1], &c__1);
			hcopy_(&mb, &f[is + (js + jj) * f_dim1], &c__1, &rhs[
				ii - 1], &c__1);
			k += mb;
			ii += mb;
/* L80: */
...
/*                 Unpack solution vector(s) */
		    ...
		    i__2 = nb - 1;
		    for (jj = 0; jj <= i__2; ++jj) {
			hcopy_(&mb, &rhs[k - 1], &c__1, &c__[is + (js + jj) * 
				c_dim1], &c__1);
			hcopy_(&mb, &rhs[ii - 1], &c__1, &f[is + (js + jj) * 
				f_dim1], &c__1);
			k += mb;
			ii += mb;
/* L100: */
...
/*                 equation. */
		    ...
		    if (i__ > 1) {
			i__2 = is - 1;
			hgemm_("N", "N", &i__2, &nb, &mb, &c_b27, &a[is * 
				a_dim1 + 1], lda, rhs, &mb, &c_b42, &c__[js * 
				c_dim1 + 1], ldc);
			i__2 = is - 1;
			hgemm_("N", "N", &i__2, &nb, &mb, &c_b27, &d__[is * 
			...
			k = mb * nb + 1;
			i__2 = *n - je;
			hgemm_("N", "N", &mb, &i__2, &nb, &c_b42, &rhs[k - 1],
...
/*                 Solve Z**T * x = RHS */
			...
			*info = ierr;
			...
			i__3 = *n;
			for (k = 1; k <= i__3; ++k) {
			    hscal_(m, &scaloc, &c__[k * c_dim1 + 1], &c__1);
			    hscal_(m, &scaloc, &f[k * f_dim1 + 1], &c__1);
/* L130: */
			}
			*scale *= scaloc;
...
/*                 equation. */
			...
			alpha = rhs[0];
			i__3 = js - 1;
			haxpy_(&i__3, &alpha, &b[js * b_dim1 + 1], &c__1, &f[
				is + f_dim1], ldf);
			alpha = rhs[1];
			i__3 = js - 1;
			haxpy_(&i__3, &alpha, &e[js * e_dim1 + 1], &c__1, &f[
				is + f_dim1], ldf);
		    }
		    if (i__ < p) {
			alpha = -rhs[0];
			i__3 = *m - ie;
			haxpy_(&i__3, &alpha, &a[is + (ie + 1) * a_dim1], lda,
				 &c__[ie + 1 + js * c_dim1], &c__1);
			alpha = -rhs[1];
			i__3 = *m - ie;
			haxpy_(&i__3, &alpha, &d__[is + (ie + 1) * d_dim1], 
...
/*                 equation. */
		    ...
		    if (j > p + 2) {
			i__3 = js - 1;
			haxpy_(&i__3, rhs, &b[js * b_dim1 + 1], &c__1, &f[is 
				+ f_dim1], ldf);
			i__3 = js - 1;
			haxpy_(&i__3, &rhs[1], &b[jsp1 * b_dim1 + 1], &c__1, &
				f[is + f_dim1], ldf);
			i__3 = js - 1;
			haxpy_(&i__3, &rhs[2], &e[js * e_dim1 + 1], &c__1, &f[
				is + f_dim1], ldf);
			i__3 = js - 1;
			haxpy_(&i__3, &rhs[3], &e[jsp1 * e_dim1 + 1], &c__1, &
				f[is + f_dim1], ldf);
		    }
		    if (i__ < p) {
			i__3 = *m - ie;
			hger_(&i__3, &nb, &c_b27, &a[is + (ie + 1) * a_dim1], 
				lda, rhs, &c__1, &c__[ie + 1 + js * c_dim1], 
				ldc);
			i__3 = *m - ie;
			hger_(&i__3, &nb, &c_b27, &d__[is + (ie + 1) * d_dim1]
...
/*                 equation. */
		    ...
		    if (j > p + 2) {
			i__3 = js - 1;
			hger_(&mb, &i__3, &c_b42, rhs, &c__1, &b[js * b_dim1 
				+ 1], &c__1, &f[is + f_dim1], ldf);
			i__3 = js - 1;
			hger_(&mb, &i__3, &c_b42, &rhs[2], &c__1, &e[js * 
				e_dim1 + 1], &c__1, &f[is + f_dim1], ldf);
		    }
		    if (i__ < p) {
			i__3 = *m - ie;
			hgemv_("T", &mb, &i__3, &c_b27, &a[is + (ie + 1) * 
				a_dim1], lda, rhs, &c__1, &c_b42, &c__[ie + 1 
				+ js * c_dim1], &c__1);
			i__3 = *m - ie;
			hgemv_("T", &mb, &i__3, &c_b27, &d__[is + (ie + 1) * 
