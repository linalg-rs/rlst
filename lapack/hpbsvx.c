...
/* > */
/*  ===================================================================== */
void  hpbsvx_(char *fact, char *uplo, integer *n, integer *kd, 
	integer *nrhs, halfreal *ab, integer *ldab, halfreal *afb, 
	integer *ldafb, char *equed, halfreal *s, halfreal *b, integer *
	ldb, halfreal *x, integer *ldx, halfreal *rcond, halfreal *ferr,
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	hpbequ_(uplo, n, kd, &ab[ab_offset], ldab, &s[1], &scond, &amax, &
		infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    hlaqsb_(uplo, n, kd, &ab[ab_offset], ldab, &s[1], &scond, &amax, 
		    equed);
	    rcequ = lsame_(equed, "Y");
	}
    }
...
/* Computing MAX */
		...
		j1 = f2cmax(i__2,1);
		i__2 = j - j1 + 1;
		hcopy_(&i__2, &ab[*kd + 1 - j + j1 + j * ab_dim1], &c__1, &
...
/* Computing MIN */
		...
		j2 = f2cmin(i__2,*n);
		i__2 = j2 - j + 1;
		hcopy_(&i__2, &ab[j * ab_dim1 + 1], &c__1, &afb[j * afb_dim1 
			+ 1], &c__1);
/* L50: */
	    }
	}

	hpbtrf_(uplo, n, kd, &afb[afb_offset], ldafb, info);
...
/*     Compute the norm of the matrix A. */
    ...
    anorm = hlansb_("1", uplo, n, kd, &ab[ab_offset], ldab, &work[1]);

/*     Compute the reciprocal of the condition number of A. */

    hpbcon_(uplo, n, kd, &afb[afb_offset], ldafb, &anorm, rcond, &work[1], &
	    iwork[1], info);

/*     Compute the solution matrix X. */

    hlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    hpbtrs_(uplo, n, kd, nrhs, &afb[afb_offset], ldafb, &x[x_offset], ldx, 
	    info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    hpbrfs_(uplo, n, kd, nrhs, &ab[ab_offset], ldab, &afb[afb_offset], ldafb, 
