...
ment of largest absolute value of a doublereal symmetric matrix. */
...
/*  ===================================================================== */
quadreal qlansy_(char *norm, char *uplo, integer *n, quadreal *a, integer 
	*lda, quadreal *work)
{
...
/*        Find normF(A). */
	...
	if (lsame_(uplo, "U")) {
	    ...
	    for (j = 2; j <= i__1; ++j) {
		i__2 = j - 1;
		qlassq_(&i__2, &a[j * a_dim1 + 1], &c__1, &scale, &sum);
/* L110: */
	    }
	} else {
	    i__1 = *n - 1;
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j;
		qlassq_(&i__2, &a[j + 1 + j * a_dim1], &c__1, &scale, &sum);
/* L120: */
	    }
	}
	sum *= 2;
	i__1 = *lda + 1;
	qlassq_(n, &a[a_offset], &i__1, &scale, &sum);
