...
/*  ===================================================================== */
void  hbbcsd_(char *jobu1, char *jobu2, char *jobv1t, char *
	jobv2t, char *trans, integer *m, integer *p, integer *q, halfreal *
	theta, halfreal *phi, halfreal *u1, integer *ldu1, halfreal *u2,
	 integer *ldu2, halfreal *v1t, integer *ldv1t, halfreal *v2t, 
	integer *ldv2t, halfreal *b11d, halfreal *b11e, halfreal *b12d, 
	halfreal *b12e, halfreal *b21d, halfreal *b21e, halfreal *
	b22d, halfreal *b22e, halfreal *work, integer *lwork, integer *
	info)
{
    ...
    halfreal r__, x1, x2, y1, y2, mu, nu, eps, tol;
    ...
    extern void  hlasr_(char *, char *, char *, integer *, 
	    integer *, halfreal *, halfreal *, halfreal *, integer *), hswap_(integer *, halfreal *, integer *
	    , halfreal *, integer *);
    integer maxit;
    halfreal dummy;
...
/*        Rotate to produce bulges in B11 and B21 */
	...
	if (mu <= nu) {
	    hlartgs_(&b11d[imin], &b11e[imin], &mu, &work[iv1tcs + imin - 1], 
		    &work[iv1tsn + imin - 1]);
	} else {
	    hlartgs_(&b21d[imin], &b21e[imin], &nu, &work[iv1tcs + imin - 1], 
		    &work[iv1tsn + imin - 1]);
	}
...
/* Computing 2nd power */
	...
	if (d__1 * d__1 + d__2 * d__2 > d__3 * d__3) {
	    hlartgp_(&b11bulge, &b11d[imin], &work[iu1sn + imin - 1], &work[
		    iu1cs + imin - 1], &r__);
	} else if (mu <= nu) {
	    hlartgs_(&b11e[imin], &b11d[imin + 1], &mu, &work[iu1cs + imin - 
		    1], &work[iu1sn + imin - 1]);
	} else {
	    hlartgs_(&b12d[imin], &b12e[imin], &nu, &work[iu1cs + imin - 1], &
		    work[iu1sn + imin - 1]);
	}
/* Computing 2nd power */
	d__1 = b21d[imin];
...
/* Computing 2nd power */
	d__3 = thresh;
	if (d__1 * d__1 + d__2 * d__2 > d__3 * d__3) {
	    hlartgp_(&b21bulge, &b21d[imin], &work[iu2sn + imin - 1], &work[
		    iu2cs + imin - 1], &r__);
	} else if (nu < mu) {
	    hlartgs_(&b21e[imin], &b21d[imin + 1], &nu, &work[iu2cs + imin - 
		    1], &work[iu2sn + imin - 1]);
	} else {
	    hlartgs_(&b22d[imin], &b22e[imin], &mu, &work[iu2cs + imin - 1], &
		    work[iu2sn + imin - 1]);
	}
...
/*           chasing by applying the original shift again. */
		...
		hlartgp_(&x2, &x1, &work[iv1tsn + i__ - 1], &work[iv1tcs + 
			i__ - 1], &r__);
	    } else if (! restart11 && restart21) {
		hlartgp_(&b11bulge, &b11e[i__ - 1], &work[iv1tsn + i__ - 1], &
			work[iv1tcs + i__ - 1], &r__);
	    } else if (restart11 && ! restart21) {
		hlartgp_(&b21bulge, &b21e[i__ - 1], &work[iv1tsn + i__ - 1], &
			work[iv1tcs + i__ - 1], &r__);
	    } else if (mu <= nu) {
		hlartgs_(&b11d[i__], &b11e[i__], &mu, &work[iv1tcs + i__ - 1],
			 &work[iv1tsn + i__ - 1]);
	    } else {
		hlartgs_(&b21d[i__], &b21e[i__], &nu, &work[iv1tcs + i__ - 1],
			 &work[iv1tsn + i__ - 1]);
	    }
	    work[iv1tcs + i__ - 1] = -work[iv1tcs + i__ - 1];
	    work[iv1tsn + i__ - 1] = -work[iv1tsn + i__ - 1];
	    if (! restart12 && ! restart22) {
		hlartgp_(&y2, &y1, &work[iv2tsn + i__ - 2], &work[iv2tcs + 
			i__ - 2], &r__);
	    } else if (! restart12 && restart22) {
		hlartgp_(&b12bulge, &b12d[i__ - 1], &work[iv2tsn + i__ - 2], &
			work[iv2tcs + i__ - 2], &r__);
	    } else if (restart12 && ! restart22) {
		hlartgp_(&b22bulge, &b22d[i__ - 1], &work[iv2tsn + i__ - 2], &
			work[iv2tcs + i__ - 2], &r__);
	    } else if (nu < mu) {
		hlartgs_(&b12e[i__ - 1], &b12d[i__], &nu, &work[iv2tcs + i__ 
			- 2], &work[iv2tsn + i__ - 2]);
	    } else {
		hlartgs_(&b22e[i__ - 1], &b22d[i__], &mu, &work[iv2tcs + i__ 
			- 2], &work[iv2tsn + i__ - 2]);
	    }
...
/*           chasing by applying the original shift again. */
		...
		hlartgp_(&x2, &x1, &work[iu1sn + i__ - 1], &work[iu1cs + i__ 
			- 1], &r__);
	    } else if (! restart11 && restart12) {
		hlartgp_(&b11bulge, &b11d[i__], &work[iu1sn + i__ - 1], &work[
			iu1cs + i__ - 1], &r__);
	    } else if (restart11 && ! restart12) {
		hlartgp_(&b12bulge, &b12e[i__ - 1], &work[iu1sn + i__ - 1], &
			work[iu1cs + i__ - 1], &r__);
	    } else if (mu <= nu) {
		hlartgs_(&b11e[i__], &b11d[i__ + 1], &mu, &work[iu1cs + i__ - 
			1], &work[iu1sn + i__ - 1]);
	    } else {
		hlartgs_(&b12d[i__], &b12e[i__], &nu, &work[iu1cs + i__ - 1], 
			&work[iu1sn + i__ - 1]);
	    }
	    if (! restart21 && ! restart22) {
		hlartgp_(&y2, &y1, &work[iu2sn + i__ - 1], &work[iu2cs + i__ 
			- 1], &r__);
	    } else if (! restart21 && restart22) {
		hlartgp_(&b21bulge, &b21d[i__], &work[iu2sn + i__ - 1], &work[
			iu2cs + i__ - 1], &r__);
	    } else if (restart21 && ! restart22) {
		hlartgp_(&b22bulge, &b22e[i__ - 1], &work[iu2sn + i__ - 1], &
			work[iu2cs + i__ - 1], &r__);
	    } else if (nu < mu) {
		hlartgs_(&b21e[i__], &b21e[i__ + 1], &nu, &work[iu2cs + i__ - 
			1], &work[iu2sn + i__ - 1]);
	    } else {
		hlartgs_(&b22d[i__], &b22e[i__], &mu, &work[iu2cs + i__ - 1], 
			&work[iu2sn + i__ - 1]);
	    }
...
/* Computing 2nd power */
	...
	if (! restart12 && ! restart22) {
	    hlartgp_(&y2, &y1, &work[iv2tsn + imax - 2], &work[iv2tcs + imax 
		    - 2], &r__);
	} else if (! restart12 && restart22) {
	    hlartgp_(&b12bulge, &b12d[imax - 1], &work[iv2tsn + imax - 2], &
		    work[iv2tcs + imax - 2], &r__);
	} else if (restart12 && ! restart22) {
	    hlartgp_(&b22bulge, &b22d[imax - 1], &work[iv2tsn + imax - 2], &
		    work[iv2tcs + imax - 2], &r__);
	} else if (nu < mu) {
	    hlartgs_(&b12e[imax - 1], &b12d[imax], &nu, &work[iv2tcs + imax - 
		    2], &work[iv2tsn + imax - 2]);
	} else {
	    hlartgs_(&b22e[imax - 1], &b22d[imax], &mu, &work[iv2tcs + imax - 
		    2], &work[iv2tsn + imax - 2]);
	}
...
/*        Update singular vectors */
	...
	if (wantu1) {
	    if (colmajor) {
		i__1 = imax - imin + 1;
		hlasr_("R", "V", "F", p, &i__1, &work[iu1cs + imin - 1], &
			work[iu1sn + imin - 1], &u1[imin * u1_dim1 + 1], ldu1);
	    } else {
		i__1 = imax - imin + 1;
		hlasr_("L", "V", "F", &i__1, p, &work[iu1cs + imin - 1], &
	...
	if (wantu2) {
		...
		i__1 = *m - *p;
		i__2 = imax - imin + 1;
		hlasr_("R", "V", "F", &i__1, &i__2, &work[iu2cs + imin - 1], &
			work[iu2sn + imin - 1], &u2[imin * u2_dim1 + 1], ldu2);
	    } else {
		i__1 = imax - imin + 1;
		i__2 = *m - *p;
		hlasr_("L", "V", "F", &i__1, &i__2, &work[iu2cs + imin - 1], &
	...
	if (wantv1t) {
	    if (colmajor) {
		i__1 = imax - imin + 1;
		hlasr_("L", "V", "F", &i__1, q, &work[iv1tcs + imin - 1], &
			work[iv1tsn + imin - 1], &v1t[imin + v1t_dim1], ldv1t);
	    } else {
		i__1 = imax - imin + 1;
		hlasr_("R", "V", "F", q, &i__1, &work[iv1tcs + imin - 1], &
	...
	if (wantv2t) {
		...
		i__1 = imax - imin + 1;
		i__2 = *m - *q;
		hlasr_("L", "V", "F", &i__1, &i__2, &work[iv2tcs + imin - 1], 
			&work[iv2tsn + imin - 1], &v2t[imin + v2t_dim1], 
			ldv2t);
	    } else {
		i__1 = *m - *q;
		i__2 = imax - imin + 1;
		hlasr_("R", "V", "F", &i__1, &i__2, &work[iv2tcs + imin - 1], 
...
/*        Fix signs on B11(IMAX-1,IMAX) and B21(IMAX-1,IMAX) */
	...
	if (b11e[imax - 1] + b21e[imax - 1] > 0.) {
	    ...
	    if (wantv1t) {
		if (colmajor) {
		    hscal_(q, &c_b35, &v1t[imax + v1t_dim1], ldv1t);
		} else {
		    hscal_(q, &c_b35, &v1t[imax * v1t_dim1 + 1], &c__1);
		}
	    }
	}
...
/*        and B22(IMAX,IMAX-1) */
	...
	if (b11d[imax] + b12e[imax - 1] < 0.) {
	    ...
	    if (wantu1) {
		if (colmajor) {
		    hscal_(p, &c_b35, &u1[imax * u1_dim1 + 1], &c__1);
		} else {
		    hscal_(p, &c_b35, &u1[imax + u1_dim1], ldu1);
		}
	    }
	}
	if (b21d[imax] + b22e[imax - 1] > 0.) {
		...
		if (colmajor) {
		    i__1 = *m - *p;
		    hscal_(&i__1, &c_b35, &u2[imax * u2_dim1 + 1], &c__1);
		} else {
		    i__1 = *m - *p;
		    hscal_(&i__1, &c_b35, &u2[imax + u2_dim1], ldu2);
		}
	    }
	}
...
/*        Fix signs on B12(IMAX,IMAX) and B22(IMAX,IMAX) */
	...
	if (b12d[imax] + b22d[imax] < 0.) {
		...
		if (colmajor) {
		    i__1 = *m - *q;
		    hscal_(&i__1, &c_b35, &v2t[imax + v2t_dim1], ldv2t);
		} else {
		    i__1 = *m - *q;
		    hscal_(&i__1, &c_b35, &v2t[imax * v2t_dim1 + 1], &c__1);
		}
	    }
	}
...
/*     Postprocessing: order THETA from least to greatest */
	...
	if (mini != i__) {
	    ...
	    if (colmajor) {
		if (wantu1) {
		    hswap_(p, &u1[i__ * u1_dim1 + 1], &c__1, &u1[mini * 
			    u1_dim1 + 1], &c__1);
		}
		if (wantu2) {
		    i__2 = *m - *p;
		    hswap_(&i__2, &u2[i__ * u2_dim1 + 1], &c__1, &u2[mini * 
			    u2_dim1 + 1], &c__1);
		}
		if (wantv1t) {
		    hswap_(q, &v1t[i__ + v1t_dim1], ldv1t, &v1t[mini + 
			    v1t_dim1], ldv1t);
		}
		if (wantv2t) {
		    i__2 = *m - *q;
		    hswap_(&i__2, &v2t[i__ + v2t_dim1], ldv2t, &v2t[mini + 
			    v2t_dim1], ldv2t);
		}
	    } else {
		if (wantu1) {
		    hswap_(p, &u1[i__ + u1_dim1], ldu1, &u1[mini + u1_dim1], 
			    ldu1);
		}
		if (wantu2) {
		    i__2 = *m - *p;
		    hswap_(&i__2, &u2[i__ + u2_dim1], ldu2, &u2[mini + 
			    u2_dim1], ldu2);
		}
		if (wantv1t) {
		    hswap_(q, &v1t[i__ * v1t_dim1 + 1], &c__1, &v1t[mini * 
			    v1t_dim1 + 1], &c__1);
		}
		if (wantv2t) {
		    i__2 = *m - *q;
		    hswap_(&i__2, &v2t[i__ * v2t_dim1 + 1], &c__1, &v2t[mini *
			     v2t_dim1 + 1], &c__1);
		}
	    }
	}
