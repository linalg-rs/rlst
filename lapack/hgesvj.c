...
/* > \verbatim */
/* > */
/* > DGESVJ computes the singular value decomposition (SVD) of a doublereal */
...
/* > */
/*  ===================================================================== */
void  hgesvj_(char *joba, char *jobu, char *jobv, integer *m, 
	integer *n, halfreal *a, integer *lda, halfreal *sva, integer *mv,
	 halfreal *v, integer *ldv, halfreal *work, integer *lwork, 
	integer *info)
{
    ...
    extern void  hscal_(integer *, halfreal *, halfreal *, 
	    integer *);
    halfreal large, apoaq, aqoap;
    extern logical lsame_(char *, char *);
    halfreal theta, small, sfmin;
    logical lsvec;
    ...
    logical uctol;
    extern void  hrotm_(integer *, halfreal *, integer *, 
	    halfreal *, integer *, halfreal *);
    logical lower, upper, rotok;
    extern void  hgsvj0_(char *, integer *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *, halfreal *,
	     integer *, halfreal *, integer *, integer *), hgsvj1_(
	    char *, integer *, integer *, integer *, halfreal *, integer *, 
...
/* [!]  (Make sure that DLAMCH() works properly on the target machine.) */
    ...
    epsln = hlamch_("Epsilon");
    rooteps = M(sqrt)(epsln);
    sfmin = hlamch_("SafeMinimum");
    rootsfmin = M(sqrt)(sfmin);
    small = sfmin / epsln;
    big = hlamch_("Overflow");
/*     BIG         = ONE    / SFMIN */
    rootbig = 1. / rootsfmin;
...
/*     Initialize the right singular vector matrix. */
    ...
    if (rsvec) {
	mvl = *n;
	hlaset_("A", &mvl, n, &c_b17, &c_b18, &v[v_offset], ldv);
    } else if (applv) {
	mvl = *mv;
    }
    rsvec = rsvec || applv;
...
/*     in A are detected, the procedure returns with INFO=-6. */
    ...
    skl = 1. / M(sqrt)((halfreal) (*m) * (halfreal) (*n));
    noscale = TRUE_;
    goscale = TRUE_;
...
/*        the input matrix is M-by-N lower triangular (trapezoidal) */
	...
	for (p = 1; p <= i__1; ++p) {
	    ...
	    aaqq = 1.;
	    i__2 = *m - p + 1;
	    hlassq_(&i__2, &a[p + p * a_dim1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("DGESVJ", &i__2);
...
/*        the input matrix is M-by-N upper triangular (trapezoidal) */
	...
	for (p = 1; p <= i__1; ++p) {
	    aapp = 0.;
	    aaqq = 1.;
	    hlassq_(&p, &a[p * a_dim1 + 1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("DGESVJ", &i__2);
...
/*        the input matrix is M-by-N general dense */
	...
	for (p = 1; p <= i__1; ++p) {
	    aapp = 0.;
	    aaqq = 1.;
	    hlassq_(m, &a[p * a_dim1 + 1], &c__1, &aapp, &aaqq);
	    if (aapp > big) {
		*info = -6;
		i__2 = -(*info);
		xerbla_("DGESVJ", &i__2);
...
/* #:) Quick return for zero matrix */
    ...
    if (aapp == 0.) {
	if (lsvec) {
	    hlaset_("G", m, n, &c_b17, &c_b18, &a[a_offset], lda);
	}
	work[1] = 1.;
	work[2] = 0.;
	work[3] = 0.;
...
/* #:) Quick return for one-column matrix */
    ...
    if (*n == 1) {
	if (lsvec) {
	    hlascl_("G", &c__0, &c__0, &sva[1], &skl, m, &c__1, &a[a_dim1 + 1]
		    , lda, &ierr);
	}
	work[1] = 1. / skl;
	if (sva[1] >= sfmin) {
...
/* Computing MIN */
	d__1 = sn / aaqq, d__2 = big / (aapp * M(sqrt)((halfreal) (*n)));
	temp1 = f2cmin(d__1,d__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else if (aaqq >= sn && aapp >= temp1) {
...
/* Computing MIN */
	d__1 = sn / aaqq, d__2 = big / (M(sqrt)((halfreal) (*n)) * aapp);
	temp1 = f2cmin(d__1,d__2);
/*         AAQQ  = AAQQ*TEMP1 */
/*         AAPP  = AAPP*TEMP1 */
    } else {
...
/*     Scale, if necessary */
    ...
    if (temp1 != 1.) {
	hlascl_("G", &c__0, &c__0, &c_b18, &temp1, n, &c__1, &sva[1], n, &
		ierr);
    }
    skl = temp1 * skl;
    if (skl != 1.) {
	hlascl_(joba, &c__0, &c__0, &c_b18, &skl, m, n, &a[a_offset], lda, &
		ierr);
	skl = 1. / skl;
    }
...
/*     [+ + x x]                    [x x].             [x x] */
	    ...
	    i__2 = *n - n34;
	    i__3 = *lwork - *n;
	    hgsvj0_(jobv, &i__1, &i__2, &a[n34 + 1 + (n34 + 1) * a_dim1], lda,
	    ...
	    i__2 = n34 - n2;
	    i__3 = *lwork - *n;
	    hgsvj0_(jobv, &i__1, &i__2, &a[n2 + 1 + (n2 + 1) * a_dim1], lda, &
	    ...
	    i__2 = *n - n2;
	    i__3 = *lwork - *n;
	    hgsvj1_(jobv, &i__1, &i__2, &n4, &a[n2 + 1 + (n2 + 1) * a_dim1], 
	    ...
	    i__2 = n2 - n4;
	    i__3 = *lwork - *n;
	    hgsvj0_(jobv, &i__1, &i__2, &a[n4 + 1 + (n4 + 1) * a_dim1], lda, &
		    work[n4 + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 1)
		     * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n + 
		    1], &i__3, &ierr);

	    i__1 = *lwork - *n;
	    hgsvj0_(jobv, m, &n4, &a[a_offset], lda, &work[1], &sva[1], &mvl, 
		    &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n 
		    + 1], &i__1, &ierr);

	    i__1 = *lwork - *n;
	    hgsvj1_(jobv, m, &n2, &n4, &a[a_offset], lda, &work[1], &sva[1], &
	    ...
	    i__1 = *lwork - *n;
	    hgsvj0_(jobv, &n4, &n4, &a[a_offset], lda, &work[1], &sva[1], &
		    mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__2, &
		    work[*n + 1], &i__1, &ierr);

	    i__1 = *lwork - *n;
	    hgsvj0_(jobv, &n2, &n4, &a[(n4 + 1) * a_dim1 + 1], lda, &work[n4 
		    + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 1) * 
		    v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n + 1], 
		    &i__1, &ierr);

	    i__1 = *lwork - *n;
	    hgsvj1_(jobv, &n2, &n2, &n4, &a[a_offset], lda, &work[1], &sva[1],
		     &mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &
		    work[*n + 1], &i__1, &ierr);

	    i__1 = n2 + n4;
	    i__2 = *lwork - *n;
	    hgsvj0_(jobv, &i__1, &n4, &a[(n2 + 1) * a_dim1 + 1], lda, &work[
...
/* Computing MIN */
		...
		for (p = igl; p <= i__3; ++p) {
		    ...
		    i__4 = *n - p + 1;
		    q = ihamax_(&i__4, &sva[p], &c__1) + p - 1;
		    if (p != q) {
			hswap_(m, &a[p * a_dim1 + 1], &c__1, &a[q * a_dim1 + 
				1], &c__1);
			if (rsvec) {
			    hswap_(&mvl, &v[p * v_dim1 + 1], &c__1, &v[q * 
				    v_dim1 + 1], &c__1);
			}
			temp1 = sva[p];
			sva[p] = sva[q];
...
/*        below should read "AAPP = DNRM2( M, A(1,p), 1 ) * WORK(p)". */
			...
			if (sva[p] < rootbig && sva[p] > rootsfmin) {
			    sva[p] = hnrm2_(m, &a[p * a_dim1 + 1], &c__1) * 
				    work[p];
			} else {
			    temp1 = 0.;
			    aapp = 1.;
			    hlassq_(m, &a[p * a_dim1 + 1], &c__1, &temp1, &
				    aapp);
			    sva[p] = temp1 * M(sqrt)(aapp) * work[p];
			}
			aapp = sva[p];
		    } else {
			aapp = sva[p];
...
/* Computing MAX */
			  ...
			  if (work[q] >= 1.) {
				  ...
				  if (rsvec) {
					...
					haxpy_(m, &temp1, &work[*n + 1], &
						c__1, &a[q * a_dim1 + 1], &
						c__1);
					hlascl_("G", &c__0, &c__0, &c_b18, &
...
/* 2000  end of the ibr-loop */
	...
	if (sva[*n] < rootbig && sva[*n] > rootsfmin) {
	    sva[*n] = hnrm2_(m, &a[*n * a_dim1 + 1], &c__1) * work[*n];
	} else {
	    t = 0.;
	    aapp = 1.;
	    hlassq_(m, &a[*n * a_dim1 + 1], &c__1, &t, &aapp);
	    sva[*n] = t * M(sqrt)(aapp) * work[*n];
	}
...
/*     Normalize the left singular vectors. */
	...
	for (p = 1; p <= i__1; ++p) {
	    d__1 = work[p] / sva[p];
	    hscal_(m, &d__1, &a[p * a_dim1 + 1], &c__1);
/* L1998: */
	}
    }
...
/*     Scale the product of Jacobi rotations (assemble the fast rotations). */
	...
	if (applv) {
	    i__1 = *n;
	    for (p = 1; p <= i__1; ++p) {
		hscal_(&mvl, &work[p], &v[p * v_dim1 + 1], &c__1);
/* L2398: */
	    }
	} else {
	    i__1 = *n;
	    for (p = 1; p <= i__1; ++p) {
		temp1 = 1. / hnrm2_(&mvl, &v[p * v_dim1 + 1], &c__1);
		hscal_(&mvl, &temp1, &v[p * v_dim1 + 1], &c__1);
/* L2399: */
	    }
	}
    }
