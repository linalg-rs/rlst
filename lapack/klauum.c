...
/*  ===================================================================== */
void  klauum_(char *uplo, integer *n, halfcomplex *a, 
	integer *lda, integer *info)
{
    ...
    extern void  ktrmm_(char *, char *, char *, char *, 
	    integer *, integer *, halfcomplex *, halfcomplex *, integer *,
	     halfcomplex *, integer *), 
	    klauu2_(char *, integer *, halfcomplex *, integer *, integer *), xerbla_(char *, integer *);
...
/*        Use unblocked code */
	...
	klauu2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/* Computing MIN */
		...
		ib = f2cmin(i__3,i__4);
		i__3 = i__ - 1;
		ktrmm_("Right", "Upper", "Conjugate transpose", "Non-unit", &
			i__3, &ib, &c_b1, &a[i__ + i__ * a_dim1], lda, &a[i__ 
			* a_dim1 + 1], lda);
		klauu2_("Upper", &ib, &a[i__ + i__ * a_dim1], lda, info);
		if (i__ + ib <= *n) {
		    i__3 = i__ - 1;
		    i__4 = *n - i__ - ib + 1;
		    kgemm_("No transpose", "Conjugate transpose", &i__3, &ib, 
			    &i__4, &c_b1, &a[(i__ + ib) * a_dim1 + 1], lda, &
			    a[i__ + (i__ + ib) * a_dim1], lda, &c_b1, &a[i__ *
			     a_dim1 + 1], lda);
		    i__3 = *n - i__ - ib + 1;
		    kherk_("Upper", "No transpose", &ib, &i__3, &c_b21, &a[
...
/* Computing MIN */
		...
		ib = f2cmin(i__3,i__4);
		i__3 = i__ - 1;
		ktrmm_("Left", "Lower", "Conjugate transpose", "Non-unit", &
			ib, &i__3, &c_b1, &a[i__ + i__ * a_dim1], lda, &a[i__ 
			+ a_dim1], lda);
		klauu2_("Lower", &ib, &a[i__ + i__ * a_dim1], lda, info);
		if (i__ + ib <= *n) {
		    i__3 = i__ - 1;
		    i__4 = *n - i__ - ib + 1;
		    kgemm_("Conjugate transpose", "No transpose", &ib, &i__3, 
			    &i__4, &c_b1, &a[i__ + ib + i__ * a_dim1], lda, &
			    a[i__ + ib + a_dim1], lda, &c_b1, &a[i__ + a_dim1]
			    , lda);
		    i__3 = *n - i__ - ib + 1;
		    kherk_("Lower", "Conjugate transpose", &ib, &i__3, &c_b21,
