...
/*  ===================================================================== */
void  wcposv_(char *uplo, integer *n, integer *nrhs, 
	quadcomplex *a, integer *lda, quadcomplex *b, integer *ldb, 
	quadcomplex *x, integer *ldx, quadcomplex *work, complex *swork, 
	quadreal *rwork, integer *iter, integer *info)
{
    ...
    extern void  whemm_(char *, char *, integer *, integer *, 
	    ...
	    integer *, quadcomplex *, quadcomplex *, integer *), waxpy_(integer *, quadcomplex *, quadcomplex *, 
	    integer *, quadcomplex *, integer *), wlag2c_(integer *, 
	    integer *, quadcomplex *, integer *, complex *, integer *, 
	    integer *), clag2z_(integer *, integer *, complex *, integer *, 
	    quadcomplex *, integer *, integer *), wlat2c_(char *, integer *,
	     quadcomplex *, integer *, complex *, integer *, integer *);
    ...
    extern void  cpotrf_(char *, integer *, complex *, integer 
	    *, integer *), wlacpy_(char *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *), 
	    cpotrs_(char *, integer *, integer *, complex *, integer *, 
	    complex *, integer *, integer *), wpotrf_(char *, integer 
	    *, quadcomplex *, integer *, integer *), wpotrs_(char *,
...
/*     Compute some constants. */
    ...
    anrm = wlanhe_("I", uplo, n, &a[a_offset], lda, &rwork[1]);
    eps = qlamch_("Epsilon");
...
/*     result in SX. */
    ...
    wlag2c_(n, nrhs, &b[b_offset], ldb, &swork[ptsx], n, info);
...
/*     result in SA. */
    ...
    wlat2c_(uplo, n, &a[a_offset], lda, &swork[ptsa], n, info);
...
/*     Compute R = B - AX (R is WORK). */
    ...
    wlacpy_("All", n, nrhs, &b[b_offset], ldb, &work[work_offset], n);

    whemm_("Left", uplo, n, nrhs, &c_b1, &a[a_offset], lda, &x[x_offset], ldx,
...
/*     stopping criterion. If yes, set ITER=0 and return. */
	...
	i__2 = iwamax_(n, &x[i__ * x_dim1 + 1], &c__1) + i__ * x_dim1;
	xnrm = (d__1 = x[i__2].r, abs(d__1)) + (d__2 = d_imag(&x[iwamax_(n, &
		x[i__ * x_dim1 + 1], &c__1) + i__ * x_dim1]), abs(d__2));
	i__2 = iwamax_(n, &work[i__ * work_dim1 + 1], &c__1) + i__ * 
		work_dim1;
	rnrm = (d__1 = work[i__2].r, abs(d__1)) + (d__2 = d_imag(&work[
		iwamax_(n, &work[i__ * work_dim1 + 1], &c__1) + i__ * 
		work_dim1]), abs(d__2));
	if (rnrm > xnrm * cte) {
	    goto L10;
	}
...
/*        and store the result in SX. */
	...
	wlag2c_(n, nrhs, &work[work_offset], n, &swork[ptsx], n, info);
...
/*        iterate. */
	...
	i__1 = *nrhs;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    waxpy_(n, &c_b2, &work[i__ * work_dim1 + 1], &c__1, &x[i__ * 
		    x_dim1 + 1], &c__1);
	}
...
/*        Compute R = B - AX (R is WORK). */
	...
	whemm_("L", uplo, n, nrhs, &c_b1, &a[a_offset], lda, &x[x_offset], 
...
/*        stopping criterion. If yes, set ITER=IITER>0 and return. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    i__2 = iwamax_(n, &x[i__ * x_dim1 + 1], &c__1) + i__ * x_dim1;
	    xnrm = (d__1 = x[i__2].r, abs(d__1)) + (d__2 = d_imag(&x[iwamax_(
...
/*     satisfactory solution, so we resort to double precision. */
    ...
    wpotrf_(uplo, n, &a[a_offset], lda, info);

    if (*info != 0) {
	return;
    }

    wlacpy_("All", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    wpotrs_(uplo, n, nrhs, &a[a_offset], lda, &x[x_offset], ldx, info);
