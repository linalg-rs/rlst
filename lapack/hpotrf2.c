...
/*  ===================================================================== */
void  hpotrf2_(char *uplo, integer *n, halfreal *a, integer *
	lda, integer *info)
{
...
/*        Test for non-positive-definiteness */
	...
	if (a[a_dim1 + 1] <= 0. || hisnan_(&a[a_dim1 + 1])) {
	    *info = 1;
	    return;
	}
...
/*        Factor A11 */
	...
	hpotrf2_(uplo, &n1, &a[a_dim1 + 1], lda, &iinfo);
	if (iinfo != 0) {
	    *info = iinfo;
	    return;
	}
...
/*           Update and scale A12 */
	    ...
	    htrsm_("L", "U", "T", "N", &n1, &n2, &c_b9, &a[a_dim1 + 1], lda, &
		    a[(n1 + 1) * a_dim1 + 1], lda);

/*           Update and factor A22 */

	    hsyrk_(uplo, "T", &n2, &n1, &c_b11, &a[(n1 + 1) * a_dim1 + 1], 
		    lda, &c_b9, &a[n1 + 1 + (n1 + 1) * a_dim1], lda);
	    hpotrf2_(uplo, &n2, &a[n1 + 1 + (n1 + 1) * a_dim1], lda, &iinfo);
	    if (iinfo != 0) {
		*info = iinfo + n1;
		return;
	    }
...
/*           Update and scale A21 */
	    ...
	    htrsm_("R", "L", "T", "N", &n2, &n1, &c_b9, &a[a_dim1 + 1], lda, &
		    a[n1 + 1 + a_dim1], lda);

/*           Update and factor A22 */

	    hsyrk_(uplo, "N", &n2, &n1, &c_b11, &a[n1 + 1 + a_dim1], lda, &
