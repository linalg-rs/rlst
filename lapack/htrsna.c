...
/* > eigenvalues and/or right eigenvectors of a doublereal upper */
...
/* > */
/*  ===================================================================== */
void  htrsna_(char *job, char *howmny, logical *select, 
	integer *n, halfreal *t, integer *ldt, halfreal *vl, integer *
	ldvl, halfreal *vr, integer *ldvr, halfreal *s, halfreal *sep, 
	integer *mm, integer *m, halfreal *work, integer *ldwork, integer *
	iwork, integer *info)
{
    ...
    halfreal prod1, prod2, scale, delta;
    ...
    halfreal bignum;
    logical wantbh;
    extern void  hlaqtr_(logical *, logical *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *, halfreal *,
	     halfreal *, halfreal *, integer *), htrexc_(char *, integer *
...
/*              Real eigenvalue. */
		...
		prod = hdot_(n, &vr[ks * vr_dim1 + 1], &c__1, &vl[ks * 
			vl_dim1 + 1], &c__1);
...
/*              Complex eigenvalue. */
		...
		prod1 = hdot_(n, &vr[ks * vr_dim1 + 1], &c__1, &vl[ks * 
			vl_dim1 + 1], &c__1);
		prod1 += hdot_(n, &vr[(ks + 1) * vr_dim1 + 1], &c__1, &vl[(ks 
			+ 1) * vl_dim1 + 1], &c__1);
		prod2 = hdot_(n, &vl[ks * vl_dim1 + 1], &c__1, &vr[(ks + 1) * 
			vr_dim1 + 1], &c__1);
		prod2 -= hdot_(n, &vl[(ks + 1) * vl_dim1 + 1], &c__1, &vr[ks *
			 vr_dim1 + 1], &c__1);
		...
		cond = hlapy2_(&prod1, &prod2) / (rnrm * lnrm);
		s[ks] = cond;
		s[ks + 1] = cond;
	    }
	}
...
/*           block beginning at T(k,k) to the (1,1) position. */
	    ...
	    hlacpy_("Full", n, n, &t[t_offset], ldt, &work[work_offset], 
		    ldwork);
	    ifst = k;
	    ilst = 1;
	    htrexc_("No Q", n, &work[work_offset], ldwork, dummy, &c__1, &
...
/*                 with negative imaginary  part. */
		    ...
		    mu = M(sqrt)((d__1 = work[(work_dim1 << 1) + 1], abs(d__1))) 
			    * M(sqrt)((d__2 = work[work_dim1 + 2], abs(d__2)));
		    delta = hlapy2_(&mu, &work[work_dim1 + 2]);
		    cs = mu / delta;
...
/*              Estimate norm(inv(C**T)) */
		...
		kase = 0;
L50:
		hlacn2_(&nn, &work[(*n + 2) * work_dim1 + 1], &work[(*n + 4) *
...
/*                       Real eigenvalue: solve C**T*x = scale*c. */
			    ...
			    i__2 = *n - 1;
			    hlaqtr_(&c_true, &c_true, &i__2, &work[(work_dim1 
...
/*                       C**T*(p+iq) = scale*(c+id) in doublereal arithmetic. */

			    i__2 = *n - 1;
			    hlaqtr_(&c_true, &c_false, &i__2, &work[(
...
/*                       Real eigenvalue: solve C*x = scale*c. */
			    ...
			    i__2 = *n - 1;
			    hlaqtr_(&c_false, &c_true, &i__2, &work[(
...
/*                       Complex eigenvalue: solve */
/*                       C*(p+iq) = scale*(c+id) in doublereal arithmetic. */

			    i__2 = *n - 1;
			    hlaqtr_(&c_false, &c_false, &i__2, &work[(
				    work_dim1 << 1) + 2], ldwork, &work[(*n + 
				    1) * work_dim1 + 1], &mu, &scale, &work[(*
				    n + 4) * work_dim1 + 1], &work[(*n + 6) * 
				    work_dim1 + 1], &ierr);
