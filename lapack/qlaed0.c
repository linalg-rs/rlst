...
/*  ===================================================================== */
void  qlaed0_(integer *icompq, integer *qsiz, integer *n, 
	quadreal *d__, quadreal *e, quadreal *q, integer *ldq, 
	quadreal *qstore, integer *ldqs, quadreal *work, integer *iwork, 
	integer *info)
{
    ...
    extern void  qcopy_(integer *, quadreal *, integer *, 
	    quadreal *, integer *);
    integer indxq, iwrem;
    extern void  qlaed1_(integer *, quadreal *, quadreal *,
	     integer *, integer *, quadreal *, integer *, quadreal *, 
	    integer *, integer *);
    integer iqptr;
    extern void  qlaed7_(integer *, integer *, integer *, 
...
/*     conquer tree. */
	...
	if (*icompq == 2) {
	    qsteqr_("I", &matsiz, &d__[submat], &e[submat], &q[submat + 
		    submat * q_dim1], ldq, &work[1], info);
	    if (*info != 0) {
		goto L130;
	    }
	} else {
	    qsteqr_("I", &matsiz, &d__[submat], &e[submat], &work[iq - 1 + 
		    iwork[iqptr + curr]], &matsiz, &work[1], info);
	    if (*info != 0) {
		goto L130;
	    }
	    if (*icompq == 1) {
		qgemm_("N", "N", qsiz, &matsiz, &matsiz, &c_b23, &q[submat * 
...
/*     tridiagonal form) are desired. */
	    ...
	    if (*icompq == 2) {
		qlaed1_(&matsiz, &d__[submat], &q[submat + submat * q_dim1], 
			ldq, &iwork[indxq + submat], &e[submat + msd2 - 1], &
			msd2, &work[1], &iwork[subpbs + 1], info);
	    } else {
		qlaed7_(icompq, &matsiz, qsiz, &tlvls, &curlvl, &curprb, &d__[
...
/*     merge step. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    j = iwork[indxq + i__];
	    work[i__] = d__[j];
	    qcopy_(qsiz, &qstore[j * qstore_dim1 + 1], &c__1, &q[i__ * q_dim1 
		    + 1], &c__1);
/* L100: */
	}
	qcopy_(n, &work[1], &c__1, &d__[1], &c__1);
    } else if (*icompq == 2) {
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    j = iwork[indxq + i__];
	    work[i__] = d__[j];
	    qcopy_(n, &q[j * q_dim1 + 1], &c__1, &work[*n * i__ + 1], &c__1);
/* L110: */
	}
	qcopy_(n, &work[1], &c__1, &d__[1], &c__1);
	qlacpy_("A", n, n, &work[*n + 1], n, &q[q_offset], ldq);
    } else {
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    j = iwork[indxq + i__];
