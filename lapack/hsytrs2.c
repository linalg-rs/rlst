...
/* > \verbatim */
/* > */
/* > DSYTRS2 solves a system of linear equations A*X = B with a doublereal */
...
/*  ===================================================================== */
void  hsytrs2_(char *uplo, integer *n, integer *nrhs, 
	halfreal *a, integer *lda, integer *ipiv, halfreal *b, integer *
	ldb, halfreal *work, integer *info)
{
    ...
    extern void  hswap_(integer *, halfreal *, integer *, 
	    halfreal *, integer *), htrsm_(char *, char *, char *, char *, 
	    integer *, integer *, halfreal *, halfreal *, integer *, 
	    halfreal *, integer *);
    logical upper;
    extern void  xerbla_(char *, integer *), hsyconv_(
	    char *, char *, integer *, halfreal *, integer *, integer *, 
	    halfreal *, integer *);
...
/*     Convert A */
    ...
    hsyconv_(uplo, "C", n, &a[a_offset], lda, &ipiv[1], &work[1], &iinfo);
...
/*           Interchange rows K-1 and -IPIV(K). */
		kp = -ipiv[k];
		if (kp == -ipiv[k - 1]) {
		    hswap_(nrhs, &b[k - 1 + b_dim1], ldb, &b[kp + b_dim1], 
			    ldb);
		}
		k += -2;
	    }
...
/*  Compute (U \P**T * B) -> B    [ (U \P**T * B) ] */
	...
	htrsm_("L", "U", "N", "U", n, nrhs, &c_b10, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*  Compute D \ B -> B   [ D \ (U \P**T * B) ] */
	...
	while(i__ >= 1) {
		...
		hscal_(nrhs, &d__1, &b[i__ + b_dim1], ldb);
	    } else if (i__ > 1) {
...
/*      Compute (U**T \ B) -> B   [ U**T \ (D \ (U \P**T * B) ) ] */
	...
	htrsm_("L", "U", "T", "U", n, nrhs, &c_b10, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*  Compute (L \P**T * B) -> B    [ (L \P**T * B) ] */
	...
	htrsm_("L", "L", "N", "U", n, nrhs, &c_b10, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*  Compute (L**T \ B) -> B   [ L**T \ (D \ (L \P**T * B) ) ] */
	...
	htrsm_("L", "L", "T", "U", n, nrhs, &c_b10, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*     Revert A */
    ...
    hsyconv_(uplo, "R", n, &a[a_offset], lda, &ipiv[1], &work[1], &iinfo);
