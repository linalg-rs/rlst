...
/* > */
/*  ===================================================================== */
void  wgeqr_(integer *m, integer *n, quadcomplex *a, integer 
	*lda, quadcomplex *t, integer *tsize, quadcomplex *work, integer *
	lwork, integer *info)
{
    ...
    logical lminws;
    extern void  wgeqrt_(integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *);
    logical lquery;
    integer mintsz;
    extern void  wlatsqr_(integer *, integer *, integer *, 
...
/* Computing MAX */
    ...
    if (*info == 0) {
	if (mint) {
	    t[1].r = (quadreal) mintsz, t[1].i = 0.;
	} else {
	    i__1 = nb * *n * nblcks + 5;
	    t[1].r = (quadreal) i__1, t[1].i = 0.;
	}
	t[2].r = (quadreal) mb, t[2].i = 0.;
	t[3].r = (quadreal) nb, t[3].i = 0.;
	if (minw) {
	    i__1 = f2cmax(1,*n);
...
/*     The QR Decomposition */
    ...
    if (*m <= *n || mb <= *n || mb >= *m) {
	wgeqrt_(m, n, &nb, &a[a_offset], lda, &t[6], &nb, &work[1], info);
    } else {
	wlatsqr_(m, n, &mb, &nb, &a[a_offset], lda, &t[6], &nb, &work[1], 
		lwork, info);
    }
