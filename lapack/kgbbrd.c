...
/*  ===================================================================== */
void  kgbbrd_(char *vect, integer *m, integer *n, integer *ncc,
	 integer *kl, integer *ku, halfcomplex *ab, integer *ldab, 
	halfreal *d__, halfreal *e, halfcomplex *q, integer *ldq, 
	halfcomplex *pt, integer *ldpt, halfcomplex *c__, integer *ldc, 
	halfcomplex *work, halfreal *rwork, integer *info)
{
    ...
    integer minmn;
    logical wantq;
    extern void  xerbla_(char *, integer *), klaset_(
	    char *, integer *, integer *, halfcomplex *, halfcomplex *, 
	    halfcomplex *, integer *), klartg_(halfcomplex *, 
	    halfcomplex *, halfreal *, halfcomplex *, halfcomplex *), 
	    klargv_(integer *, halfcomplex *, integer *, halfcomplex *, 
	    integer *, halfreal *, integer *);
    logical wantpt;
    extern void  klartv_(integer *, halfcomplex *, integer *,
	     halfcomplex *, integer *, halfreal *, halfcomplex *, 
	    integer *);
...
/*     Initialize Q and P**H to the unit matrix, if needed */
    ...
    if (wantq) {
	klaset_("Full", m, m, &c_b1, &c_b2, &q[q_offset], ldq);
    }
    if (wantpt) {
	klaset_("Full", n, n, &c_b1, &c_b2, &pt[pt_offset], ldpt);
    }
...
/*              which have been created below the band */
		...
		if (nr > 0) {
		    klargv_(&nr, &ab[klu1 + (j1 - klm - 1) * ab_dim1], &inca, 
...
/*              apply plane rotations from the left */
		...
		for (l = 1; l <= i__3; ++l) {
		    ...
		    if (nrt > 0) {
			klartv_(&nrt, &ab[klu1 - l + (j1 - klm + l - 1) * 
...
/*                    within the band, and apply rotation from the left */
			...
			klartg_(&ab[*ku + ml - 1 + i__ * ab_dim1], &ab[*ku + 
...
/*                 accumulate product of plane rotations in Q */
		    ...
		    for (j = j1; i__4 < 0 ? j >= i__3 : j <= i__3; j += i__4) 
			    {
			d_cnjg(&z__1, &work[j]);
			krot_(m, &q[(j - 1) * q_dim1 + 1], &c__1, &q[j * 
...
/*                 apply plane rotations to C */
		    ...
		    for (j = j1; i__3 < 0 ? j >= i__4 : j <= i__4; j += i__3) 
			    {
			krot_(ncc, &c__[j - 1 + c_dim1], ldc, &c__[j + c_dim1]
				, ldc, &rwork[j], &work[j]);
/* L30: */
		    }
		}
...
/*              which have been generated above the band */
		...
		if (nr > 0) {
		    klargv_(&nr, &ab[(j1 + kun - 1) * ab_dim1 + 1], &inca, &
...
/*              apply plane rotations from the right */
		...
		for (l = 1; l <= i__4; ++l) {
		    ...
		    if (nrt > 0) {
			klartv_(&nrt, &ab[l + 1 + (j1 + kun - 1) * ab_dim1], &
...
/*                    within the band, and apply rotation from the right */
			...
			klartg_(&ab[*ku - mu + 3 + (i__ + mu - 2) * ab_dim1], 
...
/* Computing MIN */
			...
			krot_(&i__4, &ab[*ku - mu + 4 + (i__ + mu - 2) * 
...
/*                 accumulate product of plane rotations in P**H */
			...
			krot_(n, &pt[j + kun - 1 + pt_dim1], ldpt, &pt[j + 
...
/* Computing MIN */
	...
	i__1 = f2cmin(i__2,*n);
	for (i__ = 1; i__ <= i__1; ++i__) {
	    klartg_(&ab[i__ * ab_dim1 + 1], &ab[i__ * ab_dim1 + 2], &rc, &rs, 
		    &ra);
	    i__2 = i__ * ab_dim1 + 1;
	    ...
	    if (wantq) {
		d_cnjg(&z__1, &rs);
		krot_(m, &q[i__ * q_dim1 + 1], &c__1, &q[(i__ + 1) * q_dim1 + 
			1], &c__1, &rc, &z__1);
	    }
	    if (wantc) {
		krot_(ncc, &c__[i__ + c_dim1], ldc, &c__[i__ + 1 + c_dim1], 
			ldc, &rc, &rs);
	    }
/* L100: */
	}
...
/*           right */
	    ...
	    for (i__ = *m; i__ >= 1; --i__) {
		klartg_(&ab[*ku + 1 + i__ * ab_dim1], &rb, &rc, &rs, &ra);
		...
		if (wantpt) {
		    d_cnjg(&z__1, &rs);
		    krot_(n, &pt[i__ + pt_dim1], ldpt, &pt[*m + 1 + pt_dim1], 
			    ldpt, &rc, &z__1);
		}
/* L110: */
	    }
...
/*     and E */
	...
	if (wantq) {
	    kscal_(m, &t, &q[i__ * q_dim1 + 1], &c__1);
	}
	if (wantc) {
	    d_cnjg(&z__1, &t);
	    kscal_(ncc, &z__1, &c__[i__ + c_dim1], ldc);
	}
	if (i__ < minmn) {
	    if (*ku == 0 && *kl == 0) {
		e[i__] = 0.;
		...
		if (wantpt) {
		    kscal_(n, &t, &pt[i__ + 1 + pt_dim1], ldpt);
		}
