...
/*  ===================================================================== */
void  wstedc_(char *compz, integer *n, quadreal *d__, 
	quadreal *e, quadcomplex *z__, integer *ldz, quadcomplex *work, 
	integer *lwork, quadreal *rwork, integer *lrwork, integer *iwork, 
	integer *liwork, integer *info)
{
    ...
    extern void  wswap_(integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *), wlaed0_(integer *, integer *, 
	    quadreal *, quadreal *, quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadreal *, integer *, integer *);
    ...
    extern void  qlascl_(char *, integer *, integer *, 
	    ...
	    quadreal *, quadreal *, quadreal *, integer *, quadreal *,
	     integer *, integer *, integer *, integer *), qlaset_(
    ...
    extern void  qsterf_(integer *, quadreal *, quadreal *,
	     integer *), wlacrm_(integer *, integer *, quadcomplex *, 
	    integer *, quadreal *, integer *, quadcomplex *, integer *, 
	    quadreal *);
    integer liwmin, icompz;
    extern void  qsteqr_(char *, integer *, quadreal *, 
	    quadreal *, quadreal *, integer *, quadreal *, integer *), wlacpy_(char *, integer *, integer *, quadcomplex *, 
...
/*        Compute the workspace requirements */
	...
	if (*n <= 1 || icompz == 0) {
	    ...
	    lgn = (integer) (M(log)((quadreal) (*n)) / M(log)(2.));
...
/*     eigenvalues, which requires (3N + 3N**2) doublereal workspace and */
...
/*     solve the problem with another solver. */
	...
	wsteqr_(compz, n, &d__[1], &e[1], &z__[z_offset], ldz, &rwork[1], 
		info);
...
/*        If COMPZ = 'I', we simply call DSTEDC instead. */
	...
	if (icompz == 2) {
	    qlaset_("Full", n, n, &c_b17, &c_b18, &rwork[1], n);
	    ll = *n * *n + 1;
	    i__1 = *lrwork - ll + 1;
	    qstedc_("I", n, &d__[1], &e[1], &rwork[1], n, &rwork[ll], &i__1, &
		    iwork[1], liwork, info);
	    i__1 = *n;
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n;
...
L40:
	    if (finish < *n) {
		tiny = eps * M(sqrt)((d__1 = d__[finish], abs(d__1))) * M(sqrt)((
...
/*              Scale. */
		...
		orgnrm = qlanst_("M", &m, &d__[start], &e[start]);
		qlascl_("G", &c__0, &c__0, &orgnrm, &c_b18, &m, &c__1, &d__[
			start], &m, info);
		i__1 = m - 1;
		i__2 = m - 1;
		qlascl_("G", &c__0, &c__0, &orgnrm, &c_b18, &i__1, &c__1, &e[
			start], &i__2, info);

		wlaed0_(n, &m, &d__[start], &e[start], &z__[start * z_dim1 + 
...
/*              Scale back. */
		...
		qlascl_("G", &c__0, &c__0, &c_b18, &orgnrm, &m, &c__1, &d__[
			start], &m, info);

	    } else {
		qsteqr_("I", &m, &d__[start], &e[start], &rwork[1], &m, &
			rwork[m * m + 1], info);
		wlacrm_(n, &m, &z__[start * z_dim1 + 1], ldz, &rwork[1], &m, &
			work[1], n, &rwork[m * m + 1]);
		wlacpy_("A", n, &m, &work[1], n, &z__[start * z_dim1 + 1], 
			ldz);
		if (*info > 0) {
