...
/* > \verbatim */
/* > */
/* > DSPTRI computes the inverse of a doublereal symmetric indefinite matrix */
...
/*  ===================================================================== */
void  hsptri_(char *uplo, integer *n, halfreal *ap, integer *
	ipiv, halfreal *work, integer *info)
{
    ...
    extern void  hcopy_(integer *, halfreal *, integer *, 
	    ...
	    *, halfreal *, integer *);
    integer kstep;
    extern void  hspmv_(char *, integer *, halfreal *, 
...
/*           Compute column K of the inverse. */
	    ...
	    if (k > 1) {
		i__1 = k - 1;
		hcopy_(&i__1, &ap[kc], &c__1, &work[1], &c__1);
		i__1 = k - 1;
		hspmv_(uplo, &i__1, &c_b11, &ap[1], &work[1], &c__1, &c_b13, &
			ap[kc], &c__1);
		i__1 = k - 1;
		ap[kc + k - 1] -= hdot_(&i__1, &work[1], &c__1, &ap[kc], &
			c__1);
	    }
	    kstep = 1;
	} else {
...
/*           Compute columns K and K+1 of the inverse. */
		...
		ap[kc + k - 1] -= hdot_(&i__1, &work[1], &c__1, &ap[kc], &
			c__1);
		i__1 = k - 1;
		ap[kcnext + k - 1] -= hdot_(&i__1, &ap[kc], &c__1, &ap[kcnext]
			, &c__1);
		i__1 = k - 1;
		hcopy_(&i__1, &ap[kcnext], &c__1, &work[1], &c__1);
		i__1 = k - 1;
		hspmv_(uplo, &i__1, &c_b11, &ap[1], &work[1], &c__1, &c_b13, &
			ap[kcnext], &c__1);
		i__1 = k - 1;
		ap[kcnext + k] -= hdot_(&i__1, &work[1], &c__1, &ap[kcnext], &
			c__1);
	    }
	    kstep = 2;
	    kcnext = kcnext + k + 1;
...
/*           submatrix A(1:k+1,1:k+1) */
	    ...
	    kpc = (kp - 1) * kp / 2 + 1;
	    i__1 = kp - 1;
	    hswap_(&i__1, &ap[kc], &c__1, &ap[kpc], &c__1);
	    kx = kpc + kp - 1;
	    i__1 = k - 1;
	    for (j = kp + 1; j <= i__1; ++j) {
		kx = kx + j - 1;
...
/*           Compute column K of the inverse. */
	    ...
	    if (k < *n) {
		i__1 = *n - k;
		hcopy_(&i__1, &ap[kc + 1], &c__1, &work[1], &c__1);
		i__1 = *n - k;
		hspmv_(uplo, &i__1, &c_b11, &ap[kc + *n - k + 1], &work[1], &
			c__1, &c_b13, &ap[kc + 1], &c__1);
		i__1 = *n - k;
		ap[kc] -= hdot_(&i__1, &work[1], &c__1, &ap[kc + 1], &c__1);
	    }
	    kstep = 1;
	} else {
...
/*           Compute columns K-1 and K of the inverse. */
		...
		ap[kc] -= hdot_(&i__1, &work[1], &c__1, &ap[kc + 1], &c__1);
		i__1 = *n - k;
		ap[kcnext + 1] -= hdot_(&i__1, &ap[kc + 1], &c__1, &ap[kcnext 
			+ 2], &c__1);
		i__1 = *n - k;
		hcopy_(&i__1, &ap[kcnext + 2], &c__1, &work[1], &c__1);
		i__1 = *n - k;
		hspmv_(uplo, &i__1, &c_b11, &ap[kc + (*n - k + 1)], &work[1], 
			&c__1, &c_b13, &ap[kcnext + 2], &c__1);
		i__1 = *n - k;
		ap[kcnext] -= hdot_(&i__1, &work[1], &c__1, &ap[kcnext + 2], &
			c__1);
	    }
	    kstep = 2;
	    kcnext -= *n - k + 3;
...
/*           submatrix A(k-1:n,k-1:n) */
	    ...
	    if (kp < *n) {
		i__1 = *n - kp;
		hswap_(&i__1, &ap[kc + kp - k + 1], &c__1, &ap[kpc + 1], &
			c__1);
	    }
	    kx = kc + kp - k;
	    i__1 = kp - 1;
