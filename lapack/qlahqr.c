...
/* > */
/*  ===================================================================== */
void  qlahqr_(logical *wantt, logical *wantz, integer *n, 
	...
	integer *ldz, integer *info)
{
    ...
    quadreal s, v[3];
    integer i1, i2;
    quadreal t1, t2, t3, v2, v3, aa, ab, ba, bb, h11, h12, h21, h22, cs;
    integer nh;
    quadreal sn;
    integer nr;
    quadreal tr;
    ...
    quadreal det, h21s;
    integer its;
    quadreal ulp, sum, tst, rt1i, rt2i, rt1r, rt2r;
    extern void  qrot_(integer *, quadreal *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *), qcopy_(
    ...
    extern void  qlarfg_(integer *, quadreal *, quadreal *,
	     integer *, quadreal *);
    quadreal safmin, safmax, rtdisc, smlnum;
...
/* Computing MIN */
	    ...
	    nr = f2cmin(i__3,i__4);
	    if (k > m) {
		qcopy_(&nr, &h__[k + (k - 1) * h_dim1], &c__1, v, &c__1);
	    }
...
/*        and compute and store the eigenvalues. */
	...
	qlanv2_(&h__[i__ - 1 + (i__ - 1) * h_dim1], &h__[i__ - 1 + i__ * 
...
/*           Apply the transformation to the rest of H. */
	    ...
	    if (i2 > i__) {
		i__1 = i2 - i__;
		qrot_(&i__1, &h__[i__ - 1 + (i__ + 1) * h_dim1], ldh, &h__[
			i__ + (i__ + 1) * h_dim1], ldh, &cs, &sn);
	    }
	    i__1 = i__ - i1 - 1;
	    qrot_(&i__1, &h__[i1 + (i__ - 1) * h_dim1], &c__1, &h__[i1 + i__ *
		     h_dim1], &c__1, &cs, &sn);
	}
	if (*wantz) {
...
/*           Apply the transformation to Z. */
	    ...
	    qrot_(&nz, &z__[*iloz + (i__ - 1) * z_dim1], &c__1, &z__[*iloz + 
