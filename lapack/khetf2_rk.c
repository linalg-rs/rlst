...
/*  ===================================================================== */
void  zhetf2_rk__(char *uplo, integer *n, halfcomplex *a, 
	integer *lda, halfcomplex *e, integer *ipiv, integer *info)
{
    ...
    halfcomplex wk;
    halfreal tt;
    halfcomplex wkm1, wkp1;
    logical done;
    integer imax, jmax;
    extern void  kher_(char *, integer *, halfreal *, 
...
/*              (4) Swap diagonal elements at row-col intersection */
		...
		if (kstep == 2) {
/*                 (*) Make sure that diagonal element of pivot is doublereal */
		    i__1 = k + k * a_dim1;
		    i__2 = k + k * a_dim1;
		    d__1 = a[i__2].r;
		    a[i__1].r = d__1, a[i__1].i = 0.;
...
/*                       = A - W(k)*1/D(k)*W(k)**T */
			...
			i__1 = k - 1;
			d__1 = -d11;
			kher_(uplo, &i__1, &d__1, &a[k * a_dim1 + 1], &c__1, &
				a[a_offset], lda);

/*                    Store U(k) in column k */

			i__1 = k - 1;
			whscal_(&i__1, &d11, &a[k * a_dim1 + 1], &c__1);
		    } else {
...
/*                       = A - W(k)*(1/D(k))*W(k)**T */
			...
			i__1 = *n - k;
			d__1 = -d11;
			kher_(uplo, &i__1, &d__1, &a[k + 1 + k * a_dim1], &
				c__1, &a[k + 1 + (k + 1) * a_dim1], lda);

/*                    Store L(k) in column k */

			i__1 = *n - k;
			whscal_(&i__1, &d11, &a[k + 1 + k * a_dim1], &c__1);
		    } else {
