...
/* > */
/*  ===================================================================== */
void  hlaed3_(integer *k, integer *n, integer *n1, halfreal *
	d__, halfreal *q, integer *ldq, halfreal *rho, halfreal *dlamda,
	 halfreal *q2, integer *indx, integer *ctot, halfreal *w, 
	halfreal *s, integer *info)
{
    ...
    extern void  hgemm_(char *, char *, integer *, integer *, 
	    ...
	    *), hlaed4_(integer *, integer *, halfreal *, halfreal *, 
...
/*     this code. */
	...
	dlamda[i__] = hlamc3_(&dlamda[i__], &dlamda[i__]) - dlamda[i__];
/* L10: */
    }

    i__1 = *k;
    for (j = 1; j <= i__1; ++j) {
	hlaed4_(k, &j, &dlamda[1], &w[1], &q[j * q_dim1 + 1], rho, &d__[j], 
		info);
...
/*     Compute updated W. */
    ...
    hcopy_(k, &w[1], &c__1, &s[1], &c__1);

/*     Initialize W(I) = Q(I,I) */

    i__1 = *ldq + 1;
    hcopy_(k, &q[q_offset], &i__1, &w[1], &c__1);
    i__1 = *k;
    for (j = 1; j <= i__1; ++j) {
	i__2 = j - 1;
...
L110:
    ...
    hlacpy_("A", &n23, k, &q[ctot[1] + 1 + q_dim1], ldq, &s[1], &n23);
    iq2 = *n1 * n12 + 1;
    if (n23 != 0) {
	hgemm_("N", "N", &n2, k, &n23, &c_b22, &q2[iq2], &n2, &s[1], &n23, &
		c_b23, &q[*n1 + 1 + q_dim1], ldq);
    } else {
	hlaset_("A", &n2, k, &c_b23, &c_b23, &q[*n1 + 1 + q_dim1], ldq);
    }

    hlacpy_("A", &n12, k, &q[q_offset], ldq, &s[1], &n12);
    if (n12 != 0) {
	hgemm_("N", "N", n1, k, &n12, &c_b22, &q2[1], n1, &s[1], &n12, &c_b23,
		 &q[q_offset], ldq);
    } else {
	hlaset_("A", n1, k, &c_b23, &c_b23, &q[q_dim1 + 1], ldq);
    }
