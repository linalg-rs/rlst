...
static halfcomplex c_b44 = {-1.,0.};
static halfcomplex c_b45 = {1.,0.};
...
/* > */
/*  ===================================================================== */
void  ktgsyl_(char *trans, integer *ijob, integer *m, integer *
	n, halfcomplex *a, integer *lda, halfcomplex *b, integer *ldb, 
	halfcomplex *c__, integer *ldc, halfcomplex *d__, integer *ldd, 
	halfcomplex *e, integer *lde, halfcomplex *f, integer *ldf, 
	halfreal *scale, halfreal *dif, halfcomplex *work, integer *
	lwork, integer *iwork, integer *info)
{
    ...
    extern void  kscal_(integer *, halfcomplex *, 
	    ...
	    integer *);
    integer lwmin;
    halfreal scale2, dscale;
    extern void  ktgsy2_(char *, integer *, integer *, integer 
...
/*     Determine  optimal block sizes MB and NB */
	...
	if (*ijob >= 3) {
	    ifunc = *ijob - 2;
	    klaset_("F", m, n, &c_b1, &c_b1, &c__[c_offset], ldc);
	    klaset_("F", m, n, &c_b1, &c_b1, &f[f_offset], ldf);
...
/*        Use unblocked Level 2 solver */
	...
	for (iround = 1; iround <= i__1; ++iround) {
	    ...
	    *scale = 1.;
	    ...
	    dsum = 1.;
	    pq = *m * *n;
	    ktgsy2_(trans, &ifunc, m, n, &a[a_offset], lda, &b[b_offset], ldb,
		...
		scale2 = *scale;
		klacpy_("F", m, n, &c__[c_offset], ldc, &work[1], m);
		klacpy_("F", m, n, &f[f_offset], ldf, &work[*m * *n + 1], m);
		klaset_("F", m, n, &c_b1, &c_b1, &c__[c_offset], ldc);
		klaset_("F", m, n, &c_b1, &c_b1, &f[f_offset], ldf)
			;
	    } else if (isolve == 2 && iround == 2) {
		klacpy_("F", m, n, &work[1], m, &c__[c_offset], ldc);
		klacpy_("F", m, n, &work[*m * *n + 1], m, &f[f_offset], ldf);
		*scale = scale2;
	    }
/* L30: */
	}
...
/*           for I = P, P - 1, ..., 1; J = 1, 2, ..., Q */
	    ...
	    *scale = 1.;
		...
		for (i__ = p; i__ >= 1; --i__) {
		    ...
		    ie = iwork[i__ + 1] - 1;
		    mb = ie - is + 1;
		    ktgsy2_(trans, &ifunc, &mb, &nb, &a[is + is * a_dim1], 
			...
			*info = linfo;
			...
			for (k = 1; k <= i__3; ++k) {
			    ...
			    kscal_(m, &z__1, &c__[k * c_dim1 + 1], &c__1);
			    z__1.r = scaloc, z__1.i = 0.;
			    kscal_(m, &z__1, &f[k * f_dim1 + 1], &c__1);
/* L80: */
			}
			i__3 = je;
			for (k = js; k <= i__3; ++k) {
			    i__4 = is - 1;
			    z__1.r = scaloc, z__1.i = 0.;
			    kscal_(&i__4, &z__1, &c__[k * c_dim1 + 1], &c__1);
			    i__4 = is - 1;
			    z__1.r = scaloc, z__1.i = 0.;
			    kscal_(&i__4, &z__1, &f[k * f_dim1 + 1], &c__1);
/* L90: */
			}
			i__3 = je;
			for (k = js; k <= i__3; ++k) {
			    i__4 = *m - ie;
			    z__1.r = scaloc, z__1.i = 0.;
			    kscal_(&i__4, &z__1, &c__[ie + 1 + k * c_dim1], &
				    c__1);
			    i__4 = *m - ie;
			    z__1.r = scaloc, z__1.i = 0.;
			    kscal_(&i__4, &z__1, &f[ie + 1 + k * f_dim1], &
				    c__1);
/* L100: */
			}
			i__3 = *n;
...
/*        for I = 1,2,..., P; J = Q, Q-1,..., 1 */
	...
	*scale = 1.;
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		je = iwork[j + 1] - 1;
		nb = je - js + 1;
		ktgsy2_(trans, &ifunc, &mb, &nb, &a[is + is * a_dim1], lda, &
...
/* L170: */
			...
			i__4 = *m - ie;
			z__1.r = scaloc, z__1.i = 0.;
			kscal_(&i__4, &z__1, &c__[ie + 1 + k * c_dim1], &c__1)
				;
			i__4 = *m - ie;
			z__1.r = scaloc, z__1.i = 0.;
			kscal_(&i__4, &z__1, &f[ie + 1 + k * f_dim1], &c__1);
/* L180: */
		    }
		    i__3 = *n;
		    for (k = je + 1; k <= i__3; ++k) {
...
/*              Substitute R(I,J) and L(I,J) into remaining equation. */
		...
		if (j > p + 2) {
		    i__3 = js - 1;
		    kgemm_("N", "C", &mb, &i__3, &nb, &c_b45, &c__[is + js * 
			    c_dim1], ldc, &b[js * b_dim1 + 1], ldb, &c_b45, &
			    f[is + f_dim1], ldf);
		    i__3 = js - 1;
		    kgemm_("N", "C", &mb, &i__3, &nb, &c_b45, &f[is + js * 
			    f_dim1], ldf, &e[js * e_dim1 + 1], lde, &c_b45, &
			    f[is + f_dim1], ldf);
		}
		if (i__ < p) {
		    i__3 = *m - ie;
		    kgemm_("C", "N", &i__3, &nb, &mb, &c_b44, &a[is + (ie + 1)
			     * a_dim1], lda, &c__[is + js * c_dim1], ldc, &
			    c_b45, &c__[ie + 1 + js * c_dim1], ldc);
		    i__3 = *m - ie;
		    kgemm_("C", "N", &i__3, &nb, &mb, &c_b44, &d__[is + (ie + 
