...
/*  ===================================================================== */
void  wgeevx_(char *balanc, char *jobvl, char *jobvr, char *
	sense, integer *n, quadcomplex *a, integer *lda, quadcomplex *w, 
	quadcomplex *vl, integer *ldvl, quadcomplex *vr, integer *ldvr, 
	integer *ilo, integer *ihi, quadreal *scale, quadreal *abnrm, 
	quadreal *rconde, quadreal *rcondv, quadcomplex *work, integer *
	lwork, quadreal *rwork, integer *info)
{
    ...
    extern void  qlascl_(char *, integer *, integer *, 
	    ...
	    integer *, integer *), wgebak_(char *, char *, integer *, 
	    integer *, integer *, quadreal *, integer *, quadcomplex *, 
	    integer *, integer *), wgebal_(char *, integer *, 
    ...
    extern void  wtrsna_(char *, char *, logical *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadreal *, quadreal *, integer *,
	     integer *, quadcomplex *, integer *, quadreal *, integer *), wunghr_(integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, quadcomplex *, 
	    integer *, integer *);
    logical wntsnn, wntsnv;
    extern void  wtrevc3_(char *, char *, logical *, integer *,
	     quadcomplex *, integer *, quadcomplex *, integer *, 
...
/*       the worst case.) */
	...
	if (*n == 0) {
	    ...
	    if (wantvl) {
		wtrevc3_("L", "B", select, n, &a[a_offset], lda, &vl[
		...
		whseqr_("S", "V", n, &c__1, n, &a[a_offset], lda, &w[1], &vl[
			vl_offset], ldvl, &work[1], &c_n1, info);
	    } else if (wantvr) {
		wtrevc3_("R", "B", select, n, &a[a_offset], lda, &vl[
			vl_offset], ldvl, &vr[vr_offset], ldvr, n, &nout, &
			work[1], &c_n1, &rwork[1], &c_n1, &ierr);
		lwork_trevc__ = (integer) work[1].r;
		maxwrk = f2cmax(maxwrk,lwork_trevc__);
		whseqr_("S", "V", n, &c__1, n, &a[a_offset], lda, &w[1], &vr[
			vr_offset], ldvr, &work[1], &c_n1, info);
	    } else {
		if (wntsnn) {
		    whseqr_("E", "N", n, &c__1, n, &a[a_offset], lda, &w[1], &
			    vr[vr_offset], ldvr, &work[1], &c_n1, info);
		} else {
		    whseqr_("S", "N", n, &c__1, n, &a[a_offset], lda, &w[1], &
			    vr[vr_offset], ldvr, &work[1], &c_n1, info);
		}
	    }
...
/*     Balance the matrix and compute ABNRM */
    ...
    wgebal_(balanc, n, &a[a_offset], lda, ilo, ihi, &scale[1], &ierr);
    *abnrm = wlange_("1", n, n, &a[a_offset], lda, dum);
    if (scalea) {
	dum[0] = *abnrm;
...
/*        Copy Householder vectors to VL */
	...
	*(unsigned char *)side = 'L';
	wlacpy_("L", n, n, &a[a_offset], lda, &vl[vl_offset], ldvl)
		;
...
/*        (RWorkspace: none) */
	...
	i__1 = *lwork - iwrk + 1;
	wunghr_(n, ilo, ihi, &vl[vl_offset], ldvl, &work[itau], &work[iwrk], &
		i__1, &ierr);
...
/*        (RWorkspace: none) */
	...
	iwrk = itau;
	i__1 = *lwork - iwrk + 1;
	whseqr_("S", "V", n, ilo, ihi, &a[a_offset], lda, &w[1], &vl[
...
/*           Copy Schur vectors to VR */
	    ...
	    *(unsigned char *)side = 'B';
	    wlacpy_("F", n, n, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr);
	}
...
/*        Copy Householder vectors to VR */
	...
	*(unsigned char *)side = 'R';
	wlacpy_("L", n, n, &a[a_offset], lda, &vr[vr_offset], ldvr)
		;
...
/*        (RWorkspace: none) */
	...
	i__1 = *lwork - iwrk + 1;
	wunghr_(n, ilo, ihi, &vr[vr_offset], ldvr, &work[itau], &work[iwrk], &
		i__1, &ierr);
...
/*        (RWorkspace: none) */
	...
	iwrk = itau;
	i__1 = *lwork - iwrk + 1;
	whseqr_("S", "V", n, ilo, ihi, &a[a_offset], lda, &w[1], &vr[
...
/*        (RWorkspace: none) */
	...
	iwrk = itau;
	i__1 = *lwork - iwrk + 1;
	whseqr_(job, "N", n, ilo, ihi, &a[a_offset], lda, &w[1], &vr[
		vr_offset], ldvr, &work[iwrk], &i__1, info);
    }
...
/*        (RWorkspace: need N) */
	...
	i__1 = *lwork - iwrk + 1;
	wtrevc3_(side, "B", select, n, &a[a_offset], lda, &vl[vl_offset], 
...
/*     (RWorkspace: need 2*N unless SENSE = 'E') */
    ...
    if (! wntsnn) {
	wtrsna_(sense, "A", select, n, &a[a_offset], lda, &vl[vl_offset], 
...
/*        Undo balancing of left eigenvectors */
	...
	wgebak_(balanc, "L", n, ilo, ihi, &scale[1], n, &vl[vl_offset], ldvl, 
		&ierr);
...
/*        Normalize left eigenvectors and make largest component doublereal */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    scl = 1. / qwnrm2_(n, &vl[i__ * vl_dim1 + 1], &c__1);
	    wqscal_(n, &scl, &vl[i__ * vl_dim1 + 1], &c__1);
	    i__2 = *n;
	    for (k = 1; k <= i__2; ++k) {
		i__3 = k + i__ * vl_dim1;
/* Computing 2nd power */
...
/*        Undo balancing of right eigenvectors */
	...
	wgebak_(balanc, "R", n, ilo, ihi, &scale[1], n, &vr[vr_offset], ldvr, 
		&ierr);
...
/*        Normalize right eigenvectors and make largest component doublereal */
	...
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    scl = 1. / qwnrm2_(n, &vr[i__ * vr_dim1 + 1], &c__1);
	    wqscal_(n, &scl, &vr[i__ * vr_dim1 + 1], &c__1);
	    i__2 = *n;
	    for (k = 1; k <= i__2; ++k) {
		i__3 = k + i__ * vr_dim1;
/* Computing 2nd power */
...
/* L30: */
	    ...
	    z__1.r = z__2.r / d__1, z__1.i = z__2.i / d__1;
	    tmp.r = z__1.r, tmp.i = z__1.i;
	    wscal_(n, &tmp, &vr[i__ * vr_dim1 + 1], &c__1);
	    i__2 = k + i__ * vr_dim1;
	    i__3 = k + i__ * vr_dim1;
	    d__1 = vr[i__3].r;
	    z__1.r = d__1, z__1.i = 0.;
...
/* Computing MAX */
	i__3 = *n - *info;
	i__2 = f2cmax(i__3,1);
	wlascl_("G", &c__0, &c__0, &cscale, &anrm, &i__1, &c__1, &w[*info + 1]
		, &i__2, &ierr);
	if (*info == 0) {
	    ...
	    i__1 = *ilo - 1;
	    wlascl_("G", &c__0, &c__0, &cscale, &anrm, &i__1, &c__1, &w[1], n,
		     &ierr);
	}
    }
