...
static quadreal c_b11 = 0.;
static quadreal c_b12 = 1.;
static integer c__1 = 1;
static integer c__2 = 2;

/* > \brief \b DLASDA computes the singular value decomposition (SVD) of a doublereal upper bidiagonal matrix with d
...
/* > value decomposition (SVD) of a doublereal upper bidiagonal N-by-M matrix */
...
/* > */
/*  ===================================================================== */
void  qlasda_(integer *icompq, integer *smlsiz, integer *n, 
	integer *sqre, quadreal *d__, quadreal *e, quadreal *u, integer 
	*ldu, quadreal *vt, integer *k, quadreal *difl, quadreal *difr, 
	quadreal *z__, quadreal *poles, integer *givptr, integer *givcol, 
	integer *ldgcol, integer *perm, quadreal *givnum, quadreal *c__, 
	quadreal *s, quadreal *work, integer *iwork, integer *info)
{
    ...
    extern void  qcopy_(integer *, quadreal *, integer *, 
	    quadreal *, integer *);
    integer sqrei;
    extern void  qlasd6_(integer *, integer *, integer *, 
    ...
    extern void  qlasdq_(char *, integer *, integer *, integer 
	    *, integer *, integer *, quadreal *, quadreal *, quadreal *,
	    ...
	    quadreal *, integer *), qlasdt_(integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *), qlaset_(
	    char *, integer *, integer *, quadreal *, quadreal *, 
...
/*     If the input matrix is too small, call DLASDQ to find the SVD. */
    ...
    if (*n <= *smlsiz) {
	if (*icompq == 0) {
	    qlasdq_("U", sqre, n, &c__0, &c__0, &c__0, &d__[1], &e[1], &vt[
		    vt_offset], ldu, &u[u_offset], ldu, &u[u_offset], ldu, &
		    work[1], info);
	} else {
	    qlasdq_("U", sqre, n, &m, n, &c__0, &d__[1], &e[1], &vt[vt_offset]
...
/*     Book-keeping and  set up the computation tree. */
    ...
    qlasdt_(n, &nlvl, &nd, &iwork[inode], &iwork[ndiml], &iwork[ndimr], 
	    smlsiz);
...
/*        NRF: starting row of the right  subproblem */
	...
	sqrei = 1;
	if (*icompq == 0) {
	    qlaset_("A", &nlp1, &nlp1, &c_b11, &c_b12, &work[nwork1], &smlszp);
	    qlasdq_("U", &sqrei, &nl, &nlp1, &nru, &ncc, &d__[nlf], &e[nlf], &
		    work[nwork1], &smlszp, &work[nwork2], &nl, &work[nwork2], 
		    &nl, &work[nwork2], info);
	    itemp = nwork1 + nl * smlszp;
	    qcopy_(&nlp1, &work[nwork1], &c__1, &work[vfi], &c__1);
	    qcopy_(&nlp1, &work[itemp], &c__1, &work[vli], &c__1);
	} else {
	    qlaset_("A", &nl, &nl, &c_b11, &c_b12, &u[nlf + u_dim1], ldu);
	    qlaset_("A", &nlp1, &nlp1, &c_b11, &c_b12, &vt[nlf + vt_dim1], 
		    ldu);
	    qlasdq_("U", &sqrei, &nl, &nlp1, &nl, &ncc, &d__[nlf], &e[nlf], &
		    vt[nlf + vt_dim1], ldu, &u[nlf + u_dim1], ldu, &u[nlf + 
		    u_dim1], ldu, &work[nwork1], info);
	    qcopy_(&nlp1, &vt[nlf + vt_dim1], &c__1, &work[vfi], &c__1);
	    qcopy_(&nlp1, &vt[nlf + nlp1 * vt_dim1], &c__1, &work[vli], &c__1)
		    ;
	}
	if (*info != 0) {
	    return;
...
/* L10: */
	...
	nrp1 = nr + sqrei;
	if (*icompq == 0) {
	    qlaset_("A", &nrp1, &nrp1, &c_b11, &c_b12, &work[nwork1], &smlszp);
	    qlasdq_("U", &sqrei, &nr, &nrp1, &nru, &ncc, &d__[nrf], &e[nrf], &
		    work[nwork1], &smlszp, &work[nwork2], &nr, &work[nwork2], 
		    &nr, &work[nwork2], info);
	    itemp = nwork1 + (nrp1 - 1) * smlszp;
	    qcopy_(&nrp1, &work[nwork1], &c__1, &work[vfi], &c__1);
	    qcopy_(&nrp1, &work[itemp], &c__1, &work[vli], &c__1);
	} else {
	    qlaset_("A", &nr, &nr, &c_b11, &c_b12, &u[nrf + u_dim1], ldu);
	    qlaset_("A", &nrp1, &nrp1, &c_b11, &c_b12, &vt[nrf + vt_dim1], 
		    ldu);
	    qlasdq_("U", &sqrei, &nr, &nrp1, &nr, &ncc, &d__[nrf], &e[nrf], &
		    vt[nrf + vt_dim1], ldu, &u[nrf + u_dim1], ldu, &u[nrf + 
		    u_dim1], ldu, &work[nwork1], info);
	    qcopy_(&nrp1, &vt[nrf + vt_dim1], &c__1, &work[vfi], &c__1);
	    qcopy_(&nrp1, &vt[nrf + nrp1 * vt_dim1], &c__1, &work[vli], &c__1)
		    ;
	}
	if (*info != 0) {
	    return;
...
/*        the current level LVL. */
	...
	for (i__ = lf; i__ <= i__1; ++i__) {
	    ...
	    beta = e[ic];
	    if (*icompq == 0) {
		qlasd6_(icompq, &nl, &nr, &sqrei, &d__[nlf], &work[vfi], &
