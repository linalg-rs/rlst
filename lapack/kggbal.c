...
static halfreal c_b36 = 10.;
static halfreal c_b72 = .5;
...
/* > */
/*  ===================================================================== */
void  kggbal_(char *job, integer *n, halfcomplex *a, integer 
	*lda, halfcomplex *b, integer *ldb, integer *ilo, integer *ihi, 
...
L160:
    ...
    i__1 = *n - k + 1;
    kswap_(&i__1, &a[i__ + k * a_dim1], lda, &a[m + k * a_dim1], lda);
    i__1 = *n - k + 1;
    kswap_(&i__1, &b[i__ + k * b_dim1], ldb, &b[m + k * b_dim1], ldb);
...
L170:
    ...
    kswap_(&l, &a[j * a_dim1 + 1], &c__1, &a[m * a_dim1 + 1], &c__1);
    kswap_(&l, &b[j * b_dim1 + 1], &c__1, &b[m * b_dim1 + 1], &c__1);
...
L350:
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	irab = ikamax_(&i__2, &a[i__ + *ilo * a_dim1], lda);
	rab = z_abs(&a[i__ + (irab + *ilo - 1) * a_dim1]);
	i__2 = *n - *ilo + 1;
	irab = ikamax_(&i__2, &b[i__ + *ilo * b_dim1], ldb);
/* Computing MAX */
...
/* Computing MIN */
	...
	icab = ikamax_(ihi, &a[i__ * a_dim1 + 1], &c__1);
	cab = z_abs(&a[icab + i__ * a_dim1]);
	icab = ikamax_(ihi, &b[i__ * b_dim1 + 1], &c__1);
/* Computing MAX */
...
/*     Row scaling of matrices A and B */
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	whscal_(&i__2, &lscale[i__], &a[i__ + *ilo * a_dim1], lda);
	i__2 = *n - *ilo + 1;
	whscal_(&i__2, &lscale[i__], &b[i__ + *ilo * b_dim1], ldb);
/* L370: */
    }
...
/*     Column scaling of matrices A and B */
    ...
    i__1 = *ihi;
    for (j = *ilo; j <= i__1; ++j) {
	whscal_(ihi, &rscale[j], &a[j * a_dim1 + 1], &c__1);
	whscal_(ihi, &rscale[j], &b[j * b_dim1 + 1], &c__1);
/* L380: */
    }
