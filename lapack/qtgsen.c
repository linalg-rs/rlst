...
/* > \verbatim */
/* > */
/* > DTGSEN reorders the generalized doublereal Schur decomposition of a doublereal */
/* > matrix pair (A, B) (in terms of an orthonormal equivalence trans- */
/* > formation Q**T * (A, B) * Z), so that a selected cluster of eigenvalues */
/* > appears in the leading diagonal blocks of the upper quasi-triangular */
/* > matrix A and the upper triangular B. The leading columns of Q and */
/* > Z form orthonormal bases of the corresponding left and right eigen- */
/* > spaces (deflating subspaces). (A, B) must be in generalized doublereal */
...
/* >          To select a doublereal eigenvalue w(j), SELECT(j) must be set to */
...
/* >          generalized doublereal Schur canonical form. */
...
/* >          the doublereal generalized Schur form of (A,B) were further reduced */
...
/* > */
/* >  If (A, B) has been obtained from the generalized doublereal Schur */
/* >  decomposition of a matrix pair (C, D) = Q*(A, B)*Z**T, then the */
/* >  reordered generalized doublereal Schur form of (C, D) is given by */
/* > */
...
/* > */
/*  ===================================================================== */
void  qtgsen_(integer *ijob, logical *wantq, logical *wantz, 
	logical *select, integer *n, quadreal *a, integer *lda, quadreal *
	b, integer *ldb, quadreal *alphar, quadreal *alphai, quadreal *
	beta, quadreal *q, integer *ldq, quadreal *z__, integer *ldz, 
	integer *m, quadreal *pl, quadreal *pr, quadreal *dif, 
	quadreal *work, integer *lwork, integer *iwork, integer *liwork, 
	integer *info)
{
    ...
    quadreal dsum;
    logical swap;
    extern void  qlag2_(quadreal *, integer *, quadreal *, 
...
/*     Quick return if possible. */
	...
	if (wantd) {
		...
		qlassq_(n, &a[i__ * a_dim1 + 1], &c__1, &dscale, &dsum);
		qlassq_(n, &b[i__ * b_dim1 + 1], &c__1, &dscale, &dsum);
/* L20: */
	    }
	    dif[1] = dscale * M(sqrt)(dsum);
	    dif[2] = dif[1];
	}
	goto L60;
    }
...
/*              Q and Z accordingly (if requested): */
		...
		kk = k;
		if (k != ks) {
		    qtgexc_(wantq, wantz, n, &a[a_offset], lda, &b[b_offset], 
...
/*        and compute PL and PR. */
	...
	i__ = n1 + 1;
	ijb = 0;
	qlacpy_("Full", &n1, &n2, &a[i__ * a_dim1 + 1], lda, &work[1], &n1);
	qlacpy_("Full", &n1, &n2, &b[i__ * b_dim1 + 1], ldb, &work[n1 * n2 + 
		1], &n1);
	i__1 = *lwork - (n1 << 1) * n2;
	qtgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * a_dim1]
...
/*        and right eigenspaces. */
	...
	dsum = 1.;
	i__1 = n1 * n2;
	qlassq_(&i__1, &work[1], &c__1, &rdscal, &dsum);
	*pl = rdscal * M(sqrt)(dsum);
	if (*pl == 0.) {
	    *pl = 1.;
	} else {
	    *pl = dscale / (M(sqrt)(dscale * dscale / *pl + *pl) * M(sqrt)(*pl));
	}
	rdscal = 0.;
	dsum = 1.;
	i__1 = n1 * n2;
	qlassq_(&i__1, &work[n1 * n2 + 1], &c__1, &rdscal, &dsum);
	*pr = rdscal * M(sqrt)(dsum);
	if (*pr == 0.) {
	    *pr = 1.;
	} else {
	    *pr = dscale / (M(sqrt)(dscale * dscale / *pr + *pr) * M(sqrt)(*pr));
	}
    }
...
/*           Frobenius norm-based Difu-estimate. */
	    ...
	    i__1 = *lwork - (n1 << 1) * n2;
	    qtgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * 
...
/*           Frobenius norm-based Difl-estimate. */
	    ...
	    i__1 = *lwork - (n1 << 1) * n2;
	    qtgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, &a[
...
L40:
	    qlacn2_(&mn2, &work[mn2 + 1], &work[1], &iwork[1], &dif[1], &kase,
		     isave);
	    if (kase != 0) {
		if (kase == 1) {
...
/*                 Solve generalized Sylvester equation. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    qtgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + 
...
/*                 Solve the transposed variant. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    qtgsyl_("T", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + 
			    i__ * a_dim1], lda, &work[1], &n1, &b[b_offset], 
			    ldb, &b[i__ + i__ * b_dim1], ldb, &work[n1 * n2 + 
			    1], &n1, &dscale, &dif[1], &work[(n1 << 1) * n2 + 
			    1], &i__1, &iwork[1], &ierr);
...
L50:
	    qlacn2_(&mn2, &work[mn2 + 1], &work[1], &iwork[1], &dif[2], &kase,
		     isave);
	    if (kase != 0) {
		if (kase == 1) {
...
/*                 Solve generalized Sylvester equation. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    qtgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, 
...
/*                 Solve the transposed variant. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    qtgsyl_("T", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, 
			    &a[a_offset], lda, &work[1], &n2, &b[i__ + i__ * 
			    b_dim1], ldb, &b[b_offset], ldb, &work[n1 * n2 + 
			    1], &n2, &dscale, &dif[2], &work[(n1 << 1) * n2 + 
			    1], &i__1, &iwork[1], &ierr);
...
/*             Compute the eigenvalue(s) at position K. */
		...
		d__1 = smlnum * eps;
		qlag2_(&work[1], &c__2, &work[5], &c__2, &d__1, &beta[k], &
