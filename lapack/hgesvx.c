...
/* > \verbatim */
/* > */
/* > DGESVX uses the LU factorization to compute the solution to a doublereal */
...
/*  ===================================================================== */
void  hgesvx_(char *fact, char *trans, integer *n, integer *
	nrhs, halfreal *a, integer *lda, halfreal *af, integer *ldaf, 
	integer *ipiv, char *equed, halfreal *r__, halfreal *c__, 
	halfreal *b, integer *ldb, halfreal *x, integer *ldx, halfreal *
	rcond, halfreal *ferr, halfreal *berr, halfreal *work, integer *
	iwork, integer *info)
{
    ...
    extern void  hlaqge_(integer *, integer *, halfreal *, 
	    integer *, halfreal *, halfreal *, halfreal *, halfreal *,
	     halfreal *, char *), hgecon_(char *, integer *, 
	    halfreal *, integer *, halfreal *, halfreal *, halfreal *,
	     integer *, integer *);
    halfreal colcnd;
    logical nofact;
    extern void  hgeequ_(integer *, integer *, halfreal *, 
	    integer *, halfreal *, halfreal *, halfreal *, halfreal *,
	     halfreal *, integer *), hgerfs_(char *, integer *, integer *, 
	    halfreal *, integer *, halfreal *, integer *, integer *, 
	    halfreal *, integer *, halfreal *, integer *, halfreal *, 
	    halfreal *, halfreal *, integer *, integer *), 
	    hgetrf_(integer *, integer *, halfreal *, integer *, integer *, 
	    integer *), hlacpy_(char *, integer *, integer *, halfreal *, 
	    integer *, halfreal *, integer *), xerbla_(char *, 
	    integer *);
    halfreal bignum;
    extern halfreal hlantr_(char *, char *, char *, integer *, integer *, 
	    halfreal *, integer *, halfreal *);
    integer infequ;
    logical colequ;
    extern void  hgetrs_(char *, integer *, integer *, 
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	hgeequ_(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &colcnd, &
		amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    hlaqge_(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &
		    colcnd, &amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed,
		     "B");
	    colequ = lsame_(equed, "C") || lsame_(equed,
...
/*        Compute the LU factorization of A. */
	...
	hlacpy_("Full", n, n, &a[a_offset], lda, &af[af_offset], ldaf);
	hgetrf_(n, n, &af[af_offset], ldaf, &ipiv[1], info);
...
/*           leading rank-deficient INFO columns of A. */
	    ...
	    rpvgrw = hlantr_("M", "U", "N", info, info, &af[af_offset], ldaf, 
		    &work[1]);
	    if (rpvgrw == 0.) {
		rpvgrw = 1.;
	    } else {
		rpvgrw = hlange_("M", n, info, &a[a_offset], lda, &work[1]) / rpvgrw;
	    }
	    work[1] = rpvgrw;
	    *rcond = 0.;
	    return;
...
/*     reciprocal pivot growth factor RPVGRW. */
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = hlange_(norm, n, n, &a[a_offset], lda, &work[1]);
    rpvgrw = hlantr_("M", "U", "N", n, n, &af[af_offset], ldaf, &work[1]);
    if (rpvgrw == 0.) {
	rpvgrw = 1.;
    } else {
	rpvgrw = hlange_("M", n, n, &a[a_offset], lda, &work[1]) / 
		rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    hgecon_(norm, n, &af[af_offset], ldaf, &anorm, rcond, &work[1], &iwork[1],
	     info);

/*     Compute the solution matrix X. */

    hlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    hgetrs_(trans, n, nrhs, &af[af_offset], ldaf, &ipiv[1], &x[x_offset], ldx,
	     info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    hgerfs_(trans, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
