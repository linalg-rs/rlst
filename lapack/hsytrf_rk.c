...
/*  ===================================================================== */
void  dsytrf_rk__(char *uplo, integer *n, halfreal *a, 
	...
	*lwork, integer *info)
{
    ...
    /* Local variables */
    integer i__, k;
    extern void  dsytf2_rk__(char *, integer *, halfreal *, 
	    integer *, halfreal *, integer *, integer *), 
	    dlasyf_rk__(char *, integer *, integer *, integer *, halfreal *,
...
/*        of the interchange with row i in both 1x1 and 2x2 pivot cases) */
	...
	if (k < *n) {
		...
		if (ip != i__) {
		    i__2 = *n - k;
		    hswap_(&i__2, &a[i__ + (k + 1) * a_dim1], lda, &a[ip + (k 
			    + 1) * a_dim1], lda);
		}
	    }
	}
...
/*        of the interchange with row i in both 1x1 and 2x2 pivot cases) */
	...
	if (k > 1) {
		...
		if (ip != i__) {
		    i__2 = k - 1;
		    hswap_(&i__2, &a[i__ + a_dim1], lda, &a[ip + a_dim1], lda)
			    ;
		}
	    }
	}
