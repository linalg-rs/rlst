...
/* > */
/*  ===================================================================== */
void  qtgsja_(char *jobu, char *jobv, char *jobq, integer *m, 
	integer *p, integer *n, integer *k, integer *l, quadreal *a, 
	integer *lda, quadreal *b, integer *ldb, quadreal *tola, 
	quadreal *tolb, quadreal *alpha, quadreal *beta, quadreal *u, 
	integer *ldu, quadreal *v, integer *ldv, quadreal *q, integer *
	ldq, quadreal *work, integer *ncycle, integer *info)
{
    ...
    /* Local variables */
    integer i__, j;
    quadreal a1, a2, a3, b1, b2, b3, csq, csu, csv, snq, rwk, snu, snv;
    ...
    logical wantu, wantv;
    extern void  qlags2_(logical *, quadreal *, quadreal *,
	     quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, quadreal *), qlapll_(integer *, quadreal *, 
	    integer *, quadreal *, integer *, quadreal *);
    integer kcycle;
    extern void  qlartg_(quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *), qlaset_(char *, 
...
/*     Initialize U, V and Q, if necessary */
    ...
    if (initu) {
	qlaset_("Full", m, m, &c_b13, &c_b14, &u[u_offset], ldu);
    }
    if (initv) {
	qlaset_("Full", p, p, &c_b13, &c_b14, &v[v_offset], ldv);
    }
    if (initq) {
	qlaset_("Full", n, n, &c_b13, &c_b14, &q[q_offset], ldq);
    }
...
/*              Update (K+I)-th and (K+J)-th rows of matrix A: U**T *A */
		...
		if (*k + j <= *m) {
		    qrot_(l, &a[*k + j + (*n - *l + 1) * a_dim1], lda, &a[*k 
			    + i__ + (*n - *l + 1) * a_dim1], lda, &csu, &snu);
		}

/*              Update I-th and J-th rows of matrix B: V**T *B */

		qrot_(l, &b[j + (*n - *l + 1) * b_dim1], ldb, &b[i__ + (*n - *
...
/* Computing MIN */
		i__4 = *k + *l;
		i__3 = f2cmin(i__4,*m);
		qrot_(&i__3, &a[(*n - *l + j) * a_dim1 + 1], &c__1, &a[(*n - *
			l + i__) * a_dim1 + 1], &c__1, &csq, &snq);

		qrot_(l, &b[(*n - *l + j) * b_dim1 + 1], &c__1, &b[(*n - *l + 
...
/*              Update orthogonal matrices U, V, Q, if desired. */
		...
		if (wantu && *k + j <= *m) {
		    qrot_(m, &u[(*k + j) * u_dim1 + 1], &c__1, &u[(*k + i__) *
			     u_dim1 + 1], &c__1, &csu, &snu);
		}

		if (wantv) {
		    qrot_(p, &v[j * v_dim1 + 1], &c__1, &v[i__ * v_dim1 + 1], 
			    &c__1, &csv, &snv);
		}

		if (wantq) {
		    qrot_(n, &q[(*n - *l + j) * q_dim1 + 1], &c__1, &q[(*n - *
...
/* Computing MIN */
	    ...
	    for (i__ = 1; i__ <= i__1; ++i__) {
		i__2 = *l - i__ + 1;
		qcopy_(&i__2, &a[*k + i__ + (*n - *l + i__) * a_dim1], lda, &
			work[1], &c__1);
		i__2 = *l - i__ + 1;
		qcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &work[*
			l + 1], &c__1);
		i__2 = *l - i__ + 1;
...
/*           change sign if necessary */
	    ...
	    if (gamma < 0.) {
		i__2 = *l - i__ + 1;
		qscal_(&i__2, &c_b43, &b[i__ + (*n - *l + i__) * b_dim1], ldb)
			;
		if (wantv) {
		    qscal_(p, &c_b43, &v[i__ * v_dim1 + 1], &c__1);
		}
	    }

	    d__1 = abs(gamma);
	    qlartg_(&d__1, &c_b14, &beta[*k + i__], &alpha[*k + i__], &rwk);

	    if (alpha[*k + i__] >= beta[*k + i__]) {
		i__2 = *l - i__ + 1;
		d__1 = 1. / alpha[*k + i__];
		qscal_(&i__2, &d__1, &a[*k + i__ + (*n - *l + i__) * a_dim1], 
			lda);
	    } else {
		i__2 = *l - i__ + 1;
		d__1 = 1. / beta[*k + i__];
		qscal_(&i__2, &d__1, &b[i__ + (*n - *l + i__) * b_dim1], ldb);
		i__2 = *l - i__ + 1;
		qcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &a[*k 
