...
/* > */
/*  ===================================================================== */
void  hlaed4_(integer *n, integer *i__, halfreal *d__, 
	halfreal *z__, halfreal *delta, halfreal *rho, halfreal *dlam,
	 integer *info)
{
    ...
    halfreal dw, zz[3];
    integer ip1;
    halfreal del, eta, phi, eps, tau, psi;
    integer iim1, iip1;
    halfreal dphi, dpsi;
    integer iter;
    halfreal temp, prew, temp1, dltlb, dltub, midpt;
    integer niter;
    logical swtch;
    extern void  hlaed5_(integer *, halfreal *, halfreal *,
	     halfreal *, halfreal *, halfreal *), hlaed6_(integer *, 
	    logical *, halfreal *, halfreal *, halfreal *, halfreal *,
	     halfreal *, integer *);
    logical swtch3;
    extern halfreal hlamch_(char *);
    logical orgati;
    halfreal erretm, rhoinv;
...
/*         Presumably, I=1 upon entry */
	...
	*dlam = d__[1] + *rho * z__[1] * z__[1];
    ...
    if (*n == 2) {
	hlaed5_(i__, &d__[1], &z__[1], &delta[1], rho, dlam);
	return;
    }
...
/*           Interpolation using THREE most relevant poles */
	    ...
	    zz[1] = z__[ii] * z__[ii];
	    hlaed6_(&niter, &orgati, &c__, &delta[iim1], zz, &w, &eta, info);
	    if (*info != 0) {
		goto L250;
	    }
	}
...
/*              Interpolation using THREE most relevant poles */
		...
		hlaed6_(&niter, &orgati, &c__, &delta[iim1], zz, &w, &eta, 
			info);
		if (*info != 0) {
		    goto L250;
		}
