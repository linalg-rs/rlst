...
/* > \verbatim */
/* > */
/* > To find the desired eigenvalues of a given doublereal symmetric */
...
/* > */
/*  ===================================================================== */
void  hlarre_(char *range, integer *n, halfreal *vl, 
	halfreal *vu, integer *il, integer *iu, halfreal *d__, halfreal 
	*e, halfreal *e2, halfreal *rtol1, halfreal *rtol2, halfreal *
	spltol, integer *nsplit, integer *isplit, integer *m, halfreal *w, 
	halfreal *werr, halfreal *wgap, integer *iblock, integer *indexw, 
	halfreal *gers, halfreal *pivmin, halfreal *work, integer *
	iwork, integer *info)
{
    ...
    extern void  hlarrd_(char *, char *, integer *, halfreal 
	    *, halfreal *, integer *, integer *, halfreal *, halfreal *,
	    ...
	    integer *);
    halfreal safmin, spdiam;
    extern void  hlarrk_(integer *, integer *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, halfreal *, 
	    halfreal *, halfreal *, halfreal *, integer *);
    logical usedqd;
    halfreal clwdth, isleft;
    extern void  hlarnv_(integer *, integer *, integer *, 
	    halfreal *);
    halfreal isrght, bsrtol, dpivot;
...
/*     Compute splitting points */
    hlarra_(n, &d__[1], &e[1], &e2[1], spltol, &spdiam, nsplit, &isplit[1], &
	    iinfo);
...
/*        DLARRD needs a WORK of size 4*N, IWORK of size 3*N */
	hlarrd_(range, "B", n, vl, vu, il, iu, &gers[1], &bsrtol, &d__[1], &e[
...
/*              Decide whether dqds or bisection is more efficient */
		usedqd = (halfreal) mb > in * .5 && ! forceb;
		wend = wbegin + mb - 1;
...
/*           Find approximations to the extremal eigenvalues of the block */
	    hlarrk_(&in, &c__1, &gl, &gu, &d__[ibegin], &e2[ibegin], pivmin, &
		    rtl, &tmp, &tmp1, &iinfo);
	    if (iinfo != 0) {
		*info = -1;
		return;
...
/* Computing MAX */
	    ...
	    hlarrk_(&in, &in, &gl, &gu, &d__[ibegin], &e2[ibegin], pivmin, &
		    rtl, &tmp, &tmp1, &iinfo);
	    if (iinfo != 0) {
		*info = -1;
		return;
...
/*        Compute the negcount at the 1/4 and 3/4 points */
	if (mb > 1) {
	    hlarrc_("T", &in, &s1, &s2, &d__[ibegin], &e[ibegin], pivmin, &
		    cnt, &cnt1, &cnt2, &iinfo);
	}
	if (mb == 1) {
	    sigma = gl;
...
/* Computing MAX */
		...
		avgap = (d__1 = clwdth / (halfreal) (wend - wbegin), abs(
			d__1));
		if (sgndef == 1.) {
/* Computing MAX */
		    d__1 = wgap[wbegin];
...
/*              here doublereally only in the case of IRANGE = VALRNG or INDRNG. */
		if (idum == 5) {
		    if (sgndef == 1.) {
...
/*        Store the shift. */
	e[iend] = sigma;
/*        Store D and L. */
	hcopy_(&in, &work[1], &c__1, &d__[ibegin], &c__1);
	i__2 = in - 1;
	hcopy_(&i__2, &work[in + 1], &c__1, &e[ibegin], &c__1);
	if (mb > 1) {
...
/* L122: */
	    ...
	    i__2 = (in << 1) - 1;
	    hlarnv_(&c__2, iseed, &i__2, &work[1]);
	    i__2 = in - 1;
	    for (i__ = 1; i__ <= i__2; ++i__) {
...
/*           use bisection to find EV from INDL to INDU */
	    i__2 = indl - 1;
	    hlarrb_(&in, &d__[ibegin], &work[ibegin], &indl, &indu, rtol1, 
