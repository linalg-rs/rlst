...
static halfreal c_b63 = 0.;
static integer c__1 = 1;
static halfreal c_b84 = 1.;
...
/* > */
/*  ===================================================================== */
void  hgesdd_(char *jobz, integer *m, integer *n, halfreal *
	a, integer *lda, halfreal *s, halfreal *u, integer *ldu, 
	halfreal *vt, integer *ldvt, halfreal *work, integer *lwork, 
	integer *iwork, integer *info)
{
    ...
    extern void  hbdsdc_(char *, char *, integer *, halfreal 
	    *, halfreal *, halfreal *, integer *, halfreal *, integer *,
	     halfreal *, integer *, halfreal *, integer *, integer *), hgebrd_(integer *, integer *, halfreal *, 
    ...
    extern void  hgelqf_(integer *, integer *, halfreal *, 
	    ...
	    xerbla_(char *, integer *), horgbr_(char *, integer *, 
	    integer *, integer *, halfreal *, integer *, halfreal *, 
	    halfreal *, integer *, integer *);
    halfreal bignum;
...
/*           Compute space preferred for each routine */
	    hgebrd_(m, n, dum, m, dum, dum, dum, dum, dum, &c_n1, &ierr);
	    lwork_dgebrd_mn__ = (integer) dum[0];

	    hgebrd_(n, n, dum, n, dum, dum, dum, dum, dum, &c_n1, &ierr);
	    lwork_dgebrd_nn__ = (integer) dum[0];

	    hgeqrf_(m, n, dum, m, dum, dum, &c_n1, &ierr);
	    lwork_dgeqrf_mn__ = (integer) dum[0];

	    horgbr_("Q", n, n, n, dum, n, dum, dum, &c_n1, &ierr);
	    lwork_dorgbr_q_nn__ = (integer) dum[0];

	    horgqr_(m, m, n, dum, m, dum, dum, &c_n1, &ierr);
	    lwork_dorgqr_mm__ = (integer) dum[0];

	    horgqr_(m, n, n, dum, m, dum, dum, &c_n1, &ierr);
	    lwork_dorgqr_mn__ = (integer) dum[0];

	    hormbr_("P", "R", "T", n, n, n, dum, n, dum, dum, n, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_prt_nn__ = (integer) dum[0];

	    hormbr_("Q", "L", "N", n, n, n, dum, n, dum, dum, n, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_qln_nn__ = (integer) dum[0];

	    hormbr_("Q", "L", "N", m, n, n, dum, m, dum, dum, m, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_qln_mn__ = (integer) dum[0];

	    hormbr_("Q", "L", "N", m, m, n, dum, m, dum, dum, m, dum, &c_n1, &
		    ierr);
...
/*           Compute space preferred for each routine */
	    ...
	    hgebrd_(m, m, &a[a_offset], m, &s[1], dum, dum, dum, dum, &c_n1, &
		    ierr);
	    lwork_dgebrd_mm__ = (integer) dum[0];

	    hgelqf_(m, n, &a[a_offset], m, dum, dum, &c_n1, &ierr);
	    lwork_dgelqf_mn__ = (integer) dum[0];

	    horglq_(n, n, m, dum, n, dum, dum, &c_n1, &ierr);
	    lwork_dorglq_nn__ = (integer) dum[0];

	    horglq_(m, n, m, &a[a_offset], m, dum, dum, &c_n1, &ierr);
	    lwork_dorglq_mn__ = (integer) dum[0];

	    horgbr_("P", m, m, m, &a[a_offset], n, dum, dum, &c_n1, &ierr);
	    lwork_dorgbr_p_mm__ = (integer) dum[0];

	    hormbr_("P", "R", "T", m, m, m, dum, m, dum, dum, m, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_prt_mm__ = (integer) dum[0];

	    hormbr_("P", "R", "T", m, n, m, dum, m, dum, dum, m, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_prt_mn__ = (integer) dum[0];

	    hormbr_("P", "R", "T", n, n, m, dum, n, dum, dum, n, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_prt_nn__ = (integer) dum[0];

	    hormbr_("Q", "L", "N", m, m, m, dum, m, dum, dum, m, dum, &c_n1, &
		    ierr);
	    lwork_dormbr_qln_mm__ = (integer) dum[0];
...
/*              Workspace: prefer N [tau] + N*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		hgeqrf_(m, n, &a[a_offset], lda, &work[itau], &work[nwork], &
			i__1, &ierr);
...
/*              Zero out below R */
		...
		i__1 = *n - 1;
		i__2 = *n - 1;
		hlaset_("L", &i__1, &i__2, &c_b63, &c_b63, &a[a_dim1 + 2], 
			lda);
		ie = 1;
		itauq = ie + *n;
		itaup = itauq + *n;
...
/*              Workspace: need   N [e] + BDSPAC */
		...
		hbdsdc_("U", "N", n, &s[1], &work[ie], dum, &c__1, dum, &c__1,
...
/*              Copy R to WORK(IR), zeroing out below it */
		...
		i__1 = *n - 1;
		i__2 = *n - 1;
		hlaset_("L", &i__1, &i__2, &c_b63, &c_b63, &work[ir + 1], &
			ldwrkr);
...
/*              Workspace: prefer N*N [R] + N [tau] + N*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		horgqr_(m, n, n, &a[a_offset], lda, &work[itau], &work[nwork],
			 &i__1, &ierr);
		ie = itau;
		itauq = ie + *n;
		itaup = itauq + *n;
...
/*              Workspace: need   N*N [R] + 3*N [e, tauq, taup] + N*N [U] + BDSPAC */
		...
		hbdsdc_("U", "I", n, &s[1], &work[ie], &work[iu], n, &vt[
...
/*              Workspace: prefer N*N [R] + 3*N [e, tauq, taup] + N*N [U] + N*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		hormbr_("Q", "L", "N", n, n, n, &work[ir], &ldwrkr, &work[
			itauq], &work[iu], n, &work[nwork], &i__1, &ierr);
		i__1 = *lwork - nwork + 1;
		hormbr_("P", "R", "T", n, n, n, &work[ir], &ldwrkr, &work[
...
/* Computing MIN */
		    ...
		    hgemm_("N", "N", &chunk, n, n, &c_b84, &a[i__ + a_dim1], 
			    lda, &work[iu], n, &c_b63, &work[ir], &ldwrkr);
		    hlacpy_("F", &chunk, n, &work[ir], &ldwrkr, &a[i__ + 
			    a_dim1], lda);
/* L10: */
		}
...
/*              Copy R to WORK(IR), zeroing out below it */
		...
		i__2 = *n - 1;
		i__1 = *n - 1;
		hlaset_("L", &i__2, &i__1, &c_b63, &c_b63, &work[ir + 1], &
			ldwrkr);
...
/*              Workspace: need   N*N [R] + 3*N [e, tauq, taup] + BDSPAC */
		...
		hbdsdc_("U", "I", n, &s[1], &work[ie], &u[u_offset], ldu, &vt[
			vt_offset], ldvt, dum, idum, &work[nwork], &iwork[1], 
			info);
...
/*              Workspace: need   N*N [R] */
		...
		hlacpy_("F", n, n, &u[u_offset], ldu, &work[ir], &ldwrkr);
		hgemm_("N", "N", m, n, n, &c_b84, &a[a_offset], lda, &work[ir]
...
/*              Workspace: prefer N*N [U] + N [tau] + M*NB [work] */
		i__2 = *lwork - nwork + 1;
		horgqr_(m, m, n, &u[u_offset], ldu, &work[itau], &work[nwork],
			 &i__2, &ierr);
...
/*              Produce R in A, zeroing out other entries */
		...
		i__2 = *n - 1;
		i__1 = *n - 1;
		hlaset_("L", &i__2, &i__1, &c_b63, &c_b63, &a[a_dim1 + 2], 
			lda);
		ie = itau;
		itauq = ie + *n;
		itaup = itauq + *n;
...
/*              Workspace: prefer N*N [U] + 3*N [e, tauq, taup] + N*NB [work] */
		...
		i__2 = *lwork - nwork + 1;
		hormbr_("Q", "L", "N", n, n, n, &a[a_offset], lda, &work[
			itauq], &work[iu], &ldwrku, &work[nwork], &i__2, &
			ierr);
		i__2 = *lwork - nwork + 1;
		hormbr_("P", "R", "T", n, n, n, &a[a_offset], lda, &work[
			itaup], &vt[vt_offset], ldvt, &work[nwork], &i__2, &
			ierr);
...
/*              Workspace: need   N*N [U] */
		...
		hgemm_("N", "N", m, n, n, &c_b84, &u[u_offset], ldu, &work[iu]
			, &ldwrku, &c_b63, &a[a_offset], lda);

/*              Copy left singular vectors of A from A to U */

		hlacpy_("F", m, n, &a[a_offset], lda, &u[u_offset], ldu);
...
/*                 WORK( IU ) is M by N */
		    ...
		    ldwrku = *m;
		    nwork = iu + ldwrku * *n;
		    hlaset_("F", m, n, &c_b63, &c_b63, &work[iu], &ldwrku);
...
/*              Workspace: need   3*N [e, tauq, taup] + N*N [U] + BDSPAC */
		...
		hbdsdc_("U", "I", n, &s[1], &work[ie], &work[iu], &ldwrku, &
...
/*                 Workspace: prefer 3*N [e, tauq, taup] + M*N [U] + N*NB [work] */
		    ...
		    i__2 = *lwork - nwork + 1;
		    hormbr_("Q", "L", "N", m, n, n, &a[a_offset], lda, &work[
			    itauq], &work[iu], &ldwrku, &work[nwork], &i__2, &
			    ierr);

/*                 Copy left singular vectors of A from WORK(IU) to A */

		    hlacpy_("F", m, n, &work[iu], &ldwrku, &a[a_offset], lda);
		} else {
...
/* Computing MIN */
			i__3 = *m - i__ + 1;
			chunk = f2cmin(i__3,ldwrkr);
			hgemm_("N", "N", &chunk, n, n, &c_b84, &a[i__ + 
...
/*              Workspace: need   3*N [e, tauq, taup] + BDSPAC */
		...
		hlaset_("F", m, n, &c_b63, &c_b63, &u[u_offset], ldu);
		hbdsdc_("U", "I", n, &s[1], &work[ie], &u[u_offset], ldu, &vt[
			vt_offset], ldvt, dum, idum, &work[nwork], &iwork[1], 
			info);
...
/*              Workspace: need   3*N [e, tauq, taup] + BDSPAC */
		...
		hlaset_("F", m, m, &c_b63, &c_b63, &u[u_offset], ldu);
		hbdsdc_("U", "I", n, &s[1], &work[ie], &u[u_offset], ldu, &vt[
			vt_offset], ldvt, dum, idum, &work[nwork], &iwork[1], 
			info);
...
/*              Set the right corner of U to identity matrix */
		...
		if (*m > *n) {
		    i__1 = *m - *n;
		    i__2 = *m - *n;
		    hlaset_("F", &i__1, &i__2, &c_b63, &c_b84, &u[*n + 1 + (*
			    n + 1) * u_dim1], ldu);
		}
...
/*              Workspace: prefer 3*N [e, tauq, taup] + M*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		hormbr_("Q", "L", "N", m, m, n, &a[a_offset], lda, &work[
			itauq], &u[u_offset], ldu, &work[nwork], &i__1, &ierr);
		i__1 = *lwork - nwork + 1;
		hormbr_("P", "R", "T", n, n, m, &a[a_offset], lda, &work[
			itaup], &vt[vt_offset], ldvt, &work[nwork], &i__1, &
			ierr);
	    }
...
/*              Workspace: prefer M [tau] + M*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		hgelqf_(m, n, &a[a_offset], lda, &work[itau], &work[nwork], &
			i__1, &ierr);
...
/*              Zero out above L */
		...
		i__1 = *m - 1;
		i__2 = *m - 1;
		hlaset_("U", &i__1, &i__2, &c_b63, &c_b63, &a[(a_dim1 << 1) + 
			1], lda);
		ie = 1;
		itauq = ie + *m;
		itaup = itauq + *m;
...
/*              Workspace: need   M [e] + BDSPAC */
		...
		hbdsdc_("U", "N", m, &s[1], &work[ie], dum, &c__1, dum, &c__1,
			 dum, idum, &work[nwork], &iwork[1], info);
...
/*              Copy L to WORK(IL), zeroing about above it */
		...
		hlacpy_("L", m, m, &a[a_offset], lda, &work[il], &ldwrkl);
		i__1 = *m - 1;
		i__2 = *m - 1;
		hlaset_("U", &i__1, &i__2, &c_b63, &c_b63, &work[il + ldwrkl],
			 &ldwrkl);
...
/*              Workspace: prefer M*M [VT] + M*M [L] + M [tau] + M*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		horglq_(m, n, m, &a[a_offset], lda, &work[itau], &work[nwork],
			 &i__1, &ierr);
		ie = itau;
		itauq = ie + *m;
		itaup = itauq + *m;
...
/*              Workspace: prefer M*M [VT] + M*M [L] + 3*M [e, tauq, taup] + 2*M*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		hgebrd_(m, m, &work[il], &ldwrkl, &s[1], &work[ie], &work[
			itauq], &work[itaup], &work[nwork], &i__1, &ierr);
...
/*              Workspace: need   M*M [VT] + M*M [L] + 3*M [e, tauq, taup] + BDSPAC */
		...
		hbdsdc_("U", "I", m, &s[1], &work[ie], &u[u_offset], ldu, &
...
/*              Workspace: prefer M*M [VT] + M*M [L] + 3*M [e, tauq, taup] + M*NB [work] */
		...
		i__1 = *lwork - nwork + 1;
		hormbr_("Q", "L", "N", m, m, m, &work[il], &ldwrkl, &work[
			itauq], &u[u_offset], ldu, &work[nwork], &i__1, &ierr);
		i__1 = *lwork - nwork + 1;
		hormbr_("P", "R", "T", m, m, m, &work[il], &ldwrkl, &work[
...
/* Computing MIN */
		    ...
		    hgemm_("N", "N", m, &blk, m, &c_b84, &work[ivt], m, &a[
			    i__ * a_dim1 + 1], lda, &c_b63, &work[il], &
			    ldwrkl);
		    hlacpy_("F", m, &blk, &work[il], &ldwrkl, &a[i__ * a_dim1 
			    + 1], lda);
/* L30: */
		}
...
/*              Copy L to WORK(IL), zeroing out above it */
		...
		i__2 = *m - 1;
		i__1 = *m - 1;
		hlaset_("U", &i__2, &i__1, &c_b63, &c_b63, &work[il + ldwrkl],
			 &ldwrkl);
...
/*              Workspace: need   M*M [L] + 3*M [e, tauq, taup] + BDSPAC */
		...
		hbdsdc_("U", "I", m, &s[1], &work[ie], &u[u_offset], ldu, &vt[
			vt_offset], ldvt, dum, idum, &work[nwork], &iwork[1], 
			info);
...
/*              Workspace: need   M*M [L] */
		...
		hlacpy_("F", m, m, &vt[vt_offset], ldvt, &work[il], &ldwrkl);
		hgemm_("N", "N", m, n, m, &c_b84, &work[il], &ldwrkl, &a[
...
/*              Workspace: prefer M*M [VT] + M [tau] + N*NB [work] */
		...
		i__2 = *lwork - nwork + 1;
		horglq_(n, n, m, &vt[vt_offset], ldvt, &work[itau], &work[
			nwork], &i__2, &ierr);
...
/*              Produce L in A, zeroing out other entries */
		...
		i__2 = *m - 1;
		i__1 = *m - 1;
		hlaset_("U", &i__2, &i__1, &c_b63, &c_b63, &a[(a_dim1 << 1) + 
			1], lda);
		ie = itau;
		itauq = ie + *m;
		itaup = itauq + *m;
...
/*              Workspace: prefer M*M [VT] + 3*M [e, tauq, taup]+ M*NB [work] */
		...
		i__2 = *lwork - nwork + 1;
		hormbr_("Q", "L", "N", m, m, m, &a[a_offset], lda, &work[
			itauq], &u[u_offset], ldu, &work[nwork], &i__2, &ierr);
		i__2 = *lwork - nwork + 1;
		hormbr_("P", "R", "T", m, m, m, &a[a_offset], lda, &work[
...
/*              Workspace: need   M*M [VT] */
		...
		hgemm_("N", "N", m, n, m, &c_b84, &work[ivt], &ldwkvt, &vt[
			vt_offset], ldvt, &c_b63, &a[a_offset], lda);

/*              Copy right singular vectors of A from A to VT */

		hlacpy_("F", m, n, &a[a_offset], lda, &vt[vt_offset], ldvt);
...
/*              Workspace: need   3*M [e, tauq, taup] + BDSPAC */
		...
		hbdsdc_("L", "N", m, &s[1], &work[ie], dum, &c__1, dum, &c__1,
			 dum, idum, &work[nwork], &iwork[1], info);
	    } else if (wntqo) {
/*              Path 5to (N > M, JOBZ='O') */
		ldwkvt = *m;
...
/*                 WORK( IVT ) is M by N */
		    ...
		    hlaset_("F", m, n, &c_b63, &c_b63, &work[ivt], &ldwkvt);
		    nwork = ivt + ldwkvt * *n;
...
/*              Workspace: need   3*M [e, tauq, taup] + M*M [VT] + BDSPAC */
		...
		hbdsdc_("L", "I", m, &s[1], &work[ie], &u[u_offset], ldu, &
			work[ivt], &ldwkvt, dum, idum, &work[nwork], &iwork[1]
			, info);
...
/*                 Workspace: prefer 3*M [e, tauq, taup] + M*N [VT] + M*NB [work] */
		    ...
		    i__2 = *lwork - nwork + 1;
		    hormbr_("P", "R", "T", m, n, m, &a[a_offset], lda, &work[
			    itaup], &work[ivt], &ldwkvt, &work[nwork], &i__2, 
			    &ierr);

/*                 Copy right singular vectors of A from WORK(IVT) to A */

		    hlacpy_("F", m, n, &work[ivt], &ldwkvt, &a[a_offset], lda);
		} else {
...
/* Computing MIN */
			i__3 = *n - i__ + 1;
			blk = f2cmin(i__3,chunk);
			hgemm_("N", "N", m, &blk, m, &c_b84, &work[ivt], &
				ldwkvt, &a[i__ * a_dim1 + 1], lda, &c_b63, &
				work[il], m);
			hlacpy_("F", m, &blk, &work[il], m, &a[i__ * a_dim1 + 
				1], lda);
/* L40: */
		    }
		}
...
/*              Workspace: need   3*M [e, tauq, taup] + BDSPAC */
		...
		hlaset_("F", m, n, &c_b63, &c_b63, &vt[vt_offset], ldvt);
		hbdsdc_("L", "I", m, &s[1], &work[ie], &u[u_offset], ldu, &vt[
			vt_offset], ldvt, dum, idum, &work[nwork], &iwork[1], 
			info);
...
/*              Workspace: need   3*M [e, tauq, taup] + BDSPAC */
		...
		hlaset_("F", n, n, &c_b63, &c_b63, &vt[vt_offset], ldvt);
		hbdsdc_("L", "I", m, &s[1], &work[ie], &u[u_offset], ldu, &vt[
			vt_offset], ldvt, dum, idum, &work[nwork], &iwork[1], 
			info);
...
/*              Set the right corner of VT to identity matrix */
		...
		if (*n > *m) {
		    i__1 = *n - *m;
		    i__2 = *n - *m;
		    hlaset_("F", &i__1, &i__2, &c_b63, &c_b84, &vt[*m + 1 + (*
			    m + 1) * vt_dim1], ldvt);
		}
