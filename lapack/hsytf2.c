...
/* > \brief \b DSYTF2 computes the factorization of a doublereal symmetric indefinite matrix, using the diagonal piv
...
/* > \verbatim */
/* > */
/* > DSYTF2 computes the factorization of a doublereal symmetric matrix A using */
...
/*  ===================================================================== */
void  hsytf2_(char *uplo, integer *n, halfreal *a, integer *
...
/*        Determine both COLMAX and IMAX. */
	...
	if (f2cmax(absakk,colmax) == 0. || hisnan_(&absakk)) {
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		i__1 = k - imax;
		jmax = imax + ihamax_(&i__1, &a[imax + (imax + 1) * a_dim1], 
			lda);
		rowmax = (d__1 = a[imax + jmax * a_dim1], abs(d__1));
		if (imax > 1) {
		    i__1 = imax - 1;
		    jmax = ihamax_(&i__1, &a[imax * a_dim1 + 1], &c__1);
/* Computing MAX */
...
/*              submatrix A(1:k,1:k) */
		...
		i__1 = kp - 1;
		hswap_(&i__1, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 + 1],
			 &c__1);
		i__1 = kk - kp - 1;
		hswap_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (kp + 
			1) * a_dim1], lda);
		t = a[kk + kk * a_dim1];
...
/*              A := A - U(k)*D(k)*U(k)**T = A - W(k)*1/D(k)*W(k)**T */
		...
		i__1 = k - 1;
		d__1 = -r1;
		hsyr_(uplo, &i__1, &d__1, &a[k * a_dim1 + 1], &c__1, &a[
			a_offset], lda);
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    jmax = imax + ihamax_(&i__1, &a[imax + 1 + imax * a_dim1],
			     &c__1);
/* Computing MAX */
		    d__2 = rowmax, d__3 = (d__1 = a[jmax + imax * a_dim1], 
			    abs(d__1));
...
/*              submatrix A(k:n,k:n) */
		...
		i__1 = kp - kk - 1;
		hswap_(&i__1, &a[kk + 1 + kk * a_dim1], &c__1, &a[kp + (kk + 
			1) * a_dim1], lda);
		t = a[kk + kk * a_dim1];
		a[kk + kk * a_dim1] = a[kp + kp * a_dim1];
		a[kp + kp * a_dim1] = t;
...
/*                 A := A - L(k)*D(k)*L(k)**T = A - W(k)*(1/D(k))*W(k)**T */
		    ...
		    i__1 = *n - k;
		    d__1 = -d11;
		    hsyr_(uplo, &i__1, &d__1, &a[k + 1 + k * a_dim1], &c__1, &
