...
/* > */
/*  ===================================================================== */
void  klatps_(char *uplo, char *trans, char *diag, char *
	normin, integer *n, halfcomplex *ap, halfcomplex *x, halfreal *
	scale, halfreal *cnorm, integer *info)
{
    ...
    extern void  kaxpy_(integer *, halfcomplex *, 
	    halfcomplex *, integer *, halfcomplex *, integer *), ktpsv_(
	    char *, char *, char *, integer *, halfcomplex *, halfcomplex 
	    *, integer *), hlabad_(halfreal *, 
	    halfreal *);
...
/*           A is upper triangular. */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = j - 1;
		cnorm[j] = hwasum_(&i__2, &ap[ip], &c__1);
		ip += j;
/* L10: */
	    }
	} else {
...
/*           A is lower triangular. */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j;
		cnorm[j] = hwasum_(&i__2, &ap[ip + 1], &c__1);
		ip = ip + *n - j + 1;
/* L20: */
	    }
	    cnorm[*n] = 0.;
...
/*        elements of X is not too small. */
	...
	ktpsv_(uplo, trans, diag, n, &ap[1], &x[1], &c__1);
    } else {
...
/*                       x(1:j-1) := x(1:j-1) - x(j) * A(1:j-1,j) */
			...
			kaxpy_(&i__3, &z__1, &ap[ip - j + 1], &c__1, &x[1], &
				c__1);
			i__3 = j - 1;
...
/*                       x(j+1:n) := x(j+1:n) - x(j) * A(j+1:n,j) */
			...
			z__2.r = -x[i__4].r, z__2.i = -x[i__4].i;
			z__1.r = tscal * z__2.r, z__1.i = tscal * z__2.i;
			kaxpy_(&i__3, &z__1, &ap[ip + 1], &c__1, &x[j + 1], &
				c__1);
			i__3 = *n - j;
...
/*                 call ZDOTU to perform the dot product. */
		    ...
		    if (upper) {
			i__3 = j - 1;
			whotu_(&z__1, &i__3, &ap[ip - j + 1], &c__1, &x[1], &
				c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    } else if (j < *n) {
			i__3 = *n - j;
			whotu_(&z__1, &i__3, &ap[ip + 1], &c__1, &x[j + 1], &
				c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    }
		} else {
...
/*                 call ZDOTC to perform the dot product. */
		    ...
		    if (upper) {
			i__3 = j - 1;
			whotc_(&z__1, &i__3, &ap[ip - j + 1], &c__1, &x[1], &
				c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    } else if (j < *n) {
			i__3 = *n - j;
			whotc_(&z__1, &i__3, &ap[ip + 1], &c__1, &x[j + 1], &
				c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    }
		} else {
