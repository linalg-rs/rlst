...
void  dsytrs_aa__(char *uplo, integer *n, integer *nrhs, 
	quadreal *a, integer *lda, integer *ipiv, quadreal *b, integer *
	ldb, quadreal *work, integer *lwork, integer *info)
{
    ...
    extern void  qswap_(integer *, quadreal *, integer *, 
	    quadreal *, integer *), qgtsv_(integer *, integer *, quadreal 
	    *, quadreal *, quadreal *, quadreal *, integer *, integer *)
	    , qtrsm_(char *, char *, char *, char *, integer *, integer *, 
...
/*        Compute (U \P**T * B) -> B    [ (U \P**T * B) ] */
	...
	i__1 = *n - 1;
	qtrsm_("L", "U", "T", "U", &i__1, nrhs, &c_b9, &a[(a_dim1 << 1) + 1], 
		lda, &b[b_dim1 + 2], ldb);

/*        Compute T \ B -> B   [ T \ (U \P**T * B) ] */

	i__1 = *lda + 1;
	qlacpy_("F", &c__1, n, &a[a_dim1 + 1], &i__1, &work[*n], &c__1);
	if (*n > 1) {
	    i__1 = *n - 1;
	    i__2 = *lda + 1;
	    qlacpy_("F", &c__1, &i__1, &a[(a_dim1 << 1) + 1], &i__2, &work[1],
		     &c__1);
	    i__1 = *n - 1;
	    i__2 = *lda + 1;
	    qlacpy_("F", &c__1, &i__1, &a[(a_dim1 << 1) + 1], &i__2, &work[*n 
		    * 2], &c__1);
	}
	qgtsv_(n, nrhs, &work[1], &work[*n], &work[*n * 2], &b[b_offset], ldb,
		 info);

/*        Compute (U**T \ B) -> B   [ U**T \ (T \ (U \P**T * B) ) ] */

	i__1 = *n - 1;
	qtrsm_("L", "U", "N", "U", &i__1, nrhs, &c_b9, &a[(a_dim1 << 1) + 1], 
		lda, &b[b_dim1 + 2], ldb);
...
/*        Compute (L \P**T * B) -> B    [ (L \P**T * B) ] */
	...
	i__1 = *n - 1;
	qtrsm_("L", "L", "N", "U", &i__1, nrhs, &c_b9, &a[a_dim1 + 2], lda, &
		b[b_dim1 + 2], ldb);
...
/*        Compute T \ B -> B   [ T \ (L \P**T * B) ] */
	...
	if (*n > 1) {
	    i__1 = *n - 1;
	    i__2 = *lda + 1;
	    qlacpy_("F", &c__1, &i__1, &a[a_dim1 + 2], &i__2, &work[1], &c__1);
	    i__1 = *n - 1;
	    i__2 = *lda + 1;
	    qlacpy_("F", &c__1, &i__1, &a[a_dim1 + 2], &i__2, &work[*n * 2], &
		    c__1);
	}
	qgtsv_(n, nrhs, &work[1], &work[*n], &work[*n * 2], &b[b_offset], ldb,
		 info);
...
/*        Compute (L**T \ B) -> B   [ L**T \ (T \ (L \P**T * B) ) ] */
	...
	i__1 = *n - 1;
	qtrsm_("L", "L", "T", "U", &i__1, nrhs, &c_b9, &a[a_dim1 + 2], lda, &
		b[b_dim1 + 2], ldb);
