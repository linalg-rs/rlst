...
/*  ===================================================================== */
void  qsbevx_(char *jobz, char *range, char *uplo, integer *n, 
	integer *kd, quadreal *ab, integer *ldab, quadreal *q, integer *
	ldq, quadreal *vl, quadreal *vu, integer *il, integer *iu, 
	quadreal *abstol, integer *m, quadreal *w, quadreal *z__, 
	integer *ldz, quadreal *work, integer *iwork, integer *ifail, 
	integer *info)
{
...
/*     Scale matrix to allowable range, if necessary. */
    ...
    if (iscale == 1) {
	if (lower) {
	    qlascl_("B", kd, kd, &c_b14, &sigma, n, n, &ab[ab_offset], ldab, 
		    info);
	} else {
	    qlascl_("Q", kd, kd, &c_b14, &sigma, n, n, &ab[ab_offset], ldab, 
		    info);
	}
	if (*abstol > 0.) {
	    abstll = *abstol * sigma;
...
/*     Call DSBTRD to reduce symmetric band matrix to tridiagonal form. */
    ...
    inde = indd + *n;
    indwrk = inde + *n;
    qsbtrd_(jobz, uplo, n, kd, &ab[ab_offset], ldab, &work[indd], &work[inde],
...
/*        form to eigenvectors returned by DSTEIN. */
	...
	for (j = 1; j <= i__1; ++j) {
	    ...
	    qgemv_("N", n, n, &c_b14, &q[q_offset], ldq, &work[1], &c__1, &
