...
/* >  conjg(s12); the diagonal elements of S are doublereal. */
/* > \endverbatim */
/* > */
/*  ===================================================================== */
void  kpbstf_(char *uplo, integer *n, integer *kd, 
	halfcomplex *ab, integer *ldab, integer *info)
{
    ...
    extern void  xerbla_(char *, integer *), whscal_(
	    integer *, halfreal *, halfcomplex *, integer *), klacgv_(
...
/*           the leading submatrix within the band. */
	    ...
	    d__1 = 1. / ajj;
	    whscal_(&km, &d__1, &ab[*kd + 1 - km + j * ab_dim1], &c__1);
	    kher_("Upper", &km, &c_b9, &ab[*kd + 1 - km + j * ab_dim1], &c__1,
...
/*           trailing submatrix within the band. */
	    ...
	    if (km > 0) {
		d__1 = 1. / ajj;
		whscal_(&km, &d__1, &ab[*kd + (j + 1) * ab_dim1], &kld);
		klacgv_(&km, &ab[*kd + (j + 1) * ab_dim1], &kld);
		kher_("Upper", &km, &c_b9, &ab[*kd + (j + 1) * ab_dim1], &kld,
...
/*           trailing submatrix within the band. */
	    ...
	    d__1 = 1. / ajj;
	    whscal_(&km, &d__1, &ab[km + 1 + (j - km) * ab_dim1], &kld);
	    klacgv_(&km, &ab[km + 1 + (j - km) * ab_dim1], &kld);
	    kher_("Lower", &km, &c_b9, &ab[km + 1 + (j - km) * ab_dim1], &kld,
...
/*           trailing submatrix within the band. */
	    ...
	    if (km > 0) {
		d__1 = 1. / ajj;
		whscal_(&km, &d__1, &ab[j * ab_dim1 + 2], &c__1);
		kher_("Lower", &km, &c_b9, &ab[j * ab_dim1 + 2], &c__1, &ab[(
			j + 1) * ab_dim1 + 1], &kld);
	    }
/* L40: */
	}
