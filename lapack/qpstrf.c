...
/*  ===================================================================== */
void  qpstrf_(char *uplo, integer *n, quadreal *a, integer *
...
/*        Use unblocked code */
	...
	qpstf2_(uplo, n, &a[a_dim1 + 1], lda, &piv[1], rank, tol, &work[1], 
		info);
	goto L200;
...
/*                    Pivot OK, so can now swap pivot rows and columns */
			...
			i__4 = j - 1;
			qswap_(&i__4, &a[j * a_dim1 + 1], &c__1, &a[pvt * 
				a_dim1 + 1], &c__1);
			if (pvt < *n) {
			    i__4 = *n - pvt;
			    qswap_(&i__4, &a[j + (pvt + 1) * a_dim1], lda, &a[
				    pvt + (pvt + 1) * a_dim1], lda);
			}
			i__4 = pvt - j - 1;
			qswap_(&i__4, &a[j + (j + 1) * a_dim1], lda, &a[j + 1 
				+ pvt * a_dim1], &c__1);
...
/*                 Compute elements J+1:N of row J. */
			...
			i__4 = j - k;
			i__5 = *n - j;
			qgemv_("Trans", &i__4, &i__5, &c_b23, &a[k + (j + 1) *
				 a_dim1], lda, &a[k + j * a_dim1], &c__1, &
				c_b25, &a[j + (j + 1) * a_dim1], lda);
			i__4 = *n - j;
			d__1 = 1. / ajj;
			qscal_(&i__4, &d__1, &a[j + (j + 1) * a_dim1], lda);
		    }
...
/*              Update trailing matrix, J already incremented */
		...
		if (k + jb <= *n) {
		    i__3 = *n - j + 1;
		    qsyrk_("Upper", "Trans", &i__3, &jb, &c_b23, &a[k + j * 
...
/*                    Pivot OK, so can now swap pivot rows and columns */
			...
			a[pvt + pvt * a_dim1] = a[j + j * a_dim1];
			i__4 = j - 1;
			qswap_(&i__4, &a[j + a_dim1], lda, &a[pvt + a_dim1], 
				lda);
			if (pvt < *n) {
			    i__4 = *n - pvt;
			    qswap_(&i__4, &a[pvt + 1 + j * a_dim1], &c__1, &a[
				    pvt + 1 + pvt * a_dim1], &c__1);
			}
			i__4 = pvt - j - 1;
			qswap_(&i__4, &a[j + 1 + j * a_dim1], &c__1, &a[pvt + 
				(j + 1) * a_dim1], lda);
...
/*                 Compute elements J+1:N of column J. */
			...
			i__4 = *n - j;
			i__5 = j - k;
			qgemv_("No Trans", &i__4, &i__5, &c_b23, &a[j + 1 + k 
				* a_dim1], lda, &a[j + k * a_dim1], lda, &
				c_b25, &a[j + 1 + j * a_dim1], &c__1);
			i__4 = *n - j;
			d__1 = 1. / ajj;
			qscal_(&i__4, &d__1, &a[j + 1 + j * a_dim1], &c__1);
		    }
...
/*              Update trailing matrix, J already incremented */
		...
		if (k + jb <= *n) {
		    i__3 = *n - j + 1;
		    qsyrk_("Lower", "No Trans", &i__3, &jb, &c_b23, &a[j + k *
			     a_dim1], lda, &c_b25, &a[j + j * a_dim1], lda);
		}
