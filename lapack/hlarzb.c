...
/* > */
/*  ===================================================================== */
void  hlarzb_(char *side, char *trans, char *direct, char *
	storev, integer *m, integer *n, integer *k, integer *l, halfreal *v,
	 integer *ldv, halfreal *t, integer *ldt, halfreal *c__, integer *
	ldc, halfreal *work, integer *ldwork)
{
...
/*        W( 1:n, 1:k ) = C( 1:k, 1:n )**T */
	...
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    hcopy_(n, &c__[j + c_dim1], ldc, &work[j * work_dim1 + 1], &c__1);
/* L10: */
	}
...
/*                        C( m-l+1:m, 1:n )**T * V( 1:k, 1:l )**T */
	...
	if (*l > 0) {
	    hgemm_("Transpose", "Transpose", n, k, l, &c_b13, &c__[*m - *l + 
...
/*        W( 1:n, 1:k ) = W( 1:n, 1:k ) * T**T  or  W( 1:m, 1:k ) * T */
	...
	htrmm_("Right", "Lower", transt, "Non-unit", n, k, &c_b13, &t[
...
/*                            V( 1:k, 1:l )**T * W( 1:n, 1:k )**T */
	...
	if (*l > 0) {
	    hgemm_("Transpose", "Transpose", l, n, k, &c_b23, &v[v_offset], 
...
/*        W( 1:m, 1:k ) = C( 1:m, 1:k ) */
	...
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    hcopy_(m, &c__[j * c_dim1 + 1], &c__1, &work[j * work_dim1 + 1], &
		    c__1);
/* L40: */
	}
...
/*                        C( 1:m, n-l+1:n ) * V( 1:k, 1:l )**T */
	...
	if (*l > 0) {
	    hgemm_("No transpose", "Transpose", m, k, l, &c_b13, &c__[(*n - *
...
/*        W( 1:m, 1:k ) = W( 1:m, 1:k ) * T  or  W( 1:m, 1:k ) * T**T */
	...
	htrmm_("Right", "Lower", trans, "Non-unit", m, k, &c_b13, &t[t_offset]
...
/*                            W( 1:m, 1:k ) * V( 1:k, 1:l ) */
	...
	if (*l > 0) {
	    hgemm_("No transpose", "No transpose", m, l, k, &c_b23, &work[
