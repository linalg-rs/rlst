...
/* > */
/*  ===================================================================== */
void  qlarre_(char *range, integer *n, quadreal *vl, 
	quadreal *vu, integer *il, integer *iu, quadreal *d__, quadreal 
	*e, quadreal *e2, quadreal *rtol1, quadreal *rtol2, quadreal *
	spltol, integer *nsplit, integer *isplit, integer *m, quadreal *w, 
	quadreal *werr, quadreal *wgap, integer *iblock, integer *indexw, 
	quadreal *gers, quadreal *pivmin, quadreal *work, integer *
	iwork, integer *info)
{
    ...
    quadreal gu;
    integer cnt;
    quadreal eps, tau, tmp, rtl;
    integer cnt1, cnt2;
    quadreal tmp1, eabs;
    integer iend, jblk;
    quadreal eold;
    integer indl;
    quadreal dmax__, emax;
    integer wend, idum, indu;
    quadreal rtol;
    integer iseed[4];
    quadreal avgap, sigma;
    ...
    extern void  qlarrd_(char *, char *, integer *, quadreal 
	    *, quadreal *, integer *, integer *, quadreal *, quadreal *,
	    ...
	    integer *);
    quadreal safmin, spdiam;
    extern void  qlarrk_(integer *, integer *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, integer *);
    logical usedqd;
    quadreal clwdth, isleft;
    extern void  qlarnv_(integer *, integer *, integer *, 
	    quadreal *);
    quadreal isrght, bsrtol, dpivot;
...
/*     Compute splitting points */
    qlarra_(n, &d__[1], &e[1], &e2[1], spltol, &spdiam, nsplit, &isplit[1], &
	    iinfo);
...
/*        DLARRD needs a WORK of size 4*N, IWORK of size 3*N */
	qlarrd_(range, "B", n, vl, vu, il, iu, &gers[1], &bsrtol, &d__[1], &e[
...
/*              Decide whether dqds or bisection is more efficient */
		usedqd = (quadreal) mb > in * .5 && ! forceb;
		wend = wbegin + mb - 1;
...
/*           Find approximations to the extremal eigenvalues of the block */
	    qlarrk_(&in, &c__1, &gl, &gu, &d__[ibegin], &e2[ibegin], pivmin, &
		    rtl, &tmp, &tmp1, &iinfo);
	    if (iinfo != 0) {
		*info = -1;
		return;
...
/* Computing MAX */
	    ...
	    qlarrk_(&in, &in, &gl, &gu, &d__[ibegin], &e2[ibegin], pivmin, &
		    rtl, &tmp, &tmp1, &iinfo);
	    if (iinfo != 0) {
		*info = -1;
		return;
...
/*        Compute the negcount at the 1/4 and 3/4 points */
	if (mb > 1) {
	    qlarrc_("T", &in, &s1, &s2, &d__[ibegin], &e[ibegin], pivmin, &
		    cnt, &cnt1, &cnt2, &iinfo);
	}
	if (mb == 1) {
	    sigma = gl;
...
/* Computing MAX */
		...
		avgap = (d__1 = clwdth / (quadreal) (wend - wbegin), abs(
			d__1));
		if (sgndef == 1.) {
/* Computing MAX */
		    d__1 = wgap[wbegin];
...
/*        Store the shift. */
	e[iend] = sigma;
/*        Store D and L. */
	qcopy_(&in, &work[1], &c__1, &d__[ibegin], &c__1);
	i__2 = in - 1;
	qcopy_(&i__2, &work[in + 1], &c__1, &e[ibegin], &c__1);
	if (mb > 1) {
...
/* L122: */
	    ...
	    i__2 = (in << 1) - 1;
	    qlarnv_(&c__2, iseed, &i__2, &work[1]);
	    i__2 = in - 1;
	    for (i__ = 1; i__ <= i__2; ++i__) {
...
/*           use bisection to find EV from INDL to INDU */
	    i__2 = indl - 1;
	    qlarrb_(&in, &d__[ibegin], &work[ibegin], &indl, &indu, rtol1, 
