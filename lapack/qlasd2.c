...
/* > */
/*  ===================================================================== */
void  qlasd2_(integer *nl, integer *nr, integer *sqre, integer 
	*k, quadreal *d__, quadreal *z__, quadreal *alpha, quadreal *
	beta, quadreal *u, integer *ldu, quadreal *vt, integer *ldvt, 
	quadreal *dsigma, quadreal *u2, integer *ldu2, quadreal *vt2, 
	...
	idxq, integer *coltyp, integer *info)
{
    ...
    extern void  qlamrg_(integer *, integer *, quadreal *, 
	    integer *, integer *, integer *), qlacpy_(char *, integer *, 
...
/* L60: */
    ...
    qlamrg_(nl, nr, &dsigma[2], &c__1, &c__1, &idx[2]);
...
/*           Find M(sqrt)(a**2+b**2) without overflow or */
...
/*           singular vector matrices. */
	    ...
	    qrot_(&n, &u[idxjp * u_dim1 + 1], &c__1, &u[idxj * u_dim1 + 1], &
		    c__1, &c__, &s);
	    qrot_(&m, &vt[idxjp + vt_dim1], ldvt, &vt[idxj + vt_dim1], ldvt, &
		    c__, &s);
...
/*     separately. */
	...
	qcopy_(&n, &u[idxj * u_dim1 + 1], &c__1, &u2[j * u2_dim1 + 1], &c__1);
	qcopy_(&m, &vt[idxj + vt_dim1], ldvt, &vt2[j + vt2_dim1], ldvt2);
/* L160: */
    }
...
/*     Move the rest of the updating row to Z. */
    ...
    i__1 = *k - 1;
    qcopy_(&i__1, &u2[u2_dim1 + 2], &c__1, &z__[2], &c__1);

/*     Determine the first column of U2, the first row of VT2 and the */
/*     last row of VT. */

    qlaset_("A", &n, &c__1, &c_b30, &c_b30, &u2[u2_offset], ldu2);
    u2[nlp1 + u2_dim1] = 1.;
    if (m > n) {
	i__1 = nlp1;
...
/* L180: */
	...
	qcopy_(&m, &vt[nlp1 + vt_dim1], ldvt, &vt2[vt2_dim1 + 1], ldvt2);
    }
    if (m > n) {
	qcopy_(&m, &vt[m + vt_dim1], ldvt, &vt2[m + vt2_dim1], ldvt2);
    }
...
/*     into the back of D, U, and V respectively. */
    ...
    if (n > *k) {
	i__1 = n - *k;
	qcopy_(&i__1, &dsigma[*k + 1], &c__1, &d__[*k + 1], &c__1);
	i__1 = n - *k;
	qlacpy_("A", &n, &i__1, &u2[(*k + 1) * u2_dim1 + 1], ldu2, &u[(*k + 1)
		 * u_dim1 + 1], ldu);
	i__1 = n - *k;
	qlacpy_("A", &i__1, &m, &vt2[*k + 1 + vt2_dim1], ldvt2, &vt[*k + 1 + 
		vt_dim1], ldvt);
    }
