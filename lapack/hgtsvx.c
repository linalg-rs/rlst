...
/* > \verbatim */
/* > */
/* > DGTSVX uses the LU factorization to compute the solution to a doublereal */
...
/*  ===================================================================== */
void  hgtsvx_(char *fact, char *trans, integer *n, integer *
	nrhs, halfreal *dl, halfreal *d__, halfreal *du, halfreal *
	dlf, halfreal *df, halfreal *duf, halfreal *du2, integer *ipiv, 
...
/*        Compute the LU factorization of A. */
	...
	hcopy_(n, &d__[1], &c__1, &df[1], &c__1);
	if (*n > 1) {
	    i__1 = *n - 1;
	    hcopy_(&i__1, &dl[1], &c__1, &dlf[1], &c__1);
	    i__1 = *n - 1;
	    hcopy_(&i__1, &du[1], &c__1, &duf[1], &c__1);
	}
	hgttrf_(n, &dlf[1], &df[1], &duf[1], &du2[1], &ipiv[1], info);
...
/*     Compute the norm of the matrix A. */
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = hlangt_(norm, n, &dl[1], &d__[1], &du[1]);

/*     Compute the reciprocal of the condition number of A. */

    hgtcon_(norm, n, &dlf[1], &df[1], &duf[1], &du2[1], &ipiv[1], &anorm, 
	    rcond, &work[1], &iwork[1], info);

/*     Compute the solution vectors X. */

    hlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    hgttrs_(trans, n, nrhs, &dlf[1], &df[1], &duf[1], &du2[1], &ipiv[1], &x[
	    x_offset], ldx, info);

/*     Use iterative refinement to improve the computed solutions and */
/*     compute error bounds and backward error estimates for them. */

    hgtrfs_(trans, n, nrhs, &dl[1], &d__[1], &du[1], &dlf[1], &df[1], &duf[1],
