...
/* > */
/*  ===================================================================== */
void  wgeqp3_(integer *m, integer *n, quadcomplex *a, 
	integer *lda, integer *jpvt, quadcomplex *tau, quadcomplex *work, 
	integer *lwork, quadreal *rwork, integer *info)
{
    ...
    extern void  wswap_(integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *), wlaqp2_(integer *, integer *, 
	    integer *, quadcomplex *, integer *, integer *, quadcomplex *,
	     quadreal *, quadreal *, quadcomplex *);
    ...
    extern void  wlaqps_(integer *, integer *, integer *, 
...
/*     Move initial columns up front. */
	...
	if (jpvt[j] != 0) {
	    if (j != nfxd) {
		wswap_(m, &a[j * a_dim1 + 1], &c__1, &a[nfxd * a_dim1 + 1], &
			c__1);
		jpvt[j] = jpvt[nfxd];
		jpvt[nfxd] = j;
	    } else {
...
/* CC      CALL ZGEQR2( M, NA, A, LDA, TAU, WORK, INFO ) */
	wgeqrf_(m, &na, &a[a_offset], lda, &tau[1], &work[1], lwork, info);
/* Computing MAX */
...
/* CC  $                   INFO ) */
	    i__1 = *n - na;
	    wunmqr_("Left", "Conjugate Transpose", m, &i__1, &na, &a[a_offset]
...
/*        store the exact column norms. */
	...
	for (j = nfxd + 1; j <= i__1; ++j) {
	    rwork[j] = qwnrm2_(&sm, &a[nfxd + 1 + j * a_dim1], &c__1);
	    rwork[*n + j] = rwork[j];
/* L20: */
	}
...
/*              Factorize JB columns among columns J:N. */
		...
		i__2 = j - 1;
		i__3 = *n - j + 1;
		wlaqps_(m, &i__1, &i__2, &jb, &fjb, &a[j * a_dim1 + 1], lda, &
...
/*        Use unblocked code to factor the last or only block. */
	...
	if (j <= minmn) {
	    i__1 = *n - j + 1;
	    i__2 = j - 1;
	    wlaqp2_(m, &i__1, &i__2, &a[j * a_dim1 + 1], lda, &jpvt[j], &tau[
