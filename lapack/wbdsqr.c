...
static quadreal c_b15 = -.125;
static integer c__1 = 1;
static quadreal c_b49 = 1.;
static quadreal c_b72 = -1.;
...
/*  ===================================================================== */
void  wbdsqr_(char *uplo, integer *n, integer *ncvt, integer *
	nru, integer *ncc, quadreal *d__, quadreal *e, quadcomplex *vt, 
	integer *ldvt, quadcomplex *u, integer *ldu, quadcomplex *c__, 
	integer *ldc, quadreal *rwork, integer *info)
{
    ...
    quadreal eps, sll, tol, abse;
    integer idir;
    quadreal abss;
    integer oldm;
    quadreal cosl;
    integer isub, iter;
    quadreal unfl, sinl, cosr, smin, smax, sinr;
    extern void  qlas2_(quadreal *, quadreal *, quadreal 
	    *, quadreal *, quadreal *);
    extern logical lsame_(char *, char *);
    quadreal oldcs;
    integer oldll;
    quadreal shift, sigmn, oldsn;
    integer maxit;
    quadreal sminl, sigmx;
    logical lower;
    extern void  wlasr_(char *, char *, char *, integer *, 
	    integer *, quadreal *, quadreal *, quadcomplex *, integer *), wqrot_(integer *, quadcomplex *, 
	    integer *, quadcomplex *, integer *, quadreal *, quadreal *)
	    , wswap_(integer *, quadcomplex *, integer *, quadcomplex *, 
	    integer *), qlasq1_(integer *, quadreal *, quadreal *, 
	    quadreal *, integer *), qlasv2_(quadreal *, quadreal *, 
    ...
    extern void  qlartg_(quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *), xerbla_(char *, 
	    integer *), wqscal_(integer *, quadreal *, 
	    quadcomplex *, integer *);
    quadreal sminoa, thresh;
    logical rotate;
    quadreal tolmul;
...
/*     If no singular vectors desired, use qd algorithm */
    ...
    if (! rotate) {
	qlasq1_(n, &d__[1], &e[1], &rwork[1], info);
...
/*        Update singular vectors if desired */
	...
	if (*nru > 0) {
	    wlasr_("R", "V", "F", nru, n, &rwork[1], &rwork[*n], &u[u_offset],
		     ldu);
	}
	if (*ncc > 0) {
	    wlasr_("L", "V", "F", n, ncc, &rwork[1], &rwork[*n], &c__[
		    c_offset], ldc);
	}
    }
...
/*        2 by 2 block, handle separately */
	...
	qlasv2_(&d__[m - 1], &e[m - 1], &d__[m], &sigmn, &sigmx, &sinr, &cosr,
		 &sinl, &cosl);
	d__[m - 1] = sigmx;
	e[m - 1] = 0.;
	d__[m] = sigmn;
...
/*        Compute singular vectors, if desired */
	...
	if (*ncvt > 0) {
	    wqrot_(ncvt, &vt[m - 1 + vt_dim1], ldvt, &vt[m + vt_dim1], ldvt, &
		    cosr, &sinr);
	}
	if (*nru > 0) {
	    wqrot_(nru, &u[(m - 1) * u_dim1 + 1], &c__1, &u[m * u_dim1 + 1], &
		    c__1, &cosl, &sinl);
	}
	if (*ncc > 0) {
	    wqrot_(ncc, &c__[m - 1 + c_dim1], ldc, &c__[m + c_dim1], ldc, &
		    cosl, &sinl);
	}
	m += -2;
	goto L60;
...
/*        Compute the shift from 2-by-2 block at end of matrix */
	...
	if (idir == 1) {
	    ...
	    qlas2_(&d__[m - 1], &e[m - 1], &d__[m], &shift, &r__);
	} else {
	    sll = (d__1 = d__[m], abs(d__1));
	    qlas2_(&d__[ll], &e[ll], &d__[ll + 1], &shift, &r__);
	}
...
/*           Save cosines and sines for later singular vector updates */
		...
		d__1 = d__[i__] * cs;
		qlartg_(&d__1, &e[i__], &cs, &sn, &r__);
		if (i__ > ll) {
		    e[i__ - 1] = oldsn * r__;
		}
		d__1 = oldcs * r__;
		d__2 = d__[i__ + 1] * sn;
		qlartg_(&d__1, &d__2, &oldcs, &oldsn, &d__[i__]);
		rwork[i__ - ll + 1] = cs;
...
/*           Update singular vectors */
	    ...
	    if (*ncvt > 0) {
		i__1 = m - ll + 1;
		wlasr_("L", "V", "F", &i__1, ncvt, &rwork[1], &rwork[*n], &vt[
			ll + vt_dim1], ldvt);
	    }
	    if (*nru > 0) {
		i__1 = m - ll + 1;
		wlasr_("R", "V", "F", nru, &i__1, &rwork[nm12 + 1], &rwork[
			nm13 + 1], &u[ll * u_dim1 + 1], ldu);
	    }
	    if (*ncc > 0) {
		i__1 = m - ll + 1;
		wlasr_("L", "V", "F", &i__1, ncc, &rwork[nm12 + 1], &rwork[
...
/*           Save cosines and sines for later singular vector updates */
		...
		d__1 = d__[i__] * cs;
		qlartg_(&d__1, &e[i__ - 1], &cs, &sn, &r__);
		if (i__ < m) {
		    e[i__] = oldsn * r__;
		}
		d__1 = oldcs * r__;
...
/*           Update singular vectors */
	    ...
	    if (*ncvt > 0) {
		i__1 = m - ll + 1;
		wlasr_("L", "V", "B", &i__1, ncvt, &rwork[nm12 + 1], &rwork[
			nm13 + 1], &vt[ll + vt_dim1], ldvt);
	    }
	    if (*nru > 0) {
		i__1 = m - ll + 1;
		wlasr_("R", "V", "B", nru, &i__1, &rwork[1], &rwork[*n], &u[
			ll * u_dim1 + 1], ldu);
	    }
	    if (*ncc > 0) {
		i__1 = m - ll + 1;
		wlasr_("L", "V", "B", &i__1, ncc, &rwork[1], &rwork[*n], &c__[
			ll + c_dim1], ldc);
	    }
...
/*           Change sign of singular vectors, if desired */
	    ...
	    if (*ncvt > 0) {
		wqscal_(ncvt, &c_b72, &vt[i__ + vt_dim1], ldvt);
	    }
	}
/* L170: */
    }
...
/*           Swap singular values and vectors */
	    ...
	    d__[*n + 1 - i__] = smin;
	    if (*ncvt > 0) {
		wswap_(ncvt, &vt[isub + vt_dim1], ldvt, &vt[*n + 1 - i__ + 
			vt_dim1], ldvt);
	    }
	    if (*nru > 0) {
		wswap_(nru, &u[isub * u_dim1 + 1], &c__1, &u[(*n + 1 - i__) * 
			u_dim1 + 1], &c__1);
	    }
	    if (*ncc > 0) {
		wswap_(ncc, &c__[isub + c_dim1], ldc, &c__[*n + 1 - i__ + 
			c_dim1], ldc);
	    }
	}
/* L190: */
