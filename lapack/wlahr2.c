...
/* > */
/*  ===================================================================== */
void  wlahr2_(integer *n, integer *k, integer *nb, 
	quadcomplex *a, integer *lda, quadcomplex *tau, quadcomplex *t, 
	integer *ldt, quadcomplex *y, integer *ldy)
{
    ...
    extern void  wscal_(integer *, quadcomplex *, 
	    quadcomplex *, integer *), wgemm_(char *, char *, integer *, 
	    ...
	    integer *), wtrmm_(char *, char *, char *, char *, integer *, 
	    integer *, quadcomplex *, quadcomplex *, integer *, 
	    quadcomplex *, integer *), 
	    waxpy_(integer *, quadcomplex *, quadcomplex *, integer *, 
	    quadcomplex *, integer *), wtrmv_(char *, char *, char *, 
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *), wlarfg_(integer *, quadcomplex *, 
	    quadcomplex *, integer *, quadcomplex *), wlacgv_(integer *, 
	    quadcomplex *, integer *), wlacpy_(char *, integer *, integer *,
...
/*           Update I-th column of A - Y * V**H */
	    ...
	    i__2 = i__ - 1;
	    wlacgv_(&i__2, &a[*k + i__ - 1 + a_dim1], lda);
	    i__2 = *n - *k;
	    i__3 = i__ - 1;
	    z__1.r = -1., z__1.i = -0.;
	    wgemv_("NO TRANSPOSE", &i__2, &i__3, &z__1, &y[*k + 1 + y_dim1], 
...
/*           w := V1**H * b1 */
	    ...
	    i__2 = i__ - 1;
	    wcopy_(&i__2, &a[*k + 1 + i__ * a_dim1], &c__1, &t[*nb * t_dim1 + 
		    1], &c__1);
	    i__2 = i__ - 1;
	    wtrmv_("Lower", "Conjugate transpose", "UNIT", &i__2, &a[*k + 1 + 
...
/*           w := w + V2**H * b2 */
	    ...
	    i__2 = *n - *k - i__ + 1;
	    i__3 = i__ - 1;
	    wgemv_("Conjugate transpose", &i__2, &i__3, &c_b2, &a[*k + i__ + 
...
/*           w := T**H * w */
	    ...
	    i__2 = i__ - 1;
	    wtrmv_("Upper", "Conjugate transpose", "NON-UNIT", &i__2, &t[
...
/*           b2 := b2 - V2*w */
	    ...
	    i__3 = i__ - 1;
	    z__1.r = -1., z__1.i = -0.;
	    wgemv_("NO TRANSPOSE", &i__2, &i__3, &z__1, &a[*k + i__ + a_dim1],
...
/*           b1 := b1 - V1*w */
	    ...
	    i__2 = i__ - 1;
	    wtrmv_("Lower", "NO TRANSPOSE", "UNIT", &i__2, &a[*k + 1 + a_dim1]
		    , lda, &t[*nb * t_dim1 + 1], &c__1);
	    i__2 = i__ - 1;
	    z__1.r = -1., z__1.i = -0.;
	    waxpy_(&i__2, &z__1, &t[*nb * t_dim1 + 1], &c__1, &a[*k + 1 + i__ 
		    * a_dim1], &c__1);
...
/* Computing MIN */
	i__3 = *k + i__ + 1;
	wlarfg_(&i__2, &a[*k + i__ + i__ * a_dim1], &a[f2cmin(i__3,*n) + i__ * 
		a_dim1], &c__1, &tau[i__]);
...
/*        Compute  Y(K+1:N,I) */
	...
	i__2 = *n - *k;
	i__3 = *n - *k - i__ + 1;
	wgemv_("NO TRANSPOSE", &i__2, &i__3, &c_b2, &a[*k + 1 + (i__ + 1) * 
	...
	i__3 = i__ - 1;
	z__1.r = -1., z__1.i = -0.;
	wgemv_("NO TRANSPOSE", &i__2, &i__3, &z__1, &y[*k + 1 + y_dim1], ldy, 
		&t[i__ * t_dim1 + 1], &c__1, &c_b2, &y[*k + 1 + i__ * y_dim1],
		 &c__1);
	i__2 = *n - *k;
	wscal_(&i__2, &tau[i__], &y[*k + 1 + i__ * y_dim1], &c__1);
...
/*        Compute T(1:I,I) */
	...
	wscal_(&i__2, &z__1, &t[i__ * t_dim1 + 1], &c__1);
	i__2 = i__ - 1;
	wtrmv_("Upper", "No Transpose", "NON-UNIT", &i__2, &t[t_offset], ldt, 
		&t[i__ * t_dim1 + 1], &c__1)
		;
	i__2 = i__ + i__ * t_dim1;
	i__3 = i__;
...
/*     Compute Y(1:K,1:NB) */
    ...
    wlacpy_("ALL", k, nb, &a[(a_dim1 << 1) + 1], lda, &y[y_offset], ldy);
    wtrmm_("RIGHT", "Lower", "NO TRANSPOSE", "UNIT", k, nb, &c_b2, &a[*k + 1 
	    + a_dim1], lda, &y[y_offset], ldy);
    if (*n > *k + *nb) {
	i__1 = *n - *k - *nb;
	wgemm_("NO TRANSPOSE", "NO TRANSPOSE", k, nb, &i__1, &c_b2, &a[(*nb + 
		2) * a_dim1 + 1], lda, &a[*k + 1 + *nb + a_dim1], lda, &c_b2, 
		&y[y_offset], ldy);
    }
    wtrmm_("RIGHT", "Upper", "NO TRANSPOSE", "NON-UNIT", k, nb, &c_b2, &t[
