...
/* > */
/*  ===================================================================== */
void  hlasd3_(integer *nl, integer *nr, integer *sqre, integer 
	*k, halfreal *d__, halfreal *q, integer *ldq, halfreal *dsigma, 
	halfreal *u, integer *ldu, halfreal *u2, integer *ldu2, 
	halfreal *vt, integer *ldvt, halfreal *vt2, integer *ldvt2, 
	integer *idxc, integer *ctot, halfreal *z__, integer *info)
{
...
/*     Quick return if possible */
    ...
    if (*k == 1) {
	d__[1] = abs(z__[1]);
	hcopy_(&m, &vt2[vt2_dim1 + 1], ldvt2, &vt[vt_dim1 + 1], ldvt);
	if (z__[1] > 0.) {
	    hcopy_(&n, &u2[u2_dim1 + 1], &c__1, &u[u_dim1 + 1], &c__1);
	} else {
	    i__1 = n;
...
/*     Keep a copy of Z. */
    ...
    hcopy_(k, &z__[1], &c__1, &q[q_offset], &c__1);

/*     Normalize Z. */

    rho = hnrm2_(k, &z__[1], &c__1);
    hlascl_("G", &c__0, &c__0, &rho, &c_b13, k, &c__1, &z__[1], k, info);
    rho *= rho;
...
/*     Find the new singular values. */
    ...
    i__1 = *k;
    for (j = 1; j <= i__1; ++j) {
	hlasd4_(k, &j, &dsigma[1], &z__[1], &u[j * u_dim1 + 1], &rho, &d__[j],
		 &vt[j * vt_dim1 + 1], info);
...
/* L70: */
	...
	temp = hnrm2_(k, &u[i__ * u_dim1 + 1], &c__1);
...
/*     Update the left singular vector matrix. */
    ...
    if (*k == 2) {
	hgemm_("N", "N", &n, k, k, &c_b13, &u2[u2_offset], ldu2, &q[q_offset],
		 ldq, &c_b26, &u[u_offset], ldu);
	goto L100;
    }
    if (ctot[1] > 0) {
	hgemm_("N", "N", nl, k, &ctot[1], &c_b13, &u2[(u2_dim1 << 1) + 1], 
		ldu2, &q[q_dim1 + 2], ldq, &c_b26, &u[u_dim1 + 1], ldu);
	if (ctot[3] > 0) {
	    ktemp = ctot[1] + 2 + ctot[2];
	    hgemm_("N", "N", nl, k, &ctot[3], &c_b13, &u2[ktemp * u2_dim1 + 1]
	...
	hlacpy_("F", nl, k, &u2[u2_offset], ldu2, &u[u_offset], ldu);
    }
    hcopy_(k, &q[q_dim1 + 1], ldq, &u[nlp1 + u_dim1], ldu);
    ktemp = ctot[1] + 2;
    ctemp = ctot[2] + ctot[3];
    hgemm_("N", "N", nr, k, &ctemp, &c_b13, &u2[nlp2 + ktemp * u2_dim1], ldu2,
...
L100:
    i__1 = *k;
    for (i__ = 1; i__ <= i__1; ++i__) {
	temp = hnrm2_(k, &vt[i__ * vt_dim1 + 1], &c__1);
...
/*     Update the right singular vector matrix. */
    ...
    if (*k == 2) {
	hgemm_("N", "N", k, &m, k, &c_b13, &q[q_offset], ldq, &vt2[vt2_offset]
		, ldvt2, &c_b26, &vt[vt_offset], ldvt);
	return;
    }
    ktemp = ctot[1] + 1;
    hgemm_("N", "N", k, &nlp1, &ktemp, &c_b13, &q[q_dim1 + 1], ldq, &vt2[
	    vt2_dim1 + 1], ldvt2, &c_b26, &vt[vt_dim1 + 1], ldvt);
    ktemp = ctot[1] + 2 + ctot[2];
    if (ktemp <= *ldvt2) {
	hgemm_("N", "N", k, &nlp1, &ctot[3], &c_b13, &q[ktemp * q_dim1 + 1], 
...
/* L140: */
    ...
    hgemm_("N", "N", k, &nrp1, &ctemp, &c_b13, &q[ktemp * q_dim1 + 1], ldq, &
