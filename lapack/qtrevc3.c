...
static quadreal c_b17 = 0.;
static logical c_false = FALSE_;
static quadreal c_b29 = 1.;
...
/* > a doublereal upper quasi-triangular matrix T. */
/* > Matrices of this type are produced by the Schur factorization of */
/* > a doublereal general matrix:  A = Q*T*Q**T, as computed by DHSEQR. */
/* > */
...
/* >          computed. */
/* >          If w(j) is a doublereal eigenvalue, the corresponding doublereal */
/* >          eigenvector is computed if SELECT(j) is .TRUE.. */
/* >          If w(j) and w(j+1) are the doublereal and imaginary parts of a */
...
/* >          doublereal part, and the second the imaginary part. */
...
/* >          Each selected doublereal eigenvector occupies one column and each */
...
/* > */
/*  ===================================================================== */
void  qtrevc3_(char *side, char *howmny, logical *select, 
	integer *n, quadreal *t, integer *ldt, quadreal *vl, integer *
	ldvl, quadreal *vr, integer *ldvr, integer *mm, integer *m, 
	quadreal *work, integer *lwork, integer *info)
{
    ...
    logical somev;
    quadreal xnorm;
    extern void  qlaln2_(logical *, integer *, integer *, 
...
/*     Zero-out the workspace to avoid potential NaN propagation. */
	...
	nb = f2cmin(nb,128);
	i__2 = (nb << 1) + 1;
	qlaset_("F", n, &i__2, &c_b17, &c_b17, &work[1], n);
    } else {
	nb = 1;
    }
...
/*     Index IP is used to specify the doublereal or complex eigenvalue: */
/*       IP = 0, doublereal eigenvalue, */
...
/*        For complex right vector, uses IV-1 for doublereal part and IV for complex part. */
...
/*              last column, so this ki must be doublereal eigenvalue */
		ip = 0;
	    } else if (t[ki + (ki - 1) * t_dim1] == 0.) {
/*              zero on sub-diagonal, so this ki is doublereal eigenvalue */
		ip = 0;
	    } else {
...
/*           Compute the KI-th eigenvalue (WR,WI). */
	    ...
	    wi = 0.;
	    if (ip != 0) {
		wi = M(sqrt)((d__1 = t[ki + (ki - 1) * t_dim1], abs(d__1))) * 
			M(sqrt)((d__2 = t[ki - 1 + ki * t_dim1], abs(d__2)));
	    }
/* Computing MAX */
...
/*                    1-by-1 diagonal block */
			...
			qlaln2_(&c_false, &c__1, &c__1, &smin, &c_b29, &t[j + 
...
/*                    Scale if necessary */
			...
			if (scale != 1.) {
			    qscal_(&ki, &scale, &work[iv * *n + 1], &c__1);
			}
			work[j + iv * *n] = x[0];
...
/*                    Update right-hand side */
			...
			i__2 = j - 1;
			d__1 = -x[0];
			qaxpy_(&i__2, &d__1, &t[j * t_dim1 + 1], &c__1, &work[
				iv * *n + 1], &c__1);
...
/*                    2-by-2 diagonal block */
			...
			qlaln2_(&c_false, &c__2, &c__1, &smin, &c_b29, &t[j - 
...
/*                    Update right-hand side */
			...
			i__2 = j - 2;
			d__1 = -x[0];
			qaxpy_(&i__2, &d__1, &t[(j - 1) * t_dim1 + 1], &c__1, 
				&work[iv * *n + 1], &c__1);
			i__2 = j - 2;
			d__1 = -x[1];
			qaxpy_(&i__2, &d__1, &t[j * t_dim1 + 1], &c__1, &work[
...
/*                 no back-transform: copy x to VR and normalize. */
		    qcopy_(&ki, &work[iv * *n + 1], &c__1, &vr[is * vr_dim1 + 
			    1], &c__1);

		    ii = iqamax_(&ki, &vr[is * vr_dim1 + 1], &c__1);
		    remax = 1. / (d__1 = vr[ii + is * vr_dim1], abs(d__1));
		    qscal_(&ki, &remax, &vr[is * vr_dim1 + 1], &c__1);
...
/*                 version 1: back-transform each vector with GEMV, Q*x. */
		    if (ki > 1) {
			i__2 = ki - 1;
			qgemv_("N", n, &i__2, &c_b29, &vr[vr_offset], ldvr, &
				work[iv * *n + 1], &c__1, &work[ki + iv * *n],
				 &vr[ki * vr_dim1 + 1], &c__1);
		    }

		    ii = iqamax_(n, &vr[ki * vr_dim1 + 1], &c__1);
		    remax = 1. / (d__1 = vr[ii + ki * vr_dim1], abs(d__1));
		    qscal_(n, &remax, &vr[ki * vr_dim1 + 1], &c__1);
...
/*                    1-by-1 diagonal block */
			...
			qlaln2_(&c_false, &c__1, &c__2, &smin, &c_b29, &t[j + 
				j * t_dim1], ldt, &c_b29, &c_b29, &work[j + (
...
/*                    2-by-2 diagonal block */
			...
			qlaln2_(&c_false, &c__2, &c__2, &smin, &c_b29, &t[j - 
				1 + (j - 1) * t_dim1], ldt, &c_b29, &c_b29, &
...
/*                 ------------------------------ */
/*                 no back-transform: copy x to VR and normalize. */
		    qcopy_(&ki, &work[(iv - 1) * *n + 1], &c__1, &vr[(is - 1) 
			    * vr_dim1 + 1], &c__1);
		    qcopy_(&ki, &work[iv * *n + 1], &c__1, &vr[is * vr_dim1 + 
			    1], &c__1);
...
/* L100: */
		    ...
		    remax = 1. / emax;
		    qscal_(&ki, &remax, &vr[(is - 1) * vr_dim1 + 1], &c__1);
		    qscal_(&ki, &remax, &vr[is * vr_dim1 + 1], &c__1);
...
/*                 version 1: back-transform each vector with GEMV, Q*x. */
			...
			qscal_(n, &work[ki - 1 + (iv - 1) * *n], &vr[(ki - 1) 
				* vr_dim1 + 1], &c__1);
			qscal_(n, &work[ki + iv * *n], &vr[ki * vr_dim1 + 1], 
				&c__1);
		    }
...
/* L120: */
		    ...
		    remax = 1. / emax;
		    qscal_(n, &remax, &vr[(ki - 1) * vr_dim1 + 1], &c__1);
		    qscal_(n, &remax, &vr[ki * vr_dim1 + 1], &c__1);
...
/*              or if this was last vector, do the GEMM */
		if (iv <= 2 || ki2 == 1) {
		    i__2 = nb - iv + 1;
		    i__3 = ki2 + nb - iv;
		    qgemm_("N", "N", n, &i__2, &i__3, &c_b29, &vr[vr_offset], 
...
/*                 normalize vectors */
		    ...
		    for (k = iv; k <= i__2; ++k) {
			if (iscomplex[k - 1] == 0) {
/*                       doublereal eigenvector */
			    ii = iqamax_(n, &work[(nb + k) * *n + 1], &c__1);
...
/*                       reuse same REMAX as previous K */
			...
			qscal_(n, &remax, &work[(nb + k) * *n + 1], &c__1);
		    }
		    i__2 = nb - iv + 1;
		    qlacpy_("F", n, &i__2, &work[(nb + iv) * *n + 1], n, &vr[
			    ki2 * vr_dim1 + 1], ldvr);
		    iv = nb;
		} else {
		    --iv;
...
/*        IV is index of column in current block. */
/*        For complex left vector, uses IV for doublereal part and IV+1 for complex part. */
...
/*                    the right-hand side. */
			...
			if (work[j] > vcrit) {
			    rec = 1. / vmax;
			    i__4 = *n - ki + 1;
			    qscal_(&i__4, &rec, &work[ki + iv * *n], &c__1);
			    vmax = 1.;
			    vcrit = bignum;
			}

			i__4 = j - ki - 1;
			work[j + iv * *n] -= qdot_(&i__4, &t[ki + 1 + j * 
...
/*                    Scale if necessary */
			...
			if (scale != 1.) {
			    i__4 = *n - ki + 1;
			    qscal_(&i__4, &scale, &work[ki + iv * *n], &c__1);
			}
			work[j + iv * *n] = x[0];
/* Computing MAX */
...
/* Computing MAX */
			...
			i__4 = j - ki - 1;
			work[j + 1 + iv * *n] -= qdot_(&i__4, &t[ki + 1 + (j 
...
/*                    [ T(J+1,J)    T(J+1,J+1)-WR ]                ( WORK2 ) */
			...
			qlaln2_(&c_true, &c__2, &c__1, &smin, &c_b29, &t[j + 
				j * t_dim1], ldt, &c_b29, &c_b29, &work[j + 
				iv * *n], n, &wr, &c_b17, x, &c__2, &scale, &
				xnorm, &ierr);
...
/*                 no back-transform: copy x to VL and normalize. */
		    i__3 = *n - ki + 1;
		    qcopy_(&i__3, &work[ki + iv * *n], &c__1, &vl[ki + is * 
			    vl_dim1], &c__1);

		    i__3 = *n - ki + 1;
		    ii = iqamax_(&i__3, &vl[ki + is * vl_dim1], &c__1) + ki - 
			    1;
		    remax = 1. / (d__1 = vl[ii + is * vl_dim1], abs(d__1));
		    i__3 = *n - ki + 1;
		    qscal_(&i__3, &remax, &vl[ki + is * vl_dim1], &c__1);
...
/*                 version 1: back-transform each vector with GEMV, Q*x. */
		    if (ki < *n) {
			i__3 = *n - ki;
			qgemv_("N", n, &i__3, &c_b29, &vl[(ki + 1) * vl_dim1 
				+ 1], ldvl, &work[ki + 1 + iv * *n], &c__1, &
				work[ki + iv * *n], &vl[ki * vl_dim1 + 1], &
				c__1);
		    }

		    ii = iqamax_(n, &vl[ki * vl_dim1 + 1], &c__1);
		    remax = 1. / (d__1 = vl[ii + ki * vl_dim1], abs(d__1));
		    qscal_(n, &remax, &vl[ki * vl_dim1 + 1], &c__1);
...
/*                    forming the right-hand side elements. */
			...
			if (work[j] > vcrit) {
			    ...
			    qscal_(&i__4, &rec, &work[ki + iv * *n], &c__1);
			    i__4 = *n - ki + 1;
			    qscal_(&i__4, &rec, &work[ki + (iv + 1) * *n], &
				    c__1);
			    vmax = 1.;
			    vcrit = bignum;
			}
			...
			i__4 = j - ki - 2;
			work[j + iv * *n] -= qdot_(&i__4, &t[ki + 2 + j * 
				t_dim1], &c__1, &work[ki + 2 + iv * *n], &
				c__1);
			i__4 = j - ki - 2;
			work[j + (iv + 1) * *n] -= qdot_(&i__4, &t[ki + 2 + j 
...
/*                    Scale if necessary */
			...
			if (scale != 1.) {
			    ...
			    qscal_(&i__4, &scale, &work[ki + iv * *n], &c__1);
			    i__4 = *n - ki + 1;
			    qscal_(&i__4, &scale, &work[ki + (iv + 1) * *n], &
				    c__1);
			}
			work[j + iv * *n] = x[0];
			work[j + (iv + 1) * *n] = x[2];
...
/* Computing MAX */
			...
			i__4 = j - ki - 2;
			work[j + 1 + iv * *n] -= qdot_(&i__4, &t[ki + 2 + (j 
				+ 1) * t_dim1], &c__1, &work[ki + 2 + iv * *n]
				, &c__1);

			i__4 = j - ki - 2;
			work[j + 1 + (iv + 1) * *n] -= qdot_(&i__4, &t[ki + 2 
...
/*                    [ (T(j+1,j) T(j+1,j+1))                  ] */
			...
			d__1 = -wi;
			qlaln2_(&c_true, &c__2, &c__2, &smin, &c_b29, &t[j + 
				j * t_dim1], ldt, &c_b29, &c_b29, &work[j + 
				iv * *n], n, &wr, &d__1, x, &c__2, &scale, &
				xnorm, &ierr);
...
/*                 no back-transform: copy x to VL and normalize. */
		    ...
		    qcopy_(&i__3, &work[ki + iv * *n], &c__1, &vl[ki + is * 
			    vl_dim1], &c__1);
		    i__3 = *n - ki + 1;
		    qcopy_(&i__3, &work[ki + (iv + 1) * *n], &c__1, &vl[ki + (
			    is + 1) * vl_dim1], &c__1);
...
/* L220: */
		    ...
		    qscal_(&i__3, &remax, &vl[ki + is * vl_dim1], &c__1);
		    i__3 = *n - ki + 1;
		    qscal_(&i__3, &remax, &vl[ki + (is + 1) * vl_dim1], &c__1)
			    ;
...
/*                 version 1: back-transform each vector with GEMV, Q*x. */
		    if (ki < *n - 1) {
			i__3 = *n - ki - 1;
			qgemv_("N", n, &i__3, &c_b29, &vl[(ki + 2) * vl_dim1 
			...
			qscal_(n, &work[ki + iv * *n], &vl[ki * vl_dim1 + 1], 
				&c__1);
			qscal_(n, &work[ki + 1 + (iv + 1) * *n], &vl[(ki + 1) 
				* vl_dim1 + 1], &c__1);
		    }
...
/* L240: */
		    ...
		    remax = 1. / emax;
		    qscal_(n, &remax, &vl[ki * vl_dim1 + 1], &c__1);
		    qscal_(n, &remax, &vl[(ki + 1) * vl_dim1 + 1], &c__1);
...
/*              or if this was last vector, do the GEMM */
		if (iv >= nb - 1 || ki2 == *n) {
		    i__3 = *n - ki2 + iv;
		    qgemm_("N", "N", n, &iv, &i__3, &c_b29, &vl[(ki2 - iv + 1)
...
/*                       reuse same REMAX as previous K */
		    ...
		    qlacpy_("F", n, &iv, &work[(nb + 1) * *n + 1], n, &vl[(
