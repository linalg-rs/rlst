...
static quadreal c_b35 = 10.;
static quadreal c_b71 = .5;
...
/* > */
/*  ===================================================================== */
void  qggbal_(char *job, integer *n, quadreal *a, integer *
	lda, quadreal *b, integer *ldb, integer *ilo, integer *ihi, 
...
L160:
    ...
    i__1 = *n - k + 1;
    qswap_(&i__1, &a[i__ + k * a_dim1], lda, &a[m + k * a_dim1], lda);
    i__1 = *n - k + 1;
    qswap_(&i__1, &b[i__ + k * b_dim1], ldb, &b[m + k * b_dim1], ldb);
...
L170:
    ...
    qswap_(&l, &a[j * a_dim1 + 1], &c__1, &a[m * a_dim1 + 1], &c__1);
    qswap_(&l, &b[j * b_dim1 + 1], &c__1, &b[m * b_dim1 + 1], &c__1);
...
L350:
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	irab = iqamax_(&i__2, &a[i__ + *ilo * a_dim1], lda);
	rab = (d__1 = a[i__ + (irab + *ilo - 1) * a_dim1], abs(d__1));
	i__2 = *n - *ilo + 1;
	irab = iqamax_(&i__2, &b[i__ + *ilo * b_dim1], ldb);
/* Computing MAX */
...
/* Computing MIN */
	...
	icab = iqamax_(ihi, &a[i__ * a_dim1 + 1], &c__1);
	cab = (d__1 = a[icab + i__ * a_dim1], abs(d__1));
	icab = iqamax_(ihi, &b[i__ * b_dim1 + 1], &c__1);
/* Computing MAX */
...
/*     Row scaling of matrices A and B */
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	qscal_(&i__2, &lscale[i__], &a[i__ + *ilo * a_dim1], lda);
	i__2 = *n - *ilo + 1;
	qscal_(&i__2, &lscale[i__], &b[i__ + *ilo * b_dim1], ldb);
/* L370: */
    }
...
/*     Column scaling of matrices A and B */
    ...
    i__1 = *ihi;
    for (j = *ilo; j <= i__1; ++j) {
	qscal_(ihi, &rscale[j], &a[j * a_dim1 + 1], &c__1);
	qscal_(ihi, &rscale[j], &b[j * b_dim1 + 1], &c__1);
/* L380: */
    }
