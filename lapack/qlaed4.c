...
/* > */
/*  ===================================================================== */
void  qlaed4_(integer *n, integer *i__, quadreal *d__, 
	quadreal *z__, quadreal *delta, quadreal *rho, quadreal *dlam,
	 integer *info)
{
    ...
    quadreal dw, zz[3];
    integer ip1;
    quadreal del, eta, phi, eps, tau, psi;
    integer iim1, iip1;
    quadreal dphi, dpsi;
    integer iter;
    quadreal temp, prew, temp1, dltlb, dltub, midpt;
    integer niter;
    logical swtch;
    extern void  qlaed5_(integer *, quadreal *, quadreal *,
	     quadreal *, quadreal *, quadreal *), qlaed6_(integer *, 
	    logical *, quadreal *, quadreal *, quadreal *, quadreal *,
	     quadreal *, integer *);
    logical swtch3;
    extern quadreal qlamch_(char *);
    logical orgati;
    quadreal erretm, rhoinv;
...
/*         Presumably, I=1 upon entry */
	...
	*dlam = d__[1] + *rho * z__[1] * z__[1];
    ...
    if (*n == 2) {
	qlaed5_(i__, &d__[1], &z__[1], &delta[1], rho, dlam);
	return;
    }
...
/* L20: */
	...
	if (w <= 0.) {
		...
		b = z__[*n] * z__[*n] * del;
		if (a < 0.) {
		    tau = b * 2. / (M(sqrt)(a * a + b * 4. * c__) - a);
		} else {
		    tau = (a + M(sqrt)(a * a + b * 4. * c__)) / (c__ * 2.);
		}
	    }
...
/*           We choose d(i) as origin. */
	    ...
	    if (a > 0.) {
		tau = b * 2. / (a + M(sqrt)((d__1 = a * a - b * 4. * c__, abs(
			d__1))));
	    } else {
		tau = (a - M(sqrt)((d__1 = a * a - b * 4. * c__, abs(d__1)))) / (
			c__ * 2.);
	    }
	    dltlb = 0.;
	    dltub = midpt;
...
/*           Interpolation using THREE most relevant poles */
	    ...
	    zz[1] = z__[ii] * z__[ii];
	    qlaed6_(&niter, &orgati, &c__, &delta[iim1], zz, &w, &eta, info);
	    if (*info != 0) {
		goto L250;
	    }
	}
...
/*              Interpolation using THREE most relevant poles */
		...
		qlaed6_(&niter, &orgati, &c__, &delta[iim1], zz, &w, &eta, 
			info);
		if (*info != 0) {
		    goto L250;
		}
