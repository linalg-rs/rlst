...
/* > \verbatim */
/* > */
/* > DSYTRF_AA computes the factorization of a doublereal symmetric matrix A */
...
/*  ===================================================================== */
void  dsytrf_aa__(char *uplo, integer *n, halfreal *a, 
	integer *lda, integer *ipiv, halfreal *work, integer *lwork, 
	integer *info)
{
    ...
    /* Local variables */
    integer j;
    extern void  dlasyf_aa__(char *, integer *, integer *, 
...
/*        Copy first row A(1, 1:N) into H(1:n) (stored in WORK(1:N)) */
	...
	hcopy_(n, &a[a_dim1 + 1], lda, &work[1], &c__1);
...
/*        Panel factorization */
	...
	i__1 = 2 - k1;
	i__2 = *n - j;
	dlasyf_aa__(uplo, &i__1, &i__2, &jb, &a[f2cmax(1,j) + (j + 1) * a_dim1], 
...
/* Computing MIN */
	...
	for (j2 = j + 2; j2 <= i__1; ++j2) {
		...
		i__2 = j1 - k1 - 2;
		hswap_(&i__2, &a[j2 * a_dim1 + 1], &c__1, &a[ipiv[j2] * 
			a_dim1 + 1], &c__1);
	    }
	}
	j += jb;
...
/*              Merge rank-1 update with BLAS-3 update */
		...
		a[j + (j + 1) * a_dim1] = 1.;
		i__1 = *n - j;
		hcopy_(&i__1, &a[j - 1 + (j + 1) * a_dim1], lda, &work[j + 1 
			- j1 + 1 + jb * *n], &c__1);
		i__1 = *n - j;
		hscal_(&i__1, &alpha, &work[j + 1 - j1 + 1 + jb * *n], &c__1);
...
/*                 Update (J2, J2) diagonal block with DGEMV */
		    ...
		    for (mj = nj - 1; mj >= 1; --mj) {
			i__3 = jb + 1;
			hgemv_("No transpose", &mj, &i__3, &c_b18, &work[j3 - 
...
/*                 Update off-diagonal block of J2-th block row with DGEMM */
		    ...
		    i__3 = *n - j3 + 1;
		    i__4 = jb + 1;
		    hgemm_("Transpose", "Transpose", &nj, &i__3, &i__4, &
...
/*           WORK(J+1, 1) stores H(J+1, 1) */
	    ...
	    i__2 = *n - j;
	    hcopy_(&i__2, &a[j + 1 + (j + 1) * a_dim1], lda, &work[1], &c__1);
	}
	goto L10;
    } else {
...
/*         (stored in WORK(1:N)) */
	...
	hcopy_(n, &a[a_dim1 + 1], &c__1, &work[1], &c__1);
...
/*        Panel factorization */
	...
	i__2 = 2 - k1;
	i__1 = *n - j;
	dlasyf_aa__(uplo, &i__2, &i__1, &jb, &a[j + 1 + f2cmax(1,j) * a_dim1], 
		lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1])
		;
...
/* Computing MIN */
	...
	for (j2 = j + 2; j2 <= i__2; ++j2) {
	    ...
	    if (j2 != ipiv[j2] && j1 - k1 > 2) {
		i__1 = j1 - k1 - 2;
		hswap_(&i__1, &a[j2 + a_dim1], lda, &a[ipiv[j2] + a_dim1], 
			lda);
	    }
	}
	j += jb;
...
/*              Merge rank-1 update with BLAS-3 update */
		...
		a[j + 1 + j * a_dim1] = 1.;
		i__2 = *n - j;
		hcopy_(&i__2, &a[j + 1 + (j - 1) * a_dim1], &c__1, &work[j + 
			1 - j1 + 1 + jb * *n], &c__1);
		i__2 = *n - j;
		hscal_(&i__2, &alpha, &work[j + 1 - j1 + 1 + jb * *n], &c__1);
...
/*                 Update off-diagonal block in J2-th block column with DGEMM */
		    ...
		    i__3 = *n - j3 + 1;
		    i__4 = jb + 1;
		    hgemm_("No transpose", "Transpose", &i__3, &nj, &i__4, &
...
/*           WORK(J+1, 1) stores H(J+1, 1) */
	    ...
	    i__1 = *n - j;
	    hcopy_(&i__1, &a[j + 1 + (j + 1) * a_dim1], &c__1, &work[1], &
		    c__1);
	}
	goto L11;
    }
