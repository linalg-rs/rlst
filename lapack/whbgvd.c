...
/*  ===================================================================== */
void  whbgvd_(char *jobz, char *uplo, integer *n, integer *ka, 
	integer *kb, quadcomplex *ab, integer *ldab, quadcomplex *bb, 
	integer *ldbb, quadreal *w, quadcomplex *z__, integer *ldz, 
	quadcomplex *work, integer *lwork, quadreal *rwork, integer *
	lrwork, integer *iwork, integer *liwork, integer *info)
{
    ...
    logical wantz;
    integer indwk2;
    extern void  xerbla_(char *, integer *), qsterf_(
	    integer *, quadreal *, quadreal *, integer *), wstedc_(char *,
	     integer *, quadreal *, quadreal *, quadcomplex *, integer *
	    , quadcomplex *, integer *, quadreal *, integer *, integer *, 
	    integer *, integer *), whbtrd_(char *, char *, integer *, 
...
/*     Reduce Hermitian band matrix to tridiagonal form. */
	...
	*(unsigned char *)vect = 'N';
    ...
    whbtrd_(vect, uplo, n, ka, &ab[ab_offset], ldab, &w[1], &rwork[inde], &
	    z__[z_offset], ldz, &work[1], &iinfo);

/*     For eigenvalues only, call DSTERF.  For eigenvectors, call ZSTEDC. */

    if (! wantz) {
	qsterf_(n, &w[1], &rwork[inde], info);
    } else {
	wstedc_("I", n, &w[1], &rwork[inde], &work[1], n, &work[indwk2], &
		llwk2, &rwork[indwrk], &llrwk, &iwork[1], liwork, info);
	wgemm_("N", "N", n, n, n, &c_b1, &z__[z_offset], ldz, &work[1], n, &
		c_b2, &work[indwk2], n);
	wlacpy_("A", n, n, &work[indwk2], n, &z__[z_offset], ldz);
    }
