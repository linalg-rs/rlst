...
/*  ===================================================================== */
void  qgeesx_(char *jobvs, char *sort, L_fp select, char *
	sense, integer *n, quadreal *a, integer *lda, integer *sdim, 
	quadreal *wr, quadreal *wi, quadreal *vs, integer *ldvs, 
	quadreal *rconde, quadreal *rcondv, quadreal *work, integer *
	lwork, integer *iwork, integer *liwork, logical *bwork, integer *info)
{
    ...
    extern void  qgehrd_(integer *, integer *, integer *, 
	    ...
	    integer *), qlacpy_(char *, integer *, integer *, 
    ...
    logical wantsb;
    extern void  qtrsen_(char *, char *, logical *, integer *, 
...
/*       in the code.) */
	...
	if (*n == 0) {
	    ...
	    qhseqr_("S", jobvs, n, &c__1, n, &a[a_offset], lda, &wr[1], &wi[1]
...
/*     (RWorkspace: need N) */
    ...
    ibal = 1;
    qgebal_("P", n, &a[a_offset], lda, &ilo, &ihi, &work[ibal], &ierr);
...
/*        Copy Householder vectors to VS */
	...
	qlacpy_("L", n, n, &a[a_offset], lda, &vs[vs_offset], ldvs)
		;
...
/*        (RWorkspace: need 3*N-1, prefer 2*N+(N-1)*NB) */
	...
	i__1 = *lwork - iwrk + 1;
	qorghr_(n, &ilo, &ihi, &vs[vs_offset], ldvs, &work[itau], &work[iwrk],
		 &i__1, &ierr);
    }
...
/*     (RWorkspace: need N+1, prefer N+HSWORK (see comments) ) */
    ...
    iwrk = itau;
    i__1 = *lwork - iwrk + 1;
    qhseqr_("S", jobvs, n, &ilo, &ihi, &a[a_offset], lda, &wr[1], &wi[1], &vs[
...
/*                     otherwise, need 0 ) */
	...
	i__1 = *lwork - iwrk + 1;
	qtrsen_(sense, jobvs, &bwork[1], n, &a[a_offset], lda, &vs[vs_offset],
...
/*        (RWorkspace: need N) */
	...
	qgebak_("P", "R", n, &ilo, &ihi, &work[ibal], n, &vs[vs_offset], ldvs,
		 &ierr);
    }
...
/*        Undo scaling for the Schur form of A */
	...
	qlascl_("H", &c__0, &c__0, &cscale, &anrm, n, n, &a[a_offset], lda, &
		ierr);
	i__1 = *lda + 1;
	qcopy_(n, &a[a_offset], &i__1, &wr[1], &c__1);
...
/*           underflows. */
		...
		if (wi[i__] == 0.) {
			...
			if (i__ > 1) {
			    i__2 = i__ - 1;
			    qswap_(&i__2, &a[i__ * a_dim1 + 1], &c__1, &a[(
				    i__ + 1) * a_dim1 + 1], &c__1);
			}
			if (*n > i__ + 1) {
			    i__2 = *n - i__ - 1;
			    qswap_(&i__2, &a[i__ + (i__ + 2) * a_dim1], lda, &
				    a[i__ + 1 + (i__ + 2) * a_dim1], lda);
			}
			qswap_(n, &vs[i__ * vs_dim1 + 1], &c__1, &vs[(i__ + 1)
				 * vs_dim1 + 1], &c__1);
