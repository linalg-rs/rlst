...
/* > */
/*  ===================================================================== */
void  wtgsja_(char *jobu, char *jobv, char *jobq, integer *m, 
	integer *p, integer *n, integer *k, integer *l, quadcomplex *a, 
	integer *lda, quadcomplex *b, integer *ldb, quadreal *tola, 
	quadreal *tolb, quadreal *alpha, quadreal *beta, quadcomplex *
	u, integer *ldu, quadcomplex *v, integer *ldv, quadcomplex *q, 
	integer *ldq, quadcomplex *work, integer *ncycle, integer *info)
{
    ...
    /* Local variables */
    integer i__, j;
    quadreal a1, b1, a3, b3;
    quadcomplex a2, b2;
    quadreal csq, csu, csv;
    quadcomplex snq;
    quadreal rwk;
    quadcomplex snu, snv;
    ...
    quadreal error, ssmin;
    logical wantu, wantv;
    extern void  wcopy_(integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *), wlags2_(logical *, quadreal *, 
	    quadcomplex *, quadreal *, quadreal *, quadcomplex *, 
	    quadreal *, quadreal *, quadcomplex *, quadreal *, 
	    quadcomplex *, quadreal *, quadcomplex *);
    integer kcycle;
    extern void  qlartg_(quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *), xerbla_(char *, 
	    integer *), wqscal_(integer *, quadreal *, 
	    quadcomplex *, integer *), wlapll_(integer *, quadcomplex *, 
	    integer *, quadcomplex *, integer *, quadreal *), wlaset_(
...
/*     Initialize U, V and Q, if necessary */
    ...
    if (initu) {
	wlaset_("Full", m, m, &c_b1, &c_b2, &u[u_offset], ldu);
    }
    if (initv) {
	wlaset_("Full", p, p, &c_b1, &c_b2, &v[v_offset], ldv);
    }
    if (initq) {
...
/*     Loop until convergence */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		wlags2_(&upper, &a1, &a2, &a3, &b1, &b2, &b3, &csu, &snu, &
			csv, &snv, &csq, &snq);
...
/*              Update (K+I)-th and (K+J)-th rows of matrix A: U**H *A */
		...
		if (*k + j <= *m) {
		    d_cnjg(&z__1, &snu);
		    wrot_(l, &a[*k + j + (*n - *l + 1) * a_dim1], lda, &a[*k 
...
/*              Update I-th and J-th rows of matrix B: V**H *B */
		...
		d_cnjg(&z__1, &snv);
		wrot_(l, &b[j + (*n - *l + 1) * b_dim1], ldb, &b[i__ + (*n - *
...
/* Computing MIN */
		i__4 = *k + *l;
		i__3 = f2cmin(i__4,*m);
		wrot_(&i__3, &a[(*n - *l + j) * a_dim1 + 1], &c__1, &a[(*n - *
			l + i__) * a_dim1 + 1], &c__1, &csq, &snq);

		wrot_(l, &b[(*n - *l + j) * b_dim1 + 1], &c__1, &b[(*n - *l + 
...
/*              Update unitary matrices U, V, Q, if desired. */
		...
		if (wantu && *k + j <= *m) {
		    wrot_(m, &u[(*k + j) * u_dim1 + 1], &c__1, &u[(*k + i__) *
			     u_dim1 + 1], &c__1, &csu, &snu);
		}

		if (wantv) {
		    wrot_(p, &v[j * v_dim1 + 1], &c__1, &v[i__ * v_dim1 + 1], 
			    &c__1, &csv, &snv);
		}

		if (wantq) {
		    wrot_(n, &q[(*n - *l + j) * q_dim1 + 1], &c__1, &q[(*n - *
...
/* Computing MIN */
	    ...
	    for (i__ = 1; i__ <= i__1; ++i__) {
		i__2 = *l - i__ + 1;
		wcopy_(&i__2, &a[*k + i__ + (*n - *l + i__) * a_dim1], lda, &
			work[1], &c__1);
		i__2 = *l - i__ + 1;
		wcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &work[*
			l + 1], &c__1);
		i__2 = *l - i__ + 1;
		wlapll_(&i__2, &work[1], &c__1, &work[*l + 1], &c__1, &ssmin);
...
/* Computing MIN */
	...
	if (a1 != 0.) {
	    ...
	    if (gamma < 0.) {
		i__2 = *l - i__ + 1;
		wqscal_(&i__2, &c_b39, &b[i__ + (*n - *l + i__) * b_dim1], 
			ldb);
		if (wantv) {
		    wqscal_(p, &c_b39, &v[i__ * v_dim1 + 1], &c__1);
		}
	    }

	    d__1 = abs(gamma);
	    qlartg_(&d__1, &c_b42, &beta[*k + i__], &alpha[*k + i__], &rwk);

	    if (alpha[*k + i__] >= beta[*k + i__]) {
		i__2 = *l - i__ + 1;
		d__1 = 1. / alpha[*k + i__];
		wqscal_(&i__2, &d__1, &a[*k + i__ + (*n - *l + i__) * a_dim1],
			 lda);
	    } else {
		i__2 = *l - i__ + 1;
		d__1 = 1. / beta[*k + i__];
		wqscal_(&i__2, &d__1, &b[i__ + (*n - *l + i__) * b_dim1], ldb)
			;
		i__2 = *l - i__ + 1;
		wcopy_(&i__2, &b[i__ + (*n - *l + i__) * b_dim1], ldb, &a[*k 
