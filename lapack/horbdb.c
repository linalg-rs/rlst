...
/* > */
/*  ===================================================================== */
void  horbdb_(char *trans, char *signs, integer *m, integer *p,
	 integer *q, halfreal *x11, integer *ldx11, halfreal *x12, 
	integer *ldx12, halfreal *x21, integer *ldx21, halfreal *x22, 
	integer *ldx22, halfreal *theta, halfreal *phi, halfreal *taup1,
	 halfreal *taup2, halfreal *tauq1, halfreal *tauq2, halfreal *
	work, integer *lwork, integer *info)
{
    ...
    extern void  haxpy_(integer *, halfreal *, halfreal *, 
	    integer *, halfreal *, integer *), xerbla_(char *, integer *);
    logical lquery;
...
/*        Reduce columns 1, ..., Q of X11, X12, X21, and X22 */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    if (i__ == 1) {
		i__2 = *p - i__ + 1;
		hscal_(&i__2, &z1, &x11[i__ + i__ * x11_dim1], &c__1);
	    } else {
		i__2 = *p - i__ + 1;
		d__1 = z1 * M(cos)(phi[i__ - 1]);
		hscal_(&i__2, &d__1, &x11[i__ + i__ * x11_dim1], &c__1);
		i__2 = *p - i__ + 1;
		d__1 = -z1 * z3 * z4 * M(sin)(phi[i__ - 1]);
		haxpy_(&i__2, &d__1, &x12[i__ + (i__ - 1) * x12_dim1], &c__1, 
			&x11[i__ + i__ * x11_dim1], &c__1);
	    }
	    if (i__ == 1) {
		i__2 = *m - *p - i__ + 1;
		hscal_(&i__2, &z2, &x21[i__ + i__ * x21_dim1], &c__1);
	    } else {
		i__2 = *m - *p - i__ + 1;
		d__1 = z2 * M(cos)(phi[i__ - 1]);
		hscal_(&i__2, &d__1, &x21[i__ + i__ * x21_dim1], &c__1);
		i__2 = *m - *p - i__ + 1;
		d__1 = -z2 * z3 * z4 * M(sin)(phi[i__ - 1]);
		haxpy_(&i__2, &d__1, &x22[i__ + (i__ - 1) * x22_dim1], &c__1, 
			&x21[i__ + i__ * x21_dim1], &c__1);
	    }

	    i__2 = *m - *p - i__ + 1;
	    i__3 = *p - i__ + 1;
	    theta[i__] = atan2(hnrm2_(&i__2, &x21[i__ + i__ * x21_dim1], &
		    c__1), hnrm2_(&i__3, &x11[i__ + i__ * x11_dim1], &c__1));

	    if (*p > i__) {
		i__2 = *p - i__ + 1;
		hlarfgp_(&i__2, &x11[i__ + i__ * x11_dim1], &x11[i__ + 1 + 
			i__ * x11_dim1], &c__1, &taup1[i__]);
	    } else if (*p == i__) {
		i__2 = *p - i__ + 1;
		hlarfgp_(&i__2, &x11[i__ + i__ * x11_dim1], &x11[i__ + i__ * 
			x11_dim1], &c__1, &taup1[i__]);
	    }
	    x11[i__ + i__ * x11_dim1] = 1.;
	    if (*m - *p > i__) {
		i__2 = *m - *p - i__ + 1;
		hlarfgp_(&i__2, &x21[i__ + i__ * x21_dim1], &x21[i__ + 1 + 
			i__ * x21_dim1], &c__1, &taup2[i__]);
	    } else if (*m - *p == i__) {
		i__2 = *m - *p - i__ + 1;
		hlarfgp_(&i__2, &x21[i__ + i__ * x21_dim1], &x21[i__ + i__ * 
		...
		i__2 = *p - i__ + 1;
		i__3 = *q - i__;
		hlarf_("L", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], &c__1, &
		...
		i__2 = *m - *p - i__ + 1;
		i__3 = *q - i__;
		hlarf_("L", &i__2, &i__3, &x21[i__ + i__ * x21_dim1], &c__1, &
		...
		hscal_(&i__2, &d__1, &x11[i__ + (i__ + 1) * x11_dim1], ldx11);
		i__2 = *q - i__;
		d__1 = z2 * z3 * M(cos)(theta[i__]);
		haxpy_(&i__2, &d__1, &x21[i__ + (i__ + 1) * x21_dim1], ldx21, 
			&x11[i__ + (i__ + 1) * x11_dim1], ldx11);
	    }
	    i__2 = *m - *q - i__ + 1;
	    d__1 = -z1 * z4 * M(sin)(theta[i__]);
	    hscal_(&i__2, &d__1, &x12[i__ + i__ * x12_dim1], ldx12);
	    i__2 = *m - *q - i__ + 1;
	    d__1 = z2 * z4 * M(cos)(theta[i__]);
	    haxpy_(&i__2, &d__1, &x22[i__ + i__ * x22_dim1], ldx22, &x12[i__ 
		    + i__ * x12_dim1], ldx12);

	    if (i__ < *q) {
		i__2 = *q - i__;
		i__3 = *m - *q - i__ + 1;
		phi[i__] = atan2(hnrm2_(&i__2, &x11[i__ + (i__ + 1) * 
			x11_dim1], ldx11), hnrm2_(&i__3, &x12[i__ + i__ * 
			x12_dim1], ldx12));
	    }
		...
		if (*q - i__ == 1) {
		    i__2 = *q - i__;
		    hlarfgp_(&i__2, &x11[i__ + (i__ + 1) * x11_dim1], &x11[
		...
		if (*m - *q == i__) {
		    i__2 = *m - *q - i__ + 1;
		    hlarfgp_(&i__2, &x12[i__ + i__ * x12_dim1], &x12[i__ + 
		...
		i__2 = *p - i__;
		i__3 = *q - i__;
		hlarf_("R", &i__2, &i__3, &x11[i__ + (i__ + 1) * x11_dim1], 
		...
		i__2 = *p - i__;
		i__3 = *m - *q - i__ + 1;
		hlarf_("R", &i__2, &i__3, &x12[i__ + i__ * x12_dim1], ldx12, &
...
/*        Reduce columns Q + 1, ..., P of X12, X22 */
	...
	for (i__ = *q + 1; i__ <= i__1; ++i__) {
	    ...
	    if (i__ >= *m - *q) {
		i__2 = *m - *q - i__ + 1;
		hlarfgp_(&i__2, &x12[i__ + i__ * x12_dim1], &x12[i__ + i__ * 
...
/*        Reduce columns P + 1, ..., M - Q of X12, X22 */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    i__2 = *m - *p - *q - i__ + 1;
	    d__1 = z2 * z4;
	    hscal_(&i__2, &d__1, &x22[*q + i__ + (*p + i__) * x22_dim1], 
		    ldx22);
	    if (i__ == *m - *p - *q) {
		i__2 = *m - *p - *q - i__ + 1;
		hlarfgp_(&i__2, &x22[*q + i__ + (*p + i__) * x22_dim1], &x22[*
		...
		i__2 = *m - *p - *q - i__;
		i__3 = *m - *p - *q - i__ + 1;
		hlarf_("R", &i__2, &i__3, &x22[*q + i__ + (*p + i__) * 
...
/*        Reduce columns 1, ..., Q of X11, X12, X21, X22 */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    if (i__ == 1) {
		i__2 = *p - i__ + 1;
		hscal_(&i__2, &z1, &x11[i__ + i__ * x11_dim1], ldx11);
	    } else {
		i__2 = *p - i__ + 1;
		d__1 = z1 * M(cos)(phi[i__ - 1]);
		hscal_(&i__2, &d__1, &x11[i__ + i__ * x11_dim1], ldx11);
		i__2 = *p - i__ + 1;
		d__1 = -z1 * z3 * z4 * M(sin)(phi[i__ - 1]);
		haxpy_(&i__2, &d__1, &x12[i__ - 1 + i__ * x12_dim1], ldx12, &
			x11[i__ + i__ * x11_dim1], ldx11);
	    }
	    if (i__ == 1) {
		i__2 = *m - *p - i__ + 1;
		hscal_(&i__2, &z2, &x21[i__ + i__ * x21_dim1], ldx21);
	    } else {
		i__2 = *m - *p - i__ + 1;
		d__1 = z2 * M(cos)(phi[i__ - 1]);
		hscal_(&i__2, &d__1, &x21[i__ + i__ * x21_dim1], ldx21);
		i__2 = *m - *p - i__ + 1;
		d__1 = -z2 * z3 * z4 * M(sin)(phi[i__ - 1]);
		haxpy_(&i__2, &d__1, &x22[i__ - 1 + i__ * x22_dim1], ldx22, &
	    ...
	    i__3 = *p - i__ + 1;
	    theta[i__] = atan2(hnrm2_(&i__2, &x21[i__ + i__ * x21_dim1], 
		    ldx21), hnrm2_(&i__3, &x11[i__ + i__ * x11_dim1], ldx11));
		...
		i__2 = *q - i__;
		i__3 = *p - i__ + 1;
		hlarf_("R", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], ldx11, &
		...
		i__2 = *q - i__;
		i__3 = *m - *p - i__ + 1;
		hlarf_("R", &i__2, &i__3, &x21[i__ + i__ * x21_dim1], ldx21, &
		...
		i__2 = *q - i__;
		d__1 = -z1 * z3 * M(sin)(theta[i__]);
		hscal_(&i__2, &d__1, &x11[i__ + 1 + i__ * x11_dim1], &c__1);
		i__2 = *q - i__;
		d__1 = z2 * z3 * M(cos)(theta[i__]);
		haxpy_(&i__2, &d__1, &x21[i__ + 1 + i__ * x21_dim1], &c__1, &
			x11[i__ + 1 + i__ * x11_dim1], &c__1);
	    }
	    i__2 = *m - *q - i__ + 1;
	    d__1 = -z1 * z4 * M(sin)(theta[i__]);
	    hscal_(&i__2, &d__1, &x12[i__ + i__ * x12_dim1], &c__1);
	    i__2 = *m - *q - i__ + 1;
	    d__1 = z2 * z4 * M(cos)(theta[i__]);
	    haxpy_(&i__2, &d__1, &x22[i__ + i__ * x22_dim1], &c__1, &x12[i__ 
		    + i__ * x12_dim1], &c__1);

	    if (i__ < *q) {
		i__2 = *q - i__;
		i__3 = *m - *q - i__ + 1;
		phi[i__] = atan2(hnrm2_(&i__2, &x11[i__ + 1 + i__ * x11_dim1],
			 &c__1), hnrm2_(&i__3, &x12[i__ + i__ * x12_dim1], &
			c__1));
	    }
		...
		if (*q - i__ == 1) {
		    i__2 = *q - i__;
		    hlarfgp_(&i__2, &x11[i__ + 1 + i__ * x11_dim1], &x11[i__ 
	    ...
	    if (*m - *q > i__) {
		i__2 = *m - *q - i__ + 1;
		hlarfgp_(&i__2, &x12[i__ + i__ * x12_dim1], &x12[i__ + 1 + 
		...
		i__2 = *q - i__;
		i__3 = *p - i__;
		hlarf_("L", &i__2, &i__3, &x11[i__ + 1 + i__ * x11_dim1], &
	    ...
	    i__2 = *m - *q - i__ + 1;
	    i__3 = *p - i__;
	    hlarf_("L", &i__2, &i__3, &x12[i__ + i__ * x12_dim1], &c__1, &
...
/*        Reduce columns Q + 1, ..., P of X12, X22 */
	...
	for (i__ = *q + 1; i__ <= i__1; ++i__) {
	    ...
	    hscal_(&i__2, &d__1, &x12[i__ + i__ * x12_dim1], &c__1);
	    i__2 = *m - *q - i__ + 1;
	    hlarfgp_(&i__2, &x12[i__ + i__ * x12_dim1], &x12[i__ + 1 + i__ * 
		    x12_dim1], &c__1, &tauq2[i__]);
	    x12[i__ + i__ * x12_dim1] = 1.;
...
/*        Reduce columns P + 1, ..., M - Q of X12, X22 */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    i__2 = *m - *p - *q - i__ + 1;
	    d__1 = z2 * z4;
	    hscal_(&i__2, &d__1, &x22[*p + i__ + (*q + i__) * x22_dim1], &
		    c__1);
	    if (*m - *p - *q == i__) {
		i__2 = *m - *p - *q - i__ + 1;
		hlarfgp_(&i__2, &x22[*p + i__ + (*q + i__) * x22_dim1], &x22[*
		...
		i__2 = *m - *p - *q - i__ + 1;
		i__3 = *m - *p - *q - i__;
		hlarf_("L", &i__2, &i__3, &x22[*p + i__ + (*q + i__) * 
