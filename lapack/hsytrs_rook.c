...
/* > a doublereal symmetric matrix A using the factorization A = U*D*U**T or */
...
/*  ===================================================================== */
void  dsytrs_rook__(char *uplo, integer *n, integer *nrhs, 
	...
	ldb, integer *info)
{
    ...
    extern void  hgemv_(char *, integer *, integer *, 
	    ...
	    halfreal *, halfreal *, integer *), hswap_(integer *, 
...
/*           stored in column K of A. */
	    ...
	    i__1 = k - 1;
	    hger_(&i__1, nrhs, &c_b7, &a[k * a_dim1 + 1], &c__1, &b[k + 
		    b_dim1], ldb, &b[b_dim1 + 1], ldb);

/*           Multiply by the inverse of the diagonal block. */

	    d__1 = 1. / a[k + k * a_dim1];
	    hscal_(nrhs, &d__1, &b[k + b_dim1], ldb);
	    --k;
	} else {
...
/*           Interchange rows K and -IPIV(K) THEN K-1 and -IPIV(K-1) */
	    ...
	    kp = -ipiv[k - 1];
	    if (kp != k - 1) {
		hswap_(nrhs, &b[k - 1 + b_dim1], ldb, &b[kp + b_dim1], ldb);
	    }
...
/*           stored in columns K-1 and K of A. */
		...
		hger_(&i__1, nrhs, &c_b7, &a[k * a_dim1 + 1], &c__1, &b[k + 
			b_dim1], ldb, &b[b_dim1 + 1], ldb);
		i__1 = k - 2;
		hger_(&i__1, nrhs, &c_b7, &a[(k - 1) * a_dim1 + 1], &c__1, &b[
...
/*           stored in column K of A. */
	    ...
	    if (k > 1) {
		i__1 = k - 1;
		hgemv_("Transpose", &i__1, nrhs, &c_b7, &b[b_offset], ldb, &a[
...
/*           stored in column K of A. */
	    ...
	    if (k < *n) {
		i__1 = *n - k;
		hger_(&i__1, nrhs, &c_b7, &a[k + 1 + k * a_dim1], &c__1, &b[k 
...
/*           stored in columns K and K+1 of A. */
	    ...
	    if (k < *n - 1) {
		i__1 = *n - k - 1;
		hger_(&i__1, nrhs, &c_b7, &a[k + 2 + k * a_dim1], &c__1, &b[k 
			+ b_dim1], ldb, &b[k + 2 + b_dim1], ldb);
		i__1 = *n - k - 1;
		hger_(&i__1, nrhs, &c_b7, &a[k + 2 + (k + 1) * a_dim1], &c__1,
...
/*           stored in column K of A. */
	    ...
	    if (k < *n) {
		i__1 = *n - k;
		hgemv_("Transpose", &i__1, nrhs, &c_b7, &b[k + 1 + b_dim1], 
