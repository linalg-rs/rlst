...
/*     part of T to control overflow in triangular solver. */
    ...
    for (j = 2; j <= i__1; ++j) {
	i__2 = j - 1;
	rwork[j] = hwasum_(&i__2, &t[j * t_dim1 + 1], &c__1);
/* L30: */
    }
...
/*           Copy the vector x or Q*x to VR and normalize. */
	    ...
	    if (! over) {
		kcopy_(&ki, &work[1], &c__1, &vr[is * vr_dim1 + 1], &c__1);
...
/*           Copy the vector x or Q*x to VL and normalize. */
	    ...
	    if (! over) {
		i__2 = *n - ki + 1;
		kcopy_(&i__2, &work[ki], &c__1, &vl[ki + is * vl_dim1], &c__1)
			;

		i__2 = *n - ki + 1;
		ii = ikamax_(&i__2, &vl[ki + is * vl_dim1], &c__1) + ki - 1;
		i__2 = ii + is * vl_dim1;
		remax = 1. / ((d__1 = vl[i__2].r, abs(d__1)) + (d__2 = d_imag(
			&vl[ii + is * vl_dim1]), abs(d__2)));
		i__2 = *n - ki + 1;
		whscal_(&i__2, &remax, &vl[ki + is * vl_dim1], &c__1);
...
/* L110: */
		...
		if (ki < *n) {
		    i__2 = *n - ki;
		    z__1.r = scale, z__1.i = 0.;
		    kgemv_("N", n, &i__2, &c_b2, &vl[(ki + 1) * vl_dim1 + 1], 
