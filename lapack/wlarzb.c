...
/* > */
/*  ===================================================================== */
void  wlarzb_(char *side, char *trans, char *direct, char *
	storev, integer *m, integer *n, integer *k, integer *l, quadcomplex 
	*v, integer *ldv, quadcomplex *t, integer *ldt, quadcomplex *c__, 
	integer *ldc, quadcomplex *work, integer *ldwork)
{
    ...
    extern void  wgemm_(char *, char *, integer *, integer *, 
	    ...
	    wlacgv_(integer *, quadcomplex *, integer *);
    char transt[1];
...
/*        W( 1:n, 1:k ) = C( 1:k, 1:n )**H */
	...
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    wcopy_(n, &c__[j + c_dim1], ldc, &work[j * work_dim1 + 1], &c__1);
/* L10: */
	}
...
/*                        C( m-l+1:m, 1:n )**H * V( 1:k, 1:l )**T */
	...
	if (*l > 0) {
	    wgemm_("Transpose", "Conjugate transpose", n, k, l, &c_b1, &c__[*
...
/*        W( 1:n, 1:k ) = W( 1:n, 1:k ) * T**T  or  W( 1:m, 1:k ) * T */
	...
	wtrmm_("Right", "Lower", transt, "Non-unit", n, k, &c_b1, &t[t_offset]
...
/*                            V( 1:k, 1:l )**H * W( 1:n, 1:k )**H */
	...
	if (*l > 0) {
	    z__1.r = -1., z__1.i = -0.;
	    wgemm_("Transpose", "Transpose", l, n, k, &z__1, &v[v_offset], 
...
/*        W( 1:m, 1:k ) = C( 1:m, 1:k ) */
	...
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    wcopy_(m, &c__[j * c_dim1 + 1], &c__1, &work[j * work_dim1 + 1], &
		    c__1);
/* L40: */
	}
...
/*                        C( 1:m, n-l+1:n ) * V( 1:k, 1:l )**H */
	...
	if (*l > 0) {
	    wgemm_("No transpose", "Transpose", m, k, l, &c_b1, &c__[(*n - *l 
...
/*                        W( 1:m, 1:k ) * T**H */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = *k - j + 1;
	    wlacgv_(&i__2, &t[j + j * t_dim1], &c__1);
/* L50: */
	}
	wtrmm_("Right", "Lower", trans, "Non-unit", m, k, &c_b1, &t[t_offset],
		 ldt, &work[work_offset], ldwork);
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    i__2 = *k - j + 1;
...
/*                            W( 1:m, 1:k ) * conjg( V( 1:k, 1:l ) ) */
	...
	i__1 = *l;
	for (j = 1; j <= i__1; ++j) {
	    wlacgv_(k, &v[j * v_dim1 + 1], &c__1);
/* L90: */
	}
	if (*l > 0) {
	    z__1.r = -1., z__1.i = -0.;
	    wgemm_("No transpose", "No transpose", m, l, k, &z__1, &work[
