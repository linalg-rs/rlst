...
/* > */
/*  ===================================================================== */
void  qgemlq_(char *side, char *trans, integer *m, integer *n, 
	...
	lwork, integer *info)
{
    ...
    /* Local variables */
    extern void  qlamswlq_(char *, char *, integer *, integer *
    ...
    extern void  qgemlqt_(char *, char *, integer *, integer *,
...
/* Computing MAX */
    ...
    if (left && *m <= *k || right && *n <= *k || nb <= *k || nb >= f2cmax(i__1,*
	    k)) {
	qgemlqt_(side, trans, m, n, k, &mb, &a[a_offset], lda, &t[6], &mb, &
		c__[c_offset], ldc, &work[1], info);
    } else {
	qlamswlq_(side, trans, m, n, k, &mb, &nb, &a[a_offset], lda, &t[6], &
