...
/* > */
/*  ===================================================================== */
void  hlasda_(integer *icompq, integer *smlsiz, integer *n, 
	integer *sqre, halfreal *d__, halfreal *e, halfreal *u, integer 
	*ldu, halfreal *vt, integer *k, halfreal *difl, halfreal *difr, 
	halfreal *z__, halfreal *poles, integer *givptr, integer *givcol, 
	integer *ldgcol, integer *perm, halfreal *givnum, halfreal *c__, 
	halfreal *s, halfreal *work, integer *iwork, integer *info)
{
    ...
    extern void  hcopy_(integer *, halfreal *, integer *, 
	    halfreal *, integer *);
    integer sqrei;
    extern void  hlasd6_(integer *, integer *, integer *, 
    ...
    extern void  hlasdq_(char *, integer *, integer *, integer 
	    *, integer *, integer *, halfreal *, halfreal *, halfreal *,
	    ...
	    integer *, integer *, integer *, integer *, integer *), hlaset_(
	    char *, integer *, integer *, halfreal *, halfreal *, 
...
/*     If the input matrix is too small, call DLASDQ to find the SVD. */
    ...
    if (*n <= *smlsiz) {
	if (*icompq == 0) {
	    hlasdq_("U", sqre, n, &c__0, &c__0, &c__0, &d__[1], &e[1], &vt[
...
/*        NRF: starting row of the right  subproblem */
	...
	sqrei = 1;
	if (*icompq == 0) {
	    hlaset_("A", &nlp1, &nlp1, &c_b11, &c_b12, &work[nwork1], &smlszp);
	    hlasdq_("U", &sqrei, &nl, &nlp1, &nru, &ncc, &d__[nlf], &e[nlf], &
		    work[nwork1], &smlszp, &work[nwork2], &nl, &work[nwork2], 
		    &nl, &work[nwork2], info);
	    itemp = nwork1 + nl * smlszp;
	    hcopy_(&nlp1, &work[nwork1], &c__1, &work[vfi], &c__1);
	    hcopy_(&nlp1, &work[itemp], &c__1, &work[vli], &c__1);
	} else {
	    hlaset_("A", &nl, &nl, &c_b11, &c_b12, &u[nlf + u_dim1], ldu);
	    hlaset_("A", &nlp1, &nlp1, &c_b11, &c_b12, &vt[nlf + vt_dim1], 
		    ldu);
	    hlasdq_("U", &sqrei, &nl, &nlp1, &nl, &ncc, &d__[nlf], &e[nlf], &
		    vt[nlf + vt_dim1], ldu, &u[nlf + u_dim1], ldu, &u[nlf + 
		    u_dim1], ldu, &work[nwork1], info);
	    hcopy_(&nlp1, &vt[nlf + vt_dim1], &c__1, &work[vfi], &c__1);
	    hcopy_(&nlp1, &vt[nlf + nlp1 * vt_dim1], &c__1, &work[vli], &c__1)
		    ;
	}
	if (*info != 0) {
	    return;
...
/* L10: */
	...
	nrp1 = nr + sqrei;
	if (*icompq == 0) {
	    hlaset_("A", &nrp1, &nrp1, &c_b11, &c_b12, &work[nwork1], &smlszp);
	    hlasdq_("U", &sqrei, &nr, &nrp1, &nru, &ncc, &d__[nrf], &e[nrf], &
		    work[nwork1], &smlszp, &work[nwork2], &nr, &work[nwork2], 
		    &nr, &work[nwork2], info);
	    itemp = nwork1 + (nrp1 - 1) * smlszp;
	    hcopy_(&nrp1, &work[nwork1], &c__1, &work[vfi], &c__1);
	    hcopy_(&nrp1, &work[itemp], &c__1, &work[vli], &c__1);
	} else {
	    hlaset_("A", &nr, &nr, &c_b11, &c_b12, &u[nrf + u_dim1], ldu);
	    hlaset_("A", &nrp1, &nrp1, &c_b11, &c_b12, &vt[nrf + vt_dim1], 
		    ldu);
	    hlasdq_("U", &sqrei, &nr, &nrp1, &nr, &ncc, &d__[nrf], &e[nrf], &
		    vt[nrf + vt_dim1], ldu, &u[nrf + u_dim1], ldu, &u[nrf + 
		    u_dim1], ldu, &work[nwork1], info);
	    hcopy_(&nrp1, &vt[nrf + vt_dim1], &c__1, &work[vfi], &c__1);
	    hcopy_(&nrp1, &vt[nrf + nrp1 * vt_dim1], &c__1, &work[vli], &c__1)
		    ;
	}
	if (*info != 0) {
	    return;
...
/*        the current level LVL. */
	...
	for (i__ = lf; i__ <= i__1; ++i__) {
	    ...
	    beta = e[ic];
	    if (*icompq == 0) {
		hlasd6_(icompq, &nl, &nr, &sqrei, &d__[nlf], &work[vfi], &
