...
/* > */
/*  ===================================================================== */
void  kheevr_(char *jobz, char *range, char *uplo, integer *n, 
	halfcomplex *a, integer *lda, halfreal *vl, halfreal *vu, 
	integer *il, integer *iu, halfreal *abstol, integer *m, halfreal *
	w, halfcomplex *z__, integer *ldz, integer *isuppz, halfcomplex *
	work, integer *lwork, halfreal *rwork, integer *lrwork, integer *
	iwork, integer *liwork, integer *info)
{
    ...
    /* Local variables */
    integer i__, j, nb, jj;
    halfreal eps, vll, vuu, tmp1, anrm;
    integer imax;
    halfreal rmin, rmax;
    logical test;
    integer itmp1;
    ...
    halfreal smlnum;
    extern void  kstein_(integer *, halfreal *, halfreal *,
	     integer *, halfreal *, integer *, integer *, halfcomplex *, 
	    integer *, halfreal *, integer *, integer *, integer *);
    logical lquery;
    integer lwkopt;
    extern halfreal klansy_(char *, char *, integer *, halfcomplex *, 
	    integer *, halfreal *);
    extern void  kstemr_(char *, char *, integer *, halfreal 
	    *, halfreal *, halfreal *, halfreal *, integer *, integer *,
	     integer *, halfreal *, halfcomplex *, integer *, integer *, 
	    integer *, logical *, halfreal *, integer *, integer *, integer 
	    *, integer *), kunmtr_(char *, char *, char *, 
...
/*     Scale matrix to allowable range, if necessary. */
    ...
    anrm = klansy_("M", uplo, n, &a[a_offset], lda, &rwork[1]);
    if (anrm > 0. && anrm < rmin) {
	iscale = 1;
	sigma = rmin / anrm;
    } else if (anrm > rmax) {
	...
	if (lower) {
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j + 1;
		whscal_(&i__2, &sigma, &a[j + j * a_dim1], &c__1);
/* L10: */
	    }
	} else {
	    i__1 = *n;
	    for (j = 1; j <= i__1; ++j) {
		whscal_(&j, &sigma, &a[j * a_dim1 + 1], &c__1);
/* L20: */
	    }
	}
	if (*abstol > 0.) {
...
/*     RWORK(INDRD:INDRD+N-1) stores the doublereal tridiagonal's diagonal */
/*     entries. */
    indrd = 1;
...
/*     -written while computing the eigenvalues in DSTERF and ZSTEMR. */
    indree = indrdd + *n;
/*     INDRWK is the starting offset of the left-over doublereal workspace, and */
...
/*     Call ZHETRD to reduce Hermitian matrix to tridiagonal form. */
    ...
    khetrd_(uplo, n, &a[a_offset], lda, &rwork[indrd], &rwork[indre], &work[
...
/*     then call DSTERF or ZSTEMR and ZUNMTR. */
	...
	if (! wantz) {
	    hcopy_(n, &rwork[indrd], &c__1, &w[1], &c__1);
	    i__1 = *n - 1;
	    hcopy_(&i__1, &rwork[indre], &c__1, &rwork[indree], &c__1);
	    hsterf_(n, &w[1], &rwork[indree], info);
	} else {
	    i__1 = *n - 1;
	    hcopy_(&i__1, &rwork[indre], &c__1, &rwork[indree], &c__1);
	    hcopy_(n, &rwork[indrd], &c__1, &rwork[indrdd], &c__1);
	    ...
	    kstemr_(jobz, "A", n, &rwork[indrdd], &rwork[indree], vl, vu, il, 
...
/*           form to eigenvectors returned by ZSTEMR. */
		...
		kunmtr_("L", uplo, "N", n, m, &a[a_offset], lda, &work[indtau]
...
/*     Also call DSTEBZ and ZSTEIN if ZSTEMR fails. */
	...
	*(unsigned char *)order = 'E';
    ...
    hstebz_(range, order, n, &vll, &vuu, il, iu, &abstll, &rwork[indrd], &
	    rwork[indre], m, &nsplit, &w[1], &iwork[indibl], &iwork[indisp], &
	    rwork[indrwk], &iwork[indiwo], info);

    if (wantz) {
	kstein_(n, &rwork[indrd], &rwork[indre], m, &w[1], &iwork[indibl], &
...
/*        form to eigenvectors returned by ZSTEIN. */
	...
	indwkn = indwk;
	llwrkn = *lwork - indwkn + 1;
	kunmtr_("L", uplo, "N", n, m, &a[a_offset], lda, &work[indtau], &z__[
