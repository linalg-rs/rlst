...
/* > */
/*  ===================================================================== */
void  htgsen_(integer *ijob, logical *wantq, logical *wantz, 
	logical *select, integer *n, halfreal *a, integer *lda, halfreal *
	b, integer *ldb, halfreal *alphar, halfreal *alphai, halfreal *
	beta, halfreal *q, integer *ldq, halfreal *z__, integer *ldz, 
	integer *m, halfreal *pl, halfreal *pr, halfreal *dif, 
	halfreal *work, integer *lwork, integer *iwork, integer *liwork, 
	integer *info)
{
    ...
    extern void  hlacpy_(char *, integer *, integer *, 
	    ...
	    integer *, halfreal *, integer *, integer *), hlassq_(integer *,
...
/*     Quick return if possible. */
	...
	if (wantd) {
		...
		hlassq_(n, &a[i__ * a_dim1 + 1], &c__1, &dscale, &dsum);
		hlassq_(n, &b[i__ * b_dim1 + 1], &c__1, &dscale, &dsum);
/* L20: */
	    }
...
/*              Q and Z accordingly (if requested): */
		...
		kk = k;
		if (k != ks) {
		    htgexc_(wantq, wantz, n, &a[a_offset], lda, &b[b_offset], 
...
/*        and compute PL and PR. */
	...
	i__ = n1 + 1;
	ijb = 0;
	hlacpy_("Full", &n1, &n2, &a[i__ * a_dim1 + 1], lda, &work[1], &n1);
	hlacpy_("Full", &n1, &n2, &b[i__ * b_dim1 + 1], ldb, &work[n1 * n2 + 
		1], &n1);
	i__1 = *lwork - (n1 << 1) * n2;
	htgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * a_dim1]
...
/*        and right eigenspaces. */
	...
	dsum = 1.;
	i__1 = n1 * n2;
	hlassq_(&i__1, &work[1], &c__1, &rdscal, &dsum);
	*pl = rdscal * M(sqrt)(dsum);
	...
	dsum = 1.;
	i__1 = n1 * n2;
	hlassq_(&i__1, &work[n1 * n2 + 1], &c__1, &rdscal, &dsum);
...
/*           Frobenius norm-based Difu-estimate. */
	    ...
	    i__1 = *lwork - (n1 << 1) * n2;
	    htgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * 
...
/*           Frobenius norm-based Difl-estimate. */
	    ...
	    i__1 = *lwork - (n1 << 1) * n2;
	    htgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, &a[
...
L40:
	    hlacn2_(&mn2, &work[mn2 + 1], &work[1], &iwork[1], &dif[1], &kase,
		     isave);
	    if (kase != 0) {
		if (kase == 1) {
...
/*                 Solve generalized Sylvester equation. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    htgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + 
...
/*                 Solve the transposed variant. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    htgsyl_("T", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + 
			    i__ * a_dim1], lda, &work[1], &n1, &b[b_offset], 
			    ldb, &b[i__ + i__ * b_dim1], ldb, &work[n1 * n2 + 
			    1], &n1, &dscale, &dif[1], &work[(n1 << 1) * n2 + 
			    1], &i__1, &iwork[1], &ierr);
...
L50:
	    hlacn2_(&mn2, &work[mn2 + 1], &work[1], &iwork[1], &dif[2], &kase,
		     isave);
	    if (kase != 0) {
		if (kase == 1) {
...
/*                 Solve generalized Sylvester equation. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    htgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, 
...
/*                 Solve the transposed variant. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    htgsyl_("T", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, 
			    &a[a_offset], lda, &work[1], &n2, &b[i__ + i__ * 
			    b_dim1], ldb, &b[b_offset], ldb, &work[n1 * n2 + 
			    1], &n2, &dscale, &dif[2], &work[(n1 << 1) * n2 + 
			    1], &i__1, &iwork[1], &ierr);
...
/*             Compute the eigenvalue(s) at position K. */
		...
		d__1 = smlnum * eps;
		hlag2_(&work[1], &c__2, &work[5], &c__2, &d__1, &beta[k], &
