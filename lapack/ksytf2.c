...
/*  ===================================================================== */
void  ksytf2_(char *uplo, integer *n, halfcomplex *a, 
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		i__1 = k - imax;
		jmax = imax + ikamax_(&i__1, &a[imax + (imax + 1) * a_dim1], 
			lda);
		...
		if (imax > 1) {
		    i__1 = imax - 1;
		    jmax = ikamax_(&i__1, &a[imax * a_dim1 + 1], &c__1);
/* Computing MAX */
...
/*              submatrix A(1:k,1:k) */
		...
		i__1 = kp - 1;
		kswap_(&i__1, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 + 1],
			 &c__1);
		i__1 = kk - kp - 1;
		kswap_(&i__1, &a[kp + 1 + kk * a_dim1], &c__1, &a[kp + (kp + 
			1) * a_dim1], lda);
		i__1 = kk + kk * a_dim1;
...
/*              A := A - U(k)*D(k)*U(k)**T = A - W(k)*1/D(k)*W(k)**T */
		...
		ksyr_(uplo, &i__1, &z__1, &a[k * a_dim1 + 1], &c__1, &a[
			a_offset], lda);
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		if (imax < *n) {
		    i__1 = *n - imax;
		    jmax = imax + ikamax_(&i__1, &a[imax + 1 + imax * a_dim1],
			     &c__1);
/* Computing MAX */
		    i__1 = jmax + imax * a_dim1;
		    d__3 = rowmax, d__4 = (d__1 = a[i__1].r, abs(d__1)) + (
...
/*              submatrix A(k:n,k:n) */
		...
		i__1 = kp - kk - 1;
		kswap_(&i__1, &a[kk + 1 + kk * a_dim1], &c__1, &a[kp + (kk + 
			1) * a_dim1], lda);
		i__1 = kk + kk * a_dim1;
		t.r = a[i__1].r, t.i = a[i__1].i;
		i__1 = kk + kk * a_dim1;
...
/*                 A := A - L(k)*D(k)*L(k)**T = A - W(k)*(1/D(k))*W(k)**T */
		    ...
		    i__1 = *n - k;
		    z__1.r = -r1.r, z__1.i = -r1.i;
		    ksyr_(uplo, &i__1, &z__1, &a[k + 1 + k * a_dim1], &c__1, &
