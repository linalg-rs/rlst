...
static quadreal c_b51 = -1.;
static quadreal c_b52 = 1.;
...
/* > */
/*  ===================================================================== */
void  qtgsyl_(char *trans, integer *ijob, integer *m, integer *
	...
	scale, quadreal *dif, quadreal *work, integer *lwork, integer *
	iwork, integer *info)
{
    ...
    quadreal scale2;
    extern void  qtgsy2_(char *, integer *, integer *, integer 
...
/*     Determine optimal block sizes MB and NB */
	...
	if (*ijob >= 3) {
	    ifunc = *ijob - 2;
	    qlaset_("F", m, n, &c_b14, &c_b14, &c__[c_offset], ldc)
		    ;
	    qlaset_("F", m, n, &c_b14, &c_b14, &f[f_offset], ldf);
	} else if (*ijob >= 1) {
	    isolve = 2;
	}
    }
...
/*           Use unblocked Level 2 solver */
	    ...
	    dsum = 1.;
	    pq = 0;
	    qtgsy2_(trans, &ifunc, m, n, &a[a_offset], lda, &b[b_offset], ldb,
		     &c__[c_offset], ldc, &d__[d_offset], ldd, &e[e_offset], 
		    lde, &f[f_offset], ldf, scale, &dsum, &dscale, &iwork[1], 
		    &pq, info);
	    if (dscale != 0.) {
		if (*ijob == 1 || *ijob == 3) {
		    *dif = M(sqrt)((quadreal) ((*m << 1) * *n)) / (dscale * 
			    M(sqrt)(dsum));
		} else {
		    *dif = M(sqrt)((quadreal) pq) / (dscale * M(sqrt)(dsum));
		}
	    }
		...
		scale2 = *scale;
		qlacpy_("F", m, n, &c__[c_offset], ldc, &work[1], m);
		qlacpy_("F", m, n, &f[f_offset], ldf, &work[*m * *n + 1], m);
		qlaset_("F", m, n, &c_b14, &c_b14, &c__[c_offset], ldc);
		qlaset_("F", m, n, &c_b14, &c_b14, &f[f_offset], ldf);
	    } else if (isolve == 2 && iround == 2) {
		qlacpy_("F", m, n, &work[1], m, &c__[c_offset], ldc);
		qlacpy_("F", m, n, &work[*m * *n + 1], m, &f[f_offset], ldf);
		*scale = scale2;
	    }
/* L30: */
	}
...
/*           for I = P, P - 1,..., 1; J = 1, 2,..., Q */
	    ...
	    *scale = 1.;
		...
		for (i__ = p; i__ >= 1; --i__) {
		    ...
		    mb = ie - is + 1;
		    ppqq = 0;
		    qtgsy2_(trans, &ifunc, &mb, &nb, &a[is + is * a_dim1], 
...
/* L80: */
			...
			for (k = js; k <= i__3; ++k) {
			    i__4 = is - 1;
			    qscal_(&i__4, &scaloc, &c__[k * c_dim1 + 1], &
				    c__1);
			    i__4 = is - 1;
			    qscal_(&i__4, &scaloc, &f[k * f_dim1 + 1], &c__1);
/* L90: */
			}
			i__3 = je;
			for (k = js; k <= i__3; ++k) {
			    i__4 = *m - ie;
			    qscal_(&i__4, &scaloc, &c__[ie + 1 + k * c_dim1], 
				    &c__1);
			    i__4 = *m - ie;
			    qscal_(&i__4, &scaloc, &f[ie + 1 + k * f_dim1], &
				    c__1);
/* L100: */
			}
			i__3 = *n;
...
/*        for I = 1,2,..., P; J = Q, Q-1,..., 1 */
	...
	*scale = 1.;
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		je = iwork[j + 1] - 1;
		nb = je - js + 1;
		qtgsy2_(trans, &ifunc, &mb, &nb, &a[is + is * a_dim1], lda, &
...
/* L160: */
		    ...
		    for (k = js; k <= i__3; ++k) {
			i__4 = is - 1;
			qscal_(&i__4, &scaloc, &c__[k * c_dim1 + 1], &c__1);
			i__4 = is - 1;
			qscal_(&i__4, &scaloc, &f[k * f_dim1 + 1], &c__1);
/* L170: */
		    }
			...
			qscal_(&i__4, &scaloc, &c__[ie + 1 + k * c_dim1], &
				c__1);
			i__4 = *m - ie;
			qscal_(&i__4, &scaloc, &f[ie + 1 + k * f_dim1], &c__1)
				;
/* L180: */
		    }
		    i__3 = *n;
...
/*              Substitute R(I, J) and L(I, J) into remaining equation. */
		...
		if (j > p + 2) {
		    i__3 = js - 1;
		    qgemm_("N", "T", &mb, &i__3, &nb, &c_b52, &c__[is + js * 
			    c_dim1], ldc, &b[js * b_dim1 + 1], ldb, &c_b52, &
			    f[is + f_dim1], ldf);
		    i__3 = js - 1;
		    qgemm_("N", "T", &mb, &i__3, &nb, &c_b52, &f[is + js * 
			    f_dim1], ldf, &e[js * e_dim1 + 1], lde, &c_b52, &
			    f[is + f_dim1], ldf);
		}
		if (i__ < p) {
		    i__3 = *m - ie;
		    qgemm_("T", "N", &i__3, &nb, &mb, &c_b51, &a[is + (ie + 1)
			     * a_dim1], lda, &c__[is + js * c_dim1], ldc, &
			    c_b52, &c__[ie + 1 + js * c_dim1], ldc);
		    i__3 = *m - ie;
		    qgemm_("T", "N", &i__3, &nb, &mb, &c_b51, &d__[is + (ie + 
