...
/*  ===================================================================== */
void  wlarf_(char *side, integer *m, integer *n, quadcomplex 
	*v, integer *incv, quadcomplex *tau, quadcomplex *c__, integer *
	ldc, quadcomplex *work)
{
    ...
    integer lastv;
    extern integer ilawlc_(integer *, integer *, quadcomplex *, integer *), 
	    ilawlr_(integer *, integer *, quadcomplex *, integer *);
...
/*     Scan for the last non-zero column in C(1:lastv,:). */
	    lastc = ilawlc_(&lastv, n, &c__[c_offset], ldc);
	} else {
/*     Scan for the last non-zero row in C(:,1:lastv). */
	    lastc = ilawlr_(m, &lastv, &c__[c_offset], ldc);
	}
    }
...
/*           w(1:lastc,1) := C(1:lastv,1:lastc)**H * v(1:lastv,1) */
	    ...
	    wgemv_("Conjugate transpose", &lastv, &lastc, &c_b1, &c__[
		    c_offset], ldc, &v[1], incv, &c_b2, &work[1], &c__1);

/*           C(1:lastv,1:lastc) := C(...) - v(1:lastv,1) * w(1:lastc,1)**H */

	    z__1.r = -tau->r, z__1.i = -tau->i;
	    wgerc_(&lastv, &lastc, &z__1, &v[1], incv, &work[1], &c__1, &c__[
		    c_offset], ldc);
	}
    } else {
...
/*           w(1:lastc,1) := C(1:lastc,1:lastv) * v(1:lastv,1) */
	    ...
	    wgemv_("No transpose", &lastc, &lastv, &c_b1, &c__[c_offset], ldc,
		     &v[1], incv, &c_b2, &work[1], &c__1);

/*           C(1:lastc,1:lastv) := C(...) - w(1:lastc,1) * v(1:lastv,1)**H */

	    z__1.r = -tau->r, z__1.i = -tau->i;
	    wgerc_(&lastc, &lastv, &z__1, &work[1], &c__1, &v[1], incv, &c__[
		    c_offset], ldc);
	}
    }
    return;
