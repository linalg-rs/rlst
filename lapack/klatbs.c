...
/* > */
/*  ===================================================================== */
void  klatbs_(char *uplo, char *trans, char *diag, char *
	normin, integer *n, integer *kd, halfcomplex *ab, integer *ldab, 
	halfcomplex *x, halfreal *scale, halfreal *cnorm, integer *info)
{
    ...
    logical upper;
    extern /* Double Complex */ VOID whotu_(halfcomplex *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, integer *);
    extern void  ktbsv_(char *, char *, char *, integer *, 
	    integer *, halfcomplex *, integer *, halfcomplex *, integer *), kaxpy_(integer *, halfcomplex *, 
	    halfcomplex *, integer *, halfcomplex *, integer *), hlabad_(
	    halfreal *, halfreal *);
...
/* Computing MIN */
		...
		cnorm[j] = hwasum_(&jlen, &ab[*kd + 1 - jlen + j * ab_dim1], &
			c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = f2cmin(i__2,i__3);
		if (jlen > 0) {
		    cnorm[j] = hwasum_(&jlen, &ab[j * ab_dim1 + 2], &c__1);
		} else {
		    cnorm[j] = 0.;
		}
/* L20: */
...
/*     greater than BIGNUM/2. */
    ...
    imax = ihamax_(n, &cnorm[1], &c__1);
    tmax = cnorm[imax];
    if (tmax <= bignum * .5) {
	tscal = 1.;
    } else {
	tscal = .5 / (smlnum * tmax);
	hscal_(n, &tscal, &cnorm[1], &c__1);
    }
...
/*        elements of X is not too small. */
	...
	ktbsv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &x[1], &c__1);
    } else {
...
/*                          Scale x by 1/b(j). */
			    ...
			    rec = 1. / xj;
			    whscal_(n, &rec, &x[1], &c__1);
			    *scale *= rec;
			    xmax *= rec;
			}
		    }
		    i__3 = j;
		    kladiv_(&z__1, &x[j], &tjjs);
...
/* Computing MIN */
			...
			kaxpy_(&jlen, &z__1, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
			i__3 = j - 1;
			i__ = ikamax_(&i__3, &x[1], &c__1);
			i__3 = i__;
...
/* Computing MIN */
			...
			z__2.r = -x[i__3].r, z__2.i = -x[i__3].i;
			z__1.r = tscal * z__2.r, z__1.i = tscal * z__2.i;
			kaxpy_(&jlen, &z__1, &ab[j * ab_dim1 + 2], &c__1, &x[
				j + 1], &c__1);
		    }
		    i__3 = *n - j;
		    i__ = j + ikamax_(&i__3, &x[j + 1], &c__1);
		    i__3 = i__;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = f2cmin(i__3,i__4);
			whotu_(&z__1, &jlen, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
...
/* Computing MIN */
			...
			jlen = f2cmin(i__3,i__4);
			if (jlen > 1) {
			    whotu_(&z__1, &jlen, &ab[j * ab_dim1 + 2], &c__1, 
				    &x[j + 1], &c__1);
			    csumj.r = z__1.r, csumj.i = z__1.i;
			}
		    }
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = f2cmin(i__3,i__4);
			whotc_(&z__1, &jlen, &ab[*kd + 1 - jlen + j * ab_dim1]
				, &c__1, &x[j - jlen], &c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    } else {
/* Computing MIN */
			...
			jlen = f2cmin(i__3,i__4);
			if (jlen > 1) {
			    whotc_(&z__1, &jlen, &ab[j * ab_dim1 + 2], &c__1, 
				    &x[j + 1], &c__1);
			    csumj.r = z__1.r, csumj.i = z__1.i;
			}
		    }
