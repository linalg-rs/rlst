...
/*  ===================================================================== */
void  wtgsy2_(char *trans, integer *ijob, integer *m, integer *
	...
	quadreal *scale, quadreal *rdsum, quadreal *rdscal, integer *
	info)
{
    ...
    /* Local variables */
    integer i__, j, k;
    quadcomplex z__[4]	/* was [2][2] */, rhs[2];
    ...
    extern void  wscal_(integer *, quadcomplex *, 
	    ...
	    integer *, quadcomplex *, integer *, quadcomplex *, integer *,
	     integer *, quadreal *), wgetc2_(integer *, quadcomplex *, 
	    integer *, integer *, integer *, integer *);
    quadreal scaloc;
    extern void  xerbla_(char *, integer *), wlatdf_(
...
/*              Solve Z * x = RHS */
		...
		wgetc2_(&c__2, z__, &c__2, ipiv, jpiv, &ierr);
		if (ierr > 0) {
		    *info = ierr;
		}
		if (*ijob == 0) {
		    wgesc2_(&c__2, z__, &c__2, rhs, ipiv, jpiv, &scaloc);
		    if (scaloc != 1.) {
			i__2 = *n;
			for (k = 1; k <= i__2; ++k) {
...
/* L10: */
			...
			*scale *= scaloc;
		    ...
		    wlatdf_(ijob, &c__2, z__, &c__2, rhs, rdsum, rdscal, ipiv,
			     jpiv);
		}
...
/*              Substitute R(I, J) and L(I, J) into remaining equation. */
		...
		if (i__ > 1) {
		    ...
		    i__2 = i__ - 1;
		    waxpy_(&i__2, &alpha, &a[i__ * a_dim1 + 1], &c__1, &c__[j 
			    * c_dim1 + 1], &c__1);
		    i__2 = i__ - 1;
		    waxpy_(&i__2, &alpha, &d__[i__ * d_dim1 + 1], &c__1, &f[j 
			    * f_dim1 + 1], &c__1);
		}
		if (j < *n) {
		    i__2 = *n - j;
		    waxpy_(&i__2, &rhs[1], &b[j + (j + 1) * b_dim1], ldb, &
			    c__[i__ + (j + 1) * c_dim1], ldc);
		    i__2 = *n - j;
		    waxpy_(&i__2, &rhs[1], &e[j + (j + 1) * e_dim1], lde, &f[
			    i__ + (j + 1) * f_dim1], ldf);
		}
