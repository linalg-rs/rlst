...
static quadreal c_b59 = 0.;
static quadreal c_b60 = 1.;
...
/* > */
/*  ===================================================================== */
void  qggevx_(char *balanc, char *jobvl, char *jobvr, char *
	sense, integer *n, quadreal *a, integer *lda, quadreal *b, 
	integer *ldb, quadreal *alphar, quadreal *alphai, quadreal *
	beta, quadreal *vl, integer *ldvl, quadreal *vr, integer *ldvr, 
	integer *ilo, integer *ihi, quadreal *lscale, quadreal *rscale, 
	quadreal *abnrm, quadreal *bbnrm, quadreal *rconde, quadreal *
	rcondv, quadreal *work, integer *lwork, integer *iwork, logical *
	bwork, integer *info)
{
...
/*     (Workspace: need 6*N if BALANC = 'S' or 'B', 1 otherwise) */
    ...
    qggbal_(balanc, n, &a[a_offset], lda, &b[b_offset], ldb, ilo, ihi, &
	    lscale[1], &rscale[1], &work[1], &ierr);

/*     Compute ABNRM and BBNRM */

    *abnrm = qlange_("1", n, n, &a[a_offset], lda, &work[1]);
    if (ilascl) {
	work[1] = *abnrm;
	qlascl_("G", &c__0, &c__0, &anrmto, &anrm, &c__1, &c__1, &work[1], &
		c__1, &ierr);
	*abnrm = work[1];
    }

    *bbnrm = qlange_("1", n, n, &b[b_offset], ldb, &work[1]);
    if (ilbscl) {
	work[1] = *bbnrm;
	qlascl_("G", &c__0, &c__0, &bnrmto, &bnrm, &c__1, &c__1, &work[1], &
		c__1, &ierr);
	*bbnrm = work[1];
    }
...
/*     (Workspace: need N, prefer N*NB) */
    ...
    if (ilvl) {
	qlaset_("Full", n, n, &c_b59, &c_b60, &vl[vl_offset], ldvl)
		;
	if (irows > 1) {
	    i__1 = irows - 1;
	    i__2 = irows - 1;
	    qlacpy_("L", &i__1, &i__2, &b[*ilo + 1 + *ilo * b_dim1], ldb, &vl[
		    *ilo + 1 + *ilo * vl_dim1], ldvl);
	}
	i__1 = *lwork + 1 - iwrk;
	qorgqr_(&irows, &irows, &irows, &vl[*ilo + *ilo * vl_dim1], ldvl, &
		work[itau], &work[iwrk], &i__1, &ierr);
    }

    if (ilvr) {
	qlaset_("Full", n, n, &c_b59, &c_b60, &vr[vr_offset], ldvr)
		;
    }
...
/*        Eigenvectors requested -- work on whole matrix. */
	...
	qgghrd_(jobvl, jobvr, n, ilo, ihi, &a[a_offset], lda, &b[b_offset], 
		ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &ierr);
    } else {
	qgghrd_("N", "N", &irows, &c__1, &irows, &a[*ilo + *ilo * a_dim1], 
...
/*     (Workspace: need N) */
	...
	*(unsigned char *)chtemp = 'E';
    ...
    qhgeqz_(chtemp, jobvl, jobvr, n, ilo, ihi, &a[a_offset], lda, &b[b_offset]
...
/*                         need N otherwise ) */
	...
	if (ilv) {
		...
		*(unsigned char *)chtemp = 'R';
	    ...
	    qtgevc_(chtemp, "B", ldumma, n, &a[a_offset], lda, &b[b_offset], 
...
/*              (compute workspace: need up to 4*N + 6*N) */
		...
		if (wantse || wantsb) {
		    qtgevc_("B", "S", &bwork[1], n, &a[a_offset], lda, &b[
		...
		i__2 = *lwork - iwrk1 + 1;
		qtgsna_(sense, "S", &bwork[1], n, &a[a_offset], lda, &b[
...
/*     (Workspace: none needed) */
    ...
    if (ilvl) {
	qggbak_(balanc, "L", n, ilo, ihi, &lscale[1], &rscale[1], n, &vl[
		vl_offset], ldvl, &ierr);
...
L70:
    ...
    if (ilvr) {
	qggbak_(balanc, "R", n, ilo, ihi, &lscale[1], &rscale[1], n, &vr[
		vr_offset], ldvr, &ierr);
	i__1 = *n;
	for (jc = 1; jc <= i__1; ++jc) {
	    if (alphai[jc] < 0.) {
