...
/*  ===================================================================== */
void  hlaein_(logical *rightv, logical *noinit, integer *n, 
	halfreal *h__, integer *ldh, halfreal *wr, halfreal *wi, 
	halfreal *vr, halfreal *vi, halfreal *b, integer *ldb, 
	halfreal *work, halfreal *eps3, halfreal *smlnum, halfreal *
	bignum, integer *info)
{
    ...
    halfreal w, x, y;
    integer i1, i2, i3;
    halfreal w1, ei, ej, xi, xr, rec;
    integer its, ierr;
    halfreal temp, norm, vmax;
...
/*           Scale supplied initial vector. */
	    ...
	    vnorm = hnrm2_(n, &vr[1], &c__1);
	    d__1 = *eps3 * rootn / f2cmax(vnorm,nrmsml);
	    hscal_(n, &d__1, &vr[1], &c__1);
	}
...
/*           overwriting x on v. */
	    ...
	    hlatrs_("Upper", trans, "Nonunit", normin, n, &b[b_offset], ldb, &
		    vr[1], &scale, &work[1], &ierr);
	    *(unsigned char *)normin = 'Y';

/*           Test for sufficient growth in the norm of v. */

	    vnorm = hasum_(n, &vr[1], &c__1);
...
/*        Normalize eigenvector. */
	...
	i__ = ihamax_(n, &vr[1], &c__1);
	d__2 = 1. / (d__1 = vr[i__], abs(d__1));
	hscal_(n, &d__2, &vr[1], &c__1);
    } else {
...
/*           Scale supplied initial vector. */
	    ...
	    d__1 = hnrm2_(n, &vr[1], &c__1);
	    d__2 = hnrm2_(n, &vi[1], &c__1);
	    norm = hlapy2_(&d__1, &d__2);
	    rec = *eps3 * rootn / f2cmax(norm,nrmsml);
	    hscal_(n, &rec, &vr[1], &c__1);
	    hscal_(n, &rec, &vi[1], &c__1);
	}
...
/* L140: */
	    ...
	    i__1 = *n - 1;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		absbii = hlapy2_(&b[i__ + i__ * b_dim1], &b[i__ + 1 + i__ * 
			b_dim1]);
		ei = h__[i__ + 1 + i__ * h_dim1];
		if (absbii < abs(ei)) {
...
/*              Compute 1-norm of offdiagonal elements of i-th row. */
		...
		i__2 = *n - i__;
		i__3 = *n - i__;
		work[i__] = hasum_(&i__2, &b[i__ + (i__ + 1) * b_dim1], ldb) 
			+ hasum_(&i__3, &b[i__ + 2 + i__ * b_dim1], &c__1);
/* L170: */
	    }
...
/* L180: */
	    ...
	    for (j = *n; j >= 2; --j) {
		ej = h__[j + (j - 1) * h_dim1];
		absbjj = hlapy2_(&b[j + j * b_dim1], &b[j + 1 + j * b_dim1]);
		if (absbjj < abs(ej)) {
...
/*              Compute 1-norm of offdiagonal elements of j-th column. */
		...
		i__1 = j - 1;
		i__2 = j - 1;
		work[j] = hasum_(&i__1, &b[j * b_dim1 + 1], &c__1) + hasum_(&
...
/*                 Divide by diagonal element of B. */
		    ...
		    hladiv_(&xr, &xi, &b[i__ + i__ * b_dim1], &b[i__ + 1 + 
...
/*           Test for sufficient growth in the norm of (VR,VI). */
	    ...
	    vnorm = hasum_(n, &vr[1], &c__1) + hasum_(n, &vi[1], &c__1);
	    if (vnorm >= growto * scale) {
		goto L280;
	    }
