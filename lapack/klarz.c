...
/* > */
/*  ===================================================================== */
void  klarz_(char *side, integer *m, integer *n, integer *l, 
	halfcomplex *v, integer *incv, halfcomplex *tau, halfcomplex *
	c__, integer *ldc, halfcomplex *work)
{
    ...
    extern void  kgerc_(integer *, integer *, halfcomplex *, 
	    ...
	    integer *, halfcomplex *, integer *), klacgv_(integer *, 
	    halfcomplex *, integer *);
...
/*           w( 1:n ) = conjg( C( 1, 1:n ) ) */
	    ...
	    kcopy_(n, &c__[c_offset], ldc, &work[1], &c__1);
	    klacgv_(n, &work[1], &c__1);

/*           w( 1:n ) = conjg( w( 1:n ) + C( m-l+1:m, 1:n )**H * v( 1:l ) ) */

	    kgemv_("Conjugate transpose", l, n, &c_b1, &c__[*m - *l + 1 + 
...
/*           C( 1, 1:n ) = C( 1, 1:n ) - tau * w( 1:n ) */
	    ...
	    kaxpy_(n, &z__1, &work[1], &c__1, &c__[c_offset], ldc);

/*           C( m-l+1:m, 1:n ) = C( m-l+1:m, 1:n ) - ... */
/*                               tau * v( 1:l ) * w( 1:n )**H */

	    z__1.r = -tau->r, z__1.i = -tau->i;
	    kgeru_(l, n, &z__1, &v[1], incv, &work[1], &c__1, &c__[*m - *l + 
		    1 + c_dim1], ldc);
	}
...
/*           w( 1:m ) = C( 1:m, 1 ) */
	    ...
	    kcopy_(m, &c__[c_offset], &c__1, &work[1], &c__1);

/*           w( 1:m ) = w( 1:m ) + C( 1:m, n-l+1:n, 1:n ) * v( 1:l ) */

	    kgemv_("No transpose", m, l, &c_b1, &c__[(*n - *l + 1) * c_dim1 + 
		    1], ldc, &v[1], incv, &c_b1, &work[1], &c__1);

/*           C( 1:m, 1 ) = C( 1:m, 1 ) - tau * w( 1:m ) */

	    z__1.r = -tau->r, z__1.i = -tau->i;
	    kaxpy_(m, &z__1, &work[1], &c__1, &c__[c_offset], &c__1);

/*           C( 1:m, n-l+1:n ) = C( 1:m, n-l+1:n ) - ... */
/*                               tau * w( 1:m ) * v( 1:l )**H */

	    z__1.r = -tau->r, z__1.i = -tau->i;
	    kgerc_(m, l, &z__1, &work[1], &c__1, &v[1], incv, &c__[(*n - *l + 
		    1) * c_dim1 + 1], ldc);
