...
/*  ===================================================================== */
void  whetrs2_(char *uplo, integer *n, integer *nrhs, 
	quadcomplex *a, integer *lda, integer *ipiv, quadcomplex *b, 
	integer *ldb, quadcomplex *work, integer *info)
{
    ...
    extern void  wswap_(integer *, quadcomplex *, integer *, 
	    ...
	    *, quadcomplex *, integer *), 
	    xerbla_(char *, integer *), wqscal_(integer *, quadreal 
	    *, quadcomplex *, integer *), wsyconv_(char *, char *, integer *
...
/*     Convert A */
    ...
    wsyconv_(uplo, "C", n, &a[a_offset], lda, &ipiv[1], &work[1], &iinfo);
...
/*           Interchange rows K-1 and -IPIV(K). */
		kp = -ipiv[k];
		if (kp == -ipiv[k - 1]) {
		    wswap_(nrhs, &b[k - 1 + b_dim1], ldb, &b[kp + b_dim1], 
			    ldb);
		}
		k += -2;
	    }
...
/*  Compute (U \P**T * B) -> B    [ (U \P**T * B) ] */
	...
	wtrsm_("L", "U", "N", "U", n, nrhs, &c_b1, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*  Compute D \ B -> B   [ D \ (U \P**T * B) ] */
	...
	while(i__ >= 1) {
		...
		i__1 = i__ + i__ * a_dim1;
		s = 1. / a[i__1].r;
		wqscal_(nrhs, &s, &b[i__ + b_dim1], ldb);
	    } else if (i__ > 1) {
...
/*      Compute (U**H \ B) -> B   [ U**H \ (D \ (U \P**T * B) ) ] */
	...
	wtrsm_("L", "U", "C", "U", n, nrhs, &c_b1, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*  Compute (L \P**T * B) -> B    [ (L \P**T * B) ] */
	...
	wtrsm_("L", "L", "N", "U", n, nrhs, &c_b1, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*  Compute (L**H \ B) -> B   [ L**H \ (D \ (L \P**T * B) ) ] */
	...
	wtrsm_("L", "L", "C", "U", n, nrhs, &c_b1, &a[a_offset], lda, &b[
		b_offset], ldb);
...
/*     Revert A */
    ...
    wsyconv_(uplo, "R", n, &a[a_offset], lda, &ipiv[1], &work[1], &iinfo);
