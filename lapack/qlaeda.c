...
/*  ===================================================================== */
void  qlaeda_(integer *n, integer *tlvls, integer *curlvl, 
	integer *curpbm, integer *prmptr, integer *perm, integer *givptr, 
	integer *givcol, quadreal *givnum, quadreal *q, integer *qptr, 
	quadreal *z__, quadreal *ztemp, integer *info)
{
    ...
    extern void  qgemv_(char *, integer *, integer *, 
	    ...
	    quadreal *, integer *, quadreal *, integer *), xerbla_(char *,
	     integer *);
...
/*     roots. */
    ...
    bsiz1 = (integer) (M(sqrt)((quadreal) (qptr[curr + 1] - qptr[curr])) + .5);
    bsiz2 = (integer) (M(sqrt)((quadreal) (qptr[curr + 2] - qptr[curr + 1])) + 
	    .5);
    i__1 = mid - bsiz1 - 1;
    for (k = 1; k <= i__1; ++k) {
	z__[k] = 0.;
/* L10: */
    ...
    qcopy_(&bsiz1, &q[qptr[curr] + bsiz1 - 1], &bsiz1, &z__[mid - bsiz1], &
	    c__1);
    qcopy_(&bsiz2, &q[qptr[curr + 1]], &bsiz2, &z__[mid], &c__1);
    i__1 = *n;
...
/*       Apply Givens at CURR and CURR+1 */
	...
	for (i__ = givptr[curr]; i__ <= i__2; ++i__) {
	    qrot_(&c__1, &z__[zptr1 + givcol[(i__ << 1) + 1] - 1], &c__1, &
...
/* L30: */
	...
	for (i__ = givptr[curr + 1]; i__ <= i__2; ++i__) {
	    qrot_(&c__1, &z__[mid - 1 + givcol[(i__ << 1) + 1]], &c__1, &z__[
...
/*        square roots. */
	...
	bsiz1 = (integer) (M(sqrt)((quadreal) (qptr[curr + 1] - qptr[curr])) + 
		.5);
	bsiz2 = (integer) (M(sqrt)((quadreal) (qptr[curr + 2] - qptr[curr + 1])
		) + .5);
	if (bsiz1 > 0) {
	...
	i__2 = psiz1 - bsiz1;
	qcopy_(&i__2, &ztemp[bsiz1 + 1], &c__1, &z__[zptr1 + bsiz1], &c__1);
	if (bsiz2 > 0) {
	    qgemv_("T", &bsiz2, &bsiz2, &c_b24, &q[qptr[curr + 1]], &bsiz2, &
		    ztemp[psiz1 + 1], &c__1, &c_b26, &z__[mid], &c__1);
	}
	i__2 = psiz2 - bsiz2;
	qcopy_(&i__2, &ztemp[psiz1 + bsiz2 + 1], &c__1, &z__[mid + bsiz2], &
		c__1);
