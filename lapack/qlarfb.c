...
/* > */
/*  ===================================================================== */
void  qlarfb_(char *side, char *trans, char *direct, char *
	storev, integer *m, integer *n, integer *k, quadreal *v, integer *
	ldv, quadreal *t, integer *ldt, quadreal *c__, integer *ldc, 
	quadreal *work, integer *ldwork)
{
...
/*              W := C1**T */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    qcopy_(n, &c__[j + c_dim1], ldc, &work[j * work_dim1 + 1],
			     &c__1);
/* L10: */
		}
...
/*              W := W * T**T  or  W * T */
		...
		qtrmm_("Right", "Upper", transt, "Non-unit", n, k, &c_b14, &t[
...
/*              W := C1 */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    qcopy_(m, &c__[j * c_dim1 + 1], &c__1, &work[j * 
			    work_dim1 + 1], &c__1);
/* L40: */
		}
...
/*              W := W * T  or  W * T**T */
		...
		qtrmm_("Right", "Upper", trans, "Non-unit", m, k, &c_b14, &t[
			t_offset], ldt, &work[work_offset], ldwork);
...
/*              W := C2**T */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    qcopy_(n, &c__[*m - *k + j + c_dim1], ldc, &work[j * 
			    work_dim1 + 1], &c__1);
/* L70: */
		}
...
/*              W := W * T**T  or  W * T */
		...
		qtrmm_("Right", "Lower", transt, "Non-unit", n, k, &c_b14, &t[
			t_offset], ldt, &work[work_offset], ldwork);
...
/*              W := C2 */
		...
		i__1 = *k;
		for (j = 1; j <= i__1; ++j) {
		    qcopy_(m, &c__[(*n - *k + j) * c_dim1 + 1], &c__1, &work[
			    j * work_dim1 + 1], &c__1);
/* L100: */
		}
...
/*              W := W * T  or  W * T**T */
		...
		qtrmm_("Right", "Lower", trans, "Non-unit", m, k, &c_b14, &t[
			t_offset], ldt, &work[work_offset], ldwork);
...
/*                 W := W + C2**T * V2**T */
		    ...
		    i__1 = *m - *k;
		    qgemm_("Transpose", "Transpose", n, k, &i__1, &c_b14, &
			    c__[*k + 1 + c_dim1], ldc, &v[(*k + 1) * v_dim1 + 
...
/*                 C2 := C2 - V2**T * W**T */
		    ...
		    i__1 = *m - *k;
		    qgemm_("Transpose", "Transpose", &i__1, n, k, &c_b25, &v[(
