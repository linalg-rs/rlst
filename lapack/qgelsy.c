...
static quadreal c_b31 = 0.;
static integer c__2 = 2;
static quadreal c_b54 = 1.;
...
/* > \verbatim */
/* > */
/* > DGELSY computes the minimum-norm solution to a doublereal linear least */
/* > squares problem: */
/* >     minimize || A * X - B || */
...
/* > */
/*  ===================================================================== */
void  qgelsy_(integer *m, integer *n, integer *nrhs, 
	quadreal *a, integer *lda, quadreal *b, integer *ldb, integer *
	jpvt, quadreal *rcond, integer *rank, quadreal *work, integer *
	lwork, integer *info)
{
    ...
    extern void  qormrz_(char *, char *, integer *, integer *, 
	    integer *, integer *, quadreal *, integer *, quadreal *, 
	    quadreal *, integer *, quadreal *, integer *, integer *);
    integer lwkopt;
    logical lquery;
    extern void  qtzrzf_(integer *, integer *, quadreal *, 
...
/*        Matrix all zero. Return zero solution. */
	...
	i__1 = f2cmax(*m,*n);
	qlaset_("F", &i__1, nrhs, &c_b31, &c_b31, &b[b_offset], ldb);
	*rank = 0;
	goto L70;
    }
...
/*        A * P = Q * R */
    ...
    i__1 = *lwork - mn;
    qgeqp3_(m, n, &a[a_offset], lda, &jpvt[1], &work[1], &work[mn + 1], &i__1,
	     info);
    wsize = mn + work[mn + 1];
...
L10:
    if (*rank < mn) {
	i__ = *rank + 1;
	qlaic1_(&c__2, rank, &work[ismin], &smin, &a[i__ * a_dim1 + 1], &a[
		i__ + i__ * a_dim1], &sminpr, &s1, &c1);
	qlaic1_(&c__1, rank, &work[ismax], &smax, &a[i__ * a_dim1 + 1], &a[
...
/*     [R11,R12] = [ T11, 0 ] * Y */
    ...
    if (*rank < *n) {
	i__1 = *lwork - (mn << 1);
	qtzrzf_(rank, n, &a[a_offset], lda, &work[mn + 1], &work[(mn << 1) + 
		1], &i__1, info);
    }
...
/*     B(1:M,1:NRHS) := Q**T * B(1:M,1:NRHS) */
    ...
    i__1 = *lwork - (mn << 1);
    qormqr_("Left", "Transpose", m, nrhs, &mn, &a[a_offset], lda, &work[1], &
...
/*     B(1:RANK,1:NRHS) := inv(T11) * B(1:RANK,1:NRHS) */
    ...
    qtrsm_("Left", "Upper", "No transpose", "Non-unit", rank, nrhs, &c_b54, &
...
/*     B(1:N,1:NRHS) := Y**T * B(1:N,1:NRHS) */
	...
	i__1 = *n - *rank;
	i__2 = *lwork - (mn << 1);
	qormrz_("Left", "Transpose", n, nrhs, rank, &i__1, &a[a_offset], lda, 
...
/* L50: */
	...
	qcopy_(n, &work[1], &c__1, &b[j * b_dim1 + 1], &c__1);
/* L60: */
    }
...
/*     Undo scaling */
	...
	qlascl_("G", &c__0, &c__0, &anrm, &smlnum, n, nrhs, &b[b_offset], ldb,
		 info);
	qlascl_("U", &c__0, &c__0, &smlnum, &anrm, rank, rank, &a[a_offset], 
		lda, info);
    } else if (iascl == 2) {
	qlascl_("G", &c__0, &c__0, &anrm, &bignum, n, nrhs, &b[b_offset], ldb,
		 info);
	qlascl_("U", &c__0, &c__0, &bignum, &anrm, rank, rank, &a[a_offset], 
		lda, info);
    }
    if (ibscl == 1) {
