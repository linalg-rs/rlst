...
/* > */
/*  ===================================================================== */
void  klarzb_(char *side, char *trans, char *direct, char *
	storev, integer *m, integer *n, integer *k, integer *l, halfcomplex 
	*v, integer *ldv, halfcomplex *t, integer *ldt, halfcomplex *c__, 
	integer *ldc, halfcomplex *work, integer *ldwork)
{
    ...
    extern void  kgemm_(char *, char *, integer *, integer *, 
	    ...
	    halfcomplex *, integer *, halfcomplex *, integer *), xerbla_(char *, integer *), 
	    klacgv_(integer *, halfcomplex *, integer *);
    char transt[1];
...
/*        W( 1:n, 1:k ) = C( 1:k, 1:n )**H */
	...
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    kcopy_(n, &c__[j + c_dim1], ldc, &work[j * work_dim1 + 1], &c__1);
/* L10: */
	}
...
/*                        C( m-l+1:m, 1:n )**H * V( 1:k, 1:l )**T */
	...
	if (*l > 0) {
	    kgemm_("Transpose", "Conjugate transpose", n, k, l, &c_b1, &c__[*
...
/*        W( 1:n, 1:k ) = W( 1:n, 1:k ) * T**T  or  W( 1:m, 1:k ) * T */
	...
	ktrmm_("Right", "Lower", transt, "Non-unit", n, k, &c_b1, &t[t_offset]
...
/*                            V( 1:k, 1:l )**H * W( 1:n, 1:k )**H */
	...
	if (*l > 0) {
	    z__1.r = -1., z__1.i = -0.;
	    kgemm_("Transpose", "Transpose", l, n, k, &z__1, &v[v_offset], 
...
/*        W( 1:m, 1:k ) = C( 1:m, 1:k ) */
	...
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    kcopy_(m, &c__[j * c_dim1 + 1], &c__1, &work[j * work_dim1 + 1], &
		    c__1);
/* L40: */
	}
...
/*                        C( 1:m, n-l+1:n ) * V( 1:k, 1:l )**H */
	...
	if (*l > 0) {
	    kgemm_("No transpose", "Transpose", m, k, l, &c_b1, &c__[(*n - *l 
...
/*                        W( 1:m, 1:k ) * T**H */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = *k - j + 1;
	    klacgv_(&i__2, &t[j + j * t_dim1], &c__1);
/* L50: */
	}
	ktrmm_("Right", "Lower", trans, "Non-unit", m, k, &c_b1, &t[t_offset],
		 ldt, &work[work_offset], ldwork);
	i__1 = *k;
	for (j = 1; j <= i__1; ++j) {
	    i__2 = *k - j + 1;
...
/*                            W( 1:m, 1:k ) * conjg( V( 1:k, 1:l ) ) */
	...
	i__1 = *l;
	for (j = 1; j <= i__1; ++j) {
	    klacgv_(k, &v[j * v_dim1 + 1], &c__1);
/* L90: */
	}
	if (*l > 0) {
	    z__1.r = -1., z__1.i = -0.;
	    kgemm_("No transpose", "No transpose", m, l, k, &z__1, &work[
