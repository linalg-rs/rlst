...
/*  ===================================================================== */
void  wungbr_(char *vect, integer *m, integer *n, integer *k, 
...
/*     Test the input arguments */
	...
	*info = -9;
	...
	if (wantq) {
	    if (*m >= *k) {
		wungqr_(m, n, k, &a[a_offset], lda, &tau[1], &work[1], &c_n1, 
			&iinfo);
	    } else {
		if (*m > 1) {
		    i__1 = *m - 1;
		    i__2 = *m - 1;
		    i__3 = *m - 1;
		    wungqr_(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &
			    tau[1], &work[1], &c_n1, &iinfo);
		}
	    }
	} else {
	    if (*k < *n) {
		wunglq_(m, n, k, &a[a_offset], lda, &tau[1], &work[1], &c_n1, 
			&iinfo);
	    } else {
		if (*n > 1) {
		    i__1 = *n - 1;
		    i__2 = *n - 1;
		    i__3 = *n - 1;
		    wunglq_(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &
			    tau[1], &work[1], &c_n1, &iinfo);
		}
	    }
	}
...
/*           If m >= k, assume m >= n >= k */
	    ...
	    wungqr_(m, n, k, &a[a_offset], lda, &tau[1], &work[1], lwork, &
		    iinfo);
...
/*              Form Q(2:m,2:m) */
		...
		i__2 = *m - 1;
		i__3 = *m - 1;
		wungqr_(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[
...
/*           If k < n, assume k <= m <= n */
	    ...
	    wunglq_(m, n, k, &a[a_offset], lda, &tau[1], &work[1], lwork, &
		    iinfo);
...
/*              Form P**H(2:n,2:n) */
		...
		i__2 = *n - 1;
		i__3 = *n - 1;
		wunglq_(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[
			1], &work[1], lwork, &iinfo);
	    }
	}
    }
