...
/* > doublereal tridiagonal matrix A. */
/* > \endverbatim */
/* > */
/* > \return DLANGT */
/* > \verbatim */
...
/*  ===================================================================== */
halfreal hlangt_(char *norm, integer *n, halfreal *dl, halfreal *d__, 
	halfreal *du)
{
    /* System generated locals */
    integer i__1;
    halfreal ret_val, d__1, d__2, d__3, d__4;

    /* Local variables */
    integer i__;
    halfreal sum, temp, scale;
...
/*        Find f2cmax(abs(A(i,j))). */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    if (anorm < (d__1 = dl[i__], abs(d__1)) || hisnan_(&d__3)) {
		anorm = (d__4 = dl[i__], abs(d__4));
	    }
	    d__3 = (d__2 = d__[i__], abs(d__2));
	    if (anorm < (d__1 = d__[i__], abs(d__1)) || hisnan_(&d__3)) {
		anorm = (d__4 = d__[i__], abs(d__4));
	    }
	    d__3 = (d__2 = du[i__], abs(d__2));
	    if (anorm < (d__1 = du[i__], abs(d__1)) || hisnan_(&d__3)) {
...
/*        Find normF(A). */
	...
	scale = 0.;
	sum = 1.;
	hlassq_(n, &d__[1], &c__1, &scale, &sum);
	if (*n > 1) {
	    i__1 = *n - 1;
	    hlassq_(&i__1, &dl[1], &c__1, &scale, &sum);
	    i__1 = *n - 1;
	    hlassq_(&i__1, &du[1], &c__1, &scale, &sum);
	}
