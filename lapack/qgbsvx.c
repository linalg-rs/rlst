...
/*  ===================================================================== */
void  qgbsvx_(char *fact, char *trans, integer *n, integer *kl,
	 integer *ku, integer *nrhs, quadreal *ab, integer *ldab, 
	quadreal *afb, integer *ldafb, integer *ipiv, char *equed, 
	quadreal *r__, quadreal *c__, quadreal *b, integer *ldb, 
	quadreal *x, integer *ldx, quadreal *rcond, quadreal *ferr, 
	quadreal *berr, quadreal *work, integer *iwork, integer *info)
{
    ...
    extern void  qcopy_(integer *, quadreal *, integer *, 
	    quadreal *, integer *);
    logical equil;
    extern quadreal qlangb_(char *, integer *, integer *, integer *, 
	    quadreal *, integer *, quadreal *), qlamch_(char *);
    extern void  qlaqgb_(integer *, integer *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, char *), 
	    qgbcon_(char *, integer *, integer *, integer *, quadreal *, 
	    integer *, integer *, quadreal *, quadreal *, quadreal *, 
	    integer *, integer *);
    quadreal colcnd;
    extern quadreal qlantb_(char *, char *, char *, integer *, integer *, 
	    quadreal *, integer *, quadreal *);
    extern void  qgbequ_(integer *, integer *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, integer *), qgbrfs_(
	    char *, integer *, integer *, integer *, integer *, quadreal *, 
	    integer *, quadreal *, integer *, integer *, quadreal *, 
	    integer *, quadreal *, integer *, quadreal *, quadreal *, 
	    quadreal *, integer *, integer *), qgbtrf_(integer *, 
    ...
    quadreal bignum;
    extern void  qgbtrs_(char *, integer *, integer *, integer 
	    *, integer *, quadreal *, integer *, integer *, quadreal *, 
	    integer *, integer *);
    integer infequ;
    logical colequ;
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	qgbequ_(n, n, kl, ku, &ab[ab_offset], ldab, &r__[1], &c__[1], &rowcnd,
		 &colcnd, &amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    qlaqgb_(n, n, kl, ku, &ab[ab_offset], ldab, &r__[1], &c__[1], &
...
/* Computing MIN */
	    ...
	    j2 = f2cmin(i__2,*n);
	    i__2 = j2 - j1 + 1;
	    qcopy_(&i__2, &ab[*ku + 1 - j + j1 + j * ab_dim1], &c__1, &afb[*
		    kl + *ku + 1 - j + j1 + j * afb_dim1], &c__1);
/* L70: */
	}

	qgbtrf_(n, n, kl, ku, &afb[afb_offset], ldafb, &ipiv[1], info);
...
/* Computing MAX */
	    ...
	    rpvgrw = qlantb_("M", "U", "N", info, &i__1, &afb[f2cmax(i__4,i__5) 
...
/*     reciprocal pivot growth factor RPVGRW. */
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = qlangb_(norm, n, kl, ku, &ab[ab_offset], ldab, &work[1]);
    i__1 = *kl + *ku;
    rpvgrw = qlantb_("M", "U", "N", n, &i__1, &afb[afb_offset], ldafb, &work[
	    1]);
    if (rpvgrw == 0.) {
	rpvgrw = 1.;
    } else {
	rpvgrw = qlangb_("M", n, kl, ku, &ab[ab_offset], ldab, &work[1]) / rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    qgbcon_(norm, n, kl, ku, &afb[afb_offset], ldafb, &ipiv[1], &anorm, rcond,
	     &work[1], &iwork[1], info);

/*     Compute the solution matrix X. */

    qlacpy_("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    qgbtrs_(trans, n, kl, ku, nrhs, &afb[afb_offset], ldafb, &ipiv[1], &x[
	    x_offset], ldx, info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    qgbrfs_(trans, n, kl, ku, nrhs, &ab[ab_offset], ldab, &afb[afb_offset], 
