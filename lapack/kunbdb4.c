...
/* > */
/*  ===================================================================== */
void  kunbdb4_(integer *m, integer *p, integer *q, 
	halfcomplex *x11, integer *ldx11, halfcomplex *x21, integer *
	ldx21, halfreal *theta, halfreal *phi, halfcomplex *taup1, 
	halfcomplex *taup2, halfcomplex *tauq1, halfcomplex *phantom, 
	halfcomplex *work, integer *lwork, integer *info)
{
    ...
    extern void  kscal_(integer *, halfcomplex *, 
	    ...
	    integer *, halfcomplex *), whrot_(integer *, 
    ...
    extern void  kunbdb5_(integer *, integer *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfcomplex *, integer *, integer *), klarfgp_(integer *, 
...
/*     Reduce columns 1, ..., M-Q of X11 and X21 */
	...
	if (i__ == 1) {
	    ...
	    i__2 = *m - *p;
	    kunbdb5_(p, &i__2, q, &phantom[1], &c__1, &phantom[*p + 1], &c__1,
		     &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &work[
		    iorbdb5], &lorbdb5, &childinfo);
	    kscal_(p, &c_b1, &phantom[1], &c__1);
	    klarfgp_(p, &phantom[1], &phantom[2], &c__1, &taup1[1]);
	    i__2 = *m - *p;
	    klarfgp_(&i__2, &phantom[*p + 1], &phantom[*p + 2], &c__1, &taup2[
		    1]);
	    theta[i__] = atan2((halfreal) phantom[1].r, (halfreal) 
		    phantom[*p + 1].r);
	    c__ = M(cos)(theta[i__]);
	    s = M(sin)(theta[i__]);
	    ...
	    d_cnjg(&z__1, &taup1[1]);
	    klarf_("L", p, q, &phantom[1], &c__1, &z__1, &x11[x11_offset], 
		    ldx11, &work[ilarf]);
	    i__2 = *m - *p;
	    d_cnjg(&z__1, &taup2[1]);
	    klarf_("L", &i__2, q, &phantom[*p + 1], &c__1, &z__1, &x21[
		    x21_offset], ldx21, &work[ilarf]);
	} else {
	    i__2 = *p - i__ + 1;
	    i__3 = *m - *p - i__ + 1;
	    i__4 = *q - i__ + 1;
	    kunbdb5_(&i__2, &i__3, &i__4, &x11[i__ + (i__ - 1) * x11_dim1], &
		    c__1, &x21[i__ + (i__ - 1) * x21_dim1], &c__1, &x11[i__ + 
		    i__ * x11_dim1], ldx11, &x21[i__ + i__ * x21_dim1], ldx21,
		     &work[iorbdb5], &lorbdb5, &childinfo);
	    i__2 = *p - i__ + 1;
	    kscal_(&i__2, &c_b1, &x11[i__ + (i__ - 1) * x11_dim1], &c__1);
	    i__2 = *p - i__ + 1;
	    klarfgp_(&i__2, &x11[i__ + (i__ - 1) * x11_dim1], &x11[i__ + 1 + (
		    i__ - 1) * x11_dim1], &c__1, &taup1[i__]);
	    i__2 = *m - *p - i__ + 1;
	    klarfgp_(&i__2, &x21[i__ + (i__ - 1) * x21_dim1], &x21[i__ + 1 + (
		    i__ - 1) * x21_dim1], &c__1, &taup2[i__]);
	    theta[i__] = atan2((halfreal) x11[i__ + (i__ - 1) * x11_dim1].r,
		     (halfreal) x21[i__ + (i__ - 1) * x21_dim1].r);
	    c__ = M(cos)(theta[i__]);
	    s = M(sin)(theta[i__]);
	    ...
	    klarf_("L", &i__2, &i__3, &x11[i__ + (i__ - 1) * x11_dim1], &c__1,
		     &z__1, &x11[i__ + i__ * x11_dim1], ldx11, &work[ilarf]);
	    i__2 = *m - *p - i__ + 1;
	    i__3 = *q - i__ + 1;
	    d_cnjg(&z__1, &taup2[i__]);
	    klarf_("L", &i__2, &i__3, &x21[i__ + (i__ - 1) * x21_dim1], &c__1,
		     &z__1, &x21[i__ + i__ * x21_dim1], ldx21, &work[ilarf]);
	}

	i__2 = *q - i__ + 1;
	d__1 = -c__;
	whrot_(&i__2, &x11[i__ + i__ * x11_dim1], ldx11, &x21[i__ + i__ * 
		x21_dim1], ldx21, &s, &d__1);
	i__2 = *q - i__ + 1;
	klacgv_(&i__2, &x21[i__ + i__ * x21_dim1], ldx21);
	i__2 = *q - i__ + 1;
	klarfgp_(&i__2, &x21[i__ + i__ * x21_dim1], &x21[i__ + (i__ + 1) * 
	...
	i__2 = *p - i__;
	i__3 = *q - i__ + 1;
	klarf_("R", &i__2, &i__3, &x21[i__ + i__ * x21_dim1], ldx21, &tauq1[
	...
	if (i__ < *m - *q) {
	    i__2 = *p - i__;
/* Computing 2nd power */
	    d__1 = hwnrm2_(&i__2, &x11[i__ + 1 + i__ * x11_dim1], &c__1);
	    i__3 = *m - *p - i__;
/* Computing 2nd power */
	    d__2 = hwnrm2_(&i__3, &x21[i__ + 1 + i__ * x21_dim1], &c__1);
...
/*     Reduce the bottom-right portion of X11 to [ I 0 ] */
	...
	i__2 = *p - i__;
	i__3 = *q - i__ + 1;
	klarf_("R", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], ldx11, &tauq1[
		i__], &x11[i__ + 1 + i__ * x11_dim1], ldx11, &work[ilarf]);
	i__2 = *q - *p;
	i__3 = *q - i__ + 1;
	klarf_("R", &i__2, &i__3, &x11[i__ + i__ * x11_dim1], ldx11, &tauq1[
...
/*     Reduce the bottom-right portion of X21 to [ 0 I ] */
	...
	i__2 = *q - i__ + 1;
	klacgv_(&i__2, &x21[*m - *q + i__ - *p + i__ * x21_dim1], ldx21);
	i__2 = *q - i__ + 1;
	klarfgp_(&i__2, &x21[*m - *q + i__ - *p + i__ * x21_dim1], &x21[*m - *
		q + i__ - *p + (i__ + 1) * x21_dim1], ldx21, &tauq1[i__]);
	i__2 = *m - *q + i__ - *p + i__ * x21_dim1;
	x21[i__2].r = 1., x21[i__2].i = 0.;
	i__2 = *q - i__;
	i__3 = *q - i__ + 1;
	klarf_("R", &i__2, &i__3, &x21[*m - *q + i__ - *p + i__ * x21_dim1], 
