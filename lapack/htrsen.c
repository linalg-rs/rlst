...
/* > \verbatim */
/* > */
/* > DTRSEN reorders the doublereal Schur factorization of a doublereal matrix */
...
/* >          select a doublereal eigenvalue w(j), SELECT(j) must be set to */
...
/* > */
/* >          The doublereal and imaginary parts, respectively, of the reordered */
...
/* > */
/* >  If T has been obtained from the doublereal Schur factorization of a matrix */
/* >  A = Q*T*Q**T, then the reordered doublereal Schur factorization of A is given */
...
/* > */
/*  ===================================================================== */
void  htrsen_(char *job, char *compq, logical *select, integer 
	*n, halfreal *t, integer *ldt, halfreal *q, integer *ldq, 
	halfreal *wr, halfreal *wi, integer *m, halfreal *s, halfreal 
	*sep, halfreal *work, integer *lwork, integer *iwork, integer *
	liwork, integer *info)
{
    ...
    extern void  htrsyl_(char *, char *, integer *, integer *, 
...
/*     Quick return if possible. */
	...
	if (wantsp) {
	    *sep = hlange_("1", n, n, &t[t_offset], ldt, &work[1]);
	}
	goto L40;
    }
...
/*              Swap the K-th block to position KS. */
		...
		kk = k;
		if (k != ks) {
		    htrexc_(compq, n, &t[t_offset], ldt, &q[q_offset], ldq, &
			    kk, &ks, &work[1], &ierr);
		}
		if (ierr == 1 || ierr == 2) {
...
/*           T11*R - R*T22 = scale*T12 */
	...
	hlacpy_("F", &n1, &n2, &t[(n1 + 1) * t_dim1 + 1], ldt, &work[1], &n1);
	htrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 
		+ 1) * t_dim1], ldt, &work[1], &n1, &scale, &ierr);

/*        Estimate the reciprocal of the condition number of the cluster */
/*        of eigenvalues. */

	rnorm = hlange_("F", &n1, &n2, &work[1], &n1, &work[1]);
	if (rnorm == 0.) {
	    *s = 1.;
	} else {
...
/*        Estimate sep(T11,T22). */
	...
	kase = 0;
L30:
	hlacn2_(&nn, &work[nn + 1], &work[1], &iwork[1], &est, &kase, isave);
	if (kase != 0) {
	    if (kase == 1) {

/*              Solve  T11*R - R*T22 = scale*X. */

		htrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 
...
/*              Solve T11**T*R - R*T22**T = scale*X. */
		...
		htrsyl_("T", "T", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 
			1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, &
			ierr);
	    }
	    goto L30;
...
/* L50: */
    ...
    for (k = 1; k <= i__1; ++k) {
	if (t[k + 1 + k * t_dim1] != 0.) {
	    wi[k] = M(sqrt)((d__1 = t[k + (k + 1) * t_dim1], abs(d__1))) * M(sqrt)((
