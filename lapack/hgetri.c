...
/*  ===================================================================== */
void  hgetri_(integer *n, halfreal *a, integer *lda, integer 
	*ipiv, halfreal *work, integer *lwork, integer *info)
{
...
/*     and the inverse is not computed. */
    ...
    htrtri_("Upper", "Non-unit", n, &a[a_offset], lda, info);
    if (*info > 0) {
	return;
    }
...
/*           Compute current column of inv(A). */
	    ...
	    if (j < *n) {
		i__1 = *n - j;
		hgemv_("No transpose", n, &i__1, &c_b20, &a[(j + 1) * a_dim1 
...
/*           Compute current block column of inv(A). */
	    ...
	    if (j + jb <= *n) {
		i__2 = *n - j - jb + 1;
		hgemm_("No transpose", "No transpose", n, &jb, &i__2, &c_b20, 
			&a[(j + jb) * a_dim1 + 1], lda, &work[j + jb], &
			ldwork, &c_b22, &a[j * a_dim1 + 1], lda);
	    }
	    htrsm_("Right", "Lower", "No transpose", "Unit", n, &jb, &c_b22, &
...
/*     Apply column interchanges. */
	...
	jp = ipiv[j];
	if (jp != j) {
	    hswap_(n, &a[j * a_dim1 + 1], &c__1, &a[jp * a_dim1 + 1], &c__1);
	}
/* L60: */
    }
