...
/* > \verbatim */
/* > */
/* > DGEESX computes for an N-by-N doublereal nonsymmetric matrix A, the */
/* > eigenvalues, the doublereal Schur form T, and, optionally, the matrix of */
/* > Schur vectors Z.  This gives the Schur factorization A = Z*T*(Z**T). */
/* > */
/* > Optionally, it also orders the eigenvalues on the diagonal of the */
/* > doublereal Schur form so that selected eigenvalues are at the top left; */
...
/* > */
/* > A doublereal matrix is in doublereal Schur form if it is upper quasi-triangular */
/* > with 1-by-1 and 2-by-2 blocks. 2-by-2 blocks will be standardized in */
/* > the form */
/* >           [  a  b  ] */
/* >           [  c  a  ] */
/* > */
/* > where b*c < 0. The eigenvalues of such a block are a +- M(sqrt)(bc). */
/* > \endverbatim */
...
/* >          An eigenvalue WR(j)+M(sqrt)(-1)*WI(j) is selected if */
...
/* >          On exit, A is overwritten by its doublereal Schur form T. */
/* > \endverbatim */
/* > */
/* > \param[in] LDA */
/* > \verbatim */
...
/* >          WR and WI contain the doublereal and imaginary parts, respectively, */
...
/*  ===================================================================== */
void  hgeesx_(char *jobvs, char *sort, L_fp select, char *
	sense, integer *n, halfreal *a, integer *lda, integer *sdim, 
	halfreal *wr, halfreal *wi, halfreal *vs, integer *ldvs, 
	halfreal *rconde, halfreal *rcondv, halfreal *work, integer *
	lwork, integer *iwork, integer *liwork, logical *bwork, integer *info)
{
    ...
    extern void  hgehrd_(integer *, integer *, integer *, 
	    ...
	    integer *), hlacpy_(char *, integer *, integer *, 
    ...
    logical wantsb;
    extern void  htrsen_(char *, char *, logical *, integer *, 
...
/*       minimal amount of doublereal workspace needed at that point in the */
...
/*       in the code.) */
	...
	if (*n == 0) {
	    ...
	    hhseqr_("S", jobvs, n, &c__1, n, &a[a_offset], lda, &wr[1], &wi[1]
...
/*     (RWorkspace: need N) */
    ...
    ibal = 1;
    hgebal_("P", n, &a[a_offset], lda, &ilo, &ihi, &work[ibal], &ierr);
...
/*        Copy Householder vectors to VS */
	...
	hlacpy_("L", n, n, &a[a_offset], lda, &vs[vs_offset], ldvs)
		;
...
/*        (RWorkspace: need 3*N-1, prefer 2*N+(N-1)*NB) */
	...
	i__1 = *lwork - iwrk + 1;
	horghr_(n, &ilo, &ihi, &vs[vs_offset], ldvs, &work[itau], &work[iwrk],
		 &i__1, &ierr);
    }
...
/*     (RWorkspace: need N+1, prefer N+HSWORK (see comments) ) */
    ...
    iwrk = itau;
    i__1 = *lwork - iwrk + 1;
    hhseqr_("S", jobvs, n, &ilo, &ihi, &a[a_offset], lda, &wr[1], &wi[1], &vs[
...
/*     Sort eigenvalues if desired */
    ...
    if (wantst && *info == 0) {
	if (scalea) {
	    hlascl_("G", &c__0, &c__0, &cscale, &anrm, n, &c__1, &wr[1], n, &
		    ierr);
	    hlascl_("G", &c__0, &c__0, &cscale, &anrm, n, &c__1, &wi[1], n, &
		    ierr);
	}
	i__1 = *n;
...
/*                     otherwise, need 0 ) */
	...
	i__1 = *lwork - iwrk + 1;
	htrsen_(sense, jobvs, &bwork[1], n, &a[a_offset], lda, &vs[vs_offset],
...
/*        (RWorkspace: need N) */
	...
	hgebak_("P", "R", n, &ilo, &ihi, &work[ibal], n, &vs[vs_offset], ldvs,
		 &ierr);
    }
...
/*        Undo scaling for the Schur form of A */
	...
	hlascl_("H", &c__0, &c__0, &cscale, &anrm, n, n, &a[a_offset], lda, &
		ierr);
	i__1 = *lda + 1;
	hcopy_(n, &a[a_offset], &i__1, &wr[1], &c__1);
	if ((wantsv || wantsb) && *info == 0) {
	    dum[0] = *rcondv;
	    hlascl_("G", &c__0, &c__0, &cscale, &anrm, &c__1, &c__1, dum, &
		    c__1, &ierr);
	    *rcondv = dum[0];
	}
	if (cscale == smlnum) {
...
/*           underflows. */
		...
		i2 = ihi - 1;
		i__1 = ilo - 1;
		hlascl_("G", &c__0, &c__0, &cscale, &anrm, &i__1, &c__1, &wi[
			1], n, &ierr);
	    } else if (wantst) {
		i1 = 1;
		i2 = *n - 1;
		...
		if (wi[i__] == 0.) {
			...
			if (i__ > 1) {
			    i__2 = i__ - 1;
			    hswap_(&i__2, &a[i__ * a_dim1 + 1], &c__1, &a[(
				    i__ + 1) * a_dim1 + 1], &c__1);
			}
			if (*n > i__ + 1) {
			    i__2 = *n - i__ - 1;
			    hswap_(&i__2, &a[i__ + (i__ + 2) * a_dim1], lda, &
				    a[i__ + 1 + (i__ + 2) * a_dim1], lda);
			}
			hswap_(n, &vs[i__ * vs_dim1 + 1], &c__1, &vs[(i__ + 1)
				 * vs_dim1 + 1], &c__1);
...
/* Computing MAX */
	i__3 = *n - ieval;
	i__2 = f2cmax(i__3,1);
	hlascl_("G", &c__0, &c__0, &cscale, &anrm, &i__1, &c__1, &wi[ieval + 
		1], &i__2, &ierr);
    }
