...
/* > */
/*  ===================================================================== */
void  ktgsen_(integer *ijob, logical *wantq, logical *wantz, 
	logical *select, integer *n, halfcomplex *a, integer *lda, 
	halfcomplex *b, integer *ldb, halfcomplex *alpha, halfcomplex *
	beta, halfcomplex *q, integer *ldq, halfcomplex *z__, integer *
	ldz, integer *m, halfreal *pl, halfreal *pr, halfreal *dif, 
	halfcomplex *work, integer *lwork, integer *iwork, integer *liwork, 
	integer *info)
{
    ...
    extern void  klacpy_(char *, integer *, integer *, 
	    ...
	    *, halfcomplex *, integer *, halfcomplex *, integer *, 
	    ...
	    klassq_(integer *, halfcomplex *, integer *, halfreal *, 
	    halfreal *);
    logical lquery;
...
/*     Quick return if possible. */
	...
	if (wantd) {
		...
		klassq_(n, &a[i__ * a_dim1 + 1], &c__1, &dscale, &dsum);
		klassq_(n, &b[i__ * b_dim1 + 1], &c__1, &dscale, &dsum);
/* L20: */
	    }
...
/*           and Z that will swap adjacent diagonal blocks in (A, B). */
	    ...
	    if (k != ks) {
		ktgexc_(wantq, wantz, n, &a[a_offset], lda, &b[b_offset], ldb,
...
/*                   B11 * R - L * B22 = B12 */
	...
	n2 = *n - *m;
	i__ = n1 + 1;
	klacpy_("Full", &n1, &n2, &a[i__ * a_dim1 + 1], lda, &work[1], &n1);
	klacpy_("Full", &n1, &n2, &b[i__ * b_dim1 + 1], ldb, &work[n1 * n2 + 
		1], &n1);
	ijb = 0;
	i__1 = *lwork - (n1 << 1) * n2;
	ktgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * a_dim1]
...
/*        left and right eigenspaces */
	...
	dsum = 1.;
	i__1 = n1 * n2;
	klassq_(&i__1, &work[1], &c__1, &rdscal, &dsum);
	*pl = rdscal * M(sqrt)(dsum);
	...
	dsum = 1.;
	i__1 = n1 * n2;
	klassq_(&i__1, &work[n1 * n2 + 1], &c__1, &rdscal, &dsum);
...
/*           Frobenius norm-based Difu estimate. */
	    ...
	    i__1 = *lwork - (n1 << 1) * n2;
	    ktgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * 
...
/*           Frobenius norm-based Difl estimate. */
	    ...
	    i__1 = *lwork - (n1 << 1) * n2;
	    ktgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, &a[
...
L40:
	    klacn2_(&mn2, &work[mn2 + 1], &work[1], &dif[1], &kase, isave);
	    if (kase != 0) {
		if (kase == 1) {
...
/*                 Solve generalized Sylvester equation */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    ktgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + 
...
/*                 Solve the transposed variant. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    ktgsyl_("C", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + 
			    i__ * a_dim1], lda, &work[1], &n1, &b[b_offset], 
			    ldb, &b[i__ + i__ * b_dim1], ldb, &work[n1 * n2 + 
			    1], &n1, &dscale, &dif[1], &work[(n1 * n2 << 1) + 
			    1], &i__1, &iwork[1], &ierr);
...
L50:
	    klacn2_(&mn2, &work[mn2 + 1], &work[1], &dif[2], &kase, isave);
	    if (kase != 0) {
		if (kase == 1) {
...
/*                 Solve generalized Sylvester equation */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    ktgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, 
...
/*                 Solve the transposed variant. */
		    ...
		    i__1 = *lwork - (n1 << 1) * n2;
		    ktgsyl_("C", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, 
...
/*     eigenvalues of reordered pair (A, B) */
	...
	if (dscale > safmin) {
	    ...
	    i__2 = *n - k;
	    kscal_(&i__2, &temp1, &b[k + (k + 1) * b_dim1], ldb);
	    i__2 = *n - k + 1;
	    kscal_(&i__2, &temp1, &a[k + k * a_dim1], lda);
	    if (*wantq) {
		kscal_(n, &temp2, &q[k * q_dim1 + 1], &c__1);
	    }
	} else {
	    i__2 = k + k * b_dim1;
