...
static quadreal c_b36 = 10.;
static quadreal c_b72 = .5;
...
/* > */
/*  ===================================================================== */
void  wggbal_(char *job, integer *n, quadcomplex *a, integer 
	*lda, quadcomplex *b, integer *ldb, integer *ilo, integer *ihi, 
	quadreal *lscale, quadreal *rscale, quadreal *work, integer *
	info)
{
    ...
    extern quadreal qdot_(integer *, quadreal *, integer *, quadreal *, 
	    integer *);
    quadreal coef2, coef5, gamma, alpha;
    extern void  qscal_(integer *, quadreal *, quadreal *, 
	    integer *);
    extern logical lsame_(char *, char *);
    quadreal sfmin, sfmax;
    integer iflow;
...
L160:
    lscale[m] = (quadreal) i__;
    if (i__ == m) {
	goto L170;
    }
    i__1 = *n - k + 1;
    wswap_(&i__1, &a[i__ + k * a_dim1], lda, &a[m + k * a_dim1], lda);
    i__1 = *n - k + 1;
    wswap_(&i__1, &b[i__ + k * b_dim1], ldb, &b[m + k * b_dim1], ldb);
...
L170:
    ...
    wswap_(&l, &a[j * a_dim1 + 1], &c__1, &a[m * a_dim1 + 1], &c__1);
    wswap_(&l, &b[j * b_dim1 + 1], &c__1, &b[m * b_dim1 + 1], &c__1);
...
L250:
    ...
    gamma = qdot_(&nr, &work[*ilo + (*n << 2)], &c__1, &work[*ilo + (*n << 2)]
...
L290:
	...
	work[i__ + (*n << 1)] = (quadreal) kount * work[i__ + *n] + sum;
/* L300: */
    }
...
L320:
	...
	work[j + *n * 3] = (quadreal) kount * work[j] + sum;
/* L330: */
    }
...
L350:
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	irab = iwamax_(&i__2, &a[i__ + *ilo * a_dim1], lda);
	rab = z_abs(&a[i__ + (irab + *ilo - 1) * a_dim1]);
	i__2 = *n - *ilo + 1;
	irab = iwamax_(&i__2, &b[i__ + *ilo * b_dim1], ldb);
/* Computing MAX */
...
/* Computing MIN */
	...
	icab = iwamax_(ihi, &a[i__ * a_dim1 + 1], &c__1);
	cab = z_abs(&a[icab + i__ * a_dim1]);
	icab = iwamax_(ihi, &b[i__ * b_dim1 + 1], &c__1);
/* Computing MAX */
...
/*     Row scaling of matrices A and B */
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	wqscal_(&i__2, &lscale[i__], &a[i__ + *ilo * a_dim1], lda);
	i__2 = *n - *ilo + 1;
	wqscal_(&i__2, &lscale[i__], &b[i__ + *ilo * b_dim1], ldb);
/* L370: */
    }
...
/*     Column scaling of matrices A and B */
    ...
    i__1 = *ihi;
    for (j = *ilo; j <= i__1; ++j) {
	wqscal_(ihi, &rscale[j], &a[j * a_dim1 + 1], &c__1);
	wqscal_(ihi, &rscale[j], &b[j * b_dim1 + 1], &c__1);
/* L380: */
    }
