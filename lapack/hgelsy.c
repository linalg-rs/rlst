...
static halfreal c_b31 = 0.;
static integer c__2 = 2;
static halfreal c_b54 = 1.;
...
/* > */
/*  ===================================================================== */
void  hgelsy_(integer *m, integer *n, integer *nrhs, 
	halfreal *a, integer *lda, halfreal *b, integer *ldb, integer *
	jpvt, halfreal *rcond, integer *rank, halfreal *work, integer *
	lwork, integer *info)
{
    ...
    /* Local variables */
    integer i__, j;
    halfreal c1, c2, s1, s2;
    integer nb, mn, nb1, nb2, nb3, nb4;
    halfreal anrm, bnrm, smin, smax;
    integer iascl, ibscl;
    ...
    halfreal sminpr, smaxpr, smlnum;
    extern void  hormrz_(char *, char *, integer *, integer *, 
	    integer *, integer *, halfreal *, integer *, halfreal *, 
	    halfreal *, integer *, halfreal *, integer *, integer *);
    integer lwkopt;
    logical lquery;
    extern void  htzrzf_(integer *, integer *, halfreal *, 
...
/*        Matrix all zero. Return zero solution. */
	...
	i__1 = f2cmax(*m,*n);
	hlaset_("F", &i__1, nrhs, &c_b31, &c_b31, &b[b_offset], ldb);
	*rank = 0;
	goto L70;
    }
...
/*        A * P = Q * R */
    ...
    i__1 = *lwork - mn;
    hgeqp3_(m, n, &a[a_offset], lda, &jpvt[1], &work[1], &work[mn + 1], &i__1,
	     info);
    wsize = mn + work[mn + 1];
...
L10:
    if (*rank < mn) {
	i__ = *rank + 1;
	hlaic1_(&c__2, rank, &work[ismin], &smin, &a[i__ * a_dim1 + 1], &a[
		i__ + i__ * a_dim1], &sminpr, &s1, &c1);
	hlaic1_(&c__1, rank, &work[ismax], &smax, &a[i__ * a_dim1 + 1], &a[
...
/*     [R11,R12] = [ T11, 0 ] * Y */
    ...
    if (*rank < *n) {
	i__1 = *lwork - (mn << 1);
	htzrzf_(rank, n, &a[a_offset], lda, &work[mn + 1], &work[(mn << 1) + 
		1], &i__1, info);
    }
...
/*     B(1:M,1:NRHS) := Q**T * B(1:M,1:NRHS) */
    ...
    i__1 = *lwork - (mn << 1);
    hormqr_("Left", "Transpose", m, nrhs, &mn, &a[a_offset], lda, &work[1], &
...
/*     B(1:RANK,1:NRHS) := inv(T11) * B(1:RANK,1:NRHS) */
    ...
    htrsm_("Left", "Upper", "No transpose", "Non-unit", rank, nrhs, &c_b54, &
...
/*     B(1:N,1:NRHS) := Y**T * B(1:N,1:NRHS) */
	...
	i__1 = *n - *rank;
	i__2 = *lwork - (mn << 1);
	hormrz_("Left", "Transpose", n, nrhs, rank, &i__1, &a[a_offset], lda, 
...
/* L50: */
	...
	hcopy_(n, &work[1], &c__1, &b[j * b_dim1 + 1], &c__1);
/* L60: */
    }
...
/*     Undo scaling */
	...
	hlascl_("G", &c__0, &c__0, &anrm, &smlnum, n, nrhs, &b[b_offset], ldb,
		 info);
	hlascl_("U", &c__0, &c__0, &smlnum, &anrm, rank, rank, &a[a_offset], 
		lda, info);
    } else if (iascl == 2) {
	hlascl_("G", &c__0, &c__0, &anrm, &bignum, n, nrhs, &b[b_offset], ldb,
		 info);
	hlascl_("U", &c__0, &c__0, &bignum, &anrm, rank, rank, &a[a_offset], 
		lda, info);
    }
    if (ibscl == 1) {
