...
/* > \verbatim */
/* > */
/* > DLATRF_AA factorizes a panel of a doublereal symmetric matrix A using */
...
/*  ===================================================================== */
void  dlasyf_aa__(char *uplo, integer *j1, integer *m, integer 
	*nb, halfreal *a, integer *lda, integer *ipiv, halfreal *h__, 
	integer *ldh, halfreal *work)
{
    ...
    extern void  hgemv_(char *, integer *, integer *, 
	    ...
	    *, halfreal *, integer *, halfreal *, integer *), haxpy_(
...
/*           first column */
	    ...
	    i__1 = j - k1;
	    hgemv_("No transpose", &mj, &i__1, &c_b6, &h__[j + k1 * h_dim1], 
...
/*        Copy H(i:M, i) into WORK */
	...
	hcopy_(&mj, &h__[j + j * h_dim1], &c__1, &work[1], &c__1);
...
/*            where A(J-1, J) stores T(J-1, J) and A(J-2, J:M) stores U(J-1, J:M) */
	    ...
	    haxpy_(&mj, &alpha, &a[k - 2 + j * a_dim1], lda, &work[1], &c__1);
	}
...
/*            where A(J, J) stores T(J, J) and A(J-1, (J+1):M) stores U(J, (J+1):M) */
		...
		alpha = -a[k + j * a_dim1];
		i__1 = *m - j;
		haxpy_(&i__1, &alpha, &a[k - 1 + (j + 1) * a_dim1], lda, &
			work[2], &c__1);
	    }
...
/*              Swap A(I1, I1+1:M) with A(I1+1:M, I2) */
		...
		i2 = i2 + j - 1;
		i__1 = i2 - i1 - 1;
		hswap_(&i__1, &a[*j1 + i1 - 1 + (i1 + 1) * a_dim1], lda, &a[*
			j1 + i1 + i2 * a_dim1], &c__1);

/*              Swap A(I1, I2+1:M) with A(I2, I2+1:M) */

		i__1 = *m - i2;
		hswap_(&i__1, &a[*j1 + i1 - 1 + (i2 + 1) * a_dim1], lda, &a[*
...
/*              Swap H(I1, 1:J1) with H(I2, 1:J1) */
		...
		i__1 = i1 - 1;
		hswap_(&i__1, &h__[i1 + h_dim1], ldh, &h__[i2 + h_dim1], ldh);
		ipiv[i1] = i2;
...
/*                  skipping the first column */
		    ...
		    i__1 = i1 - k1 + 1;
		    hswap_(&i__1, &a[i1 * a_dim1 + 1], &c__1, &a[i2 * a_dim1 
			    + 1], &c__1);
		}
	    } else {
		ipiv[j + 1] = j + 1;
...
/*              Copy A(J+1:M, J+1) into H(J:M, J), */
		...
		i__1 = *m - j;
		hcopy_(&i__1, &a[k + 1 + (j + 1) * a_dim1], lda, &h__[j + 1 + 
			(j + 1) * h_dim1], &c__1);
	    }
...
/*            where A(J, J+1) = T(J, J+1) and A(J+2:M, J) = L(J+2:M, J+1) */
		...
		i__1 = *m - j - 1;
		hcopy_(&i__1, &work[3], &c__1, &a[k + (j + 2) * a_dim1], lda);
		i__1 = *m - j - 1;
		hscal_(&i__1, &alpha, &a[k + (j + 2) * a_dim1], lda);
	    } else {
		i__1 = *m - j - 1;
		hlaset_("Full", &c__1, &i__1, &c_b22, &c_b22, &a[k + (j + 2) *
			 a_dim1], lda);
	    }
	}
	++j;
...
/*            where A(J-1, J) = T(J-1, J) and A(J, J-2) = L(J, J-1) */
	    ...
	    haxpy_(&mj, &alpha, &a[j + (k - 2) * a_dim1], &c__1, &work[1], &
		    c__1);
	}
...
/*            where A(J, J) = T(J, J) and A((J+1):M, J-1) = L((J+1):M, J) */
		...
		alpha = -a[j + k * a_dim1];
		i__1 = *m - j;
		haxpy_(&i__1, &alpha, &a[j + 1 + (k - 1) * a_dim1], &c__1, &
			work[2], &c__1);
	    }
...
/*              Swap A(I1+1:M, I1) with A(I2, I1+1:M) */
		...
		i2 = i2 + j - 1;
		i__1 = i2 - i1 - 1;
		hswap_(&i__1, &a[i1 + 1 + (*j1 + i1 - 1) * a_dim1], &c__1, &a[
			i2 + (*j1 + i1) * a_dim1], lda);

/*              Swap A(I2+1:M, I1) with A(I2+1:M, I2) */

		i__1 = *m - i2;
		hswap_(&i__1, &a[i2 + 1 + (*j1 + i1 - 1) * a_dim1], &c__1, &a[
...
/*                  skipping the first column */
		    ...
		    i__1 = i1 - k1 + 1;
		    hswap_(&i__1, &a[i1 + a_dim1], lda, &a[i2 + a_dim1], lda);
		}
	    } else {
		ipiv[j + 1] = j + 1;
	    }
...
/*              Copy A(J+1:M, J+1) into H(J+1:M, J), */
		...
		i__1 = *m - j;
		hcopy_(&i__1, &a[j + 1 + (k + 1) * a_dim1], &c__1, &h__[j + 1 
			+ (j + 1) * h_dim1], &c__1);
	    }
...
/*            where A(J, J+1) = T(J, J+1) and A(J+2:M, J) = L(J+2:M, J+1) */
		...
		i__1 = *m - j - 1;
		hcopy_(&i__1, &work[3], &c__1, &a[j + 2 + k * a_dim1], &c__1);
		i__1 = *m - j - 1;
		hscal_(&i__1, &alpha, &a[j + 2 + k * a_dim1], &c__1);
	    } else {
		i__1 = *m - j - 1;
		hlaset_("Full", &i__1, &c__1, &c_b22, &c_b22, &a[j + 2 + k * 
			a_dim1], lda);
	    }
	}
	++j;
