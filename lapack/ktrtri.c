...
/*  ===================================================================== */
void  ktrtri_(char *uplo, char *diag, integer *n, 
	halfcomplex *a, integer *lda, integer *info)
{
    ...
    extern void  ktrmm_(char *, char *, char *, char *, 
	    ...
	    integer *), ktrti2_(char *, char *
	    , integer *, halfcomplex *, integer *, integer *), xerbla_(char *, integer *);
...
/*        Use unblocked code */
	...
	ktrti2_(uplo, diag, n, &a[a_offset], lda, info);
    } else {
...
/*              Compute rows 1:j-1 of current block column */
		...
		i__4 = j - 1;
		ktrmm_("Left", "Upper", "No transpose", diag, &i__4, &jb, &
			c_b1, &a[a_offset], lda, &a[j * a_dim1 + 1], lda);
		i__4 = j - 1;
		z__1.r = -1., z__1.i = -0.;
		ktrsm_("Right", "Upper", "No transpose", diag, &i__4, &jb, &
			z__1, &a[j + j * a_dim1], lda, &a[j * a_dim1 + 1], 
			lda);

/*              Compute inverse of current diagonal block */

		ktrti2_("Upper", diag, &jb, &a[j + j * a_dim1], lda, info);
/* L20: */
	    }
	} else {
...
/*                 Compute rows j+jb:n of current block column */
		    ...
		    i__1 = *n - j - jb + 1;
		    ktrmm_("Left", "Lower", "No transpose", diag, &i__1, &jb, 
			    &c_b1, &a[j + jb + (j + jb) * a_dim1], lda, &a[j 
			    + jb + j * a_dim1], lda);
		    i__1 = *n - j - jb + 1;
		    z__1.r = -1., z__1.i = -0.;
		    ktrsm_("Right", "Lower", "No transpose", diag, &i__1, &jb,
...
/*              Compute inverse of current diagonal block */
		...
		ktrti2_("Lower", diag, &jb, &a[j + j * a_dim1], lda, info);
/* L30: */
	    }
	}
    }
