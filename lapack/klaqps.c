...
/*  ===================================================================== */
void  klaqps_(integer *m, integer *n, integer *offset, integer 
	*nb, integer *kb, halfcomplex *a, integer *lda, integer *jpvt, 
	halfcomplex *tau, halfreal *vn1, halfreal *vn2, halfcomplex *
	auxv, halfcomplex *f, integer *ldf)
{
...
/*        Determine ith pivot column and swap if necessary */
	...
	i__1 = *n - k + 1;
	pvt = k - 1 + ihamax_(&i__1, &vn1[k], &c__1);
	if (pvt != k) {
	    kswap_(m, &a[pvt * a_dim1 + 1], &c__1, &a[k * a_dim1 + 1], &c__1);
	    i__1 = k - 1;
	    kswap_(&i__1, &f[pvt + f_dim1], ldf, &f[k + f_dim1], ldf);
	    itemp = jpvt[pvt];
	    jpvt[pvt] = jpvt[k];
	    jpvt[k] = itemp;
	    vn1[pvt] = vn1[k];
...
/* L20: */
	    ...
	    i__2 = k - 1;
	    z__1.r = -1., z__1.i = -0.;
	    kgemv_("No transpose", &i__1, &i__2, &z__1, &a[rk + a_dim1], lda, 
...
/*        Generate elementary reflector H(k). */
	...
	if (rk < *m) {
	    i__1 = *m - rk + 1;
	    klarfg_(&i__1, &a[rk + k * a_dim1], &a[rk + 1 + k * a_dim1], &
		    c__1, &tau[k]);
	} else {
	    klarfg_(&c__1, &a[rk + k * a_dim1], &a[rk + k * a_dim1], &c__1, &
		    tau[k]);
	}
...
/*        Compute  F(K+1:N,K) := tau(K)*A(RK:M,K+1:N)**H*A(RK:M,K). */
	...
	if (k < *n) {
	    i__1 = *m - rk + 1;
	    i__2 = *n - k;
	    kgemv_("Conjugate transpose", &i__1, &i__2, &tau[k], &a[rk + (k + 
...
/*                    *A(RK:M,K). */
	...
	if (k > 1) {
	    ...
	    kgemv_("Conjugate transpose", &i__1, &i__2, &z__1, &a[rk + a_dim1]
		    , lda, &a[rk + k * a_dim1], &c__1, &c_b1, &auxv[1], &c__1);

	    i__1 = k - 1;
	    kgemv_("No transpose", n, &i__1, &c_b2, &f[f_dim1 + 1], ldf, &
...
/*        A(RK,K+1:N) := A(RK,K+1:N) - A(RK,1:K)*F(K+1:N,1:K)**H. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    z__1.r = -1., z__1.i = -0.;
	    kgemm_("No transpose", "Conjugate transpose", &c__1, &i__1, &k, &
...
/* Computing MIN */
	...
	i__2 = *n - *kb;
	z__1.r = -1., z__1.i = -0.;
	kgemm_("No transpose", "Conjugate transpose", &i__1, &i__2, kb, &z__1,
...
L60:
	...
	i__1 = *m - rk;
	vn1[lsticc] = hwnrm2_(&i__1, &a[rk + 1 + lsticc * a_dim1], &c__1);
