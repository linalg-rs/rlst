...
/*  ===================================================================== */
void  wuncsd2by1_(char *jobu1, char *jobu2, char *jobv1t, 
	integer *m, integer *p, integer *q, quadcomplex *x11, integer *
	ldx11, quadcomplex *x21, integer *ldx21, quadreal *theta, 
	quadcomplex *u1, integer *ldu1, quadcomplex *u2, integer *ldu2, 
	quadcomplex *v1t, integer *ldv1t, quadcomplex *work, integer *
	lwork, quadreal *rwork, integer *lrwork, integer *iwork, integer *
	info)
{
    ...
    integer lworkmin, lworkopt, i__, j, r__, childinfo, lorglqmin, lorgqrmin, 
	    lorglqopt, lrworkmin, lorgqropt, lrworkopt;
    quadreal dum[1];
    integer ib11d, ib11e, ib12d, ib12e, ib21d, ib21e, ib22d, ib22e;
    quadcomplex cdum[1]	/* was [1][1] */;
    integer iphi;
    ...
    extern void  wbbcsd_(char *, char *, char *, char *, char *
	    , integer *, integer *, integer *, quadreal *, quadreal *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, quadreal *, quadreal *, quadreal *, 
	    quadreal *, integer *, integer *), xerbla_(char *, integer *);
    integer iorglq, lorglq;
    extern void  wlacpy_(char *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *);
    integer iorgqr;
    extern void  wlapmr_(logical *, integer *, integer *, 
	    quadcomplex *, integer *, integer *);
    integer lorgqr;
    extern void  wlapmt_(logical *, integer *, integer *, 
	    quadcomplex *, integer *, integer *);
    logical lquery;
    extern void  wunglq_(integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, quadcomplex *, 
	    integer *, integer *), wungqr_(integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, quadcomplex *, 
	    integer *, integer *), wunbdb1_(integer *, integer *, integer *, 
	    quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadreal *, quadreal *, quadcomplex *, quadcomplex *, 
	    quadcomplex *, quadcomplex *, integer *, integer *), wunbdb2_(
	    ...
	    , integer *, integer *), wunbdb3_(integer *, integer *, integer *,
	     quadcomplex *, integer *, quadcomplex *, integer *, 
	    quadreal *, quadreal *, quadcomplex *, quadcomplex *, 
	    quadcomplex *, quadcomplex *, integer *, integer *), wunbdb4_(
	    integer *, integer *, integer *, quadcomplex *, integer *, 
	    quadcomplex *, integer *, quadreal *, quadreal *, 
	    quadcomplex *, quadcomplex *, quadcomplex *, quadcomplex *
	    , quadcomplex *, integer *, integer *);
...
/* Computing MAX */
	...
	lorglqopt = 1;
	if (r__ == *q) {
	    wunbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, 
		    &childinfo);
	    lorbdb = (integer) work[1].r;
	    if (wantu1 && *p > 0) {
		wungqr_(p, p, q, &u1[u1_offset], ldu1, cdum, &work[1], &c_n1, 
			&childinfo);
...
/* Computing MAX */
		...
		i__1 = *m - *p;
		i__2 = *m - *p;
		wungqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, cdum, &work[1],
			 &c_n1, &childinfo);
/* Computing MAX */
...
/* Computing MAX */
		...
		i__2 = *q - 1;
		i__3 = *q - 1;
		wunglq_(&i__1, &i__2, &i__3, &v1t[v1t_offset], ldv1t, cdum, &
...
/* Computing MAX */
	    ...
	    wbbcsd_(jobu1, jobu2, jobv1t, "N", "N", m, p, q, &theta[1], dum, &
	    ...
	    wunbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, 
		    &childinfo);
	    lorbdb = (integer) work[1].r;
	    if (wantu1 && *p > 0) {
		...
		i__2 = *p - 1;
		i__3 = *p - 1;
		wungqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, 
...
/* Computing MAX */
	    ...
	    if (wantv1t && *q > 0) {
		wunglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, cdum, &work[1], &
			c_n1, &childinfo);
...
/* Computing MAX */
		...
		i__2 = *m - *p - 1;
		i__3 = *m - *p - 1;
		wungqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, 
			cdum, &work[1], &c_n1, &childinfo);
/* Computing MAX */
...
/* Computing MAX */
	    ...
	    i__1 = *m - *q;
	    i__2 = *m - *p;
	    wbbcsd_("N", jobv1t, jobu2, jobu1, "T", m, &i__1, &i__2, &theta[1]
	    ...
	    wunbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], 
		    ldx21, &theta[1], dum, cdum, cdum, cdum, cdum, &work[1], &
		    c_n1, &childinfo);
	    lorbdb = *m + (integer) work[1].r;
	    if (wantu1 && *p > 0) {
		i__1 = *m - *q;
		wungqr_(p, p, &i__1, &u1[u1_offset], ldu1, cdum, &work[1], &
			c_n1, &childinfo);
		lorgqrmin = f2cmax(lorgqrmin,*p);
/* Computing MAX */
		i__1 = lorgqropt, i__2 = (integer) work[1].r;
		...
		i__2 = *m - *p;
		i__3 = *m - *q;
		wungqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, cdum, &
			work[1], &c_n1, &childinfo);
/* Computing MAX */
		i__1 = lorgqrmin, i__2 = *m - *p;
		lorgqrmin = f2cmax(i__1,i__2);
/* Computing MAX */
	    ...
	    if (wantv1t && *q > 0) {
		wunglq_(q, q, q, &v1t[v1t_offset], ldv1t, cdum, &work[1], &
			c_n1, &childinfo);
		lorglqmin = f2cmax(lorglqmin,*q);
/* Computing MAX */
		i__1 = lorglqopt, i__2 = (integer) work[1].r;
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *q;
	    wbbcsd_(jobu2, jobu1, "N", jobv1t, "N", m, &i__1, &i__2, &theta[1]
...
/* Computing MAX */
	...
	work[1].r = (quadreal) lworkopt, work[1].i = 0.;
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	wunbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    wlacpy_("L", p, q, &x11[x11_offset], ldx11, &u1[u1_offset], ldu1);
	    wungqr_(p, p, q, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorgqr], &lorgqr, &childinfo);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    wlacpy_("L", &i__1, q, &x21[x21_offset], ldx21, &u2[u2_offset], 
		    ldu2);
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    wungqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, &work[itaup2], &
	...
	if (wantv1t && *q > 0) {
	    ...
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    wlacpy_("U", &i__1, &i__2, &x21[(x21_dim1 << 1) + 1], ldx21, &v1t[
		    (v1t_dim1 << 1) + 2], ldv1t);
	    i__1 = *q - 1;
	    i__2 = *q - 1;
	    i__3 = *q - 1;
	    wunglq_(&i__1, &i__2, &i__3, &v1t[(v1t_dim1 << 1) + 2], ldv1t, &
		    work[itauq1], &work[iorglq], &lorglq, &childinfo);
	}

/*        Simultaneously diagonalize X11 and X21. */

	wbbcsd_(jobu1, jobu2, jobv1t, "N", "N", m, p, q, &theta[1], &rwork[
...
/*        preferred positions */
	...
	if (*q > 0 && wantu2) {
	    ...
	    i__1 = *m - *p;
	    i__2 = *m - *p;
	    wlapmt_(&c_false, &i__1, &i__2, &u2[u2_offset], ldu2, &iwork[1]);
	}
    } else if (r__ == *p) {
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	wunbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &rwork[iphi], &work[itaup1], &work[itaup2], &work[
		itauq1], &work[iorbdb], &lorbdb, &childinfo);
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    ...
	    i__1 = *p - 1;
	    i__2 = *p - 1;
	    wlacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &u1[(
		    u1_dim1 << 1) + 2], ldu1);
	    i__1 = *p - 1;
	    i__2 = *p - 1;
	    i__3 = *p - 1;
	    wungqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, &work[
	...
	if (wantv1t && *q > 0) {
	    wlacpy_("U", p, q, &x11[x11_offset], ldx11, &v1t[v1t_offset], 
		    ldv1t);
	    wunglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}

/*        Simultaneously diagonalize X11 and X21. */

	wbbcsd_(jobv1t, "N", jobu1, jobu2, "T", m, q, p, &theta[1], &rwork[
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	wunbdb3_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &rwork[iphi], &work[itaup1], &work[itaup2], &work[
		itauq1], &work[iorbdb], &lorbdb, &childinfo);
...
/*        Accumulate Householder reflectors */
	...
	if (wantu2 && *m - *p > 0) {
	    ...
	    i__1 = *m - *p - 1;
	    i__2 = *m - *p - 1;
	    wlacpy_("L", &i__1, &i__2, &x21[x21_dim1 + 2], ldx21, &u2[(
		    u2_dim1 << 1) + 2], ldu2);
	    i__1 = *m - *p - 1;
	    i__2 = *m - *p - 1;
	    i__3 = *m - *p - 1;
	    wungqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, &work[
		    itaup2], &work[iorgqr], &lorgqr, &childinfo);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *m - *p;
	    wlacpy_("U", &i__1, q, &x21[x21_offset], ldx21, &v1t[v1t_offset], 
		    ldv1t);
	    wunglq_(q, q, &r__, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}
...
/*        preferred positions */
	...
	if (*q > r__) {
	    ...
	    if (wantu1) {
		wlapmt_(&c_false, p, q, &u1[u1_offset], ldu1, &iwork[1]);
	    }
	    if (wantv1t) {
		wlapmr_(&c_false, q, q, &v1t[v1t_offset], ldv1t, &iwork[1]);
	    }
	}
    } else {
...
/*        Simultaneously bidiagonalize X11 and X21 */
	...
	i__1 = lorbdb - *m;
	wunbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &
		theta[1], &rwork[iphi], &work[itaup1], &work[itaup2], &work[
...
/*        Accumulate Householder reflectors */
	...
	if (wantu1 && *p > 0) {
	    wcopy_(p, &work[iorbdb], &c__1, &u1[u1_offset], &c__1);
	    i__1 = *p;
	    for (j = 2; j <= i__1; ++j) {
		i__2 = j * u1_dim1 + 1;
		u1[i__2].r = 0., u1[i__2].i = 0.;
	    ...
	    wlacpy_("L", &i__1, &i__2, &x11[x11_dim1 + 2], ldx11, &u1[(
		    u1_dim1 << 1) + 2], ldu1);
	    i__1 = *m - *q;
	    wungqr_(p, p, &i__1, &u1[u1_offset], ldu1, &work[itaup1], &work[
		    iorgqr], &lorgqr, &childinfo);
	}
	if (wantu2 && *m - *p > 0) {
	    i__1 = *m - *p;
	    wcopy_(&i__1, &work[iorbdb + *p], &c__1, &u2[u2_offset], &c__1);
	    i__1 = *m - *p;
	    for (j = 2; j <= i__1; ++j) {
		i__2 = j * u2_dim1 + 1;
		u2[i__2].r = 0., u2[i__2].i = 0.;
	    ...
	    i__2 = *m - *p;
	    i__3 = *m - *q;
	    wungqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, &work[itaup2], 
		    &work[iorgqr], &lorgqr, &childinfo);
	}
	if (wantv1t && *q > 0) {
	    i__1 = *m - *q;
	    ...
	    i__1 = *p - (*m - *q);
	    i__2 = *q - (*m - *q);
	    wlacpy_("U", &i__1, &i__2, &x11[*m - *q + 1 + (*m - *q + 1) * 
		    x11_dim1], ldx11, &v1t[*m - *q + 1 + (*m - *q + 1) * 
		    v1t_dim1], ldv1t);
	    i__1 = -(*p) + *q;
	    i__2 = *q - *p;
	    wlacpy_("U", &i__1, &i__2, &x21[*m - *q + 1 + (*p + 1) * x21_dim1]
		    , ldx21, &v1t[*p + 1 + (*p + 1) * v1t_dim1], ldv1t);
	    wunglq_(q, q, q, &v1t[v1t_offset], ldv1t, &work[itauq1], &work[
		    iorglq], &lorglq, &childinfo);
	}
...
/*        preferred positions */
	...
	if (*p > r__) {
	    ...
	    if (wantu1) {
		wlapmt_(&c_false, p, p, &u1[u1_offset], ldu1, &iwork[1]);
	    }
	    if (wantv1t) {
		wlapmr_(&c_false, p, q, &v1t[v1t_offset], ldv1t, &iwork[1]);
	    }
	}
    }
