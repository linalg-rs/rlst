...
static halfreal c_b14 = 0.;
static halfreal c_b15 = 1.;
...
/* > \verbatim */
/* > */
/* > DGGHD3 reduces a pair of doublereal matrices (A,B) to generalized upper */
...
/* > */
/*  ===================================================================== */
void  hgghd3_(char *compq, char *compz, integer *n, integer *
	ilo, integer *ihi, halfreal *a, integer *lda, halfreal *b, 
	integer *ldb, halfreal *q, integer *ldq, halfreal *z__, integer *
	ldz, halfreal *work, integer *lwork, integer *info)
{
    ...
    integer nbmin;
    extern void  horm22_(char *, char *, integer *, integer *, 
	    integer *, integer *, halfreal *, integer *, halfreal *, 
	    integer *, halfreal *, integer *, integer *);
    integer nblst;
    logical initq, wantq;
    extern void  htrmv_(char *, char *, char *, integer *, 
    ...
    extern void  hgghrd_(char *, char *, integer *, integer *, 
	    ...
	    halfreal *, integer *, halfreal *, integer *, integer *), hlaset_(char *, integer *, integer *, halfreal 
	    *, halfreal *, halfreal *, integer *), hlartg_(
...
/*     Initialize Q and Z if desired. */
    ...
    if (initq) {
	hlaset_("All", n, n, &c_b14, &c_b15, &q[q_offset], ldq);
    }
    if (initz) {
	hlaset_("All", n, n, &c_b14, &c_b15, &z__[z_offset], ldz);
    }
...
/*     Zero out lower triangle of B. */
	...
	i__1 = *n - 1;
	i__2 = *n - 1;
	hlaset_("Lower", &i__1, &i__2, &c_b14, &c_b14, &b[b_dim1 + 2], ldb);
    }
...
/*                  factor. */
	    ...
	    hlaset_("All", &nblst, &nblst, &c_b14, &c_b15, &work[1], &nblst);
	    pw = nblst * nblst + 1;
	    i__3 = n2nb;
		...
		i__5 = nnb << 1;
		i__6 = nnb << 1;
		hlaset_("All", &i__4, &i__5, &c_b14, &c_b15, &work[pw], &i__6);
		pw += (nnb << 2) * nnb;
	    }
...
/*              column of A and B, respectively. */
		...
		for (i__ = *ihi; i__ >= i__4; --i__) {
		    ...
		    hlartg_(&temp, &a[i__ + j * a_dim1], &c__, &s, &a[i__ - 1 
			    + j * a_dim1]);
		    a[i__ + j * a_dim1] = c__;
		    b[i__ + j * b_dim1] = s;
		}
...
/*                 Annihilate B( JJ+1, JJ ). */
			...
			hlartg_(&temp, &b[jj + 1 + jj * b_dim1], &c__, &s, &b[
				jj + 1 + (jj + 1) * b_dim1]);
			b[jj + 1 + jj * b_dim1] = 0.;
			i__6 = jj - top;
			hrot_(&i__6, &b[top + 1 + (jj + 1) * b_dim1], &c__1, &
...
/*     $                     A( TOP+1, J+1 ), LDA ) */
		...
		if (jj > 0) {
			...
			hrot_(&i__5, &a[top + 1 + (j + i__ + 1) * a_dim1], &
...
/*                 triangular. */
		    ...
		    jrow = *ihi - nblst + 1;
		    hgemv_("Transpose", &nblst, &len, &c_b15, &work[1], &
		    ...
		    i__5 = nblst - len;
		    htrmv_("Lower", "Transpose", "Non-unit", &i__5, &work[len 
			    * nblst + 1], &nblst, &work[pw + len], &c__1);
		    i__5 = nblst - len;
		    hgemv_("Transpose", &len, &i__5, &c_b15, &work[(len + 1) *
...
/*                 and U12 is an NNB-by-NNB lower triangular matrix. */
			...
			i__4 = nnb << 1;
			htrmv_("Upper", "Transpose", "Non-unit", &len, &work[
				ppwo + nnb], &i__4, &work[pw], &c__1);
			i__4 = nnb << 1;
			htrmv_("Lower", "Transpose", "Non-unit", &nnb, &work[
				ppwo + (len << 1) * nnb], &i__4, &work[pw + 
				len], &c__1);
			i__4 = nnb << 1;
			hgemv_("Transpose", &nnb, &len, &c_b15, &work[ppwo], &
				i__4, &a[jrow + (j + 1) * a_dim1], &c__1, &
				c_b15, &work[pw], &c__1);
			i__4 = nnb << 1;
			hgemv_("Transpose", &len, &nnb, &c_b15, &work[ppwo + (
...
/*           Apply accumulated orthogonal matrices to A. */
	    ...
	    cola = *n - jcol - nnb + 1;
	    j = *ihi - nblst + 1;
	    hgemm_("Transpose", "No Transpose", &nblst, &cola, &nblst, &c_b15,
		     &work[1], &nblst, &a[j + (jcol + nnb) * a_dim1], lda, &
		    c_b14, &work[pw], &nblst);
	    hlacpy_("All", &nblst, &cola, &work[pw], &nblst, &a[j + (jcol + 
		    nnb) * a_dim1], lda);
	    ppwo = nblst * nblst + 1;
	    j0 = j - nnb;
	    i__3 = jcol + 1;
...
/*                 triangular and U12 is lower triangular. */
		    ...
		    i__4 = nnb << 1;
		    i__7 = *lwork - pw + 1;
		    horm22_("Left", "Transpose", &i__5, &cola, &nnb, &nnb, &
...
/*                 Ignore the structure of U. */
		    ...
		    i__7 = nnb << 1;
		    i__8 = nnb << 1;
		    hgemm_("Transpose", "No Transpose", &i__5, &cola, &i__4, &
			    c_b15, &work[ppwo], &i__7, &a[j + (jcol + nnb) * 
			    a_dim1], lda, &c_b14, &work[pw], &i__8);
		    i__5 = nnb << 1;
		    i__4 = nnb << 1;
		    hlacpy_("All", &i__5, &cola, &work[pw], &i__4, &a[j + (
			    jcol + nnb) * a_dim1], lda);
		}
		ppwo += (nnb << 2) * nnb;
	    }
...
/* Computing MAX */
		...
		hgemm_("No Transpose", "No Transpose", &nh, &nblst, &nblst, &
			c_b15, &q[topq + j * q_dim1], ldq, &work[1], &nblst, &
			c_b14, &work[pw], &nh);
		hlacpy_("All", &nh, &nblst, &work[pw], &nh, &q[topq + j * 
			q_dim1], ldq);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__6 = jcol + 1;
...
/*                    Exploit the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = *lwork - pw + 1;
			horm22_("Right", "No Transpose", &nh, &i__5, &nnb, &
...
/*                    Ignore the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = nnb << 1;
			hgemm_("No Transpose", "No Transpose", &nh, &i__5, &
				i__4, &c_b15, &q[topq + j * q_dim1], ldq, &
				work[ppwo], &i__7, &c_b14, &work[pw], &nh);
			i__5 = nnb << 1;
			hlacpy_("All", &nh, &i__5, &work[pw], &nh, &q[topq + 
				j * q_dim1], ldq);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
...
/*              accumulated Givens rotations in workspace. */
		...
		hlaset_("All", &nblst, &nblst, &c_b14, &c_b15, &work[1], &
			nblst);
		pw = nblst * nblst + 1;
		i__3 = n2nb;
		for (i__ = 1; i__ <= i__3; ++i__) {
		    ...
		    i__5 = nnb << 1;
		    i__4 = nnb << 1;
		    hlaset_("All", &i__6, &i__5, &c_b14, &c_b15, &work[pw], &
			    i__4);
		    pw += (nnb << 2) * nnb;
		}
...
/*              Accumulate Givens rotations into workspace array. */
		...
		i__3 = *ihi - jcol - 1;
		hlaset_("Lower", &i__3, &nnb, &c_b14, &c_b14, &a[jcol + 2 + 
			jcol * a_dim1], lda);
		i__3 = *ihi - jcol - 1;
		hlaset_("Lower", &i__3, &nnb, &c_b14, &c_b14, &b[jcol + 2 + 
			jcol * b_dim1], ldb);
	    }
...
/*           Apply accumulated orthogonal matrices to A and B. */
	    ...
	    if (top > 0) {
		j = *ihi - nblst + 1;
		hgemm_("No Transpose", "No Transpose", &top, &nblst, &nblst, &
			c_b15, &a[j * a_dim1 + 1], lda, &work[1], &nblst, &
			c_b14, &work[pw], &top);
		hlacpy_("All", &top, &nblst, &work[pw], &top, &a[j * a_dim1 + 
			1], lda);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__3 = jcol + 1;
...
/*                    Exploit the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = *lwork - pw + 1;
			horm22_("Right", "No Transpose", &top, &i__6, &nnb, &
...
/*                    Ignore the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = nnb << 1;
			hgemm_("No Transpose", "No Transpose", &top, &i__6, &
				i__4, &c_b15, &a[j * a_dim1 + 1], lda, &work[
				ppwo], &i__7, &c_b14, &work[pw], &top);
			i__6 = nnb << 1;
			hlacpy_("All", &top, &i__6, &work[pw], &top, &a[j * 
				a_dim1 + 1], lda);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
		...
		hgemm_("No Transpose", "No Transpose", &top, &nblst, &nblst, &
			...
			c_b14, &work[pw], &top);
		hlacpy_("All", &top, &nblst, &work[pw], &top, &b[j * b_dim1 + 
			1], ldb);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__5 = jcol + 1;
...
/*                    Ignore the structure of U. */
			...
			hgemm_("No Transpose", "No Transpose", &top, &i__6, &
				...
				ppwo], &i__7, &c_b14, &work[pw], &top);
			i__6 = nnb << 1;
			hlacpy_("All", &top, &i__6, &work[pw], &top, &b[j * 
				b_dim1 + 1], ldb);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
...
/* Computing MAX */
		...
		hgemm_("No Transpose", "No Transpose", &nh, &nblst, &nblst, &
			c_b15, &z__[topq + j * z_dim1], ldz, &work[1], &nblst,
			 &c_b14, &work[pw], &nh);
		hlacpy_("All", &nh, &nblst, &work[pw], &nh, &z__[topq + j * 
			z_dim1], ldz);
		ppwo = nblst * nblst + 1;
		j0 = j - nnb;
		i__3 = jcol + 1;
...
/*                    Exploit the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = *lwork - pw + 1;
			horm22_("Right", "No Transpose", &nh, &i__6, &nnb, &
...
/*                    Ignore the structure of U. */
			...
			i__4 = nnb << 1;
			i__7 = nnb << 1;
			hgemm_("No Transpose", "No Transpose", &nh, &i__6, &
				i__4, &c_b15, &z__[topq + j * z_dim1], ldz, &
				work[ppwo], &i__7, &c_b14, &work[pw], &nh);
			i__6 = nnb << 1;
			hlacpy_("All", &nh, &i__6, &work[pw], &nh, &z__[topq 
				+ j * z_dim1], ldz);
		    }
		    ppwo += (nnb << 2) * nnb;
		}
...
/*     Avoid re-initialization of modified Q and Z. */
    ...
    *(unsigned char *)compz2 = *(unsigned char *)compz;
    ...
    if (jcol < *ihi) {
	hgghrd_(compq2, compz2, n, &jcol, ihi, &a[a_offset], lda, &b[b_offset]
