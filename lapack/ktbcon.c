...
/*  ===================================================================== */
void  ktbcon_(char *norm, char *uplo, char *diag, integer *n, 
	integer *kd, halfcomplex *ab, integer *ldab, halfreal *rcond, 
...
/*     Quick return if possible */
    ...
    *rcond = 0.;
    smlnum = hlamch_("Safe minimum") * (halfreal) f2cmax(*n,1);

/*     Compute the 1-norm of the triangular matrix A or A**H. */

    anorm = klantb_(norm, uplo, diag, n, kd, &ab[ab_offset], ldab, &rwork[1]);
...
/*              Multiply by inv(A). */
		...
		klatbs_(uplo, "No transpose", diag, normin, n, kd, &ab[
			ab_offset], ldab, &work[1], &scale, &rwork[1], info);
	    } else {

/*              Multiply by inv(A**H). */

		klatbs_(uplo, "Conjugate transpose", diag, normin, n, kd, &ab[
			ab_offset], ldab, &work[1], &scale, &rwork[1], info);
	    }
