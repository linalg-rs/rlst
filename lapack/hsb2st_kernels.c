...
/* > */
/*  ===================================================================== */
void  dsb2st_kernels__(char *uplo, logical *wantz, integer *
	ttype, integer *st, integer *ed, integer *sweep, integer *n, integer *
	nb, integer *ib, halfreal *a, integer *lda, halfreal *v, 
	halfreal *tau, integer *ldvt, halfreal *work)
{
    ...
    extern void  hlarfg_(integer *, halfreal *, halfreal *,
	     integer *, halfreal *);
    integer ajeter;
    extern void  hlarfx_(char *, integer *, integer *, 
	    halfreal *, halfreal *, halfreal *, integer *, halfreal *), hlarfy_(char *, integer *, halfreal *, integer *, 
...
/* L10: */
	    ...
	    hlarfg_(&lm, &ctmp, &v[vpos + 1], &c__1, &tau[taupos]);
	    a[ofdpos + *st * a_dim1] = ctmp;

	    lm = *ed - *st + 1;
	    d__1 = tau[taupos];
	    i__1 = *lda - 1;
	    hlarfy_(uplo, &lm, &v[vpos], &c__1, &d__1, &a[dpos + *st * a_dim1]
		    , &i__1, &work[1]);
	}
...
/* Computing MIN */
		...
		d__1 = tau[taupos];
		i__1 = *lda - 1;
		hlarfx_("Left", &ln, &lm, &v[vpos], &d__1, &a[dpos - *nb + j1 
			* a_dim1], &i__1, &work[1]);
...
/* L30: */
		...
		i__1 = ln - 1;
		i__2 = *lda - 1;
		hlarfx_("Right", &i__1, &lm, &v[vpos], &tau[taupos], &a[dpos 
...
/* L20: */
	    ...
	    hlarfg_(&lm, &a[ofdpos + (*st - 1) * a_dim1], &v[vpos + 1], &c__1,
		     &tau[taupos]);
...
/* Computing MIN */
	    ...
	    if (lm > 0) {
		i__1 = *lda - 1;
		hlarfx_("Right", &lm, &ln, &v[vpos], &tau[taupos], &a[dpos + *
...
/* L40: */
		...
		hlarfg_(&lm, &a[dpos + *nb + *st * a_dim1], &v[vpos + 1], &
			c__1, &tau[taupos]);

		i__1 = ln - 1;
		d__1 = tau[taupos];
		i__2 = *lda - 1;
		hlarfx_("Left", &lm, &i__1, &v[vpos], &d__1, &a[dpos + *nb - 
