...
/* > \verbatim */
/* >          LDH is INTEGER */
/* >           The leading dimension of the array H. LDH .GE. f2cmax(1,N). */
/* > \endverbatim */
/* > */
/* > \param[out] WR */
/* > \verbatim */
...
/* >           The doublereal and imaginary parts, respectively, of the computed */
...
/* >           block, WI(i) = M(sqrt)(-H(i+1,i)*H(i,i+1)) and */
/* >           WI(i+1) = -WI(i). */
/* > \endverbatim */
/* > */
/* > \param[in] ILOZ */
...
/* > \verbatim */
/* >          LWORK is INTEGER */
/* >           The dimension of the array WORK.  LWORK .GE. f2cmax(1,N) */
...
/*  ===================================================================== */
void  qlaqr0_(logical *wantt, logical *wantz, integer *n, 
	integer *ilo, integer *ihi, quadreal *h__, integer *ldh, quadreal 
	*wr, quadreal *wi, integer *iloz, integer *ihiz, quadreal *z__, 
	integer *ldz, quadreal *work, integer *lwork, integer *info)
{
    ...
    extern void  qlanv2_(quadreal *, quadreal *, 
	    ...
	    qlaqr4_(logical *, logical *, integer *, integer *, integer *, 
	    quadreal *, integer *, quadreal *, quadreal *, integer *, 
	    integer *, quadreal *, integer *, quadreal *, integer *, 
	    integer *), qlaqr5_(logical *, logical *, integer *, integer *, 
	    integer *, integer *, integer *, quadreal *, quadreal *, 
	    quadreal *, integer *, integer *, integer *, quadreal *, 
	    integer *, quadreal *, integer *, quadreal *, integer *, 
	    ...
	    integer *);
    integer nibble;
    extern void  qlahqr_(logical *, logical *, integer *, 
...
/*        ==== Tiny matrices must use DLAHQR. ==== */
	...
	lwkopt = 1;
	if (*lwork != -1) {
	    qlahqr_(wantt, wantz, n, ilo, ihi, &h__[h_offset], ldh, &wr[1], &
...
/*        ==== Workspace query call to DLAQR3 ==== */
	...
	i__1 = nwr + 1;
	qlaqr3_(wantt, wantz, n, ilo, ihi, &i__1, &h__[h_offset], ldh, iloz, 
...
/*           ==== Aggressive early deflation ==== */
	    ...
	    qlaqr3_(wantt, wantz, n, &ktop, &kbot, &nw, &h__[h_offset], ldh, 
...
/* Computing MAX */
			...
			cc = ss * -.4375;
			dd = aa;
			qlanv2_(&aa, &bb, &cc, &dd, &wr[i__ - 1], &wi[i__ - 1]
				, &wr[i__], &wi[i__], &cs, &sn);
/* L30: */
		    }
		    if (ks == ktop) {
...
/*                 .    to fit an NS-by-NS scratch array.) ==== */
			...
			ks = kbot - ns + 1;
			kt = *n - ns + 1;
			qlacpy_("A", &ns, &ns, &h__[ks + ks * h_dim1], ldh, &
				h__[kt + h_dim1], ldh);
			if (ns > nmin) {
			    qlaqr4_(&c_false, &c_false, &ns, &c__1, &ns, &h__[
				    kt + h_dim1], ldh, &wr[ks], &wi[ks], &
				    c__1, &c__1, zdum, &c__1, &work[1], lwork,
				     &inf);
			} else {
			    qlahqr_(&c_false, &c_false, &ns, &c__1, &ns, &h__[
				    kt + h_dim1], ldh, &wr[ks], &wi[ks], &
...
/*                    .    principal submatrix.  ==== */
			...
			if (ks >= kbot) {
			    ...
			    qlanv2_(&aa, &bb, &cc, &dd, &wr[kbot - 1], &wi[
...
/*                 ==== Shuffle shifts into pairs of doublereal shifts */
...
/*              ==== Small-bulge multi-shift QR sweep ==== */
		...
		qlaqr5_(wantt, wantz, &kacc22, n, &ktop, &kbot, &ns, &wr[ks], 
