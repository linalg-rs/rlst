...
/*  ===================================================================== */
void  kstedc_(char *compz, integer *n, halfreal *d__, 
	halfreal *e, halfcomplex *z__, integer *ldz, halfcomplex *work, 
	integer *lwork, halfreal *rwork, integer *lrwork, integer *iwork, 
	integer *liwork, integer *info)
{
    ...
    extern void  hsterf_(integer *, halfreal *, halfreal *,
	     integer *), klacrm_(integer *, integer *, halfcomplex *, 
	    integer *, halfreal *, integer *, halfcomplex *, integer *, 
	    halfreal *);
    integer liwmin, icompz;
    extern void  hsteqr_(char *, integer *, halfreal *, 
	    halfreal *, halfreal *, integer *, halfreal *, integer *), klacpy_(char *, integer *, integer *, halfcomplex *, 
...
/*        Compute the workspace requirements */
	...
	if (*n <= 1 || icompz == 0) {
	    ...
	    lgn = (integer) (M(log)((halfreal) (*n)) / M(log)(2.));
...
/*     solve the problem with another solver. */
	...
	ksteqr_(compz, n, &d__[1], &e[1], &z__[z_offset], ldz, &rwork[1], 
		info);
...
/*        If COMPZ = 'I', we simply call DSTEDC instead. */
	...
	if (icompz == 2) {
	    hlaset_("Full", n, n, &c_b17, &c_b18, &rwork[1], n);
	    ll = *n * *n + 1;
	    i__1 = *lrwork - ll + 1;
	    hstedc_("I", n, &d__[1], &e[1], &rwork[1], n, &rwork[ll], &i__1, &
		    iwork[1], liwork, info);
	    i__1 = *n;
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n;
...
/*              Scale. */
		...
		orgnrm = hlanst_("M", &m, &d__[start], &e[start]);
		hlascl_("G", &c__0, &c__0, &orgnrm, &c_b18, &m, &c__1, &d__[
			start], &m, info);
		i__1 = m - 1;
		i__2 = m - 1;
		hlascl_("G", &c__0, &c__0, &orgnrm, &c_b18, &i__1, &c__1, &e[
			start], &i__2, info);

		klaed0_(n, &m, &d__[start], &e[start], &z__[start * z_dim1 + 
...
/*              Scale back. */
		...
		hlascl_("G", &c__0, &c__0, &c_b18, &orgnrm, &m, &c__1, &d__[
			start], &m, info);

	    } else {
		hsteqr_("I", &m, &d__[start], &e[start], &rwork[1], &m, &
			rwork[m * m + 1], info);
		klacrm_(n, &m, &z__[start * z_dim1 + 1], ldz, &rwork[1], &m, &
			work[1], n, &rwork[m * m + 1]);
		klacpy_("A", n, &m, &work[1], n, &z__[start * z_dim1 + 1], 
			ldz);
		if (*info > 0) {
