...
/* > */
/*  ===================================================================== */
void  khbtrd_(char *vect, char *uplo, integer *n, integer *kd, 
	halfcomplex *ab, integer *ldab, halfreal *d__, halfreal *e, 
	halfcomplex *q, integer *ldq, halfcomplex *work, integer *info)
{
    ...
    extern void  xerbla_(char *, integer *), klacgv_(
	    integer *, halfcomplex *, integer *), klaset_(char *, integer *,
	     integer *, halfcomplex *, halfcomplex *, halfcomplex *, 
	    integer *), klartg_(halfcomplex *, halfcomplex *, 
	    halfreal *, halfcomplex *, halfcomplex *), klargv_(integer *
	    , halfcomplex *, integer *, halfcomplex *, integer *, 
	    halfreal *, integer *), klartv_(integer *, halfcomplex *, 
	    integer *, halfcomplex *, integer *, halfreal *, 
	    halfcomplex *, integer *);
...
/*                    elements which have been created outside the band */
			...
			klargv_(&nr, &ab[(j1 - 1) * ab_dim1 + 1], &inca, &
...
/*                    ZLARTV or ZROT is used */
			...
			if (nr >= (*kd << 1) - 1) {
			    i__2 = *kd - 1;
			    for (l = 1; l <= i__2; ++l) {
				klartv_(&nr, &ab[l + 1 + (j1 - 1) * ab_dim1], 
...
/* L10: */
			    ...
			    for (jinc = j1; i__3 < 0 ? jinc >= i__2 : jinc <= 
				    i__2; jinc += i__3) {
				krot_(&kdm1, &ab[(jinc - 1) * ab_dim1 + 2], &
...
/*                       within the band */
			    ...
			    klartg_(&ab[*kd - k + 3 + (i__ + k - 2) * ab_dim1]
				    , &ab[*kd - k + 2 + (i__ + k - 1) * 
...
/*                 blocks */
		    ...
		    if (nr > 0) {
			klar2v_(&nr, &ab[kd1 + (j1 - 1) * ab_dim1], &ab[kd1 + 
...
/* Computing MIN */
				...
				d_cnjg(&z__1, &work[j]);
				krot_(&nq, &q[iqb + (j - 1) * q_dim1], &c__1, 
...
/*           make off-diagonal elements doublereal and copy them to E */
		...
		if (wantq) {
		    d_cnjg(&z__1, &t);
		    kscal_(n, &z__1, &q[(i__ + 1) * q_dim1 + 1], &c__1);
		}
/* L100: */
	    }
	} else {
...
/*                    elements which have been created outside the band */
			...
			klargv_(&nr, &ab[kd1 + (j1 - kd1) * ab_dim1], &inca, &
				work[j1], &kd1, &d__[j1], &kd1);
...
/* L130: */
			    ...
			    for (jinc = j1; i__2 < 0 ? jinc >= i__3 : jinc <= 
				    i__3; jinc += i__2) {
				krot_(&kdm1, &ab[*kd + (jinc - *kd) * ab_dim1]
					, &incx, &ab[kd1 + (jinc - *kd) * 
...
/*                       within the band */
			    ...
			    klartg_(&ab[k - 1 + i__ * ab_dim1], &ab[k + i__ * 
				    ab_dim1], &d__[i__ + k - 1], &work[i__ + 
				    k - 1], &temp);
...
/*                       apply rotation from the left */
			    ...
			    i__3 = *ldab - 1;
			    i__4 = *ldab - 1;
			    krot_(&i__2, &ab[k - 2 + (i__ + 1) * ab_dim1], &
...
/*                 blocks */
		    ...
		    if (nr > 0) {
			klar2v_(&nr, &ab[(j1 - 1) * ab_dim1 + 1], &ab[j1 * 
...
/* Computing MIN */
			    ...
			    last = j1end + kd1;
			    if (lend > 0) {
				krot_(&lend, &ab[(last - 1) * ab_dim1 + 3], &
...
/*           make off-diagonal elements doublereal and copy them to E */
		...
		if (wantq) {
		    kscal_(n, &t, &q[(i__ + 1) * q_dim1 + 1], &c__1);
		}
/* L220: */
	    }
	} else {
