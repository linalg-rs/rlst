...
/*  ===================================================================== */
void  hgbbrd_(char *vect, integer *m, integer *n, integer *ncc,
	 integer *kl, integer *ku, halfreal *ab, integer *ldab, halfreal *
	d__, halfreal *e, halfreal *q, integer *ldq, halfreal *pt, 
	integer *ldpt, halfreal *c__, integer *ldc, halfreal *work, 
	integer *info)
{
...
/*     Initialize Q and P**T to the unit matrix, if needed */
    ...
    if (wantq) {
	hlaset_("Full", m, m, &c_b8, &c_b9, &q[q_offset], ldq);
    }
    if (wantpt) {
	hlaset_("Full", n, n, &c_b8, &c_b9, &pt[pt_offset], ldpt);
    }
...
/*              which have been created below the band */
		...
		if (nr > 0) {
		    hlargv_(&nr, &ab[klu1 + (j1 - klm - 1) * ab_dim1], &inca, 
...
/*              apply plane rotations from the left */
		...
		for (l = 1; l <= i__3; ++l) {
		    ...
		    if (nrt > 0) {
			hlartv_(&nrt, &ab[klu1 - l + (j1 - klm + l - 1) * 
...
/*                    within the band, and apply rotation from the left */
			...
			hlartg_(&ab[*ku + ml - 1 + i__ * ab_dim1], &ab[*ku + 
...
/*                 accumulate product of plane rotations in Q */
		    ...
		    for (j = j1; i__4 < 0 ? j >= i__3 : j <= i__3; j += i__4) 
			    {
			hrot_(m, &q[(j - 1) * q_dim1 + 1], &c__1, &q[j * 
...
/*                 apply plane rotations to C */
		    ...
		    for (j = j1; i__3 < 0 ? j >= i__4 : j <= i__4; j += i__3) 
			    {
			hrot_(ncc, &c__[j - 1 + c_dim1], ldc, &c__[j + c_dim1]
...
/*              which have been generated above the band */
		...
		if (nr > 0) {
		    hlargv_(&nr, &ab[(j1 + kun - 1) * ab_dim1 + 1], &inca, &
...
/*              apply plane rotations from the right */
		...
		for (l = 1; l <= i__4; ++l) {
		    ...
		    if (nrt > 0) {
			hlartv_(&nrt, &ab[l + 1 + (j1 + kun - 1) * ab_dim1], &
...
/*                    within the band, and apply rotation from the right */
			...
			hlartg_(&ab[*ku - mu + 3 + (i__ + mu - 2) * ab_dim1], 
...
/* Computing MIN */
			...
			hrot_(&i__4, &ab[*ku - mu + 4 + (i__ + mu - 2) * 
...
/*                 accumulate product of plane rotations in P**T */
		    ...
		    for (j = j1; i__3 < 0 ? j >= i__4 : j <= i__4; j += i__3) 
			    {
			hrot_(n, &pt[j + kun - 1 + pt_dim1], ldpt, &pt[j + 
...
/* Computing MIN */
	...
	i__1 = f2cmin(i__2,*n);
	for (i__ = 1; i__ <= i__1; ++i__) {
	    hlartg_(&ab[i__ * ab_dim1 + 1], &ab[i__ * ab_dim1 + 2], &rc, &rs, 
		    &ra);
	    d__[i__] = ra;
	    if (i__ < *n) {
	    ...
	    if (wantq) {
		hrot_(m, &q[i__ * q_dim1 + 1], &c__1, &q[(i__ + 1) * q_dim1 + 
			1], &c__1, &rc, &rs);
	    }
	    if (wantc) {
		hrot_(ncc, &c__[i__ + c_dim1], ldc, &c__[i__ + 1 + c_dim1], 
			ldc, &rc, &rs);
	    }
/* L100: */
	}
...
/*           elements in E */
	    ...
	    for (i__ = *m; i__ >= 1; --i__) {
		hlartg_(&ab[*ku + 1 + i__ * ab_dim1], &rb, &rc, &rs, &ra);
		d__[i__] = ra;
		if (i__ > 1) {
		...
		if (wantpt) {
		    hrot_(n, &pt[i__ + pt_dim1], ldpt, &pt[*m + 1 + pt_dim1], 
			    ldpt, &rc, &rs);
		}
/* L110: */
	    }
