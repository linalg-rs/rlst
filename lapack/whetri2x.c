...
/*  ===================================================================== */
void  whetri2x_(char *uplo, integer *n, quadcomplex *a, 
	...
	integer *info)
{
    ...
    /* Local variables */
    extern void  wheswapr_(char *, integer *, quadcomplex *, 
...
/*       U11**H*invD1*U11->U11 */
	    ...
	    i__1 = *n + *nb + 1;
	    wtrmm_("L", "U", "C", "U", &nnb, &nnb, &c_b1, &a[cut + 1 + (cut + 
...
/*          U01**H*invD*U01->A(CUT+I,CUT+J) */
	    ...
	    i__1 = *n + *nb + 1;
	    i__2 = *n + *nb + 1;
	    wgemm_("C", "N", &nnb, &nnb, &cut, &c_b1, &a[(cut + 1) * a_dim1 + 
...
/*        U01 =  U00**H*invD0*U01 */
	    ...
	    i__1 = *n + *nb + 1;
	    wtrmm_("L", uplo, "C", "U", &cut, &nnb, &c_b1, &a[a_offset], lda, 
...
/*        Apply PERMUTATIONS P and P**H: P * inv(U**H)*inv(D)*inv(U) *P**H */
	...
	while(i__ <= *n) {
		...
		ip = ipiv[i__];
		if (i__ < ip) {
		    wheswapr_(uplo, n, &a[a_offset], lda, &i__, &ip);
		}
		if (i__ > ip) {
		    wheswapr_(uplo, n, &a[a_offset], lda, &ip, &i__);
		}
	    } else {
		ip = -ipiv[i__];
		++i__;
		if (i__ - 1 < ip) {
		    i__1 = i__ - 1;
		    wheswapr_(uplo, n, &a[a_offset], lda, &i__1, &ip);
		}
		if (i__ - 1 > ip) {
		    i__1 = i__ - 1;
		    wheswapr_(uplo, n, &a[a_offset], lda, &ip, &i__1);
		}
	    }
	    ++i__;
	}
...
/*       L11**H*invD1*L11->L11 */
	    ...
	    i__1 = *n + *nb + 1;
	    wtrmm_("L", uplo, "C", "U", &nnb, &nnb, &c_b1, &a[cut + 1 + (cut 
		    + 1) * a_dim1], lda, &work[u11 + 1 + work_dim1], &i__1);
...
/*          L21**H*invD2*L21->A(CUT+I,CUT+J) */
		...
		i__2 = *n + *nb + 1;
		i__3 = *n + *nb + 1;
		wgemm_("C", "N", &nnb, &nnb, &i__1, &c_b1, &a[cut + nnb + 1 + 
...
/*        L01 =  L22**H*invD2*L21 */
		...
		i__1 = *n - nnb - cut;
		i__2 = *n + *nb + 1;
		wtrmm_("L", uplo, "C", "U", &i__1, &nnb, &c_b1, &a[cut + nnb 
