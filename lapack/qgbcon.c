...
/* > \verbatim */
/* > */
/* > DGBCON estimates the reciprocal of the condition number of a doublereal */
...
/*  ===================================================================== */
void  qgbcon_(char *norm, integer *n, integer *kl, integer *ku,
	 quadreal *ab, integer *ldab, integer *ipiv, quadreal *anorm, 
	quadreal *rcond, quadreal *work, integer *iwork, integer *info)
{
...
/* Computing MIN */
		    ...
		    d__1 = -t;
		    qaxpy_(&lm, &d__1, &ab[kd + 1 + j * ab_dim1], &c__1, &
			    work[j + 1], &c__1);
/* L20: */
		}
	    }
...
/*           Multiply by inv(U**T). */
	    ...
	    i__1 = *kl + *ku;
	    qlatbs_("Upper", "Transpose", "Non-unit", normin, n, &i__1, &ab[
...
/* Computing MIN */
		    i__1 = *kl, i__2 = *n - j;
		    lm = f2cmin(i__1,i__2);
		    work[j] -= qdot_(&lm, &ab[kd + 1 + j * ab_dim1], &c__1, &
			    work[j + 1], &c__1);
		    jp = ipiv[j];
		    if (jp != j) {
			t = work[jp];
